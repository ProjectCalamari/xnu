...
/*
 * Unit tests that wants to use fibers must redefine this global with a value
 * not 0. The test executable should not do this directly, instead it should
 * call macro UT_USE_FIBERS in its global scope.
 *
 * We use a weak global and not a macro that defines a constructor to avoid
 * initialization code running before such constructor to run with
 * ut_mocks_use_fibers=0 before that the constructor change its value. Switching
 * from the pthread mocks to fibers is not supported, we must be consistent from
 * the very beginning.
 */
int ut_mocks_use_fibers __attribute__((weak)) = 0;

/*
 * Unit tests that wants to use fibers with data race checking must redefine
 * this global with a value not 0. FIBERS_CHECKER=1 as env var will do the same
 * job too.
 */
int ut_fibers_use_data_race_checker __attribute__((weak)) = 0;

/*
 * Unit tests can set this variable to force `lck_rw_lock_shared_to_exclusive`
 * to fail.
 *
...
/*
 * This constructor is used to set the configuration variables of the fibers
 * using env vars. The main use case is fuzzing, unit tests should set the
 * variables in the test function or by calling the correspondig macros
 * (UT_FIBERS_*, see mock_thread.h) in their global scope.
 */
__attribute__((constructor)) static void initialize_fiber_settings(void) {
  ...
  if (checker_env != NULL) {
    ...
    raw_printf("You cannot enable the data race checker if the "
               "FIBERS_PREEMPTION=1 flag was to not used as make parameter.");
    return;
#else
    if (!ut_mocks_use_fibers) {
      raw_printf("==== Fibers data race checker disabled ====\n");
      raw_printf("You cannot enable the data race checker if the test is not "
                 "using fibers (see UT_USE_FIBERS in the readme).");
      return;
    }
...
extern void thread_ro_create(task_t parent_task, thread_t th,
                             thread_ro_t tro_tpl);
...
struct pthread_mock_event_table_entry {
  event_t ev;
  pthread_cond_t cond;
  // the condition variable is owned by the table and is initialized on the
...
struct mock_process_state {
  ...
  pthread_mutex_t
      interrupts_mutex; // if this mutex is locked interrupts are disabled
...
static void mock_destroy_thread(void *th_p) {
...
static struct mock_thread *mock_init_new_thread(struct mock_process_state *s) {
  ...
  // for the main thread this happens before zalloc init so don't do the
  // following which uses zalloc
...
void fake_init_task(task_t new_task) {
...
static void mock_init_threads_state(struct mock_process_state *s) {
  // task_zone_init();
...
struct mock_thread *get_mock_thread(void) {
...
T_MOCK(thread_t, current_thread_fast, (void)) { return &get_mock_thread()->th; }

T_MOCK(uint32_t, kauth_cred_getuid, (void *cred)) { return 0; }

// --------------- interrupts disable (spl) ---------------------

T_MOCK(boolean_t, ml_get_interrupts_enabled, (void)) {
  if (ut_mocks_use_fibers) {
    ...
    PT_QUIET;
    PT_ASSERT_TRUE(r == EBUSY, "unexpected value in get_interrupts_enabled");
    return true;
  }
}

// original calls DAIF
// interupts disable is mocked by disabling context switches with
// fiber_t.may_yield_disabled
T_MOCK(boolean_t, ml_set_interrupts_enabled, (boolean_t enable)) {
  if (ut_mocks_use_fibers) {
    bool prev_interrupts_disabled = get_mock_thread()->interrupts_disabled;

    FIBERS_LOG(FIBERS_LOG_DEBUG,
               "ml_set_interrupts_enabled: enable=%d, previous state=%d, "
               "may_yield_disabled=%d",
               enable, !get_mock_thread()->interrupts_disabled,
               fibers_current->may_yield_disabled);

    fibers_may_yield_internal_with_reason(
        (enable ? FIBERS_YIELD_REASON_PREEMPTION_WILL_ENABLE
    ...
    FIBERS_LOG(FIBERS_LOG_DEBUG,
               "ml_set_interrupts_enabled exit: enable=%d, state=%d, "
               "may_yield_disabled=%d",
               enable, !get_mock_thread()->interrupts_disabled,
               fibers_current->may_yield_disabled);

    fibers_may_yield_internal_with_reason(
        (enable ? FIBERS_YIELD_REASON_PREEMPTION_DID_ENABLE
    ...
    if (enable) {
      ...
      PT_QUIET;
      PT_ASSERT_POSIX_ZERO(ret, "interrupts pthread_mutex_unlock");
    } else {
      // disable interrupts locks
      int ret = pthread_mutex_lock(m);
      PT_QUIET;
      PT_ASSERT_POSIX_ZERO(ret, "interrupts pthread_mutex_lock");
    }
  }
  return true;
}

T_MOCK(boolean_t, ml_set_interrupts_enabled_with_debug,
       (boolean_t enable, boolean_t __unused debug)) {
  return MOCK_ml_set_interrupts_enabled(enable);
}

T_MOCK(void, _disable_preemption, (void)) {
  if (ut_mocks_use_fibers) {
    fibers_may_yield_internal_with_reason(
        FIBERS_YIELD_REASON_PREEMPTION_WILL_DISABLE |
    ...
    FIBERS_LOG(FIBERS_LOG_DEBUG, "disable_preemption: may_yield_disabled=%d",
               fibers_current->may_yield_disabled);
    ...
    int ret = pthread_mutex_lock(m);
    PT_QUIET;
    PT_ASSERT_POSIX_ZERO(ret, "_disable_preemption pthread_mutex_lock");
...
T_MOCK(void, _disable_preemption_without_measurements, (void)) {
  MOCK__disable_preemption();
}

T_MOCK(void, lock_disable_preemption_for_thread, (thread_t t)) {
  MOCK__disable_preemption();
}

T_MOCK(void, _enable_preemption, (void)) {
  if (ut_mocks_use_fibers) {
    fibers_may_yield_internal_with_reason(
    ...
    FIBERS_LOG(FIBERS_LOG_DEBUG, "enable_preemption: may_yield_disabled=%d",
               fibers_current->may_yield_disabled);
    ...
    int ret = pthread_mutex_unlock(m);
    PT_QUIET;
    PT_ASSERT_POSIX_ZERO(ret, "_enable_preemption pthread_mutex_unlock");
  }
}
...
void fake_init_lock(lck_mtx_t *lck) {
  ...
  if (ut_mocks_use_fibers) {
    ...
    PT_QUIET;
    PT_ASSERT_POSIX_ZERO(ret, "pthread_mutex_init");
  }
}

T_MOCK(void, lck_mtx_init,
       (lck_mtx_t * lck, lck_grp_t *grp, lck_attr_t *attr)) {
  fake_init_lock(lck);
}

T_MOCK(void, lck_mtx_destroy, (lck_mtx_t * lck, lck_grp_t *grp)) {
  struct mock_lck_mtx_t *mlck = (struct mock_lck_mtx_t *)lck;
  if (ut_mocks_use_fibers) {
    ...
    PT_QUIET;
    PT_ASSERT_POSIX_ZERO(ret, "pthread_mutex_destroy");
    free(mlck->pt_m);
    mlck->pt_m = NULL;
  }
}

T_MOCK(void, lck_mtx_lock, (lck_mtx_t * lock)) {
  ...
  if (ut_mocks_use_fibers) {
    ...
    PT_QUIET;
    PT_ASSERT_POSIX_ZERO(ret, "pthread_mutex_lock");
  }
  mlck->lck_mtx.owner = ctid;
}

T_MOCK(void, lck_mtx_lock_spin, (lck_mtx_t * lock)) {
  uint32_t ctid = MOCK_current_thread_fast()->ctid;

  struct mock_lck_mtx_t *mlck = (struct mock_lck_mtx_t *)lock;
  if (ut_mocks_use_fibers) {
    fibers_mutex_lock(
        mlck->f_m, false); // do not check for disabled preemption if spinlock
  } else {
    int ret = pthread_mutex_lock(mlck->pt_m);
    PT_QUIET;
    PT_ASSERT_POSIX_ZERO(ret, "pthread_mutex_lock");
...
T_MOCK(boolean_t, lck_mtx_try_lock, (lck_mtx_t * lock)) {
  uint32_t ctid = MOCK_current_thread_fast()->ctid;
...
T_MOCK(void, lck_mtx_unlock, (lck_mtx_t * lock)) {
  struct mock_lck_mtx_t *mlck = (struct mock_lck_mtx_t *)lock;
  ...
  if (ut_mocks_use_fibers) {
    ...
    PT_QUIET;
    PT_ASSERT_POSIX_ZERO(ret, "pthread_mutex_unlock");
  }
}

T_MOCK(void, mutex_pause, (uint32_t collisions)) {
  if (ut_mocks_use_fibers) {
...
void fake_init_rwlock(struct mock_lck_rw_t *mlck) {
  mlck->rw = calloc(1, sizeof(fibers_rwlock_t));
  fibers_rwlock_init(mlck->rw);
}

static boolean_t fake_rw_try_lock(struct mock_lck_rw_t *mlck,
...
static bool fake_rw_lock_would_yield_exclusive(struct mock_lck_rw_t *mlck,
                                               lck_rw_yield_t mode) {
...
T_MOCK(void, lck_rw_init, (lck_rw_t * lck, lck_grp_t *grp, lck_attr_t *attr)) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(void, lck_rw_destroy, (lck_rw_t * lck, lck_grp_t *grp)) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(void, lck_rw_unlock, (lck_rw_t * lck, lck_rw_type_t lck_rw_type)) {
  if (!ut_mocks_use_fibers) {
...
static void lck_rw_old_mock_unlock_shared(lck_rw_t *lck) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(void, lck_rw_unlock_shared, (lck_rw_t * lck)) {
  lck_rw_old_mock_unlock_shared(lck);
}

T_MOCK(void, lck_rw_unlock_exclusive, (lck_rw_t * lck)) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(void, lck_rw_lock_exclusive, (lck_rw_t * lck)) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(void, lck_rw_lock_shared, (lck_rw_t * lck)) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(boolean_t, lck_rw_try_lock,
       (lck_rw_t * lck, lck_rw_type_t lck_rw_type)) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(boolean_t, lck_rw_try_lock_exclusive, (lck_rw_t * lck)) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(boolean_t, lck_rw_try_lock_shared, (lck_rw_t * lck)) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(lck_rw_type_t, lck_rw_done, (lck_rw_t * lck)) {
  if (!ut_mocks_use_fibers) {
  ...
  struct mock_lck_rw_t *mlck = (struct mock_lck_rw_t *)lck;
  mlck->lck_rw_owner = 0;
  // If there is a writer locking it must be the current fiber or will trigger
  // an assertion in fibers_rwlock_wrunlock
  lck_rw_type_t ret =
      mlck->rw->writer_active ? LCK_RW_TYPE_EXCLUSIVE : LCK_RW_TYPE_SHARED;
  fibers_rwlock_unlock(mlck->rw);
...
T_MOCK(boolean_t, lck_rw_lock_shared_to_exclusive, (lck_rw_t * lck)) {
...
T_MOCK(void, lck_rw_lock_exclusive_to_shared, (lck_rw_t * lck)) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(void, lck_rw_assert, (lck_rw_t * lck, unsigned int type)) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(bool, lck_rw_lock_would_yield_exclusive,
       (lck_rw_t * lck, lck_rw_yield_t mode)) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(bool, lck_rw_lock_would_yield_shared, (lck_rw_t * lck)) {
  if (!ut_mocks_use_fibers) {
...
// Note: No need to mock lck_rw_sleep as it uses lck_rw_* API and waitq, we
// already mock everything the function uses

// --------------- waitq ------------------

/*
 *   If the 4 bytes of mock_waitq.mock_magic are not matching MOCK_WAITQ_MAGIC
 *   it means the waitq comes from an unsupported location and was not created
 * with mock_waitq_init().
 */
...
static bool waitq_use_real_impl(waitq_t wq) {
  return !ut_mocks_use_fibers || waitq_type(wq) != WQT_QUEUE;
}

int mock_waitq_init(struct mock_waitq *wq) {
  if (!wq) {
    return EINVAL;
  }
...
int mock_waitq_destroy(struct mock_waitq *wq) {
  if (!wq) {
    return EINVAL;
  }
  PT_QUIET;
  PT_ASSERT_TRUE(wq->mock_magic == MOCK_WAITQ_MAGIC,
                 "missing mock_waitq magic");
...
static inline bool waitq_should_unlock(waitq_wakeup_flags_t flags) {
  return (flags & (WAITQ_UNLOCK | WAITQ_KEEP_LOCKED)) == WAITQ_UNLOCK;
}

static inline bool waitq_should_enable_interrupts(waitq_wakeup_flags_t flags) {
  return (flags &
          (WAITQ_UNLOCK | WAITQ_KEEP_LOCKED | WAITQ_ENABLE_INTERRUPTS)) ==
         (WAITQ_UNLOCK | WAITQ_ENABLE_INTERRUPTS);
}

T_MOCK(void, waitq_init, (waitq_t wq, waitq_type_t type, int policy)) {
  ...
  *wq.wq_q = (struct waitq){
  ...
  PT_QUIET;
  PT_ASSERT_TRUE(type == WQT_QUEUE, "invalid waitq type");
...
T_MOCK(void, waitq_deinit, (waitq_t wq)) {
  ...
  PT_QUIET;
  PT_ASSERT_TRUE(MWQCAST(wq)->mock_magic == MOCK_WAITQ_MAGIC,
                 "missing mock_waitq magic");
  mock_waitq_destroy(MWQCAST(wq));
}

T_MOCK(void, waitq_lock, (waitq_t wq)) {
  if (waitq_use_real_impl(wq)) {
    waitq_lock(wq);
    return;
  }
...
T_MOCK(void, waitq_unlock, (waitq_t wq)) {
  if (waitq_use_real_impl(wq)) {
    waitq_unlock(wq);
    return;
  }
...
T_MOCK(bool, waitq_is_valid, (waitq_t wq)) {
  if (waitq_use_real_impl(wq)) {
...
T_MOCK(void, waitq_invalidate, (waitq_t wq)) {
  if (waitq_use_real_impl(wq)) {
...
T_MOCK(bool, waitq_held, (waitq_t wq)) {
  if (waitq_use_real_impl(wq)) {
...
T_MOCK(void, waitq_lock_wait, (waitq_t wq, uint32_t ticket)) {
  MOCK_waitq_lock(wq);
}

T_MOCK(bool, waitq_lock_try, (waitq_t wq)) {
  if (waitq_use_real_impl(wq)) {
...
static void global_mock_waitqs_init(void) {
  for (int i = 0; i < MOCK_WAITQS_NUM; ++i) {
    MOCK_waitq_init((struct waitq *)&global_mock_waitqs[i], WQT_QUEUE,
                    SYNC_POLICY_FIFO);
  }
  global_mock_waitqs_inited = 1;
}

struct mock_waitq *find_mock_waitq(event64_t event) {
...
struct mock_waitq *find_or_alloc_mock_waitq(event64_t event) {
  if (!global_mock_waitqs_inited) {
    global_mock_waitqs_init();
  }
  int first_free = -1;
  ...
  PT_QUIET;
  PT_ASSERT_TRUE(first_free >= 0, "no more space in global_mock_waitqs");
...
T_MOCK_DYNAMIC(kern_return_t, thread_wakeup_prim,
               (event_t event, boolean_t one_thread, wait_result_t result),
               (event, one_thread, result), {
                 if (ut_mocks_use_fibers) {
                   // fibers is mocking waitq apis, go forward calling the real
                   // thread_wakeup_prim
                 ...
                 PT_QUIET;
                 PT_ASSERT_POSIX_ZERO(ret, "thread_wakeup pthread_mutex_lock");

                 struct pthread_mock_event_table_entry *event_entry =
                     find_pthread_mock_event_entry(s, event);
                 if (event_entry == NULL) {
                   kr = KERN_NOT_WAITING;
                   goto done;
                 }
                 if (one_thread) {
                   ...
                   PT_QUIET;
                   PT_ASSERT_POSIX_ZERO(ret,
                                        "thread_wakeup pthread_cond_signal");
                 } else {
                   ret = pthread_cond_broadcast(&event_entry->cond);
                   PT_QUIET;
                   PT_ASSERT_POSIX_ZERO(ret,
                                        "thread_wakeup pthread_cond_broadcast");
                 }
               done:
...
wait_result_t pthread_mock_thread_block_reason(thread_continue_t continuation,
                                               void *parameter, ast_t reason) {
  PT_QUIET;
  PT_ASSERT_TRUE(continuation == THREAD_CONTINUE_NULL && parameter == NULL &&
                     reason == AST_NONE,
  ...
  int ret = pthread_mutex_lock(&s->events_mutex);
  PT_QUIET;
  PT_ASSERT_POSIX_ZERO(ret, "thread_block pthread_mutex_lock");

  // find empty entry in table
  struct pthread_mock_event_table_entry *event_entry =
      find_pthread_mock_event_entry(s, 0);
  PT_QUIET;
  PT_ASSERT_NOTNULL(event_entry, "empty entry not found");
  ...
  if (!event_entry->cond_inited) {
    ...
    PT_QUIET;
    PT_ASSERT_POSIX_ZERO(ret, "thread_block pthread_cond_init");
    event_entry->cond_inited = true;
  }

  // wait on variable. This releases the mutex, waits and reaquires it before
  // returning
  ret = pthread_cond_wait(&event_entry->cond, &s->events_mutex);
  PT_QUIET;
  PT_ASSERT_POSIX_ZERO(ret, "thread_block pthread_cond_wait");

  // reset the entry so that it can be reused (will be done by all waiters that
  // woke up)
  event_entry->ev = 0;

  ret = pthread_mutex_unlock(&s->events_mutex);
  PT_QUIET;
  PT_ASSERT_POSIX_ZERO(ret, "thread_block pthread_mutex_unlock");

  return THREAD_AWAKENED;
}

kern_return_t pthread_mock_clear_wait(thread_t thread, wait_result_t result) {
  struct mock_process_state *s = get_proc_state();
  int ret = pthread_mutex_lock(&s->events_mutex);
  PT_QUIET;
  PT_ASSERT_POSIX_ZERO(ret, "clear_wait pthread_mutex_lock");
  ...
  ret = pthread_mutex_unlock(&s->events_mutex);
  PT_QUIET;
  PT_ASSERT_POSIX_ZERO(ret, "clear_wait pthread_mutex_unlock");
  return KERN_SUCCESS;
}

// fibers mocks

T_MOCK(struct waitq *, _global_eventq, (event64_t event)) {
  if (!ut_mocks_use_fibers) {
...
T_MOCK(wait_result_t, waitq_assert_wait64_locked,
       (waitq_t waitq, event64_t wait_event, wait_interrupt_t interruptible,
        wait_timeout_urgency_t urgency, uint64_t deadline, uint64_t leeway,
        thread_t thread)) {
  if (waitq_use_real_impl(waitq)) {
    return waitq_assert_wait64_locked(waitq, wait_event, interruptible, urgency,
                                      deadline, leeway, thread);
  }
  ...
  PT_QUIET;
  PT_ASSERT_TRUE(wq->current_event == wait_event,
                 "waitq_assert_wait64_locked another event queue");

  struct mock_thread *mock_thread = (struct mock_thread *)
      thread; // !!! ASSUME every thread_t is created from mock_thread
...
T_MOCK(wait_result_t, waitq_assert_wait64,
       (struct waitq * waitq, event64_t wait_event,
  ...
  wait_result_t res = MOCK_waitq_assert_wait64_locked(
      waitq, wait_event, interruptible, TIMEOUT_URGENCY_SYS_NORMAL, deadline,
      TIMEOUT_NO_LEEWAY, thread);
  MOCK_waitq_unlock(waitq);
  return res;
}

static void mock_waitq_clear_wait(struct mock_thread *thread,
                                  struct mock_waitq *wq) {
  ...
  PT_QUIET;
  PT_ASSERT_TRUE(removed, "thread_block thread not in wq");
  thread->wq_next = NULL;
  ...
  PT_QUIET;
  PT_ASSERT_TRUE(wq->extra->waiting_thread_count >= 0, "something bad");
}

static struct mock_thread *mock_waitq_pop_wait(struct mock_waitq *wq) {
  if (wq->extra->waiting_thread_count == 0) {
    return NULL;
  }
...
T_MOCK_DYNAMIC(wait_result_t, thread_block_reason,
               (thread_continue_t continuation, void *parameter, ast_t reason),
               (continuation, parameter, reason), {
                 if (!ut_mocks_use_fibers) {
                   return pthread_mock_thread_block_reason(continuation,
                                                           parameter, reason);
                 }

                 PT_QUIET;
                 PT_ASSERT_TRUE(continuation == THREAD_CONTINUE_NULL &&
                                    parameter == NULL && reason == AST_NONE,
                                "thread_block argument");
                 ...
                 PT_ASSERT_TRUE(
                     ...
                     "thread_block called but thread state is not TH_WAIT");

                 /*
                  * In case of a window between assert_wait and thread_block
                  * another thread could wake up the current thread after being
                  * added to the waitq but before the block. In this case, the
                  * thread will still be TH_WAIT but without an assigned waitq.
                 ...
                 if (wq == NULL) {
                   ...
                   PT_ASSERT_TRUE(
                       ...
                       "with waitq == NULL there must be TH_WAKING set");
                   thread->state &= ~TH_WAKING;
                   goto awake_thread;
                 }
                 ...
                 if (thread->state & TH_WAKING) {
                   ...
                   // is this possible? TH_WAKING is always set ATM in the
                   // mocks, keep this code to be more robust
...
T_MOCK(kern_return_t, clear_wait, (thread_t thread, wait_result_t wresult)) {
  if (!ut_mocks_use_fibers) {
  ...
  struct mock_waitq *wq = MWQCAST(thread->waitq);
  PT_QUIET;
  PT_ASSERT_TRUE(wq != NULL, "thread->waitq is NULL");
...
static void waitq_wakeup_fiber_callback(void *arg, fiber_t target) {
...
T_MOCK(uint32_t, waitq_wakeup64_nthreads_locked,
       (waitq_t waitq, event64_t wake_event, wait_result_t result,
        waitq_wakeup_flags_t flags, uint32_t nthreads)) {
  if (waitq_use_real_impl(waitq)) {
    return waitq_wakeup64_nthreads_locked(waitq, wake_event, result, flags,
                                          nthreads);
  }

  // RANGELOCKINGTODO rdar://150846598 flags
  waitq_wakeup_args_t wakeup_args = {.wait_result = result};

  struct mock_waitq *wq = MWQCAST(waitq);
  PT_QUIET;
  PT_ASSERT_TRUE(wq->current_event == wake_event,
                 "waitq_wakeup64_nthreads current_event is wrong");

  // Avoid to trigger a switch in fibers_condition_wakeup_some before a valid
  // state in the waitq
  fibers_current->may_yield_disabled++;

  FIBERS_LOG(FIBERS_LOG_DEBUG,
             "waitq_wakeup64_nthreads_locked nthreads=%u wake_event=%lld",
             nthreads, wake_event);

  int count = fibers_condition_wakeup_some(
      &wq->extra->cond, nthreads, &waitq_wakeup_fiber_callback, &wakeup_args);

  /*
   * In case of a window in which a thread is pushed to the waitq but
   * thread_block was still not called when another thread wakes up the threads
   * in the waitq here. fibers_condition_wakeup_some will not find these fibers
   * as they are not waiting on the condition, In this case these fibers must be
   * in FIBER_STOP that means that they are ready to be scheduled, but we still
   * need to take action here to remove them from the waitq and clear the state.
   */
  while (wq->extra->waiting_thread_count && count < nthreads) {
    struct mock_thread *thread = mock_waitq_pop_wait(wq);
    PT_QUIET;
    PT_ASSERT_TRUE(thread->fiber->state & FIBER_STOP,
                   "leftover fiber in waitq not in FIBER_STOP");
    thread->th.state |= TH_WAKING;
    thread->th.waitq.wq_q = NULL;
...
T_MOCK(thread_t, waitq_wakeup64_identify_locked,
       (waitq_t waitq, event64_t wake_event, waitq_wakeup_flags_t flags)) {
  if (waitq_use_real_impl(waitq)) {
  ...
  PT_QUIET;
  PT_ASSERT_TRUE(wq->current_event == wake_event,
                 "waitq_wakeup64_identify_locked current_event is wrong");

  // RANGELOCKINGTODO rdar://150845975 for fuzzing select random, not the top of
  // the queue
  struct mock_thread *mock_thread = wq->extra->waiting_threads;
  if (mock_thread == NULL) {
    return THREAD_NULL;
  ...
  // Preemption will be re-enabled when the thread is resumed in
  // `waitq_resume_identify_thread`
  MOCK__disable_preemption();
  ...
  FIBERS_LOG(FIBERS_LOG_DEBUG,
             "waitq_wakeup64_identify_locked identified fiber %d",
             mock_thread->fiber->id);
...
T_MOCK(void, waitq_resume_identified_thread,
       (waitq_t waitq, thread_t thread, wait_result_t result,
  ...
  bool found =
      fibers_condition_wakeup_identified(&wq->extra->cond, mock_thread->fiber);
  if (!found) {
    /*
     * In case of a window in which a thread is pushed to the waitq but
     * thread_block was still not called when the thread is identified by
     * another one and resumed, we pop it from the waitq in
     * waitq_wakeup64_identify_locked but we will not find it in
     * wq->cond.wait_queue. In this case it is not needed any action as the
     * fiber must be in FIBER_STOP and can already be scheduled.
     */
    PT_QUIET;
    PT_ASSERT_TRUE(mock_thread->fiber->state & FIBER_STOP,
                   "waitq_resume_identified_thread fiber not found in "
                   "condition and not in FIBER_STOP");
  }

  // Paired with the call to `waitq_wakeup64_identify_locked`
  MOCK__enable_preemption();

  fibers_may_yield_internal_with_reason(FIBERS_YIELD_REASON_WAKEUP |
                                        FIBERS_YIELD_REASON_ERROR_IF(!found));
}

// Allow to cause a context switch from a function that can be called from XNU
T_MOCK(void, ut_fibers_ctxswitch, (void)) {
  if (ut_mocks_use_fibers) {
    fibers_yield();
  }
}

// Allow to cause a context switch to a specific fiber from a function that can
// be called from XNU
T_MOCK(void, ut_fibers_ctxswitch_to, (int fiber_id)) {
  if (ut_mocks_use_fibers) {
...
T_MOCK(int, ut_fibers_current_id, (void)) {
  if (ut_mocks_use_fibers) {
...
#define IS_ALIGNED(ptr, size)                                                  \
  ((((uintptr_t)(ptr)) & (((uintptr_t)(size)) - 1)) == 0)
#define IS_ATOMIC(ptr, size)                                                   \
  ((size) <= sizeof(uint64_t) && IS_ALIGNED(ptr, size))

// These functions can be called from XNU to enter/exit atomic regions in which
// the data checker is disabled
T_MOCK(void, data_race_checker_atomic_begin, (void)) {
  fibers_checker_atomic_begin();
}
T_MOCK(void, data_race_checker_atomic_end, (void)) {
  fibers_checker_atomic_end();
}

/*
 * Detecting data races on memory operations:
 * Memory operation functions are used to check for data races using the fibers
 * checkers API, a software implementation of DataCollider. The idea is to set a
 * watchpoint before context switching and report a data race every time a
 * concurrent access (watchpoint hit) is in between a write or a write in
 * between a load. To be more robust, we also check that the value pointed the
 * memory operation address before the context switch is still the same after
 * the context switch. If not, very likely it is a data race. Atomic memory
 * operations should be excluded from this, we use the IS_ATOMIC macro to filter
 * memory loads. Note: atomic_fetch_add_explicit() et al. on ARM64 are compiled
 * to LDADD et al. that seem to not be supported by __sanitizer_cov_loadX, ok
 * for us we want to skip atomic operations.
 */
#define SANCOV_LOAD_STORE_DATA_CHECKER(type, size, access_type)                \
  do {                                                                         \
    if (fibers_current->may_yield_disabled) {                                  \
      return;                                                                  \
    }                                                                          \
    if (fibers_scheduler->fibers_should_yield(                                 \
            fibers_scheduler_context, fibers_may_yield_probability,            \
            FIBERS_YIELD_REASON_PREEMPTION_TRIGGER)) {                         \
      volatile type before = *addr;                                            \
      void *pc = __builtin_return_address(0);                                  \
      bool has_wp =                                                            \
          check_and_set_watchpoint(pc, (uintptr_t)addr, size, access_type);    \
                                                                               \
...
/*
 * Mock the SanitizerCoverage load/store instrumentation callbacks (original in
 * san_attached.c). The functions are execute at every memory operations in
 * libxnu and in the test binary, libmocks is excluded. Functions and files in
 * tools/sanitizers-ignorelist are excluded from instrumentation.
 */
#define MOCK_SANCOV_LOAD_STORE(type, size)                                     \
  __attribute__((optnone)) T_MOCK(void, __sanitizer_cov_load##size,            \
                                  (type * addr)) {                             \
    if (!ut_fibers_use_data_race_checker || IS_ATOMIC(addr, size) ||           \
    ...
    SANCOV_LOAD_STORE_DATA_CHECKER(type, size, ACCESS_TYPE_LOAD);              \
                                                                               ...
                                                                               \
  __attribute__((optnone)) T_MOCK(                                             \
      void, __sanitizer_cov_store##size,                                       \
      (type * addr)) { /* do not care about atomicity for stores */            \
    if (!ut_fibers_use_data_race_checker ||                                    \
        fibers_current->disable_race_checker) {                                \
      fibers_may_yield_with_reason(FIBERS_YIELD_REASON_PREEMPTION_TRIGGER);    \
      return;                                                                  \
