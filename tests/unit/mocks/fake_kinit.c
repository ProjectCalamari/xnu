...
// This define is supposed to come from the .CFLAGS parsing. if it's not,
...
void fake_kernel_bootstrap(void) {
...
// This is the first function that is called before any initialization in
// libkernel. It's made to be first by the order of object files in the linker
// command line in Makefile
__attribute__((constructor)) void fake_kinit(void) {
  fake_init_bootargs();
