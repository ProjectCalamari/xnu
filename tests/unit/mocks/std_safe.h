...
// check that we're being compiled from the unit-tests makefile and that
// UT_MODULE was found
#if defined(UT_MODULE) && (UT_MODULE == -1)
#error                                                                         \
    "UT_MODULE not defined, did you forget to add a `#define UT_MODULE <module>` in your test?"
#endif
...
// The testers are built with -nostdlibinc so the system headers folders (from
// the SDK) are not available when compiling these files. Having system headers
// folders available for the includes search would create conflicts when XNU
// code includes headers like string.h, sys/types.h etc' (which need to come
// from XNU). This is why it's not possible do add headers like <stdlib.h> here.
// Furthermore, even if we could have included system headers like stdlib.h,
// Having that and XNU headers in the same translation unit would have created
// conflicts with XNU-defined types which have the same name but a different
// type from the system includes definition. For example, every type in
// mach/mach_types.h is typedef'd as mach_port_t in the system headers.
