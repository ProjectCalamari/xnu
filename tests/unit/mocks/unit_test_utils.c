...
int64_t run_sysctl_test(const char *t, int64_t value, int argc,
                        char *const *argv) {
  char name[1024];
  int64_t result = 0;
  ...
  PT_QUIET;
  PT_ASSERT_POSIX_ZERO(rc, "sysctlbyname()");
  return result;
}

void *checked_alloc_align(size_t size, size_t align) {
  void *ptr = NULL;
  if (align < sizeof(void *)) {
    ptr = calloc(1, size);
    PT_QUIET;
    PT_ASSERT_NOTNULL(ptr, "failed alloc");
  } else {
...
struct backtrace_array *collect_current_backtrace(void) {
  struct backtrace_array *bt = malloc(sizeof(struct backtrace_array));
  bt->nptrs = backtrace(bt->buffer, 100);
  return bt;
}

void print_collected_backtrace(struct backtrace_array *bt) {
#ifdef __BUILDING_WITH_SANITIZER__
  // If compiled with any sanitizer, use __sanitizer_symbolize_pc as it gives
  // much more info compared to backtrace_symbols
  ...
  PT_QUIET;
  PT_ASSERT_NOTNULL(strings, "backtrace_symbols");
  for (int idx = 0; idx < bt->nptrs; idx++) {
    raw_printf("%s\n", strings[idx]);
  }
  free(strings);
...
void print_current_backtrace(void) {
