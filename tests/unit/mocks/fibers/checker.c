...
static inline size_t hash_address(uintptr_t addr, size_t capacity) {
...
struct watchpoint_entry {
  union {
    ...
    struct backtrace_array *backtrace; /* backtrace collected at program point
                                          of the memory operation instruction */
  };
  uintptr_t address; /* address of the memory operation happening on the fiber
                        with id=fiber_id */
...
static void watchpoint_entry_init(struct watchpoint_entry *entry,
                                  uintptr_t address, enum access_type type,
                                  size_t size, fiber_t fiber) {
  FIBERS_ASSERT(entry != NULL, "watchpoint_entry_init: null entry");
  FIBERS_ASSERT(size <= 16,
                "watchpoint_entry_init: invalid size"); // no access is bigger
                                                        // than sizeof(int128)
...
// A simple hashmap to store watchpoint_entry indexed by
// watchpoint_entry.address
...
static bool watchpoint_map_is_initialized(struct watchpoint_map *map) {
  return map->capacity != 0 && map->buckets != NULL;
}

void watchpoint_map_init(struct watchpoint_map *map) {
  map->count = 0;
  map->capacity = WATCHPOINT_MAP_INITIAL_CAPACITY;
  map->buckets = calloc(map->capacity, sizeof(struct watchpoint_node *));
}

// Currently the watchpoint map has a global scope, so this function is
// unnecessary We keep it here for future usage
/*
 *  void
...
static bool watchpoint_map_resize(struct watchpoint_map *map,
                                  size_t new_capacity) {
  if (new_capacity < map->count) {
    return false;
  }
  struct watchpoint_node **new_buckets =
      calloc(new_capacity, sizeof(struct watchpoint_node *));
  if (new_buckets == NULL) {
    return false;
  }
  ...
  for (size_t i = 0; i < map->capacity; ++i) {
    ...
    while (current != NULL) {
      ...
      size_t new_index =
          hash_address(node_to_move->entry.address, new_capacity);
...
void watchpoint_map_add(struct watchpoint_map *map,
                        struct watchpoint_entry entry) {
...
bool watchpoint_map_find_remove(struct watchpoint_map *map, uintptr_t address,
                                fiber_t fiber,
  ...
  while (current != NULL) {
    if (current->entry.address == address &&
        current->entry.fiber_id == fiber->id) {
      if (removed_entry) {
...
static void report_race(uintptr_t current_addr, size_t current_size,
                        enum access_type current_type,
                        struct watchpoint_entry *conflicting_entry) {
  raw_printf("==== Warning: Fibers data race checker violation ====\n");
  raw_printf("%s of size %d at %p by fiber %d\n",
             current_type == ACCESS_TYPE_STORE ? "Write" : "Read", current_size,
             (void *)current_addr, fibers_current->id);
  if (fibers_debug) {
    print_current_backtrace();
  }

  raw_printf("Previous %s of size %d at %p by fiber %d\n",
             conflicting_entry->access_type == ACCESS_TYPE_STORE ? "write"
                                                                 : "read",
             conflicting_entry->size, (void *)conflicting_entry->address,
             conflicting_entry->fiber_id);
  if (conflicting_entry->has_backtrace) {
    print_collected_backtrace(conflicting_entry->backtrace);
  } else {
    struct backtrace_array bt = {.buffer = {(void *)conflicting_entry->pc},
                                 .nptrs = 1};
...
static inline void report_missing_race(uintptr_t current_addr,
                                       size_t current_size,
                                       enum access_type current_type) {
  raw_printf("==== Warning: Fibers data race checker violation ====\n");
  raw_printf("%s of size %d at %p by fiber %d\n",
  ...
  raw_printf("Watchpoint was unexpectedly missing or modified by another fiber "
             "during yield.\n");
  if (fibers_abort_on_error) {
    abort();
  }
...
void report_value_race(uintptr_t current_addr, size_t current_size,
                       enum access_type current_type) {
  raw_printf("==== Warning: Fibers data race checker violation ====\n");
  raw_printf("%s of size %d at %p by fiber %d\n",
             current_type == ACCESS_TYPE_STORE ? "Write" : "Read", current_size,
  ...
  raw_printf("Value was modified in between the operation by another fiber "
             "during yield.\n");
  if (fibers_abort_on_error) {
    abort();
  }
...
static inline bool ranges_overlap(uintptr_t addr1, size_t size1,
                                  uintptr_t addr2, size_t size2) {
  if (size1 == 0 || size2 == 0) {
    return false;
  }
...
/*
 * Check for conflicting memory accesses to the same region happening in another
 * fiber. Concurrent loads are allowed, loads in-between a store are not.
 */
static inline bool check_for_conflicts(struct watchpoint_map *map,
  ...
  for (uintptr_t check_addr = start_check_addr;; ++check_addr) {
    ...
    while (node != NULL) {
      ...
      if (ranges_overlap(current_addr, current_size, existing_entry->address,
                         existing_entry->size)) {
...
bool check_and_set_watchpoint(void *pc, uintptr_t address, size_t size,
  ...
  if (check_for_conflicts(&checker_watchpoints, address, size, access_type)) {
    ...
    watchpoint_entry_init(&new_watchpoint, address, access_type, size,
                          fibers_current);
    if (fibers_debug) {
...
void post_check_and_remove_watchpoint(uintptr_t address, size_t size,
                                      enum access_type access_type) {
  struct watchpoint_entry removed_entry;
  if (watchpoint_map_find_remove(&checker_watchpoints, address, fibers_current,
                                 &removed_entry)) {
    FIBERS_ASSERT(removed_entry.address == address, "race? internal error");
    FIBERS_ASSERT(removed_entry.access_type == access_type,
                  "race? internal error");
    FIBERS_ASSERT(removed_entry.size == size, "race? internal error");
    FIBERS_ASSERT(removed_entry.fiber_id == fibers_current->id,
                  "race? internal error");
