...
struct fibers_queue fibers_existing_queue = {.top = &fibers_main,
                                             .count = 1}; /* existing fibers */

static void fibers_default_choose_next(__unused void *arg, int state) {
  fibers_switch_random(state);
}

static bool fibers_default_should_yield(__unused void *arg,
...
void fibers_scheduler_get(struct fibers_scheduler_t **scheduler,
                          void **context) {
  *scheduler = fibers_scheduler;
  *context = fibers_scheduler_context;
}

void fibers_scheduler_set(struct fibers_scheduler_t *scheduler, void *context) {
...
static void fibers_create_trampoline(int arg1, int arg2) {
  struct fibers_create_trampoline_args *args =
      (struct fibers_create_trampoline_args *)(((uintptr_t)arg1 << 32) |
                                               (uintptr_t)arg2);
  // Copy fiber and arg to the local scope as by the time start_routine is
  // called the parent fibers_create stack may have been deallocated
  fiber_t fiber = args->fiber;
  void *start_routine_arg = args->start_routine_arg;

#ifdef __BUILDING_WITH_ASAN__
  __sanitizer_finish_switch_fiber(&fiber->sanitizer_fake_stack,
                                  &fiber->stack_bottom, &fiber->stack_size);
#endif

  // setjmp/longjmp are faster context switch primitives compared to swapcontext
  if (setjmp(fiber->env) == 0) {
    // The first time the setjmp is called to save the current context in
    // fiber->env we end un in this branch in which we switch back to
    // fibers_create When the fiber will be scheduled for the first time,
    // setjmp(fiber->env) != 0 and thus the execution will continue in the other
    // branch that calls args.start_routine
#ifdef __BUILDING_WITH_ASAN__
    __sanitizer_start_switch_fiber(&fibers_current->sanitizer_fake_stack,
                                   fibers_current->stack_bottom,
                                   fibers_current->stack_size);
#endif
#ifdef __BUILDING_WITH_TSAN__
...
fiber_t fibers_create(size_t stack_size, void *(*start_routine)(void *),
                      void *arg) {
  if (fibers_current == &fibers_main && fibers_main.stack_bottom == NULL) {
    // fibers_main has no stack_bottom or stack_size, get them here the first
    // time
  ...
  FIBERS_ASSERT(fibers_last_forged_id != 0,
                "fibers_create: new fiber id integer overflow");
  ...
  makecontext(&child_uc, (void (*)())fibers_create_trampoline, 2,
              trampoline_args1, trampoline_args2);

  // switch to the trampoline to setup the setjmp env of the fiber on the newly
  // created stack, then switch back setjmp/longjmp are faster context switch
  // primitives, swapcontext will never be used again for this fiber ref. the
  // ThreadSanitizer fibers example in LLVM at
  // compiler-rt/test/tsan/fiber_longjmp.cpp
  if (setjmp(trampoline_args.parent_env) == 0) {
#ifdef __BUILDING_WITH_ASAN__
    __sanitizer_start_switch_fiber(&fiber->sanitizer_fake_stack,
                                   fiber->stack_bottom, fiber->stack_size);
#endif
#ifdef __BUILDING_WITH_TSAN__
    __tsan_switch_to_fiber(fiber->tsan_fiber, 0);
#endif
    FIBERS_ASSERT(swapcontext(&tmp_uc, &child_uc) == 0,
                  "fibers_create: swapcontext");
  }

#ifdef __BUILDING_WITH_ASAN__
  // fibers_create_trampoline did not change fibers_current
  __sanitizer_finish_switch_fiber(&fibers_current->sanitizer_fake_stack,
                                  &fibers_current->stack_bottom,
                                  &fibers_current->stack_size);
#endif
...
static void fibers_dispose(fiber_t fiber) {
...
void fibers_exit(void *ret_value) {
  FIBERS_ASSERT(fibers_current->may_yield_disabled == 0,
                "fibers_exit: fibers_current->may_yield_disabled is not 0");

  fibers_current->ret_value = ret_value;
  if (fibers_current->joiner) {
    FIBERS_LOG(FIBERS_LOG_INFO, "exiting, joined by %d",
               fibers_current->joiner->id);
...
void *fibers_join(fiber_t target) {
  FIBERS_ASSERT(fibers_current->may_yield_disabled == 0,
                "fibers_join: fibers_current->may_yield_disabled is not 0");

  fibers_may_yield_internal_with_reason(FIBERS_YIELD_REASON_JOIN |
                                        FIBERS_YIELD_REASON_ORDER_PRE);

  FIBERS_LOG(FIBERS_LOG_INFO, "join %d", target->id);
  if (target->state != FIBER_DEAD) {
    FIBERS_ASSERT(target->joiner == NULL,
                  "fibers_join: %d already joined by %d", target->id,
                  target->joiner->id);

    target->joiner = fibers_current;
    fibers_current->joining = target;

    // RANGELOCKINGTODO rdar://150845975 maybe have a queue for fibers in join
    // to output debug info in case of deadlock
...
void fibers_switch_helper(fiber_t target, int state) {
...
#ifdef __BUILDING_WITH_ASAN__
    __sanitizer_start_switch_fiber(&target->sanitizer_fake_stack,
                                   target->stack_bottom, target->stack_size);
#endif
#ifdef __BUILDING_WITH_TSAN__
    __tsan_switch_to_fiber(
        target->tsan_fiber,
        state == FIBER_DEAD ? 0 : __tsan_switch_to_fiber_no_sync);
#endif
    longjmp(target->env, 1337);
  }
#ifdef __BUILDING_WITH_ASAN__
  __sanitizer_finish_switch_fiber(&save->sanitizer_fake_stack,
                                  &save->stack_bottom, &save->stack_size);
#endif
...
void fibers_choose_next(int state) {
  fibers_scheduler->fibers_choose_next(fibers_scheduler_context, state);
}

void fibers_switch_to(fiber_t target, int state) {
  FIBERS_ASSERT(fibers_queue_remove(&fibers_run_queue, target),
                "fibers_switch_to");
  fibers_switch_helper(target, state);
}

void fibers_switch_to_by_id(int target_id, int state) {
  fiber_t target = fibers_queue_remove_by_id(&fibers_run_queue, target_id);
  FIBERS_ASSERT(target != NULL, "fibers_switch_to_by_id");
  fibers_switch_helper(target, state);
}

void fibers_switch_top(int state) {
  fiber_t target = fibers_queue_pop(&fibers_run_queue, 0);
  fibers_switch_helper(target, state);
}

void fibers_switch_random(int state) {
  fiber_t target =
      fibers_queue_pop(&fibers_run_queue, random_below(fibers_run_queue.count));
  fibers_switch_helper(target, state);
}

void fibers_yield_to(int fiber_id) {
  fibers_queue_push(&fibers_run_queue, fibers_current);
  fibers_switch_to_by_id(fiber_id, FIBER_STOP);
}

void fibers_yield(void) {
  fibers_queue_push(&fibers_run_queue, fibers_current);
  fibers_choose_next(FIBER_STOP);
}

bool fibers_may_yield_internal(void) {
  return fibers_may_yield_with_prob_and_reason(FIBERS_INTERNAL_YIELD_PROB,
                                               FIBERS_YIELD_REASON_UNKNOWN);
}

bool fibers_may_yield_internal_with_reason(fiber_yield_reason_t reason) {
  return fibers_may_yield_with_prob_and_reason(FIBERS_INTERNAL_YIELD_PROB,
                                               reason);
}

bool fibers_may_yield(void) {
  return fibers_may_yield_with_prob(fibers_may_yield_probability);
}

bool fibers_may_yield_with_prob(uint64_t probability) {
  return fibers_may_yield_with_prob_and_reason(probability,
                                               FIBERS_YIELD_REASON_UNKNOWN);
}

bool fibers_may_yield_with_reason(fiber_yield_reason_t reason) {
  return fibers_may_yield_with_prob_and_reason(fibers_may_yield_probability,
                                               reason);
}

bool fibers_may_yield_with_prob_and_reason(uint64_t probability,
                                           fiber_yield_reason_t reason) {
