...
#define FIBERS_INTERNAL_YIELD_PROB                                             \
  4 // switch on internal yield points 1/4 of the times
#define FIBERS_DEFAULT_YIELD_PROB 256

/* Configuration variables */
extern int fibers_log_level; // see FIBERS_LOG and the levels FIBERS_LOG_*
extern bool fibers_debug; // Mostly used to collect backtraces at fibers events
                          // points. The slowdown is huge.
extern int fibers_abort_on_error; // By default do not stop execution at errors,
                                  // set to not 0 to abort.
extern uint64_t
    fibers_may_yield_probability; // FIBERS_DEFAULT_YIELD_PROB by default
...
#define FIBERS_YIELD_REASON_ORDER_POST                                         \
  (1 << FIBERS_YIELD_REASON_ORDER_PRE_SHIFT)
#define FIBERS_YIELD_REASON_ORDER(x)                                           \
  ((x) & (1 << FIBERS_YIELD_REASON_ORDER_PRE_SHIFT))
...
#define FIBERS_YIELD_REASON_MUTEX_STATE(x)                                     \
  ((x) & (3 << FIBERS_YIELD_REASON_MUTEX_DESTROY))
...
#define FIBERS_YIELD_REASON_PREEMPTION_WILL_ENABLE                             \
  (FIBERS_YIELD_REASON_PREEMPTION_CONTROL | FIBERS_YIELD_REASON_MUTEX_UNLOCK | \
   FIBERS_YIELD_REASON_ORDER_PRE)

#define FIBERS_YIELD_REASON_PREEMPTION_DID_ENABLE                              \
  (FIBERS_YIELD_REASON_PREEMPTION_CONTROL | FIBERS_YIELD_REASON_MUTEX_UNLOCK | \
   FIBERS_YIELD_REASON_ORDER_POST)

#define FIBERS_YIELD_REASON_PREEMPTION_WILL_DISABLE                            \
  (FIBERS_YIELD_REASON_PREEMPTION_CONTROL | FIBERS_YIELD_REASON_MUTEX_LOCK |   \
   FIBERS_YIELD_REASON_ORDER_PRE)
...
#define FIBERS_YIELD_REASON_MUTEX_WILL_LOCK                                    \
...
#define FIBERS_YIELD_REASON_MUTEX_TRY_LOCK_FAIL                                \
  (FIBERS_YIELD_REASON_MUTEX_DID_LOCK | FIBERS_YIELD_REASON_ERROR)

#define FIBERS_YIELD_REASON_MUTEX_WILL_UNLOCK                                  \
  (FIBERS_YIELD_REASON_MUTEX | FIBERS_YIELD_REASON_MUTEX_UNLOCK |              \
   FIBERS_YIELD_REASON_ORDER_PRE)

#define FIBERS_YIELD_REASON_MUTEX_DID_UNLOCK                                   \
  (FIBERS_YIELD_REASON_MUTEX | FIBERS_YIELD_REASON_MUTEX_UNLOCK |              \
   FIBERS_YIELD_REASON_ORDER_POST)
...
struct fibers_scheduler_t {
  ...
  bool (*fibers_should_yield)(void *arg, uint64_t probability,
                              fiber_yield_reason_t reason);
};

extern void fibers_scheduler_get(struct fibers_scheduler_t **scheduler,
                                 void **context);
extern void fibers_scheduler_set(struct fibers_scheduler_t *scheduler,
                                 void *context);
...
static void fibers_checker_atomic_begin(void) {
  fibers_current->disable_race_checker++;
}

static void fibers_checker_atomic_end(void) {
  fibers_current->disable_race_checker--;
}
...
static inline void fibers_queue_push(struct fibers_queue *queue,
                                     fiber_t fiber) {
  FIBERS_ASSERT(fiber->next == NULL,
                "fibers_queue_push: already on another queue");
  fiber->next = queue->top;
  queue->top = fiber;
  queue->count++;
}

static inline fiber_t fibers_queue_pop(struct fibers_queue *queue,
                                       size_t index) {
...
static inline fiber_t fibers_queue_peek(struct fibers_queue *queue) {
  for (fiber_t *iter = &queue->top; *iter != NULL; iter = &(*iter)->next) {
    if ((*iter)->next == NULL) {
      return *iter;
    }
  }
...
static inline bool fibers_queue_contains(struct fibers_queue *queue,
                                         fiber_t fiber) {
  fiber_t iter = queue->top;
  while (iter != NULL) {
    if (iter == fiber) {
...
static inline bool fibers_queue_remove(struct fibers_queue *queue,
                                       fiber_t fiber) {
  fiber_t *iter = &queue->top;
  while (*iter != NULL) {
    if (*iter == fiber) {
...
static inline fiber_t fibers_queue_remove_by_id(struct fibers_queue *queue,
                                                int fiber_id) {
  fiber_t *iter = &queue->top;
  while (*iter != NULL) {
    if ((*iter)->id == fiber_id) {
...
static inline size_t fibers_queue_count(struct fibers_queue *queue) {
  fiber_t iter = queue->top;
  size_t count = 0;
  while (iter != NULL) {
    count++;
...
static inline void fibers_existing_push(fiber_t fiber) {
  FIBERS_ASSERT(fiber->next_existing == NULL,
                "fibers_existing_push: already on existing queue");
  fiber->next_existing = fibers_existing_queue.top;
  fibers_existing_queue.top = fiber;
  fibers_existing_queue.count++;
}

static inline bool fibers_existing_remove(fiber_t fiber) {
...
extern fiber_t fibers_create(size_t stack_size, void *(*start_routine)(void *),
                             void *arg);
...
extern bool fibers_may_yield_with_prob_and_reason(uint64_t probability,
                                                  fiber_yield_reason_t reason);
