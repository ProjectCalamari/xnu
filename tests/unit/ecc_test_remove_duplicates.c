...
extern uint32_t remove_bad_ram_duplicates(uint32_t bad_pages_count,
                                          ppnum_t *bad_pages);

#define UT_MODULE osfmk
T_GLOBAL_META(T_META_NAMESPACE("xnu.unit.ecc_test_remove_duplicates"),
...
T_DECL(ecc_rm_dups_0, "make sure 1 is returned for [0]") {
  ...
  if (bp_count != 1 || bp[0] != 0) {
    T_FAIL("Expected: bp_count == 1 and bp[0] == 0 but got: bp_count == %u, "
           "bp[0] == %u",
           bp_count, bp[0]);
  } else {
...
T_DECL(ecc_rm_dups_001, "make sure 2 is returned for [0, 0, 1]") {
  ...
  if (bp_count != 2 || bp[0] != 0 || bp[1] != 1) {
    T_FAIL("Expected: bp_count == 2 and bp == [0, 1] but got: bp_count == %u, "
           "bp == [%u, %u]",
           bp_count, bp[0], bp[1]);
  } else {
...
T_DECL(ecc_rm_dups_201, "make sure 3 is returned for [2, 0, 1]") {
  ...
  if (bp_count != 3 || bp[0] != 0 || bp[1] != 1 || bp[2] != 2) {
    T_FAIL("Expected: bp_count == 3 and bp == [0, 1, 2] but got: bp_count == "
           "%u, bp == [%u, %u, %u]",
           bp_count, bp[0], bp[1], bp[2]);
  } else {
...
T_DECL(ecc_rm_dups_large, "make sure large input is handled correctly") {
  ...
  if (!valid) {
    T_FAIL("Expected: bp_count == 10 and bp == [0-9] but got: bp_count == %u",
           bp_count);
  } else {
