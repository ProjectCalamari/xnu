...
T_GLOBAL_META(T_META_NAMESPACE("xnu.unit.mocks"),
...
// from unistd.h.
// This can't be in stdsafe.h since it conflicts with a definition in
// bsd/sys/proc_internal.h
unsigned int sleep(unsigned int seconds);

void *do_sleep_and_wake(void *arg) {
...
// this test is meant to fail in order to verify that we're linking with the
// mock unimplemented sptm functions it's useful when debugging the Makefile
void libsptm_init(void);
T_DECL(sptm_link_unimpl, "sptm_link_unimpl",
       T_META_EXPECTFAIL("fail due to unimplemented sptm mock")) {
  libsptm_init();
}
...
// disabled in release since the kernel_funcX() functions are not defined by xnu
// in release

T_DECL(mock_with_callback, "mock_with_callback") {
  size_t ret1 = kernel_func1(1, 2);
  T_ASSERT_EQ(ret1, (size_t)0,
              "expected return before - default value from mock");
  {
    T_MOCK_SET_CALLBACK(kernel_func1, size_t, (int a, char b), {
...
T_DECL(mock_with_retval, "mock_with_retval") {
...

  {
    ...
    T_ASSERT_EQ(r3, (size_t)43,
                "expected value with mock second in the same scope");
  }
...
T_MOCK_SET_PERM_FUNC(size_t, kernel_func2, (int a, char b)) {
...
T_MOCK_SET_PERM_FUNC(size_t, kernel_func4, (int a, char b)) {
...
// a mock that calls the original function implicitly through
// _T_MOCK_DYNAMIC_DEFAULT_IMPL
T_DECL(mock_default_calling_original_implicit,
       "mock_default_calling_original_auto_define") {
...
T_DECL(mock_void_ret, "mock_void_ret") {
  ...
  kernel_func6_was_called = 0;
  T_MOCK_SET_CALLBACK(kernel_func6, void, (int a, char b), {
    T_ASSERT_EQ(a, 3, "expected a");
    T_ASSERT_EQ(b, 4, "expected b");
  });
  kernel_func6(3, 4);
