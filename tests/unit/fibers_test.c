...
T_GLOBAL_META(T_META_NAMESPACE("xnu.unit.fibers"),
              T_META_RADAR_COMPONENT_NAME("xnu"), T_META_OWNER("a_fioraldi"),
...
static void *coop_fibers_func(void *x) {
  int *cooperative_counter = (int *)x;

  if (*cooperative_counter == 0) {
    // main thread can jump here just after fibers_create
    fibers_yield_to(
        0); // switch back to main thread and finish the fibers creation
  }

  T_QUIET;
  T_ASSERT_EQ(*cooperative_counter, fibers_current->id,
              "invalid cooperative_counter");
...
T_DECL(coop_fibers, "cooperative scheduling using fibers") {
  ...
  // context switches will still happen before and after locks / interrupt
  // enable/disable / fibers creation
  ...
  fiber_t first = fibers_create(FIBERS_DEFAULT_STACK_SIZE, coop_fibers_func,
                                (void *)&cooperative_counter);
  fiber_t second = fibers_create(FIBERS_DEFAULT_STACK_SIZE, coop_fibers_func,
                                 (void *)&cooperative_counter);
  fiber_t third = fibers_create(FIBERS_DEFAULT_STACK_SIZE, coop_fibers_func,
                                (void *)&cooperative_counter);
  ...
  T_ASSERT_EQ(cooperative_counter, third->id + 1,
              "invalid cooperative schedule");
...
static void *tiny_race_func(void *x) {
  global_var = 42;
  return x;
}

// Standard ThreadSanitizer example in the llvm doc to showcase a race
// TSan will not fail the test by default, you beed to set halt_on_error=1 in
// TSAN_OPTIONS the test will just run fine without TSan, the data race between
// fibers can be detected with the fibers data race checker too
T_DECL(tsan_tiny_race, "tsan_tiny_race") {
  // This sometimes triggers a ThreadSanitizer data race depending on the OS
  // scheduler
  pthread_t thread;
  ...
  fiber_t fiber =
      fibers_create(FIBERS_DEFAULT_STACK_SIZE, tiny_race_func, NULL);
  global_var = 43;
  fibers_join(fiber);
...
T_DECL(mutex_mock_increment_int, "mutex mock test") {
  ...
  for (int i = 0; i < NUM_THREADS; i++) {
    mythreads[i] =
        fibers_create(FIBERS_DEFAULT_STACK_SIZE, increment_counter, (void *)&s);
  }
  ...
  T_ASSERT_EQ(s.counter, (int64_t)(NUM_INCREMENTS * NUM_THREADS),
              "race detected on counter");
