...
#pragma unused(arg)
  ...
  kret = task_set_exception_ports(
      mach_task_self(), EXC_MASK_CRASH | EXC_MASK_CORPSE_NOTIFY, exception_port,
...
T_HELPER_DECL(exc_resource_helper, "exc_resource helper") {
  ...
  for (int i = 0; i < THREAD_LIMIT; i++) {
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(pthread_create(&tid, NULL, dummy_thread, NULL),
                           "pthread_create");
  }
  while (1) {
    sleep(60);
...
T_DECL(exc_resource_threads,
       "Ensures that a process with a thread_limit set will receive an "
       "exc_resource when it crosses its thread limit",
  ...
  T_EXPECT_POSIX_ZERO(posix_spawnattr_set_threadlimit_ext(&attr, THREAD_LIMIT),
                      "posix_spawnattr_set_threadlimit_ext");

  T_EXPECT_POSIX_ZERO(
      posix_spawn(&helper_pid, args[0], NULL, &attr, args, NULL),
      "posix_spawn");

  T_ASSERT_POSIX_ZERO(posix_spawnattr_destroy(&attr),
