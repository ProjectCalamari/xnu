...
static void attempt_kernel_inspection(task_t task) {
  pid_t pid = (pid_t)-1;
  ...
  T_EXPECT_MACH_SUCCESS(
      task_info(task, TASK_BASIC_INFO_64, (task_info_t)&ti, &count),
  ...
  for (i = 0; i < thcnt; i++) {
    ...
    kr = thread_info(threads[i], THREAD_BASIC_INFO, (thread_info_t)&basic_info,
                     &bi_count);
    /*
    ...
    kr = thread_get_exception_ports_info(threads[i], EXC_MASK_ALL, masks,
                                         &msk_count, ports_info, behaviors,
                                         flavors);
    T_QUIET;
    T_EXPECT_MACH_SUCCESS(
        kr, "thread_get_exception_ports_info() on kernel thread: 0x%x", kr);
  ...
  T_EXPECT_MACH_SUCCESS(mach_port_space_basic_info(task, &basic_info),
  ...
  T_EXPECT_MACH_SUCCESS(
      mach_port_space_info(task, &info_space, &table, &tblcnt, &tree, &treecnt),
      "mach_port_space_info");
  if (tblcnt > 0) {
    mach_vm_deallocate(mach_task_self(), (mach_vm_address_t)(uintptr_t)table,
                       tblcnt * sizeof(*table));
  }
  if (treecnt > 0) {
    mach_vm_deallocate(mach_task_self(), (mach_vm_address_t)(uintptr_t)tree,
                       treecnt * sizeof(*tree));
  }
  ...
  exception_behavior_t behaviors[EXC_TYPES_COUNT];
  thread_state_flavor_t flavors[EXC_TYPES_COUNT];
  kern_return_t kr = task_get_exception_ports_info(
      task, EXC_MASK_ALL, masks, &msk_count, ports_info, behaviors, flavors);
  T_EXPECT_MACH_SUCCESS(
      kr, "task_get_exception_ports_info() on kernel_task: 0x%x", kr);

  /* READ (with eval) interfaces should fail */
  vm_offset_t data;
  mach_msg_type_number_t cnt;
  mach_vm_address_t addr = 0x10000000; /* can be whatever, the call should fail
                                          before getting to VM */
  ...
  T_EXPECT_EQ(kr, KERN_INVALID_TASK,
              "task_get_mach_voucher() should fail with KERN_INVALID_TASK");

  /* Control interfaces should absolutely fail */
  kr = task_set_mach_voucher(
      task, mach_task_self()); /* voucher arg is unused, can be whatever port */
  T_EXPECT_EQ(kr, KERN_INVALID_TASK,
              "task_set_mach_voucher() should fail with KERN_INVALID_TASK");
}

T_DECL(inspect_kernel_task, "ensure that kernel task can be inspected",
  ...
  T_ASSERT_MACH_SUCCESS(host_processor_sets(self, &psets, &pcnt), NULL);
  ...
  for (i = 0; i < pcnt; i++) {
    ...
    /* free tasks array */
    mach_vm_deallocate(mach_task_self(), (mach_vm_address_t)(uintptr_t)tasks,
                       tcnt * sizeof(*tasks));

    T_LOG("Attempting kernel inspection with read port...");
    T_ASSERT_MACH_SUCCESS(
        processor_set_tasks_with_flavor(pset, TASK_FLAVOR_READ, &tasks, &tcnt),
        NULL);
  ...
  mach_vm_deallocate(mach_task_self(), (mach_vm_address_t)(uintptr_t)psets,
                     pcnt * sizeof(*psets));
