...
T_DECL(task_fatal_port,
       "Create a child and kill it using the task fatal port") {
  ...
  if (child == 0) {
    ...
    ret = sysctlbyname("machdep.task_get_fatal_port", &old_port_name,
                       &port_name_size, &new_port_name, sizeof(new_port_name));
    if (ret < 0) {
