...
static kern_return_t test_eventlink_create(mach_port_t *port_pair) {
  kern_return_t kr;

  kr =
      mach_eventlink_create(mach_task_self(), MELC_OPTION_NO_COPYIN, port_pair);
  T_ASSERT_MACH_SUCCESS(kr, "mach_eventlink_create");

  return kr;
}

static pthread_t thread_create_for_test(void *(*function)(void *), void *arg) {
  pthread_t pthread;
  pthread_attr_t attr;
...
static void *while1loop(__unused void *arg) {
  while (1) {
    ;
  }
  return NULL;
...
static void *test_eventlink_wait_with_timeout(void *arg) {
  kern_return_t kr;
  ...
  kr = mach_eventlink_associate(eventlink_port, self, 0, 0, 0, 0,
                                MELA_OPTION_NONE);
  ...
  T_EXPECT_MACH_ERROR(kr, KERN_OPERATION_TIMED_OUT,
                      "mach_eventlink_wait_until returned expected error");
  T_EXPECT_EQ(count, (uint64_t)0,
              "mach_eventlink_wait_until returned correct count value");

  return NULL;
}

static void *test_eventlink_wait_no_wait(void *arg) {
  kern_return_t kr;
  mach_port_t eventlink_port = (mach_port_t)(uintptr_t)arg;
  mach_port_t self = mach_thread_self();
  uint64_t count = 1;
...
static void *test_eventlink_wait_destroy(void *arg) {
  kern_return_t kr;
  mach_port_t eventlink_port = (mach_port_t)(uintptr_t)arg;
  mach_port_t self = mach_thread_self();
  uint64_t count = 1;
  ...
  T_EXPECT_MACH_ERROR(kr, KERN_TERMINATED,
                      "mach_eventlink_wait_until returned expected error");

  return NULL;
}

static void *test_eventlink_wait_for_signal(void *arg) {
  kern_return_t kr;
  mach_port_t eventlink_port = (mach_port_t)(uintptr_t)arg;
  mach_port_t self = mach_thread_self();
  uint64_t count = 0;
  ...
  T_EXPECT_EQ(count, (uint64_t)1,
              "mach_eventlink_wait_until returned correct count value");

  return NULL;
}

static void *test_eventlink_wait_then_signal(void *arg) {
  kern_return_t kr;
  mach_port_t eventlink_port = (mach_port_t)(uintptr_t)arg;
  mach_port_t self = mach_thread_self();
  uint64_t count = 0;
...
static void *test_eventlink_wait_then_wait_signal_with_no_wait(void *arg) {
  kern_return_t kr;
  mach_port_t eventlink_port = (mach_port_t)(uintptr_t)arg;
  mach_port_t self = mach_thread_self();
  uint64_t count = 0;
...
static void *test_eventlink_wait_then_wait_signal_with_prepost(void *arg) {
  kern_return_t kr;
  mach_port_t eventlink_port = (mach_port_t)(uintptr_t)arg;
  mach_port_t self = mach_thread_self();
  uint64_t count = 0;
...
static void *test_eventlink_wait_then_signal_loop(void *arg) {
  kern_return_t kr;
  mach_port_t eventlink_port = (mach_port_t)(uintptr_t)arg;
  mach_port_t self = mach_thread_self();
  uint64_t count = 0;
  ...
  if (g_sem_done != SEMAPHORE_NULL) {
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "semaphore_wait(g_sem_done)");
  }
 ...
 */
T_DECL(test_eventlink_create, "eventlink create test", T_META_ASROOT(true),
 ...
 */
T_DECL(test_eventlink_destroy, "eventlink destroy test", T_META_ASROOT(true),
       T_META_RUN_CONCURRENTLY(true), T_META_TAG_VM_PREFERRED) {
  kern_return_t kr;
  mach_port_t port_pair[2];
...
T_DECL(test_eventlink_associate, "eventlink associate test",
       T_META_ASROOT(true), T_META_RUN_CONCURRENTLY(true),
  ...
  kr = mach_eventlink_associate(MACH_PORT_NULL, self, 0, 0, 0, 0,
                                MELA_OPTION_NONE);
  T_EXPECT_MACH_ERROR(
  ...
  T_EXPECT_MACH_ERROR(kr, MACH_SEND_INVALID_DEST,
                      "mach_eventlink_disassociate with null eventlink "
  ...
  for (int i = 0; i < 3; i++) {
    ...
    kr = mach_eventlink_associate(port_pair[0], self, 0, 0, 0, 0,
                                  MELA_OPTION_NONE);
    T_ASSERT_MACH_SUCCESS(kr, "mach_eventlink_associate for object 1");

    kr = mach_eventlink_associate(port_pair[1], other_thread, 0, 0, 0, 0,
                                  MELA_OPTION_NONE);
    T_ASSERT_MACH_SUCCESS(kr, "mach_eventlink_associate for object 2");

    /* Try to associate again with diff threads, expect failure */
    kr = mach_eventlink_associate(port_pair[0], other_thread, 0, 0, 0, 0,
                                  MELA_OPTION_NONE);
    T_EXPECT_MACH_ERROR(kr, KERN_NAME_EXISTS,
                        "mach_eventlink_associate for associated "
                        "objects returned expected error");

    kr = mach_eventlink_associate(port_pair[1], self, 0, 0, 0, 0,
                                  MELA_OPTION_NONE);
    T_EXPECT_MACH_ERROR(kr, KERN_NAME_EXISTS,
                        "mach_eventlink_associate for associated "
 ...
 *
 * Create an eventlink object, associate threads and test eventlink wait with
 * timeout.
 */
T_DECL(test_eventlink_wait_timeout, "eventlink wait timeout test",
  ...
  pthread = thread_create_for_test(test_eventlink_wait_with_timeout,
                                   (void *)(uintptr_t)port_pair[0]);
  sleep(10);

  /* destroy the eventlink object, the wake status of thread will check if the
   * test passsed or failed */
  mach_port_deallocate(mach_task_self(), port_pair[0]);
  mach_port_deallocate(mach_task_self(), port_pair[1]);
 ...
 *
 * Create an eventlink object, associate threads and test eventlink wait with no
 * wait flag.
 */
T_DECL(test_eventlink_wait_no_wait, "eventlink wait no wait test",
  ...
  pthread = thread_create_for_test(test_eventlink_wait_no_wait,
                                   (void *)(uintptr_t)port_pair[0]);
  pthread_join(pthread, NULL);
...
T_DECL(test_eventlink_wait_and_destroy, "eventlink wait and destroy",
  ...
  pthread = thread_create_for_test(test_eventlink_wait_destroy,
                                   (void *)(uintptr_t)port_pair[0]);

  sleep(5);

  /* Increase the send right count for port before destroy to make sure no
   * sender does not fire on destroy */
  kr = mach_port_mod_refs(mach_task_self(), port_pair[0], MACH_PORT_RIGHT_SEND,
                          2);
 ...
 *
 * Create an eventlink object, associate threads, wait and destroy the remote
 * eventlink port.
 */
T_DECL(test_eventlink_wait_and_destroy_remote,
       "eventlink wait and remote destroy", T_META_ASROOT(true),
       T_META_RUN_CONCURRENTLY(true), T_META_TAG_VM_PREFERRED) {
  kern_return_t kr;
  mach_port_t port_pair[2];
  pthread_t pthread;
  ...
  /* Increase the send right count for port before destroy to make sure no
   * sender does not fire on destroy */
  kr = mach_port_mod_refs(mach_task_self(), port_pair[1], MACH_PORT_RIGHT_SEND,
                          2);
  T_ASSERT_MACH_SUCCESS(kr, "mach_port_mod_refs");
 ...
 *
 * Create an eventlink object, associate threads, wait and deallocate the
 * eventlink port.
 */
 ...
 *
 * Create an eventlink object, associate threads, wait and disassociate thread
 * from the eventlink port.
 */
...
T_DECL(test_eventlink_wait_and_signal, "eventlink wait and signal",
  ...
  pthread = thread_create_for_test(test_eventlink_wait_for_signal,
                                   (void *)(uintptr_t)port_pair[0]);
 ...
 */
T_DECL(test_eventlink_wait_signal, "eventlink wait_signal", T_META_ASROOT(true),
       T_META_RUN_CONCURRENTLY(true), T_META_TAG_VM_PREFERRED) {
  kern_return_t kr;
  mach_port_t port_pair[2];
  pthread_t pthread;
  ...
  pthread = thread_create_for_test(test_eventlink_wait_then_signal,
                                   (void *)(uintptr_t)port_pair[0]);
  ...
  /* Wait on the eventlink with timeout */
  kr = mach_eventlink_signal_wait_until(port_pair[1], &count, 0,
                                        MELSW_OPTION_NONE,
                                        KERN_CLOCK_MACH_ABSOLUTE_TIME, 0);

  T_ASSERT_MACH_SUCCESS(kr, "mach_eventlink_signal_wait_until");
  T_EXPECT_EQ(count, (uint64_t)1,
              "mach_eventlink_signal_wait_until returned correct count value");
 ...
 *
 * Create an eventlink object, associate threads and test wait_signal with no
 * wait.
 */
 ...
 *
 * Create an eventlink object, associate threads and test wait_signal with
 * prepost.
 */
 ...
 *
 * Create an eventlink object, set associate on wait on one side and test
 * wait_signal.
 */
T_DECL(test_eventlink_wait_signal_associate_on_wait,
       "eventlink wait_signal associate on wait", T_META_ASROOT(true),
       T_META_RUN_CONCURRENTLY(true), T_META_TAG_VM_PREFERRED) {
  kern_return_t kr;
  mach_port_t port_pair[2];
  pthread_t pthread;
  ...
  kr = mach_eventlink_associate(port_pair[1], MACH_PORT_NULL, 0, 0, 0, 0,
                                MELA_OPTION_ASSOCIATE_ON_WAIT);
  T_ASSERT_MACH_SUCCESS(
      kr, "mach_eventlink_associate with associate on wait for object 2");
...
T_DECL(test_eventlink_wait_signal_loop, "eventlink wait_signal in loop",
  ...
  pthread = thread_create_for_test(test_eventlink_wait_then_signal_loop,
                                   (void *)(uintptr_t)port_pair[0]);
  ...
  for (i = 0; i < g_loop_iterations; i++) {
    ...
    T_QUIET;
    T_EXPECT_EQ(count, (uint64_t)(i + 1),
                "main thread: mach_eventlink_signal_wait_until returned "
                "correct count value");
  }
...
static void *test_suspend_resume_thread(void *arg) {
  ...
  T_QUIET;
  T_EXPECT_MACH_SUCCESS(kr, "semaphore_signal(g_sem_done)");
 ...
 */
T_DECL(test_eventlink_wait_signal_suspend_loop,
       "eventlink wait_signal + thread_suspend/resume in loop",
  ...
  kr = sysctlbyname("kern.mach_eventlink_handoff_success_count",
                    &handoffs_start, &handoffs_start_size, NULL, 0);
  T_QUIET;
  ...
  pthread = thread_create_for_test(test_eventlink_wait_then_signal_loop,
                                   (void *)(uintptr_t)port_pair[0]);
  mach_port_t suspend_resume_other_thread_port =
  ...
  suspend_thread = thread_create_for_test(
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "semaphore_destroy");

  size_t handoffs_end_size = sizeof(handoffs_end);
  kr = sysctlbyname("kern.mach_eventlink_handoff_success_count", &handoffs_end,
                    &handoffs_end_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      kr, "sysctlbyname(kern.mach_eventlink_handoff_success_count)");
  ...
  T_QUIET;
  T_ASSERT_GE(handoffs_end, handoffs_start,
              "kern.mach_eventlink_handoff_success_count did not overflow");
  const uint64_t successful_handoffs = handoffs_end - handoffs_start;
  const uint64_t min_handoffs = MAX(2 * g_loop_iterations, 2) - 2;
  T_EXPECT_GE(successful_handoffs, min_handoffs, "found at least %llu handoffs",
              min_handoffs);
}
