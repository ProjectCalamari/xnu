...
T_DECL(SET_TREATMENT_ID,
       "Verifies that EXPERIMENT sysctls can only be set with the entitlement",
       T_META_ASROOT(false)) {
...
#define IDENTIFIER_LENGTH 36
  ...
  ret = sysctlbyname("kern.trial_treatment_id", val, &len, new_val,
                     strlen(new_val));
  err = errno;
#if ENTITLED
  len = sizeof(val);
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "reset kern.trial_treatment_id");
#else
...
T_DECL(experiment_factor_numeric_limits,
       "Can only set factors within the legal range.", T_META_ASROOT(false)) {
...
#define kMaxVal 10 /* The max value allowed for the testing factor. */
  ...
  new_val = kMinVal - 1;
  ret = sysctlbyname("kern.testing_experiment_factor", &current_val, &len,
                     &new_val, sizeof(new_val));
  err = errno;
  T_ASSERT_POSIX_FAILURE(ret, EINVAL,
  ...
  T_ASSERT_POSIX_SUCCESS(
      ret, "set kern.testing_experiment_factor at bottom of range.");
}

static uint64_t original_libmalloc_experiment_value = 0;

static void reset_libmalloc_experiment(void) {
  int ret = sysctlbyname("kern.libmalloc_experiments", NULL, NULL,
                         &original_libmalloc_experiment_value,
                         sizeof(original_libmalloc_experiment_value));
  T_ASSERT_POSIX_SUCCESS(ret, "reset kern.libmalloc_experiments");
}

static void set_libmalloc_experiment(uint64_t val) {
  T_LOG("Setting kern.libmalloc_experiments to %llu", val);
  size_t len = sizeof(original_libmalloc_experiment_value);
  int ret = sysctlbyname("kern.libmalloc_experiments",
                         &original_libmalloc_experiment_value, &len, &val,
                         sizeof(val));
 ...
 */
static char **get_apple_array(size_t *num_array_entries, const char *filename) {
  if (filename == NULL) {
  ...
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(stdout_f, "open(%s)", stdout_path);
 ...
 */
static bool get_apple_array_key(char **apple_array, size_t num_array_entries,
                                uint64_t *factors, const char *key) {
  bool found = false;
...
T_DECL(libmalloc_hardened_browser_present,
  ...
  uint32_t mask_val = BrowserHostEntitlementMask | BrowserGPUEntitlementMask |
                      BrowserWebContentEntitlementMask;
  apple_array =
      get_apple_array(&num_array_entries, "tools/print_apple_array_HR1");
  found = get_apple_array_key(apple_array, num_array_entries, &apple_array_val,
                              HARDENED_RUNTIME_KEY);
  ...
  apple_array =
      get_apple_array(&num_array_entries, "tools/print_apple_array_HR2");
  found = get_apple_array_key(apple_array, num_array_entries, &apple_array_val,
                              HARDENED_RUNTIME_KEY);
  T_ASSERT_TRUE(found, "Found " HARDENED_RUNTIME_KEY " in apple array");
  T_ASSERT_EQ(apple_array_val, mask_val, "Bitmask value matches");
...
T_DECL(libmalloc_security_config_hardened_heap_entitlements,
       "parse security_config values to verify security configs hardened_heap "
  ...
  apple_array = get_apple_array(&num_array_entries,
                                "tools/print_apple_array_hardened_proc");
  found = get_apple_array_key(apple_array, num_array_entries, &apple_array_val,
                              SECURITY_CONFIG_KEY);
  ...
  T_EXPECT_FALSE(apple_array_val & SECURITY_CONFIG_HARDENED_HEAP_ENTRY,
                 "Hardened-heap is disabled");
  free(apple_array);

  apple_array = get_apple_array(&num_array_entries,
                                "tools/print_apple_array_hardened_heap");
  found = get_apple_array_key(apple_array, num_array_entries, &apple_array_val,
                              SECURITY_CONFIG_KEY);
  T_ASSERT_TRUE(found, "Found " SECURITY_CONFIG_KEY " in apple array");

  T_EXPECT_TRUE(apple_array_val & SECURITY_CONFIG_HARDENED_HEAP_ENTRY,
                "Hardened-heap is enabled");
  free(apple_array);

  /* Verify that the same config is mirrored with the com.apple.security
   * namespace */
  apple_array = get_apple_array(
      &num_array_entries, "tools/print_apple_array_hardened_heap_security");
  found = get_apple_array_key(apple_array, num_array_entries, &apple_array_val,
                              SECURITY_CONFIG_KEY);
  T_ASSERT_TRUE(found, "Found " SECURITY_CONFIG_KEY " in apple array");
...
T_DECL(libmalloc_hardened_browser_absent,
       "platform restrictions binary flags do not show up in apple array for "
       "normal third party processes",
       T_META_ASROOT(false)) {
  ...
  bool found = false;
  apple_array =
      get_apple_array(&num_array_entries, NULL); // todo apple_array_3p?
  found = get_apple_array_key(apple_array, num_array_entries, &apple_array_val,
                              HARDENED_RUNTIME_KEY);
...
T_DECL(libmalloc_experiment,
       "libmalloc experiment flags show up in apple array if we're doing an "
       "experiment",
       T_META_ASROOT(false)) {
  uint64_t new_val, apple_array_val = 0;
  size_t num_array_entries = 0;
  ...
  found = get_apple_array_key(apple_array, num_array_entries, &apple_array_val,
                              LIBMALLOC_EXPERIMENT_FACTORS_KEY);
  T_ASSERT_TRUE(found,
                "Found " LIBMALLOC_EXPERIMENT_FACTORS_KEY " in apple array");
  T_ASSERT_EQ(apple_array_val, new_val, "Experiment value matches");
  free(apple_array);
}

T_DECL(libmalloc_experiment_not_in_array,
       "libmalloc experiment flags do not show up in apple array if we're not "
       "doing an experiment",
       T_META_ASROOT(false)) {
  size_t num_array_entries = 0;
  char **apple_array;
  ...
  apple_array = get_apple_array(&num_array_entries, NULL);
  found = get_apple_array_key(apple_array, num_array_entries, NULL,
                              LIBMALLOC_EXPERIMENT_FACTORS_KEY);
  T_ASSERT_TRUE(!found, "Did not find " LIBMALLOC_EXPERIMENT_FACTORS_KEY
                        " in apple array");
  free(apple_array);
}
#endif /* ENTITLED */
