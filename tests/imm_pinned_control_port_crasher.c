 ...
 * expected to generate fatal EXC_GUARD.
 *
 * The type of exception raised (if any) is checked on control_port_options
 * side.
 */
...
static int attempt_send_immovable_port(mach_port_name_t port,
  ...
  kr = mach_port_insert_right(mach_task_self(), server, server,
...
static void pinned_test_main_thread_mod_ref(void) {
  printf("[Crasher]: Mod refs main thread's self port to 0\n");
  mach_port_t thread_self = mach_thread_self();
  kern_return_t kr = mach_port_mod_refs(mach_task_self(), thread_self,
                                        MACH_PORT_RIGHT_SEND, -2);

  printf("[Crasher pinned_test_main_thread_mod_ref] mach_port_mod_refs "
         "returned %s \n.",
         mach_error_string(kr));
}

static void *_Nullable pthread_run(void *_Nullable) {
  ...
  printf("[Crasher pinned_test_pthread_dealloc] mach_port_deallocate returned "
         "%s \n.",
         mach_error_string(kr));
  return NULL;
}

static void pinned_test_pthread_dealloc(void) {
...
static void pinned_test_task_self_dealloc(void) {
  ...
  printf("[Crasher pinned_test_task_self_dealloc] mach_port_deallocate "
         "returned %s \n.",
         mach_error_string(kr));
}

static void pinned_test_task_self_mod_ref(void) {
  printf("[Crasher]: Mod refs mach_task_self() to 0\n");
  kern_return_t kr = mach_port_mod_refs(mach_task_self(), mach_task_self(),
                                        MACH_PORT_RIGHT_SEND, -3);

  printf("[Crasher pinned_test_task_self_mod_ref] mach_port_mod_refs returned "
         "%s \n.",
         mach_error_string(kr));
}

static void pinned_test_task_threads_mod_ref(void) {
  printf("[Crasher]: task_threads should return pinned thread ports. Mod refs "
         "them to 0\n");
  ...
  kr = mach_port_mod_refs(mach_task_self(), th_list[0], MACH_PORT_RIGHT_SEND,
                          -1);

  printf("[Crasher pinned_test_task_threads_mod_ref] mach_port_mod_refs "
         "returned %s \n.",
         mach_error_string(kr));
}

static void pinned_test_mach_port_destroy(void) {
  kern_return_t kr = mach_port_destroy(mach_task_self(), mach_task_self());
  printf("[Crasher pinned_test_mach_port_destroy] mach_port_destroy returned "
         "%s \n.",
         mach_error_string(kr));
}

static void pinned_test_move_send_as_remote_port(void) {
  struct {
    mach_msg_header_t header;
  } msg;
  ...
  /*
   * We allow move send on remote kobject port but this should trip on pinning
   * on last ref. See: IPC_OBJECT_COPYIN_FLAGS_ALLOW_IMMOVABLE_SEND.
   */
  ...
  printf("[Crasher pinned_test_move_send_as_remote_port] mach_msg_send "
         "returned %s \n.",
         mach_error_string(kr));
}

static void immovable_test_move_send_as_remote_port(void) {
  struct {
    mach_msg_header_t header;
  } msg;
  ...
  printf("[Crasher immovable_test_move_send_as_remote_port] mach_msg_send "
         "returned %s \n.",
         mach_error_string(kr));
}

static void immovable_test_move_send_task_self(void) {
  kern_return_t kr;
  printf("[Crasher]: Move send mach_task_self_\n");
  kr = attempt_send_immovable_port(mach_task_self(), MACH_MSG_TYPE_MOVE_SEND);

  printf("[Crasher immovable_test_move_send_task_self] "
         "attempt_send_immovable_port returned %s \n.",
         mach_error_string(kr));
}

static void immovable_test_copy_send_task_self(void) {
  kern_return_t kr;
  printf("[Crasher]: Copy send mach_task_self_\n");
  kr = attempt_send_immovable_port(mach_task_self(), MACH_MSG_TYPE_COPY_SEND);

  printf("[Crasher immovable_test_copy_send_task_self] "
         "attempt_send_immovable_port returned %s \n.",
         mach_error_string(kr));
}

static void immovable_test_move_send_thread_self(void) {
  kern_return_t kr;
  printf("[Crasher]: Move send main thread's self port\n");
  kr = attempt_send_immovable_port(mach_thread_self(), MACH_MSG_TYPE_MOVE_SEND);

  printf("[Crasher immovable_test_move_send_thread_self] "
         "attempt_send_immovable_port returned %s \n.",
         mach_error_string(kr));
}

static void immovable_test_copy_send_thread_self(void) {
  kern_return_t kr;
  mach_port_t port;
  ...
  printf("[Crasher immovable_test_copy_send_thread_self] "
         "attempt_send_immovable_port returned %s \n.",
         mach_error_string(kr));
...
static void immovable_test_copy_send_task_read(void) {
  kern_return_t kr;
  mach_port_t port;
  ...
  assert(kr == 0);
  kr = attempt_send_immovable_port(port, MACH_MSG_TYPE_COPY_SEND);
  printf("[Crasher immovable_test_copy_send_task_read] "
         "attempt_send_immovable_port returned %s \n.",
         mach_error_string(kr));
...
static void immovable_test_copy_send_task_inspect(void) {
  kern_return_t kr;
  mach_port_t port;
  ...
  printf("[Crasher immovable_test_copy_send_task_inspect] "
         "attempt_send_immovable_port returned %s \n.",
         mach_error_string(kr));
}

static void immovable_test_move_send_thread_inspect(void) {
  kern_return_t kr;
  mach_port_t port;
  ...
  assert(kr == 0);
  kr = attempt_send_immovable_port(port, MACH_MSG_TYPE_MOVE_SEND);
  printf("[Crasher immovable_test_move_send_thread_inspect] "
         "attempt_send_immovable_port returned %s \n.",
         mach_error_string(kr));
...
static void immovable_test_move_send_raw_thread(void) {
  kern_return_t kr;
  mach_port_t port;
  ...
  kr = mach_port_deallocate(mach_task_self(),
                            port); /* not pinned, should not crash */

  kr = thread_create(mach_task_self(), &port);
  assert(kr == 0);
  kr = attempt_send_immovable_port(
      port, MACH_MSG_TYPE_MOVE_SEND); /* immovable, should crash here */
  printf("[Crasher immovable_test_move_send_raw_thread] "
         "attempt_send_immovable_port returned %s \n.",
         mach_error_string(kr));
...
static void immovable_test_copy_send_thread_read(void) {
  kern_return_t kr;
  mach_port_t port;
  mach_port_t th_port = mach_thread_self();
  ...
  assert(kr == 0);
  kr = attempt_send_immovable_port(port, MACH_MSG_TYPE_COPY_SEND);
  printf("[Crasher immovable_test_copy_send_thread_read] "
         "attempt_send_immovable_port returned %s \n.",
         mach_error_string(kr));
...
static void cfi_test_no_bit_set(void) {
  ...
  kr = mach_msg2(&header, MACH64_SEND_MSG, header, header.msgh_size, 0,
...
static void cfi_test_two_bits_set(void) {
  ...
  kr = mach_msg2(&header,
                 MACH64_SEND_MSG | MACH64_SEND_ANY | MACH64_SEND_KOBJECT_CALL,
                 header, header.msgh_size, 0, MACH_PORT_NULL, 0,
                 MACH_MSG_PRIORITY_UNSPECIFIED);
  /* crash */
  printf("[Crasher cfi_test_two_bits_set]: mach_msg2() returned %d\n", kr);
}

static void cfi_test_msg_to_timer_port(void) {
  ...
  kr = mach_msg2(&msg, MACH64_SEND_MSG | MACH64_SEND_MQ_CALL, msg.header,
                 msg.header.msgh_size, 0, MACH_PORT_NULL, 0,
                 MACH_MSG_PRIORITY_UNSPECIFIED);
  assert(kr == KERN_SUCCESS);
  printf("Message sent to timer port successfully\n");

  /* Using MACH64_SEND_KOBJECT_CALL should crash */
  kr = mach_msg2(&msg, MACH64_SEND_MSG | MACH64_SEND_KOBJECT_CALL, msg.header,
                 msg.header.msgh_size, 0, MACH_PORT_NULL, 0,
                 MACH_MSG_PRIORITY_UNSPECIFIED);
  /* crash */
...
static void cfi_test_wrong_bit_set(void) {
  ...
  kr = mach_msg2(&header, MACH64_SEND_MSG | MACH64_SEND_MQ_CALL, header,
                 header.msgh_size, 0, MACH_PORT_NULL, 0,
                 MACH_MSG_PRIORITY_UNSPECIFIED);
  /* crash */
...
int main(int argc, char *argv[]) {
  ...
  bool third_party_hardened =
      !strcmp(argv[0], "imm_pinned_control_port_crasher_3P_hardened");
  if (third_party_hardened) {
    ...
    if (remove_platform_binary() != 0) {
       ...
       * fail depending on global/mac policies of the BATS container (ref:
       * csops_internal). Skip instead of failing the test.
       */
