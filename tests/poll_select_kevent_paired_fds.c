...
T_GLOBAL_META(T_META_NAMESPACE("xnu.kevent"), T_META_CHECK_LEAKS(false),
...
static mach_timespec_t READ_SETUP_timeout = {.tv_sec = READ_SETUP_TIMEOUT_SECS,
                                             .tv_nsec = 0};
static mach_timespec_t READ_timeout = {.tv_sec = READ_TIMEOUT_SECS,
                                       .tv_nsec = 0};
static mach_timespec_t WRITE_timeout = {.tv_sec = WRITE_TIMEOUT_SECS,
                                        .tv_nsec = 0};

enum fd_pair { PTY_PAIR, FIFO_PAIR, PIPE_PAIR, SOCKET_PAIR };
...
static void wake_writer(void) {
  ...
  case PROCESS_WRITER: {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(write(shared.wr_wait.in_fd, &tmp, 1), NULL);
    break;
  }
  }
}
...
static void writer_wait(void) {
  ...
  case THREAD_WRITER:
    ...
    kern_return_t kret =
        semaphore_timedwait(shared.wr_wait.sem, READ_SETUP_timeout);

    if (kret == KERN_OPERATION_TIMED_OUT) {
      T_ASSERT_FAIL("THREAD_WRITER semaphore timedout after %d seconds",
                    READ_SETUP_timeout.tv_sec);
    }
    T_QUIET;
  ...
  case PROCESS_WRITER: {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(read(shared.wr_wait.out_fd, &tmp, 1), NULL);
    break;
  }
  }
...
static bool handle_writing(enum fd_pair __unused fd_pair, int fd) {
  static unsigned int cur_char = 0;
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(write(fd, &(EXPECTED_STRING[cur_char]), 1), NULL);
  cur_char++;
...
static void reenable_workq(int fd, int16_t filt) {
  struct kevent_qos_s events[] = {
      {.ident = (uint64_t)fd,
       .filter = filt,
       .flags = EV_ENABLE | EV_UDATA_SPECIFIC | EV_DISPATCH,
       .qos = (int32_t)_pthread_qos_class_encode(EXPECTED_QOS, 0, 0),
       .fflags = NOTE_LOWAT,
       .data = 1}};
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kev, "reenable workq in kevent_qos");
}

static void workqueue_write_fn(void **__unused buf, int *__unused count) {
  // T_MAYFAIL;
...
static void workqueue_fn(pthread_priority_t __unused priority) {
...
static void drive_kq(bool reading, union mode mode, enum fd_pair fd_pair,
  ...
  EV_SET(&events, fd, reading ? EVFILT_READ : EVFILT_WRITE, EV_ADD, NOTE_LOWAT,
         1, NULL);
  ...
  int kq_fd = kqueue();
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kq_fd, "kqueue");
  ...
  for (;;) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(kev, "kevent");
    T_QUIET;
    T_ASSERT_NE(kev, 0, "kevent timed out");
...
static void *write_to_fd(void *__unused ctx) {
  ssize_t bytes_wr = 0;
  ...
  case FULL_WRITE:
    do {
      ...
      bytes_wr = write(shared.wr_fd, EXPECTED_STRING, EXPECTED_LEN);
    } while (bytes_wr == -1 && errno == EINTR);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(bytes_wr, "write");
    T_QUIET;
    T_ASSERT_EQ(bytes_wr, (ssize_t)EXPECTED_LEN, "wrote enough bytes");
    break;

  case INCREMENTAL_WRITE:
    for (unsigned int i = 0; i < EXPECTED_LEN; i++) {
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(write(shared.wr_fd, &(EXPECTED_STRING[i]), 1),
                             NULL);
  ...
  case WORKQ_INCREMENTAL_WRITE: {
    ...
    T_ASSERT_MACH_SUCCESS(semaphore_create(mach_task_self(),
                                           &shared.wr_finished,
                                           SYNC_POLICY_FIFO, 0),
                          "semaphore_create shared.wr_finished");

    T_QUIET;
    T_ASSERT_NE_UINT(shared.wr_finished, (unsigned)MACH_PORT_NULL,
                     "wr_finished semaphore_create");

    T_QUIET;
    T_ASSERT_POSIX_ZERO(_pthread_workqueue_init_with_kevent(
                            workqueue_fn, workqueue_write_fn, 0, 0),
                        NULL);
    ...
    for (;;) {
      int kev =
          kevent_qos(-1, changes == 0 ? NULL : events, changes, events, 1, NULL,
                     NULL, KEVENT_FLAG_WORKQ | KEVENT_FLAG_ERROR_EVENTS);
      if (kev == -1 && errno == EINTR) {
        changes = 0;
      ...
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(kev, "kevent_qos");
      break;
    }
    break;
  }
  ...
  if (shared.wr_finished) {
    ...
    if (kret == KERN_OPERATION_TIMED_OUT) {
      T_ASSERT_FAIL("write side semaphore timedout after %d seconds",
                    WRITE_timeout.tv_sec);
    }
    T_QUIET;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(close(shared.wr_fd), NULL);
  return NULL;
}
...
static bool handle_reading(enum fd_pair fd_pair, int fd) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(bytes_rd, "reading from file");
  T_QUIET;
  T_ASSERT_LE(bytes_rd, (ssize_t)EXPECTED_LEN, "read too much from file");
...
static void workqueue_read_fn(void **__unused buf, int *__unused count) {
  // T_MAYFAIL;
  // T_QUIET; T_ASSERT_EFFECTIVE_QOS_EQ(EXPECTED_QOS,
...
static void read_from_fd(int fd, enum fd_pair fd_pair, enum read_mode mode) {
  int fd_flags;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(fd_flags, "fcntl(F_GETFL)");

  if (!(fd_flags & O_NONBLOCK)) {
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(fcntl(fd, F_SETFL, fd_flags | O_NONBLOCK), NULL);
  }
  ...
  case POLL_READ: {
    ...
    for (;;) {
      ...
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(pol, "poll");
      T_QUIET;
      T_ASSERT_NE(pol, 0,
      ...
      T_QUIET;
      T_ASSERT_FALSE(fds[0].revents & POLLERR,
                     "should not see an error on the device");
      T_QUIET;
      T_ASSERT_FALSE(fds[0].revents & POLLNVAL,
  ...
  case SELECT_READ:
    ...
    for (;;) {
      ...
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(sel, "select");

      T_QUIET;
      T_ASSERT_NE(sel, 0, "select waited for %d seconds and timed out",
                  READ_TIMEOUT_SECS);

      /* didn't fail or time out, therefore data is ready */
      T_QUIET;
      T_ASSERT_NE(FD_ISSET(fd, &read_fd), 0,
  ...
  case WORKQ_READ: {
    ...
    _dispatch_prohibit_transition_to_multithreaded(true);
    T_ASSERT_POSIX_ZERO(_pthread_workqueue_init_with_kevent(
                            workqueue_fn, workqueue_read_fn, 0, 0),
                        NULL);
    ...
    T_QUIET;
    T_ASSERT_NE_UINT(shared.rd_finished, (unsigned)MACH_PORT_NULL,
                     "semaphore_create");
  ...
  case DISPATCH_READ: {
    ...
    read_src = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, (uintptr_t)fd,
                                      0, NULL);
    T_QUIET;
...
static void fd_pair_init(enum fd_pair fd_pair, int *rd_fd, int *wr_fd) {
  switch (fd_pair) {
  case PTY_PAIR:
    T_ASSERT_POSIX_SUCCESS(openpty(rd_fd, wr_fd, NULL, NULL, NULL), NULL);
    break;

  case FIFO_PAIR: {
    char fifo_path[] = "/tmp/async-io-fifo.XXXXXX";
    T_QUIET;
    T_ASSERT_NOTNULL(mktemp(fifo_path), NULL);
    ...
    *rd_fd = open(fifo_path, O_RDONLY | O_NONBLOCK);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(*rd_fd, "open(... O_RDONLY)");
    *wr_fd = open(fifo_path, O_WRONLY | O_NONBLOCK);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(*wr_fd, "open(... O_WRONLY)");
    break;
  }
  ...
  case SOCKET_PAIR: {
    int sock_fds[2];
    T_ASSERT_POSIX_SUCCESS(socketpair(AF_UNIX, SOCK_STREAM, 0, sock_fds), NULL);
    *rd_fd = sock_fds[0];
    *wr_fd = sock_fds[1];
    break;
  }
  ...
  T_QUIET;
  T_ASSERT_NE(*rd_fd, -1, "reading descriptor");
  T_QUIET;
  T_ASSERT_NE(*wr_fd, -1, "writing descriptor");
}

#pragma mark single process

static void drive_threads(enum fd_pair fd_pair, enum read_mode rd_mode,
                          enum write_mode wr_mode) {
  pthread_t thread;
  ...
  T_ASSERT_MACH_SUCCESS(semaphore_create(mach_task_self(), &shared.wr_wait.sem,
                                         SYNC_POLICY_FIFO, 0),
                        "semaphore_create shared.wr_wait.sem");

  T_QUIET;
  T_ASSERT_POSIX_ZERO(pthread_create(&thread, NULL, write_to_fd, NULL), NULL);
...
static void __attribute__((noreturn)) drive_processes(enum fd_pair fd_pair,
                                                      enum read_mode rd_mode,
                                                      enum write_mode wr_mode) {
  shared.fd_pair = fd_pair;
  shared.rd_mode = rd_mode;
  ...
  int fds[2];
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(pipe(fds), NULL);
  shared.wr_wait.out_fd = fds[0];
  shared.wr_wait.in_fd = fds[1];

  T_LOG("starting subprocesses");
  dt_helper_t helpers[2] = {dt_fork_helper("reader_helper"),
...
#define WR_DECL_PROCESSES(desc_name, fd_pair, write_name, write_str,           \
  ...
  T_DECL(desc_name##_r##read_name##_w##write_name##_procs,                     \
         "read changes to a " #desc_name " with " #read_name                   \
         " and writing " #write_str " across two processes",                   \
         T_META_TAG_VM_PREFERRED) {                                            \
    drive_processes(fd_pair, read_mode, write_mode);                           \
  }
#define WR_DECL_THREADS(desc_name, fd_pair, write_name, write_str, write_mode, \
                        read_name, read_mode)                                  \
  T_DECL(desc_name##_r##read_name##_w##write_name##_thds,                      \
         "read changes to a " #desc_name " with " #read_name                   \
         " and writing " #write_str,                                           \
         T_META_TAG_VM_PREFERRED) {                                            \
...
#define WR_DECL(desc_name, fd_pair, write_name, write_str, write_mode,         \
                read_name, read_mode)                                          \
  WR_DECL_PROCESSES(desc_name, fd_pair, write_name, write_str, write_mode,     \
                    read_name, read_mode)                                      \
  WR_DECL_THREADS(desc_name, fd_pair, write_name, write_str, write_mode,       \
                  read_name, read_mode)

#define RD_DECL_SAFE(desc_name, fd_pair, read_name, read_mode)                 \
  WR_DECL(desc_name, fd_pair, full, "the full string", FULL_WRITE, read_name,  \
          read_mode)                                                           \
  WR_DECL(desc_name, fd_pair, inc, "incrementally", INCREMENTAL_WRITE,         \
          read_name, read_mode)
...
#define RD_DECL_WORKQ_ONLY(suffix, desc_name, fd_pair, read_name, read_mode)   \
  WR_DECL##suffix(desc_name, fd_pair, inc_workq,                               \
                  "incrementally with the workqueue", WORKQ_INCREMENTAL_WRITE, \
                  read_name, read_mode)
...
#define RD_DECL_DISPATCH(desc_name, fd_pair, read_name, read_mode)             \
  RD_DECL_SAFE(desc_name, fd_pair, read_name, read_mode)                       \
  RD_DECL_DISPATCH_ONLY(, desc_name, fd_pair, read_name, read_mode)            \
  RD_DECL_WORKQ_ONLY(_PROCESSES, desc_name, fd_pair, read_name, read_mode)
 ...
 *               RD_DECL_SAFE(desc_name, fd_pair, read_name, read_mode) \
 *               RD_DECL_DISPATCH_ONLY(_PROCESSES, desc_name, fd_pair,
 read_name, \
 *                               read_mode) \
