...
static void handler(int sig, siginfo_t *sip, __unused void *uconp) {
  /* Should handle the SIGCHLD signal */
  T_ASSERT_EQ_INT(sig, SIGCHLD,
  ...
  T_ASSERT_EQ_INT(sip->si_code, CLD_EXITED,
                  "si_code returns 0x%x, expected CLD_EXITED (0x%x).",
                  sip->si_code, CLD_EXITED);
  T_ASSERT_EQ_INT(
      sip->si_status, exitcode,
      "si_status returns 0x%08X, expected the child's exit code (0x%08X).",
      sip->si_status, exitcode);

  T_END;
}

T_DECL(sigchldreturn,
       "checks that a child process exited with an exitcode returns correctly "
       "to parent",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sigaction(SIGCHLD, &act, NULL),
                         "Calling sigaction() failed for SIGCHLD");

  /* Now fork a child that just exits */
  pid = fork();
  T_QUIET;
  T_ASSERT_NE_INT(pid, -1, "fork() failed!");
...
T_DECL(sigabrt_test,
       "check that child process' exitcode contains signum = SIGABRT",
       T_META_CHECK_LEAKS(false), T_META_TAG_VM_PREFERRED) {
  int ret;
  siginfo_t siginfo;
  pid_t pid = fork();
...
T_DECL(sigkill_test,
       "check that child process' exitcode contains signum = SIGKILL",
       T_META_CHECK_LEAKS(false), T_META_TAG_VM_PREFERRED) {
  int ret;
  siginfo_t siginfo;
  pid_t pid = fork();
...
T_DECL(sigchild_posix_spawn_fail,
       "check SIGCHLD is correctly delivered when posix_spawn fails",
       T_META_CHECK_LEAKS(false), T_META_TAG_VM_PREFERRED) {
  struct sigaction act;
  int pid;
  char *args[4];
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(posix_spawn(&pid, "/bin/sh", NULL, NULL, args, NULL),
                         "posix_spawn failed");

  for (int i = 0; i < 500; i++) {
    int ret = posix_spawn(&pid, "does not exist", NULL, NULL, args, NULL);
    T_QUIET;
    T_ASSERT_EQ(ret, ENOENT, "posix_spawn should fail with ENOENT");
  }
