...
void set_expects_may_fail(bool may_fail) { may_fail_status = may_fail; }

bool expects_may_fail(void) { return may_fail_status; }
...
bool has_user_system_times(void) {
  ...
  dispatch_once(&user_system_once, ^{
    ...
    T_ASSERT_POSIX_SUCCESS(sysctlbyname("kern.precise_user_kernel_time",
                                        &precise_times_int, &precise_times_size,
                                        NULL, 0),
...
bool has_cpi(void) {
  ...
  dispatch_once(&cpi_once, ^{
    ...
    T_SETUPBEGIN;
    int ret =
        sysctlbyname("kern.monotonic.supported", &cpi, &cpi_size, NULL, 0);
...
bool has_energy(void) {
  ...
  dispatch_once(&energy_once, ^{
    ...
    T_ASSERT_POSIX_SUCCESS(
        sysctlbyname("kern.pervasive_energy", &energy, &energy_size, NULL, 0),
...
static const char **_perf_level_names(void) {
  ...
  dispatch_once(&names_once, ^{
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      snprintf(sysctl_name, sizeof(sysctl_name), "hw.perflevel%d.name", i);
      T_QUIET;
...
char *sched_policy_name(void) {
  ...
  dispatch_once(&policy_name_once, ^{
    ...
    T_ASSERT_POSIX_SUCCESS(sysctlbyname("kern.sched", &policy_name,
                                        &(size_t){sizeof(policy_name)}, NULL,
                                        0),
                           "kern.sched");
    T_SETUPEND;
  });
...
static void _unbind_from_cluster(void) {
  // Best effort.
...
static void _run_on_exclaves(void) {
  int64_t output = 0;
...
uint64_t ns_from_time_value(struct time_value tv) {
  return (uint64_t)tv.seconds * NSEC_PER_SEC + (uint64_t)tv.microseconds * 1000;
}
...
static void *spin_role(void *arg) {
...
static void *wait_role(void *arg) {
...
struct scene *scene_start(unsigned int n, role_t *roles) {
  ...
  kern_return_t kr =
      semaphore_create(mach_task_self(), &ws.ws_wait, SYNC_POLICY_FIFO, 0);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "semaphore_create(... wait ...)");
  kr = semaphore_create(mach_task_self(), &ws.ws_start, SYNC_POLICY_FIFO, 0);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "semaphore_create(... start ...)");
...
pid_t launch_helper(char *name) {
  ...
  pid_t pid = 0;
  ret =
      dt_launch_tool(&pid, (char *[]){bin_path, name, NULL}, false, NULL, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "dt_launch_tool(... %s, %s ...)", bin_path, name);
  T_SETUPEND;
