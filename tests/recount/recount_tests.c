...
T_GLOBAL_META(T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_OWNER("mwidmann"), T_META_CHECK_LEAKS(false));

static void proc_pidtaskinfo_increasing(pid_t pid, struct proc_taskinfo *last,
                                        const char *desc) {
  ...
  T_LOG("%s: usr = %llu, sys = %llu, th_usr = %llu, th_sys = %llu, "
        "term_usr = %llu, term_sys = %llu",
        name, info.pti_total_user, info.pti_total_system, info.pti_threads_user,
        info.pti_threads_system, info.pti_total_user - info.pti_threads_user,
...
static void thread_start(struct usage_thread *th, const char *name,
                         enum usage_style style) {
  th->ut_style = style;
  th->ut_name = name;
  th->ut_arg = 1;
  T_SETUPBEGIN;
  int error = pthread_create(&th->ut_thread, NULL,
                             style == USAGE_SPIN ? spin_thread : sleep_thread,
                             &th->ut_arg);
  T_QUIET;
...
static void thread_end(struct usage_thread *th) {
  th->ut_arg = 0;
  T_SETUPBEGIN;
...
static void _assert_increasing(struct usr_sys_times *before,
                               struct usr_sys_times *after, const char *name,
                               const char *desc) {
...
static void test_usr_sys_time_sanity(struct usr_sys_times (*fn)(pid_t),
                                     const char *name) {
...
static void _get_proc_pid_rusage(pid_t pid, struct rusage_info_v6 *info) {
  T_SETUPBEGIN;
...
static struct usr_sys_times proc_pid_rusage_times(pid_t pid) {
...
static struct usr_sys_times task_basic_info_times(pid_t __unused pid) {
...
static struct usr_sys_times task_power_info_times(pid_t __unused pid) {
...
static struct usr_sys_times task_absolutetime_info_times(pid_t __unused pid) {
...
static struct usr_sys_times getrusage_times(pid_t __unused pid) {
...
T_DECL(proc_pid_rusage_perf_levels,
       ...
       REQUIRE_RECOUNT_PMCS, REQUIRE_MULTIPLE_PERF_LEVELS,
       SET_THREAD_BIND_BOOTARG, T_META_TAG_VM_NOT_ELIGIBLE) {
  T_QUIET;
  T_ASSERT_GT(perf_level_count(), 1, "Platform should be AMP");
  ...
  T_EXPECT_GE(after.ri_user_time, before.ri_user_time, "user_time increasing");
...
T_DECL(proc_pid_rusage_secure_perf_levels,
       "ensure proc_pid_rusage fills in per-perf level information",
       REQUIRE_RECOUNT_PMCS, REQUIRE_MULTIPLE_PERF_LEVELS, REQUIRE_EXCLAVES,
       SET_THREAD_BIND_BOOTARG, T_META_TAG_VM_PREFERRED) {
  int status = 0;
  ...
  T_EXPECT_GT(after.ri_secure_ptime_in_system, before.ri_secure_ptime_in_system,
              "secure time in system on P-cores increasing");

  uint64_t system_time_delta = after.ri_system_time - before.ri_system_time;
  uint64_t secure_time_delta =
      after.ri_secure_time_in_system - before.ri_secure_time_in_system;
  T_EXPECT_LE(secure_time_delta, system_time_delta,
              "secure time is less than system time");
  uint64_t system_ptime_delta = after.ri_system_ptime - before.ri_system_ptime;
  uint64_t secure_ptime_delta =
      after.ri_secure_ptime_in_system - before.ri_secure_ptime_in_system;
...
_proc_pidthreadcounts_increasing(struct proc_threadcounts_data *before,
                                 struct proc_threadcounts_data *after,
                                 const char *perf_level) {
  const char *name = "PROC_PIDTHREADCOUNTS";
  T_LOG("%s %s before: usr = %llu, sys = %llu, instrs = %llu, cycles = %llu, "
        "energy = %llu",
        name, perf_level, before->ptcd_user_time_mach,
  ...
  T_LOG("%s %s after: usr = %llu, sys = %llu, instrs = %llu, cycles = %llu, "
        "energy = %llu",
        name, perf_level, after->ptcd_user_time_mach,
  ...
  T_EXPECT_NE(before->ptcd_cycles, 0ULL, "%s cycles should be non-zero",
              perf_level);
...
static void _threadcounts_to_rusage_info(struct proc_threadcounts_data *counts,
...
static void _rusage_info_le(struct rusage_info_v6 *lhs, const char *lhs_name,
                            struct rusage_info_v6 *rhs, const char *rhs_name) {
  T_EXPECT_LE(lhs->ri_user_time, rhs->ri_user_time, "%s user time <= %s",
              lhs_name, rhs_name);
  T_EXPECT_LE(lhs->ri_system_time, rhs->ri_system_time, "%s system time <= %s",
              lhs_name, rhs_name);
  T_EXPECT_LE(lhs->ri_instructions, rhs->ri_instructions,
              "%s instructions <= %s", lhs_name, rhs_name);
  T_EXPECT_LE(lhs->ri_cycles, rhs->ri_cycles, "%s cycles <= %s", lhs_name,
              rhs_name);
  T_EXPECT_LE(lhs->ri_energy_nj, rhs->ri_energy_nj, "%s energy <= %s", lhs_name,
              rhs_name);
  ...
  T_EXPECT_LE(lhs->ri_pcycles, rhs->ri_pcycles, "%s P-core cycles <= %s",
              lhs_name, rhs_name);
  T_EXPECT_LE(lhs->ri_penergy_nj, rhs->ri_penergy_nj, "%s energy <= %s",
              lhs_name, rhs_name);
}
...
static void *_thread_runs_on_perf_levels(void *vsequence) {
...
T_DECL(proc_pidthreadcounts_sanity,
       ...
       REQUIRE_RECOUNT_PMCS, SET_THREAD_BIND_BOOTARG,
  ...
  int error =
      pthread_create(&target_thread, NULL, _thread_runs_on_perf_levels, &seq);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(error, "pthread_create");
  error = pthread_threadid_np(target_thread, &target_tid);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(error, "pthread_threadid_np");
...
T_DECL(proc_pidthreadcounts_invalid_tid,
       ...
       REQUIRE_RECOUNT_PMCS, T_META_ASROOT(true), T_META_TAG_VM_PREFERRED) {
  T_SETUPBEGIN;
  unsigned int level_count = perf_level_count();
  int counts_size =
      (int)sizeof(struct proc_threadcounts) +
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(counts, "allocate counts");
  T_SETUPEND;
...
static void *_thread_spin_and_exit(void *__unused arg) {
...
static uint64_t _rusage_to_time_us(struct rusage *usage) {
  return usage->ru_utime.tv_sec * USEC_PER_SEC + usage->ru_utime.tv_usec;
}

T_DECL(getrusage_thread_terminate_increasing,
       "check that getrusage(2) is monotonically increasing, even with threads "
       "terminating",
       T_META_TAG_VM_PREFERRED) {
  ...
  while (clock_gettime_nsec_np(CLOCK_MONOTONIC) < end_ns) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(getrusage(RUSAGE_SELF, &usage), NULL);
  ...
  T_PASS(
      "checked getrusage(2) times for %llu second%s while threads terminated",
