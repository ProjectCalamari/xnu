...
T_GLOBAL_META(T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("RM"), T_META_OWNER("mwidmann"),
              T_META_CHECK_LEAKS(false), T_META_TAG_PERF);
...
static int next_bit(uint32_t *bits) {
  if (*bits == 0) {
    return -1;
  }
...
static bool measures_stable(struct measure *measures) {
...
static void interface_perf_test(struct usage_scenario *s) {
  ...
  for (int metric = next_bit(&metrics); metric >= 0;
    ...
    case METRIC_SYS_TIME:
      measures[metric].m_stat = (dt_stat_t)dt_stat_time_create(
          "%s_%s", s->us_stat_name, metric_names[metric]);
      break;
    case METRIC_INSNS:
    case METRIC_CYCLES:
      measures[metric].m_stat = dt_stat_create(
          metric_units[metric], "%s_%s", s->us_stat_name, metric_names[metric]);
      break;
    default:
...
static void interface_scaling_test(struct usage_scenario *s) {
  ...
  for (unsigned int i = 0; i < ARRAY_COUNT(thread_counts); i++) {
    ...
    struct scene *scene = scene_start(n - 1, (role_t[]){
                                                 ROLE_WAIT,
                                                 ROLE_NONE,
                                             });
    T_LOG("%u threads", n);
...
static void proc_pidtaskinfo_usage(void *ctx) {
  struct proc_taskinfo before = {0};
  struct proc_taskinfo after = {0};
  pid_t pid = getpid();
  int before_ret =
      proc_pidinfo(pid, PROC_PIDTASKINFO, 0, &before, sizeof(before));
  int after_ret = proc_pidinfo(pid, PROC_PIDTASKINFO, 0, &after, sizeof(after));
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(after_ret, "proc_pidinfo(..., PROC_PIDTASKINFO, ...)");
  T_SETUPEND;

  struct proc_taskinfo *delta = ctx;
  delta->pti_total_user =
      ns_from_mach(after.pti_total_user - before.pti_total_user);
  delta->pti_total_system =
      ns_from_mach(after.pti_total_system - before.pti_total_system);
}

static double proc_pidtaskinfo_measurement(metrics_t metric, void *ctx) {
...
static void proc_pid_rusage_usage(void *ctx) {
  struct rusage_info_v5 before = {0};
  struct rusage_info_v5 after = {0};
  pid_t pid = getpid();
  int before_ret =
      proc_pid_rusage(pid, RUSAGE_INFO_V5, (rusage_info_t *)&before);
  int after_ret = proc_pid_rusage(pid, RUSAGE_INFO_V5, (rusage_info_t *)&after);
  ...
  delta->ri_user_time = ns_from_mach(after.ri_user_time - before.ri_user_time);
  delta->ri_system_time =
      ns_from_mach(after.ri_system_time - before.ri_system_time);
  delta->ri_cycles = after.ri_cycles - before.ri_cycles;
  delta->ri_instructions = after.ri_instructions - before.ri_instructions;
}

static double proc_pid_rusage_measurement(metrics_t metric, void *ctx) {
...
static double getrusage_measurement(metrics_t metric, void *ctx) {
  struct rusage *usage = ctx;
  switch (metric) {
  case METRIC_CPU_TIME:
    return (double)(ns_from_timeval(usage->ru_utime) +
                    ns_from_timeval(usage->ru_stime)) /
           1e9 / RUSAGE_ITERS;
  case METRIC_SYS_TIME:
...
static void getrusage_usage(void *ctx) {
...
static void thread_selfcounts_usage(void *ctx) {
  ...
  counts->ttci_user_time_mach =
      after.ttci_user_time_mach - before.ttci_user_time_mach;
  counts->ttci_system_time_mach =
      after.ttci_system_time_mach - before.ttci_system_time_mach;
  counts->ttci_instructions =
      after.ttci_instructions - before.ttci_instructions;
  counts->ttci_cycles = after.ttci_cycles - before.ttci_cycles;
}

static double thread_selfcounts_measurement(metrics_t metric, void *ctx) {
  struct thsc_time_cpi *counts = ctx;
  switch (metric) {
  case METRIC_CPU_TIME:
...
static void thread_selfusage_usage(void *ctx) {
...
static double thread_selfusage_measurement(metrics_t metric, void *ctx) {
  uint64_t *delta = ctx;
...
static void task_power_info_usage(void *ctx) {
...
static double task_power_info_measurement(metrics_t metric, void *ctx) {
  struct task_power_info *info = ctx;
  uint64_t ns = 0;
  switch (metric) {
  case METRIC_CPU_TIME:
...
static void thread_basic_info_usage(void *ctx) {
  ...
  for (int i = 0; i < THREAD_BASIC_INFO_ITERS; i++) {
    ...
    thread_info(mach_thread_self(), THREAD_BASIC_INFO, (thread_info_t)&after,
                &info_count);
  }
  info_count = THREAD_BASIC_INFO_COUNT;
  ...
  info->system_time =
      time_value_from_ns(ns_from_time_value(after.system_time) -
                         ns_from_time_value(before.system_time));
}

static double thread_basic_info_measurement(metrics_t metric, void *ctx) {
  struct thread_basic_info *info = ctx;
  uint64_t ns = 0;
  switch (metric) {
  case METRIC_CPU_TIME:
...
static void task_inspect_basic_counts_usage(void *ctx) {
  ...
  kern_return_t before_kr =
      task_inspect(mach_task_self(), TASK_INSPECT_BASIC_COUNTS,
                   (task_inspect_info_t)&before, &info_count);
  info_count = TASK_INSPECT_BASIC_COUNTS_COUNT;
  kern_return_t after_kr =
      task_inspect(mach_task_self(), TASK_INSPECT_BASIC_COUNTS,
                   (task_inspect_info_t)&after, &info_count);
...
static double task_inspect_basic_counts_measurement(metrics_t metric,
                                                    void *ctx) {
  struct task_inspect_basic_counts *counts = ctx;
  switch (metric) {
  case METRIC_INSNS:
...
static void task_absolutetime_info_usage(void *ctx) {
  ...
  kern_return_t before_kr = task_info(mach_task_self(), TASK_ABSOLUTETIME_INFO,
                                      (task_info_t)&before, &info_count);
  info_count = TASK_ABSOLUTETIME_INFO_COUNT;
  kern_return_t after_kr = task_info(mach_task_self(), TASK_ABSOLUTETIME_INFO,
                                     (task_info_t)&after, &info_count);
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(after_kr, "task_info(... TASK_ABSOLUTETIME_INFO ...)");
  T_SETUPEND;
...
static double task_absolutetime_info_measurement(metrics_t metric, void *ctx) {
  task_absolutetime_info_data_t *counts = ctx;
  switch (metric) {
  case METRIC_CPU_TIME:
  ...
  case METRIC_CYCLES:
  default:
    T_ASSERT_FAIL("unsupported metric %d for task_absolutetime_info", metric);
  }
}
...
T_DECL(task_usage_perf, "measure the performance of task usage interfaces",
  ...
  struct usage_scenario ppr_scenario = {
      ...
      .us_context = &rui,
      .us_metrics =
          METRIC_INSNS | METRIC_CYCLES | METRIC_CPU_TIME | METRIC_SYS_TIME,
  };
