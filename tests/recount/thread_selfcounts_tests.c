...
static void _check_cpi(struct thsc_cpi *before, struct thsc_cpi *after,
                       const char *name) {
  T_QUIET;
  ...
  T_QUIET;
  T_MAYFAIL_IF_ENABLED(amp_fail_reason);
  T_EXPECT_GT(before->tcpi_cycles, UINT64_C(0), "%s: cycles non-zero", name);
...
static void _check_no_cpi(struct thsc_cpi *before, struct thsc_cpi *after,
                          const char *name) {
  T_MAYFAIL_IF_ENABLED(amp_fail_reason);
...
static struct thsc_cpi _remove_time_from_cpi(struct thsc_time_cpi *time_cpi) {
...
static void _check_time_cpi(struct thsc_time_cpi *before,
                            struct thsc_time_cpi *after, const char *name) {
...
static void _check_no_time_cpi(struct thsc_time_cpi *before,
                               struct thsc_time_cpi *after, const char *name) {
  struct thsc_cpi before_cpi = _remove_time_from_cpi(before);
  struct thsc_cpi after_cpi = _remove_time_from_cpi(after);
...
static void _check_usage(struct thsc_time_energy_cpi *before,
...
static void _check_no_usage(struct thsc_time_energy_cpi *before,
...
T_DECL(thread_selfcounts_perf_level_sanity,
       "check per-perf level time, energy, and CPI", REQUIRE_RECOUNT_PMCS,
       REQUIRE_MULTIPLE_PERF_LEVELS, SET_THREAD_BIND_BOOTARG,
       T_META_ASROOT(true), T_META_TAG_VM_NOT_ELIGIBLE) {
  unsigned int level_count = perf_level_count();
  T_QUIET;
  T_ASSERT_GT(level_count, 1, "Platform should be AMP");
  ...
  T_ASSERT_POSIX_ZERO(
      err, "thread_selfcounts(THSC_TIME_ENERGY_CPI_PER_PERF_LEVEL, ...)");
...
static void _expect_counts_on_perf_level(unsigned int perf_level_index,
...
T_DECL(thread_selfcounts_perf_level_correct,
       ...
       SET_THREAD_BIND_BOOTARG, T_META_ASROOT(true),
  ...
  for (unsigned int i = 0; i < level_count; i++) {
    ...
    if (!is_edge_scheduler && (perf_level_name(i)[0] == 'P')) {
      T_QUIET;
      T_EXPECT_EQ_STR(sched_policy_name(), "amp",
                      "Unexpected multicluster scheduling policy");
...
T_DECL(thread_selfcounts_cpi_perf,
  ...
  T_SETUPBEGIN;
  dt_stat_t instrs =
      dt_stat_create("thread_selfcounts_cpi_instrs", "instructions");
  ...
  while (!dt_stat_stable(instrs) || !dt_stat_stable(cycles)) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_ZERO(r1, "thread_selfcounts(THSC_CPI, ...)");
    T_QUIET;
    T_ASSERT_POSIX_ZERO(r2, "thread_selfcounts(THSC_CPI, ...)");

    dt_stat_add(instrs,
                counts[1].tcpi_instructions - counts[0].tcpi_instructions);
