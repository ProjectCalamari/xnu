...
T_GLOBAL_META(T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_RUN_CONCURRENTLY(true), T_META_ASROOT(true),
...
// In particular, S3_2_C15_C1_0 (instructions retired) seems to be zero on some
// devices (rdar://143157256).
static void skip_if_fastsim(void) {
  char buffer[64] = "";
...
T_DECL(coalition_resource_info_counters,
  ...
  ret =
      coalition_info_resource_usage(resid, &coalusage[0], sizeof(coalusage[0]));
  ...
  T_EXPECT_GT(coalusage[0].cpu_cycles, UINT64_C(0), "cycle count is non-zero");

  sleep(1);

  T_SETUPBEGIN;
  ret =
      coalition_info_resource_usage(resid, &coalusage[1], sizeof(coalusage[1]));
  T_ASSERT_POSIX_SUCCESS(ret, "coalition_info_resource_usage()");
  T_SETUPEND;
...
T_DECL(
    coalition_resource_info_kernel_ptime_sane,
    "ensure that coalition resource info for the kernel has a sane P-CPU time",
    T_META_TAG_VM_PREFERRED) {
  T_SETUPBEGIN;
  struct proc_pidcoalitioninfo idinfo = {};
  int ret = proc_pidinfo(0, PROC_PIDCOALITIONINFO, 0, &idinfo, sizeof(idinfo));
  T_ASSERT_POSIX_SUCCESS(ret, "proc_pidinfo(... PROC_PIDCOALITIONINFO ...)");

  uint64_t resid = idinfo.coalition_id[COALITION_TYPE_RESOURCE];

  struct coalition_resource_usage coalusage = {};
  ret = coalition_info_resource_usage(resid, &coalusage, sizeof(coalusage));
  T_ASSERT_POSIX_SUCCESS(ret, "coalition_info_resource_usage()");
  T_SETUPEND;
