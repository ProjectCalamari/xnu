...
#else
static void log_hexdump(const void *inp, size_t len) {
#pragma unused(inp, len)
}
#endif

static bool is_netagent_enabled(void) {
  int enabled = 0;
  size_t len = sizeof(enabled);
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(sysctlbyname("net.link.generic.system.enable_netagent",
                                   &enabled, &len, NULL, 0),
                      NULL);
  T_QUIET;
  T_ASSERT_EQ(len, sizeof(enabled), NULL);
  return enabled == 1;
}
...
static int create_tunsock(int enable_netif, int enable_flowswitch,
                          int channel_count, uuid_t uuid[]);

static void setup_ipsec_test(void) {
...
static void setup_utun_test(void) {
...
static bool setblocking(int s, bool blocking) {
  ...
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(flags = fcntl(s, F_GETFL, 0), NULL);
  ...
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(flags = fcntl(s, F_SETFL, flags), NULL);

  return ret;
}

static void check_enables(int tunsock, int enable_netif, int enable_flowswitch,
                          int channel_count, uuid_t uuid[]) {
  int scratch;
  ...
  if (g_is_ipsec_test && channel_count && !enable_netif) {
    /* Unfortunately, the connect incorrectly unwinds the bind if it get an
     * error. until that is fixed, expect EINVAL here
     */
    scratchlen = sizeof(scratch);
    T_QUIET;
    T_EXPECT_POSIX_FAILURE(getsockopt(tunsock, SYSPROTO_CONTROL,
                                      g_OPT_ENABLE_NETIF, &scratch,
                                      &scratchlen),
                           EINVAL, NULL);
    T_QUIET;
    T_EXPECT_POSIX_FAILURE(getsockopt(tunsock, SYSPROTO_CONTROL,
                                      g_OPT_ENABLE_FLOWSWITCH, &scratch,
                                      &scratchlen),
                           EINVAL, NULL);
    T_QUIET;
    T_EXPECT_POSIX_FAILURE(getsockopt(tunsock, SYSPROTO_CONTROL,
                                      g_OPT_ENABLE_CHANNEL, &scratch,
                                      &scratchlen),
                           EINVAL, NULL);
    ...
    T_QUIET;
    T_EXPECT_POSIX_FAILURE(getsockopt(tunsock, SYSPROTO_CONTROL,
                                      g_OPT_GET_CHANNEL_UUID, uuid, &uuidlen),
                           EINVAL, NULL);
    for (int i = 0; i < channel_count; i++) {
      T_QUIET;
      T_EXPECT_TRUE(uuid_is_null(uuid[i]), NULL);
    }
    return;
  }
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(getsockopt(tunsock, SYSPROTO_CONTROL, g_OPT_ENABLE_NETIF,
                                 &scratch, &scratchlen),
                      NULL);
  T_QUIET;
  T_EXPECT_EQ_ULONG((unsigned long)scratchlen, sizeof(scratch), NULL);
  T_QUIET;
  T_EXPECT_EQ(scratch, enable_netif, NULL);

  scratchlen = sizeof(scratch);
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(getsockopt(tunsock, SYSPROTO_CONTROL,
                                 g_OPT_ENABLE_FLOWSWITCH, &scratch,
                                 &scratchlen),
                      NULL);
  T_QUIET;
  ...
  if (is_netagent_enabled()) {
    if (enable_netif) {
      T_QUIET;
      T_EXPECT_EQ(scratch, enable_flowswitch, NULL);
    } else {
      T_QUIET;
      T_EXPECT_EQ(scratch, 0, NULL);
    }
  } else {
    T_QUIET;
    T_EXPECT_EQ(scratch, 0, NULL);
  ...
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(getsockopt(tunsock, SYSPROTO_CONTROL,
                                 g_OPT_ENABLE_CHANNEL, &scratch, &scratchlen),
                      NULL);
  T_QUIET;
  T_EXPECT_EQ_ULONG((unsigned long)scratchlen, sizeof(scratch), NULL);
  if (g_is_ipsec_test && !enable_netif) {
    ...
    T_QUIET;
    T_EXPECT_EQ(scratch, (int)channel_count, NULL);
  }
  ...
  if (scratch) {
    ...
    T_EXPECT_POSIX_ZERO(getsockopt(tunsock, SYSPROTO_CONTROL,
                        ...
                        NULL);
    T_QUIET;
    T_EXPECT_EQ_ULONG((unsigned long)uuidlen,
                      sizeof(uuid_t) * (unsigned long)channel_count, NULL);
    for (int i = 0; i < channel_count; i++) {
      T_QUIET;
      T_EXPECT_FALSE(uuid_is_null(uuid[i]), NULL);
    }
  } else {
    for (int i = 0; i < channel_count; i++) {
      uuid_clear(uuid[i]);
...
static void tunsock_get_ifname(int s, char ifname[IFXNAMSIZ]) {
  socklen_t optlen = IFXNAMSIZ;
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_POSIX_ZERO(
      getsockopt(s, SYSPROTO_CONTROL, g_OPT_IFNAME, ifname, &optlen), NULL);
  T_QUIET;
  T_ASSERT_TRUE(optlen > 0, NULL);
  T_QUIET;
  T_ASSERT_TRUE(ifname[optlen - 1] == '\0', NULL);
  T_QUIET;
  T_ASSERT_TRUE(strlen(ifname) + 1 == optlen,
                "got ifname \"%s\" len %zd expected %u", ifname, strlen(ifname),
                optlen);
}

static short ifnet_get_flags(int s, const char ifname[IFNAMSIZ]) {
  struct ifreq ifr;
  memset(&ifr, 0, sizeof(ifr));
  strlcpy(ifr.ifr_name, ifname, sizeof(ifr.ifr_name));
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(ioctl(s, SIOCGIFFLAGS, (caddr_t)&ifr), NULL);
  return ifr.ifr_flags;
}

static void ifnet_add_addr4(const char ifname[IFNAMSIZ], struct in_addr *addr,
  ...
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(s = socket(AF_INET, SOCK_DGRAM, 0), NULL);
  ...
  if (broadaddr != NULL ||
      (addr != NULL && (ifnet_get_flags(s, ifname) & IFF_POINTOPOINT) != 0)) {
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(ioctl(s, SIOCAIFADDR, &ifra), NULL);

  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(close(s), NULL);
}

static int g_pfkeyso = -1;
static struct in_addr g_addr1, g_addr2;

static void create_sa(const char ifname[IFXNAMSIZ], uint8_t type, uint32_t spi,
                      struct in_addr *src, struct in_addr *dst) {
  if (g_pfkeyso == -1) {
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(g_pfkeyso = socket(PF_KEY, SOCK_RAW, PF_KEY_V2),
                           NULL);
  }
  ...
  ssize_t slen;
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(slen = send(g_pfkeyso, &addcmd, sizeof(addcmd), 0),
                         NULL);
  T_QUIET;
  T_EXPECT_EQ(slen, (ssize_t)sizeof(addcmd), NULL);
}
...
startover:
  ...
  T_EXPECT_POSIX_SUCCESS(
      tunsock = socket(PF_SYSTEM, SOCK_DGRAM, SYSPROTO_CONTROL), NULL);

  memset(&kernctl_info, 0, sizeof(kernctl_info));
  strlcpy(kernctl_info.ctl_name, g_CONTROL_NAME, sizeof(kernctl_info.ctl_name));
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(ioctl(tunsock, CTLIOCGINFO, &kernctl_info), NULL);
  ...
  T_QUIET;
  T_EXPECT_POSIX_FAILURE(setsockopt(tunsock, SYSPROTO_CONTROL,
                                    g_OPT_ENABLE_NETIF, &enable_netif,
                                    sizeof(enable_netif)),
                         EINVAL, NULL);
  T_QUIET;
  T_EXPECT_POSIX_FAILURE(setsockopt(tunsock, SYSPROTO_CONTROL,
                                    g_OPT_ENABLE_FLOWSWITCH, &enable_flowswitch,
                                    sizeof(enable_flowswitch)),
                         EINVAL, NULL);
  T_QUIET;
  T_EXPECT_POSIX_FAILURE(setsockopt(tunsock, SYSPROTO_CONTROL,
                                    g_OPT_ENABLE_CHANNEL, &channel_count,
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(setsockopt(tunsock, SYSPROTO_CONTROL, g_OPT_ENABLE_NETIF,
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(setsockopt(tunsock, SYSPROTO_CONTROL,
                                 g_OPT_ENABLE_CHANNEL, &channel_count,
                                 sizeof(channel_count)),
                      NULL);
  ...
  int error =
      connect(tunsock, (struct sockaddr *)&kernctl_addr, sizeof(kernctl_addr));
  if (error == -1 && errno == EBUSY) {
    /* XXX remove this retry nonsense when this is fixed:
     * <rdar://problem/37340313> creating an interface without specifying
     * specific interface name should not return EBUSY
     */
    close(tunsock);
  ...
  if (g_is_ipsec_test && channel_count && !enable_netif) {
    ...
    T_QUIET;
    T_EXPECT_POSIX_FAILURE(error, ENOTSUP,
                           "connect() == -1 && errno == ENOTSUP");
  } else {
    T_QUIET;
    T_WITH_ERRNO;
    T_EXPECT_POSIX_ZERO(error, "connect() == 0");
  }
  ...
  if (g_is_ipsec_test && channel_count && !enable_netif) {
    ...
    T_EXPECT_POSIX_FAILURE(
        setsockopt(tunsock, SYSPROTO_CONTROL, g_OPT_ENABLE_FLOWSWITCH,
                   &enable_flowswitch, sizeof(enable_flowswitch)),
        EINVAL, NULL);
  } else {
    if (is_netagent_enabled()) {
      if (enable_netif) {
...
static void permute_enables(void) {
  int tunsock;
  T_EXPECT_GE(tunsock = create_tunsock(false, false, false, NULL), 0, NULL);
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(close(tunsock), NULL);
...
T_DECL(ipsec_enables,
       "This test checks combinations of netif/channel/flowswitch on ipsec",
...
T_DECL(utun_enables,
       "This test checks combinations of netif/channel/flowswitch on utun",
       T_META_TAG_VM_PREFERRED) {
  setup_utun_test();
  permute_enables();
}
...
static void cleanup_tunsock(void) {
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(close(g_tunsock), NULL);
  T_QUIET;
  T_EXPECT_POSIX_FAILURE(close(g_tunsock), EBADF, NULL);
  if (g_is_ipsec_test) {
    T_QUIET;
    T_WITH_ERRNO;
    T_EXPECT_POSIX_ZERO(close(g_pfkeyso), NULL);
    T_QUIET;
    T_EXPECT_POSIX_FAILURE(close(g_pfkeyso), EBADF, NULL);
  }
}

static void setup_tunsock(int channel_count, uuid_t uuids[]) {
  T_ASSERT_GE(g_tunsock = create_tunsock(true, false, channel_count, uuids), 0,
              NULL);
...
static void setup_channels_and_rings(int kq, int channel_count,
                                     channel_t channels[],
                                     channel_ring_t rxrings[],
                                     channel_ring_t txrings[], uuid_t uuids[],
                                     int cfds[]) {
  ...
  for (int ri = 0; ri < channel_count; ri++) {
    if (rxrings) {
      T_QUIET;
      T_WITH_ERRNO;
      T_ASSERT_NOTNULL(channels[ri] = os_channel_create(uuids[ri], 0), NULL);
      T_QUIET;
      T_WITH_ERRNO;
      T_ASSERT_NOTNULL(
          rxrings[ri] = os_channel_rx_ring(
              channels[ri],
              os_channel_ring_id(channels[ri], CHANNEL_FIRST_RX_RING)),
          NULL);
    }
    if (txrings) {
      T_QUIET;
      ...
      T_ASSERT_NOTNULL(
          rxrings[ri] = os_channel_rx_ring(
              channels[ri],
              os_channel_ring_id(channels[ri], CHANNEL_FIRST_TX_RING)),
          NULL);
    }

    struct kevent kev;
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(cfds[ri] = os_channel_get_fd(channels[ri]), NULL);
    EV_SET(&kev, cfds[ri], EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0,
           (void *)(uintptr_t)ri);
    T_QUIET;
    T_WITH_ERRNO;
    T_EXPECT_POSIX_ZERO(kevent(kq, &kev, 1, NULL, 0, NULL), NULL);
  }
}

static void cleanup_channels_and_rings(int channel_count, channel_t channels[],
                                       channel_ring_t rxrings[],
                                       channel_ring_t txrings[],
                                       uuid_t uuids[]) {
  for (int ri = 0; ri < channel_count; ri++) {
    if (rxrings) {
      rxrings[ri] = NULL;
...
static void setup_sockets(int sockets[SO_TC_MAX], int type) {
  for (int si = 0; si < SO_TC_MAX; si++) {
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(sockets[si] = socket(PF_INET, type, 0), NULL);

    T_QUIET;
    T_WITH_ERRNO;
    T_EXPECT_POSIX_ZERO(setsockopt(sockets[si], SOL_SOCKET, SO_TRAFFIC_CLASS,
                                   &SOCKET_TRAFFIC_CLASSES[si],
                                   sizeof(SOCKET_TRAFFIC_CLASSES[si])),
                        NULL);
    ...
    T_QUIET;
    T_WITH_ERRNO;
    T_EXPECT_POSIX_ZERO(bind(sockets[si], (struct sockaddr *)&sin, sizeof(sin)),
                        NULL);
...
static void cleanup_sockets(int sockets[SO_TC_MAX]) {
  for (int si = 0; si < SO_TC_MAX; si++) {
    T_QUIET;
    T_WITH_ERRNO;
    T_EXPECT_POSIX_ZERO(close(sockets[si]), NULL);
    sockets[si] = -1;
  }
}

static void drain_ring(channel_ring_t rxring) {
  ...
  for (i = 0; i < sc; i++) {
    ...
    T_QUIET;
    T_WITH_ERRNO;
    T_EXPECT_NOTNULL(rxslot = os_channel_get_next_slot(rxring, rxprev, &rxprop),
                     NULL);
    T_QUIET;
    T_ASSERT_NE_UINT(0, rxprop.sp_len, NULL);
    T_QUIET;
    T_ASSERT_NOTNULL((void *)rxprop.sp_buf_ptr, NULL);
  ...
  if (sc) {
    T_QUIET;
    T_EXPECT_POSIX_ZERO(os_channel_advance_slot(rxring, rxprev), NULL);
  }
}

static void send_one_packet(int s, int type) {
  struct sockaddr_in sin;
  memset(&sin, 0, sizeof(sin));
  sin.sin_len = sizeof(sin);
  sin.sin_family = AF_INET;
  ...
  if (type == SOCK_STREAM) {
    T_QUIET;
    T_EXPECT_POSIX_FAILURE(connect(s, (struct sockaddr *)&sin, sizeof(sin)),
                           EINPROGRESS, NULL);
  }
  if (type == SOCK_DGRAM) {
    T_QUIET;
    ...
    T_EXPECT_EQ_LONG(
        (long)sizeof(s),
        sendto(s, &s, sizeof(s), 0, (struct sockaddr *)&sin, sizeof(sin)),
        NULL);
  }
}

static void expect_empty_rings(int channel_count, channel_ring_t rings[]) {
  /* Check all the rings and make sure there are no packets */
  for (int ri = 0; ri < channel_count; ri++) {
    T_QUIET;
    T_EXPECT_EQ_UINT(0U, os_channel_available_slot_count(rings[ri]), NULL);
  }
}

static void xfer_1_packet_singly(int channel_count, int type) {
  ...
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(kq = kqueue(), NULL);

  setup_channels_and_rings(kq, channel_count, channels, rxrings, NULL, uuids,
                           cfds);
  ...
  for (int si = 0; si < SO_TC_MAX; si++) {
    ...
    /* Wait for the packet delivery and check that it's only one packet and on
     * the correct ring */
    ...
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(
        nev = kevent(kq, NULL, 0, kev, channel_count + 1, &to), NULL);
    T_QUIET;
    T_EXPECT_EQ_INT(nev, 1, NULL);
    T_QUIET;
    T_EXPECT_EQ_PTR((void *)kev[0].ident,
                    (void *)(uintptr_t)cfds[expected_ring], NULL);
    T_QUIET;
    T_EXPECT_EQ_PTR(kev[0].udata, (void *)(uintptr_t)expected_ring, NULL);
    T_QUIET;
    T_EXPECT_EQ_SHORT(kev[0].filter, (short)EVFILT_READ, NULL);
    T_QUIET;
    T_EXPECT_FALSE(kev[0].flags & EV_ERROR, NULL);
    ...
    for (int ri = 0; ri < channel_count; ri++) {
       ...
       */
      T_QUIET;
      T_EXPECT_EQ_UINT(ri == expected_ring, sc, NULL);

      if ((ri == expected_ring) == sc) {
        T_PASS("tc index %d ring %d expected ring %d slot count %u", si, ri,
               expected_ring, sc);
      } else {
        T_FAIL("tc index %d ring %d expected ring %d slot count %u", si, ri,
               expected_ring, sc);
      }
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(close(kq), NULL);
}

T_DECL(ipsec35889979u1s,
       "transfers 1 packet at a time of each sevice class over udp to a single "
       "ring",
       T_META_TAG_VM_PREFERRED) {
  setup_ipsec_test();
...
T_DECL(ipsec35889979u4s,
       "transfers 1 packet at a time of each sevice class over udp to 4 rings",
       T_META_TAG_VM_PREFERRED) {
  setup_ipsec_test();
  xfer_1_packet_singly(4, SOCK_DGRAM);
}

T_DECL(ipsec35889979t1s,
       "transfers 1 packet at a time of each sevice class over tcp to a single "
       "ring",
       T_META_TAG_VM_PREFERRED) {
  setup_ipsec_test();
...
T_DECL(ipsec35889979t4s,
       "transfers 1 packet at a time of each sevice class over tcp to 4 rings",
...
static void xfer_1_packet_together(int channel_count, int type) {
  uuid_t uuids[channel_count];
  channel_t channels[channel_count];
  int sockets[SO_TC_MAX];
  channel_ring_t rxrings[channel_count];
  ...
  memset(kev, 0, sizeof(kev));
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(nev = kevent(kq, NULL, 0, kev, channel_count + 1, &to),
                         NULL);
  T_QUIET;
  T_EXPECT_EQ_INT(nev, channel_count, NULL);

  uint32_t found[channel_count];
  memset(found, 0, sizeof(found));
  for (int e = 0; e < nev; e++) {
    T_LOG("kevent %lu filter 0x%4x flags 0x%04x fflags 0x%08x data %" PRIdPTR
          " udata %p",
          kev[e].ident, kev[e].filter, kev[e].flags, kev[e].fflags, kev[e].data,
          kev[e].udata);

    T_QUIET;
    T_ASSERT_GE_PTR(kev[e].udata, (void *)0, NULL);
    T_QUIET;
    T_ASSERT_LT_PTR(kev[e].udata, (void *)(intptr_t)channel_count, NULL);
    int ri = (int)kev[e].udata;
    T_QUIET;
    T_EXPECT_EQ_UINT(found[ri], 0U, NULL);

    T_QUIET;
    T_EXPECT_EQ_ULONG(kev[e].ident, (uintptr_t)cfds[ri], NULL);
    T_QUIET;
    T_EXPECT_EQ_SHORT(kev[e].filter, (short)EVFILT_READ, NULL);
    T_QUIET;
    T_EXPECT_FALSE(kev[e].flags & EV_ERROR, NULL);

    if (channel_count == 1) {
      T_QUIET;
      T_EXPECT_EQ_LONG(kev[e].data, (long)SO_TC_MAX, NULL);
    } else {
      T_QUIET;
      T_EXPECT_EQ_LONG(kev[e].data, (long)RING_TO_TC_COUNT[ri], NULL);
    }
  ...
  for (int ri = 0; ri < channel_count; ri++) {
    T_QUIET;
    T_EXPECT_NE_UINT(found[ri], 0U, NULL);
  }
  ...
  for (int ri = 0; ri < channel_count; ri++) {
    ...
    if (channel_count == 1) {
      if (sc == SO_TC_MAX) {
        T_PASS("ring %d got %" PRIu32 " slots expecting %" PRIu32 "", ri, sc,
               SO_TC_MAX);
      } else {
        T_FAIL("ring %d got %" PRIu32 " slots expecting %" PRIu32 "", ri, sc,
               SO_TC_MAX);
      }
    } else {
      if (sc == (uint32_t)RING_TO_TC_COUNT[ri]) {
        T_PASS("ring %d got %" PRIu32 " slots expecting %" PRIu32 "", ri, sc,
               (uint32_t)RING_TO_TC_COUNT[ri]);
      } else {
        T_FAIL("ring %d got %" PRIu32 " slots expecting %" PRIu32 "", ri, sc,
               (uint32_t)RING_TO_TC_COUNT[ri]);
      }
...
T_DECL(ipsec35889979u1m,
       "transfers 1 packet together of each sevice class over udp to a single "
       "ring",
       T_META_TAG_VM_PREFERRED) {
  setup_ipsec_test();
...
T_DECL(ipsec35889979u4m,
       "transfers 1 packet together of each sevice class over udp to 4 rings",
       T_META_TAG_VM_PREFERRED) {
  setup_ipsec_test();
  xfer_1_packet_together(4, SOCK_DGRAM);
}

T_DECL(ipsec35889979t1m,
       "transfers 1 packet together of each sevice class over tcp to a single "
       "ring",
       T_META_TAG_VM_PREFERRED) {
  setup_ipsec_test();
...
T_DECL(ipsec35889979t4m,
       "transfers 1 packet together of each sevice class over tcp to 4 rings",
       /* This test will fail because tcp syn packets get elevated
        * due to ack prioritization
        */
       T_META_ENABLED(false), T_META_TAG_VM_PREFERRED) {
