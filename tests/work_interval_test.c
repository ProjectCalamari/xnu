...
static void *joining_thread_fn(__unused void *arg) {
  int ret = 0;
...
T_DECL(work_interval, "work interval interface") {
  ...
  ret = work_interval_create(&handle, WORK_INTERVAL_FLAG_JOINABLE |
  ...
  ret = work_interval_join_port(port);
  T_ASSERT_POSIX_SUCCESS(
      ret, "work_interval_join_port, join before handing to another thread");

  pthread_t joining_thread;

  T_ASSERT_POSIX_ZERO(
      pthread_create(&joining_thread, NULL, joining_thread_fn, NULL),
      "pthread_create");
...
T_DECL(work_interval_audio_realtime_only,
...
T_DECL(work_interval_get_flags, "querying a port for create flags") {
  ...
  T_ASSERT_EQ(expected_flags, flags,
  ...
  ret = mach_port_insert_right(mach_task_self(), fake_port, fake_port,
