...
T_GLOBAL_META(T_META_NAMESPACE("xnu.monotonic"), T_META_CHECK_LEAKS(false),
              XNU_T_META_REQUIRES_DEVELOPMENT_KERNEL);

static bool device_supports_uncore(void) {
  int r;
  int type, subtype;
  unsigned int family;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(r, "sysctlbyname(\"hw.cputype\")");
  r = sysctlbyname("hw.cpusubtype", &subtype, &size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(r, "sysctlbyname(\"hw.cpusubtype\")");
  r = sysctlbyname("hw.cpufamily", &family, &size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(r, "sysctlbyname(\"hw.cpufamily\")");
...
static int open_uncore_error(int *error) {
  guardid_t guard;
  int fd;
...
static void uncore_counts(int fd, uint64_t ctr_mask, uint64_t *counts) {
  int r;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(r, "MT_IOC_COUNTS got counter values");
}
...
static uint32_t uncore_add(int fd, uint64_t event, uint64_t allowed_ctrs,
                           int error) {
  int save_errno;
  int r;
  uint32_t ctr;
  ...
  if (error) {
    ...
    T_EXPECT_EQ(save_errno, error,
                "adding event to counter should fail with %d: %s", error,
                strerror(error));
    return UINT32_MAX;
  } else {
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(r, "added event %#" PRIx64 " to counters", event);
  }

  ctr = add_ctl.out.ctr;
  T_QUIET;
  T_ASSERT_LT(ctr, (uint32_t)CTRS_MAX, "counter returned should be sane");
  return ctr;
}
...
static void uncore_enable(int fd) {
  union monotonic_ctl_enable en_ctl = {.in = {.enable = true}};
...
T_DECL(uncore_reset, "ensure that resetting the counters works",
       T_META_TAG_VM_NOT_ELIGIBLE) {
  int fd;
  int r;
...
static int uncore_add_all(int fd, uint64_t event, int *nmonitors) {
  int nctrs = 0;
  int r;
  ...
  if (nmonitors) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(r, "got info about uncore counters");
...
T_DECL(
    uncore_accuracy, "ensure that the uncore counters count accurately",
  ...
  for (int mon = 0; mon < nmonitors; mon++) {
    for (int i = 0; i < nctrs; i++) {
      ...
      T_LOG("%d,%2d: %12" PRIu64 "%12" PRIu64 " = %10" PRIu64, mon, i, before,
            after, delta);
      T_QUIET;
      T_EXPECT_GT(after, before,
                  "uncore %d counter %d value is monotonically increasing", mon,
                  i);
      T_QUIET;
      T_EXPECT_GT(counts[1][i * mon] - counts[0][i * mon], times[1] - times[0],
                  "reference timebase on uncore %d counter %d satisfies "
                  "lower bound from mach_absolute_time",
                  mon, i);
    }
  }
}

T_DECL(uncore_ownership, "ensure the dev node cannot be open in two places",
       T_META_ASROOT(true), T_META_TAG_VM_NOT_ELIGIBLE) {
  int fd;
  int other_fd;
  int error;
...
T_DECL(perf_uncore, "measure the latency of accessing the counters",
       XNU_T_META_SOC_SPECIFIC, T_META_TAG_PERF, T_META_TAG_VM_NOT_ELIGIBLE) {
  int fd;
  int nctrs;
  int nmonitors;
  int r;
  ...
  do {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(r, "getting uncore counter values %#" PRIx64,
                           ctr_mask);
