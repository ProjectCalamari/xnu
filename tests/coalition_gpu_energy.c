...
static uint64_t get_res_id(void) {
...
static uint64_t get_energy_id(void) {
  int ret;
  uint64_t out_val = 0;
  ...
  ret = sysctlbyname("kern.coalition_gpu_energy_test", &out_val, &out_size,
                     param, sizeof(param));
...
static int test_task_id_token_to_energy_id(mach_port_name_t name,
                                           uint64_t *energy_id_out) {
  int ret;
  uint64_t out_val = 0;
  size_t out_size = sizeof(out_val);
...
static int test_energy_id_report_energy(uint64_t self_id,
                                        uint64_t on_behalf_of_id,
                                        uint64_t energy) {
  uint64_t out_val = 0;
  size_t out_size = sizeof(out_val);
  ...
  return sysctlbyname("kern.coalition_gpu_energy_test", &out_val, &out_size,
                      param, sizeof(param));
}
...
T_DECL(task_id_token_to_energy_id,
       "task_id_token_to_energy_id looks up energy id") {
  uint64_t energy_id = get_energy_id();
  ...
  T_ASSERT_POSIX_SUCCESS(ret, "task_id_token_to_energy_id(0x%x) = %lld", token,
                         energy_id_out);
...
T_DECL(energy_id_report_energy_time_bad_value,
       "energy_id_report_energy_time handles bad value") {
...
T_DECL(energy_id_report_energy_time_self,
       "energy_id_report_energy_time increments gpu energy for self") {
  uint64_t res_id = get_res_id();
  uint64_t energy_id = get_energy_id();

  struct coalition_resource_usage coalusage_1 = {0};
  int ret =
      coalition_info_resource_usage(res_id, &coalusage_1, sizeof(coalusage_1));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "coalition_info_resource_usage() 1");

  T_LOG("before: gpu_energy_nj: %lld, gpu_energy_nj_billed_to_me: %lld, "
        "gpu_energy_nj_billed_to_others: %lld",
  ...
  ret =
      coalition_info_resource_usage(res_id, &coalusage_2, sizeof(coalusage_2));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "coalition_info_resource_usage() 2");

  T_LOG("after: gpu_energy_nj: %lld, gpu_energy_nj_billed_to_me: %lld, "
        "gpu_energy_nj_billed_to_others: %lld",
...
T_DECL(energy_id_report_energy_time_billed,
       "energy_id_report_energy_time increments billed energy") {
  uint64_t res_id = get_res_id();
  uint64_t energy_id = get_energy_id();
  ...
  T_ASSERT_POSIX_SUCCESS(ret, "energy_id_report_energy(%lld, %lld, 1)",
                         energy_id, energy_id);
  ...
  T_EXPECT_LT(coalusage_1.gpu_energy_nj, coalusage_2.gpu_energy_nj,
              "gpu_energy_nj should have increased");
  T_EXPECT_LT(coalusage_1.gpu_energy_nj_billed_to_me,
              coalusage_2.gpu_energy_nj_billed_to_me,
              "gpu_energy_nj_billed_to_me should have increased");
  T_EXPECT_LT(coalusage_1.gpu_energy_nj_billed_to_others,
              coalusage_2.gpu_energy_nj_billed_to_others,
