...
extern kern_return_t catch_mach_exception_raise_identity_protected(
    __unused mach_port_t exception_port, uint64_t thread_id,
    mach_port_t task_id_token, exception_type_t exception,
    mach_exception_data_t codes, mach_msg_type_number_t codeCnt);

extern kern_return_t catch_mach_exception_raise_backtrace(
    __unused mach_port_t exception_port, mach_port_t kcdata_object,
...
extern kern_return_t catch_mach_exception_raise_state(
    mach_port_t exception_port, exception_type_t type, exception_data_t codes,
...
extern kern_return_t catch_mach_exception_raise_state_identity(
    ...
    exception_type_t type, exception_data_t codes,
    mach_msg_type_number_t code_count, int *flavor, thread_state_t in_state,
    mach_msg_type_number_t in_state_count, thread_state_t out_state,
    mach_msg_type_number_t *out_state_count);
 ...
 */
static uint64_t get_exception_pc(thread_state_t in_state) {
#if __arm64__
 ...
 */
static void advance_exception_pc(size_t advance_pc, thread_state_t out_state) {
...
kern_return_t catch_mach_exception_raise_state_identity_protected(
    mach_port_t exception_port __unused, uint64_t thread_id,
    mach_port_t task_id_token, exception_type_t type, exception_data_t codes,
    mach_msg_type_number_t code_count, int *flavor, thread_state_t in_state,
    mach_msg_type_number_t in_state_count, thread_state_t out_state,
    mach_msg_type_number_t *out_state_count) {
  ...
  T_QUIET;
  T_ASSERT_EQ(code_count, 2,
              "Two code values were provided with the mach exception");
  ...
  T_QUIET;
  T_ASSERT_EQ(*flavor, EXCEPTION_THREAD_STATE,
              "The thread state flavor is EXCEPTION_THREAD_STATE");
  T_QUIET;
  T_ASSERT_EQ(in_state_count, EXCEPTION_THREAD_STATE_COUNT,
  ...
  *out_state_count = in_state_count; /* size of state object in 32-bit words */
  ...
  size_t advance_pc = tls_callbacks.state_protected_callback(
      task_id_token, thread_id, type, codes_64, in_state, in_state_count,
  ...
  if (advance_pc != 0) {
    T_FAIL("unimplemented PC change from EXCEPTION_STATE_IDENTITY_PROTECTED "
           "callback");
    return KERN_FAILURE;
  }
...
kern_return_t catch_mach_exception_raise_identity_protected(
  ...
  size_t advance_pc = tls_callbacks.protected_callback(task_id_token, thread_id,
                                                       exception, codes_64);
  ...
  if (advance_pc != 0) {
    T_FAIL(
        "unimplemented PC change from EXCEPTION_IDENTITY_PROTECTED callback");
    return KERN_FAILURE;
  }
...
kern_return_t catch_mach_exception_raise_state(
    ...
    mach_msg_type_number_t in_state_count, thread_state_t out_state,
    mach_msg_type_number_t *out_state_count) {
#pragma unused(exception_port, type, codes, code_count, flavor, in_state,      \
               in_state_count, out_state, out_state_count)
  T_FAIL(
      "Triggered catch_mach_exception_raise_state() which shouldn't happen...");
  __builtin_unreachable();
}
...
kern_return_t catch_mach_exception_raise_state_identity(
    mach_port_t exception_port __unused, mach_port_t thread, mach_port_t task,
    exception_type_t type, exception_data_t codes,
    mach_msg_type_number_t code_count, int *flavor, thread_state_t in_state,
    mach_msg_type_number_t in_state_count, thread_state_t out_state,
    mach_msg_type_number_t *out_state_count) {
  ...
  size_t advance_pc =
      tls_callbacks.state_callback(task, thread, type, codes_64, exception_pc);
...
mach_port_t create_exception_port(exception_mask_t exception_mask) {
  return create_exception_port_behavior64(exception_mask,
                                          EXCEPTION_STATE_IDENTITY);
}

void set_thread_exception_port(mach_port_t exc_port,
                               exception_mask_t exception_mask) {
  set_thread_exception_port_behavior64(exc_port, exception_mask,
                                       EXCEPTION_STATE_IDENTITY);
}

void set_thread_exception_port_behavior64(exception_port_t exc_port,
                                          exception_mask_t exception_mask,
                                          exception_behavior_t behavior) {
  mach_port_t thread = mach_thread_self();
  kern_return_t kr;

  if (((unsigned int)behavior & ~MACH_EXCEPTION_MASK) !=
          EXCEPTION_STATE_IDENTITY &&
      ((unsigned int)behavior & ~MACH_EXCEPTION_MASK) !=
  ...
  kr = thread_set_exception_ports(
      ...
      (exception_behavior_t)((unsigned int)behavior), EXCEPTION_THREAD_STATE);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "Set the exception port to my custom handler");
}

mach_port_t create_exception_port_behavior64(exception_mask_t exception_mask,
                                             exception_behavior_t behavior) {
...
#define MACH_MSG_REPLY_SIZE 4096
  ...
  if (run_once) {
    kr =
        mach_msg_server_once(mach_exc_server, MACH_MSG_REPLY_SIZE, exc_port, 0);
  } else {
...
static void _run_exception_handler(mach_port_t exc_port,
                                   void *preferred_callback, void *callback,
                                   bool run_once,
                                   exception_behavior_t behavior) {
  ...
  if (behavior & MACH_EXCEPTION_BACKTRACE_PREFERRED) {
    T_QUIET;
    T_ASSERT_NE(NULL, preferred_callback, "Require a preferred callback");
    params->callbacks.backtrace_callback =
        (exc_handler_backtrace_callback_t)preferred_callback;
  }
  ...
  case EXCEPTION_STATE_IDENTITY_PROTECTED:
    params->callbacks.state_protected_callback =
        (exc_handler_state_protected_callback_t)callback;
    break;
  case EXCEPTION_IDENTITY_PROTECTED:
    params->callbacks.protected_callback =
        (exc_handler_protected_callback_t)callback;
    break;
  default:
...
void run_exception_handler(mach_port_t exc_port,
                           exc_handler_callback_t callback) {
  run_exception_handler_behavior64(exc_port, NULL, (void *)callback,
                                   EXCEPTION_STATE_IDENTITY, true);
}

void run_exception_handler_behavior64(mach_port_t exc_port,
                                      void *preferred_callback, void *callback,
                                      exception_behavior_t behavior,
                                      bool run_once) {
  if (((unsigned int)behavior & ~MACH_EXCEPTION_MASK) !=
  ...
  _run_exception_handler(exc_port, (void *)preferred_callback, (void *)callback,
                         run_once, behavior);
}

void repeat_exception_handler(mach_port_t exc_port,
                              exc_handler_callback_t callback) {
  _run_exception_handler(exc_port, NULL, (void *)callback, false,
                         EXCEPTION_STATE_IDENTITY);
}
