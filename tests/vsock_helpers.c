...
static int _vsock_new_socket(uint16_t protocol) {
...
static int _vsock_bind(uint32_t cid, uint32_t port, struct sockaddr_vm *addr,
                       int *socket, uint16_t protocol) {
...
int vsock_bind(uint32_t cid, uint32_t port, struct sockaddr_vm *addr,
               int *socket) {
  return _vsock_bind(cid, port, addr, socket, VSOCK_PROTO_STANDARD);
}

int vsock_private_bind(uint32_t cid, uint32_t port, struct sockaddr_vm *addr,
                       int *socket) {
  return _vsock_bind(cid, port, addr, socket, VSOCK_PROTO_PRIVATE);
}

int vsock_listen(uint32_t cid, uint32_t port, struct sockaddr_vm *addr,
                 int backlog, int *socket) {
  int result = vsock_bind(cid, port, addr, socket);
  T_ASSERT_POSIX_SUCCESS(result, "vsock bind");
  return listen(*socket, backlog);
}

int vsock_private_listen(uint32_t cid, uint32_t port, struct sockaddr_vm *addr,
                         int backlog, int *socket) {
...
static int _vsock_connect(uint32_t cid, uint32_t port, int *socket,
                          uint16_t protocol) {
  int sock = _vsock_new_socket(protocol);
  if (sock < 0) {
    return sock;
...
int vsock_connect(uint32_t cid, uint32_t port, int *socket) {
  return _vsock_connect(cid, port, socket, VSOCK_PROTO_STANDARD);
}

int vsock_private_connect(uint32_t cid, uint32_t port, int *socket) {
  return _vsock_connect(cid, port, socket, VSOCK_PROTO_PRIVATE);
}

struct sockaddr_vm vsock_getsockname(int socket) {
...
void vsock_close(int socket) {
  int result = close(socket);
  T_ASSERT_POSIX_SUCCESS(result, "vsock close");
}

void vsock_connect_peers(uint32_t cid, uint32_t port, int backlog, int *socketA,
                         int *socketB) {
  // Listen.
  struct sockaddr_vm addr;
  int listen_socket;
  ...
  int accepted_socket =
      accept(listen_socket, (struct sockaddr *)&accepted_addr, &addrlen);
  T_ASSERT_GT(accepted_socket, 0, "accepted socket");
  T_ASSERT_EQ_INT((int)sizeof(accepted_addr), addrlen,
...
void vsock_send(int socket, char *msg) {
...
bool vsock_address_exists(struct xvsockpgen *buffer, struct sockaddr_vm addr) {
  ...
  for (xvg = (struct xvsockpgen *)((char *)xvg + xvg->xvg_len);
    ...
    if (xpcb->xvp_local_cid == addr.svm_cid &&
        xpcb->xvp_remote_cid == VMADDR_CID_ANY &&
        xpcb->xvp_local_port == addr.svm_port &&
        xpcb->xvp_remote_port == VMADDR_PORT_ANY) {
      found = true;
      break;
    }
  }
...
uint32_t vsock_get_available_port(void) {
  int socket;
  struct sockaddr_vm addr;
...
int vsock_bind_family(sa_family_t family) {
