...
int main(int argc, char *argv[]) {
  ...
  page = mmap(NULL, page_size, PROT_READ | PROT_EXEC, MAP_ANON | MAP_SHARED, -1,
              0);
  if (!page) {
    fprintf(stderr, "%s:%d mmap() error %d (%s)\n", cmdname, __LINE__, errno,
            strerror(errno));
    exit(1);
  }
  if (opt_interactive) {
  ...
  if (opt_interactive) {
    fprintf(stdout, "changed page protection to r/w and copied code at %p\n",
            page);
  ...
  if (opt_interactive) {
    fprintf(stdout, "changed page protection to r/x at %p\n", page);
    fprintf(stdout, "pausing...\n");
    fflush(stdout);
    getchar();
  }
  ...
  mychdir =
      ptrauth_sign_unauthenticated(mychdir, ptrauth_key_function_pointer, 0);
#endif
  mychdir(getenv("HOME"));
  if (opt_interactive) {
