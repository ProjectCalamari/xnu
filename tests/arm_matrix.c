...
static size_t bad_instruction_exception_handler(
    __unused mach_port_t task, __unused mach_port_t thread,
    exception_type_t type, mach_exception_data_t codes,
    __unused uint64_t exception_pc) {
  T_QUIET;
  T_ASSERT_EQ(type, EXC_BAD_INSTRUCTION,
              "Caught an EXC_BAD_INSTRUCTION exception");
  T_QUIET;
  T_ASSERT_EQ(codes[0], (uint64_t)EXC_ARM_UNDEFINED,
              "The subcode is EXC_ARM_UNDEFINED");
...
#ifdef __arm64__
static void test_matrix_not_started(const struct arm_matrix_operations *ops) {
  if (!ops->is_available()) {
  ...
  ops->stop();
  T_PASS("%s instruction after start instruction should not cause an exception",
         ops->name);
  ...
  T_EXPECT_TRUE(
      mach_exc_caught,
      "%s instruction before start instruction should cause an exception",
      ops->name);
...
static void test_thread_barrier(void) {
...
static void test_thread_notify_exited(void) {
...
static void wait_for_test_threads(void) {
  pthread_mutex_lock(&end_barrier_lock);
  while (end_barrier) {
...
static uint32_t ncpus(void) {
  uint32_t ncpu;
  size_t ncpu_size = sizeof(ncpu);
  int err = sysctlbyname("hw.ncpu", &ncpu, &ncpu_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(err, "Retrieved CPU count");

  return ncpu;
}

static int thread_bind_cpu_unchecked(uint32_t cpuid) {
  /*
...
static void test_on_each_cpu(thread_fn_t thread_fn,
                             const struct arm_matrix_operations *ops,
                             const char *desc) {
  uint32_t ncpu = ncpus();
  ...
  for (uint32_t i = 0; i < nthreads; i++) {
    ...
    int const err =
        pthread_create(&threads[i].thread, NULL, test_thread_shim, &threads[i]);
    T_QUIET;
    T_ASSERT_EQ(err, 0, "%s: created thread #%u", desc, i);
  ...
  // like pthread_join()ing all threads, but without the priority boosting
  // shenanigans.
  ...
  for (uint32_t i = 0; i < nthreads; i++) {
    ...
    T_QUIET;
    T_ASSERT_EQ(err, 0, "%s: joined thread #%u", desc, i);
...
static bool active_context_switch_thread(struct test_thread const *thread) {
  ...
  thread_t const companion_thread =
      pthread_mach_thread_np(thread->companion_thread);
  T_QUIET;
  T_ASSERT_NE(companion_thread, THREAD_NULL, "pthread_mach_thread_np");
...
static bool inactive_context_switch_thread(struct test_thread const *thread) {
  const struct arm_matrix_operations *ops = thread->ops;
  const uint32_t thread_id = thread->thread_id;
  size_t size = ops->data_size();
  uint8_t *d1 = ops->alloc_data();
...
static void test_thread_migration(const struct arm_matrix_operations *ops) {
  size_t size = ops->data_size();
  uint8_t *d = ops->alloc_data();
...
T_DECL(sme_context_switch,
       "Test that SME contexts are migrated during context switch and do not "
       "leak between process contexts.",
  ...
  test_on_each_cpu(active_context_switch_thread, &sme_operations,
                   "SME context migrates when active");
  test_on_each_cpu(inactive_context_switch_thread, &sme_operations,
                   "SME context does not leak across processes");
#endif
}
...
static void thread_set_state_test_state(enum thread_state_test_state state) {
  pthread_mutex_lock(&thread_state_test_lock);
...
static void *test_matrix_thread_state_child(void *arg __unused) {
...
static void test_matrix_thread_state(const struct arm_matrix_operations *ops) {
  if (!ops->is_available()) {
    T_SKIP("Running on non-%s target, skipping...", ops->name);
  }
...
#pragma clang diagnostic pop
  ...
  T_QUIET;
  T_ASSERT_EQ(err, 0, "pthread_create()");

  mach_port_t mach_thread = pthread_mach_thread_np(thread);
  T_QUIET;
  T_ASSERT_NE(mach_thread, MACH_PORT_NULL, "pthread_mach_thread_np()");

  wait_for_thread_state_test_state(CHILD_READY);
  kern_return_t kr = ops->thread_set_state(mach_thread, d);
  T_QUIET;
  T_ASSERT_EQ(kr, KERN_SUCCESS, "%s thread_set_state()", ops->name);
  ...
  T_QUIET;
  T_ASSERT_EQ(kr, KERN_SUCCESS, "%s thread_get_state()", ops->name);
  for (size_t i = 0; i < size; i++) {
    d[i]++;
  }
  T_EXPECT_EQ(memcmp(d, thread_d, size), 0,
              "thread_get_state() read expected %s data from child thread",
              ops->name);
...
T_DECL(sme_thread_state, "Test thread_{get,set}_state with SME thread state.",
...
T_DECL(sme_exception_ports,
  ...
  kern_return_t kr =
      mach_port_allocate(task, MACH_PORT_RIGHT_RECEIVE, &exc_port);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "Allocated mach exception port");
  kr =
      mach_port_insert_right(task, exc_port, exc_port, MACH_MSG_TYPE_MAKE_SEND);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "Inserted a SEND right into the exception port");

  kr = thread_set_exception_ports(thread, EXC_MASK_ALL, exc_port,
                                  EXCEPTION_STATE, ARM_THREAD_STATE64);
  T_EXPECT_MACH_SUCCESS(kr, "thread_set_exception_ports accepts flavor %u",
                        (unsigned int)ARM_THREAD_STATE64);

  for (thread_state_flavor_t flavor = ARM_SME_STATE; flavor <= ARM_SME2_STATE;
       flavor++) {
    kr = thread_set_exception_ports(thread, EXC_MASK_ALL, exc_port,
                                    EXCEPTION_STATE, flavor);
    T_EXPECT_MACH_ERROR(kr, KERN_INVALID_ARGUMENT,
                        "thread_set_exception_ports rejects flavor %u",
                        (unsigned int)flavor);
  }
}

T_DECL(sme_max_svl_b_sysctl, "Test the hw.optional.arm.sme_max_svl_b sysctl",
       XNU_T_META_SOC_SPECIFIC) {
  unsigned int max_svl_b;
  size_t max_svl_b_size = sizeof(max_svl_b);

  int err = sysctlbyname("hw.optional.arm.sme_max_svl_b", &max_svl_b,
                         &max_svl_b_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(err, "sysctlbyname(hw.optional.arm.sme_max_svl_b)");
  if (sme_operations.is_available()) {
    ...
    T_EXPECT_EQ(__builtin_popcount(max_svl_b), 1,
                "Maximum SVL_B is a power of 2");
    T_EXPECT_GE(max_svl_b, ARCH_MIN_SVL_B,
                "Maximum SVL_B >= architectural minimum");
    T_EXPECT_LE(max_svl_b, ARCH_MAX_SVL_B,
                "Maximum SVL_B <= architectural maximum");
  } else {
