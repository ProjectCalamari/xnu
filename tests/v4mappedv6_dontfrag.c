...
T_DECL(v4mappedv6_dontfrag_sockopt,
       "Tests setting IPV6_DONTFRAG on an IPv4-mapped IPv6 address") {
  int sockfd = 0;
  ssize_t n;
  ...
  T_ASSERT_EQ(inet_pton(AF_INET6, "::ffff:169.254.1.2", &sin6.sin6_addr), 1,
              "inet_pton");

  T_ASSERT_POSIX_SUCCESS(sockfd = socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP),
                         "create socket");

  // This send should succeed (and fragment)
  n = sendto(sockfd, buf, MAX_BUFFER_SIZE, 0, (struct sockaddr *)&sin6,
             sizeof(sin6));
  T_EXPECT_EQ(n, (ssize_t)MAX_BUFFER_SIZE, "Ensure we wrote MAX_BUFFER_SIZE");

  int Option = 1;
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(sockfd, IPPROTO_IPV6, IPV6_DONTFRAG, &Option, sizeof(Option)),
      "setsockopt IPV6_DONTFRAG to 1");
  ...
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(sockfd, IPPROTO_IPV6, IPV6_DONTFRAG, &Option, sizeof(Option)),
      "setsockopt IPV6_DONTFRAG back to 0");

  // This send should succeeed (and fragment) because we turned the option back
  // off
  n = sendto(sockfd, buf, MAX_BUFFER_SIZE, 0, (struct sockaddr *)&sin6,
             sizeof(sin6));
  T_EXPECT_EQ(n, (ssize_t)MAX_BUFFER_SIZE, "Ensure we wrote MAX_BUFFER_SIZE");
