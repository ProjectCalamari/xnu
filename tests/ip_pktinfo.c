...
static void network_interface_assign_address(network_interface_t netif,
                                             unsigned int unit,
                                             unsigned int address_index) {
  get_ipv4_address(unit, address_index, &netif->ip);
  ifnet_add_ip_address(netif->if_name, netif->ip, inet_class_c_subnet_mask);
  route_add_inet_scoped_subnet(netif->if_name, netif->if_index, netif->ip,
                               inet_class_c_subnet_mask);
  ifnet_start_ipv6(netif->if_name);
  T_ASSERT_EQ(inet6_get_linklocal_address(netif->if_index, &netif->ip6), 1,
              NULL);
}

static void initialize_feth_pairs(u_int n, bool need_address) {
...
static int setup_receiver(char *bind_to_ifname, bool bind_to_port,
                          in_addr_t bind_to_addr, in_port_t *bound_port) {
  int receiver_fd;
  socklen_t solen;
  ...
  T_ASSERT_POSIX_SUCCESS(setsockopt(receiver_fd, SOL_SOCKET, SO_RCVTIMEO, &tv,
                                    sizeof(struct timeval)),
                         NULL);

  optval = 1;
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(receiver_fd, SOL_SOCKET, SO_DEBUG, &optval, sizeof(int)),
      NULL);

  optval = 1;
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(receiver_fd, IPPROTO_IP, IP_RECVPKTINFO, &optval, sizeof(int)),
      NULL);

  optval = 1;
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(receiver_fd, IPPROTO_UDP, UDP_NOCKSUM, &optval, sizeof(int)),
      NULL);

  if (bind_to_ifname != NULL) {
    solen = strlen(bind_to_ifname);
    T_ASSERT_POSIX_SUCCESS(setsockopt(receiver_fd, SOL_SOCKET, SO_BINDTODEVICE,
                                      bind_to_ifname, solen),
                           NULL);
  }
  ...
  if (bind_to_port || bind_to_addr != INADDR_ANY) {
    ...
    T_ASSERT_POSIX_SUCCESS(
        bind(receiver_fd, (struct sockaddr *)&sin, sizeof(struct sockaddr_in)),
        NULL);
  }
  solen = sizeof(struct sockaddr_in);
  T_ASSERT_POSIX_SUCCESS(
      getsockname(receiver_fd, (struct sockaddr *)&sin, &solen), NULL);
  inet_ntop(AF_INET, &sin.sin_addr, laddr_str, sizeof(laddr_str));

  solen = sizeof(ifname);
  T_ASSERT_POSIX_SUCCESS(
      getsockopt(receiver_fd, SOL_SOCKET, SO_BINDTODEVICE, ifname, &solen),
      NULL);

  T_LOG("receiver bound to %s:%u over '%s'", laddr_str, ntohs(sin.sin_port),
        ifname);
...
int setup_sender(char *bind_to_ifname, in_addr_t connect_to_addr,
                 in_port_t connect_to_port) {
  int sender_fd;
  ...
  T_ASSERT_POSIX_SUCCESS(setsockopt(sender_fd, SOL_SOCKET, SO_RCVTIMEO, &tv,
                                    sizeof(struct timeval)),
                         NULL);

  optval = 1;
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(sender_fd, SOL_SOCKET, SO_DEBUG, &optval, sizeof(int)), NULL);

  optval = 1;
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(sender_fd, IPPROTO_IP, IP_RECVPKTINFO, &optval, sizeof(int)),
      NULL);

  optval = 1;
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(sender_fd, IPPROTO_UDP, UDP_NOCKSUM, &optval, sizeof(int)),
      NULL);

  if (bind_to_ifname != NULL) {
    solen = strlen(bind_to_ifname);
    T_ASSERT_POSIX_SUCCESS(setsockopt(sender_fd, SOL_SOCKET, SO_BINDTODEVICE,
                                      bind_to_ifname, solen),
                           NULL);
  }
  ...
  T_ASSERT_POSIX_SUCCESS(connect(sender_fd, (struct sockaddr *)&connect_to_sin,
                                 sizeof(struct sockaddr_in)),
                         NULL);

  solen = sizeof(struct sockaddr_in);
  T_ASSERT_POSIX_SUCCESS(
      getsockname(sender_fd, (struct sockaddr *)&sin, &solen), NULL);
  inet_ntop(AF_INET, &sin.sin_addr, laddr_str, sizeof(laddr_str));
  inet_ntop(AF_INET, &connect_to_sin.sin_addr, faddr_str, sizeof(faddr_str));

  solen = sizeof(ifname);
  T_ASSERT_POSIX_SUCCESS(
      getsockopt(sender_fd, SOL_SOCKET, SO_BINDTODEVICE, ifname, &solen), NULL);

  T_LOG("sender_fd connected from %s:%u to %s:%u over '%s'", laddr_str,
        ntohs(sin.sin_port), faddr_str, ntohs(connect_to_sin.sin_port), ifname);

  return sender_fd;
}

static void echo(int receiver_fd, bool by_ip_addr) {
  ...
  for (cmsg = CMSG_FIRSTHDR(&recvmsghdr); cmsg != NULL;
       cmsg = CMSG_NXTHDR(&recvmsghdr, cmsg)) {
  ...
  inet_ntop(AF_INET, &recv_in_pktinfo.ipi_spec_dst, spec_dst_str,
            sizeof(spec_dst_str));
  ...
  T_LOG("received %ld bytes from %s:%u with IP_RECVPKTINFO ipi_ifindex: %u "
        "(%s) ipi_spec_dst: %s ipi_addr: %s",
  ...
  inet_ntop(AF_INET, &send_in_pktinfo.ipi_spec_dst, spec_dst_str,
            sizeof(spec_dst_str));
  inet_ntop(AF_INET, &send_in_pktinfo.ipi_addr, addr_str, sizeof(addr_str));

  T_LOG("sending %ld bytes to %s:%u with IP_PKTINFO ipi_ifindex: %u (%s) "
...
static void echo_and_check(int receiver_fd, bool by_ip_addr) {
  socklen_t solen;
  ...
  solen = sizeof(struct sockaddr_in);
  T_ASSERT_POSIX_SUCCESS(
      getsockname(receiver_fd, (struct sockaddr *)&before_sin, &solen), NULL);
  inet_ntop(AF_INET, &before_sin.sin_addr, before_addr_str,
            sizeof(before_addr_str));

  solen = sizeof(before_ifname);
  T_ASSERT_POSIX_SUCCESS(getsockopt(receiver_fd, SOL_SOCKET, SO_BINDTODEVICE,
                                    before_ifname, &solen),
                         NULL);
  ...
  solen = sizeof(struct sockaddr_in);
  T_ASSERT_POSIX_SUCCESS(
      getsockname(receiver_fd, (struct sockaddr *)&after_sin, &solen), NULL);
  inet_ntop(AF_INET, &after_sin.sin_addr, after_addr_str,
            sizeof(after_addr_str));
  ...
  T_LOG("before bound to %s:%u over '%s'/%u", before_addr_str,
        ntohs(before_sin.sin_port), before_ifname, before_ifindex);
  T_LOG("after bound to %s:%u over '%s'/%u", after_addr_str,
        ntohs(after_sin.sin_port), after_ifname, after_ifindex);

  T_ASSERT_EQ_USHORT(before_sin.sin_port, after_sin.sin_port, "same port");
  T_ASSERT_EQ_UINT(before_sin.sin_addr.s_addr, after_sin.sin_addr.s_addr,
                   "same IP address");
  T_ASSERT_EQ_UINT(before_ifindex, after_ifindex, "same interface index");
}

static void do_test_ip_pktinfo(bool bind_to_device, bool bind_to_port,
                               in_addr_t bind_to_addr) {
  int receiver_fd;
  ...
  receiver_fd = setup_receiver(bind_to_device ? ifname1 : NULL, bind_to_port,
                               bind_to_addr ? S_feth_pairs->list->one.ip.s_addr
                                            : INADDR_ANY,
                               &receiver_port);
  sender_fd =
      setup_sender(ifname2, S_feth_pairs->list->one.ip.s_addr, receiver_port);

  T_ASSERT_POSIX_SUCCESS(retval = send(sender_fd, data, strlen(data) + 1, 0),
                         NULL);
...
T_DECL(ip_pktinfo_010,
       "IP_PTKINFO bind_to_device=false bind_to_port=true bind_to_addr=false") {
  do_test_ip_pktinfo(false, true, false);
}

T_DECL(ip_pktinfo_011,
       "IP_PTKINFO bind_to_device=false bind_to_port=true bind_to_addr=true") {
  do_test_ip_pktinfo(false, true, true);
}

T_DECL(ip_pktinfo_110,
       "IP_PTKINFO bind_to_device=true bind_to_port=true bind_to_addr=false") {
  do_test_ip_pktinfo(true, true, false);
}

T_DECL(ip_pktinfo_111,
       "IP_PTKINFO bind_to_device=true bind_to_port=true bind_to_addr=true") {
