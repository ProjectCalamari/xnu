...
T_GLOBAL_META(T_META_NAMESPACE("xnu.sync"), T_META_RUN_CONCURRENTLY(true),
              T_META_CHECK_LEAKS(false), T_META_RADAR_COMPONENT_NAME("xnu"),
...
T_DECL(hw_lck_ticket_allow_invalid, "hw_lck_ticket_allow_invalid",
  ...
  T_EXPECT_EQ(1ll, run_sysctl_test("hw_lck_ticket_allow_invalid", 0),
              "test succeeded");
}
...
static void clpc_set_core_count(int ncpus) {
#if __arm64__
  char arg[20];
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rc, "posix_spawn");
  waitpid(pid, &rc, 0);
#else
  (void)ncpus;
#endif
...
static void *toggle_cpus_thread(void *donep) {
  int ncpus = dt_ncpu();
...
T_DECL(smr_sleepable_stress, "smr_sleepable_stress_test",
  ...
  T_EXPECT_EQ(1ll, run_sysctl_test("smr_sleepable_stress", secs),
              "test succeeded");
