...
T_GLOBAL_META(T_META_NAMESPACE("xnu.cpu_counters"),
              T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("cpu counters"),
              T_META_OWNER("mwidmann"), T_META_ASROOT(true),
...
static void skip_if_unsupported(void) {
  int r;
  int supported = 0;
...
static void start_kpc(void) {
  T_SETUPBEGIN;

  kpc_classmask_t classes = KPC_CLASS_FIXED_MASK | KPC_CLASS_CONFIGURABLE_MASK;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "got rusage information");
...
static void _assert_kpep_ok(int kpep_err, const char *fmt, ...) {
  char msg[1024] = "";
  va_list args;
  va_start(args, fmt);
...
static void prepare_kpc(struct machine *mch, unsigned int n,
                        const char *event_name, uint64_t period) {
  T_SETUPBEGIN;
  ...
  ret = sysctlbyname("hw.logicalcpu_max", &mch->ncpus, &ncpus_sz, NULL, 0);
  T_QUIET;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kpc_force_all_ctrs_set(1)");

  int forcing = 0;
  ret = kpc_force_all_ctrs_get(&forcing);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kpc_force_all_ctrs_get");
  T_QUIET;
  T_ASSERT_EQ(forcing, 1, "counters must be forced");
  ...
  for (uint32_t i = 0; i < nconfigs; i++) {
    if (period != 0 && (n == 0 || i == 0)) {
      ret = kpep_config_add_event_trigger(config, &event, 0, period + i * 1000,
                                          NULL);
    } else {
    ...
    _assert_kpep_ok(ret, "adding %d event %s to configuration", i, event_name);
  }

  uint64_t *configs = calloc(nconfigs, sizeof(*configs));
  T_QUIET;
  T_ASSERT_NOTNULL(configs, "allocated config words");
  ...
  T_QUIET;
  T_ASSERT_NE(mch->selector, 0ULL, "found event selector to check");
  ret = kpc_set_config(KPC_CLASS_CONFIGURABLE_MASK, configs);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kpc_set_config");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kpc_set_period");
...
static pthread_t *start_threads(const struct machine *mch,
                                void *(*func)(void *), void *arg) {
  T_SETUPBEGIN;

  pthread_t *threads = calloc((unsigned int)mch->ncpus, sizeof(*threads));
  T_QUIET;
  T_ASSERT_NOTNULL(threads, "allocated array of threads");
...
static void end_threads(const struct machine *mch, pthread_t *threads) {
  for (unsigned int i = 0; i < mch->ncpus; i++) {
    int error = pthread_join(threads[i], NULL);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(error, "joined thread %d", i);
  }
  free(threads);
}
...
static void check_counters(unsigned int ncpus, unsigned int nctrs,
                           struct tally *tallies, uint64_t *counts) {
...
static void check_tally(unsigned int ncpus, unsigned int nctrs,
  ...
  for (unsigned int i = 0; i < ncpus; i++) {
    for (unsigned int j = 0; j < nctrs; j++) {
      ...
      T_LOG("CPU %2u PMC %u: nchecks = %llu, last value = %llx, "
            "delta = %llu, nstuck = %llu",
            i, j, tly->nchecks, tly->lastvalue,
            tly->lastvalue - tly->firstvalue, tly->nstuck);
  ...
  T_EXPECT_EQ(ndecrease, 0ULL, "found 0x%" PRIx64 " decreasing values",
              ndecrease);
}
...
T_DECL(kpc_cpu_direct_configurable,
  ...
  while (clock_gettime_nsec_np(CLOCK_MONOTONIC) - startns < TESTDUR_NS) {
    int ret =
        kpc_get_cpu_counters(true, KPC_CLASS_CONFIGURABLE_MASK, NULL, counts);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "kpc_get_cpu_counters");
...
T_DECL(kpc_thread_direct_instrs_cycles,
  ...
  for (uint32_t i = 0; i < ctrs_cnt; i++) {
    ...
    T_QUIET;
    T_EXPECT_LT(ctrs_a[i], ctrs_b[i], "counter %d is increasing", i);
  }
...
T_DECL(kpc_pmi_configurable,
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kpc_set_actionid");
  free(actions);

  (void)kperf_action_count_set(1);
  ret =
      kperf_action_samplers_set(1, KPERF_SAMPLER_TINFO | KPERF_SAMPLER_KSTACK);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kperf_action_samplers_set");

  ktrace_config_t ktconfig = ktrace_config_create_current();
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(ktconfig, "create current config");
  ret = ktrace_config_print_description(ktconfig, stdout);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "print config description");

  struct cpu *cpus = calloc(mch.ncpus, sizeof(*cpus));
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(cpus, "allocate CPUs array");
  ...
  ktrace_events_single(sess, PERF_KPC_PMI, ^(struct trace_point *tp) {
    ...
    uint64_t cur_ns = 0;
    int cret =
        ktrace_convert_timestamp_to_nanoseconds(sess, tp->timestamp, &cur_ns);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(cret, "convert timestamp");

    uint64_t desc = tp->arg1;
    uint64_t config = desc & UINT32_MAX;
    T_QUIET;
    T_EXPECT_EQ(config & UINT16_MAX, mch.selector & UINT16_MAX,
    ...
    double slice =
        (double)(cur_ns - first_ns) / PMI_TEST_DURATION_NS * NTIMESLICES;
  ...
  void (^sched_handler)(struct trace_point *tp) = ^(struct trace_point *tp) {
    uint64_t cur_ns = 0;
    int cret =
        ktrace_convert_timestamp_to_nanoseconds(sess, tp->timestamp, &cur_ns);
    T_QUIET;
  ...
  ktrace_events_single(sess, PERF_STK_KHDR, ^(struct trace_point *__unused tp) {
    callstack_count++;
  });

  ktrace_events_single(sess, END_EVENT, ^(struct trace_point *tp) {
    int cret =
        ktrace_convert_timestamp_to_nanoseconds(sess, tp->timestamp, &last_ns);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(cret, "convert timestamp");
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(counts, "allocated counter values array");
  memset(counts, 0, sizeof(*counts) * mch.ncpus * (mch.nfixed + mch.nconfig));
  struct tally *tly =
      calloc(mch.ncpus * (mch.nconfig + mch.nfixed), sizeof(*tly));
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(tly, "allocated tallies array");

  dispatch_source_t cpu_count_timer = dispatch_source_create(
      DISPATCH_SOURCE_TYPE_TIMER, 0, 0, dispatch_get_main_queue());
  dispatch_source_set_timer(
      cpu_count_timer,
      dispatch_time(DISPATCH_TIME_NOW, PERIODIC_CPU_COUNT_MS * NSEC_PER_MSEC),
  ...
  dispatch_source_set_event_handler(cpu_count_timer, ^{
    int cret = kpc_get_cpu_counters(
        true, KPC_CLASS_FIXED_MASK | KPC_CLASS_CONFIGURABLE_MASK, NULL, counts);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(cret, "kpc_get_cpu_counters");
  ...
  ktrace_events_class(sess, DBG_PERF,
  ...
  ktrace_set_completion_handler(sess, ^{
    ...
    int ruret =
        proc_pid_rusage(getpid(), RUSAGE_INFO_V4, (rusage_info_t *)&post_ru);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ruret, "got rusage information");
    ...
    for (unsigned int i = 0; i < mch.ncpus; i++) {
      ...
      T_LOG("CPU %2u: %4up:%4un/%u, %6u/%llu, max skid = %llu (%.4f%%), "
            "last contiguous = %u, scheduled outside = %u",
            i, pmi_slice_count, no_sched_slice_count, NTIMESLICES, sample_count,
            delta / PMI_PERIOD, abs_max_skid,
    ...
    T_EXPECT_EQ(cpu_sample_count, mch.ncpus, "should see PMIs on every CPU");
    T_END;
  });

  int dbglvl = 3;
  ret = sysctlbyname("kperf.debug_level", NULL, NULL, &dbglvl, sizeof(dbglvl));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "set kperf debug level");
...
T_DECL(kpc_pmu_config, "ensure PMU can be configured", XNU_T_META_SOC_SPECIFIC,
...
T_DECL(pmi_pc_capture, "ensure PC capture works for PMCs 5, 6, and 7",
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(periods, "allocate periods array");
...
#define KPC_FLAG_PC_CAPTURED (0x08)
  ...
  ktrace_set_completion_handler(sess, ^{
    ...
    for (unsigned int i = 0; i < 3; i++) {
      ...
      T_EXPECT_GT(pc_captured[i], 0ULL, "saw PC capture for counter %u", i + 5);
    }
    T_END;
  });
