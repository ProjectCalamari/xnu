...
T_GLOBAL_META(T_META_NAMESPACE("xnu.cpc"), T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_ENABLED(HAS_CPC_SECURITY), _T_META_REQUIRES_CPC_SUPPORT);

// Several of these tests have two variants to support running on development
// and release kernels. Tests prefixed with `secure_` put the development kernel
// into a secure CPC mode while tests prefixed with `release_` can run on the
// RELEASE build variant.

// Metadata for running on a development kernel in CPC secure mode.
#define _T_META_CPC_SECURE_ON_DEV                                              \
  T_META_SYSCTL_INT("kern.cpc.secure=1"), XNU_T_META_REQUIRES_DEVELOPMENT_KERNEL
...
static void _skip_for_db(const char *kind, int kpep_err) {
...
static void check_secure_cpmu(void) {
  ...
  kpep_event_t *internal_events =
      calloc(internal_event_count, sizeof(internal_events[0]));
  T_QUIET;
  T_WITH_ERRNO;
  ...
  for (size_t i = 0; i < internal_event_count; i++) {
    ...
    ret = kpep_config_kpc(config, (uint8_t *)kpc_configs,
                          kpc_count * sizeof(kpc_config_t));
    ...
    if (not_permitted) {
      if (!internal_only) {
        T_LOG("failed to configure public event %s (0x%04llx)", name,
              kpc_config);
      }
      filtered++;
    } else if (internal_only) {
      T_FAIL("configured internal-only event %s (0x%04llx) with secure CPC",
             name, kpc_config);
    } else {
      public_tested++;
    }
...
T_DECL(secure_cpmu_event_restrictions,
       "secured CPMU should be restricted to known events",
       _T_META_CPC_SECURE_ON_DEV, T_META_TAG_VM_NOT_ELIGIBLE,
       T_META_ENABLED(false) /* rdar://153473281 */) {
  check_secure_cpmu();
}

T_DECL(release_cpmu_event_restrictions,
       "release CPMU should be restricted to known events",
...
static void check_secure_upmu(void) {
  guardid_t guard;
  int fd;
  ...
  fd = guarded_open_np(UNCORE_DEV_PATH, &guard,
                       GUARD_CLOSE | GUARD_DUP | GUARD_WRITE,
                       O_CLOEXEC | O_EXCL);
...
T_DECL(secure_upmu_event_restrictions,
       "secured UPMU should be restricted to no events",
       _T_META_CPC_SECURE_ON_DEV, T_META_TAG_VM_NOT_ELIGIBLE) {
  check_secure_upmu();
}

T_DECL(release_upmu_event_restrictions,
       "release UPMU should be restricted to no events",
       XNU_T_META_REQUIRES_RELEASE_KERNEL, T_META_TAG_VM_NOT_ELIGIBLE) {
  check_secure_upmu();
}
...
static void check_event_coverage(kpep_db_flags_t flag, const char *kind) {
  kpep_db_t db = NULL;
  ...
  for (size_t i = 0; i < event_count; i++) {
    ...
    if (ret == KPEP_ERR_ERRNO && errno == EPERM) {
      T_FAIL("failed to configure %s event %s (0x%04llx) with secure CPC", kind,
             name, kpc_config);
    } else {
      _assert_kpep_ok(ret, "applying configuration with event %s (0x%04llx)",
                      name, kpc_config);
    }
    ret = kpep_config_remove_event(config, 0);
    _assert_kpep_ok(ret, "removing event %s from configuration", name);
...
T_DECL(secure_public_event_coverage,
       "all public events in kpep should be allowed", _T_META_CPC_SECURE_ON_DEV,
       T_META_TAG_VM_NOT_ELIGIBLE) {
  check_event_coverage(KPEP_DB_FLAG_PUBLIC_ONLY, "public");
}

T_DECL(release_public_event_coverage,
       "all public events in kpep should be allowed",
       XNU_T_META_REQUIRES_RELEASE_KERNEL, T_META_TAG_VM_NOT_ELIGIBLE) {
  check_event_coverage(KPEP_DB_FLAG_PUBLIC_ONLY, "public");
}
...
T_DECL(insecure_cpmu_unrestricted, "insecure CPMU should be unrestricted",
       ...
       T_META_SYSCTL_INT("kern.cpc.secure=0"), T_META_TAG_VM_NOT_ELIGIBLE) {
  check_event_coverage(KPEP_DB_FLAG_INTERNAL_ONLY, "internal");
}

T_DECL(secure_kpc_counting_system,
       "kpc should not allow counting the kernel when secure",
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(configs, "allocated %u * %zu", config_count,
                   sizeof(configs[0]));
  ret = kpc_get_config(KPC_CLASS_CONFIGURABLE_MASK, configs);
  for (uint32_t i = 0; i < config_count; i++) {
    if ((configs[i] & 0x40000)) {
      T_FAIL("found configurable counter %u with configuration 0x%llx", i,
             configs[i]);
    }
  }
