...
T_DECL(sendmsg_x_test, "exercise sendmsg_x() return value",
  ...
  T_EXPECT_POSIX_SUCCESS(socketpair(AF_UNIX, SOCK_DGRAM, 0, socket_fds),
                         "socketpair");
  ...
  T_EXPECT_POSIX_SUCCESS(setsockopt(socket_fds[0], SOL_SOCKET, SO_SNDBUF,
                                    &send_buffer_size, opt_len),
                         "setsockopt() SO_SNDBUF");
  T_EXPECT_POSIX_SUCCESS(setsockopt(socket_fds[0], SOL_SOCKET, SO_RCVBUF,
                                    &receive_buffer_size, opt_len),
                         "setsockopt() SO_RCVBUF");
  T_EXPECT_POSIX_SUCCESS(setsockopt(socket_fds[1], SOL_SOCKET, SO_SNDBUF,
                                    &send_buffer_size, opt_len),
                         "setsockopt() SO_SNDBUF");
  T_EXPECT_POSIX_SUCCESS(setsockopt(socket_fds[1], SOL_SOCKET, SO_RCVBUF,
                                    &receive_buffer_size, opt_len),
                         "setsockopt() SO_RCVBUF");
  ...
  ssize_t recv_result =
      recv(socket_fds[1], receive_buffer1, sizeof(receive_buffer1), 0);
  T_EXPECT_POSIX_SUCCESS(recv_result, "first recv() recv_result %zd errno %d",
                         recv_result, errno);
  ...
  T_EXPECT_POSIX_SUCCESS(fcntl(socket_fds[1], F_SETFL, flags | O_NONBLOCK),
                         "fcntl() O_NONBLOCK");

  recv_result =
      recv(socket_fds[1], receive_buffer1, sizeof(receive_buffer1), 0);
  T_EXPECT_POSIX_ERROR(errno, EWOULDBLOCK,
                       "second recv() recv_result %zd errno %d", recv_result,
                       errno);
