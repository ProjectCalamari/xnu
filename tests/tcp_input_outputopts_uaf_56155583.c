...
static void *setopt_thread(void *data) {
  int s = *(int *)data;
  ...
  while (!finished) {
    T_ASSERT_POSIX_SUCCESS(setsockopt(s, IPPROTO_IPV6, IPV6_2292PKTOPTIONS,
                                      optbuf, sizeof(optbuf)),
                           NULL);

    /* force an error to free: */
    T_ASSERT_EQ(setsockopt(s, IPPROTO_IPV6, IPV6_2292PKTOPTIONS, optbuf, 1), -1,
                NULL);
...
static void *connect_thread(void *data) {
  ...
  while (!finished) {
    T_ASSERT_POSIX_SUCCESS(s = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP),
                           NULL);
...
T_DECL(tcp_input_outputopts_uaf_56155583,
  ...
  T_ASSERT_POSIX_SUCCESS(fcntl(s, F_SETFL, fcntl(s, F_GETFL) | O_NONBLOCK),
                         NULL);
  T_SETUPEND;
