...
static size_t pac_exception_handler(__unused mach_port_t task,
  ...
  T_ASSERT_EQ(codes[0], (uint64_t)EXC_ARM_PAC_FAIL,
              "The subcode is EXC_ARM_PAC_FAIL");
...
static bool have_fpac(void) {
  uint64_t id_aa64isar1_el1;
  size_t id_aa64isar1_el1_size = sizeof(id_aa64isar1_el1);

  int err = sysctlbyname("machdep.cpu.sysreg_ID_AA64ISAR1_EL1",
                         &id_aa64isar1_el1, &id_aa64isar1_el1_size, NULL, 0);
  if (err) {
    return false;
  }
...
static int virtual_address_size(void) {
  int ret;
  size_t ret_size = sizeof(ret);

  int err =
      sysctlbyname("machdep.virtual_address_size", &ret, &ret_size, NULL, 0);
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_POSIX_SUCCESS(err, "sysctlbyname()");
  return ret;
}

static void *canonical_address(void *ptr) {
...
T_DECL(thread_set_state_corrupted_pc,
       "Test that ptrauth failures in thread_set_state() poison the respective "
       "register.",
  ...
  T_QUIET;
  T_ASSERT_EQ(err, KERN_SUCCESS, "Created thread");

  arm_thread_state64_t state;
  mach_msg_type_number_t count = ARM_THREAD_STATE64_COUNT;
  err = thread_get_state(mach_thread_self(), ARM_THREAD_STATE64,
                         (thread_state_t)&state, &count);
  T_QUIET;
  T_ASSERT_EQ(err, KERN_SUCCESS, "Got own thread state");

  void *corrupted_pc = (void *)((uintptr_t)state.__opaque_pc ^ 0x4);
  state.__opaque_pc = corrupted_pc;
  err = thread_set_state(thread, ARM_THREAD_STATE64, (thread_state_t)&state,
                         count);
  T_QUIET;
  T_ASSERT_EQ(err, KERN_SUCCESS,
              "Set child thread's PC to a corrupted pointer");

  err = thread_get_state(thread, ARM_THREAD_STATE64, (thread_state_t)&state,
                         &count);
  T_QUIET;
  T_ASSERT_EQ(err, KERN_SUCCESS, "Got child's thread state");
  T_EXPECT_NE(
      state.__opaque_pc, corrupted_pc,
      "thread_set_state() with a corrupted PC should poison the PC value");
  T_EXPECT_EQ(canonical_address(state.__opaque_pc),
              canonical_address(corrupted_pc),
              "thread_set_state() with a corrupted PC should preserve the "
              "canonical address bits");

  err = thread_terminate(thread);
  T_QUIET;
  T_EXPECT_EQ(err, KERN_SUCCESS, "Terminated thread");
#endif // __arm64e__
}

T_DECL(ptrauth_exception, "Test naked ptrauth failures.",
  ...
  void *func_ptr_stripped =
      ptrauth_strip(func_ptr, ptrauth_key_function_pointer);
  ...
  T_EXPECT_FALSE(mach_exc_caught,
                 "Authing valid pointer should not cause an exception");
  T_EXPECT_EQ(func_ptr_authed, func_ptr_stripped,
              "Valid pointer should auth to stripped value");
  ...
  T_ASSERT_TRUE(stat == SIGKILL,
                "Expect a PAC EXCEPTION to SIGKILL the process");
#endif // __arm64e__
}
