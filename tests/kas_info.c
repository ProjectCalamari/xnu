...
T_GLOBAL_META(T_META_NAMESPACE("xnu.kas_info"), T_META_CHECK_LEAKS(false),
              T_META_ASROOT(true));

static bool slide_enabled(void) {
...
static uint64_t kernel_slide(int selector) {
  uint64_t slide;
...
static void test_kas_info_invalid_args(int selector) {
  uint64_t slide;
  size_t size = 0;
  int err;
  ...
  T_ASSERT_POSIX_FAILURE(err, EINVAL, "kas_info %d with invalid size",
                         selector);
}

T_DECL(
    kernel_text_slide_invalid,
...
T_DECL(sptm_txm_text_slide,
  ...
  if (sptm_enabled()) {
    T_ASSERT_GT_ULLONG(
        sptm_slide, 0ULL,
        "Ensure the SPTM slide is non-zero on an SPTM-enabled system");
    T_ASSERT_GT_ULLONG(
        txm_slide, 0ULL,
        "Ensure the TXM slide is non-zero on an SPTM-enabled system");
  } else {
    /* When not running on an SPTM-enabled system, this selector should return
     * zero. */
    T_ASSERT_EQ(sptm_slide, 0ULL,
                "Ensure the SPTM slide is zero on a non-SPTM system");
    T_ASSERT_EQ(txm_slide, 0ULL,
                "Ensure the TXM slide is zero on a non-SPTM system");
  }
}

T_DECL(sptm_txm_text_slide_invalid,
       "ensures that kas_info handles invalid input to "
       "[SPTM|TXM]_TEXT_SLIDE_SELECTOR",
       T_META_TAG_VM_NOT_ELIGIBLE) {
  test_kas_info_invalid_args(KAS_INFO_SPTM_TEXT_SLIDE_SELECTOR);
  test_kas_info_invalid_args(KAS_INFO_TXM_TEXT_SLIDE_SELECTOR);
}

static char const *kernel_path(void) {
  ...
  dispatch_once(&once, ^{
    ...
    symbolicator =
        CSSymbolicatorCreateWithMachKernelFlagsAndNotification(flags, NULL);
    T_QUIET;
    ...
    T_QUIET;
    T_ASSERT_NOTNULL(path, "CSSymbolOwnerGetPath/CSSymbolicatorGetSymbolOwner");
  });
  return path;
}

static void disk_kernel_segments(uint64_t **segs_out, size_t *nsegs_out) {
...
static bool is_fileset_kc(void) {
  char uuid[1024];
  int err;
...
T_DECL(kernel_segment_location,
       "ensures that KAS_INFO_KERNEL_SEGMENT_LOCATION returns correct segment "
       "locations",
       T_META_TAG_VM_NOT_ELIGIBLE) {
  int err;
  ...
  T_ASSERT_POSIX_SUCCESS(
      err, "kas_info KAS_INFO_KERNEL_SEGMENT_VMADDR_SELECTOR for size");
  ...
  T_ASSERT_POSIX_SUCCESS(
      err, "kas_info KAS_INFO_KERNEL_SEGMENT_VMADDR_SELECTOR for data");

  T_LOG("Kernel has %zu segments on disk, %zu in memory:", disk_nsegs,
        mem_nsegs);
