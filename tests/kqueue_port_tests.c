...
static void send(mach_port_t send_port) {
  kern_return_t kr = 0;
  mach_msg_base_t msg = {
      .header =
          {
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "client mach_msg");
}

static kern_return_t receive(mach_port_t rcv_port) {
  mach_msg_base_t msg = {
      .header =
          {
  ...
  return mach_msg(&msg.header, MACH_RCV_MSG | MACH_RCV_TIMEOUT, 0,
                  msg.header.msgh_size, rcv_port, 5000, 0);
}

static void fill_kevent(struct kevent *ke, uint16_t action, mach_port_t mp) {
  *ke = (struct kevent){
...
#define TS(s)                                                                  \
  (struct timespec) { .tv_sec = s }

static void *pthread_async_do(void *arg) {
  void (^block)(void) = arg;
  block();
  Block_release(block);
...
static void pthread_async(void (^block)(void)) {
  pthread_t th;
  int rc;
...
static int kevent_attach_event(mach_port_t port, uint16_t flags,
                               uint32_t fflags, int *error) {
  int rc;
...
T_DECL(kqueue_machport_no_toggle_flags,
       "don't allow turnstile flags to be toggled for EVFILT_MACHPORT",
       T_META_TAG_VM_PREFERRED) {
  kern_return_t kr;
  int rc, error = 0;
  ...
  T_QUIET;
  T_EXPECT_EQ_INT(rc, 1, "registration failed");
  ...
  T_QUIET;
  T_EXPECT_EQ_INT(rc, 1, "registration failed");
  T_EXPECT_EQ_INT(error, EINVAL,
                  "cannot modify filter flag MACH_RCV_SYNC_PEEK");
}
