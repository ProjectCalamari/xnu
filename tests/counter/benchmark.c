...
int main(int argc, char **argv) {
  ...
  if (sysctlbyname("kern.development", &is_development_kernel,
                   &is_development_kernel_size, NULL, 0) != 0 ||
  ...
  printf("rate,loss\n");
  writes_per_second =
      globals.tg_args.num_writes / ((double)duration / kNumNanosecondsInSecond);
...
static void *writer(void *arg) {
  ...
  if (atomic_fetch_add(&(globals->tg_threads_ready), 1) ==
      globals->tg_args.n_threads - 1) {
  ...
  while (true) {
    unsigned long long remaining =
        atomic_fetch_sub(&(globals->tg_num_writes_remaining), value);
...
static uint64_t counter_read(test_variant_t variant) {
...
static void parse_arguments(int argc, char **argv, test_args_t *args) {
  ...
  if (strncasecmp(argv[current_argument], kScalableCounterArgument,
                  strlen(kScalableCounterArgument)) == 0) {
    args->variant = VARIANT_SCALABLE_COUNTER;
  } else if (strncasecmp(argv[current_argument], kAtomicCounterArgument,
                         strlen(kAtomicCounterArgument)) == 0) {
    args->variant = VARIANT_ATOMIC;
  } else if (strncasecmp(argv[current_argument], kRacyCounterArgument,
                         strlen(kRacyCounterArgument)) == 0) {
