...
static uint64_t sysctl_read(const char *name) {
  int result;
  uint64_t value;
  size_t size = sizeof(value);
  result = sysctlbyname(name, &value, &size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(result, "Read from %s", name);
  return value;
}

static void sysctl_write(const char *name, int64_t amount) {
  kern_return_t result;
  result = sysctlbyname(name, NULL, NULL, &amount, sizeof(int64_t));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(result, "Write to %s", name);
}

static void scalable_counter_add(int64_t amount) {
  sysctl_write("kern.scalable_counter_test_add", amount);
}

static void static_scalable_counter_add(int64_t amount) {
  sysctl_write("kern.static_scalable_counter_test_add", amount);
}

static int64_t scalable_counter_load(void) {
  return (int64_t)sysctl_read("kern.scalable_counter_test_load");
}

static int64_t static_scalable_counter_load(void) {
 ...
 * @param num_iterations How many times to bang on the counter. Each iteration
 * makes the counter bigger by 100.
 */
static void *background_scalable_counter_thread(void *num_iterations_ptr) {
...
static void darwin_test_fini_scalable_counter_test() {
  int ret = fini_scalable_counter_test();
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "fini_scalable_counter_test");
}

static void darwin_test_setup(void) {
  T_SETUPBEGIN;
  int dev_kernel = is_development_kernel();
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(dev_kernel, "sysctlbyname kern.development");
...
T_DECL(test_scalable_counters_single_threaded,
       "Test single threaded operations on scalable_counters",
...
T_DECL(test_static_counter, "Test staticly declared counter",
       T_META_ASROOT(true), T_META_TAG_VM_PREFERRED) {
  ...
  T_QUIET;
  T_EXPECT_EQ(static_scalable_counter_load(),
              (long long)kNumIterations + start_value,
              "Counter value is correct");
  T_END;
}

T_DECL(test_scalable_counters_multithreaded,
       "Test multi-threaded operations on scalable_counters",
       T_META_ASROOT(true), T_META_TAG_VM_PREFERRED) {
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(threads, "Out of memory");
  ...
  for (i = 0; i < kNumThreads; i++) {
    ret = pthread_create(&threads[i], &pthread_attr,
                         background_scalable_counter_thread, (void *)(i));
    T_QUIET;
  ...
  for (i = 0; i < kNumThreads; i++) {
    ...
    T_QUIET;
    T_ASSERT_EQ((ptrdiff_t)exit_code, (ptrdiff_t)0,
                "Background thread exited sucessfully.");
  }
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "pthread_attr_destroy");
  free(threads);
}
...
static unsigned int ncpu() {
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(result, "hw.npu");
