...
static int get_sched_pri(thread_t thread_port) {
  kern_return_t kr;
...
static int get_base_pri(thread_t thread_port) {
  kern_return_t kr;
...
static void turnstile_prim_lock(int type) {
  int ret;
  uint64_t tid;
  int in_val = type;
  pthread_threadid_np(NULL, &tid);
  T_LOG("sysctlbyname lock type %d called from thread %llu \n", type, tid);
  ret = sysctlbyname("kern.turnstiles_test_lock", NULL, 0, &in_val,
                     sizeof(in_val));
  T_LOG("sysctlbyname lock returned from thread %llu with value %d \n", tid,
        ret);
}

static void turnstile_prim_unlock(int type) {
  int ret;
  uint64_t tid;
  int in_val = type;
  pthread_threadid_np(NULL, &tid);
  T_LOG("sysctlbyname unlock type %d called from thread %llu \n", type, tid);
  ret = sysctlbyname("kern.turnstiles_test_unlock", NULL, 0, &in_val,
                     sizeof(in_val));
...
static void *chain_locking(void *args) {
...
static void *take_lock_check_priority(void *arg) {
  ...
  T_ASSERT_EQ(old_pri, 37,
              "thread(%llu) priority before acquiring the lock is %d\n", tid,
              old_pri);
  ...
  T_ASSERT_EQ(
      unboosted_pri, 37,
      "thread(%llu) priority after acquiring the lock (uncontended) is %d\n",
      tid, unboosted_pri);
  ...
  T_ASSERT_EQ(boosted_pri, 47,
              "thread(%llu) priority after contention by 47 thread is %d\n",
              tid, boosted_pri);
  ...
  T_ASSERT_EQ(after_unlock_pri, 37,
              "thread(%llu) priority after dropping lock is %d\n", tid,
              after_unlock_pri);

  return NULL;
}

static void *try_to_take_lock_and_unlock(void *arg) {
  uint64_t tid;
  int type = (int)arg;
...
static void *take_lock_and_exit(void *arg) {
  int old_pri = get_base_pri(mach_thread_self());
  int unboosted_pri;
  int boosted_pri;
  uint64_t tid;
...
static void *unlock_an_owner_exited_lock(void *arg) {
  uint64_t tid;
  int type = (int)arg;
  ...
  /* Unlock the test lock causing the turnstile code to call
...
static void test1(int type) {
  T_LOG("Test 1: test if lock contended by a UI thread boosts the owner to UI "
        "qos");

  /* Create a thread at IN and take lock */
  thread_create_at_qos(QOS_CLASS_USER_INITIATED, &take_lock_check_priority,
                       type);

  /* Create a thread at UI and try to take lock */
  thread_create_at_qos(QOS_CLASS_USER_INTERACTIVE, &try_to_take_lock_and_unlock,
                       type);
...
static void test2(int type) {
  T_LOG("Test 2: test if lock contended by a 2 UI thread boosts the owner to "
        "UI qos");
...
/*
 * Test 3: test if lock owner thread exiting without unlocking allows turnstile
 * to work correctly.
 */
static void test3(int type) {
  T_LOG("Test 3: test if lock owner thread exiting without unlocking allows "
        "turnstile to work correctly");
  ...
  /* Create a thread at UI and try to take lock */
  thread_create_at_qos(QOS_CLASS_USER_INTERACTIVE, &unlock_an_owner_exited_lock,
                       type);
...
/*
 * Test 4: test if a chain of user-space turnstile primitives followed by kernel
 * primitives works correctly.
 */
static void test4(void) {
  pthread_t threads[5] = {};
  struct thread_data data[5] = {};

  T_LOG("Test 4: test if a chain of user-space turnstile primitives followed "
        "by kernel primitives works correctly");
  ...
  /* Create a thread at priority 4 and take SYSCTL_TURNSTILE_TEST_KERNEL_DEFAULT
   * lock */
  data[0].pri_to_set = 4;
  data[0].lock1 =
      SYSCTL_TURNSTILE_TEST_KERNEL_DEFAULT; /* this should be not locked */
  data[0].lock2 = 0;
  ...
  /* Create a thread at priority 31 and take
   * SYSCTL_TURNSTILE_TEST_KERNEL_HASHTABLE lock followed by
   * SYSCTL_TURNSTILE_TEST_KERNEL_DEFAULT */
  data[1].pri_to_set = 31;
  data[1].lock1 =
      SYSCTL_TURNSTILE_TEST_KERNEL_HASHTABLE; /* this should be not locked */
  data[1].lock2 =
      SYSCTL_TURNSTILE_TEST_KERNEL_DEFAULT; /* this should be locked */
  data[1].sleep = 0; /* no need to sleep, everything should be pushing by the
                        time it acquires the lock */
  data[1].sched_pri_to_check = 60;
  data[1].base_pri_to_check = 31;
  pthread_create(&threads[1], NULL, chain_locking, (void *)&data[1]);
  sleep(2); /* give the thread time to acquire the lock */

  /* Create a thread at priority 40 and take
   * SYSCTL_TURNSTILE_TEST_USER_HASHTABLE lock followed by
   * SYSCTL_TURNSTILE_TEST_KERNEL_HASHTABLE */
  data[2].pri_to_set = 40;
  data[2].lock1 =
      SYSCTL_TURNSTILE_TEST_USER_HASHTABLE; /* this should be not locked */
  data[2].lock2 =
      SYSCTL_TURNSTILE_TEST_KERNEL_HASHTABLE; /* this should be locked */
  data[2].sleep = 0; /* no need to sleep, everything should be pushing by the
                        time it acquires the lock */
  ...
  /* Create a thread at priority 47 and take SYSCTL_TURNSTILE_TEST_USER_DEFAULT
   * lock followed by SYSCTL_TURNSTILE_TEST_USER_HASHTABLE */
  data[3].pri_to_set = 47;
  data[3].lock1 =
      SYSCTL_TURNSTILE_TEST_USER_DEFAULT; /* this should be not locked */
  data[3].lock2 =
      SYSCTL_TURNSTILE_TEST_USER_HASHTABLE; /* this should be locked */
  data[3].sleep = 0; /* no need to sleep, everything should be pushing by the
                        time it acquires the lock */
  ...
  data[4].lock1 =
      SYSCTL_TURNSTILE_TEST_USER_DEFAULT; /* this should be locked */
  data[4].lock2 = 0;
  data[4].sleep = 0; /* no need to sleep, nothing should be pushing by the time
...
/*
 * Test 5: test if a chain of user-space turnstile primitives interleaved by
 * kernel primitives works correctly.
 */
static void test5(void) {
  pthread_t threads[5] = {};
  struct thread_data data[5] = {};

  T_LOG("Test 5: test if a chain of user-space turnstile primitives "
        "interleaved by kernel primitives works correctly");
  ...
  /* Create a thread at priority 41 and take
   * SYSCTL_TURNSTILE_TEST_KERNEL_HASHTABLE lock followed by
  ...
  /* Create a thread at priority 47 and take SYSCTL_TURNSTILE_TEST_USER_DEFAULT
   * lock followed by SYSCTL_TURNSTILE_TEST_KERNEL_HASHTABLE */
  data[3].pri_to_set = 47;
  data[3].lock1 =
      SYSCTL_TURNSTILE_TEST_USER_DEFAULT; /* this should be not locked */
  data[3].lock2 =
...
T_DECL(turnstile_test, "Turnstile test", T_META_ASROOT(YES),
   ...
   * These tests are using a sysctl to lock a dummy kernel resource that uses
   * turnstile. However a thread holding a kernel push from turnstile should
   * never return in userspace, and rdar://problem/24194397 adds an assert for
   * it.
   */
  // test4();
  // test5();
