...
static int run_ping(char *dest, char *payload_size) {
  int ping_ret, pid;
  // eg ping -t 3 -c 2 -s 120 17.253.144.10
  char *ping_args[] = {"/sbin/ping", "-t",         "3",  "-c", "2",
                       "-s",         payload_size, dest, NULL};
...
static void require_internet(void) {
  int ret;
...
T_DECL(icmp_internet_root, "test small Internet pings as root",
       T_META_ASROOT(true), T_META_REQUIRES_NETWORK(true)) {
  int ret;
...
T_DECL(icmp_internet_non_root, "test small Internet pings as non-root",
       T_META_ASROOT(false), T_META_REQUIRES_NETWORK(true)) {
  int ret;
