...
static const char *reply_type_str(ReplyType rt) {
  switch (rt) {
...
static mach_voucher_t create_task_voucher(void) {
  ...
  kr = host_create_mach_voucher(
      ...
      sizeof(task_create_recipe), &voucher);
...
static void *handle_exceptions(void *arg) {
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "ool page allocation of %d bytes", PG_ALLOC);
  ...
  while (1) {
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "exception msg recv");
...
#pragma pack()
      ...
      reply->hdr.msgh_bits = MACH_MSGH_BITS_SET(MACH_MSG_TYPE_MOVE_SEND_ONCE, 0,
...
#pragma pack()
    ...
    if (voucher) {
      kr = mach_port_mod_refs(mach_task_self(), voucher, MACH_PORT_RIGHT_SEND,
                              -1);
      T_QUIET;
      T_ASSERT_MACH_SUCCESS(kr, "destroy voucher");
    }

    T_LOG("sending exception reply of type (%s)", reply_type_str(reply_type));
    kr = mach_msg_send((mach_msg_header_t *)reply_store);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "exception reply msg send");

    T_PASS("Successfully delivered exception reply message of type %s",
           reply_type_str(reply_type));
    T_END;
    return NULL;
  }
}
...
static void signal_handler(int sig, siginfo_t *sip __unused,
                           void *ucontext __unused) {
  if (sigmask(sig) &
      s_sigmask) { /* TODO: check that the fault was generated by us */
    siglongjmp(jb, sig);
  } else {
    siglongjmp(jb, -sig);
  }
...
static int handle_signals(void) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(sigaction(SIGTRAP, &sa, NULL), NULL);
  mask |= sigmask(SIGTRAP);

  T_QUIET;
  T_ASSERT_POSIX_ZERO(sigaction(SIGSEGV, &sa, NULL), NULL);
...
static void test_exc_reply_type(ReplyType reply_type) {
  kern_return_t kr;
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "allocate receive right");

  kr = mach_port_insert_right(me, ePort, ePort, MACH_MSG_TYPE_MAKE_SEND);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "insert right into port=[%d]", ePort);

  kr = thread_set_exception_ports(self, EXC_MASK_ALL, ePort, EXCEPTION_DEFAULT,
                                  THREAD_STATE_NONE);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "set exception ports on self=[%d], handler=[%d]",
                        self, ePort);
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(ta, "exception handler thread args allocation");
  ta->port = ePort;
  ta->rt = reply_type;

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      pthread_create(&handler_thread, &attr, handle_exceptions, (void *)ta),
      "pthread creation");
...
T_DECL(mach_exc_ReplyNoError, "exception server reply with no error",
       ...
       T_META_IGNORECRASHES(".*mach_exception_reply.*"),
...
T_DECL(mach_exc_ReplyWithReplyPortMove,
...
T_DECL(mach_exc_ReplyWithReplyPortCplxBit,
...
T_DECL(
    mach_exc_ReplyWithReplyPortMoveCplxBit,
...
T_DECL(mach_exc_ReplyWithOOLPort,
...
T_DECL(mach_exc_ReplyWithOOLDesc,
...
T_DECL(mach_exc_ReplyWithVoucherGarbage,
