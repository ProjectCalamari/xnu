...
static int sockv6_bind(int s, in_port_t port) {
...
static void sockv6_set_v6only(int s) {
  int on = 1;
  int ret;
...
static bool alloc_and_bind_ports(in_port_t port_start, in_port_t port_end,
                                 int bind_attempts) {
  int bound_count = 0;
  bool success = true;
  ...
  T_ASSERT_TRUE(bound_count == bind_attempts,
                "number of successful binds %d (out of %d)", bound_count,
                bind_attempts);
