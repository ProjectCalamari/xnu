...
T_GLOBAL_META(T_META_NAMESPACE("xnu.scheduler"), T_META_RUN_CONCURRENTLY(true),
...
static void set_nonrealtime(pthread_t thread) {
  kern_return_t kr;
  ...
  kr = thread_policy_set(target_thread, THREAD_STANDARD_POLICY,
                         (thread_policy_t)&pol, THREAD_STANDARD_POLICY_COUNT);
  T_ASSERT_MACH_SUCCESS(kr, "thread_policy_set(THREAD_STANDARD_POLICY)");
}

T_DECL(unentitled_work_intervals,
  ...
  ret = work_interval_create(&handle, WORK_INTERVAL_FLAG_GROUP |
  ...
  T_ASSERT_EQ(ret, -1,
              "work_interval_copy_port on non-joinable interval returns -1");

  ret =
      work_interval_notify(handle, now - 1000, now, now + 1000, now + 2000, 0);
  ...
  uint32_t flags[] = {
      WORK_INTERVAL_FLAG_JOINABLE | WORK_INTERVAL_FLAG_GROUP |
          WORK_INTERVAL_FLAG_UNRESTRICTED | WORK_INTERVAL_TYPE_DEFAULT,
  };
  ...
  for (uint32_t i = 0; i < sizeof(flags) / sizeof(flags[0]); i++) {
    ...
    ret = work_interval_notify(handle, now - 1000, now, now + 1000, now + 2000,
                               0);
    T_ASSERT_EQ(ret, -1,
                "work_interval_notify on non-joined thread returns -1");
    T_ASSERT_EQ(errno, EINVAL,
    ...
    T_ASSERT_POSIX_SUCCESS(
        ret, "work_interval_join_port, join the same interval after destroy");
    ...
    ret = work_interval_notify(handle, now - 1000, now, now + 1000, now + 2000,
                               0);
    T_ASSERT_POSIX_SUCCESS(
        ret, "work_interval_notify, on joined thread after destroy");
...
T_DECL(unentitled_work_interval_create_while_joined,
  ...
  uint32_t flags = WORK_INTERVAL_FLAG_JOINABLE | WORK_INTERVAL_FLAG_GROUP |
                   WORK_INTERVAL_FLAG_UNRESTRICTED | WORK_INTERVAL_TYPE_DEFAULT;
  ...
  T_ASSERT_POSIX_SUCCESS(
      ret, "work_interval_create, joinable, while already joined");

  ret = work_interval_copy_port(handle2, &port2);
  T_ASSERT_POSIX_SUCCESS(
      ret, "work_interval_copy_port, joinable, while already joined");

  ret = work_interval_join_port(port2);
  T_ASSERT_POSIX_SUCCESS(
      ret, "work_interval_join_port, joinable, while already joined");
...
T_DECL(work_interval_audio_unentitled, "unentitled work interval for audio") {
  ...
  uint32_t flags = WORK_INTERVAL_FLAG_GROUP | WORK_INTERVAL_FLAG_JOINABLE |
  ...
  ret = work_interval_join_port(port);
  T_EXPECT_POSIX_FAILURE(ret, EINVAL,
