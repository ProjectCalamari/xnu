...
static void skt_flow_add_del(bool expect_success, sa_family_t af, void *src,
                             void *dst, uint8_t protocol, uint16_t sport,
                             uint16_t dport) {
...
static void skt_flow_req_should_success(sa_family_t af, void *src, void *dst,
                                        uint8_t protocol, uint16_t sport,
                                        uint16_t dport) {
  skt_flow_add_del(true, af, src, dst, protocol, sport, dport);
}

static void skt_flow_req_should_fail(sa_family_t af, void *src, void *dst,
                                     uint8_t protocol, uint16_t sport,
                                     uint16_t dport) {
  skt_flow_add_del(false, af, src, dst, protocol, sport, dport);
}

static void skt_flow_req_low_latency(sa_family_t af, void *src, void *dst,
                                     uint8_t protocol, uint16_t sport,
                                     uint16_t dport) {
  ...
  /* add a regular flow */
  regular_flow_0 =
      sktu_create_nexus_flow(&handles, af, src, dst, protocol, sport, dport);
  ...
  assert(regular_flow_0->nfr.nfr_nx_port == regular_flow_1->nfr.nfr_nx_port);

  /* add a low-latency flow */
  sport++;
  dport++;
  low_latency_flow_0 = sktu_create_nexus_low_latency_flow(
      &handles, af, src, dst, protocol, sport, dport);
  ...
  sport++;
  dport++;
  low_latency_flow_1 = sktu_create_nexus_low_latency_flow(
      &handles, af, src, dst, protocol, sport, dport);
...
int skt_flow_req_ll_main(int argc, char *argv[]) {
  ifname = FETH0_NAME;
...
int skt_flow_config_main(int argc, char *argv[]) {
  ifname = FETH0_NAME;
  our_mask = sktc_make_in_addr(IN_CLASSC_NET);
  our_ip = sktc_feth0_in_addr();
  dst_ip = sktc_feth1_in_addr();
  ...
  T_LOG("add a flow\n");
  struct sktu_flow *flow;
  flow = sktu_create_nexus_flow(&handles, AF_INET, &our_ip, &dst_ip,
                                IPPROTO_TCP, 1234, 1234);
  assert(flow);
  ...
  ret =
      os_nexus_flow_set_wake_from_sleep(handles.fsw_nx_uuid, rand_uuid, false);
  assert(ret != 0);
  assert(errno == ENOENT);
  ...
  if (child_pid == 0) {
    ret = os_nexus_flow_set_wake_from_sleep(handles.fsw_nx_uuid, flow->uuid,
                                            false);
    exit(errno);
  } else {
    int child_status;
  ...
  ret =
      os_nexus_flow_set_wake_from_sleep(handles.fsw_nx_uuid, flow->uuid, false);
  assert(ret == 0);
  ...
  ret =
      os_nexus_flow_set_wake_from_sleep(handles.fsw_nx_uuid, flow->uuid, true);
  assert(ret == 0);
  ...
  ret = os_nexus_flow_set_wake_from_sleep(handles.netif_nx_uuid, flow->uuid,
                                          true);
  assert(ret != 0);
  assert(errno == EPERM);
...
int skt_flow_conn_idle_main(int argc, char *argv[]) {
  ifname = FETH0_NAME;
  our_mask = sktc_make_in_addr(IN_CLASSC_NET);
  our_ip = sktc_feth0_in_addr();
  dst_ip = sktc_feth1_in_addr();
  ...
  T_LOG("verify ENOENT with INVALID flow\n");
  ret =
      os_nexus_flow_set_connection_idle(handles.fsw_nx_uuid, rand_uuid, false);
  assert(ret != 0);
  assert(errno == ENOENT);
  ...
  if (child_pid == 0) {
    ret = os_nexus_flow_set_connection_idle(handles.fsw_nx_uuid, flow->uuid,
                                            false);
    exit(errno);
  } else {
    int child_status;
  ...
  ret =
      os_nexus_flow_set_connection_idle(handles.fsw_nx_uuid, flow->uuid, true);
  assert(ret == 0);
  ...
  ret =
      os_nexus_flow_set_connection_idle(handles.fsw_nx_uuid, flow->uuid, false);
  assert(ret == 0);
  ...
  T_LOG("verify EPERM with netif nexus\n");
  ret = os_nexus_flow_set_connection_idle(handles.netif_nx_uuid, flow->uuid,
                                          true);
  assert(ret != 0);
  assert(errno == EPERM);
...
int skt_flow_req_main(int argc, char *argv[]) {
  ifname = FETH0_NAME;
  our_mask = sktc_make_in_addr(IN_CLASSC_NET);
  our_ip = sktc_feth0_in_addr();
  dst_ip = sktc_feth1_in_addr();
  ...
  skt_flow_req_should_success(AF_INET, &our_ip, &dst_ip, IPPROTO_TCP, 1234,
                              1234);
  ...
  skt_flow_req_should_success(AF_INET, &zero_ip, &zero_ip, IPPROTO_TCP, 1234,
                              0);
  ...
  skt_flow_req_should_fail(AF_INET, &multicast_ip, &dst_ip, IPPROTO_TCP, 1234,
                           1234);
  // 5 tuple loopback
  skt_flow_req_should_fail(AF_INET, &our_ip, &lo_ip, IPPROTO_TCP, 1234, 1234);
  // 3 tuple invalid src ip
  skt_flow_req_should_fail(AF_INET, &nowhere_ip, &zero_ip, IPPROTO_TCP, 1234,
                           0);
  // 3 tuple multicast src ip
  skt_flow_req_should_fail(AF_INET, &multicast_ip, &zero_ip, IPPROTO_TCP, 1234,
                           0);
...
void skt_flow_req_net_init(void) {
  sktc_ifnet_feth_pair_create(FETH_FLAGS_TXSTART);
}

void skt_flow_req_net_fini(void) { sktc_ifnet_feth_pair_destroy(); }

struct skywalk_test skt_flow_req = {
    "flowreq",
    "test skywalk flow request api",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS,
    skt_flow_req_main,
    {NULL},
    skt_flow_req_net_init,
    skt_flow_req_net_fini,
};

struct skywalk_test skt_flow_req_ll = {
    "flowreqll",
    "test skywalk flow request api for low latency flows",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS |
        SK_FEATURE_DEV_OR_DEBUG,
    skt_flow_req_ll_main,
    {NULL},
    skt_flow_req_net_init,
    skt_flow_req_net_fini,
};
...
struct skywalk_test skt_flow_config = {
    "flowconfig",
    "test skywalk flow config api",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS,
    skt_flow_config_main,
    {NULL},
    skt_flow_req_net_init,
    skt_flow_req_net_fini,
};
...
struct skywalk_test skt_flow_conn_idle = {
    "flowconnidle",
    "test skywalk flow connection idle api",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS,
    skt_flow_conn_idle_main,
    {NULL},
    skt_flow_req_net_init,
    skt_flow_req_net_fini,
};
