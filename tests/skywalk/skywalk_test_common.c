...
static int sktc_ifnet_add_addr_with_socket(int s, char *ifname,
                                           struct in_addr *addr,
                                           struct in_addr *mask,
...
struct protox protox[] = {{NULL, arp_stats, NULL, "arp", 0}};

void sktc_setup_nexus(struct sktc_nexus_attr *sktc_attr) {
  nexus_attr_t attr;
  int error;
  ...
  error =
      os_nexus_attr_get(attr, NEXUS_ATTR_SLOT_BUF_SIZE, &sktc_nexus_slotsize);
  SKTC_ASSERT_ERR(!error);
  ...
  error =
      os_nexus_attr_get(attr, NEXUS_ATTR_SLOT_META_SIZE, &sktc_nexus_metasize);
  SKTC_ASSERT_ERR(!error);
  assert(sktc_nexus_metasize != -1);
  assert(sktc_attr->metasize == -1 ||
         sktc_attr->metasize == sktc_nexus_metasize);

  error = os_nexus_attr_get(attr, NEXUS_ATTR_MAX_FRAGS, &sktc_nexus_max_frags);
  SKTC_ASSERT_ERR(!error);
  ...
  error =
      os_nexus_attr_get(attr, NEXUS_ATTR_REJECT_ON_CLOSE, &sktc_rejectonclose);
  SKTC_ASSERT_ERR(!error);
...
int sktc_bind_nexus_key(nexus_port_t port, const void *key, size_t keylen) {
  return os_nexus_controller_bind_provider_instance(
      sktc_nexus_controller, sktc_instance_uuid, port, -1, NULL, key, keylen,
      NEXUS_BIND_KEY);
}

int sktc_unbind_nexus_key(nexus_port_t port) {
...
static void *sktc_channel_worker_echo(void *arg) {
  int index = (pthread_t *)arg - sktc_threads;
  ring_id_t ringid;
  channel_ring_t rxring, txring;
  struct kevent kev;
  int error;
  uint64_t ntxrings, nrxrings, ntxslots, nrxslots, slotsize, metasize;
  ...
  while (!sktc_channel_worker_die) {
    ...
    do {
      ...
      memcpy((void *)txprop.sp_buf_ptr, (void *)rxprop.sp_buf_ptr,
             rxprop.sp_len);
      txprop.sp_len = rxprop.sp_len;
      /* XXX: can't do this since it'll corrupt __user_quantum */
      // memcpy((void *)txprop.sp_mdata_ptr, (void *)rxprop.sp_mdata_ptr,
      // metasize);
...
static void *sktc_channel_worker_sink(void *arg) {
  int index = (pthread_t *)arg - sktc_threads;
  ring_id_t ringid;
...
static void *sktc_channel_worker_source(void *arg) {
  int index = (pthread_t *)arg - sktc_threads;
  ring_id_t ringid;
...
void sktc_setup_channel_worker(uuid_t instance_uuid, nexus_port_t channel_port,
                               ring_id_t ringid, char *key, size_t keylen,
                               bool echo, bool defunct_ok) {
  ...
  sktc_channel = sktu_channel_create_extended(
      instance_uuid, channel_port, CHANNEL_DIR_TX_RX, ringid, attr, -1, -1, -1,
      -1, -1, -1, defunct_ok ? 1 : -1, -1, -1);
  assert(sktc_channel);
  ...
  for (size_t i = 0; i < sktc_nrings; i++) {
    ...
    EV_SET(&kev, (uintptr_t)&sktc_channel_worker_die, EVFILT_USER,
    ...
    if (echo) {
      error = pthread_create(&sktc_threads[i], NULL, sktc_channel_worker_echo,
                             &sktc_threads[i]);
      SKTC_ASSERT_ERR(!error);
    } else {
      error = pthread_create(&sktc_threads[i], NULL, sktc_channel_worker_source,
                             &sktc_threads[i]);
      SKTC_ASSERT_ERR(!error);
      error = pthread_create(&sktc_threads2[i], NULL, sktc_channel_worker_sink,
                             &sktc_threads[i]);
      SKTC_ASSERT_ERR(!error);
    }
  }
...
void sktc_cleanup_channel_worker(void) {
  ...
  for (size_t i = 0; i < sktc_nrings; i++) {
    ...
    EV_SET(&kev, (uintptr_t)&sktc_channel_worker_die, EVFILT_USER, 0,
           NOTE_TRIGGER, 0, NULL);
    error = kevent(sktc_rxkqs[i], &kev, 1, NULL, 0, NULL);
    SKTC_ASSERT_ERR(!error);
    error = kevent(sktc_txkqs[i], &kev, 1, NULL, 0, NULL);
...
void sktc_generic_upipe_nexus_init(void) {
...
void sktc_generic_upipe_echo_init(void) {
  sktc_generic_upipe_nexus_init();
  sktc_setup_channel_worker(sktc_instance_uuid, 1, CHANNEL_RING_ID_ANY, NULL, 0,
                            true, true);
}

void sktc_generic_upipe_null_init(void) {
  sktc_generic_upipe_nexus_init();
  sktc_setup_channel_worker(sktc_instance_uuid, 1, CHANNEL_RING_ID_ANY, NULL, 0,
                            false, true);
}

void sktc_generic_upipe_fini(void) {
...
void sktc_generic_kpipe_init(void) {
  int enabled = 1;
  size_t len = sizeof(sktc_kpipe_loopback_was_enabled);

  assert(uuid_is_null(sktc_instance_uuid));
  sysctlbyname("kern.skywalk.kpipe.loopback.enabled",
               &sktc_kpipe_loopback_was_enabled, &len, &enabled,
               sizeof(enabled));

  len = sizeof(sktc_instance_uuid_string);
  sysctlbyname("kern.skywalk.kpipe.loopback.nx_uuid", sktc_instance_uuid_string,
               &len, NULL, 0);
...
void sktc_generic_kpipe_fini(void) {
  uuid_clear(sktc_instance_uuid);
  memset(sktc_instance_uuid_string, 0, sizeof(sktc_instance_uuid_string));
  sysctlbyname("kern.skywalk.kpipe.loopback.enabled", NULL, 0,
               &sktc_kpipe_loopback_was_enabled,
               sizeof(sktc_kpipe_loopback_was_enabled));
}

static int sktc_memory_test_was_enabled;

void sktc_generic_memory_init(void) {
  size_t len = sizeof(sktc_memory_test_was_enabled);
  int enabled = 1;

  sysctlbyname("kern.skywalk.mem.test", &sktc_memory_test_was_enabled, &len,
               &enabled, sizeof(enabled));
}

void sktc_generic_memory_fini(void) {
  sysctlbyname("kern.skywalk.mem.test", NULL, 0, &sktc_memory_test_was_enabled,
               sizeof(sktc_memory_test_was_enabled));
}

channel_slot_t send_bytes(channel_ring_t txring, uint32_t count) {
  int error;
...
int chew_slots(channel_ring_t rxring, uint32_t max) {
...
void set_watermark(channel_t channel, bool is_tx, channel_threshold_unit_t unit,
                   uint32_t value) {
...
int wait_on_fd(int kq, int16_t filter, channel_t channel, uint32_t lowat,
               enum timeout_behavior b) {
  ...
  EV_SET(&kev, 0, filter, EV_ADD | EV_ENABLE | EV_ONESHOT,
         (lowat > 0) ? NOTE_LOWAT : 0, lowat, NULL);
...
void test_stage_init(struct stage_ctx *stage, uint32_t start) {
  int error;
...
void test_stage_wait(struct stage_ctx *stage, uint32_t target) {
...
void test_stage_change(struct stage_ctx *stage, uint32_t new) {
  pthread_mutex_lock(&stage->change_mtx);
...
void test_stage_destroy(struct stage_ctx *stage) {
  int error;
...
static int inet_dgram_socket(void) {
  int s;
...
static int inet6_dgram_socket(void) {
  int s;
...
bool sktc_get_netif_nexus(const char *ifname, uuid_t netif) {
  bool found = FALSE;
...
bool sktc_get_flowswitch_nexus(const char *ifname, uuid_t netif) {
  bool found = FALSE;
  struct if_nexusreq ifnr;
  int s;
...
int sktc_get_mac_addr(const char *ifname, uint8_t *addr) {
  int s, err = 0;
  struct ifreq ifr;
...
static int sktc_ifnet_destroy(int s, const char *ifname) {
  int error = 0;
  struct ifreq ifr;
...
static int sktc_ifnet_set_flags(int s, const char *ifname, uint16_t flags_set,
  ...
  if (flags_before == flags_after) {
    ...
    if (ret != 0) {
      SKT_LOG("SIOCSIFFLAGS %s 0x%x: %s\n", ifr.ifr_name,
              (uint16_t)ifr.ifr_flags, strerror(errno));
    } else {
      T_LOG("setflags(%s set 0x%x clear 0x%x) "
            "0x%x => 0x%x\n",
            ifr.ifr_name, flags_set, flags_clear, flags_before, flags_after);
    }
  }
  return ret;
}
...
static int sktc_ifnet_create(int s, const char *ifname) {
  int error = 0;
  struct ifreq ifr;
...
static int sktc_ifnet_create_with_type(int s, const char *ifname,
                                       uint8_t type) {
  int error = 0;
...
int sktc_ifnet_feth_create(int unit) {
  int error = 0;
  int s;
...
int sktc_ifnet_feth_destroy(int unit) {
  int error = 0;
  int s;
...
void sktc_ifnet_feth0_create(void) {
  sktc_feth_set_flags(FETH_FLAGS_TXSTART);
  sktc_ifnet_feth_create(0);
  sktc_feth_restore_flags();
}

void sktc_ifnet_feth0_destroy(void) { sktc_ifnet_feth_destroy(0); }

void sktc_ifnet_feth1_create(void) {
  sktc_feth_set_flags(FETH_FLAGS_TXSTART);
  sktc_ifnet_feth_create(1);
  sktc_feth_restore_flags();
}

void sktc_ifnet_feth1_destroy(void) { sktc_ifnet_feth_destroy(1); }

void sktc_ifnet_feth0_1_create(void) {
  sktc_feth_set_flags(FETH_FLAGS_TXSTART);
  sktc_ifnet_feth_create(0);
  sktc_ifnet_feth_create(1);
  sktc_feth_restore_flags();
...
void sktc_ifnet_feth0_1_destroy(void) {
  sktc_ifnet_feth_destroy(0);
  sktc_ifnet_feth_destroy(1);
}

static int fake_set_dequeue_stall(const char *feth, boolean_t enable) {
  struct ifdrv ifd;
...
int sktc_ifnet_feth0_set_dequeue_stall(boolean_t enable) {
  return fake_set_dequeue_stall(FETH0_NAME, enable);
}

int sktc_ifnet_feth1_set_dequeue_stall(boolean_t enable) {
  return fake_set_dequeue_stall(FETH1_NAME, enable);
}

static void fake_set_peer(int s, const char *feth, const char *feth_peer) {
  struct ifdrv ifd;
  struct if_fake_request iffr;
  ...
  ifd.ifd_data = &iffr;
  if (feth_peer != NULL) {
    strlcpy(iffr.iffr_peer_name, feth_peer, sizeof(iffr.iffr_peer_name));
  }
  if (ioctl(s, SIOCSDRVSPEC, &ifd) < 0) {
    SKT_LOG("SIOCDRVSPEC set peer %s %s failed, %s\n", feth,
            (feth_peer != NULL) ? feth_peer : "<none>", strerror(errno));
  }
  return;
}
...
static void sktc_feth_set_flags(uint32_t flags) {
  ...
  error = sysctlbyname("net.link.fake.bsd_mode", &fake_bsd_mode_value, &len,
                       &bsd_mode, sizeof(bsd_mode));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.bsd_mode) failed, %s\n",
            strerror(errno));
  }
  SKTC_ASSERT_ERR(error == 0);
  ...
  error = sysctlbyname("net.link.fake.llink_cnt", &fake_llink_cnt_value, &len,
                       &llink_cnt, sizeof(llink_cnt));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.llink_cnt) failed, %s\n",
            strerror(errno));
  }
  SKTC_ASSERT_ERR(error == 0);
  ...
  error = sysctlbyname("net.link.fake.txstart", &fake_txstart_value, &len,
                       &txstart, sizeof(txstart));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.txstart) failed, %s\n",
            strerror(errno));
  }
  SKTC_ASSERT_ERR(error == 0);
  ...
  error = sysctlbyname("net.link.fake.wmm_mode", &fake_wmm_mode_value, &len,
                       &wmm_mode, sizeof(wmm_mode));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.wmm_mode) failed, %s\n",
            strerror(errno));
  }
  SKTC_ASSERT_ERR(error == 0);
  ...
  error = sysctlbyname("net.link.fake.multibuflet", &fake_multi_buflet_value,
                       &len, &multi_buflet, sizeof(multi_buflet));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.multibuflet) failed, %s\n",
            strerror(errno));
  }
  SKTC_ASSERT_ERR(error == 0);
  ...
  error = sysctlbyname("net.link.fake.user_access", &fake_user_access_value,
                       &len, &user_access, sizeof(user_access));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.user_access) failed, %s\n",
            strerror(errno));
  }
  SKTC_ASSERT_ERR(error == 0);
  ...
  if ((flags & FETH_FLAGS_LOW_LATENCY) != 0) {
    ...
    error = sysctlbyname("kern.skywalk.netif.sk_ll_prefix", fake_sk_ll_prefix,
                         &len, ifname, sizeof(ifname));
    if (error != 0) {
  ...
  assert(
      (flags & (FETH_FLAGS_NONSHAREDPOOL | FETH_FLAGS_NONSHAREDSPLITPOOLS)) !=
      (FETH_FLAGS_NONSHAREDPOOL | FETH_FLAGS_NONSHAREDSPLITPOOLS));
  ...
  error = sysctlbyname("net.link.fake.pktpool_mode", &fake_pktpool_mode_value,
                       &len, &pp_mode, sizeof(pp_mode));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.pktpool_mode) failed, %s\n",
            strerror(errno));
  }
...
#define FETH_HEADROOM_MUL_8_MAX 4
  ...
  error = sysctlbyname("net.link.fake.tx_headroom", &fake_tx_headroom_value,
                       &len, &tx_headroom, sizeof(tx_headroom));
  SKTC_ASSERT_ERR(error == 0);

  nxattach = ((flags & FETH_FLAGS_NXATTACH) != 0) ? 1 : 0;
  len = sizeof(fake_nxattach_value);
  error = sysctlbyname("net.link.fake.nxattach", &fake_nxattach_value, &len,
                       &nxattach, sizeof(nxattach));
  SKTC_ASSERT_ERR(error == 0);

  fcs = ((flags & FETH_FLAGS_FCS) != 0) ? 1 : 0;
  len = sizeof(fake_fcs_value);
  error = sysctlbyname("net.link.fake.fcs", &fake_fcs_value, &len, &fcs,
                       sizeof(fcs));
  SKTC_ASSERT_ERR(error == 0);
  ...
  error =
      sysctlbyname("net.link.fake.trailer_length", &fake_trailer_length_value,
...
static void sktc_feth_restore_flags(void) {
  ...
  error = sysctlbyname("net.link.fake.txstart", NULL, 0, &fake_txstart_value,
                       sizeof(fake_txstart_value));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.txstart) failed, %s\n",
            strerror(errno));
  }
  error = sysctlbyname("net.link.fake.bsd_mode", NULL, 0, &fake_bsd_mode_value,
                       sizeof(fake_bsd_mode_value));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.bsd_mode) failed, %s\n",
            strerror(errno));
  }
  error = sysctlbyname("net.link.fake.llink_cnt", NULL, 0,
                       &fake_llink_cnt_value, sizeof(fake_llink_cnt_value));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.llink) failed, %s\n", strerror(errno));
  }
  error = sysctlbyname("net.link.fake.wmm_mode", NULL, 0, &fake_wmm_mode_value,
                       sizeof(fake_wmm_mode_value));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.wmm_mode) failed, %s\n",
            strerror(errno));
  }
  error =
      sysctlbyname("net.link.fake.multibuflet", NULL, 0,
  ...
  error =
      sysctlbyname("net.link.fake.pktpool_mode", NULL, 0,
                   &fake_pktpool_mode_value, sizeof(fake_pktpool_mode_value));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.pktpool_mode) failed, %s\n",
            strerror(errno));
  }
  error = sysctlbyname("net.link.fake.tx_headroom", NULL, 0,
                       &fake_tx_headroom_value, sizeof(fake_tx_headroom_value));
  if (error != 0) {
    SKT_LOG("sysctlbyname(net.link.fake.tx_headroom) failed, %s\n",
            strerror(errno));
  }
  ...
  if (strlen(fake_sk_ll_prefix) != 0) {
    error = sysctlbyname("kern.skywalk.netif.sk_ll_prefix", NULL, 0,
                         &fake_sk_ll_prefix, sizeof(fake_sk_ll_prefix));
    if (error != 0) {
      SKT_LOG("sysctlbyname(sk_ll_prefix) failed, %s\n", strerror(errno));
    }
  }
  error = sysctlbyname("net.link.fake.nxattach", NULL, 0, &fake_nxattach_value,
                       sizeof(fake_nxattach_value));
  SKTC_ASSERT_ERR(error == 0);
  error = sysctlbyname("net.link.fake.fcs", NULL, 0, &fake_fcs_value,
                       sizeof(fake_fcs_value));
  SKTC_ASSERT_ERR(error == 0);
  error = sysctlbyname("net.link.fake.trailer_length", NULL, 0,
                       &fake_trailer_length_value,
                       sizeof(fake_trailer_length_value));
  SKTC_ASSERT_ERR(error == 0);
}

void sktc_ifnet_feth_pair_create(uint32_t flags) {
  int error;
  ...
  error =
      sktc_ifnet_add_addr_with_socket(s, FETH0_NAME, &feth0_addr, &mask, NULL);
  if (error != 0) {
    return;
  }
  ...
  error =
      sktc_ifnet_add_addr_with_socket(s, FETH1_NAME, &feth1_addr, &mask, NULL);
  if (error != 0) {
    return;
  }
...
void sktc_ifnet_feth_pair_destroy(void) {
  sktc_feth_restore_flags();
  sktc_ifnet_feth0_1_destroy();
}

static void redirect_set_delegate(int s, const char *redirect,
                                  const char *delegate) {
  struct ifdrv ifd;
  ...
  ifd.ifd_data = &iffr;
  if (delegate != NULL) {
    strlcpy(iffr.ifrr_delegate_name, delegate, sizeof(iffr.ifrr_delegate_name));
  }
  if (ioctl(s, SIOCSDRVSPEC, &ifd) < 0) {
    SKT_LOG("SIOCDRVSPEC set delegate %s %s failed, %s\n", redirect,
            (delegate != NULL) ? delegate : "<none>", strerror(errno));
  }
  return;
}

void sktc_ifnet_rd_create(void) {
  int error;
  ...
  error = sktc_ifnet_add_addr_with_socket(s, RD0_NAME, &rd_addr, &mask, NULL);
  if (error != 0) {
    return;
  }
...
void sktc_ifnet_rd_destroy(void) {
  int error = 0;
  int s;
...
static short sktc_ifnet_get_flags(int s, char *ifname) {
  struct ifreq ifr;
  u_int flags;
...
static int sktc_ifnet_add_addr_with_socket(int s, char *ifname,
  ...
  if (broadaddr != NULL || (addr != NULL && (sktc_ifnet_get_flags(s, ifname) &
                                             IFF_POINTOPOINT) != 0)) {
...
int sktc_ifnet_add_addr(char *ifname, struct in_addr *addr,
...
static void in6_len2mask(struct in6_addr *mask, int len) {
  int i;
...
static int sktc_ifnet_add_addr6_with_socket(int s, char *ifname,
...
int sktc_ifnet_add_addr6(char *ifname, struct in6_addr *addr,
  ...
  err = sktc_ifnet_add_addr6_with_socket(s, ifname, addr, dstaddr, prefix_len,
                                         flags);
  (void)close(s);
  return err;
}

int sktc_ifnet_del_addr(char *ifname, struct in_addr *addr) {
  struct sockaddr_in *sin;
  struct ifreq ifr;
  int s, err = 0;
...
int sktc_ifnet_del_addr6(char *ifname, struct in6_addr *addr) {
 ...
 */
#define ROUTE_MSG_ADDRS_SPACE                                                  \
  (3 * sizeof(struct sockaddr_in) + 2 * sizeof(struct sockaddr_dl) + 128)
typedef struct {
 ...
 */
static int IPv4RouteApply(IPv4RouteRef route, int cmd, int sockfd) {
  size_t len;
  int ret = 0;
  route_msg rtmsg;
  union {
...
static int open_routing_socket(void) {
  int sockfd;
...
int sktc_ifnet_add_scoped_default_route(char *ifname, struct in_addr ifa) {
  int error;
  IPv4Route route;
  int sockfd;
...
int sktc_set_classq_update_interval(uint64_t ns) {
  int error;
  char *sysctl_name = "net.classq.fq_codel.update_interval";

  error = sysctlbyname(sysctl_name, NULL, NULL, &ns, sizeof(ns));

  if (error != 0) {
    SKT_LOG("sysctlbyname(%s) failed, %s\n", sysctl_name, strerror(errno));
  }
  return error;
}

/* interval in nanoseconds */
int sktc_set_classq_update_intervals(uint64_t ns) {
  int error;
...
int sktc_reset_classq_update_interval() {
  return sktc_set_classq_update_interval(0);
}

int sktc_reset_classq_update_intervals(void) {
  return sktc_set_classq_update_intervals(0);
}

/* interval in nanoseconds */
int sktc_set_classq_target_qdelay(uint64_t ns) {
  int error;
...
/* interval in nanoseconds */
int sktc_set_classq_target_qdelays(uint64_t ns) {
  int error;
...
int sktc_reset_classq_target_qdelays(void) {
  return sktc_set_classq_target_qdelays(0);
}

static int sktc_tcp_msl;
/* interval in milliseconds */
void sktc_set_tcp_msl(int ms) {
  int error;
  size_t len = sizeof(sktc_tcp_msl);

  error =
      sysctlbyname("net.inet.tcp.msl", &sktc_tcp_msl, &len, &ms, sizeof(ms));

  if (error != 0) {
    SKT_LOG("sysctlbyname(net.inet.tcp.msl) failed, %s\n", strerror(errno));
  }
  SKTC_ASSERT_ERR(!error);
}

void sktc_restore_tcp_msl(void) {
  int error;

  error = sysctlbyname("net.inet.tcp.msl", NULL, 0, &sktc_tcp_msl,
                       sizeof(sktc_tcp_msl));
...
static void sktc_toggle_ip_reass(int new_value, int *old_value) {
  size_t old_value_size = sizeof(*old_value);
  const char *ip_reass_sysctl = "kern.skywalk.flowswitch.ip_reass";
  int error;

  error = sysctlbyname(ip_reass_sysctl, old_value,
                       old_value != NULL ? &old_value_size : NULL, &new_value,
                       sizeof(new_value));
  if (error) {
    SKT_LOG("sysctlbyname(%s) failed,%s\n", ip_reass_sysctl, strerror(errno));
  }
  SKTC_ASSERT_ERR(!error);
}

void sktc_enable_ip_reass() {
  sktc_toggle_ip_reass(1, &old_ip_reass_sysctl_value);
}

void sktc_restore_ip_reass() {
  assert(old_ip_reass_sysctl_value != -1);
  sktc_toggle_ip_reass(old_ip_reass_sysctl_value, NULL);
}

bool sktc_is_ip_reass_enabled() {
  int enabled;
...
bool sktc_is_netagent_enabled(void) {
  int enabled = 0;
  size_t len = sizeof(enabled);
  const char *enable_netagent_sysctl =
      "net.link.generic.system.enable_netagent";
  int error;
...
uint64_t sktc_get_channel_attr(const channel_t chd, channel_attr_type_t type) {
  channel_attr_t attr;
  uint64_t attrval = -1;
  int error;
...
void sktc_config_fsw_rx_agg_tcp(uint32_t agg) {
  int error;
...
void sktc_restore_fsw_rx_agg_tcp(void) {
  int error;

  if (sktc_fsw_rx_agg_tcp == (uint32_t)-1) {
    return;
  }
  error = sysctlbyname("kern.skywalk.flowswitch.rx_agg_tcp", NULL, 0,
                       &sktc_fsw_rx_agg_tcp, sizeof(sktc_fsw_rx_agg_tcp));
...
void sktc_enable_channel_buflet_alloc(void) {
  int error;
  uint32_t enable = 1;
  size_t len = sizeof(sktc_channel_buflet_alloc);

  error =
      sysctlbyname("kern.skywalk.chan_buf_alloc", &sktc_channel_buflet_alloc,
                   &len, &enable, sizeof(enable));
...
void sktc_restore_channel_buflet_alloc(void) {
  int error;

  if (sktc_channel_buflet_alloc == (uint32_t)-1) {
    return;
  }
  error = sysctlbyname("kern.skywalk.chan_buf_alloc", NULL, 0,
                       &sktc_channel_buflet_alloc,
...
void skt_process_if_adv(nexus_port_t port, channel_t chan) {
  int error;
...
static void skt_process_chan_event_common(
    channel_t chan, uint8_t payload_type, uint32_t stream_id,
    os_channel_event_type_t *captured_event_type, uint8_t *captured_event_data,
    size_t *captured_event_dlen, size_t max_event_dlen) {
  int error;
  uint32_t nevents;
  ...
  error = os_channel_get_next_event_handle(chan, &eh, &etype, &nevents);
  SKTC_ASSERT_ERR(error == 0);
...
#if SKT_COMMON_DEBUG
  T_LOG("chan event: packet: [%03hhu-%03hhu-%05hu-%05u-%05u-(%05u)] "
        "[%03hhu-%03hhu-xxxxx-xxxxx-%05u-(xxxxx)] event [%1u %03hu] [%1u "
        "%03hu]\n",
        packet_id->pktid_version, packet_id->pktid_payload_type,
        packet_id->pktid_sequence_number, packet_id->pktid_timestamp,
        packet_id->pktid_stream_identifier, packet_id->_reserved,

        (uint8_t)OS_PACKET_PKTID_VERSION_CURRENT, payload_type, stream_id,

        ed.event_type, ed.event_data_length, etype, (uint16_t)max_event_dlen);
#endif /* SKT_COMMON_DEBUG */
  assert(nevents == 1);
...
void skt_process_channel_event(
    channel_t chan, uint8_t payload_type, uint32_t stream_id,
  ...
  size_t event_dlen;
  skt_process_chan_event_common(chan, payload_type, stream_id, &event_type,
                                event_data, &event_dlen, sizeof(event_data));

#if SKT_COMMON_DEBUG
  T_LOG("expiry_event=%p len=%lu [%lu]\n", event_data, event_dlen,
        sizeof(event_data));
#endif /* SKT_COMMON_DEBUG */
...
static struct sockaddr_inarp *getaddr(struct in_addr host) {
...
static struct rt_msghdr *rtmsg(int cmd, struct sockaddr_inarp *dst,
...
int skt_add_arp_entry(struct in_addr host, struct ether_addr *eaddr) {
  ...
  *ea = *eaddr;
  ...
  for (;;) { /* try at most twice */
    ...
    if (sdl->sdl_family == AF_LINK && (rtm->rtm_flags & RTF_LLINFO) &&
        !(rtm->rtm_flags & RTF_GATEWAY) && valid_type(sdl->sdl_type)) {
      break;
    }
  }
...
static void update_avg(struct if_ifclassq_stats *ifcqs,
  ...
  if (b >= qs->prev_bytes) {
    qs->avg_bytes = ((qs->avg_bytes * (n - 1)) + (b - qs->prev_bytes)) / n;
  }

  if (p >= qs->prev_packets) {
    qs->avg_packets =
        ((qs->avg_packets * (n - 1)) + (p - qs->prev_packets)) / n;
  }
...
static char *nsec_to_str(unsigned long long nsec) {
  static char buf[32];
  const char *u;
...
void skt_aqstatpr(const char *interface) {
  ...
  os_log(OS_LOG_DEFAULT,
         ...
         interface, sched2str(ifcqs->ifqs_scheduler), ifcqs->ifqs_len,
         ifcqs->ifqs_maxlen);
  os_log(OS_LOG_DEFAULT,
         "     [ dequeued pkts: %10llu  bytes: %10llu "
  ...
  for (n = 0; n < IFCQ_SC_MAX && scheduler != PKTSCHEDT_NONE; n++) {
    ...
    switch (scheduler) {
    case PKTSCHEDT_FQ_CODEL:
      print_fq_codel_stats(n, &ifcqs->ifqs_fq_codel_stats, &qstats[n]);
      break;
    case PKTSCHEDT_NONE:
    default:
      break;
...
static void print_fq_codel_stats(int pri, struct fq_codel_classstats *fqst,
                                 struct queue_stats *qs) {
  int i = 0;
  ...
  os_log(OS_LOG_DEFAULT,
         "=====================================================\n");
  os_log(OS_LOG_DEFAULT,
         "     [ pri: %s (%u)\tsrv_cl: 0x%x\tquantum: %u\tdrr_max: %u ]\n",
         pri2str(fqst->fcls_pri), fqst->fcls_pri, fqst->fcls_service_class,
         fqst->fcls_quantum, fqst->fcls_drr_max);
  ...
  os_log(OS_LOG_DEFAULT,
         "     [ ce marked:%llu\tce mark failures:%llu\tce reported:%llu\tL4S "
         "pkts:%llu   ]\n",
         fqst->fcls_ce_marked, fqst->fcls_ce_mark_failures,
         fqst->fcls_ce_reported, fqst->fcls_l4s_pkts);
  os_log(OS_LOG_DEFAULT, "     [ flows total: %u\tnew: %u\told: %u ]\n",
         fqst->fcls_flows_cnt, fqst->fcls_newflows_cnt,
         fqst->fcls_oldflows_cnt);
  ...
  if (fqst->fcls_flowstats_cnt > 0) {
    ...
    for (i = 0; i < fqst->fcls_flowstats_cnt; i++) {
      ...
      if (fqst->fcls_flowstats[i].fqst_flags & FQ_FLOWSTATS_OLD_FLOW) {
        os_log(OS_LOG_DEFAULT, "O");
      }
      if (fqst->fcls_flowstats[i].fqst_flags & FQ_FLOWSTATS_NEW_FLOW) {
        os_log(OS_LOG_DEFAULT, "N");
      }
      if (fqst->fcls_flowstats[i].fqst_flags & FQ_FLOWSTATS_LARGE_FLOW) {
        os_log(OS_LOG_DEFAULT, "L");
      }
      if (fqst->fcls_flowstats[i].fqst_flags & FQ_FLOWSTATS_DELAY_HIGH) {
...
char *plural(int n) { return n > 1 ? "s" : ""; }

char *plurales(int n) { return n > 1 ? "es" : ""; }

char *pluralies(int n) { return n > 1 ? "ies" : "y"; }
...
static void arp_stats(uint32_t off, char *name, int af) {
  ...
  if (sysctlbyname("net.link.ether.inet.stats", &arpstat, &len, 0, 0) < 0) {
...
#define p(f, m) os_log(OS_LOG_DEFAULT, m, ARPDIFF(f), plural(ARPDIFF(f)))
#define p2(f, m) os_log(OS_LOG_DEFAULT, m, ARPDIFF(f), pluralies(ARPDIFF(f)))
#define p3(f, m)                                                               \
