...
static int test_block_with_leeway(int sec_leeway, test_block_t test_block) {
  int ret;
...
static void test_tcp_flow_send() {
  struct sktu_flow *flow;
  my_payload tx_payload;
  struct sktu_frame *tx_frame;

  flow = sktu_create_nexus_flow_with_nx_port(
      &handles, NX_PORT, AF_INET, &our_ip, &dst_ip, IPPROTO_TCP, 0, dport);
  ...
  flow->create_output_frames(flow, &tx_frame, 1, &tx_payload,
                             sizeof(tx_payload), NO_CSUM_OFFLOAD);
...
static void test_tcp_flow_receive() {
  struct sktu_flow *flow;
  ...
  flow->create_input_frames(flow, &tx_frame, 1, &tx_payload,
                            sizeof(tx_payload));
...
static void test_tcp_flow_listen(bool any_src_ip) {
  struct sktu_flow *listener, *connection;
  my_payload rx_payload, tx_payload;
  struct sktu_frame *rx_frame, *tx_frame;

  T_LOG("%s\n", __func__);
  listener = sktu_create_nexus_flow_with_nx_port(
      &handles, NX_PORT, AF_INET, any_src_ip ? &zero_ip : &our_ip, &zero_ip,
      IPPROTO_TCP, 0, 0);
  assert(listener);
  ...
  sktu_create_tcp_frames(&tx_frame, 1, IPVERSION, &dst_ip, &our_ip, dport,
                         sport, &tx_payload, sizeof(tx_payload), 1500,
                         NO_CSUM_OFFLOAD);
  sktu_utun_fd_tx_burst(utun_fd, &tx_frame, 1);
  ...
  connection = sktu_create_nexus_flow_with_nx_port(
      &handles, NX_PORT, AF_INET, &our_ip, &dst_ip, IPPROTO_TCP, sport, dport);
  assert(connection);

  bzero(&tx_payload, sizeof(tx_payload));
  tx_payload.packet_number = 1;
  strlcpy(tx_payload.data, "tcp_flow_conected", sizeof(tx_payload.data));
  connection->create_input_frames(connection, &tx_frame, 1, &tx_payload,
                                  sizeof(tx_payload));
  sktu_utun_fd_tx_burst(utun_fd, &tx_frame, 1);
...
static void test_tcp_flow_listen_v6() {
  struct sktu_flow *listener, *connection;
  my_payload rx_payload, tx_payload;
  struct sktu_frame *rx_frame, *tx_frame;
  ...
  T_LOG("%s\n", __func__);
  listener = sktu_create_nexus_flow_with_nx_port(
      &handles, NX_PORT, AF_INET6, &zero_ip_v6, &zero_ip_v6, IPPROTO_TCP, 0, 0);
  assert(listener);
  sport = listener->sport;
  dport = 1234;
...
static void test_ip_flow_send() {
  struct sktu_flow *ip_flow;
  my_payload tx_payload;
  struct sktu_frame *rx_frame, *tx_frame;

  T_LOG("%s\n", __func__);
  ip_flow = sktu_create_nexus_flow_with_nx_port(
      &handles, NX_PORT, AF_INET, &our_ip, &dst_ip, IPPROTO_IPEIP, 0, 0);
  assert(ip_flow);

  bzero(&tx_payload, sizeof(tx_payload));
  tx_payload.packet_number = 0;
  strlcpy(tx_payload.data, "ip_send", sizeof(tx_payload.data));
  ip_flow->create_output_frames(ip_flow, &tx_frame, 1, &tx_payload,
                                sizeof(tx_payload), false);
...
static void test_ip_flow_receive() {
  struct sktu_flow *ip_flow;
  my_payload tx_payload;
  struct sktu_frame *rx_frame, *tx_frame;
  ...
  ip_flow->create_input_frames(ip_flow, &tx_frame, 1, &tx_payload,
                               sizeof(tx_payload));
...
static void test_ip_flow_listen(bool any_src_ip) {
  struct sktu_flow *ip_listener, *ip_connection;
  my_payload rx_payload, tx_payload;
  struct sktu_frame *rx_frame, *tx_frame;

  T_LOG("%s\n", __func__);
  ip_listener = sktu_create_nexus_flow_with_nx_port(
      &handles, NX_PORT, AF_INET, any_src_ip ? &zero_ip : &our_ip, &zero_ip,
      IPPROTO_IPEIP, 0, 0);
  assert(ip_listener);
  ...
  ip_listener->create_input_frames(ip_listener, &tx_frame, 1, &tx_payload,
                                   sizeof(tx_payload));
  ...
  sktu_dump_buffer(stderr, "Received", &rx_frame->bytes, rx_frame->len);
  assert(uuid_compare(rx_frame->flow_uuid, ip_listener->nfr.nfr_flow_uuid) ==
         0);
  uint32_t rx_payload_len = 0;
  ...
  // connect a new flow based on the listener flow
  ip_connection = sktu_create_nexus_flow_with_nx_port(
      &handles, NX_PORT, AF_INET, &our_ip, &dst_ip, IPPROTO_IPEIP, 0, 0);
  ...
  ip_connection->create_input_frames(ip_connection, &tx_frame, 1, &tx_payload,
                                     sizeof(tx_payload));
  ...
  assert(ret == 1);
  sktu_dump_buffer(stderr, "Received", &rx_frame->bytes, rx_frame->len);
  assert(uuid_compare(rx_frame->flow_uuid, ip_connection->nfr.nfr_flow_uuid) ==
         0);
  ret = sktu_parse_ipv4_frame(rx_frame, &rx_payload, &rx_payload_len);
  assert(ret == 0);
  assert(rx_payload.packet_number == 1);
...
static void dev_init() {
  ...
  error = os_nexus_controller_bind_provider_instance(
      handles.controller, handles.fsw_nx_uuid, NX_PORT, getpid(), NULL, NULL, 0,
      NEXUS_BIND_PID);
...
int skt_flowlookup_main(int argc, char *argv[]) {
  atexit(dev_fini);
  dev_init();
