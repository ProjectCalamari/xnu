...
#define SKT_ETH_IPV6_UDP_HDR_LEN                                               \
  (sizeof(struct ether_header) + sizeof(struct ip6_hdr) + sizeof(struct udphdr))
...
static void init_expiration_deadline_mach(void) {
...
static void skt_add_netif_ipv6_flow(struct sktc_nexus_handles *handles,
                                    const struct in6_addr *our_ip,
                                    const struct in6_addr *peer_ip,
                                    uint16_t flags, nexus_port_t *nx_port) {
  int error;
  ...
  error =
      __os_nexus_flow_add(handles->controller, handles->netif_nx_uuid, &nfr);
...
static void skt_setup_netif_with_ipv6_flow(struct sktc_nexus_handles *handles,
                                           const char *ifname,
  ...
  assert(sktc_get_netif_nexus(handles->netif_ifname, handles->netif_nx_uuid));
  /*
  ...
  skt_add_netif_ipv6_flow(handles, our_ip, peer_ip, NXFLOWREQF_IPV6_ULA,
                          nx_port);
}

static size_t skt_netif_ipv6_udp_frame_populate(
    packet_t ph, struct ether_addr *src_mac, struct in6_addr *src_ip,
    uint16_t src_port, struct ether_addr *dst_mac, struct in6_addr *dst_ip,
...
static size_t skt_netif_ipv6_udp_frame_process(packet_t ph, void *data,
                                               size_t data_max) {
  buflet_t buflet;
...
static void skt_netif_channel_send(channel_port_t port, uint16_t src_port,
                                   struct ether_addr *dst_mac,
                                   struct in6_addr *dst_ip, uint16_t dst_port,
                                   netif_payload_t payload, int payload_length,
                                   uint32_t limit,
                                   void (^packet_prehook)(packet_t p)) {
  int error;
  channel_slot_t last_slot = NULL;
  packet_id_t pktid = {OS_PACKET_PKTID_VERSION_CURRENT,
                       NETIF_PKTID_PAYLOAD_TYPE,
                       0,
                       0,
                       NETIF_PKTID_STREAM_ID,
                       0};
  ...
  while (1) {
    ...
    frame_length = skt_netif_ipv6_udp_frame_populate(
        pkt, &port->mac_addr, &port->ip6_addr, src_port, dst_mac, dst_ip,
        dst_port, (void *)payload, payload_length);
...
static void skt_netif_channel_receive(int child, channel_port_t port,
  ...
  *pkts_dropped = 0;
  ...
  while (1) {
    ...
    (void)skt_netif_ipv6_udp_frame_process(pkt, &payload, sizeof(payload));
#if SKT_NETIF_DIRECT_DEBUG
...
static void skt_netif_send_and_receive(channel_port_t port, uint16_t src_port,
                                       struct ether_addr *dst_mac,
                                       struct in6_addr *dst_ip,
                                       uint16_t dst_port, uint32_t how_many,
                                       uint32_t batch_size, int child,
...
#define N_EVENTS_MAX 4
  ...
  uint32_t __block n_tx_expired_chan_events = 0,
                   __block n_tx_status_chan_events = 0;
  uint64_t __block total_tx_exp_notif_delay = 0,
                   __block total_tx_exp_prop_delay = 0;
    ...
    for (int i = 0; i < error; i++) {
      if (evlist[i].flags & EV_ERROR) {
        ...
        T_LOG("child %d: ev_filter %d, "
              "flags 0x%u fflags 0x%u data %" PRIxPTR "\n",
              child, evlist[i].filter, evlist[i].flags, evlist[i].fflags,
              evlist[i].data);
        if (err == EAGAIN) {
          break;
        }
      ...
      case EVFILT_NW_CHANNEL: {
        ...
        if ((evlist[i].fflags & NOTE_CHANNEL_EVENT) != 0) {
          skt_process_channel_event(
              port->chan, NETIF_PKTID_PAYLOAD_TYPE, NETIF_PKTID_STREAM_ID,
              ...
              ^(const os_channel_event_packet_transmit_expired_t *pkt_ev) {
                ...
                exp_notif_delay = mach_absolute_time() -
                                  pkt_ev->packet_tx_expiration_timestamp;
                total_tx_exp_notif_delay += exp_notif_delay;
                exp_prop_delay = pkt_ev->packet_tx_expiration_timestamp -
                ...
                T_LOG("tx_expired_event=%p "
                      ...
                      "exp=%lldm %lldns "
                      "notif=%lldm %lldns\n",
                      pkt_ev, pkt_ev->packet_tx_expiration_status_code,
                      pkt_ev->packet_tx_expiration_deadline,
                      pkt_ev->packet_tx_expiration_timestamp, exp_prop_delay,
                      SKT_NETIF_TIMESTAMP_MACH_TO_NS(exp_prop_delay),
                      exp_notif_delay,
                      SKT_NETIF_TIMESTAMP_MACH_TO_NS(exp_notif_delay));
      ...
      case EVFILT_READ: {
        skt_netif_channel_receive(child, port, how_many, &receive_packet_count,
      ...
      case EVFILT_WRITE: {
        ...
        if (event_flags & SKT_NETIF_DIRECT_EVFLAG_EXPIRY) {
          packet_prehook = ^(packet_t pkt) {
            uint64_t packet_expire_time =
                mach_absolute_time() + expiration_deadline_mach;
            SKTC_ASSERT_ERR(
                os_packet_set_expire_time(pkt, packet_expire_time) == 0);
          };
        }
        ...
        skt_netif_channel_send(port, src_port, dst_mac, dst_ip, dst_port,
                               &payload, sizeof(payload), next_batch,
                               packet_prehook);
  ...
  T_LOG("child %d: received %u ifadv events, %u chan events\n", child,
        n_ifadv_events, n_total_chan_events);
  T_LOG("child %d: received %u tx status events, %u tx expired events %u total "
        "events\n",
        child, n_tx_status_chan_events, n_tx_expired_chan_events,
        n_total_chan_events);

  if (n_tx_expired_chan_events > 0) {
    int64_t total_tx_exp_notif_delay_ns =
        SKT_NETIF_TIMESTAMP_MACH_TO_NS(total_tx_exp_notif_delay);
    int64_t total_tx_exp_prop_delay_ns =
        SKT_NETIF_TIMESTAMP_MACH_TO_NS(total_tx_exp_prop_delay);
    int64_t avg_tx_exp_prop_delay_ns =
        total_tx_exp_prop_delay_ns / n_tx_expired_chan_events;
    int64_t avg_tx_notif_delay_ns =
        total_tx_exp_notif_delay_ns / n_tx_expired_chan_events;
...
int skt_netifdirect_xfer_ipv6(int child, uint32_t test_id) {
  ...
  skt_setup_netif_with_ipv6_flow(&handles, ifname, &our_ip6, &peer_ip6,
                                 &nx_port);
  sktu_channel_port_init(&port, handles.netif_nx_uuid, nx_port, true,
                         (test_id == SKT_NETIF_DIRECT_TEST_CHANNEL_EVENTS ||
                          test_id == SKT_NETIF_DIRECT_TEST_EXPIRY_EVENTS)
                             ? true
                             : false,
                         false);
  ...
  case SKT_NETIF_DIRECT_TEST_EXPIRY_EVENTS: {
    event_flags |=
        (SKT_NETIF_DIRECT_EVFLAG_EXPIRY | SKT_NETIF_DIRECT_EVFLAG_CHANNEL);
    break;
  }
  default:
  ...
  skt_netif_send_and_receive(&port, our_port, &peer_mac, &peer_ip6, peer_port,
                             NETIF_TXRX_PACKET_COUNT, NETIF_TXRX_BATCH_COUNT,
                             child, event_flags, ifadv_enabled);
  return 0;
}

static int skt_netifdirect_main(int argc, char *argv[]) {
  int child, test_id;
...
static void skt_netifdirect_init_native_user_access(void) {
  ...
  sktc_ifnet_feth_pair_create(FETH_FLAGS_NATIVE | FETH_FLAGS_USER_ACCESS |
                              FETH_FLAGS_LOW_LATENCY | FETH_FLAGS_LLINK);
}

static void skt_netifdirect_init_native_user_access_splitpools(void) {
  uint32_t nxctl_check = 1;
  size_t len = sizeof(skt_netif_nxctl_check);
...
static void skt_netifdirect_init_ifadv(void) {
...
static void skt_netifdirect_init_chan_events(void) {
...
static void skt_netifdirect_init_expiry_events(void) {
  int tx_exp_policy = 1; /* IFF_TX_EXP_POLICY_DROP_AND_NOTIFY */
  assert(sysctlbyname("net.link.fake.tx_exp_policy", NULL, 0, &tx_exp_policy,
                      sizeof(tx_exp_policy)) == 0);
  skt_netifdirect_init_native_user_access();
}

static void skt_netifdirect_fini(void) {
  assert(sysctlbyname("kern.skywalk.disable_nxctl_check", NULL, NULL,
                      &skt_netif_nxctl_check,
                      sizeof(skt_netif_nxctl_check)) == 0);
  sktc_ifnet_feth_pair_destroy();
}

static void skt_netifdirect_fini_ifadv(void) {
  int intvl = 0; /* disable */
  assert(sysctlbyname("net.link.fake.if_adv_intvl", NULL, 0, &intvl,
                      sizeof(intvl)) == 0);
...
static void skt_netifdirect_fini_chan_events(void) {
  int drops = 0;
  assert(sysctlbyname("net.link.fake.tx_drops", NULL, 0, &drops,
                      sizeof(drops)) == 0);
  skt_netifdirect_fini();
...
static void skt_netifdirect_fini_expiry_events(void) {
...
static void skt_netifdirect_init_native_copy_mode(void) {
  uint32_t nxctl_check = 1;
  size_t len = sizeof(skt_netif_nxctl_check);
...
struct skywalk_mptest skt_netifdirecttxrxcopymode = {
    ...
    skt_netifdirect_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_NETIF_DIRECT_TEST_TXRX)},
    skt_netifdirect_init_native_copy_mode,
    skt_netifdirect_fini,
    {},
};
...
struct skywalk_mptest skt_netifdirectifadvenable = {
    ...
    2,
    skt_netifdirect_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_NETIF_DIRECT_TEST_IF_ADV_ENABLED)},
    skt_netifdirect_init_ifadv,
    skt_netifdirect_fini_ifadv,
    {},
};
...
struct skywalk_mptest skt_netifdirectifadvdisable = {
    ...
    2,
    skt_netifdirect_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_NETIF_DIRECT_TEST_IF_ADV_DISABLED)},
    skt_netifdirect_init_ifadv,
    skt_netifdirect_fini_ifadv,
    {},
};
...
struct skywalk_mptest skt_netifdirectchanevents = {
    ...
    2,
    skt_netifdirect_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_NETIF_DIRECT_TEST_CHANNEL_EVENTS)},
    skt_netifdirect_init_chan_events,
    skt_netifdirect_fini_chan_events,
    {},
};
...
struct skywalk_mptest skt_netifdirectexpiryevents = {
    ...
    2,
    skt_netifdirect_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_NETIF_DIRECT_TEST_EXPIRY_EVENTS)},
    skt_netifdirect_init_expiry_events,
    skt_netifdirect_fini_expiry_events,
    {},
};
