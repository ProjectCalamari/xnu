...
/* This file contains useful utility routines, but contrary to
 * skywalk_test_common Do not operate on a single set of static objects
 */
...
void sktc_build_nexus(nexus_controller_t ncd, struct sktc_nexus_attr *sktc_attr,
  ...
  if (sktc_attr->anonymous != -1) {
    error = os_nexus_attr_set(attr, NEXUS_ATTR_ANONYMOUS, sktc_attr->anonymous);
  ...
  if (sktc_attr->ntxrings != -1) {
    error = os_nexus_attr_set(attr, NEXUS_ATTR_TX_RINGS, sktc_attr->ntxrings);
    SKTC_ASSERT_ERR(!error);
  }
  if (sktc_attr->nrxrings != -1) {
    error = os_nexus_attr_set(attr, NEXUS_ATTR_RX_RINGS, sktc_attr->nrxrings);
    SKTC_ASSERT_ERR(!error);
  }
  if (sktc_attr->ntxslots != -1) {
    error = os_nexus_attr_set(attr, NEXUS_ATTR_TX_SLOTS, sktc_attr->ntxslots);
    SKTC_ASSERT_ERR(!error);
  }
  if (sktc_attr->nrxslots != -1) {
    error = os_nexus_attr_set(attr, NEXUS_ATTR_RX_SLOTS, sktc_attr->nrxslots);
    SKTC_ASSERT_ERR(!error);
  }
  if (sktc_attr->slotsize != -1) {
    error =
        os_nexus_attr_set(attr, NEXUS_ATTR_SLOT_BUF_SIZE, sktc_attr->slotsize);
    SKTC_ASSERT_ERR(!error);
  }
  if (sktc_attr->metasize != -1) {
    error =
        os_nexus_attr_set(attr, NEXUS_ATTR_SLOT_META_SIZE, sktc_attr->metasize);
    SKTC_ASSERT_ERR(error == ENOTSUP);
  }
  if (sktc_attr->maxfrags != -1) {
    error = os_nexus_attr_set(attr, NEXUS_ATTR_MAX_FRAGS, sktc_attr->maxfrags);
    SKTC_ASSERT_ERR(!error);
  }
  ...
  error = os_nexus_controller_register_provider(
      ncd, sktc_attr->name, sktc_attr->type, attr, providerp);
  SKTC_ASSERT_ERR(!error);
  ...
  error = os_nexus_controller_read_provider_attr(ncd, *providerp, attr);
  SKTC_ASSERT_ERR(!error);
  ...
  SKTC_ASSERT_ERR(!error);
  assert(scratch != -1);
  assert(sktc_attr->userchannel == -1 || sktc_attr->userchannel == scratch);
  ...
  SKTC_ASSERT_ERR(!error);
  assert(scratch != -1);
  assert(sktc_attr->rejectonclose == -1 || sktc_attr->rejectonclose == scratch);
  ...
  if (instancep) {
    ...
    error =
        os_nexus_controller_alloc_provider_instance(ncd, *providerp, instancep);
    SKTC_ASSERT_ERR(!error);
...
channel_t sktu_channel_create_extended(
    const uuid_t uuid, const nexus_port_t port, const ring_dir_t dir,
    const ring_id_t rid, const channel_attr_t attr, uint64_t exclusive,
    uint64_t txlowatunit, uint64_t txlowatval, uint64_t rxlowatunit,
    uint64_t rxlowatval, uint64_t userpacketpool, uint64_t defunctok,
  ...
  if (txlowatunit != -1) {
    error =
        os_channel_attr_set(tmpattr, CHANNEL_ATTR_TX_LOWAT_UNIT, txlowatunit);
    SKTC_ASSERT_ERR(!error);
  }

  if (txlowatval != -1) {
    error =
        os_channel_attr_set(tmpattr, CHANNEL_ATTR_TX_LOWAT_VALUE, txlowatval);
    SKTC_ASSERT_ERR(!error);
  }

  if (rxlowatunit != -1) {
    error =
        os_channel_attr_set(tmpattr, CHANNEL_ATTR_RX_LOWAT_UNIT, rxlowatunit);
    SKTC_ASSERT_ERR(!error);
  }

  if (rxlowatval != -1) {
    error =
        os_channel_attr_set(tmpattr, CHANNEL_ATTR_RX_LOWAT_VALUE, rxlowatval);
    SKTC_ASSERT_ERR(!error);
  }

  if (userpacketpool != -1) {
    error = os_channel_attr_set(tmpattr, CHANNEL_ATTR_USER_PACKET_POOL,
                                userpacketpool);
    SKTC_ASSERT_ERR(!error);
  }
  ...
  if (defunctok != -1) {
    error =
        os_channel_attr_set(tmpattr, CHANNEL_ATTR_NEXUS_DEFUNCT_OK, defunctok);
    SKTC_ASSERT_ERR(!error);
  }
...
static inline void swap(int *permute, int i, int j) {
  int tmp = permute[i];
  permute[i] = permute[j];
  permute[j] = tmp;
}
      ...
      count++;
      {
        ...
        if (now > then) {
          T_LOG("time %ld on %d of %d (%2.2f%%, est %ld secs left)\n",
                now - start, count, total, (double)count * 100 / total,
...
void permutefuncR(int n, int *permute, void (*func)(int, int *permute),
...
void sktc_create_flowswitch_no_address(struct sktc_nexus_handles *handles,
                                       uint64_t ntxslots, uint64_t nrxslots,
                                       uint64_t buf_size, uint64_t max_frags,
                                       uint64_t anonymous) {
  char buf[256];
  int error;
  ...
  if (!sktc_get_netif_nexus(handles->netif_ifname, handles->netif_nx_uuid)) {
    ...
    attr.ntxslots = -1;
    attr.nrxslots = -1;
    sktc_build_nexus(handles->controller, &attr, &handles->netif_prov_uuid,
                     &handles->netif_nx_uuid);
    error = __os_nexus_ifattach(handles->controller, handles->netif_nx_uuid,
                                handles->netif_ifname, NULL, false,
                                &handles->netif_nx_attach_uuid);
    if (error != 0) {
      SKT_LOG("__os_nexus_ifattach(%s) failed, %s (%d)\n", buf, strerror(errno),
              errno);
      return;
    }
  }
  error = __os_nexus_ifattach(handles->controller, handles->fsw_nx_uuid, NULL,
                              handles->netif_nx_uuid, false,
                              &handles->fsw_nx_dev_attach_uuid);
  if (error != 0) {
...
void sktc_nexus_handles_assign_address(struct sktc_nexus_handles *handles) {
  ...
  error = sktc_ifnet_add_addr(handles->netif_ifname, &handles->netif_addr,
...
void sktc_create_flowswitch(struct sktc_nexus_handles *handles, int i) {
  ...
  /* assign the name */
  snprintf(handles->netif_ifname, sizeof(handles->netif_ifname), FETH_FORMAT,
           i);
...
int sktc_bind_tcp4_flow(nexus_controller_t ncd, const uuid_t fsw,
                        in_port_t in_port, nexus_port_t nx_port,
                        const uuid_t flow) {
...
int sktc_unbind_flow(nexus_controller_t ncd, const uuid_t fsw,
                     const uuid_t flow) {
  struct nx_flow_req nfr;
  int error;
...
uint32_t sktc_chew_random(channel_t channel, channel_ring_t ring,
                          sync_mode_t mode, bool dosync, uint32_t nslots) {
  uint64_t count = 0;
  int error;
  channel_slot_t slot;
...
void sktc_pump_ring_nslots_kq(channel_t channel, channel_ring_t ring,
                              sync_mode_t mode, bool dosync, uint64_t nslots,
                              bool verbose) {
  uint64_t count = 0;
  int channelfd;
  int kq;
  ...
  kq = kqueue();
  assert(kq != -1);
  EV_SET(&kev, channelfd, mode == CHANNEL_SYNC_TX ? EVFILT_WRITE : EVFILT_READ,
  ...
  while (count < nslots) {
    ...
    if (verbose) {
      ...
      if (now > then) {
        T_LOG("time %ld pump %" PRId64 " of %" PRId64
              " (%2.2f%%, est %ld secs left)\n",
              now - start, count, nslots, (double)count * 100 / nslots,
    ...
    count += sktc_chew_random(channel, ring, mode, dosync,
                              MIN(nslots - count, avail));
  }

  if (verbose) {
    time_t now = time(NULL);
    T_LOG("total time %ld for %" PRId64 " slots (rate %.2f)\n", now - start,
          nslots, (double)nslots / (now - start));
  }
...
void sktc_pump_ring_nslots_select(channel_t channel, channel_ring_t ring,
                                  sync_mode_t mode, bool dosync,
                                  uint64_t nslots, bool verbose) {
  uint64_t count = 0;
  int channelfd;
...
void sktc_pump_ring_nslots_poll(channel_t channel, channel_ring_t ring,
                                sync_mode_t mode, bool dosync, uint64_t nslots,
                                bool verbose) {
  uint64_t count = 0;
  int channelfd;
...
int sktu_create_interface(sktu_if_type_t type, sktu_if_flag_t flags) {
   ...
   * <rdar://problem/37340313> creating an interface without specifying specific
   * interface name should not return EBUSY
   */
  ...
  for (int i = 0; i < 10; i++) {
    ...
    error =
        bind(tunsock, (struct sockaddr *)&kernctl_addr, sizeof(kernctl_addr));
    ...
    error = setsockopt(tunsock, SYSPROTO_CONTROL, OPT_ENABLE_NETIF,
                       &enable_netif, sizeof(enable_netif));
    SKTC_ASSERT_ERR(!error);
    socklen_t scratchlen = sizeof(scratch);
    error = getsockopt(tunsock, SYSPROTO_CONTROL, OPT_ENABLE_NETIF, &scratch,
                       &scratchlen);
    SKTC_ASSERT_ERR(!error);
    assert(scratchlen == sizeof(scratch));
    assert(enable_netif == scratch);

    error = setsockopt(tunsock, SYSPROTO_CONTROL, OPT_ENABLE_CHANNEL,
                       &enable_channel, sizeof(enable_channel));
    SKTC_ASSERT_ERR(!error);
    scratchlen = sizeof(scratch);
    error = getsockopt(tunsock, SYSPROTO_CONTROL, OPT_ENABLE_CHANNEL, &scratch,
                       &scratchlen);
    SKTC_ASSERT_ERR(!error);
    assert(scratchlen == sizeof(scratch));
    ...
    if (type == SKTU_IFT_UTUN) {
      error = setsockopt(tunsock, SYSPROTO_CONTROL, OPT_ATTACH_FSW, &attach_fsw,
                         sizeof(attach_fsw));
      SKTC_ASSERT_ERR(!error);
    }

    error = connect(tunsock, (struct sockaddr *)&kernctl_addr,
                    sizeof(kernctl_addr));
    if (error == -1 && errno == EBUSY) {
...
channel_t sktu_create_interface_channel(sktu_if_type_t type, int tunsock) {
  ...
  uuidlen = sizeof(uuid);
  error = getsockopt(tunsock, SYSPROTO_CONTROL, OPT_GET_CHANNEL_UUID, uuid,
                     &uuidlen);
  SKTC_ASSERT_ERR(error == 0);
  ...
  channel = sktu_channel_create_extended(
      uuid, NEXUS_PORT_KERNEL_PIPE_CLIENT, CHANNEL_DIR_TX_RX,
      CHANNEL_RING_ID_ANY, attr, -1, -1, -1, -1, -1, -1, 1, -1, -1);
  assert(channel);
...
void sktu_dump_buffer(FILE *f, const char *desc, const void *buf, size_t len) {
  int i;
...
int sysctl_buf(char *oid_name, void **buffer, size_t *len, void *newp,
               size_t newlen) {
  int ret, err;
  int try = 0;
...
uint32_t sktu_set_inject_error_rmask(uint32_t *mask) {
  uint32_t old_mask;
  size_t size = sizeof(old_mask);
  int error;

  error = sysctlbyname("kern.skywalk.inject_error_rmask", &old_mask, &size,
...
void sktu_create_sa(int keysock, const char ifname[IFXNAMSIZ], uint32_t spi,
  ...
  memset(addcmd.ipsecif.sadb_x_ipsecif_internal_if, 0,
         sizeof(addcmd.ipsecif.sadb_x_ipsecif_internal_if));
  memset(addcmd.ipsecif.sadb_x_ipsecif_outgoing_if, 0,
         sizeof(addcmd.ipsecif.sadb_x_ipsecif_outgoing_if));
  strlcpy(addcmd.ipsecif.sadb_x_ipsecif_ipsec_if, ifname,
          sizeof(addcmd.ipsecif.sadb_x_ipsecif_ipsec_if));
  ...
  addcmd.src.addr.sadb_address_prefixlen = sizeof(struct in_addr)
                                           << 3; // XXX Why?
  ...
  addcmd.dst.addr.sadb_address_prefixlen = sizeof(struct in_addr)
                                           << 3; // XXX Why?
...
static __inline__ void reduce(int *sum) {
  long_union_t l_util;
...
unsigned short in_cksum(void *pkt, int len, int sum0) {
  u_short *w;
  int sum = sum0;
...
uint16_t in_pseudo(uint32_t a, uint32_t b, uint32_t c) {
  uint64_t sum;
  union q_util q_util;
  union l_util l_util;
...
void sktu_channel_port_init(channel_port_t ch_port, uuid_t instance,
                            nexus_port_t nx_port, bool enable_upp,
                            bool enable_event_ring, bool low_latency) {
  channel_t chan;
  ...
  chan = sktu_channel_create_extended(
      instance, port, CHANNEL_DIR_TX_RX, CHANNEL_RING_ID_ANY, NULL, -1, -1, -1,
      -1, -1, enable_upp ? 1 : -1, 1, enable_event_ring ? 1 : -1,
      low_latency ? 1 : -1);
  if (chan == NULL) {
...
static inline uint16_t sktu_fold_sum_final(uint32_t sum) {
...
packet_t sktu_channel_port_frame_to_pkt(channel_port_t port,
  ...
  if (frame->csum_flags != 0) {
    os_packet_set_inet_checksum(pkt, frame->csum_flags, frame->csum_start,
                                frame->csum_stuff);
  }
  if (pbuf == NULL) {
...
int sktu_channel_port_tx(channel_port_t port, packet_t pkt) {
  int error;
  slot_prop_t prop;
  channel_slot_t slot;
 ...
 */
uint32_t sktu_channel_port_tx_burst_pkt(channel_port_t port, packet_t *pkts,
                                        uint32_t n) {
 ...
 * Returns number of actually completed Tx.
 */
uint32_t sktu_channel_port_tx_burst(channel_port_t port,
 ...
 */
void sktu_channel_port_tx_bulk(channel_port_t port, struct sktu_frame **frames,
                               uint32_t n) {
  uint32_t ret = 0;
...
int sktu_parse_ipv4_frame(struct sktu_frame *frame, void *ip_payload,
...
int sktu_parse_tcp4_frame(struct sktu_frame *frame, void *tcp_payload,
...
int sktu_parse_udp4_frame(struct sktu_frame *frame, void *udp_payload,
  ...
  if (ip->ip_p != IPPROTO_UDP) {
    sktu_dump_buffer(stderr, "sktu_parse_udp4_frame: non-UDP packet", buf,
                     pkt_len);
    return EINVAL;
  }
 ...
 */
struct sktu_frame *sktu_channel_port_rx(channel_port_t port) {
  int error;
  slot_prop_t prop;
  channel_slot_t slot;
  struct sktu_frame *frame;
...
uint32_t sktu_channel_port_rx_burst(channel_port_t port,
                                    struct sktu_frame **frames, uint32_t n) {
  struct timespec timeout = {
      .tv_sec = 10,
      .tv_nsec = 0,
...
void sktu_channel_port_rx_bulk(channel_port_t port, struct sktu_frame **frames,
                               uint32_t n) {
  uint32_t ret = 0;
 ...
 */
uint32_t sktu_utun_fd_rx_burst(int utun_fd, struct sktu_frame **frames,
                               uint32_t n) {
...
void sktu_utun_fd_tx_burst(int utun_fd, struct sktu_frame **frames,
                           uint32_t n) {
  struct timeval timeout = {
      .tv_sec = 10,
      .tv_usec = 0,
...
struct sktu_frame *sktu_frame_alloc() {
...
void sktu_frames_free(struct sktu_frame **frames, size_t n) {
...
size_t sktu_create_ip_frames(struct sktu_frame **frames, size_t n, void *src_ip,
                             void *dst_ip, uint8_t proto, const void *sdu,
                             size_t sdu_len, size_t mtu, uint16_t csum_flags,
                             uint16_t csum_start, uint16_t csum_stuff) {
...
size_t sktu_create_ip6_frames(struct sktu_frame **frames, size_t n,
                              void *src_ip, void *dst_ip, uint8_t proto,
                              const void *sdu, size_t sdu_len, size_t mtu,
                              uint16_t csum_flags, uint16_t csum_start,
                              uint16_t csum_stuff) {
  size_t off = 0, remaining_sdu_len = sdu_len;
  size_t i = 0;
  uint16_t ip_id = sktu_ip_id();
...
size_t sktu_create_tcp_frames(struct sktu_frame **frames, size_t n,
                              uint8_t ipver, void *src_ip, void *dst_ip,
                              uint16_t sport, uint16_t dport, const void *data,
                              size_t data_len, size_t mtu, bool csum_offload) {
  uint32_t n_frames;
  ...
  if (ipver == IPVERSION) {
    n_frames =
        sktu_create_ip_frames(frames, n, src_ip, dst_ip, IPPROTO_TCP, sdu,
                              sdu_len, mtu, csum_flags, csum_start, csum_stuff);
  } else {
    n_frames = sktu_create_ip6_frames(frames, n, src_ip, dst_ip, IPPROTO_TCP,
                                      sdu, sdu_len, mtu, csum_flags, csum_start,
                                      csum_stuff);
  }
...
size_t sktu_create_udp_frames(struct sktu_frame **frames, size_t n,
                              uint8_t ipver, void *src_ip, void *dst_ip,
                              uint16_t sport, uint16_t dport, const void *data,
                              size_t data_len, size_t mtu, bool csum_offload) {
  uint32_t n_frames;
  ...
  if (ipver == IPVERSION) {
    ...
    pseudo_sum = os_inet_checksum(
        &udp_pseudo, sizeof(udp_pseudo) + sizeof(struct udphdr), 0);
  } else {
  ...
  if (ipver == IPVERSION) {
    n_frames =
        sktu_create_ip_frames(frames, n, src_ip, dst_ip, IPPROTO_UDP, sdu,
                              sdu_len, mtu, csum_flags, csum_start, csum_stuff);
  } else {
    n_frames = sktu_create_ip6_frames(frames, n, src_ip, dst_ip, IPPROTO_UDP,
                                      sdu, sdu_len, mtu, csum_flags, csum_start,
                                      csum_stuff);
  }
...
void sktu_attach_flow_metadata_to_frames(struct sktu_flow *flow,
...
static size_t _sktu_create_udp_flow_input_frames(struct sktu_flow *flow,
                                                 struct sktu_frame **frames,
                                                 size_t n, const void *data,
                                                 size_t data_len) {
  n = sktu_create_udp_frames(frames, n, flow->ipver, flow->dst_ip, flow->src_ip,
                             flow->dport, flow->sport, data, data_len,
                             flow->mtu, NO_CSUM_OFFLOAD);
  sktu_attach_flow_metadata_to_frames(flow, frames, n);
  return n;
}

static size_t _sktu_create_udp_flow_output_frames(struct sktu_flow *flow,
                                                  struct sktu_frame **frames,
                                                  size_t n, const void *data,
                                                  size_t data_len,
                                                  bool csum_offload) {
  n = sktu_create_udp_frames(frames, n, flow->ipver, flow->src_ip, flow->dst_ip,
                             flow->sport, flow->dport, data, data_len,
                             flow->mtu, csum_offload);
  sktu_attach_flow_metadata_to_frames(flow, frames, n);
  return n;
}

static size_t _sktu_create_tcp_flow_input_frames(struct sktu_flow *flow,
                                                 struct sktu_frame **frames,
                                                 size_t n, const void *data,
                                                 size_t data_len) {
  n = sktu_create_tcp_frames(frames, n, flow->ipver, flow->dst_ip, flow->src_ip,
                             flow->dport, flow->sport, data, data_len,
                             flow->mtu, NO_CSUM_OFFLOAD);
  sktu_attach_flow_metadata_to_frames(flow, frames, n);
  return n;
...
static size_t _sktu_create_tcp_flow_output_frames(struct sktu_flow *flow,
                                                  struct sktu_frame **frames,
                                                  size_t n, const void *data,
                                                  size_t data_len,
                                                  bool csum_offload) {
  n = sktu_create_tcp_frames(frames, n, flow->ipver, flow->src_ip, flow->dst_ip,
                             flow->sport, flow->dport, data, data_len,
                             flow->mtu, csum_offload);
  sktu_attach_flow_metadata_to_frames(flow, frames, n);
  return n;
...
static size_t _sktu_create_ip_flow_input_frames(struct sktu_flow *flow,
                                                struct sktu_frame **frames,
                                                size_t n, const void *data,
                                                size_t data_len) {
  n = sktu_create_ip_frames(frames, n, flow->dst_ip, flow->src_ip,
                            flow->ip_protocol, data, data_len, flow->mtu, 0, 0,
                            0);
  sktu_attach_flow_metadata_to_frames(flow, frames, n);
  return n;
}

static size_t _sktu_create_ip_flow_output_frames(struct sktu_flow *flow,
                                                 struct sktu_frame **frames,
                                                 size_t n, const void *data,
                                                 size_t data_len,
                                                 bool csum_offload) {
...
char *sktu_nfr_to_string(struct nx_flow_req *nfr) {
  ...
  if (nfr->nfr_saddr.sa.sa_family == AF_INET) {
    ...
    inet_ntop(AF_INET6, &nfr->nfr_saddr.sin6.sin6_addr, sa_buf, sizeof(sa_buf));
    inet_ntop(AF_INET6, &nfr->nfr_daddr.sin6.sin6_addr, da_buf, sizeof(da_buf));
  }
  snprintf(
      buf, sizeof(buf),
      "nx_port[%d] %s src=%s,dst=%s,proto=%d,sport=%d,dport=%d, flags=0x%x",
      nfr->nfr_nx_port, uuidstr, sa_buf, da_buf, nfr->nfr_ip_protocol,
      ntohs(nfr->nfr_saddr.sin.sin_port), ntohs(nfr->nfr_daddr.sin.sin_port),
      nfr->nfr_flags);

  return buf;
}

char *sktu_flow_to_string(struct sktu_flow *flow) {
  return sktu_nfr_to_string(&flow->nfr);
}

struct sktu_flow *_sktu_create_nexus_flow(sktu_nexus_t nexus,
                                          nexus_port_t nx_port, uint8_t af,
                                          void *src, void *dst, uint8_t proto,
...
struct sktu_flow *sktu_create_nexus_flow(sktu_nexus_t nexus, uint8_t af,
                                         void *src, void *dst, uint8_t proto,
                                         uint16_t sport, uint16_t dport) {
  return _sktu_create_nexus_flow(nexus, NEXUS_PORT_ANY, af, src, dst, proto,
                                 sport, dport, 0);
}

struct sktu_flow *sktu_create_nexus_flow_with_nx_port(
    sktu_nexus_t nexus, nexus_port_t nx_port, uint8_t af, void *src, void *dst,
    uint8_t proto, uint16_t sport, uint16_t dport) {
  return _sktu_create_nexus_flow(nexus, nx_port, af, src, dst, proto, sport,
                                 dport, 0);
}

struct sktu_flow *sktu_create_nexus_low_latency_flow(sktu_nexus_t nexus,
                                                     uint8_t af, void *src,
                                                     void *dst, uint8_t proto,
                                                     uint16_t sport,
                                                     uint16_t dport) {
  return _sktu_create_nexus_flow(nexus, NEXUS_PORT_ANY, af, src, dst, proto,
                                 sport, dport, NXFLOWREQF_LOW_LATENCY);
}
...
int sktu_get_nexus_flowswitch_stats(struct sk_stats_flow_switch **sfsw,
                                    size_t *len) {
  int ret;
  void *buffer = NULL;
  size_t length = 0;
