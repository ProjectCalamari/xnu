...
#define SEPARATOR(opts) (*(opts) != '\0' ? ", " : "")
#define SKTDBG(name, opts, fmt, ...)                                           \
  SKD1("%s: %s%s" fmt, (name), (opts), SEPARATOR((opts)), ##__VA_ARGS__)
...
static void connect_to_server(uint16_t port, int *client_fd) {
...
static uint8_t read_cmd(int fd) {
  int r, error = 0;
  uint8_t cmd;
...
static void write_cmd(int fd, uint8_t cmd) {
  int w, error = 0;
...
static void socket_test_status(char *name) {
  int i;
  ...
  SKD1("%s: %.2f%% complete\n", name, (total_bytes * 100) / total_expected);
}

static void put_packet(channel_t ch, ring_id_t ring_id, packet_t pkt) {
...
static int get_packet(channel_t ch, ring_id_t r, packet_t *pktp) {
  int error = 0;
  slot_prop_t prop;
...
static void custom_ether_build_packet(void *buf, custom_ether_args_t *args,
                                      size_t *len) {
  if (!args->is_tagged) {
...
static void custom_ether_send(channel_t ch, custom_ether_args_t *args) {
  packet_t pkt;
  void *buf;
  size_t pkt_len;
  buflet_t buflet;
...
static void custom_ether_status(char *name, char *options) {
  int i;
 ...
 */
static void custom_ether_cleanup(void) {
  int i;
  custom_ether_args_t *args;
...
static bool custom_ether_verify(packet_t pkt, custom_ether_args_t *args) {
  buflet_t buflet;
  size_t len;
  void *buf;
  int error = 0;
...
static void custom_ether_receive(channel_t ch, custom_ether_args_t *args,
                                 packet_t pkt) {
  bool valid;
...
static void custom_ether_setup_args(int index, bool sender, bool tagged) {
...
static void custom_ether_setup_flow(nexus_controller_t ncd, uuid_t uuid,
                                    custom_ether_args_t *args,
...
static void custom_ether_handler(channel_t ch, custom_ether_args_t *args) {
  int error;
  packet_t pkt = 0;
...
static void custom_ether_thread(custom_ether_args_t *args) {
  ...
  custom_ether_setup_flow(nx_ncd, if_uuid, args, &nx_port, bind_key);
...
static void filter_server_setup(uint16_t port, int *server_fd) {
  struct sockaddr_in sin;
...
static void filter_channel_setup(void) {
  bool r;
...
static void filter_wait_for_clients(void) {
  fd_set server_fds;
  ...
  maxfd = (send_server_fd > recv_server_fd) ? send_server_fd : recv_server_fd;
  ...
  for (;;) {
    ...
    if (FD_ISSET(recv_server_fd, &server_fds)) {
      sz = sizeof(r);
      recv_client_fd = accept(recv_server_fd, (struct sockaddr *)&r, &sz);
      SKTC_ASSERT_ERR(recv_client_fd != -1);
      SKD1("filter: accepted receiver connection\n");
    } else if (FD_ISSET(send_server_fd, &server_fds)) {
      sz = sizeof(s);
      send_client_fd = accept(send_server_fd, (struct sockaddr *)&s, &sz);
...
static void process_recv_client_cmd(void) {
  uint8_t rcmd;
...
static void process_send_client_cmd(void) { /* nothing yet */ }

static int process_ring(channel_t ch, ring_id_t r) {
  int error, cnt = 0;
  packet_t pkt = 0;
...
static void filter_handler(channel_t ch) {
  int cnt = 0;
...
static void filter_setup_flow(nexus_controller_t ncd, uuid_t uuid,
                              nexus_port_t *nx_port, uuid_t bind_key) {
  struct nx_flow_req nfr;
  uuid_t flow_uuid;
  uuid_string_t uuidstr;
...
static void *filter_thread(void *unused) {
  channel_attr_t ch_attr;
  channel_t ch;
  struct kevent evlist, kev[1];
  nexus_controller_t nx_ncd;
...
static void filter_threads_start(void) {
  int error, i;
  pthread_t t;
...
static void filter_loop(void) {
  ...
  maxfd = (send_client_fd > recv_client_fd) ? send_client_fd : recv_client_fd;
...
static void receiver_socket_setup(void) {
  struct sockaddr_in sin;
  int fd, error = 0, on = 1;
...
static void receiver_channel_setup(void) {
  int err;
  bool found;
...
static void receiver_setup(void) {
  receiver_socket_setup();
  receiver_channel_setup();
}

static void receiver_connect_to_server(void) {
  connect_to_server(FILTER_RECV_PORT, &receiver_client_fd);
}

static void *receiver_socket_thread(void *arg) {
...
static void receiver_socket_start(void) {
  int i, fd, error = 0;
  socket_args_t *args;
  socklen_t sz;
  struct sockaddr_in sin;
...
static void receiver_socket_wait(void) {
  for (;;) {
...
static void *receiver_channel_thread(void *args) {
  custom_ether_thread(args);
  return NULL;
}

static void receiver_channel_start(char *name, char *options, bool tagged) {
  int error, i;
  pthread_t t;
...
static void receiver_channel_wait(char *name, char *options) {
  for (;;) {
...
static void receiver_custom_ether_test(char *name, char *options, bool tagged) {
  receiver_channel_start(name, options, tagged);
  receiver_channel_wait(name, options);
}

static void receiver_done(void) {
  write_cmd(receiver_client_fd, CMD_RECV_ALL_DONE);
}

static void receiver_start(void) {
...
static void receiver(int testid) {
  SKD1("receiver: start\n");
  receiver_setup();
...
static void sender_socket_setup(void) { /* nothing to do */ }

static void sender_channel_setup(void) {
  int err;
  bool found;
...
static void sender_setup(void) {
  sender_socket_setup();
  sender_channel_setup();
}

static void sender_connect_to_server(void) {
  connect_to_server(FILTER_SEND_PORT, &sender_client_fd);
}

static void *sender_socket_thread(void *arg) {
  socket_args_t *args = arg;
  struct sockaddr_in sin;
  ...
  assert(ifscope != 0);
  error = setsockopt(fd, IPPROTO_IP, IP_BOUND_IF, &ifscope, sizeof(ifscope));
  SKTC_ASSERT_ERR(error == 0);
...
static void sender_socket_start(void) {
  uint8_t cmd;
  socket_args_t *args;
  int i, error = 0;
  pthread_t t;
...
static void sender_socket_wait(void) {
  for (;;) {
...
static void *sender_channel_thread(void *args) {
  custom_ether_thread(args);
  return NULL;
}

static void sender_channel_start(char *name, char *options, bool tagged) {
  int error = 0, i;
  pthread_t t;
  uint8_t cmd;
...
static void sender_channel_wait(char *name, char *options) {
  for (;;) {
    custom_ether_status(name, options);
    if (custom_ether_done == CUSTOM_ETHER_THREADS) {
      break;
...
static void sender_custom_ether_test(char *name, char *options, bool tagged) {
  sender_channel_start(name, options, tagged);
  sender_channel_wait(name, options);
}

static void sender_start(void) {
  sender_socket_start();
  sender_socket_wait();
 ...
 ****************************************************************
 */
static int skt_filter_main(int argc, char *argv[]) {
  int child, test_id;
...
static bool skt_filter_supported(void) {
  uint32_t if_attach_nx;
  size_t len = sizeof(if_attach_nx);
  bool supported;

  assert(sysctlbyname("net.link.generic.system.if_attach_nx", &if_attach_nx,
                      &len, NULL, 0) == 0);
...
static void skt_filter_init(uint32_t flags) {
...
static void skt_filter_fini(void) {
...
static void skt_filter_native_init(void) {
  skt_filter_init(FETH_FLAGS_NATIVE | FETH_FLAGS_NXATTACH);
}

static void skt_filter_native_fini(void) { skt_filter_fini(); }

static void skt_filter_compat_init(void) {
  skt_filter_init(FETH_FLAGS_TXSTART | FETH_FLAGS_NXATTACH);
}

static void skt_filter_compat_fini(void) { skt_filter_fini(); }
...
struct skywalk_mptest skt_filternative = {
    ...
    skt_filter_native_init,
    skt_filter_native_fini,
    {},
};
...
struct skywalk_mptest skt_filtercompat = {
    ...
    skt_filter_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_filter_compat_init,
    skt_filter_compat_fini,
    {},
};

struct skywalk_mptest_check skt_filternative_check = {
    NATIVE_TEST,
    skt_filter_supported,
};
