...
static int skt_oneslot_common(int argc, char *argv[], int method,
                              bool defunct) {
  int error;
  channel_t channel;
  uuid_t channel_uuid;
  ...
  if (defunct) {
     ...
     */
    error =
        pid_shutdown_sockets(getpid(), SHUTDOWN_SOCKET_LEVEL_DISCONNECT_ALL);
    SKTC_ASSERT_ERR(!error);
  }
...
static int skt_oneslot_select_main(int argc, char *argv[]) {
  return skt_oneslot_common(argc, argv, 0, false);
}

static int skt_oneslot_poll_main(int argc, char *argv[]) {
  return skt_oneslot_common(argc, argv, 1, false);
}

static int skt_oneslot_kqueue_main(int argc, char *argv[]) {
  return skt_oneslot_common(argc, argv, 2, false);
}

static int skt_oneslot_kqueue_defunct_main(int argc, char *argv[]) {
  return skt_oneslot_common(argc, argv, 2, true);
}

void skt_oneslot_upipe_echo_defunct_init(void) {
...
struct skywalk_test skt_oneslotks = {
    "oneslotks",
    "test sends one slot of data on kpipe loopback using select",
...
struct skywalk_test skt_oneslotup = {
    "oneslotup",
    "test sends one slot of data on user pipe loopback using poll",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_USER_PIPE,
    skt_oneslot_poll_main,
    SKTC_GENERIC_UPIPE_ARGV,
    sktc_generic_upipe_echo_init,
    sktc_generic_upipe_fini,
};
...
struct skywalk_test skt_oneslotkp = {
    "oneslotkp",
    "test sends one slot of data on kpipe loopback using poll",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_KERNEL_PIPE |
        SK_FEATURE_NEXUS_KERNEL_PIPE_LOOPBACK,
    skt_oneslot_poll_main,
    SKTC_GENERIC_KPIPE_ARGV,
...
struct skywalk_test skt_oneslotkk = {
    "oneslotkk",
    "test sends one slot of data on kpipe loopback using kqueue",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_KERNEL_PIPE |
        SK_FEATURE_NEXUS_KERNEL_PIPE_LOOPBACK,
    skt_oneslot_kqueue_main,
    SKTC_GENERIC_KPIPE_ARGV,
...
struct skywalk_test skt_oneslotuk_defunct = {
    "oneslotukd",
    "test sends one slot of data on user pipe loopback using kqueue with one "
    "end of the pipe defuncted",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_USER_PIPE,
    skt_oneslot_kqueue_defunct_main,
    SKTC_GENERIC_UPIPE_ARGV,
