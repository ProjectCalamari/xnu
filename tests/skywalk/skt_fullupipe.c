...
void skt_fullupipe_fini(void) { sktc_cleanup_nexus(); }

int skt_fullupipe_main(int argc, char *argv[]) {
  int error;
  ...
  channel0 = sktu_channel_create_extended(instance_uuid, 0, CHANNEL_DIR_TX_RX,
                                          CHANNEL_RING_ID_ANY, NULL, -1, -1, -1,
                                          -1, -1, -1, 1, -1, -1);
  assert(channel0);
  channel1 = sktu_channel_create_extended(instance_uuid, 1, CHANNEL_DIR_TX_RX,
                                          CHANNEL_RING_ID_ANY, NULL, -1, -1, -1,
                                          -1, -1, -1, 1, -1, -1);
  assert(channel1);

  txring0 = os_channel_tx_ring(
      channel0, os_channel_ring_id(channel0, CHANNEL_FIRST_TX_RING));
  assert(txring0);
  rxring1 = os_channel_rx_ring(
      channel1, os_channel_ring_id(channel1, CHANNEL_FIRST_RX_RING));
  assert(rxring1);
...
int skt_upipepeerclosure_main(int argc, char *argv[]) {
  int error;
  uuid_t instance_uuid;
  channel_t channel0, channel1;
  channel_ring_t txring0, rxring1;
...
struct skywalk_test skt_upipepeerclosure = {
    "upipepeerclosure",
    "test channel operations on upipe with no peer",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_USER_PIPE,
    skt_upipepeerclosure_main,
    SKTC_GENERIC_UPIPE_ARGV,
    skt_fullupipe_init,
    skt_fullupipe_fini,
};
