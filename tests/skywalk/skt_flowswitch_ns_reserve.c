...
static inline boolean_t ErrorFlagsAreSet(ErrorFlags flags, ErrorFlags check) {
  return (flags & check) != 0;
}

const struct in6_addr in6addr_any = IN6ADDR_ANY_INIT;

static void *reader_thread_bsd(void *arg) {
...
static void skt_flowswitch_ns_bsd(struct sktc_nexus_handles *handles, int proto,
                                  void *addr) {
  ...
  T_LOG("BSD: Testing %s over IPv%u %s\n",
        ...
        (saddr.sa.sa_family == AF_INET) ? 4 : 6, buf1);
  ...
  T_LOG("On %s (%s), reserve <%s,%u> through skywalk "
        "(flow %s)\n",
        handles->netif_ifname, buf0, buf1, ntohs(nfr.nfr_saddr.sin.sin_port),
        uuidstr);
...
static void skt_flowswitch_ns_sky(struct sktc_nexus_handles *handles, int proto,
                                  void *addr) {
  union sockaddr_in_4_6 saddr = *(union sockaddr_in_4_6 *)addr;
  int error, lsock;
  ...
  if (saddr.sa.sa_family == AF_INET) {
    ...
    (void)inet_ntop(AF_INET, &saddr.sin.sin_addr, buf1, sizeof(buf1));
  } else {
    pf = PF_INET6;
    is_wild = IN6_IS_ADDR_UNSPECIFIED(&saddr.sin6.sin6_addr);
    (void)inet_ntop(AF_INET6, &saddr.sin6.sin6_addr, buf1, sizeof(buf1));
  }
  ...
  T_LOG("On %s, reserve <%s,%u> through skywalk "
        "(flow %s)\n",
        handles->netif_ifname, buf1, ntohs(nfr.nfr_saddr.sin.sin_port),
        uuidstr);
  error = __os_nexus_flow_add(handles->controller, handles->fsw_nx_uuid, &nfr);
  SKTC_ASSERT_ERR(error == 0);
...
static int skt_ns_sky_bind(struct sktc_nexus_handles *handles, int proto,
                           void *addr, ErrorFlags error_flags, uuid_t flow,
                           uint16_t *bound_port) {
  ...
  if (sky_saddr.sa.sa_family == AF_INET) {
    ...
    (void)inet_ntop(AF_INET, &sky_saddr.sin.sin_addr, buf, sizeof(buf));
    port = sky_saddr.sin.sin_port;
  } else {
    is_wild = IN6_IS_ADDR_UNSPECIFIED(&sky_saddr.sin6.sin6_addr);
    (void)inet_ntop(AF_INET6, &sky_saddr.sin6.sin6_addr, buf, sizeof(buf));
  ...
  T_LOG("On %s, reserve <%s,%u> through skywalk "
        "(flow %s)\n",
        handles->netif_ifname, buf, ntohs(nfr.nfr_saddr.sin.sin_port), uuidstr);
  error = __os_nexus_flow_add(handles->controller, handles->fsw_nx_uuid, &nfr);
...
static int skt_ns_sock_bind(struct sktc_nexus_handles *handles, int proto,
                            void *addr, ErrorFlags error_flags, int *lsock,
                            boolean_t ipv6_only, uint16_t *bound_port) {
  char buf[32];
  int error;
  int on = 0;
  ...
  if (bsd_saddr.sa.sa_family == AF_INET) {
    (void)inet_ntop(AF_INET, &bsd_saddr.sin.sin_addr, buf, sizeof(buf));
    port = bsd_saddr.sin.sin_port;
  } else {
    (void)inet_ntop(AF_INET6, &bsd_saddr.sin6.sin6_addr, buf, sizeof(buf));
  ...
  *lsock = socket(bsd_saddr.sa.sa_family,
  ...
  if (bsd_saddr.sa.sa_family == AF_INET6) {
    ...
    error = setsockopt(*lsock, IPPROTO_IPV6, IPV6_V6ONLY, &on, sizeof(on));
    SKTC_ASSERT_ERR(error == 0);
  }
...
static void skt_flowswitch_ns_sky_bsd(
    struct sktc_nexus_handles *handles, int proto, void *sky_addr,
    void *bsd_addr, boolean_t sky_bind_first, ErrorFlags sky_err_flags,
    ErrorFlags bsd_err_flags, boolean_t ipv6_only, boolean_t use_bound_port) {
  int error;
  struct nx_flow_req nfr;
  uint16_t bound_port;
  uuid_t flow;
  ...
  if (sky_bind_first) {
    skt_ns_sky_bind(handles, proto, sky_addr, sky_err_flags, flow, &bound_port);
    if (use_bound_port) {
      ((struct sockaddr_in *)bsd_addr)->sin_port = bound_port;
    }
    skt_ns_sock_bind(handles, proto, bsd_addr, bsd_err_flags, &lsock, ipv6_only,
                     &bound_port);
  } else {
    skt_ns_sock_bind(handles, proto, bsd_addr, bsd_err_flags, &lsock, ipv6_only,
                     &bound_port);
  ...
  if (!ErrorFlagsAreSet(sky_err_flags, kErrorFlagsExpectFailure)) {
    ...
    uuid_copy(nfr.nfr_flow_uuid, flow);
    error =
        __os_nexus_flow_del(handles->controller, handles->fsw_nx_uuid, &nfr);
    SKTC_ASSERT_ERR(!error);
  }
  close(lsock);
  T_LOG(" %s done\n", __func__);
...
skt_flowswitch_ns_check_v4mappedv6addr2(struct sktc_nexus_handles *handles) {
  ...
  error = skt_ns_sky_bind(handles, IPPROTO_TCP, &sin, ERROR_FLAGS_NO_ASSERT,
                          flow, &bound_port);
  if (error == 0) {
    /* this shouldn't have worked <rdar://problem/35525592> */
    T_LOG("Binding port %d in skywalk should have failed!\n",
          ntohs(nam.sin6_port));
  }
  ...
  assert(lsock >= 0);
  error = bind(lsock, (struct sockaddr *)&blank_sin6, blank_sin6.sin6_len);
  assert(error == 0);
  error = listen(lsock, 1);
  SKTC_ASSERT_ERR(error == 0);
  nam_len = sizeof(nam);
...
static int skt_flowswitch_ns_reserve_main2(int argc, char *argv[]) {
  ...
  skt_flowswitch_ns_sky_bsd(&handles, IPPROTO_TCP, sin, sin6, TRUE,
                            ...
                            ERROR_FLAGS_ASSERT_FAILURE, FALSE, TRUE);
  sin->sin_port = 0;
  sin6->sin6_port = 0;
  skt_flowswitch_ns_sky_bsd(&handles, IPPROTO_TCP, sin, sin6, FALSE,
                            ERROR_FLAGS_ASSERT_FAILURE,
                            ERROR_FLAGS_ASSERT_SUCCESS, FALSE, TRUE);
  sin->sin_port = 0;
  sin6->sin6_port = 0;
  skt_flowswitch_ns_sky_bsd(&handles, IPPROTO_TCP, sin, sin6, TRUE,
                            ERROR_FLAGS_ASSERT_SUCCESS,
                            ERROR_FLAGS_ASSERT_SUCCESS, TRUE, TRUE);
  sin->sin_port = 0;
  sin6->sin6_port = 0;
  skt_flowswitch_ns_sky_bsd(&handles, IPPROTO_TCP, sin, sin6, FALSE,
                            ERROR_FLAGS_ASSERT_FAILURE,
...
static int skt_flowswitch_ns_reserve_main(int argc, char *argv[]) {
  char buf0[32], buf1[32];
  size_t retries = 0;
  int error;
  struct sktc_nexus_handles handles;
...
start:
  ...
  T_LOG("On %s (%s), reserve <ANY,%u> through skywalk "
        "(flow %s)\n",
        handles.netif_ifname, buf0, ntohs(nfr.nfr_saddr.sin.sin_port), uuidstr);
  error = __os_nexus_flow_add(handles.controller, handles.fsw_nx_uuid, &nfr);
  SKTC_ASSERT_ERR(!error);
  ...
  (void)inet_ntop(AF_INET, &handles.netif_addr, buf0, sizeof(buf0));
  (void)inet_ntop(AF_INET, &nfr.nfr_saddr.sin.sin_addr, buf1, sizeof(buf1));
  uuid_generate_random(nfr.nfr_flow_uuid);
  uuid_unparse_upper(nfr.nfr_flow_uuid, uuidstr);
  ...
  T_LOG("On %s (%s), confirm <%s,%u> binding is not allowed "
        "(flow %s)\n",
        handles2.netif_ifname, buf0, buf1, ntohs(nfr.nfr_saddr.sin.sin_port),
        uuidstr);
  error = __os_nexus_flow_add(handles2.controller, handles2.fsw_nx_uuid, &nfr);
...
struct skywalk_test skt_flowswitch_ns_reserve2 = {
    "flowswitch_ns_reserve2",
    "throrough test of netns for both BSD & flowswitch, IPv4/v6",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS,
    skt_flowswitch_ns_reserve_main2,
    {NULL},
