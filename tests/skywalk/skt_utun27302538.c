...
static void *thread1(void *unused) {
  int kq = kqueue();
  ...
  EV_SET(&kevin, os_channel_get_fd(channel), EVFILT_READ, EV_ADD | EV_ENABLE, 0,
         0, NULL);
  ...
  T_LOG("kevin ident 0x%" PRIxPTR ", filter %d, "
        ...
        kevin.ident, kevin.filter, kevin.flags, kevin.fflags, kevin.data,
        kevin.udata);
  T_LOG("kevout ident 0x%" PRIxPTR ", filter %d, "
        "flags 0x%04x fflags 0x%08x data 0x%" PRIxPTR " udata %p\n",
        kevout.ident, kevout.filter, kevout.flags, kevout.fflags, kevout.data,
        kevout.udata);
  assert(0);
}

static void skt_utun27302538_common(void) {
  tunsock = sktu_create_interface(SKTU_IFT_UTUN, SKTU_IFF_ENABLE_NETIF |
                                                     SKTU_IFF_ENABLE_CHANNEL);
  assert(tunsock);
...
/*
 * <rdar://problem/27302538> kernel panic during skywalk channel closure: panic
 * in sk_free_rings()
 */

static int skt_utun27302538a_main(int argc, char *argv[]) {
  int error;
...
static int skt_utun27302538b_main(int argc, char *argv[]) {
  int error;
...
static int skt_utun27302538c_main(int argc, char *argv[]) {
  int error;
...
static int skt_utun27302538d_main(int argc, char *argv[]) {
  int error;
...
/****************************************************************/
/*
 * <rdar://problem/27646755> cleanup if_utun pcb lock and fix race in utun
 * disable channel
 */

static void *thread2(void *unused) {
  close(tunsock);
  return 0;
}

static int skt_utun27646755_common(int iterations) {
  int i;
  ...
  for (i = 0; i < iterations; i++) {
    ...
    if (now > then) {
      T_LOG(
          "time %ld completed iteration %d of %d (%2.2f%% est %ld secs left)\n",
...
static int skt_utun27646755_main(int argc, char *argv[]) {
  return skt_utun27646755_common(20);
}

static int skt_utun27646755slow_main(int argc, char *argv[]) {
  return skt_utun27646755_common(1000);
}

struct skywalk_test skt_utun27646755 = {
    "utun27646755",
    "race cleaning up channel and utun socket (20 iterations)",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_KERNEL_PIPE,
...
struct skywalk_test skt_utun27646755slow = {
    "utun27646755slow",
    "race cleaning up channel and utun socket (1000 iterations)",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_KERNEL_PIPE,
