...
static uint32_t __packet_copy_and_sum(const void *src, void *dst, uint32_t len,
                                      uint32_t sum0) {
...
static inline uint16_t packet_fold_sum_final(uint32_t sum) {
...
uint32_t m_copydata_sum(struct mbuf *m, int off, int len, void *vp,
                        uint32_t initial_sum, boolean_t *odd_start) {
  ...
  while (off > 0) {
    if (__improbable(m == NULL)) {
      T_LOG("%s: invalid mbuf chain %p [off %d, len %d]", __func__, m0, off0,
            len0);
      /* NOTREACHED */
      __builtin_unreachable();
    }
 ...
 */
uint32_t reference_cksum_mbuf(struct mbuf *m, int len, int off,
...
static void randomise_buffer(uint8_t *buffer, u_int len) {
  while (len--) {
    *buffer++ = (uint8_t)rand();
  }
}

static uint32_t reference_cksum_single(u_int offset, u_int len,
                                       uint32_t init_sum) {
  struct mbuf m;
...
static uint32_t target_cksum_single(u_int soffset, u_int doffset, u_int len,
                                    uint32_t init_sum) {
  uint32_t rv;
...
static uint32_t reference_cksum_multi(u_int offset, u_int len,
                                      uint32_t init_sum) {
  struct mbuf m[2];
...
static uint32_t target_cksum_multi(u_int soffset, u_int doffset, u_int len,
                                   uint32_t init_sum) {
  struct mbuf m[2];
  uint32_t sum;
  ...
  sum = m_copydata_sum(&m[0], 0, len, &st_dst_buffer[doffset], init_sum, NULL);
...
static int do_test_multi(u_int soffset, u_int doffset, u_int len,
                         uint32_t init_sum) {
  ...
  if (len > 1) {
    memcpy(&st_ref_buffer[doffset], &st_src_buffer1[soffset], m1_len);
    memcpy(&st_ref_buffer[doffset + m1_len], &st_src_buffer2[soffset],
           len - m1_len);
    if (verbose) {
    ...
    if (verbose) {
      sktu_dump_buffer(stderr, "Multi Source1", &st_src_buffer1[soffset], 1);
    }
  }
...
static int do_test_single(u_int soffset, u_int doffset, u_int len,
                          uint32_t init_sum) {
  uint32_t ref_sum, tgt_sum;
  int rv;
...
static int do_test_cksum(u_int soffset, u_int len, uint32_t init_sum) {
  uint32_t ref_sum, tgt_sum;
  int rv;
...
static int skt_copy_cksum_common(int argc, char **argv,
                                 int (*test_func)(u_int, u_int, u_int,
                                                  uint32_t)) {
  uint32_t init_sum;
...
static int skt_cksum_common(int argc, char **argv,
...
static int skt_copy_cksum_single_main(int argc, char **argv) {
  return skt_copy_cksum_common(argc, argv, do_test_single);
}

static int skt_copy_cksum_multi_main(int argc, char **argv) {
  return skt_copy_cksum_common(argc, argv, do_test_multi);
}

static int skt_cksum_main(int argc, char **argv) {
  return skt_cksum_common(argc, argv, do_test_cksum);
}

struct skywalk_test skt_copy_cksum_single = {
    "copycksum-single",
    "test copy/checksum code: single buffer",
    SK_FEATURE_SKYWALK,
    skt_copy_cksum_single_main,
    {NULL},
    NULL,
    NULL,
};
...
struct skywalk_test skt_copy_cksum_multi = {
    "copycksum-multi",
    "test copy/checksum code: buffer chain",
    SK_FEATURE_SKYWALK,
    skt_copy_cksum_multi_main,
    {NULL},
    NULL,
    NULL,
};
