...
static void skt_flow_req_should_succeed(sa_family_t af, void *src, void *dst,
...
static int skt_protons_main(int argc, char *argv[]) {
  ifname = FETH0_NAME;
...
static void skt_protons_net_init(void) {
  sktc_ifnet_feth_pair_create(FETH_FLAGS_TXSTART);
}

static void skt_protons_net_fini(void) {
...
static int skt_mpprotons_main(int argc, char *argv[]) {
  char buf[1] = {0};
  const char *ifname;
...
struct skywalk_mptest skt_mpprotons = {
    ...
    .skt_testdesc = "test skywalk protocol namespace with two process doing "
