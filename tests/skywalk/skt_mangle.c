...
static void skt_mangle_init(void) {
  uint32_t disable_panic_on_sync_err = 1;
  struct sktc_nexus_attr attr = SKTC_NEXUS_ATTR_INIT();

  sysctlbyname("kern.skywalk.disable_panic_on_sync_err", NULL, 0,
               &disable_panic_on_sync_err, sizeof(disable_panic_on_sync_err));
...
static void skt_mangle_fini(void) {
...
static void *skt_mangle_verify_internalize_metadata(void *_ctx) {
  ...
  sktu_create_udp_frames(&tx_frame, 1, IPVERSION, &ctx->our_ip, &ctx->dst_ip,
                         sport, dport, &tx_payload, sizeof(tx_payload), 1500,
                         CSUM_OFFLOAD);
...
static void *skt_mangle_rx(void *ctx_) {
...
static void *skt_mangle_tx(void *ctx_) {
  struct context *ctx = (struct context *)ctx_;
  channel_t channel;
...
static int skt_mangle_main(int argc, char *argv[], int test_selector) {
...
static int skt_change_len_main(int argc, char *argv[]) {
  return skt_mangle_main(argc, argv, SKT_CHANGE_LEN);
}

static int skt_big_len_main(int argc, char *argv[]) {
  return skt_mangle_main(argc, argv, SKT_BIG_LEN);
}

static int skt_mangle_verify_internalize_metadata_main(int argc, char *argv[]) {
#pragma unused(argc, argv)
  ...
  ctx.utun_fd = sktu_create_interface(
      SKTU_IFT_UTUN, SKTU_IFF_ENABLE_NETIF | SKTU_IFF_NO_ATTACH_FSW);
  sktu_get_interface_name(SKTU_IFT_UTUN, ctx.utun_fd, utun_ifname);

  if (sktc_ifnet_add_addr(utun_ifname, &ctx.our_ip, &mask, &broad_ip) != 0) {
  ...
  error = os_nexus_controller_bind_provider_instance(
      ctx.handles.controller, ctx.handles.fsw_nx_uuid, 3, getpid(), NULL, NULL,
      0, NEXUS_BIND_PID);
  SKTC_ASSERT_ERR(error == 0);

  sktu_channel_port_init(&ctx.port, ctx.handles.fsw_nx_uuid, 3, true, false,
                         false);
...
struct skywalk_test skt_change_len = {
    "change_len",
    "tests kernel resilience to modified slot lengths",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_USER_PIPE | SK_FEATURE_DEV_OR_DEBUG,
    skt_change_len_main,
...
struct skywalk_test skt_big_len = {
    "big_len",
    "tests unrealistically large slot lengths",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_USER_PIPE | SK_FEATURE_DEV_OR_DEBUG,
    skt_big_len_main,
    SKTC_GENERIC_UPIPE_ARGV,
    skt_mangle_init,
...
struct skywalk_test skt_internalizemetdata = {
    ...
    .skt_required_features = SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_FLOWSWITCH |
                             SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NETNS,
