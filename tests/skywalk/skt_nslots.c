...
int skt_nslots_common(int argc, char *argv[], uint32_t nslots,
                      uint32_t interval, int method) {
  int error;
  channel_t channel;
  uuid_t channel_uuid;
  ring_id_t ringid;
  ...
  channel = sktu_channel_create_extended(channel_uuid, 0, CHANNEL_DIR_TX_RX,
                                         CHANNEL_RING_ID_ANY, NULL, -1, -1, -1,
                                         -1, -1, -1, 1, -1, -1);
  assert(channel);
  ...
  while (recvcount < nslots) {
    ...
    if (sendcount < nslots) {
      ...
      case 1:
        ...
        break;
      case 2:
        EV_SET(&kev, channelfd, EVFILT_WRITE, EV_ADD | EV_ENABLE | EV_ONESHOT,
               0, 0, NULL);
    ...
    case 1:
      ...
      break;
    case 2:
      EV_SET(&kev, channelfd, EVFILT_READ, EV_ADD | EV_ENABLE | EV_ONESHOT, 0,
             0, NULL);
      error = kevent(kq, &kev, 1, &kev, 1, NULL);
      SKTC_ASSERT_ERR(error != -1);
      SKTC_ASSERT_ERR(error == 1);
...
int skt_nslots_select_main(int argc, char *argv[]) {
  return skt_nslots_common(argc, argv, -1, -1, 0);
}

int skt_nslots_poll_main(int argc, char *argv[]) {
  return skt_nslots_common(argc, argv, -1, -1, 1);
}

int skt_nslots_kqueue_main(int argc, char *argv[]) {
...
struct skywalk_test skt_nslotsks = {
    "nslotsks",
    "test sends TX_SLOTS of data on kpipe loopback using select",
...
struct skywalk_test skt_nslotsup = {
    "nslotsup",
    "test sends TX_SLOTS of data on user pipe loopback using poll",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_USER_PIPE,
    skt_nslots_poll_main,
    SKTC_GENERIC_UPIPE_ARGV,
    sktc_generic_upipe_echo_init,
    sktc_generic_upipe_fini,
};
...
struct skywalk_test skt_nslotskp = {
    "nslotskp",
    "test sends TX_SLOTS of data on kpipe loopback using poll",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_KERNEL_PIPE |
        SK_FEATURE_NEXUS_KERNEL_PIPE_LOOPBACK,
    skt_nslots_poll_main,
    SKTC_GENERIC_KPIPE_ARGV,
...
struct skywalk_test skt_nslotskk = {
    "nslotskk",
    "test sends TX_SLOTS of data on kpipe loopback using kqueue",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_KERNEL_PIPE |
        SK_FEATURE_NEXUS_KERNEL_PIPE_LOOPBACK,
    skt_nslots_kqueue_main,
    SKTC_GENERIC_KPIPE_ARGV,
...
/****************************************************************/
// <rdar://problem/25995625> Need skywalk unit test that streams packets over
// upipe for a long time

#define MSLOTS 1000000
int skt_mslots_select_main(int argc, char *argv[]) {
  return skt_nslots_common(argc, argv, MSLOTS, -1, 0);
}

int skt_mslots_poll_main(int argc, char *argv[]) {
  return skt_nslots_common(argc, argv, MSLOTS, -1, 1);
}

int skt_mslots_kqueue_main(int argc, char *argv[]) {
...
struct skywalk_test skt_mslotskp = {
    "mslotskp",
    "test sends 1000000 slots of data on kpipe loopback using poll",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_KERNEL_PIPE |
        SK_FEATURE_NEXUS_KERNEL_PIPE_LOOPBACK,
    skt_mslots_poll_main,
    SKTC_GENERIC_KPIPE_ARGV,
...
int skt_mmslots_select_main(int argc, char *argv[]) {
  return skt_nslots_common(argc, argv, MMSLOTS, -1, 0);
}

int skt_mmslots_poll_main(int argc, char *argv[]) {
  return skt_nslots_common(argc, argv, MMSLOTS, -1, 1);
}

int skt_mmslots_kqueue_main(int argc, char *argv[]) {
