...
static void skt_teardown_pass(int count, int *permute) {
  int error;
  ...
  strncpy((char *)attr.name, "skt_teardown_netif", sizeof(nexus_name_t) - 1);
  attr.type = NEXUS_TYPE_NET_IF;
  attr.anonymous = 1;
  sktc_build_nexus(ncd, &attr, &nf_provider, &nf_instance0);

  uuid_clear(feth0_attach);
  error = __os_nexus_ifattach(ncd, nf_instance0, FETH0_NAME, NULL, false,
                              &feth0_attach);
  ...
  uuid_clear(nf_instance1);
  error = os_nexus_controller_alloc_provider_instance(ncd, nf_provider,
                                                      &nf_instance1);
  SKTC_ASSERT_ERR(!error);
  assert(!uuid_is_null(nf_instance1));

  uuid_clear(feth1_attach);
  error = __os_nexus_ifattach(ncd, nf_instance1, FETH1_NAME, NULL, false,
                              &feth1_attach);
  SKTC_ASSERT_ERR(!error);
  ...
  uuid_clear(nf_attach0);
  error = __os_nexus_ifattach(ncd, ms_instance, NULL, nf_instance0, false,
                              &nf_attach0);
  // T_LOG("%s:%d error %d errno %d\n", __func__, __LINE__, error, errno);
  SKTC_ASSERT_ERR(!error);
  ...
  assert(
      sktu_channel_create_extended(ms_instance, NEXUS_PORT_FLOW_SWITCH_CLIENT,
                                   CHANNEL_DIR_TX_RX, CHANNEL_RING_ID_ANY, NULL,
                                   -1, -1, -1, -1, -1, -1, 1, -1, -1) == NULL);

  channel = sktu_channel_create_extended(
      ms_instance, NEXUS_PORT_FLOW_SWITCH_CLIENT, CHANNEL_DIR_TX_RX,
...
static int skt_teardown_main(int argc, char *argv[]) {
  int permute[9];
  for (int i = 0; i < 9; i++) {
    permute[i] = 8 - i;
  }
...
static int skt_teardownb_main(int argc, char *argv[]) {
  int permute[9];
  for (int i = 0; i < 9; i++) {
    permute[i] = i;
  }
...
static int skt_teardownr_main(int argc, char *argv[]) {
  int permute[9];
  for (int i = 0; i < 9; i++) {
    permute[i] = 8 - i;
  }
...
static int skt_teardownz_main(int argc, char *argv[]) {
  int permute[9];
  for (int i = 0; i < 9; i++) {
    permute[i] = 8 - i;
  }
...
struct skywalk_test skt_teardown = {
    "teardown",
    "setup complicated topology tear it down",
...
struct skywalk_test skt_teardownb = {
    "teardownb",
    "setup complicated topology tear it down backwards",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    skt_teardownb_main,
    {NULL},
...
struct skywalk_test skt_teardownz = {
    "teardownz",
    "setup complicated topology tear it down with each stage in an out of "
    "order position",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    skt_teardownz_main,
