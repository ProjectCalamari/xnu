...
static void skt_netifcompat_common(void) {
  int error;
  struct sktc_nexus_attr attr = SKTC_NEXUS_ATTR_INIT();

  strncpy((char *)attr.name, "skywalk_test_net_if", sizeof(nexus_name_t) - 1);
  ...
  channel = sktu_channel_create_extended(
      sktc_instance_uuid, NEXUS_PORT_NET_IF_HOST, CHANNEL_DIR_TX_RX,
      CHANNEL_RING_ID_ANY, NULL, -1, -1, -1, -1, -1, -1, 1, -1, -1);
  assert(!channel);

  channel = sktu_channel_create_extended(
      sktc_instance_uuid, NEXUS_PORT_NET_IF_DEV, CHANNEL_DIR_TX_RX,
      CHANNEL_RING_ID_ANY, NULL, -1, -1, -1, -1, -1, -1, 1, -1, -1);
  assert(!channel);

  uuid_generate_random(if_uuid);
  error =
      __os_nexus_ifdetach(sktc_nexus_controller, sktc_instance_uuid, if_uuid);
...
static int skt_netifcompata_main(int argc, char *argv[]) {
  int error;
...
static int skt_netifcompatb_main(int argc, char *argv[]) {
  skt_netifcompat_common();
...
static int skt_netifcompatc_main(int argc, char *argv[]) {
  int error;
  skt_netifcompat_common();
...
struct skywalk_test skt_netifcompata = {
    "netifcompata",
    "setup and teardown netifcompat on feth0",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF,
    skt_netifcompata_main,
    {NULL},
...
struct skywalk_test skt_netifcompatb = {
    "netifcompatb",
    "setup and teardown netifcompat on feth0 with deferred close channel",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF,
    skt_netifcompatb_main,
    {NULL},
    sktc_ifnet_feth0_create,
    sktc_ifnet_feth0_destroy,
};
...
struct skywalk_test skt_netifcompatc = {
    "netifcompatc",
    "setup and teardown netifcompat on feth0 with deferred detach and close "
    "channel",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF,
    skt_netifcompatc_main,
    {NULL},
    sktc_ifnet_feth0_create,
    sktc_ifnet_feth0_destroy,
};
...
static void skt_fsw_common(const char *name) {
  int error;
  uuid_t ms_provider;
  uuid_t ms_instance;
  uuid_t ms_attach;
  ...
  sktc_build_nexus(sktc_nexus_controller, &attr, &ms_provider, &ms_instance);
  ...
  if (channel != NULL) {
     ...
     * attach will fail
     */
    error = __os_nexus_ifattach(sktc_nexus_controller, ms_instance, NULL,
                                sktc_instance_uuid, false, &ms_attach);
  ...
  uuid_clear(scratch);
  error = __os_nexus_ifattach(sktc_nexus_controller, ms_instance, NULL,
                              sktc_instance_uuid, false, &scratch);
  SKT_LOG("%s:%d error %d errno %d\n", __func__, __LINE__, error, errno);
  SKTC_ASSERT_ERR(error == -1);
  ...
  error = sktc_bind_tcp4_flow(sktc_nexus_controller, ms_instance, 0,
                              NEXUS_PORT_FLOW_SWITCH_CLIENT, flow_id);
  SKTC_ASSERT_ERR(error == 0);

  /* must fail without user packet pool set (flow switch) */
  assert(sktu_channel_create_extended(ms_instance, 2, CHANNEL_DIR_TX_RX,
                                      CHANNEL_RING_ID_ANY, NULL, -1, -1, -1, -1,
                                      -1, -1, 1, -1, -1) == NULL);

  /* Open and close channel to the flow switch */
  channel = sktu_channel_create_extended(ms_instance, 2, CHANNEL_DIR_TX_RX,
  ...
  error = __os_nexus_ifdetach(sktc_nexus_controller, ms_instance, ms_attach);
  SKT_LOG("%s:%d error %d errno %d\n", __func__, __LINE__, error, errno);
  SKTC_ASSERT_ERR(!error);
...
static int skt_netifms_main(int argc, char *argv[]) {
...
struct skywalk_test skt_netiffsw = {
    "netifms",
    "setup compat netif and attach to flowswitch",
...
int skt_netifdelete_main(int argc, char *argv[]) {
  uuid_t attach;
  int error;
  struct sktc_nexus_attr attr = SKTC_NEXUS_ATTR_INIT();

  strncpy((char *)attr.name, "skt_netifdelete_netif", sizeof(nexus_name_t) - 1);
  attr.type = NEXUS_TYPE_NET_IF;
  attr.anonymous = 1;
  attr.userchannel = 1;
  ...
  error =
      __os_nexus_ifdetach(sktc_nexus_controller, sktc_instance_uuid, attach);
  SKT_LOG("%s:%d error %d errno %d\n", __func__, __LINE__, error, errno);
  SKTC_ASSERT_ERR(!error);
...
struct skywalk_test skt_netifdelete = {
    "netifdelete",
    "tear down an ifp while it's attached to a netif",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF,
