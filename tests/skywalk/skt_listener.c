...
void skt_listener_test_socket_listen(bool expect_success, struct in_addr *sip,
                                     in_port_t *sport) {
  ...
  error = bind(sock, (struct sockaddr *)&listener_addr, sizeof(listener_addr));
...
void skt_listener_cleanup(uuid_t flow_id) {
...
int skt_listener_main(int argc, char *argv[]) {
  ...
  /* SUCCESS: test creating a listener flow (let netns picks a ephemeral port
   * for us)*/
  listener = sktu_create_nexus_flow(&handles, AF_INET, &zero_ip, &zero_ip,
                                    IPPROTO_TCP, 0, 0);
  assert(listener);
  ...
  no_flow = sktu_create_nexus_flow(&handles, AF_INET, &zero_ip, &zero_ip,
                                   IPPROTO_TCP, our_port, 0);
  assert(!no_flow);

  /* FAILURE: try add duplicate 3-tuple listener flow (with same lport) */
  no_flow = sktu_create_nexus_flow(&handles, AF_INET, &our_ip, &zero_ip,
                                   IPPROTO_TCP, our_port, 0);
  assert(!no_flow);
  ...
  connection_1 = sktu_create_nexus_flow(&handles, AF_INET, &our_ip, &peer_ip,
                                        IPPROTO_TCP, our_port, peer_port_1);
  assert(connection_1);

  /* test connecting a new flow off the listener flow - 2 */
  connection_2 = sktu_create_nexus_flow(&handles, AF_INET, &our_ip, &peer_ip,
                                        IPPROTO_TCP, our_port, peer_port_2);

  /* test connecting a duplicate connected flow off the listener flow */
  /* test connecting a new flow off the listener flow - 2 */
  no_flow = sktu_create_nexus_flow(&handles, AF_INET, &our_ip, &peer_ip,
                                   IPPROTO_TCP, our_port, peer_port_2);
  assert(!no_flow);
  ...
  listener = sktu_create_nexus_flow(&handles, AF_INET, &our_ip, &zero_ip,
                                    IPPROTO_TCP, 0, 0);
  assert(listener);
  ...
  listener = sktu_create_nexus_flow(&handles, AF_INET6, &zero_ip6, &zero_ip6,
                                    IPPROTO_TCP, 0, 0);
  assert(listener);
...
int skt_listener_stress_main(int argc, char *argv[]) {
  ...
  for (i = 0; i < 511; i++) {
    flows[i] = sktu_create_nexus_flow(&handles, AF_INET, &our_ip, &peer_ip,
                                      IPPROTO_TCP, our_port, 10000 + i);
    assert(flows[i]);
  }
...
int skt_listen_stress_main(int argc, char *argv[]) {
#define LISTEN_MAX                                                             \
  INT16_MAX /* might exhaust socache zone due to late cache purge */
...
int skt_listener_reuse_main(int argc, char *argv[]) {
  ifname = FETH0_NAME;
  our_mask = sktc_make_in_addr(IN_CLASSC_NET);
  our_ip = sktc_feth0_in_addr();
  peer_ip = sktc_feth1_in_addr();
  ...
  listener = _sktu_create_nexus_flow(&handles, NEXUS_PORT_ANY, AF_INET6,
                                     &zero_ip6, &zero_ip6, IPPROTO_TCP, 0, 0,
                                     NXFLOWREQF_REUSEPORT);
  assert(listener);

  our_port = ntohs(listener->nfr.nfr_saddr.sin.sin_port);

  // dup listen should fail
  dup_listener = _sktu_create_nexus_flow(&handles, NEXUS_PORT_ANY, AF_INET6,
                                         &zero_ip6, &zero_ip6, IPPROTO_TCP,
                                         our_port, 0, NXFLOWREQF_REUSEPORT);
  assert(!dup_listener);

  // 5 tuple TCP connection from listener
  connection = _sktu_create_nexus_flow(&handles, NEXUS_PORT_ANY, AF_INET,
                                       &our_ip, &peer_ip, IPPROTO_TCP, our_port,
                                       our_port, NXFLOWREQF_REUSEPORT);
  assert(connection);

  // kill listener
  sktu_destroy_nexus_flow(listener);

  // sock listener should fail(skywalk listener port could only be reused by
  // skywalk)
  skt_listener_test_socket_listen(false, &zero_ip, &our_port);
...
void skt_listener_net_init(void) {
  sktc_ifnet_feth_pair_create(FETH_FLAGS_TXSTART);
}

void skt_listener_net_fini(void) { sktc_ifnet_feth_pair_destroy(); }

struct skywalk_test skt_listener = {
    "listener",
    "test skywalk listener flow creation check",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS,
    skt_listener_main,
    {NULL},
    skt_listener_net_init,
    skt_listener_net_fini,
};

struct skywalk_test skt_listen_stress = {
    "listen_stress",
    "stress posix socket listen",
    SK_FEATURE_SKYWALK | SK_FEATURE_NETNS,
    skt_listen_stress_main,
    {NULL},
    NULL,
    NULL,
};
...
struct skywalk_test skt_listener_stress = {
    "listener_stress",
    "stress skywalk listener flows",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS,
    skt_listener_stress_main,
    {NULL},
    skt_listener_net_init,
    skt_listener_net_fini,
};
...
struct skywalk_test skt_listener_reuse = {
    "listener_reuse",
    "test skywalk listener reuse",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS,
    skt_listener_reuse_main,
    {NULL},
    skt_listener_net_init,
    skt_listener_net_fini,
};
