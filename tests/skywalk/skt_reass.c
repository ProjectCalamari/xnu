...
static void die_perror(const char *str) {
  perror(str);
  assert(0);
}
...
void reass_fini(void) { close(utun_fd); }

void reass_interface_send(reass_packet_builder_fn builder) {
...
void reass_interface_send_rece(reass_packet_builder_fn builder,
                               reass_packet_matcher_fn matcher,
                               struct timeval *allowed) {
...
void reass_common(channel_port *ch_port, struct sktu_flow *flow, void *data,
                  size_t data_len) {
  ...
  n_tx_frames =
      flow->create_input_frames(flow, tx_frames, MAX_FRAMES, data, data_len);
  ...
  assert(memcmp(rx_frames[0]->bytes, tx_frames[0]->bytes, tx_frames[0]->len) ==
         0);
  assert(rx_frames[1]->len == tx_frames[1]->len);
  assert(memcmp(rx_frames[1]->bytes, tx_frames[1]->bytes, tx_frames[1]->len) ==
         0);
...
size_t bad_fraglen_build(uint8_t *buf) {
  size_t i, len = 0;
  size_t plen = 35;
...
int bad_fraglen_match(uint8_t *pkt_out, size_t len_out, uint8_t *pkt_in,
                      size_t len_in) {
  ...
  assert(icmp6->icmp6_pptr ==
         htonl(__builtin_offsetof(struct ip6_hdr, ip6_plen)));

  return 0;
}

size_t timeout_build(uint8_t *buf) {
  size_t i, len = 0;
  size_t plen = 128;
  uint32_t address_family = htonl(AF_INET6);
...
int timeout_match(uint8_t *pkt_out, size_t len_out, uint8_t *pkt_in,
                  size_t len_in) {
  uint8_t *scan = pkt_out;
  uint32_t af = ntohl(*(uint32_t *)pkt_out);
...
size_t atomic_build(uint8_t *buf) {
  size_t i, len = 0;
  size_t plen, dlen = 16;
  ...
  uint16_t csum;
  icmp6->icmp6_cksum = in6_pseudo(
      &src, &dst, htonl(IPPROTO_ICMPV6 + sizeof(struct icmp6_hdr) + dlen));
...
int atomic_match(uint8_t *pkt_out, size_t len_out, uint8_t *pkt_in,
                 size_t len_in) {
  uint8_t *scan = pkt_out;
  uint32_t af = ntohl(*(uint32_t *)pkt_out);
...
size_t queue_limit_build(uint8_t *buf) {
  size_t len = 0;
  struct in6_addr src, dst;
  uint32_t address_family = htonl(AF_INET6);
...
void reass_lower_timeout(int *old_timeout, int new_timeout) {
  size_t old_size = sizeof(*old_timeout);
  int error;

  error = sysctlbyname("kern.skywalk.flowswitch.ipfm_frag_ttl", old_timeout,
                       &old_size, &new_timeout, sizeof(int));
  if (error) {
    die_perror("kern.skywalk.flowswitch.ipfm_frag_ttl");
  }
}

void reass_restore_timeout(int old_timeout) {
  int error;

  error = sysctlbyname("kern.skywalk.flowswitch.ipfm_frag_ttl", NULL, NULL,
                       &old_timeout, sizeof(int));
  if (error) {
    die_perror("kern.skywalk.flowswitch.ipfm_frag_ttl");
  }
}

int skt_reass_main(int argc, char *argv[]) {
  int error;

  // setup full interface
  utun_fd = sktu_create_interface(SKTU_IFT_UTUN, SKTU_IFF_ENABLE_NETIF |
                                                     SKTU_IFF_NO_ATTACH_FSW);
  ...
  error = os_nexus_controller_bind_provider_instance(
      nexus.controller, nexus.fsw_nx_uuid, NX_PORT, getpid(), NULL, NULL, 0,
      NEXUS_BIND_PID);
  SKTC_ASSERT_ERR(error == 0);

  struct sktu_flow *flow_ipv4, *flow_ipv6;

  flow_ipv4 = sktu_create_nexus_flow_with_nx_port(
      &nexus, NX_PORT, AF_INET, &our_ip, &dst_ip, IPPROTO_UDP, 0, 1234);
  assert(flow_ipv4);

  flow_ipv6 = sktu_create_nexus_flow_with_nx_port(
      &nexus, NX_PORT, AF_INET6, &our_ip_v6, &dst_ip_v6, IPPROTO_UDP, 0, 1234);
  assert(flow_ipv4);

  assert(flow_ipv4->nfr.nfr_nx_port == flow_ipv6->nfr.nfr_nx_port);

  sktu_channel_port_init(&port, nexus.fsw_nx_uuid, flow_ipv4->nfr.nfr_nx_port,
                         true, false, false);
...
int skt_reass_main_default_setting(int argc, char *argv[]) {
...
int skt_reass_timeout_main(int argc, char *argv[]) {
  int old_timeout;
  int new_timeout = 5;
...
int skt_reass_bad_fraglen_main(int argc, char *argv[]) {
  REASS_TEST_ALL(bad_fraglen_build, bad_fraglen_match, timeout);
  return 0;
}

int skt_reass_atomic_main(int argc, char *argv[]) {
  REASS_TEST_ALL(atomic_build, atomic_match, timeout);
  return 0;
}

int skt_reass_fuzz_queue_limit_main(int argc, char *argv[]) {
...
struct skywalk_test skt_reass_default_setting = {
    ...
    "UDP fragmentation reassembly (channel flow Rx) (without forcing ip_reass "
    "sysctl)",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    skt_reass_main_default_setting,
    {NULL},
    NULL,
    NULL,
};
...
struct skywalk_test skt_reass_timeout = {
    "reass_timeout",
    "send partial fragment to flowswitch and check for ICMPv6 time exceeded "
    "reply",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    skt_reass_timeout_main,
    {NULL},
    sktc_enable_ip_reass,
    sktc_restore_ip_reass,
};
...
struct skywalk_test skt_reass_bad_fraglen = {
    "reass_bad_fraglen",
    "send fragment with bad fragment length (!= 8*) to flowswitch and check "
    "for ICMPv6 param header reply",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    skt_reass_bad_fraglen_main,
    {NULL},
    sktc_enable_ip_reass,
    sktc_restore_ip_reass,
};
...
struct skywalk_test skt_reass_fuzz_queue_limit = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    skt_reass_fuzz_queue_limit_main,
    {NULL},
    sktc_enable_ip_reass,
    sktc_restore_ip_reass,
};
