 ...
 * <rdar://problem/29003665> need to control skywalk behavior at limit of too
 * many flows
 */
...
static void do_flows(nexus_controller_t ncd, const uuid_t fsw,
                     nexus_port_t nx_port_start, int nports,
                     int nflows_per_port, uuid_t flows[]) {
  for (int i = 0; i < nports; i++) {
    for (int j = 0; j < nflows_per_port; j++) {
      int error;
      uuid_generate_random(flows[i * nflows_per_port + j]);

      error = sktc_bind_tcp4_flow(ncd, fsw, 0, nx_port_start + i,
                                  flows[i * nflows_per_port + j]);
      SKTC_ASSERT_ERR(!error); // XXX some of these are expected
    }
  }
}

static void undo_flows(nexus_controller_t ncd, const uuid_t fsw, int nflows,
                       const uuid_t flows[]) {
...
static int skt_manyflows_common(int nports, int nflows_per_port) {
  struct sktc_nexus_handles handles;
  sktc_create_flowswitch(&handles, 0);

  uuid_t flows[nports * nflows_per_port];

  do_flows(handles.controller, handles.fsw_nx_uuid,
           NEXUS_PORT_FLOW_SWITCH_CLIENT, nports, nflows_per_port, flows);
  undo_flows(handles.controller, handles.fsw_nx_uuid,
             sizeof(flows) / sizeof(flows[0]), flows);
...
static int skt_mf10x10_main(int argc, char *argv[]) {
  return skt_manyflows_common(10, 10);
}

static int skt_mf10x100_main(int argc, char *argv[]) {
  return skt_manyflows_common(10, 100);
}

static int skt_mf100x10_main(int argc, char *argv[]) {
  return skt_manyflows_common(100, 10);
}

static int skt_mf100x100_main(int argc, char *argv[]) {
  return skt_manyflows_common(100, 100);
}

struct skywalk_test skt_mf10x10 = {
    "mf10x10",
    "test binds 10 ports with 10 flows per port",
...
struct skywalk_test skt_mf10x100 = {
    "mf10x100",
    "test binds 10 ports with 100 flows per port",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    skt_mf10x100_main,
    {NULL},
...
struct skywalk_test skt_mf100x10 = {
    "mf100x10",
    "test binds 100 ports with 10 flows per port",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    skt_mf100x10_main,
    {NULL},
...
struct skywalk_test skt_mf100x100 = {
    "mf100x100",
    "test binds 100 ports with 100 flows per port",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    skt_mf100x100_main,
    {NULL},
...
static int skt_mf1xall_common(bool stop_on_error) {
  int error;
  int first, last;
  size_t size;
  int i, nports, count;
  ...
  for (i = 0; i < nports; i++) {
    ...
    error = sktc_bind_tcp4_flow(handles.controller, handles.fsw_nx_uuid, 0,
                                NEXUS_PORT_FLOW_SWITCH_CLIENT, flows[i]);
    if (error) {
  ...
  error = sktc_bind_tcp4_flow(handles.controller, handles.fsw_nx_uuid, 0,
                              NEXUS_PORT_FLOW_SWITCH_CLIENT, extraflow);
  ...
  for (i = 0; i < nports; i++) {
    if (!uuid_is_null(flows[i])) {
      error =
          sktc_unbind_flow(handles.controller, handles.fsw_nx_uuid, flows[i]);
      SKTC_ASSERT_ERR(!error);
    }
  }
...
int skt_mf1xall_main(int argc, char *argv[]) {
  return skt_mf1xall_common(true);
}

int skt_mf1xallslow_main(int argc, char *argv[]) {
  return skt_mf1xall_common(false);
}

struct skywalk_test skt_mf1xall = {
    "mf1xall",
    "test binds all the ephemeral port flows on a single port until it hits "
    "failure",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    skt_mf1xall_main,
...
struct skywalk_test skt_mf1xallslow = {
    "mf1xallslow",
    "test binds all the ephemeral port flows on a single port ",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    skt_mf1xallslow_main,
    {NULL},
    sktc_ifnet_feth0_create,
    sktc_ifnet_feth0_destroy,
};
...
static void skt_mc10x10_init(void) {
  sktc_ifnet_feth0_create();
  sktc_create_flowswitch(&mchandles, 0);
  do_flows(mchandles.controller, mchandles.fsw_nx_uuid,
           NEXUS_PORT_FLOW_SWITCH_CLIENT, 10, 10, mcflows);
  uuid_unparse(mchandles.fsw_nx_uuid, fsw_uuid_string);
}

static void skt_mc10x100_init(void) {
  sktc_ifnet_feth0_create();
  sktc_create_flowswitch(&mchandles, 0);
  do_flows(mchandles.controller, mchandles.fsw_nx_uuid,
           NEXUS_PORT_FLOW_SWITCH_CLIENT, 10, 100, mcflows);
  uuid_unparse(mchandles.fsw_nx_uuid, fsw_uuid_string);
}

static void skt_mc100x10_init(void) {
  sktc_ifnet_feth0_create();
  sktc_create_flowswitch(&mchandles, 0);
  do_flows(mchandles.controller, mchandles.fsw_nx_uuid,
           NEXUS_PORT_FLOW_SWITCH_CLIENT, 100, 10, mcflows);
  uuid_unparse(mchandles.fsw_nx_uuid, fsw_uuid_string);
}

static void skt_mc100x100_init(void) {
  sktc_ifnet_feth0_create();
  sktc_create_flowswitch(&mchandles, 0);
  do_flows(mchandles.controller, mchandles.fsw_nx_uuid,
           NEXUS_PORT_FLOW_SWITCH_CLIENT, 100, 100, mcflows);
  uuid_unparse(mchandles.fsw_nx_uuid, fsw_uuid_string);
}

static void skt_mc100_fini(void) {
  undo_flows(mchandles.controller, mchandles.fsw_nx_uuid, 100, mcflows);
  sktc_cleanup_flowswitch(&mchandles);
  sktc_ifnet_feth0_destroy();
}

static void skt_mc1000_fini(void) {
  undo_flows(mchandles.controller, mchandles.fsw_nx_uuid, 10 * 100, mcflows);
  sktc_cleanup_flowswitch(&mchandles);
  sktc_ifnet_feth0_destroy();
}

static void skt_mc10000_fini(void) {
  undo_flows(mchandles.controller, mchandles.fsw_nx_uuid, 100 * 100, mcflows);
  sktc_cleanup_flowswitch(&mchandles);
  sktc_ifnet_feth0_destroy();
}

int skt_mcflows_main(int argc, char *argv[]) {
  char buf[1] = {0};
  ssize_t ret;
  int error;
  uuid_t fsw_uuid;
  ...
  T_LOG("opening channel %d on fsw %s\n", NEXUS_PORT_FLOW_SWITCH_CLIENT + child,
        argv[5]);

  channel = sktu_channel_create_extended(
      fsw_uuid, NEXUS_PORT_FLOW_SWITCH_CLIENT + child, CHANNEL_DIR_TX_RX,
...
struct skywalk_mptest skt_mc10x10 = {
    "mc10x10",
    "test forks 10 processes each opening a channel with 10 flows",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    10,
    skt_mcflows_main,
    {NULL, NULL, NULL, NULL, NULL, fsw_uuid_string},
    skt_mc10x10_init,
    skt_mc100_fini,
};
...
struct skywalk_mptest skt_mc10x100 = {
    "mc10x100",
    "test forks 10 processes each opening a channel with 100 flows",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    10,
    skt_mcflows_main,
...
struct skywalk_mptest skt_mc100x10 = {
    "mc100x10",
    "test forks 100 processes each opening a channel with 10 flows",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    100,
    skt_mcflows_main,
...
struct skywalk_mptest skt_mc100x100 = {
    "mc100x100",
    "test forks 100 processes each opening a channel with 100 flows",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    100,
    skt_mcflows_main,
