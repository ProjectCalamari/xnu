...
static int skt_shutdown_common(int argc, char *argv[], int method) {
  int error;
  channel_t channel;
  uuid_t channel_uuid;
  ring_id_t ringid;
  ...
  case 1:
    ...
    T_LOG("poll sock POLLWRNORM: error(%d) events(0x%x)"
          " revents(0x%x)\n",
          error, sock_fds.events, sock_fds.revents);
    SKTC_ASSERT_ERR(error != -1);
    SKTC_ASSERT_ERR(error == 1);
  ...
  case 2:
    ...
    assert(kq != -1);
    /* event registration */
    EV_SET(&sock_kev, sock_fd, EVFILT_WRITE, EV_ADD | EV_ENABLE, 0, 0, NULL);
    ...
    kq = kqueue();
    assert(kq != -1);
    EV_SET(&kev, channelfd, EVFILT_NW_CHANNEL, EV_ADD | EV_ENABLE, 0, 0, NULL);
    error = kevent(kq, &kev, 1, &kev, 1, NULL);
  ...
  case 2:
    ...
    assert(kq != -1);
    /* event registration */
    EV_SET(&sock_kev, sock_fd, EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, NULL);
    error = kevent(kq, &sock_kev, 1, &sock_kev, 1, NULL);
...
static int skt_shutdown_select_main(int argc, char *argv[]) {
  return skt_shutdown_common(argc, argv, 0);
}

static int skt_shutdown_poll_main(int argc, char *argv[]) {
  return skt_shutdown_common(argc, argv, 1);
}

static int skt_shutdown_kqueue_main(int argc, char *argv[]) {
  return skt_shutdown_common(argc, argv, 2);
}

struct skywalk_test skt_shutdownus = {
    "shutdownus",
    "shuts down channel on upipe and calls select",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_USER_PIPE,
    skt_shutdown_select_main,
...
struct skywalk_test skt_shutdownks = {
    "shutdownks",
    "shuts down channel on kpipe and calls select",
...
struct skywalk_test skt_shutdownup = {
    "shutdownup",
    "shuts down channel on upipe and calls poll",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_USER_PIPE,
    skt_shutdown_poll_main,
    SKTC_GENERIC_UPIPE_ARGV,
    sktc_generic_upipe_nexus_init,
    sktc_cleanup_nexus,
};
...
struct skywalk_test skt_shutdownkp = {
    "shutdownkp",
    "shuts down channel on kpipe and calls poll",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_KERNEL_PIPE |
        SK_FEATURE_NEXUS_KERNEL_PIPE_LOOPBACK,
    skt_shutdown_poll_main,
    SKTC_GENERIC_KPIPE_ARGV,
...
struct skywalk_test skt_shutdownuk = {
    "shutdownuk",
    "shuts down channel on upipe and calls kqueue",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_USER_PIPE,
    skt_shutdown_kqueue_main,
    SKTC_GENERIC_UPIPE_ARGV,
    sktc_generic_upipe_nexus_init,
    sktc_cleanup_nexus,
};
...
struct skywalk_test skt_shutdownkk = {
    "shutdownkk",
    "shuts down channel on kpipe and calls kqueue",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_KERNEL_PIPE |
        SK_FEATURE_NEXUS_KERNEL_PIPE_LOOPBACK,
    skt_shutdown_kqueue_main,
    SKTC_GENERIC_KPIPE_ARGV,
