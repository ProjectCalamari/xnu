...
static int skt_debug_verify_main(int argc, char *argv[]) {
  int error;
  channel_t channel;
  uuid_t channel_uuid;
  ring_id_t ringid;
 ...
 */
void skt_debug_verify_u_init(void) {
  int error;
  ...
  if (error) {
    T_LOG("%s: warning sysctl(\"kern.skywalk.debug\" returned %d: %s", __func__,
...
void skt_debug_verify_u_fini(void) {
  int error;
...
void skt_debug_verify_k_init(void) {
  int error;
...
void skt_debug_verify_k_fini(void) {
  int error;
...
struct skywalk_test skt_debug_verify_u = {
    "debug_verify_u",
    "test confirms that skywalk is storing checksums of slots received on a "
    "upipe when in SKF_VERIFY debug mode",
    SK_FEATURE_SKYWALK | SK_FEATURE_DEV_OR_DEBUG | SK_FEATURE_NEXUS_USER_PIPE,
    skt_debug_verify_main,
    SKTC_GENERIC_UPIPE_ARGV,
    skt_debug_verify_u_init,
    skt_debug_verify_u_fini,
};

struct skywalk_test skt_debug_verify_k = {
    "debug_verify_k",
    "test confirms that skywalk is storing checksums of slots received on a "
    "kpipe when in SKF_VERIFY debug mode",
    SK_FEATURE_SKYWALK | SK_FEATURE_DEV_OR_DEBUG |
        SK_FEATURE_NEXUS_KERNEL_PIPE | SK_FEATURE_NEXUS_KERNEL_PIPE_LOOPBACK,
    skt_debug_verify_main,
    SKTC_GENERIC_KPIPE_ARGV,
    skt_debug_verify_k_init,
    skt_debug_verify_k_fini,
};
