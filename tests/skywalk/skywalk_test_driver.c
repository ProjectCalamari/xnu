...
static void skywalk_test_driver_init(bool test_shutdown, bool ignore_failures) {
  kern_return_t kr;
  ...
  if ((kr = mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE,
                               &exc_port)) != KERN_SUCCESS) {
    mach_error("mach_port_allocate", kr);
    test_exit(1);
  }

  if ((kr = mach_port_insert_right(mach_task_self(), exc_port, exc_port,
  ...
  if ((error = pthread_create(&exception_thread, NULL, server_thread, 0)) !=
      0) {
...
int skywalk_test_driver_run(struct skywalk_test *skt, int argc, char **argv,
                            uint32_t memfail, bool ignorefail, bool doshutdown,
                            int itersecs) {
  kern_return_t kr;
  ...
  if (sysctlbyname("kern.skywalk.mem.region_mtbf", &memfail_original, &len,
                   NULL, 0) != 0) {
  ...
  len = sizeof(memfail);
  if (sysctlbyname("kern.skywalk.mem.region_mtbf", NULL, NULL, &memfail, len) !=
      0) {
  ...
  do {
    ...
    if ((kr = task_get_exception_ports(
             mach_task_self(), EXC_MASK_GUARD | EXC_MASK_CRASH, masks,
             &maskCount, handlers, behaviors, flavors)) != KERN_SUCCESS) {
    ...
    for (int j = 0; j < maskCount; j++) {
      ...
      if ((kr = task_set_exception_ports(
               mach_task_self(), masks[j], exc_port,
               EXCEPTION_DEFAULT | MACH_EXCEPTION_CODES, flavors[j])) !=
          KERN_SUCCESS) {
    ...
    if (pid == 0) {
      ...
      if (sysctlbyname("kern.skywalk.mem.region_mtbf", &memfail_original, &len,
                       NULL, 0) != 0) {
        SKT_LOG(
            "warning got errno %d getting kern.skywalk.mem.region_mtbf: %s\n",
            errno, strerror(errno));
      }
      if (memfail_original) {
        CRSetCrashLogMessage(
            "kern.skywalk.mem.region_mtbf was found already set");
      }
    ...
    for (int j = 0; j < maskCount; j++) {
      if ((kr = task_set_exception_ports(
               mach_task_self(), masks[j], handlers[j],
               EXCEPTION_DEFAULT | MACH_EXCEPTION_CODES, flavors[j])) !=
          KERN_SUCCESS) {
    ...
    if (WIFSIGNALED(child_status)) {
      T_LOG("Child signaled with signal %d coredump %d", WTERMSIG(child_status),
