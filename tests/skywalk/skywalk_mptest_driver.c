 ...
 */
static void print_extra_stats(struct skywalk_mptest *test) {
  struct protox *tp;
...
static void print_fsw_stats(void) {
...
void skywalk_mptest_driver_SIGABRT_handler(int s) { print_fsw_stats(); }

void skywalk_mptest_driver_init(void) {
  size_t len;
...
bool skywalk_mptest_supported(const char *name) {
  int i;
...
int skywalk_mptest_driver_run(struct skywalk_mptest *skt, bool ignoreskip) {
  ...
  for (int j = 0; j < skt->skt_nchildren; j++) {
    ...
    if (WIFEXITED(child_status)) {
      if (WEXITSTATUS(child_status)) {
        T_LOG("Child %d (pid %d) exited with status %d", child, childpid,
              WEXITSTATUS(child_status));
      }
    }
    if (WIFSIGNALED(child_status)) {
      T_LOG("Child %d (pid %d) signaled with signal %d coredump %d", child,
            childpid, WTERMSIG(child_status), WCOREDUMP(child_status));
    }
      ...
      if (childfail == 1) {
        ...
        for (int k = 0; k < skt->skt_nchildren; k++) {
          if (pids[k]) {
            ...
            if (error == 0) {
              T_LOG("Delivered SIGTERM to child %d pid %d", k, pids[k]);
            } else if (errno != ESRCH) {
  ...
  if (childfail) {
    ...
    if ((skt->skt_required_features & SK_FEATURE_NEXUS_KERNEL_PIPE) != 0 &&
        geteuid() != 0) {
      T_LOG("%sPlease try running test as root%s", BOLD, NORMAL);
    }
  } else {
