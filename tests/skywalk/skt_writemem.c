...
static int skt_writeif_main(int argc, char *argv[]) {
  int error;
  channel_t channel;
  uuid_t channel_uuid;
...
static int skt_writering_main(int argc, char *argv[]) {
  int error;
  channel_t channel;
  uuid_t channel_uuid;
  error = uuid_parse(argv[3], channel_uuid);
...
static int skt_readsmap_main(int argc, char *argv[]) {
  int error;
  channel_t channel;
  uuid_t channel_uuid;
  uint8_t byte;
  ...
  memcpy(&byte,
         (void *)channel->chd_schema +
             channel->chd_schema->csm_ring_ofs[0].sd_off,
         1);
...
static int skt_writesmap_main(int argc, char *argv[]) {
  int error;
  channel_t channel;
  uuid_t channel_uuid;
  uint8_t byte;
  ...
  // Now try to write it
  memcpy((void *)channel->chd_schema +
             channel->chd_schema->csm_ring_ofs[0].sd_off,
         &byte, 1);
...
static void skt_ms_fsw_init(void) {
  uuid_t flow_id;
  int error;
  ...
  error =
      sktc_bind_tcp4_flow(ms_fsw_handle.controller, ms_fsw_handle.fsw_nx_uuid,
...
static void skt_ms_fsw_fini(void) {
  sktc_cleanup_flowswitch(&ms_fsw_handle);
  sktc_ifnet_feth0_destroy();
}

static void skt_verify_nxadv_region(channel_t channel) {
...
int skt_nxregion_verify_main(int argc, char *argv[]) {
  int error;
  int test_id;
  uuid_t fsw_uuid;
  channel_t channel;
  ...
  assert(sktu_channel_create_extended(fsw_uuid, NEXUS_PORT_FLOW_SWITCH_CLIENT,
                                      CHANNEL_DIR_TX_RX, CHANNEL_RING_ID_ANY,
                                      NULL, -1, -1, -1, -1, -1, -1, 1, -1,
                                      -1) == NULL);

  channel = sktu_channel_create_extended(
      fsw_uuid, NEXUS_PORT_FLOW_SWITCH_CLIENT, CHANNEL_DIR_TX_RX,
...
struct skywalk_test skt_verifynxadv = {
    ...
    .skt_argv = {NULL, NULL, NULL, ms_fsw_uuid_string,
                 SKT_VERIFY_NXADV_REGION_STR},
