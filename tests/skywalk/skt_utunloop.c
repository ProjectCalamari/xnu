  ...
  do {                                                                         \
    if (g_verbose) {                                                           \
      ...
      fprintf(g_dumpfile, "% 10ld.%06d % 10ld.%06d %s: " _fmt "\n",            \
              _stamp.tv_sec, _stamp.tv_usec, _delta.tv_sec, _delta.tv_usec,    \
              threadname, ##__VA_ARGS__);                                      \
      fflush(g_dumpfile);                                                      \
      prevtime = _stamp;                                                       \
    }                                                                          \
  } while (0)
...
static void skt_utunloop_xfer_slots(int kq, channel_t rxchannel, int rxindex,
  ...
  rxring = os_channel_rx_ring(
      rxchannel,
      rxindex + os_channel_ring_id(rxchannel, CHANNEL_FIRST_RX_RING));
  assert(rxring);
  txring = os_channel_tx_ring(
      txchannel,
      txindex + os_channel_ring_id(txchannel, CHANNEL_FIRST_TX_RING));
  assert(txring);
  ...
  while (!g_die) {
    ...
    do {
      ...
      if (!rxavail || !txavail) {
        ...
        VLOG("waiting rxen %d rx %u txen %d tx %u", rxenable, rxavail, txenable,
             txavail);
      ...
      if (now > then) {
        T_LOG("%s: time %ld slotcount %llu "
              ...
              threadname, now - start, slotcount - prevslotcount, slotcount,
        ...
        if ((now - start) > 0 && (slotcount - prevslotcount) == 0) {
          ...
          if ((xfer12 && g_assert_stalls12) || (!xfer12 && g_assert_stalls21)) {
            ...
            if (stallcount == 5) {
              ...
              //						T_LOG("%s:
              //Sleeping\n", threadname); 						sleep(3600000);
              T_LOG("%s: exiting because of stall\n", threadname);
              exit(252);
#elif 0
              T_LOG("%s: enabling verbose\n", threadname);
              uint64_t verbose = (1ULL << 50);
              error = sysctlbyname("kern.skywalk.verbose", NULL, NULL, &verbose,
                                   sizeof(verbose));
              SKTC_ASSERT_ERR(!error);
#endif
            }
...
static void *sktc_channel_worker_xfer12(void *ignored) {
  pthread_setname_np(__func__);
  skt_utunloop_xfer_slots(g_kq1, g_channel1, 0, g_channel2, 0, __func__, true);
  return NULL;
}

static void *sktc_channel_worker_xfer21(void *ignored) {
  pthread_setname_np(__func__);
...
static int makesocket(int type, in_addr_t addr) {
  int error;
  int s;
  ...
  T_LOG("%s socket %d bound to %s port %d\n",
        type == SOCK_DGRAM ? "udp" : "tcp", s, sbuf, ntohs(sin.sin_port));

  return s;
}

static void connectsocks(int s1, int s2, bool block) {
  int error;
  struct sockaddr_in sin;
  socklen_t slen;
  char sbuf[INET6_ADDRSTRLEN];
  ...
  T_LOG("socket %d connect%s to %s port %d\n", s1, block ? "ed" : "ing", sbuf,
        ntohs(sin.sin_port));
}

static int acceptsock(int s) {
  int error;
  struct sockaddr_in sin;
  socklen_t slen;
  char sbuf[INET6_ADDRSTRLEN];
...
static void *sinkudp(void *sockfd) {
  int s = *(int *)sockfd;
  ssize_t len;
  char buf[UDPSIZE];
  char threadname[20];
...
static void *sourceudp(void *sockfd) {
  int s = *(int *)sockfd;
  ssize_t len;
  char buf[UDPSIZE];
  char threadname[20];
  ...
  T_LOG("%s wrote %d packets, looped %u times (avg %f) exiting\n", threadname,
        udpxfer, loops, (double)totalloop / loops);
...
static void *sinktcp(void *sockfd) {
  int s = *(int *)sockfd;
  ssize_t len;
  char *buf;
  int buflen;
...
static void *sourcetcp(void *sockfd) {
  int s = *(int *)sockfd;
  ssize_t len;
  char *buf;
  int buflen;
...
static void dotraffic(void *(*sourcefunc)(void *), void *(*sinkfunc)(void *),
                      int sourcesock1, int sinksock2, int sourcesock2,
                      int sinksock1) {
  int error;
...
static void skt_tunloop_common(bool doutun, bool enable_netif,
                               bool enable_channel, bool udp, bool udpduplex,
                               bool tcp, bool tcpduplex, bool dualstream) {
  int error;
  int utun1, utun2;
  ...
  len = sizeof(memfail);
  if (sysctlbyname("kern.skywalk.mem.region_mtbf", &memfail, &len, NULL, 0) !=
      0) {
  ...
  snprintf(debugcmd, sizeof(debugcmd),
           "netstat -qq -I %s > netstatqq.%s.txt; netstat -qq -I %s > "
           "netstatqq.%s.txt; skywalkctl netstat --flowswitch --netif > "
           "skywalkctl.txt",
  ...
  g_kq1 = kqueue();
  EV_SET(&kev, (uintptr_t)&g_die, EVFILT_USER, EV_ADD | EV_ENABLE, 0, 0, NULL);
...
static int skt_utunloopy4u1_main(int argc, char *argv[]) {
  g_assert_stalls12 = true;
  skt_tunloop_common(true, true, true, true, false, false, false, false);
  return 0;
}

static int skt_utunloopy4u2_main(int argc, char *argv[]) {
  g_assert_stalls12 = true;
...
static int skt_utunloopy4t1_main(int argc, char *argv[]) {
  g_assert_stalls12 = true;
  skt_tunloop_common(true, true, true, false, false, true, false, false);
  return 0;
}

static int skt_utunloopy4t2_main(int argc, char *argv[]) {
  g_assert_stalls12 = true;
  g_assert_stalls21 = true;
...
static int skt_utunloopy1000_main(int argc, char *argv[]) {
...
struct skywalk_test skt_utunloopy1000 = {
    "utunloopy1000",
    "open 2 utuns with netif and sleeps for 1000 seconds",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_KERNEL_PIPE,
...
static int skt_ipsecloopy4u1_main(int argc, char *argv[]) {
  g_assert_stalls12 = true;
  skt_tunloop_common(false, true, true, true, false, false, false, false);
  return 0;
}

static int skt_ipsecloopy4u2_main(int argc, char *argv[]) {
  g_assert_stalls12 = true;
  g_assert_stalls21 = true;
...
static int skt_ipsecloopy4t1_main(int argc, char *argv[]) {
  g_assert_stalls12 = true;
  skt_tunloop_common(false, true, true, false, false, true, false, false);
  return 0;
}

static int skt_ipsecloopy4t2_main(int argc, char *argv[]) {
  g_assert_stalls12 = true;
  g_assert_stalls21 = true;
...
static int skt_ipsecloopy1000_main(int argc, char *argv[]) {
...
struct skywalk_test skt_ipsecloopy1000 = {
    "ipsecloopy1000",
    "open 2 ipsecs with netif and sleeps for 1000 seconds",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_KERNEL_PIPE,
