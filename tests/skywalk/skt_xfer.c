...
#define _S2(code, a, b)                                                        \
  {                                                                            \
    (code), IC_RMASK_UNSPEC, { (a), (b) }                                      \
  }
#define _S3(code, a, b, c)                                                     \
  {                                                                            \
    (code), a, { (b), (c) }                                                    \
  }
...
static const struct fsw_inject_codes fsw_inject_codes[] = {
    ...
    /* fsw_resolve() returns error other than EJUSTRETURN but flow route has
       stale entry */
...
#define INJECT_CODE_COUNT                                                      \
  (sizeof(fsw_inject_codes) / sizeof(fsw_inject_codes[0]))

static packet_svc_class_t packet_svc_class[] = {
    PKT_SC_BK_SYS, PKT_SC_BK, PKT_SC_BE, PKT_SC_RD, PKT_SC_OAM,
    PKT_SC_AV,     PKT_SC_RV, PKT_SC_VI, PKT_SC_VO, PKT_SC_CTL};

#define NUM_SVC_CLASS (sizeof(packet_svc_class) / sizeof(packet_svc_class[0]))
#define XFER_WMM_PING_BATCH_COUNT 8
#define XFER_WMM_PING_PACKET_COUNT (XFER_WMM_PING_BATCH_COUNT * NUM_SVC_CLASS)
...
static inline uint16_t skt_xfer_fold_sum_final(uint32_t sum) {
...
static int connect_flow(nexus_controller_t ncd, const uuid_t fsw,
                        nexus_port_t nx_port, const uuid_t flow, int protocol,
                        uint16_t flags, struct in_addr src_addr,
                        in_port_t src_port, struct in_addr dst_addr,
                        in_port_t dst_port, flowadv_idx_t *flowadv_idx,
                        uint64_t qset_id) {
...
static int connect_child_flow(
    nexus_controller_t ncd, const uuid_t fsw, nexus_port_t nx_port,
    const uuid_t flow, int protocol, uint16_t flags, struct in_addr src_addr,
    in_port_t src_port, struct in_addr dst_addr, in_port_t dst_port,
...
static inline uint32_t skt_xfer_get_chan_max_frags(const channel_t chd) {
  return (uint32_t)sktc_get_channel_attr(chd, CHANNEL_ATTR_MAX_FRAGS);
}

static inline void sktc_xfer_copy_data_to_packet(
    channel_port_t port, packet_t ph, const void *data, uint16_t data_len,
    uint16_t start_offset, bool csum_offload, uint32_t *partial_csum) {
  char *baddr;
  ...
  while (len != 0) {
    ...
    if (csum_offload) {
      ...
      partial = ~os_copy_and_inet_checksum(data, baddr, clen, partial);
    }
    len -= clen;
    blen -= clen;
    data += clen;
...
#if SKT_XFER_DEBUG
static const char *inet_ptrtoa(const void *ptr) {
  struct in_addr ip;
...
static void ip_frame_dump(const void *buf, size_t buf_len) {
  ...
  case IPPROTO_UDP: {
    ...
    T_LOG("UDP src 0x%x dst 0x%x len %d csum 0x%x datalen %d\n",
          ntohs(ip_udp->udp.uh_sport), ntohs(ip_udp->udp.uh_dport), udp_len,
          ntohs(ip_udp->udp.uh_sum), data_len);
    break;
  }
  case IPPROTO_TCP: {
    assert(buf_len >= sizeof(*ip_tcp));
    T_LOG("TCP src 0x%x dst 0x%x seq %u ack %u "
          "off %d flags 0x%x win %d csum 0x%x\n",
          ntohs(ip_tcp->tcp.th_sport), ntohs(ip_tcp->tcp.th_dport),
          ntohl(ip_tcp->tcp.th_seq), ntohl(ip_tcp->tcp.th_ack),
          ip_tcp->tcp.th_off, ip_tcp->tcp.th_flags, ntohs(ip_tcp->tcp.th_win),
...
static size_t tcp_frame_populate(channel_port_t port, packet_t ph,
                                 struct in_addr src_ip, uint16_t src_port,
                                 struct in_addr dst_ip, uint16_t dst_port,
                                 const void *data, size_t data_len,
                                 bool connect, bool csum_offload) {
  int error;
  size_t frame_length;
  ip_tcp_header_t *ip_tcp;
  char *baddr;
  ...
  assert((skt_xfer_get_chan_max_frags(port->chan) * bdlim) >= frame_length);
#else
  ...
  tcp_pseudo =
      (tcp_pseudo_hdr_t *)(((char *)&ip_tcp->tcp) - sizeof(*tcp_pseudo));
  baddr += sizeof(*ip_tcp);
  ...
  if (csum_offload) {
    ip_tcp->tcp.th_sum =
        in_pseudo(src_ip.s_addr, dst_ip.s_addr,
                  htons(data_len + sizeof(ip_tcp->tcp) + IPPROTO_TCP));
    os_packet_set_inet_checksum(ph, PACKET_CSUM_PARTIAL, sizeof(struct ip),
                                sizeof(struct ip) +
    ...
    partial = os_inet_checksum(
        tcp_pseudo, sizeof(*tcp_pseudo) + sizeof(ip_tcp->tcp), partial);
...
static size_t udp_frame_populate(channel_port_t port, packet_t ph,
                                 struct in_addr src_ip, uint16_t src_port,
                                 struct in_addr dst_ip, uint16_t dst_port,
                                 const void *data, size_t data_len,
                                 bool csum_offload, uint16_t fragment_id,
                                 size_t total_udp_len) {
  int error;
  size_t frame_length;
  ip_udp_header_t *ip_udp;
  char *baddr;
  ...
  udp_pseudo =
      (udp_pseudo_hdr_t *)(((char *)&ip_udp->udp) - sizeof(*udp_pseudo));
  baddr += sizeof(*ip_udp);
  ...
  if (csum_offload) {
    ...
    os_packet_set_inet_checksum(
        ph, PACKET_CSUM_PARTIAL | PACKET_CSUM_ZERO_INVERT, sizeof(struct ip),
    ...
    partial = os_inet_checksum(
        udp_pseudo, sizeof(*udp_pseudo) + sizeof(ip_udp->udp), partial);
...
static size_t ip_frame_populate(channel_port_t port, packet_t ph,
                                uint8_t protocol, struct in_addr src_ip,
                                struct in_addr dst_ip, const void *data,
                                size_t data_len, uint16_t fragment_id,
                                uint16_t fragment_offset, bool last_fragment) {
  int error;
  size_t frame_length;
  struct ip *ip;
  char *baddr;
  ...
  /* copy the data */
  sktc_xfer_copy_data_to_packet(port, ph, data, data_len, sizeof(*ip), TRUE,
                                NULL);
  return frame_length;
}

static size_t frame_populate(channel_port_t port, packet_t ph, int protocol,
                             struct in_addr src_ip, uint16_t src_port,
                             struct in_addr dst_ip, uint16_t dst_port,
                             const void *data, size_t data_len, uuid_t flow_id,
                             bool connect, packet_svc_class_t svc_class,
                             bool csum_offload, uint16_t fragment_id,
                             size_t total_data_len, uint16_t fragment_offset,
                             bool last_fragment) {
  size_t ret;
  int error;
  ...
  switch (protocol) {
  case IPPROTO_TCP:
    ret = tcp_frame_populate(port, ph, src_ip, src_port, dst_ip, dst_port, data,
                             data_len, connect, csum_offload);
    break;
  case IPPROTO_UDP:
    assert(connect == FALSE);
    if (fragment_offset > 0) {
      ret =
          ip_frame_populate(port, ph, protocol, src_ip, dst_ip, data, data_len,
                            fragment_id, fragment_offset, last_fragment);
    } else {
      ret = udp_frame_populate(port, ph, src_ip, src_port, dst_ip, dst_port,
                               data, data_len, csum_offload, fragment_id,
                               total_data_len);
    }
    break;
  default:
    ret = ip_frame_populate(port, ph, protocol, src_ip, dst_ip, data, data_len,
                            fragment_id, fragment_offset, last_fragment);
    break;
  }
...
static size_t tcp_frame_process(packet_t ph, void *data, size_t data_max) {
  buflet_t buflet;
  ...
  buf =
      os_buflet_get_object_address(buflet) + os_buflet_get_data_offset(buflet);
  ip_tcp = (ip_tcp_header_t *)buf;
  ...
  csum =
      in_pseudo(ip_tcp->ip.ip_src.s_addr, ip_tcp->ip.ip_dst.s_addr,
...
static size_t udp_frame_process(packet_t ph, void *data, size_t data_max) {
  buflet_t buflet;
  ...
  /* verify UDP checksum */
  if (ip_udp->ip.ip_off == 0 && ip_udp->udp.uh_sum != 0) {
    csum =
        in_pseudo(ip_udp->ip.ip_src.s_addr, ip_udp->ip.ip_dst.s_addr,
...
static size_t ip_frame_process(packet_t ph, void *data, size_t data_max) {
  buflet_t buflet;
...
static size_t frame_process(packet_t ph, void *data, size_t data_max,
                            bool verify_qos) {
  buflet_t buflet;
  void *buf;
  struct ip *ip;
...
static void channel_port_send(channel_port_t port, uuid_t flow_id, int protocol,
                              uint16_t src_port, struct in_addr dst_ip,
                              uint16_t dst_port, my_payload_t payload,
                              int payload_length, uint32_t limit,
                              bool must_complete_batch, bool connect,
                              packet_svc_class_t svc_class, bool csum_offload,
                              void (^packet_prehook)(packet_t p)) {
  int error;
  channel_slot_t last_slot = NULL;
  packet_id_t pktid = {OS_PACKET_PKTID_VERSION_CURRENT,
                       XFER_PKTID_PAYLOAD_TYPE,
                       0,
                       0,
                       XFER_PKTID_STREAM_ID,
                       0};
  ...
  while (1) {
    ...
    slot = os_channel_get_next_slot(port->tx_ring, last_slot, &prop);
    if (slot == NULL) {
      if (must_complete_batch && payload->packet_number < limit) {
        /* couldn't complete batch */
        T_LOG("TX didn't complete batch (%u < %u)\n", payload->packet_number,
              limit);
        assert(0);
      }
      break;
    ...
    frame_length = frame_populate(port, pkt, protocol, port->ip_addr, src_port,
                                  dst_ip, dst_port, (void *)payload,
                                  payload_length, flow_id, connect, svc_class,
                                  csum_offload, 0, payload_length, 0, FALSE);
    ...
    if (port->user_packet_pool) {
      ...
      os_channel_set_slot_properties(port->tx_ring, slot, &prop);
    }
    last_slot = slot;
    payload->packet_number++;
...
static void channel_port_send_fragments(
    channel_port_t port, uuid_t flow_id, int protocol, uint16_t src_port,
    struct in_addr dst_ip, uint16_t dst_port, my_payload_t payload,
    int payload_length, uint16_t fragment_count, packet_svc_class_t svc_class,
  ...
  for (int fragment_i = 0; fragment_i < fragment_count; fragment_i++) {
    ...
    if (slot == NULL) {
      if (fragment_i < fragment_count) {
        ...
        T_LOG("TX didn't complete fragment (%u < %u)\n", fragment_i,
              fragment_count);
        assert(0);
      }
      break;
    ...
    if (fragment_i == 0) {
      frame_length = frame_populate(
          port, pkt, protocol, port->ip_addr, src_port, dst_ip, dst_port,
          (void *)payload, payload_length, flow_id, FALSE, svc_class,
          csum_offload, fragment_id, fragment_count * payload_length, 0, FALSE);
    } else {
      frame_length = frame_populate(
          port, pkt, protocol, port->ip_addr, src_port, dst_ip, dst_port,
          (void *)payload, payload_length, flow_id, FALSE, svc_class,
          csum_offload, fragment_id, fragment_count * payload_length,
...
static int channel_port_receive_payload(channel_port_t port,
                                        my_payload_t payload, bool verify_qos) {
  int error;
  slot_prop_t prop;
  channel_slot_t slot;
  packet_t pkt;
  ...
  assert(pkt != 0);
  if (port->user_packet_pool) {
    error = os_channel_slot_detach_packet(port->rx_ring, slot, pkt);
    SKTC_ASSERT_ERR(error == 0);
  }
...
static void channel_port_receive(int child, channel_port_t port,
                                 uint16_t our_port, struct in_addr peer_ip,
                                 uint32_t limit, uint32_t *receive_count,
                                 uint32_t *receive_index, bool errors_ok,
  ...
  *pkts_dropped = 0;
  ...
  while (1) {
    ...
    slot = os_channel_get_next_slot(port->rx_ring, last_slot, &prop);
    if (slot == NULL) {
      break;
    }
    assert(prop.sp_buf_ptr != 0);
...
#if SKT_XFER_DEBUG
      T_LOG("child %d dropped %u (received #%u, expected #%u)\n", child,
            dropped, payload.packet_number, *receive_index);
#endif
...
static void channel_port_receive_all(channel_port_t port, uuid_t flow_id,
                                     uint16_t src_port, struct in_addr dst_ip,
                                     uint16_t dst_port,
                                     int16_t should_receive_count,
                                     bool verify_qos) {
  int error;
  ...
  for (;;) {
    ...
    if (error == 0) {
      /* time out */
      T_LOG("Error, timeout for final right packet\n");
      assert(0);
    }
    ...
    if (evlist.filter == EVFILT_READ) {
      ...
      T_LOG("Received [%d/%d] %s\n", received_count, should_receive_count,
            payload.data);
      if (should_receive_count >= 0 && received_count > should_receive_count) {
...
static void send_and_receive(channel_port_t port, uuid_t flow_id,
                             uint16_t src_port, struct in_addr dst_ip,
                             uint16_t dst_port, uint32_t how_many,
...
#define N_EVENTS_MAX 3
  ...
  if ((event_flags & SKT_FSW_EVFLAG_IFADV) != 0) {
    ...
    EV_SET(kev + n_events, port->fd, EVFILT_NW_CHANNEL, EV_ADD | EV_ENABLE,
           NOTE_IF_ADV_UPD, 0, NULL);
    n_events++;
  }
  if ((event_flags & SKT_FSW_EVFLAG_CHANNEL) != 0) {
    assert(n_events < N_EVENTS_MAX);
    EV_SET(kev + n_events, port->fd, EVFILT_NW_CHANNEL, EV_ADD | EV_ENABLE,
           NOTE_CHANNEL_EVENT, 0, NULL);
    n_events++;
  }
  ...
  while (!rx_complete || !tx_complete) {
    ...
    if (error == 0) {
      ...
      if (!errors_ok) {
        T_LOG("child %d: timed out, TX %s "
              "RX %s\n",
              child, tx_complete ? "complete" : "incomplete",
    ...
    for (int i = 0; i < error; i++) {
      ...
      case EVFILT_NW_CHANNEL: {
        ...
        if ((evlist[i].fflags & NOTE_CHANNEL_EVENT) != 0) {
          skt_process_channel_event(
              port->chan, XFER_PKTID_PAYLOAD_TYPE, XFER_PKTID_STREAM_ID,
      ...
      case EVFILT_WRITE: {
        ...
        channel_port_send(port, flow_id, IPPROTO_UDP, src_port, dst_ip,
                          dst_port, &payload, sizeof(payload), next_batch,
                          FALSE, FALSE, PKT_SC_BE, TRUE, NULL);
        if (payload.packet_number >= how_many) {
          assert(payload.packet_number == how_many);
          T_LOG("TX child %d: completed %u\n", child, how_many);
          tx_complete = TRUE;
#if SKT_XFER_DEBUG
          T_LOG("child %d: disable TX\n", child);
#endif
          EV_SET(kev, port->fd, EVFILT_WRITE, EV_DELETE, 0, 0, NULL);
          error = kevent(kq, kev, 1, NULL, 0, NULL);
          SKTC_ASSERT_ERR(error == 0);
        }
        break;
      ...
      case EVFILT_READ: {
        channel_port_receive(child, port, src_port, dst_ip, how_many,
                             &receive_packet_count, &receive_packet_index,
                             errors_ok, &pkts_dropped);
        if (receive_packet_index >= how_many) {
          assert(receive_packet_index == how_many);
#if SKT_XFER_DEBUG
          T_LOG("child %d: disable RX\n", child);
#endif
          EV_SET(kev, port->fd, EVFILT_READ, EV_DELETE, 0, 0, NULL);
          error = kevent(kq, kev, 1, NULL, 0, NULL);
          SKTC_ASSERT_ERR(error == 0);
          rx_complete = TRUE;
        }
      ...
      default:
        T_LOG("%lu event %d?\n", evlist[i].ident, evlist[i].filter);
        assert(0);
        break;
      }
    }
  ...
  T_LOG("RX child %d: received %u (of %u) %1.02f%%\n", child,
        receive_packet_count, how_many, percent);
...
static void ping_pong(channel_port_t port, uuid_t flow_id, uint16_t src_port,
                      struct in_addr dst_ip, uint16_t dst_port,
                      uint32_t how_many, uint32_t batch_size, int child,
                      bool wrong_flow_id, flowadv_idx_t flowadv_idx,
                      bool test_aqm, bool test_wmm, uint16_t demux_offset) {
  int error;
#define N_EVENTS 2
  ...
  if (test_aqm) {
     ...
     * check for that.
     */
    EV_SET(kev + 0, port->fd, EVFILT_NW_CHANNEL, EV_ADD, NOTE_FLOW_ADV_UPDATE,
           0, NULL);
    error = kevent(kq, kev, 1, NULL, 0, NULL);
    SKTC_ASSERT_ERR(error == 0);
  ...
  while (!rx_complete || !tx_complete) {
    ...
    if (error == 0) {
      T_LOG("child %d: timed out TX %s RX %s FA %s\n", child,
    ...
    if (expect_flowadv) {
      ...
      error = os_channel_flow_admissible(port->tx_ring, flow_id, flowadv_idx);
      if (expect_stall) {
        /*
        ...
        T_LOG("child %d, enable dequeue "
              "on feth0\n",
              child);
        error = sktc_ifnet_feth0_set_dequeue_stall(FALSE);
        SKTC_ASSERT_ERR(error == 0);
        expect_stall = FALSE;
        expect_flowadv = TRUE;
#if SKT_XFER_DEBUG
         ...
         * filter.
         */
        EV_SET(kev + 0, port->fd, EVFILT_NW_CHANNEL, EV_DELETE, 0, 0, NULL);
        ...
        assert(!rx_complete);
        /* enable RX */
        EV_SET(kev + n_kev, port->fd, EVFILT_READ, EV_ADD, 0, 0, NULL);
        n_kev++;
#if SKT_XFER_DEBUG
    ...
    if (sending) {
      ...
      if (test_wmm) {
        svc_class = packet_svc_class[(snd_batch_cnt % NUM_SVC_CLASS)];
      }
      snd_batch_cnt++;
      next_batch = payload.packet_number + batch_size;
      if (next_batch > how_many) {
      ...
      if (test_aqm && child == 0 && snd_batch_cnt == 2) {
        ...
        T_LOG("child %d, disable dequeue on"
              " feth0\n",
              child);
        error = sktc_ifnet_feth0_set_dequeue_stall(TRUE);
        SKTC_ASSERT_ERR(error == 0);
      }
      ...
      if (test_aqm && child == 0 && snd_batch_cnt == 3) {
        ...
        T_LOG("child %d, sleep for update"
              " interval (%d ms)\n",
              child, XFER_CLASSQ_UPDATE_INTERVAL_ELAPSE_DELAY);
        usleep(XFER_CLASSQ_UPDATE_INTERVAL_ELAPSE_DELAY * 1000);
      }
      ...
      channel_port_send(port, flow_id, IPPROTO_UDP, src_port, dst_ip, dst_port,
                        &payload, sizeof(payload), next_batch, TRUE, FALSE,
                        svc_class, TRUE, NULL);
#if SKT_XFER_DEBUG
      T_LOG("TX child %d: %s %u of %u\n", child, (child == 0) ? "ping" : "pong",
            next_batch, how_many);
#endif
      if (payload.packet_number >= how_many) {
        assert(payload.packet_number == how_many);
      ...
      T_LOG("child %d disable TX\n", child);
#endif
      EV_SET(kev + n_kev, port->fd, EVFILT_WRITE, EV_DELETE, 0, 0, NULL);
      n_kev++;
      ...
      pkts_dropped = 0;
      channel_port_receive(child, port, src_port, dst_ip, how_many,
                           &receive_packet_count, &receive_packet_index, false,
                           &pkts_dropped);
      ...
      if (pkts_dropped != 0) {
         ...
         */
        assert(test_aqm);
        assert(pkts_dropped == XFER_AQM_PING_BATCH_COUNT);
      }
      if (receive_packet_index >= how_many) {
        assert(receive_packet_index == how_many);
        rx_complete = TRUE;
      }
      if (rx_complete || receive_packet_index >= next_receive_count) {
        int n_kev;
#if SKT_XFER_DEBUG
        T_LOG("child %d: disable RX\n", child);
#endif
        EV_SET(kev, port->fd, EVFILT_READ, EV_DELETE, 0, 0, NULL);
        n_kev = 1;
        next_receive_count = receive_packet_index + batch_size;
        if (next_receive_count >= how_many) {
          next_receive_count = how_many;
        }
        if (!tx_complete) {
          /* re-enable TX */
          EV_SET(kev + n_kev, port->fd, EVFILT_WRITE, EV_ADD, 0, 0, NULL);
#if SKT_XFER_DEBUG
          T_LOG("child %d: enable TX\n", child);
#endif
          n_kev++;
          sending = TRUE;
          if (child == 1) {
            payload.packet_number += pkts_dropped;
          }
        } else if (!rx_complete) {
          assert(tx_complete);
          /*
...
static void send_tcp(channel_port_t port, uuid_t flow_id, uint16_t src_port,
                     struct in_addr dst_ip, uint16_t dst_port,
                     uint32_t how_many, uint32_t batch_size, int child,
                     bool connect) {
  int error;
  ...
  while (!tx_complete) {
    ...
    case EVFILT_WRITE: {
      ...
      channel_port_send(port, flow_id, IPPROTO_TCP, src_port, dst_ip, dst_port,
                        &payload, sizeof(payload), next_batch, FALSE, connect,
                        PKT_SC_BE, TRUE, NULL);
      if (payload.packet_number >= how_many) {
        assert(payload.packet_number == how_many);
        T_LOG("TX child %d: completed %u\n", child, how_many);
...
static uint64_t set_error_inject_mask(uint64_t *mask) {
  uint64_t old_mask = 0;
  size_t old_size = sizeof(old_mask);
  int error;

  error = sysctlbyname("kern.skywalk.flowswitch.fsw_inject_error", &old_mask,
                       &old_size, mask, mask ? sizeof(*mask) : 0);
...
static void do_error_receive(int child, channel_port_t port, uuid_t flow_id,
                             uint16_t src_port, struct in_addr dst_ip,
                             uint16_t dst_port, uint32_t how_many) {
  int error;
  struct kevent evlist, kev;
  ...
  for (;;) {
    ...
    if (evlist.filter == EVFILT_READ) {
      channel_port_receive(child, port, src_port, dst_ip, how_many,
                           &receive_packet_count, &receive_packet_index, true,
                           &pkts_dropped);
    } else {
      T_LOG("%lu event %d?\n", evlist.ident, evlist.filter);
      assert(0);
...
static void do_error_send(channel_port_t port, uuid_t flow_id,
                          uint16_t src_port, struct in_addr dst_ip,
                          uint16_t dst_port, uint32_t how_many,
                          uint32_t batch_size) {
  int error;
  ...
  while (!tx_complete) {
    ...
    if (evlist.filter == EVFILT_WRITE) {
      ...
      channel_port_send(port, flow_id, IPPROTO_UDP, src_port, dst_ip, dst_port,
                        &payload, sizeof(payload), next_batch, FALSE, FALSE,
                        PKT_SC_BE, TRUE, NULL);
      if (payload.packet_number >= how_many) {
        assert(payload.packet_number == how_many);
        tx_complete = true;
...
static void get_qset_id_from_llinks(struct sktc_nexus_handles *handles,
                                    uint64_t *qset_id) {
  ...
  err = __os_nexus_get_llink_info(handles->controller, handles->netif_nx_uuid,
                                  nlir, len);
  if (err != 0) {
...
static int setup_flowswitch_and_flow(
    struct sktc_nexus_handles *handles, const char *ifname, int protocol,
    uint16_t flags, struct in_addr our_ip, struct in_addr our_mask,
    uint16_t our_port, pid_t the_pid, struct in_addr peer_ip,
    uint16_t peer_port, uuid_t flow_id, flowadv_idx_t *flowadv_idx,
    int tx_ring_size, int rx_ring_size, int buf_size, int max_frags,
    bool multi_llink) {
  int error;
  uint64_t qset_id = 0;
  ...
  sktc_create_flowswitch_no_address(handles, tx_ring_size, rx_ring_size,
                                    buf_size, max_frags, 0);
  error = os_nexus_controller_bind_provider_instance(
      handles->controller, handles->fsw_nx_uuid, OUR_FLOWSWITCH_PORT, the_pid,
  ...
  error = connect_flow(handles->controller, handles->fsw_nx_uuid,
                       OUR_FLOWSWITCH_PORT, flow_id, protocol, flags,
                       handles->netif_addr, our_port, peer_ip, peer_port,
                       flowadv_idx, qset_id);
  return error;
}

static void setup_flowswitch(struct sktc_nexus_handles *handles,
                             const char *ifname, pid_t the_pid,
                             int tx_ring_size, int rx_ring_size, int buf_size,
                             int max_frags) {
  bzero(handles, sizeof(*handles));
  strlcpy(handles->netif_ifname, ifname, sizeof(handles->netif_ifname));
  sktc_create_flowswitch_no_address(handles, tx_ring_size, rx_ring_size,
...
static int fetch_if_flowswitch_and_setup_flow(
    struct sktc_nexus_handles *handles, const char *ifname, int protocol,
    uint16_t flags, struct in_addr our_ip, struct in_addr our_mask,
    uint16_t our_port, pid_t the_pid, struct in_addr peer_ip,
    uint16_t peer_port, uuid_t flow_id, flowadv_idx_t *flowadv_idx,
    int tx_ring_size, int rx_ring_size, int buf_size, int max_frags,
    bool multi_llink, uuid_t parent_flow_id,
    struct flow_demux_pattern *demux_patterns, uint8_t demux_pattern_count) {
  int error;
  uint64_t qset_id = 0;
  ...
  if (handles->controller == NULL) {
    SKT_LOG("%s: os_nexus_controller_create failed, %s (%d)\n", __func__,
            strerror(errno), errno);
    return ENOMEM;
  }

  if ((sktc_get_netif_nexus(handles->netif_ifname, handles->netif_nx_uuid) &&
       sktc_get_flowswitch_nexus(handles->netif_ifname,
                                 handles->fsw_nx_uuid))) {
    if (child_flow) {
      error = os_nexus_controller_bind_provider_instance(
          handles->controller, handles->fsw_nx_uuid, CHILD_FLOWSWITCH_PORT,
          the_pid, NULL, NULL, 0, NEXUS_BIND_PID);
    } else {
      error = os_nexus_controller_bind_provider_instance(
          handles->controller, handles->fsw_nx_uuid, OUR_FLOWSWITCH_PORT,
          the_pid, NULL, NULL, 0, NEXUS_BIND_PID);
    }
    if (error != 0) {
    ...
    if (child_flow) {
      error = connect_child_flow(
          handles->controller, handles->fsw_nx_uuid, CHILD_FLOWSWITCH_PORT,
          flow_id, protocol, flags, handles->netif_addr, our_port, peer_ip,
          peer_port, flowadv_idx, qset_id, parent_flow_id, demux_patterns,
          demux_pattern_count);
    } else {
      error = connect_flow(handles->controller, handles->fsw_nx_uuid,
                           OUR_FLOWSWITCH_PORT, flow_id, protocol, flags,
    ...
    T_LOG("%s: failed to find existing netif/flowswitch instance\n", __func__);
    return ENOENT;
  }
...
static void set_feth_mac_addr(struct ether_addr *feth_macaddr, uint32_t unit) {
  /*
...
static int skt_xfer_udp_common(int child, uint32_t how_many,
                               uint32_t batch_size, bool do_ping_pong,
                               bool wrong_flow_id, bool test_aqm, bool test_wmm,
                               int tx_ring_size, int rx_ring_size, int buf_size,
                               int max_frags, int event_test_id,
                               bool low_latency, bool multi_llink,
                               bool test_redirect) {
  char buf[1] = {0};
  int error;
  const char *ifname;
  ...
  if (test_redirect && child == 0) {
    setup_flowswitch(&handles, FETH0_NAME, getpid(), tx_ring_size, rx_ring_size,
                     buf_size, max_frags);

    setup_flowswitch(&handles, RD0_NAME, getpid(), tx_ring_size, rx_ring_size,
                     buf_size, max_frags);

    error = fetch_if_flowswitch_and_setup_flow(
        &handles, RD0_NAME, IPPROTO_UDP, 0, our_ip, our_mask, our_port,
        getpid(), peer_ip, peer_port, flow_id, &flowadv_idx, -1, -1, -1, -1,
        false, NULL, NULL, 0);
  } else {
    /* set up the flowswitch over the right interface */
    error = setup_flowswitch_and_flow(
        &handles, ifname, IPPROTO_UDP, nfr_flags, our_ip, our_mask, our_port,
        getpid(), peer_ip, peer_port, flow_id, &flowadv_idx, tx_ring_size,
        rx_ring_size, buf_size, max_frags, multi_llink);
  }

  if (error == 0) {
    sktu_channel_port_init(
        &port, handles.fsw_nx_uuid, OUR_FLOWSWITCH_PORT, ENABLE_UPP,
  ...
  if (do_ping_pong) {
    ping_pong(&port, flow_id, our_port, peer_ip, peer_port, how_many,
              batch_size, child, wrong_flow_id, flowadv_idx, test_aqm, test_wmm,
              MAX_DEMUX_OFFSET + 1);
  } else {
    ...
    send_and_receive(&port, flow_id, our_port, peer_ip, peer_port, how_many,
                     batch_size, child, wrong_flow_id, errors_ok, event_flags,
                     ifadv_enabled);
  }
...
static int get_fsw_stats(struct fsw_stats *result) {
  int i, ret;
  size_t length = 0;
  ...
  while (scan < (buffer + length)) {
    ...
    for (i = 0; i < (sizeof(*result) / sizeof(STATS_VAL(result, 0))); i++) {
...
static int skt_xfer_udp_with_errors_common(int child, uint32_t how_many,
                                           uint32_t batch_size) {
  char buf[1] = {0};
  int error;
  const char *ifname;
  ...
  /* set up the flowswitch over the right interface */
  error = setup_flowswitch_and_flow(
      &handles, ifname, IPPROTO_UDP, 0, our_ip, our_mask, our_port, getpid(),
      peer_ip, peer_port, flow_id, &flowadv_idx, -1, -1, -1, -1, false);
  if (error == 0) {
  ...
  ping_pong(&port, flow_id, our_port, peer_ip, peer_port, 1, 1, child, FALSE,
            flowadv_idx, FALSE, FALSE, MAX_DEMUX_OFFSET + 1);
  ...
  for (errbit = 0; errbit < INJECT_CODE_COUNT; errbit++) {
    ...
    do_error_send(&port, flow_id, our_port, peer_ip, peer_port, how_many,
                  batch_size);
    ...
    for (sidx = ic->ic_stat_idx, st = 0; st < INJECT_CODE_IDX_MAX;
         st++, sidx++) {
      uint64_t counter;
      ...
      if (counter == 0) {
        T_LOG("Counter idx %d didn't "
              ...
              "After %lld\n",
              st, ic->ic_code, STATS_VAL(&stats_before, *sidx),
...
static int skt_xfer_tcpflood(int child, uint32_t how_many, uint32_t batch_size,
                             bool synflood) {
  char buf[1] = {0};
  int error;
  const char *ifname;
  ...
  /* set up the flowswitch over the right interface */
  error = setup_flowswitch_and_flow(
      &handles, ifname, IPPROTO_TCP, 0, our_ip, our_mask, our_port, getpid(),
      peer_ip, peer_port, flow_id, &flowadv_idx, -1, -1, -1, -1, false);
  if (error == 0) {
    sktu_channel_port_init(&port, handles.fsw_nx_uuid, OUR_FLOWSWITCH_PORT,
                           ENABLE_UPP, false, false);
...
#endif
  port.ip_addr = our_ip;
  send_tcp(&port, flow_id, our_port, peer_ip, peer_port, how_many, batch_size,
           child, synflood);
  return 0;
}

static int skt_xfer_portzero(int child, int protocol) {
  char buf[1] = {0};
  int error;
  const char *ifname;
  uuid_t flow_id = {};
  ...
  error = setup_flowswitch_and_flow(
      &handles, ifname, protocol, 0, our_ip, our_mask, our_port, getpid(),
      peer_ip, peer_port, flow_id, &flowadv_idx, -1, -1, -1, -1, false);
  ...
  if (error != EINVAL) {
    T_LOG("expected %d but got %s (%d)\n", EINVAL, strerror(error), error);
    return 1;
  }
  return 0;
}

static int skt_xfer_setuponly(int child) {
  char buf[1] = {0};
  int error;
  const char *ifname;
  uuid_t flow_id = {};
...
static void send_bad_flow(channel_port_t port, uuid_t flow_id, int protocol,
                          uint16_t src_port, struct in_addr dst_ip,
                          uint16_t dst_port, my_payload_t payload) {
  struct fsw_stats stats_before, stats_after;
  uint64_t counter;
  ...
  channel_port_send(port, flow_id, protocol, src_port, dst_ip, dst_port,
                    payload, sizeof(*payload), 1, FALSE, FALSE, PKT_SC_BE,
                    FALSE, NULL);
...
static int skt_xfer_flowmatch(int child) {
  char buf[1] = {0};
  int error;
  const char *ifname;
  uuid_t flow_id = {};
  ...
  /* Start the receiver */
  if (child == 0) {
    channel_port_receive_all(&port, flow_id, our_port, peer_ip, peer_port, 0,
                             FALSE);
    return 0;
  }
  ...
  send_bad_flow(&port, nowhere_flow_id, IPPROTO_UDP, our_port, peer_ip,
                peer_port, &payload);
  ...
  send_bad_flow(&port, flow_id, IPPROTO_TCP, our_port, peer_ip, peer_port,
                &payload);
  ...
  send_bad_flow(&port, flow_id, IPPROTO_UDP, our_port + 1, peer_ip, peer_port,
                &payload);
  ...
  send_bad_flow(&port, flow_id, IPPROTO_UDP, our_port, nowhere_ip, peer_port,
                &payload);
  ...
  send_bad_flow(&port, flow_id, IPPROTO_UDP, our_port, peer_ip, peer_port + 1,
                &payload);

  /* Send something right to single receiver to end */
  payload.packet_number = 0;
  strncpy(payload.data, XFER_RECV_END_PAYLOAD, sizeof(payload.data));
  channel_port_send(&port, flow_id, IPPROTO_UDP, our_port, peer_ip, peer_port,
                    &payload, sizeof(payload), 1, FALSE, FALSE, PKT_SC_BE,
                    FALSE, NULL);
...
static int skt_xfer_flowcleanup(int child, uint32_t how_many,
                                uint32_t batch_size) {
  char buf[1] = {0};
  int error;
  const char *ifname;
  ...
  error = os_nexus_controller_bind_provider_instance(
      handles.controller, handles.fsw_nx_uuid, OUR_FLOWSWITCH_PORT, getpid(),
      NULL, NULL, 0, NEXUS_BIND_PID);
  ...
  error =
      connect_flow(handles.controller, handles.fsw_nx_uuid, OUR_FLOWSWITCH_PORT,
                   flow_id, IPPROTO_UDP, 0, handles.netif_addr, our_port,
                   peer_ip, peer_port, &flowadv_idx, 0);
  SKTC_ASSERT_ERR(!error);
  assert(flowadv_idx != FLOWADV_IDX_NONE);
  ...
  ping_pong(&port, flow_id, our_port, peer_ip, peer_port, how_many, batch_size,
            child, FALSE, flowadv_idx, FALSE, FALSE, MAX_DEMUX_OFFSET + 1);

  return 0;
}

static int skt_xfer_csumoffload(int child, int protocol) {
  char buf[1] = {0};
  int error;
  const char *ifname;
  uuid_t flow_id = {};
  ...
  /* Start the receiver */
  if (child == 0) {
    channel_port_receive_all(&port, flow_id, our_port, peer_ip, peer_port, 2,
                             FALSE);
    return 0;
  }
  ...
  channel_port_send(&port, flow_id, IPPROTO_UDP, our_port, peer_ip, peer_port,
                    &payload, sizeof(payload), 1, FALSE, FALSE, PKT_SC_BE, TRUE,
                    NULL);
...
static void skt_xfer_enable_qos_marking_interface(const char *ifname,
                                                  uint32_t mode) {
...
static int skt_xfer_qosmarking(int child, uint32_t mode) {
  char buf[1] = {0};
  int error;
  const char *ifname;
  uuid_t flow_id = {};
  ...
  if (child == 0) {
    channel_port_receive_all(&port, flow_id, our_port, peer_ip, peer_port, -1,
                             TRUE);
    return 0;
  }
...
#define __SEND_SC(svc, csum_offload)                                           \
  ...
  if (mode == IFRTYPE_QOSMARKING_FASTLANE) {                                   \
    strlcpy(payload.data, XFER_QOSMARKING_FASTLANE_PREFIX,                     \
            sizeof(payload.data));                                             \
  } else if (mode == IFRTYPE_QOSMARKING_RFC4594) {                             \
    strlcpy(payload.data, XFER_QOSMARKING_RFC4594_PREFIX,                      \
            sizeof(payload.data));                                             \
  }                                                                            \
  strlcat(payload.data, #svc, sizeof(payload.data));                           \
  channel_port_send(&port, flow_id, IPPROTO_UDP, our_port, peer_ip, peer_port, \
                    &payload, sizeof(payload), 1, FALSE, FALSE, svc,           \
                    csum_offload, NULL);
...
static int skt_xfer_listener_tcp_rst(int child) {
  char buf[1] = {0};
  int error;
  const char *ifname;
  uuid_t flow_id = {};
  ...
  if (child == 0) {
    ...
    error = connect_flow(handles.controller, handles.fsw_nx_uuid,
                         OUR_FLOWSWITCH_PORT, connecting_flow_id, IPPROTO_TCP,
                         0, our_ip, our_port, peer_ip, listener_port,
                         &tmp_flowadv_idx, 0);
    SKTC_ASSERT_ERR(!error);
    ...
    flowadv_idx_t tmp_flowadv_idx = FLOWADV_IDX_NONE;
    error =
        connect_flow(handles.controller, handles.fsw_nx_uuid,
  ...
  /* Start the receiver */
  if (child == 0) {
    channel_port_receive_all(&port, flow_id, listener_port, peer_ip, peer_port,
                             1, FALSE);
    return 0;
  }
  ...
  channel_port_send(&port, listener_flow_id, IPPROTO_TCP, listener_port,
                    peer_ip, peer_port, &payload, sizeof(payload), 1, FALSE,
                    FALSE, PKT_SC_BE, TRUE, NULL);
  ...
  strncpy(payload.data, XFER_RECV_END_PAYLOAD, sizeof(payload.data));
  channel_port_send(&port, flow_id, IPPROTO_UDP, our_port, peer_ip, peer_port,
                    &payload, sizeof(payload), 2, FALSE, FALSE, PKT_SC_BE,
                    FALSE, NULL);

  return 0;
}

int skt_xfer_udp_frags(int child, bool error_ids) {
  char buf[1] = {0};
  int error;
  const char *ifname;
  uuid_t flow_id = {};
  ...
  /* Start the receiver */
  if (child == 0) {
    channel_port_receive_all(&port, flow_id, our_port, peer_ip, peer_port,
                             error_ids ? 0 : 4, FALSE);
    return 0;
  }
  ...
  channel_port_send_fragments(
      &port, flow_id, IPPROTO_UDP, our_port, peer_ip, peer_port, &payload,
      (sizeof(payload.data) & ~0x7), 4, PKT_SC_BE, FALSE, error_ids);
...
static int skt_xfer_udp_parent_child(int id, uint16_t demux_offset) {
#define CHILD_ID 0
#define REMOTE_ID 1
#define PARENT_ID 2
...
#define CHILD_FLOW_UUID "1B4E28BA-2FA1-11D2-883F-B9A761BDE3FD"
  ...
  if (id == PARENT_ID || id == REMOTE_ID) {
    ...
    error = setup_flowswitch_and_flow(&handles, ifname, IPPROTO_UDP, flags,
                                      our_ip, our_mask, our_port, getpid(),
                                      peer_ip, peer_port, flow_id, &flowadv_idx,
                                      -1, -1, -1, -1, false);
  } else if (id == CHILD_ID) {
    ...
    error = fetch_if_flowswitch_and_setup_flow(
        &handles, ifname, IPPROTO_UDP, 0, our_ip, our_mask, our_port, getpid(),
        peer_ip, peer_port, flow_id, &flowadv_idx, -1, -1, -1, -1, false,
        parent_flow_id, demux_patterns, 1);
  }
  if (error == 0) {
    sktu_channel_port_init(&port, handles.fsw_nx_uuid, nx_port, ENABLE_UPP,
                           false, false);
    assert(port.chan != NULL);
  }
  ...
  if (id == CHILD_ID || id == REMOTE_ID) {
    T_LOG("child %d: Warm up ARP cache\n", id);
    ping_pong(&port, flow_id, our_port, peer_ip, peer_port, 1, 1, id, FALSE,
              flowadv_idx, FALSE, FALSE, demux_offset);

    T_LOG("child %d: Test Start\n", id);
    ping_pong(&port, flow_id, our_port, peer_ip, peer_port, 5, 5, id, FALSE,
              flowadv_idx, FALSE, FALSE, demux_offset);
...
static int skt_xfer_rx_flow_steering_drop_packets(int child, bool drop_tx) {
  char buf[1] = {0};
  int error;
  const char *ifname;
  uuid_t flow_id = {};
  ...
  ping_pong(&port, flow_id, our_port, peer_ip, peer_port, 1, 1, child, TRUE,
            flowadv_idx, FALSE, FALSE, MAX_DEMUX_OFFSET + 1);
...
static int skt_xfer_udp_main(int argc, char *argv[]) {
  int child;

  assert(!strcmp(argv[3], "--child"));
  child = atoi(argv[4]);

  return skt_xfer_udp_common(
      child, XFER_TXRX_PACKET_COUNT, XFER_TXRX_BATCH_COUNT, FALSE, FALSE, FALSE,
      FALSE, -1, -1, -1, -1, SKT_FSW_EVENT_TEST_NONE, false, false, false);
}

static int skt_xfer_udp_long_main(int argc, char *argv[]) {
  int child;
  ...
  return skt_xfer_udp_common(child, XFER_TXRX_PACKET_COUNT_LONG,
                             ...
                             -1, -1, -1, -1, SKT_FSW_EVENT_TEST_NONE, false,
                             false, false);
}

static int skt_xfer_udp_overwhelm_main(int argc, char *argv[]) {
  int child;
  ...
  return skt_xfer_udp_common(child, XFER_TXRX_PACKET_COUNT,
                             XFER_TXRX_OVERWHELM_BATCH_COUNT, FALSE, FALSE,
                             FALSE, FALSE, XFER_TXRX_OVERWHELM_FSW_TX_RING_SIZE,
                             XFER_TXRX_OVERWHELM_FSW_RX_RING_SIZE, -1, -1,
                             SKT_FSW_EVENT_TEST_NONE, false, false, false);
}

static int skt_xfer_udp_overwhelm_long_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_udp_ping_pong_main(int argc, char *argv[]) {
  int child, test_id;
  bool low_latency;
  bool multi_llink;
  ...
  return skt_xfer_udp_common(child, XFER_PING_PACKET_COUNT,
                             XFER_PING_BATCH_COUNT, TRUE, FALSE, FALSE, FALSE,
                             -1, -1, -1, -1, SKT_FSW_EVENT_TEST_NONE,
                             low_latency, multi_llink, false);
}

static int skt_xfer_rd_udp_ping_pong_main(int argc, char *argv[]) {
  int child, test_id;
  ...
  return skt_xfer_udp_common(
      child, XFER_PING_PACKET_COUNT, XFER_PING_BATCH_COUNT, TRUE, FALSE, FALSE,
      FALSE, -1, -1, -1, -1, SKT_FSW_EVENT_TEST_NONE, false, false, true);
}

static int skt_xfer_udp_ping_pong_one_main(int argc, char *argv[]) {
  int child;

  assert(!strcmp(argv[3], "--child"));
  child = atoi(argv[4]);

  return skt_xfer_udp_common(child, 1, 1, TRUE, FALSE, FALSE, FALSE, -1, -1, -1,
                             -1, SKT_FSW_EVENT_TEST_NONE, false, false, false);
}

static int skt_xfer_udp_ping_pong_long_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_udp_ping_pong_one_wrong_main(int argc, char *argv[]) {
  int child;

  assert(!strcmp(argv[3], "--child"));
  child = atoi(argv[4]);

  return skt_xfer_udp_common(child, 1, 1, TRUE, TRUE, FALSE, FALSE, -1, -1, -1,
                             -1, SKT_FSW_EVENT_TEST_NONE, false, false, false);
}

static int skt_xfer_tcp_syn_flood_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_tcp_rst_flood_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_udp_ping_pong_aqm_main(int argc, char *argv[]) {
  int child;
  ...
  return skt_xfer_udp_common(child, XFER_AQM_PING_PACKET_COUNT,
                             XFER_AQM_PING_BATCH_COUNT, TRUE, FALSE, TRUE,
                             FALSE, -1, -1, -1, -1, SKT_FSW_EVENT_TEST_NONE,
                             false, false, false);
}

static int skt_xfer_udp_with_errors_main(int argc, char *argv[]) {
  int child;

  assert(!strcmp(argv[3], "--child"));
  child = atoi(argv[4]);

  return skt_xfer_udp_with_errors_common(child, XFER_TXRX_PACKET_COUNT,
                                         XFER_TXRX_BATCH_COUNT);
}

static int skt_xfer_tcp_port_zero_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_udp_port_zero_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_setuponly_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_udp_ping_pong_wmm_main(int argc, char *argv[]) {
  int child;
  ...
  return skt_xfer_udp_common(child, XFER_WMM_PING_PACKET_COUNT,
                             XFER_WMM_PING_BATCH_COUNT, TRUE, FALSE, FALSE,
                             TRUE, -1, -1, -1, -1, SKT_FSW_EVENT_TEST_NONE,
                             false, false, false);
}

int skt_xfer_flowmatch_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_flowcleanup_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_udp_ping_pong_multi_buflet_main(int argc, char *argv[]) {
  int child;
  ...
  return skt_xfer_udp_common(child, XFER_PING_PACKET_COUNT,
                             XFER_PING_BATCH_COUNT, TRUE, FALSE, FALSE, FALSE,
                             -1, -1, XFER_TXRX_MULTI_BUFLET_BUF_SIZE,
                             XFER_TXRX_MULTI_BUFLET_MAX_FRAGS,
                             SKT_FSW_EVENT_TEST_NONE, false, false, false);
}

static int skt_xfer_csumoffload_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_fastlane_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_rfc4594_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_listener_tcp_rst_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_udp_frags_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_udp_bad_frags_main(int argc, char *argv[]) {
  int child;
...
static int skt_xfer_udp_ifadv_main(int argc, char *argv[]) {
  int child, test_id;
...
static int skt_xfer_parent_child_flow_main(int argc, char *argv[]) {
  int child, test_id;
...
static int skt_xfer_parent_child_flow_main_offset_400(int argc, char *argv[]) {
  int child, test_id;
...
static int skt_xfer_rx_flow_steering_drop_tx_packets_main(int argc,
                                                          char *argv[]) {
  int child;
...
static int skt_xfer_rx_flow_steering_drop_rx_packets_main(int argc,
                                                          char *argv[]) {
  int child;
...
static void skt_xfer_init_txstart(void) {
  sktc_set_classq_update_intervals(XFER_CLASSQ_UPDATE_INTERVAL * 1000 * 1000);
...
static void skt_xfer_init_native(void) {
  sktc_set_classq_update_intervals(XFER_CLASSQ_UPDATE_INTERVAL * 1000 * 1000);
...
static void skt_xfer_rd_init(void) {
  int err;
  ...
  sktc_set_classq_update_intervals(XFER_CLASSQ_UPDATE_INTERVAL * 1000 * 1000);
  err =
      sysctlbyname("kern.skywalk.disable_nxctl_check", &skt_disable_nxctl_check,
                   &len, &disable_nxctl_check, sizeof(disable_nxctl_check));
  assert(err == 0);
  sktc_ifnet_feth_pair_create(FETH_FLAGS_NATIVE);
...
static void skt_xfer_init_enable_qos_marking_policy(void) {
  int zero = 0;
  size_t restricted_old_size = sizeof(restricted_old);
  assert(sysctlbyname("net.qos.policy.restricted", &restricted_old,
                      &restricted_old_size, &zero, sizeof(zero)) == 0);
}

static void skt_xfer_init_txstart_fastlane(void) {
  skt_xfer_init_txstart();
  skt_xfer_init_enable_qos_marking_policy();
}

static void skt_xfer_init_txstart_fcs(void) {
  sktc_set_classq_update_intervals(XFER_CLASSQ_UPDATE_INTERVAL * 1000 * 1000);
  sktc_ifnet_feth_pair_create(FETH_FLAGS_TXSTART | FETH_FLAGS_FCS);
  sktc_reset_classq_update_intervals();
  sktc_enable_ip_reass();
  sktc_config_fsw_rx_agg_tcp(0);
}

static void skt_xfer_init_txstart_trailer(void) {
  sktc_set_classq_update_intervals(XFER_CLASSQ_UPDATE_INTERVAL * 1000 * 1000);
  sktc_ifnet_feth_pair_create(FETH_FLAGS_TXSTART | FETH_FLAGS_TRAILER);
  sktc_reset_classq_update_intervals();
  sktc_enable_ip_reass();
  sktc_config_fsw_rx_agg_tcp(0);
}

static void skt_xfer_init_native_fastlane(void) {
  skt_xfer_init_native();
  skt_xfer_init_enable_qos_marking_policy();
}

static void skt_xfer_init_native_split_pools(void) {
  sktc_set_classq_update_intervals(XFER_CLASSQ_UPDATE_INTERVAL * 1000 * 1000);
  sktc_ifnet_feth_pair_create(FETH_FLAGS_NATIVE |
...
static void skt_xfer_init_native_fcs(void) {
  sktc_set_classq_update_intervals(XFER_CLASSQ_UPDATE_INTERVAL * 1000 * 1000);
  sktc_ifnet_feth_pair_create(FETH_FLAGS_NATIVE | FETH_FLAGS_FCS);
  sktc_reset_classq_update_intervals();
  sktc_enable_ip_reass();
  sktc_config_fsw_rx_agg_tcp(0);
}

static void skt_xfer_init_native_trailer(void) {
  sktc_set_classq_update_intervals(XFER_CLASSQ_UPDATE_INTERVAL * 1000 * 1000);
  sktc_ifnet_feth_pair_create(FETH_FLAGS_NATIVE | FETH_FLAGS_TRAILER);
  sktc_reset_classq_update_intervals();
  sktc_enable_ip_reass();
  sktc_config_fsw_rx_agg_tcp(0);
}

static void skt_xfer_init_llink(void) {
  sktc_set_classq_update_intervals(XFER_CLASSQ_UPDATE_INTERVAL * 1000 * 1000);
...
static void skt_xfer_init_llink_wmm(void) {
  sktc_set_classq_update_intervals(XFER_CLASSQ_UPDATE_INTERVAL * 1000 * 1000);
  sktc_ifnet_feth_pair_create(FETH_FLAGS_NATIVE | FETH_FLAGS_LLINK |
                              FETH_FLAGS_WMM);
  sktc_reset_classq_update_intervals();
...
static void skt_xfer_init_llink_multi(void) {
  int err;
  uint32_t disable_nxctl_check = 1;
  size_t len = sizeof(skt_disable_nxctl_check);
...
static void skt_xfer_fini(void) {
#if SKT_XFER_DEBUG
  T_LOG("Waiting");
  fflush(stdout);
  for (int i = 0; i < 5; i++) {
...
static void skt_xfer_rd_fini(void) {
#if SKT_XFER_DEBUG
  T_LOG("Waiting");
  fflush(stdout);
  for (int i = 0; i < 5; i++) {
  ...
  err = sysctlbyname("kern.skywalk.disable_nxctl_check", NULL, NULL,
                     &skt_disable_nxctl_check, sizeof(skt_disable_nxctl_check));
  assert(err == 0);
  sktc_ifnet_feth0_set_dequeue_stall(FALSE);
  sktc_ifnet_feth1_set_dequeue_stall(FALSE);
  sktc_ifnet_feth_pair_destroy();
...
static void skt_xfer_fini_fastlane(void) {
  /* restore sysctl */
  assert(sysctlbyname("net.qos.policy.restricted", NULL, NULL, &restricted_old,
                      sizeof(restricted_old)) == 0);

  skt_xfer_fini();
}

static void skt_xfer_fini_llink_multi(void) {
  int err;
...
static void skt_xfer_errors_init(void) {
...
static void skt_xfer_errors_compat_init(void) {
  uint64_t emask = (1ull << 63);
  uint32_t rmask = 0x7ff;
...
static void skt_xfer_errors_fini(void) {
  uint64_t emask = 0;
...
static void skt_xfer_multi_buflet_fini() {
  sktc_restore_channel_buflet_alloc();
  skt_xfer_fini();
}

static void skt_xfer_init_native_wmm(void) {
  sktc_ifnet_feth_pair_create(FETH_FLAGS_NATIVE | FETH_FLAGS_WMM);
  sktc_enable_ip_reass();
  sktc_config_fsw_rx_agg_tcp(0);
}

static void skt_xfer_init_native_multi_buflet(void) {
...
static void skt_xfer_init_native_multi_buflet_copy(void) {
  sktc_ifnet_feth_pair_create(FETH_FLAGS_NATIVE | FETH_FLAGS_MULTI_BUFLET |
                              FETH_FLAGS_NONSHAREDPOOL |
                              FETH_FLAGS_TX_HEADROOM);
  sktc_enable_ip_reass();
  sktc_enable_channel_buflet_alloc();
  sktc_config_fsw_rx_agg_tcp(0);
...
static void skt_xfer_init_ifadv(void) {
  int intvl = XFER_IFADV_INTERVAL; /* in milliseconds */

  assert(sysctlbyname("net.link.fake.if_adv_intvl", NULL, 0, &intvl,
                      sizeof(intvl)) == 0);
  skt_xfer_init_native();
}

static void skt_xfer_fini_ifadv(void) {
  int intvl = 0; /* disable */
...
static void skt_xfer_init_chan_event(void) {
  int drops = XFER_TX_PKT_DROP_RATE;
  assert(sysctlbyname("net.link.fake.tx_drops", NULL, 0, &drops,
                      sizeof(drops)) == 0);

  skt_xfer_init_native();
}

static void skt_xfer_fini_chan_event(void) {
  skt_xfer_fini();
  int drops = 0;
  assert(sysctlbyname("net.link.fake.tx_drops", NULL, 0, &drops,
                      sizeof(drops)) == 0);
...
static void skt_xfer_init_chan_event_async(void) {
  int tx_compl_mode = 1; /* async mode */
  assert(sysctlbyname("net.link.fake.tx_completion_mode", NULL, 0,
                      &tx_compl_mode, sizeof(tx_compl_mode)) == 0);
  skt_xfer_init_chan_event();
}

static void skt_xfer_fini_chan_event_async(void) {
...
static void skt_xfer_init_parent_child_flow(void) {
  int err;
  uint32_t disable_nxctl_check = 1;
  size_t len = sizeof(skt_disable_nxctl_check);
...
static void skt_xfer_init_parent_child_flow_native(void) {
  int err;
  uint32_t disable_nxctl_check = 1;
  size_t len = sizeof(skt_disable_nxctl_check);
...
static void skt_xfer_fini_parent_child_flow(void) {
#if SKT_XFER_DEBUG
  T_LOG("Waiting");
  fflush(stdout);
  for (int i = 0; i < 5; i++) {
...
static void skt_xfer_init_rx_flow_steering(void) {
  int rx_flow_steering = 1;

  assert(sysctlbyname("net.link.fake.rx_flow_steering_support", NULL, 0,
                      &rx_flow_steering, sizeof(rx_flow_steering)) == 0);
  skt_xfer_init_native();
}

static void skt_xfer_fini_rx_flow_steering(void) {
...
struct skywalk_mptest skt_xferudpnsp = {
    ...
    skt_xfer_udp_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_native_split_pools,
    skt_xfer_fini,
    {},
};
...
struct skywalk_mptest skt_xferudpoverwhelmlong = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_udp_overwhelm_long_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_txstart,
    skt_xfer_fini,
    {},
...
struct skywalk_mptest skt_xferudpping = {
    "xferudpping",
    "UDP ping-pong over fake ethernet pair",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_udp_ping_pong_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_FSW_PING_PONG_TEST_DEFAULT)},
    skt_xfer_init_txstart,
    skt_xfer_fini,
    {},
};
...
struct skywalk_mptest skt_xferudppingn = {
    "xferudppingn",
    "UDP ping-pong over native fake ethernet pair",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_udp_ping_pong_main,
...
struct skywalk_mptest skt_xferudpping1 = {
    "xferudpping1",
    "UDP ping-pong once over fake ethernet pair",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_udp_ping_pong_one_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_txstart,
    skt_xfer_fini,
    {},
...
struct skywalk_mptest skt_xferudpping1n = {
    "xferudpping1n",
    "UDP ping-pong once over native fake ethernet pair",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_udp_ping_pong_one_main,
...
struct skywalk_mptest skt_xferudppinglong = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_udp_ping_pong_long_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_txstart,
    skt_xfer_fini,
    {},
...
struct skywalk_mptest skt_xferudpping1wrong = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_udp_ping_pong_one_wrong_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_txstart,
    skt_xfer_fini,
    {},
...
struct skywalk_mptest skt_xferrdudpping = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_rd_udp_ping_pong_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_FSW_PING_PONG_TEST_DEFAULT)},
    skt_xfer_rd_init,
    skt_xfer_rd_fini,
    {},
...
struct skywalk_mptest skt_xferudpping_aqm = {
    "xferudppingaqm",
    "UDP ping-pong over fake ethernet pair with AQM",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_udp_ping_pong_aqm_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_txstart,
    skt_xfer_fini,
    {},
...
struct skywalk_mptest skt_xferudppingn_aqm = {
    "xferudppingnaqm",
    "UDP ping-pong over native fake ethernet pair with"
    " AQM",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
...
struct skywalk_mptest skt_xferudpwitherrors = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS | SK_FEATURE_DEV_OR_DEBUG,
    2,
    skt_xfer_udp_with_errors_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_errors_init,
    skt_xfer_errors_fini,
    {},
};
...
struct skywalk_mptest skt_xfersetuponly = {
    "xfersetuponly",
    "setup fake ethernet pair only",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_setuponly_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_txstart,
    skt_xfer_fini,
    {},
...
struct skywalk_mptest skt_xfersetuponlyn = {
    "xfersetuponlyn",
    "setup native fake ethernet pair only",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_setuponly_main,
...
struct skywalk_mptest skt_xferudppingn_wmm = {
    "xferudppingnwmm",
    "UDP ping-pong over native fake ethernet pair in wmm"
    " mode",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_udp_ping_pong_wmm_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_native_wmm,
    skt_xfer_fini,
    {},
};
...
struct skywalk_mptest skt_xferudppingn_mb = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS | SK_FEATURE_DEV_OR_DEBUG,
    2,
    skt_xfer_udp_ping_pong_multi_buflet_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_native_multi_buflet,
...
struct skywalk_mptest skt_xferudppingn_mbc = {
    ...
    skt_xfer_udp_ping_pong_multi_buflet_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_native_multi_buflet_copy,
    skt_xfer_multi_buflet_fini,
    {},
};
...
struct skywalk_mptest skt_xferfastlane = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_fastlane_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_txstart_fastlane,
    skt_xfer_fini_fastlane,
    {},
};
...
struct skywalk_mptest skt_xferlistenertcprst = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_listener_tcp_rst_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_native,
    skt_xfer_fini,
    {},
...
struct skywalk_mptest skt_xferudpifadvenable = {
    ...
    2,
    skt_xfer_udp_ifadv_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_FSW_EVENT_TEST_IF_ADV_ENABLED)},
    skt_xfer_init_ifadv,
    skt_xfer_fini_ifadv,
    {},
};
...
struct skywalk_mptest skt_xferudpifadvdisable = {
    ...
    2,
    skt_xfer_udp_ifadv_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_FSW_EVENT_TEST_IF_ADV_DISABLED)},
    skt_xfer_init_ifadv,
    skt_xfer_fini_ifadv,
    {},
};
...
struct skywalk_mptest skt_xferudppingnll = {
    ...
    2,
    skt_xfer_udp_ping_pong_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_FSW_PING_PONG_TEST_LOW_LATENCY)},
    skt_xfer_init_native,
    skt_xfer_fini,
    {},
};
...
struct skywalk_mptest skt_xferudppingllink = {
    "xferudppingllink",
    "UDP ping-pong over fake ethernet pair in llink mode",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_udp_ping_pong_main,
...
struct skywalk_mptest skt_xferudppingllink_wmm = {
    "xferudppingllinkwmm",
    "UDP ping-pong over fake ethernet pair in llink & wmm mode",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_udp_ping_pong_wmm_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_llink_wmm,
    skt_xfer_fini,
    {},
};

struct skywalk_mptest skt_xferudppingllink_multi = {
    "xferudppingllinkmulti",
    "UDP ping-pong over fake ethernet pair in multi llink mode",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH |
        SK_FEATURE_NETNS,
    2,
    skt_xfer_udp_ping_pong_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_FSW_PING_PONG_TEST_MULTI_LLINK)},
...
struct skywalk_mptest skt_xferudpchanevents = {
    ...
    2,
    skt_xfer_udp_ifadv_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_FSW_EVENT_TEST_CHANNEL_EVENTS)},
    skt_xfer_init_chan_event,
    skt_xfer_fini_chan_event,
    {},
};
...
struct skywalk_mptest skt_xferudpchaneventsasync = {
    ...
    skt_xfer_udp_ifadv_main,
    {NULL, NULL, NULL, NULL, NULL, STR(SKT_FSW_EVENT_TEST_CHANNEL_EVENTS)},
    skt_xfer_init_chan_event_async,
    skt_xfer_fini_chan_event_async,
    {},
};
...
struct skywalk_mptest skt_xferparentchildflow = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_DEV_OR_DEBUG |
        SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS,
    3,
    skt_xfer_parent_child_flow_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_parent_child_flow,
    skt_xfer_fini_parent_child_flow,
    {},
};
...
struct skywalk_mptest skt_xferparentchildflow_offset_400 = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_DEV_OR_DEBUG |
        SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS,
    3,
    skt_xfer_parent_child_flow_main_offset_400,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_parent_child_flow,
    skt_xfer_fini_parent_child_flow,
    {},
...
struct skywalk_mptest skt_xferparentchildflown = {
    ...
    skt_xfer_parent_child_flow_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_parent_child_flow_native,
    skt_xfer_fini_parent_child_flow,
    {},
};
...
struct skywalk_mptest skt_xferparentchildflown_offset_400 = {
    ...
    "flowswitch parent child flows on native fake ethernet interface test with "
    "demux offset 400",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_DEV_OR_DEBUG |
        SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS,
    3,
...
struct skywalk_mptest skt_xferrxflowsteeringdroptxpackets = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_DEV_OR_DEBUG |
        SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS,
    2,
    skt_xfer_rx_flow_steering_drop_tx_packets_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_rx_flow_steering,
    skt_xfer_fini_rx_flow_steering,
    {},
};
...
struct skywalk_mptest skt_xferrxflowsteeringdroprxpackets = {
    ...
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_DEV_OR_DEBUG |
        SK_FEATURE_NEXUS_FLOWSWITCH | SK_FEATURE_NETNS,
    2,
    skt_xfer_rx_flow_steering_drop_rx_packets_main,
    {NULL, NULL, NULL, NULL, NULL, NULL},
    skt_xfer_init_rx_flow_steering,
    skt_xfer_fini_rx_flow_steering,
    {},
