...
static void skt_fswloop_common(const char *name) {
  uuid_t attach;
  int error;
  ...
  error = __os_nexus_ifattach(sktc_nexus_controller, sktc_instance_uuid, NULL,
                              sktc_instance_uuid, false, &attach);
  SKTC_ASSERT_ERR(error == -1);
  SKTC_ASSERT_ERR(errno == EINVAL);
}

static int skt_fswloopfsw_main(int argc, char *argv[]) {
...
struct skywalk_test skt_fswloopfsw = {
    "fswloopfsw",
    "create a flow-switch and attach it to itself",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_FLOWSWITCH,
    skt_fswloopfsw_main,
};

void skt_fswloop2_common(boolean_t add_netif) {
  int error;
  ...
  strncpy((char *)attr.name, "skt_fswloop2_zero", sizeof(nexus_name_t) - 1);
  attr.type = NEXUS_TYPE_FLOW_SWITCH;
  attr.anonymous = 1;
  sktc_build_nexus(ncd, &attr, &provider0, &instance0);

  strncpy((char *)attr.name, "skt_fswloop2_one", sizeof(nexus_name_t) - 1);
  sktc_build_nexus(ncd, &attr, &provider1, &instance1);

  if (add_netif) {
    strncpy((char *)attr.name, "skt_netif_feth", sizeof(nexus_name_t) - 1);
    attr.type = NEXUS_TYPE_NET_IF;
    sktc_build_nexus(ncd, &attr, &netif_provider, &netif0_instance);

    error = __os_nexus_ifattach(ncd, netif0_instance, FETH0_NAME, NULL, false,
                                &feth0_attach);
    SKTC_ASSERT_ERR(!error);

    error = os_nexus_controller_alloc_provider_instance(ncd, netif_provider,
                                                        &netif1_instance);
    SKTC_ASSERT_ERR(!error);

    error = __os_nexus_ifattach(ncd, netif1_instance, FETH1_NAME, NULL, false,
                                &feth1_attach);
    SKTC_ASSERT_ERR(!error);
  }
...
int skt_fswloop2ff_main(int argc, char *argv[]) {
...
struct skywalk_test skt_fswloop2ff = {
    "fswloop2mm",
    "attach a flowswitch to a flowswitch without any netif",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_FLOWSWITCH,
    skt_fswloop2ff_main,
};

int skt_fswloop2nff_main(int argc, char *argv[]) {
...
struct skywalk_test skt_fswloop2nmm = {
    "fswloop2nmm",
    "attach a flowswitch to a flowswitch and back to itself",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH,
    skt_fswloop2nff_main,
    {NULL},
