...
/* <rdar://problem/24849324> os_channel_{rx,tx}_ring() needs to check bounds of
 * the ring index */
...
static void skt_ringid_init(void) {
...
static void skt_ringid_fini(void) { sktc_cleanup_nexus(); }

/****************************************************************/

static int skt_ringid_main_common(
    int argc, char *argv[], uint32_t num, ring_id_type_t first,
    ring_id_type_t last,
  ...
  for (ringid = fringid; ringid <= lringid; ringid++) {
    ...
    channel =
        sktu_channel_create_extended(channel_uuid, 0, CHANNEL_DIR_TX_RX, ringid,
    ...
    for (ringid2 = fringid; ringid2 <= lringid + 1; ringid2++) {
      ...
      assert(ringid2 == ringid ||
             !ring); // This verifies rdar://problem/24849324
    }
...
static int skt_ringidtx_main(int argc, char *argv[]) {
  return skt_ringid_main_common(argc, argv, NUM_TX_RINGS, CHANNEL_FIRST_TX_RING,
                                CHANNEL_LAST_TX_RING, &os_channel_tx_ring);
}

static int skt_ringidrx_main(int argc, char *argv[]) {
  return skt_ringid_main_common(argc, argv, NUM_RX_RINGS, CHANNEL_FIRST_RX_RING,
                                CHANNEL_LAST_RX_RING, &os_channel_rx_ring);
}

struct skywalk_test skt_ringidtx = {
    "ringidtx",
    "tests opening tx ringids",
