...
static void skt_bind_init(const char *name, nexus_type_t type, int anon,
                          int add_netif) {
  int error;
  ...
  if (add_netif) {
    strncpy((char *)attr.name, "skt_netif_feth0", sizeof(nexus_name_t) - 1);
    attr.type = NEXUS_TYPE_NET_IF;
    attr.ntxrings = -1;
    attr.nrxrings = -1;

    sktc_build_nexus(sktc_nexus_controller, &attr, &netif_provider,
                     &netif_instance);

    error = __os_nexus_ifattach(sktc_nexus_controller, netif_instance,
                                FETH0_NAME, NULL, false, &netif_attach);
    SKTC_ASSERT_ERR(error == 0);

    error = __os_nexus_ifattach(sktc_nexus_controller, sktc_instance_uuid, NULL,
                                netif_instance, 0, &netif_attach);
    SKTC_ASSERT_ERR(error == 0);
  }
...
static void skt_bind_fini(void) {
  if (ch00 != NULL) {
  ...
  if (!uuid_is_null(netif_attach)) {
    ...
    error = __os_nexus_ifdetach(sktc_nexus_controller, sktc_instance_uuid,
                                netif_attach);
    SKTC_ASSERT_ERR(error == 0);
...
static void skt_bind_common(nexus_type_t type, nexus_port_t port0,
                            nexus_port_t port1) {
  ring_id_t ringid;
  uuid_t flow_id;
  int error, upp;
  ...
  if (type == NEXUS_TYPE_FLOW_SWITCH) {
    ...
    error = sktc_bind_tcp4_flow(sktc_nexus_controller, sktc_instance_uuid, 0,
                                port0, flow_id);
    SKTC_ASSERT_ERR(error == 0);
    upp = 1;
  } else {
  ...
  ch00 = sktu_channel_create_extended(sktc_instance_uuid, port0,
                                      CHANNEL_DIR_TX_RX, CHANNEL_RING_ID_ANY,
                                      NULL, -1, -1, -1, -1, -1, upp, 1, -1, -1);
  assert(ch00 == NULL);
  SKTC_ASSERT_ERR(errno == EACCES);

  /* this must work (key attributes match) */
  ch00 = sktu_channel_create_extended(
      sktc_instance_uuid, port0, CHANNEL_DIR_TX_RX, CHANNEL_RING_ID_ANY, attr0,
      -1, -1, -1, -1, -1, upp, 1, -1, -1);
  assert(ch00 != NULL);
  ...
  /* this must fail since the key attribute is missing */
  ch10 = sktu_channel_create_extended(sktc_instance_uuid, port1,
                                      CHANNEL_DIR_TX_RX, CHANNEL_RING_ID_ANY,
                                      NULL, -1, -1, -1, -1, -1, upp, 1, -1, -1);
  assert(ch10 == NULL);
  SKTC_ASSERT_ERR(errno == EACCES);
  ...
  /* this must work (key attributes match) */
  ch10 = sktu_channel_create_extended(
      sktc_instance_uuid, port1, CHANNEL_DIR_TX_RX, CHANNEL_RING_ID_ANY, attr1,
      -1, -1, -1, -1, -1, upp, 1, -1, -1);
  assert(ch10 != NULL);
  ...
  ch00 = sktu_channel_create_extended(
      sktc_instance_uuid, port0, CHANNEL_DIR_TX_RX, CHANNEL_RING_ID_ANY, attr1,
      -1, -1, -1, -1, -1, upp, 1, -1, -1);
  assert(ch00 == NULL);
  SKTC_ASSERT_ERR(errno == EACCES);
   ...
   */
  ch00 = sktu_channel_create_extended(sktc_instance_uuid, port0,
                                      CHANNEL_DIR_TX_RX, ringid, NULL, -1, -1,
                                      -1, -1, -1, upp, 1, -1, -1);
  assert(ch00 == NULL);
  SKTC_ASSERT_ERR(errno == EACCES);

  assert(ch01 == NULL);
  ch01 = sktu_channel_create_extended(sktc_instance_uuid, port0,
                                      CHANNEL_DIR_TX_RX, (ringid + 1), NULL, -1,
                                      -1, -1, -1, -1, upp, 1, -1, -1);
  assert(ch01 == NULL);
  SKTC_ASSERT_ERR(errno == EACCES);
  ...
  ch00 = sktu_channel_create_extended(sktc_instance_uuid, port0,
                                      CHANNEL_DIR_TX_RX, ringid, attr0, -1, -1,
                                      -1, -1, -1, upp, 1, -1, -1);
  assert(ch00 != NULL);

  ch01 = sktu_channel_create_extended(sktc_instance_uuid, port0,
                                      CHANNEL_DIR_TX_RX, (ringid + 1), attr0,
                                      -1, -1, -1, -1, -1, upp, 1, -1, -1);
  assert(ch01 != NULL);
  ...
  /* this must fail (key attributes swapped) */
  ch10 = sktu_channel_create_extended(
      sktc_instance_uuid, port1, CHANNEL_DIR_TX_RX, CHANNEL_RING_ID_ANY, attr0,
      -1, -1, -1, -1, -1, upp, 1, -1, -1);
  assert(ch10 == NULL);
  SKTC_ASSERT_ERR(errno == EACCES);
  ...
  assert(ch11 == NULL);
  ch11 = sktu_channel_create_extended(sktc_instance_uuid, port1,
                                      CHANNEL_DIR_TX_RX, (ringid + 1), NULL, -1,
                                      -1, -1, -1, -1, upp, 1, -1, -1);
  assert(ch11 == NULL);
  SKTC_ASSERT_ERR(errno == EACCES);
  ...
  /* these all must work (key attributes match) */
  ch10 = sktu_channel_create_extended(sktc_instance_uuid, port1,
                                      CHANNEL_DIR_TX_RX, ringid, attr1, -1, -1,
                                      -1, -1, -1, upp, 1, -1, -1);
  assert(ch10 != NULL);

  ch11 = sktu_channel_create_extended(sktc_instance_uuid, port1,
                                      CHANNEL_DIR_TX_RX, (ringid + 1), attr1,
                                      -1, -1, -1, -1, -1, upp, 1, -1, -1);
  assert(ch11 != NULL);
...
static int skt_bindupipeanon_main(int argc, char *argv[]) {
  int error;

  skt_bind_init("skywalk_test_upipe_anon", NEXUS_TYPE_USER_PIPE, 1, 0);

  error = sktc_bind_nexus_key(NEXUS_PORT_USER_PIPE_CLIENT, key0, sizeof(key0));
  SKTC_ASSERT_ERR(error == -1);
  SKTC_ASSERT_ERR(errno == ENXIO);

  error = sktc_bind_nexus_key(NEXUS_PORT_USER_PIPE_SERVER, key1, sizeof(key1));
  SKTC_ASSERT_ERR(error == -1);
  SKTC_ASSERT_ERR(errno == ENXIO);
...
static int skt_bindupipekey_main(int argc, char *argv[]) {
...
static int skt_bindfswanon_common(const char *name, int add_netif) {
  int error;

  skt_bind_init(name, NEXUS_TYPE_FLOW_SWITCH, 1, add_netif);

  error =
      sktc_bind_nexus_key(NEXUS_PORT_FLOW_SWITCH_CLIENT, key0, sizeof(key0));
  SKTC_ASSERT_ERR(error == -1);
  SKTC_ASSERT_ERR(errno == ENXIO);

  error = sktc_bind_nexus_key((NEXUS_PORT_FLOW_SWITCH_CLIENT + 1), key1,
                              sizeof(key1));
  SKTC_ASSERT_ERR(error == -1);
  SKTC_ASSERT_ERR(errno == ENXIO);
...
static int skt_bindfswkey_common(const char *name, int add_netif) {
  skt_bind_init(name, NEXUS_TYPE_FLOW_SWITCH, 0, add_netif);

  skt_bind_common(NEXUS_TYPE_FLOW_SWITCH, NEXUS_PORT_FLOW_SWITCH_CLIENT,
...
static int skt_bindnetifkey_common(const char *name, int add_netif) {
  ...
  for (int i = NEXUS_PORT_NET_IF_CLIENT; i < 128; i++) {
    error = os_nexus_controller_bind_provider_instance(
        sktc_nexus_controller, netif_instance, i, getpid(), NULL, key0,
        sizeof(key0), NEXUS_BIND_PID | NEXUS_BIND_KEY);
    if (error != 0) {
  ...
  error = os_nexus_controller_bind_provider_instance(
      sktc_nexus_controller, netif_instance, 128, getpid(), NULL, key0,
      sizeof(key0), NEXUS_BIND_PID | NEXUS_BIND_KEY);
  SKTC_ASSERT_ERR(error != 0);

  for (int i = NEXUS_PORT_NET_IF_CLIENT; i < 128; i++) {
    error = os_nexus_controller_unbind_provider_instance(sktc_nexus_controller,
                                                         netif_instance, i);
    SKTC_ASSERT_ERR(error == 0);
  }
...
static int skt_bindfswanon_main(int argc, char *argv[]) {
  return skt_bindfswanon_common("skywalk_test_fsw_anon", 1);
}

static int skt_bindfswkey_main(int argc, char *argv[]) {
  return skt_bindfswkey_common("skywalk_test_fsw", 1);
}

static int skt_bindnetifkey_main(int argc, char *argv[]) {
  return skt_bindnetifkey_common("skywalk_test_netif", 1);
}

struct skywalk_test skt_bindupipeanon = {
    "bindupipeanon",
    "bind a channel to an anonymous user pipe nexus",
...
struct skywalk_test skt_bindupipekey = {
    "bindupipekey",
    "bind a channel to a non-anonymous user pipe nexus",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_USER_PIPE,
