...
/*
 * <rdar://problem/29301703> Skywalk: Add unit tests to check that flowswitch
 * can grow beyond 64 ports
 *
...
static int skt_fsw29301703_common(int nchannels) {
  int error;
  int result = 0;
  ...
  /* Create a netif */
  strncpy((char *)attr.name, "skt_29301703_netif", sizeof(nexus_name_t) - 1);
  attr.type = NEXUS_TYPE_NET_IF;
  sktc_build_nexus(ncd, &attr, &netif_provider, &netif_instance);

  error = __os_nexus_ifattach(ncd, netif_instance, FETH0_NAME, NULL, false,
                              &netif_attach);

  /* attach the netif to the fsw */
  error = __os_nexus_ifattach(ncd, fsw_instance, NULL, netif_instance, false,
                              &fsw_if_attach);
  SKTC_ASSERT_ERR(!error);

  /* must fail without user packet pool set (flow switch) */
  assert(sktu_channel_create_extended(fsw_instance, 2, CHANNEL_DIR_TX_RX,
  ...
  for (int i = 0; i < sizeof(channels) / sizeof(channels[0]); i++) {
    channels[i] = sktu_channel_create_extended(
        fsw_instance, i + 2, CHANNEL_DIR_TX_RX, CHANNEL_RING_ID_ANY, NULL, -1,
        -1, -1, -1, -1, 1, 1, -1, -1);
    if (!channels[i]) {
...
static int skt_fsw29301703a_main(int argc, char *argv[]) {
  return skt_fsw29301703_common(63);
}

static int skt_fsw29301703b_main(int argc, char *argv[]) {
  return skt_fsw29301703_common(200);
}

static int skt_fsw29301703c_main(int argc, char *argv[]) {
  /*
   * Expect failure
...
struct skywalk_test skt_fsw29301703b = {
    "fsw29301703b",
    "open 200 channels to a flow switch",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH,
    skt_fsw29301703b_main,
    {NULL},
    sktc_ifnet_feth0_create,
    sktc_ifnet_feth0_destroy,
};
...
struct skywalk_test skt_fsw29301703c = {
    "fsw29301703c",
    "open too many channels to a flow switch",
    SK_FEATURE_SKYWALK | SK_FEATURE_NEXUS_NETIF | SK_FEATURE_NEXUS_FLOWSWITCH,
    skt_fsw29301703c_main,
    {NULL},
    sktc_ifnet_feth0_create,
    sktc_ifnet_feth0_destroy,
};
