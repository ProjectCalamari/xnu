...
#define SKTU_IFF_ENABLE_NETIF                                                  \
  0x00000001 // no-netif (txstart bsd interface) by default
#define SKTU_IFF_NO_ATTACH_FSW                                                 \
  0x00000002 // auto-attach fsw for netif by default
...
#define SKTU_MAX_MTU_PAYLOAD_LEN                                               \
  (1500 - (sizeof(struct ip6_hdr) + sizeof(struct tcphdr) - sizeof(uint32_t)))
typedef struct {
...
struct sktu_flow_ops {
  size_t (*create_input_frames)(struct sktu_flow *flow,
                                struct sktu_frame **frames, size_t max_frames,
...
void sktc_create_flowswitch_no_address(struct sktc_nexus_handles *handles,
                                       uint64_t tx_slots, uint64_t rx_slots,
...
uint32_t sktc_chew_random(channel_t channel, channel_ring_t ring,
                          sync_mode_t mode, bool dosync, uint32_t avail);
...
extern uint16_t in6_pseudo(const struct in6_addr *src,
                           const struct in6_addr *dst, uint32_t x);
...
extern void skt_channel_port_send(
    ...
    int payload_length, uint32_t limit, boolean_t must_complete_batch,
    boolean_t connect, packet_svc_class_t svc_class, boolean_t csum_offload,
...
extern size_t sktu_create_ip6_frames(struct sktu_frame **frames, size_t n,
                                     ...
                                     size_t mtu, uint16_t csum_flags,
