...
#define SKTC_ASSERT_ERR(t)                                                     \
  do {                                                                         \
    if (!(t))                                                                  \
      SKT_LOG("%s:%d Unexpected: error %d errno %d: %s", __func__, __LINE__,   \
...
#define SKTC_GENERIC_UPIPE_ARGV                                                \
  {NULL, NULL, NULL, sktc_instance_uuid_string, NULL}

extern void sktc_generic_kpipe_init(void);
extern void sktc_generic_kpipe_fini(void);
#define SKTC_GENERIC_KPIPE_ARGV                                                \
  {NULL, NULL, NULL, sktc_instance_uuid_string, NULL}
...
extern int wait_on_fd(int, int16_t, channel_t, uint32_t, enum timeout_behavior);
...
extern void sktc_build_nexus(nexus_controller_t ncd,
                             struct sktc_nexus_attr *sktc_attr,
...
static inline void sktc_feth_name_for_unit(char *namebuf, size_t namebuf_size,
                                           int unit) {
...
static inline void sktc_rd_name_for_unit(char *namebuf, size_t namebuf_size,
                                         int unit) {
...
extern int sktc_ifnet_add_addr(char *ifname, struct in_addr *addr,
                               struct in_addr *mask, struct in_addr *broadaddr);
extern int sktc_ifnet_add_addr6(char *ifname, struct in6_addr *addr,
                                struct in6_addr *dstaddr, int prefix_len,
                                int flags);
...
extern int sktc_ifnet_add_scoped_default_route(char *ifname,
                                               struct in_addr ifa);

static inline struct in_addr sktc_make_in_addr(uint32_t s) {
  struct in_addr ip;
...
static inline struct in_addr sktc_feth0_in_addr(void) {
  return sktc_make_in_addr(FETH0_INADDR);
}

static inline struct in_addr sktc_feth1_in_addr(void) {
  return sktc_make_in_addr(FETH1_INADDR);
}

#define RD0_INADDR 0x0a00fa01

static inline struct in_addr sktc_rd0_in_addr(void) {
  static_assert(RD0_INADDR == FETH0_INADDR);
  return sktc_make_in_addr(RD0_INADDR);
}

static inline struct in_addr sktc_nowhere_in_addr(void) {
...
static inline void sktc_feth0_inet6_addr(struct in6_addr *ip6_addr) {
  assert(inet_pton(AF_INET6, FETH0_INET6_ADDR, ip6_addr) == 1);
}

static inline void sktc_feth1_inet6_addr(struct in6_addr *ip6_addr) {
...
typedef void (^wildcard_event_handler_t)(os_channel_event_type_t event_type,
