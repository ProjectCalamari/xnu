 ...
 */
T_GLOBAL_META(T_META_RUN_CONCURRENTLY(false), T_META_OWNER("josephb_22"),
              T_META_OWNER("y_feigelson"), T_META_NAMESPACE("xnu.vm"),
 ...
 */
static int my_system(const char *command, const char *arg) {
  pid_t pid;
  int status = 0;
  int signal = 0;
  int ret;
  const char *argv[] = {command, "-v", arg, NULL};
...
static void cleanup_after_injections(void) {
 ...
 *
 * atan() was picked as a shared region function that isn't likely used by any
 * normal daemons.
 *
...
static void test_body(void) {
  ...
  ret = run_helper("Yfoo");
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "First call of foo");

  ret = run_helper("Xfoo");
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "Failed to recover from UE in clean app text page");
  ...
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "First call of atan");

  ret = run_helper("Xatan");
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "Failed to recover from UE in clean shared region page");
  ...
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "Failed to recover from UE in clean page");
  ...
  T_QUIET;
  T_ASSERT_NE(ret, 0, "Expected to fail from UE in dirty DATA page");

  /*
   * test of clean dynamically allocated page
   * since the page is clean, we expect to recover
   *
   * Test is disabled - rdar://124132874 (XNU ECC unit tests - "Xmmap_clean"
   * fails)
   */
  // ret = run_helper("Xmmap_clean");
  // T_QUIET; T_ASSERT_EQ(ret, 0, "Failed to recover from ECC to clean
  ...
  T_QUIET;
  T_ASSERT_NE(ret, 0,
              "Expected to fail from UE in dirty dynamically allocated page");
  ...
  T_QUIET;
  T_ASSERT_NE(ret, 0, "Uncorrected ECC copyout didn't fail");
}

static void cleanup_ecc_test(void) {
  uint value;
...
static void run_test(bool use_dcs) {
  int err;
  uint value = 0;
  size_t s = sizeof value;
...
T_DECL(ecc_uncorrected_test,
       "test detection and handling of non-fatal ECC uncorrected errors",
       T_META_IGNORECRASHES(".*ecc_test_helper.*"), T_META_ASROOT(true),
...
T_DECL(dcs_uncorrected_test,
       "test detection and handling from non-fatal ECC uncorrected errors "
       "(injected via DCS)",
       T_META_IGNORECRASHES(".*ecc_test_helper.*"), T_META_ASROOT(true),
       T_META_ENABLED(false /* TARGET_CPU_ARM64 && TARGET_OS_OSX */), /* rdar://133461215
                                                                       */
