 ...
 */
T_DECL(posix_spawn_filtering, "Check posix_spawn_filtering",
  ...
  strlcat(tmp_path_filter_rules, tmpdir ? tmpdir : "/tmp",
          sizeof(tmp_path_filter_rules));
  strlcat(tmp_path_filter_rules, "/filter.rules.XXXXX",
          sizeof(tmp_path_filter_rules));
  int filter_rules_fd = mkstemp(tmp_path_filter_rules);
  T_ASSERT_POSIX_SUCCESS(filter_rules_fd, "create temporary file 1");

  const char *filter_rules_contents = "binary_name:sh\n"
                                      "add_env:ADDED_VAR=VIA_RULES\n";
  ssize_t bytes_written = write(filter_rules_fd, filter_rules_contents,
                                strlen(filter_rules_contents));
  T_ASSERT_EQ(bytes_written, (long)strlen(filter_rules_contents),
              "write should write all contents");
  close(filter_rules_fd);

  strlcat(tmp_path_env_output, tmpdir ? tmpdir : "/tmp",
          sizeof(tmp_path_env_output));
  strlcat(tmp_path_env_output, "/env.output.XXXXX",
          sizeof(tmp_path_env_output));
  ...
  sprintf(rules_path_env, "POSIX_SPAWN_FILTERING_RULES_PATH=%s",
          tmp_path_filter_rules);
  ...
  T_ASSERT_POSIX_SUCCESS(posix_spawn_file_actions_init(&file_actions),
                         "posix_spawn_file_actions_init");
  T_ASSERT_POSIX_SUCCESS(posix_spawn_file_actions_adddup2(
                             &file_actions, env_output_fd, STDOUT_FILENO),
                         "posix_spawn_file_actions_addup2");

  int ret;
  ret = posix_spawn(&child_pid, prog, &file_actions, NULL, argv_child,
                    envp_child);
  ...
  T_ASSERT_EQ(waitpid_result, child_pid,
  ...
  T_ASSERT_EQ((long)read(env_output_fd, env_file_content, (size_t)s.st_size),
              (long)s.st_size, "read should load the whole file");
  ...
  T_SKIP("posix_spawn_filtering only supported with "
         "POSIX_SPAWN_FILTERING_ENABLED");
