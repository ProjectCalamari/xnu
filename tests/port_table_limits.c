...
T_DECL(test_port_table_setting_limits, "Allocate ports upto hard limit",
  ...
  err =
      posix_spawn(&child_pid, child_args[0], NULL, &attrs, child_args, environ);
...
void server_setup(struct args *args) {
  ...
  ret = bootstrap_register(bsport, (const char *)args->server_port_name,
                           args->server_port);
...
void server_run(struct args *args) {
  ...
  int err =
      task_for_pid(mach_task_self(), (int)args->client_pid, &client_task_port);
  T_ASSERT_MACH_SUCCESS(err, "server: task_for_pid");
  ret = task_set_special_port(client_task_port, TASK_RESOURCE_NOTIFY_PORT,
  ...
  reply->msgh_bits = MACH_MSGH_BITS_SET(MACH_MSG_TYPE_COPY_SEND, 0, 0, 0) |
  ...
  ret = mach_msg(reply, option, (mach_msg_size_t)args->reply_msg_size, 0,
                 MACH_PORT_NULL, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
  T_ASSERT_MACH_SUCCESS(ret, "server: mach_msg reply sent");
}

static mach_port_t create_resource_notify_port() {
  kern_return_t kret;
  ...
  kret =
      mach_port_insert_right(task, rn_port, rn_port, MACH_MSG_TYPE_MAKE_SEND);
...
T_DECL(test_port_table_hard_limit_with_resource_notify_port,
  ...
  /*
   * Recover the service port because the port must have been destroyed and sent
   * the notification by now
   */
  kr = mach_msg_server_once(resource_notify_server, 4096, resource_notify_port,
                            0);
...
kern_return_t receive_cpu_usage_violation(
    __unused mach_port_t receiver, __unused proc_name_t procname,
    __unused pid_t pid, __unused posix_path_t killed_proc_path,
    __unused mach_timespec_t timestamp, __unused int64_t observed_cpu_nsecs,
    __unused int64_t observation_nsecs, __unused int64_t cpu_nsecs_allowed,
...
kern_return_t receive_cpu_wakes_violation(
    __unused mach_port_t receiver, __unused proc_name_t procname,
    __unused pid_t pid, __unused posix_path_t killed_proc_path,
    __unused mach_timespec_t timestamp, __unused int64_t observed_cpu_wakes,
    __unused int64_t observation_nsecs, __unused int64_t cpu_wakes_allowed,
    __unused int64_t limit_window_nsecs,
    __unused resource_notify_flags_t flags) {
...
kern_return_t receive_disk_writes_violation(
    __unused mach_port_t receiver, __unused proc_name_t procname,
    __unused pid_t pid, __unused posix_path_t killed_proc_path,
    __unused mach_timespec_t timestamp, __unused int64_t observed_bytes_dirtied,
    __unused int64_t observation_nsecs, __unused int64_t bytes_dirtied_allowed,
    __unused int64_t limit_window_nsecs,
    __unused resource_notify_flags_t flags) {
  return KERN_SUCCESS;
}
...
kern_return_t receive_port_space_violation(
    __unused mach_port_t receiver, __unused proc_name_t procname,
    __unused pid_t pid, __unused mach_timespec_t timestamp,
    int64_t observed_ports, int64_t ports_allowed,
    __unused mach_port_t fatal_port, __unused resource_notify_flags_t flags) {
  T_LOG("Received a notification on the resource notify port");
  T_LOG("Ports_allowed = %lld, observed_ports = %lld", ports_allowed,
        observed_ports);
  if (fatal_port) {
...
kern_return_t receive_file_descriptors_violation(
    __unused mach_port_t receiver, __unused proc_name_t procname,
    __unused pid_t pid, __unused mach_timespec_t timestamp,
    __unused int64_t observed_filedesc, __unused int64_t filedesc_allowed,
    __unused mach_port_t fatal_port, __unused resource_notify_flags_t flags) {
  return KERN_SUCCESS;
}
