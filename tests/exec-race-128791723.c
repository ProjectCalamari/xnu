...
static int32_t get_pidversion_for_pid(pid_t pid) {
  struct proc_bsdinfowithuniqid bsd_info;
  int ret = proc_pidinfo(pid, PROC_PIDT_BSDINFOWITHUNIQID, 0, &bsd_info,
                         sizeof(bsd_info));
  T_ASSERT_EQ((unsigned long)ret, sizeof(bsd_info),
...
T_DECL(ensure_pidversion_changes_on_exec,
       ...
       T_META_NAMESPACE("xnu.exec"), T_META_TAG_VM_PREFERRED) {
  T_SETUPBEGIN;

  // Given we exec a helper program (in a forked child, so this runner can stick
  // around) (And we set up some resources to communicate with the forked
  // process)
  int pipefd[2];
  ...
  if (forked_pid == 0) {
    ...
    // And we ask the kernel to groom things such that `nextpidversion ==
    // current_proc->p_idversion + 1`
    int64_t val = 0;
    size_t val_len = sizeof(val);
    sysctlbyname("debug.test.setup_ensure_pidversion_changes_on_exec", &val,
                 &val_len, &val, sizeof(val));

    // (And we send the parent's pidversion back to the test runner, for
    // comparison with the exec'd process)
    T_ASSERT_POSIX_SUCCESS(
        write(pipefd[1], (void *)&forked_proc_pidv, sizeof(forked_proc_pidv)),
        "write");
  ...
  // (And we read the parent's pidversion from our forked counterpart, for
  // comparison with the exec'd process)
  int32_t forked_proc_pidversion;
  T_ASSERT_POSIX_SUCCESS(
      read(pipefd[0], &forked_proc_pidversion, sizeof(forked_proc_pidversion)),
      "read");
  ...
  T_ASSERT_NE(exec_proc_pidversion, forked_proc_pidversion,
              "Prevent pidversion reuse");
