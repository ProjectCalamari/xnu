...
T_DECL(posix_sem_open, "POSIX sem_open", T_META_TAG_VM_PREFERRED) {
  sem_t *sem;

  T_SETUPBEGIN;
  srand(time(NULL));
  snprintf(open_test_prefix, sizeof(open_test_prefix), "xnutest%d",
           rand() % 10000);
  snprintf(open_sem_invalid, sizeof(open_sem_invalid), "%s/invalid",
           open_test_prefix);
  ...
  T_EXPECT_EQ_PTR(sem, SEM_FAILED,
                  "sem_open(O_CREAT | O_EXCL) on existing fails");
  T_EXPECT_EQ(errno, EEXIST,
              "sem_open(O_CREAT | O_EXCL) on existing gives EEXIST");

  sem = sem_open(open_sem_b, O_CREAT | O_EXCL, 0755, 0);
  T_WITH_ERRNO;
  T_EXPECT_NE_PTR(sem, SEM_FAILED,
                  "sem_open(O_CREAT | O_EXCL) on non-existing succeeds");
  ...
  for (int i = 0; i < NUM_TEST_SEMAPHORES; i++) {
    ...
    T_WITH_ERRNO;
    T_EXPECT_NE_PTR(sem, SEM_FAILED, "sem_open name=%s oflag=%d succeeds",
                    name_buf, oflag);
  }
  ...
  for (int i = 0; i < NUM_TEST_SEMAPHORES; i++) {
    ...
    snprintf(name_buf, sizeof(name_buf), "%s/many%d", open_test_prefix,
             semaphore);
...
static int find_helper(char *test_path, int team_id) {
  ...
  snprintf(test_path, MAXPATHLEN, "%s/posix_sem_namespace_helper_team%d",
           dirpath, team_id);
...
static void do_namespace_op(const char *namespace, const char *op) {
  ...
  dt_pipe_data_handler_t stderr_handler =
      ^bool(char *data, __unused size_t data_size,
            __unused dt_pipe_data_handler_context_t *context) {
  ...
  pid_t pid = dt_launch_tool_pipe(
      (char *[]){(char *)namespace, namespace_test_sem_name, (char *)op, NULL},
      false, NULL, stdout_handler, stderr_handler, BUFFER_PATTERN_LINE,
      (void *)namespace);

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(pid, "dt_launch_tool_pipe %s (%s) - %s", op, namespace,
                         namespace_test_sem_name);

  ret = dt_waitpid(pid, &exit_status, &signum, 60 * 5);
  T_QUIET;
  T_ASSERT_EQ(ret, 1, "dt_waitpid (exit=%d,signum=%d)", exit_status, signum);
  T_QUIET;
...
static void cleanup_namespace() { sem_unlink(namespace_test_sem_name); }

/*
 * Unfortunately this test suffers from two issues that mean we must leave it
 * disabled on BATS:
 *   1. rdar://75835929 means that XBS strips the team ID from our helper
 * binaries after we've signed them.
 *   2. BATS infrastructure boots with amfi_get_out_of_my_way=1, which treats
 * signatures as CS_PLATFORM_BINARY and causes the team ID to be ignored.
 */
T_DECL(posix_sem_open_team_id_namespace, "POSIX sem_open team ID namespace",
       T_META_BOOTARGS_SET("amfi_allow_any_signature=1"), T_META_ENABLED(FALSE),
       T_META_TAG_VM_PREFERRED) {
  T_SETUPBEGIN;
  srand(time(NULL));
  snprintf(namespace_test_sem_name, sizeof(namespace_test_sem_name),
           "xnutest%d/ns", rand() % 10000);
  ...
  /* Check that semaphores created by 1st party applications can be discovered
  ...
  do_namespace_op(team1_helper, "check_no_access");
#else
  /* 1st party applications should not be able to look up semaphores created by
