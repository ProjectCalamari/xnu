...
T_GLOBAL_META(T_META_NAMESPACE("xnu"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("arm"), T_META_OWNER("p_tennen"));

static size_t exception_handler_expect_not_called(
    mach_port_t task __unused, mach_port_t thread __unused,
...
static void signal_handler_expect_not_called(int sig, siginfo_t *sip __unused,
...
T_DECL(uncatchable_fatal_trap_developer_mode_disabled,
       "Ensure a maybe-unrecoverable trap label is uncatchable with "
       "!developer_mode",
       T_META_REQUIRES_SYSCTL_EQ("security.mac.amfi.developer_mode_status", 0),
       T_META_ENABLED(TARGET_CPU_ARM64)) {
  /* Given a child process that sets up some mechanisms to catch an
   * exception/signal */
  ...
  if (pid == 0) {
    ...
    run_exception_handler(
        exc_port, (exc_handler_callback_t)exception_handler_expect_not_called);

    struct sigaction sa = {.sa_sigaction = signal_handler_expect_not_called,
                           .sa_flags = SA_SIGINFO};
    ...
    /* Then the child does not have an opportunity to run its exception
     * handlers, and is immediately killed */
...
T_DECL(uncatchable_fatal_trap_developer_mode_enabled,
       "Ensure an maybe-unrecoverable trap label is uncatchable with "
       "developer_mode",
...
static size_t exception_handler_expect_called(
    mach_port_t task __unused, mach_port_t thread __unused,
    exception_type_t type __unused, mach_exception_data_t codes __unused) {
...
static void signal_handler_expect_called(int sig, siginfo_t *sip __unused,
                                         void *ucontext __unused) {
...
T_DECL(uncatchable_fatal_trap_debugged,
       ...
       T_META_REQUIRES_SYSCTL_EQ("security.mac.amfi.developer_mode_status", 1),
       /* It's not straightforward to ptrace on platforms other than macOS, so
          don't bother */
  ...
  shared_was_mach_exception_handler_called = (bool *)mmap(
      NULL, sizeof(bool), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
  shared_was_posix_signal_handler_called = (bool *)mmap(
      NULL, sizeof(bool), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
  bool *has_parent_connected = (bool *)mmap(
      NULL, sizeof(bool), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
  *has_parent_connected = false;
  ...
  if (pid == 0) {
    ...
    mach_port_t exc_port = create_exception_port(EXC_MASK_ALL);
    run_exception_handler(
        exc_port, (exc_handler_callback_t)exception_handler_expect_called);

    struct sigaction sa = {.sa_sigaction = signal_handler_expect_called,
                           .sa_flags = SA_SIGINFO};
    sigfillset(&sa.sa_mask);
