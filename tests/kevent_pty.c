...
static void child_tty_client(void) {
...
T_DECL(pty_master_teardown,
  ...
  T_ASSERT_POSIX_SUCCESS(master_pid,
                         "forked child master PTY with pid %d, at pty %s",
                         master_pid, pty_filename);
...
static void redispatch(dispatch_group_t grp, dispatch_source_type_t type,
                       int fd) {
  __block int iters = 0;
  ...
  __block void (^redispatch_blk)(void) = Block_copy(^{
    ...
    dispatch_source_t src = dispatch_source_create(type, (uintptr_t)fd, 0,
...
T_DECL(master_read_data_set,
  ...
  T_QUIET;
  T_ASSERT_GE(master, 0, "master fd is valid");
  T_QUIET;
  T_ASSERT_GE(slave, 0, "slave fd is valid");

  dispatch_source_t src =
      dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, (uintptr_t)master, 0,
                             dispatch_get_main_queue());
