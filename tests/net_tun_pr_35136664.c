...
T_GLOBAL_META(T_META_NAMESPACE("xnu.net"), T_META_RUN_CONCURRENTLY(true),
              T_META_ASROOT(true));

T_DECL(PR_35136664_utun, "This bind a utun and close it without connecting",
...
T_DECL(PR_35136664_ipsec, "This bind a ipsec and close it without connecting",
       T_META_TAG_VM_PREFERRED) {
  int tunsock;
  struct ctl_info kernctl_info;
  struct sockaddr_ctl kernctl_addr;
  ...
  memset(&kernctl_info, 0, sizeof(kernctl_info));
  strlcpy(kernctl_info.ctl_name, IPSEC_CONTROL_NAME,
          sizeof(kernctl_info.ctl_name));
  T_ASSERT_POSIX_SUCCESS(ioctl(tunsock, CTLIOCGINFO, &kernctl_info), NULL);
