...
static const char *str_bpf_direction(u_int direction) {
  switch (direction) {
  case BPF_D_NONE:
    return "BPF_D_NONE";
  case BPF_D_IN:
...
static void test_set_direction(int fd, u_int direction) {
  T_ASSERT_POSIX_SUCCESS(bpf_set_direction(fd, direction),
                         "bpf_set_direction(%d): %u (%s)", fd, direction,
                         str_bpf_direction(direction));

  u_int get_direction = (u_int)(-2);
  T_ASSERT_POSIX_SUCCESS(bpf_get_direction(fd, &get_direction),
                         "bpf_get_direction(%d): %u (%s)", fd, get_direction,
                         str_bpf_direction(get_direction));
  T_ASSERT_EQ(get_direction, direction, "get_direction %d == direction %d",
...
T_DECL(bpf_direction, "test BPF set and grep direction",
  ...
  T_ASSERT_POSIX_SUCCESS(bpf_get_direction(fd, &direction),
                         "bpf_get_direction(%d): %u (%s)", fd, direction,
                         str_bpf_direction(direction));
...
T_DECL(bpf_seesent, "test BIOCGSEESENT and BIOCSSEESENT",
  ...
  T_ASSERT_POSIX_SUCCESS(ioctl(fd, BIOCGSEESENT, &get_see_sent),
                         "BIOCGSEESENT");
  ;
  ...
  T_ASSERT_POSIX_SUCCESS(ioctl(fd, BIOCSSEESENT, &set_see_sent),
                         "BIOCSSEESENT");
  ;
