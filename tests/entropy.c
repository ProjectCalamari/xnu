...
T_DECL(entropy_collect, "Collect entropy for offline analysis",
  ...
  ret = sysctlbyname("kern.entropy.analysis.buffer_size", &entropy_size, &size,
                     NULL, 0);
  ...
  for (uint32_t i = 0; i < entropy_count; i += 1) {
    T_QUIET;
    T_EXPECT_NE(entropy[i], 0, "entropy buffer null sample %u", i);
  }
...
T_DECL(entropy_filter_rate, "Sample entropy filter rate") {
  ...
  ret = sysctlbyname("kern.entropy.filter.total_sample_count",
                     &total_sample_count, &size, NULL, 0);
  T_ASSERT_POSIX_SUCCESS(ret, "kern.entropy.filter.total_sample_count");

  size = sizeof(rejected_sample_count);
  ret = sysctlbyname("kern.entropy.filter.rejected_sample_count",
                     &rejected_sample_count, &size, NULL, 0);
  T_ASSERT_POSIX_SUCCESS(ret, "kern.entropy.filter.rejected_sample_count");

  double rejection_rate =
      (double)rejected_sample_count / (double)total_sample_count;

  pdwriter_t writer =
      pdwriter_open_tmp("xnu", "entropy_filter_rate", 0, 0, NULL, 0);
  T_ASSERT_NOTNULL(writer, "pdwriter_open_tmp");

  pdwriter_new_value(writer, "Rejection Rate", PDUNIT_CUSTOM(rejectrate),
                     rejection_rate);
