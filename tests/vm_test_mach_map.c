...
static void test_memory_entry_tagging(int override_tag) {
  int pass;
  int do_copy;
  kern_return_t kr;
  ...
  vmaddr_orig = 0;
  kr = mach_vm_allocate(mach_task_self(), &vmaddr_orig, vmsize_orig,
                        VM_FLAGS_ANYWHERE);
  T_QUIET;
  T_EXPECT_MACH_SUCCESS(kr, "[override_tag:%d] vm_allocate(%lld)", override_tag,
                        vmsize_orig);
  ...
  for (i = 0; i < vmsize_orig / vmsize_chunk; i++) {
    ...
    kr = mach_vm_allocate(
        mach_task_self(), &vmaddr_chunk, vmsize_chunk,
        (VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE | VM_MAKE_TAG(100 + (int)i)));
    T_QUIET;
  ...
  for (vmoff = 0; vmoff < vmsize_orig; vmoff += PAGE_SIZE) {
...
again:
  *((unsigned char *)(uintptr_t)vmaddr_orig) = 'x';
  ...
  kr = mach_make_memory_entry_64(
      mach_task_self(), vmsize_ptr, vmaddr_orig, /* offset */
      (vm_flags | VM_PROT_READ | VM_PROT_WRITE), mem_entry_ptr, MACH_PORT_NULL);
  T_QUIET;
  T_EXPECT_MACH_SUCCESS(
      kr, "[override_tag:%d][do_copy:%d] mach_make_memory_entry()",
  ...
  T_QUIET;
  T_EXPECT_NOTNULL(*mem_entry_ptr,
  ...
  *vmaddr_ptr = 0;
  ...
  kr = mach_vm_map(mach_task_self(), vmaddr_ptr, vmsize_orig, 0,    /* mask */
                   vm_flags | VM_FLAGS_ANYWHERE, *mem_entry_ptr, 0, /* offset */
                   FALSE,                                           /* copy */
  ...
  *((unsigned char *)(uintptr_t)vmaddr_orig) = 'X';
  if (*(unsigned char *)(uintptr_t)*vmaddr_ptr == 'X') {
    T_QUIET;
    T_EXPECT_EQ(do_copy, 0,
                "[override_tag:%d][do_copy:%d] memory shared instead of copied",
                override_tag, do_copy);
    if (T_RESULT == T_RESULT_FAIL) {
      goto done;
    }
    ...
    T_QUIET;
    T_EXPECT_NE(do_copy, 0,
                "[override_tag:%d][do_copy:%d] memory copied instead of shared",
                override_tag, do_copy);
    if (T_RESULT == T_RESULT_FAIL) {
      goto done;
    }
  ...
  for (i = 0; i < vmsize_orig / vmsize_chunk; i++) {
    ...
    kr = mach_vm_region_recurse(mach_task_self(), &vmaddr_info, &vmsize_info,
                                &depth, (vm_region_recurse_info_t)&ri,
                                &ri_count);
    T_QUIET;
    T_EXPECT_MACH_SUCCESS(
        kr,
    ...
    T_QUIET;
    T_EXPECT_EQ(vmaddr_info, *vmaddr_ptr + (i * vmsize_chunk),
                "[override_tag:%d][do_copy:%d] "
                "mach_vm_region_recurse(0x%llx+0x%llx) returned addr 0x%llx",
                override_tag, do_copy, *vmaddr_ptr,
                (mach_vm_size_t)i * vmsize_chunk, vmaddr_info);
    if (T_RESULT == T_RESULT_FAIL) {
      goto done;
    }
    T_QUIET;
    T_EXPECT_EQ(
        vmsize_info, vmsize_chunk,
        "[override_tag:%d][do_copy:%d] mach_vm_region_recurse(0x%llx+0x%llx) "
        "returned size 0x%llx expected 0x%llx",
        override_tag, do_copy, *vmaddr_ptr, (mach_vm_size_t)i * vmsize_chunk,
        vmsize_info, vmsize_chunk);
    if (T_RESULT == T_RESULT_FAIL) {
      goto done;
    }
    ...
    T_QUIET;
    T_EXPECT_EQ(ri.user_tag, expected_tag,
                "[override_tag:%d][do_copy:%d] i=%u tag=%u expected %u",
  ...
  if (vmaddr_orig != 0) {
    mach_vm_deallocate(mach_task_self(), vmaddr_orig, vmsize_orig);
    vmaddr_orig = 0;
    vmsize_orig = 0;
  }
  if (vmaddr_copied != 0) {
    mach_vm_deallocate(mach_task_self(), vmaddr_copied, vmsize_copied);
    vmaddr_copied = 0;
    vmsize_copied = 0;
  }
  if (vmaddr_shared != 0) {
    mach_vm_deallocate(mach_task_self(), vmaddr_shared, vmsize_shared);
    vmaddr_shared = 0;
    vmsize_shared = 0;
  }
...
static void test_map_memory_entry(void) {
  kern_return_t kr;
  ...
  vmsize1 = 1;
  vmaddr1 = 0;
  kr = mach_vm_allocate(mach_task_self(), &vmaddr1, vmsize1, VM_FLAGS_ANYWHERE);
  T_QUIET;
  ...
  *cp1 = '1';
  ...
  mem_entry = MACH_PORT_NULL;
  kr = mach_make_memory_entry_64(
      mach_task_self(), &vmsize2, vmaddr1, /* offset */
      (MAP_MEM_VM_COPY | VM_PROT_READ | VM_PROT_WRITE), &mem_entry,
      MACH_PORT_NULL);
  T_QUIET;
  ...
  vmaddr2 = 0;
  kr = mach_vm_map(mach_task_self(), &vmaddr2, vmsize2, 0, /* mask */
                   VM_FLAGS_ANYWHERE, mem_entry, 0,        /* offset */
                   TRUE,                                   /* copy */
                   VM_PROT_READ | VM_PROT_WRITE, VM_PROT_READ | VM_PROT_WRITE,
                   VM_INHERIT_DEFAULT);
  T_QUIET;
  ...
  T_QUIET;
  T_EXPECT_TRUE(((*cp1 == '1') && (*cp2 == '1')),
                "*cp1/*cp2 0x%x/0x%x expected 0x%x/0x%x", *cp1, *cp2, '1', '1');
  if (T_RESULT == T_RESULT_FAIL) {
    goto done;
  }
  ...
  *cp2 = '2';
  T_QUIET;
  T_EXPECT_TRUE(((*cp1 == '1') && (*cp2 == '2')),
                "*cp1/*cp2 0x%x/0x%x expected 0x%x/0x%x", *cp1, *cp2, '1', '2');
  if (T_RESULT == T_RESULT_FAIL) {
    goto done;
  }
...
static char *vm_purgable_state[4] = {"NONVOLATILE", "VOLATILE", "EMPTY",
                                     "DENY"};
...
static uint64_t task_footprint(void) {
  ...
  kr = task_info(mach_task_self(), TASK_VM_INFO, (task_info_t)&ti, &count);
  T_QUIET;
...
T_DECL(purgeable_empty_to_volatile, "test task physical footprint when \
  ...
  for (cp = (char *)vm_addr; cp < (char *)(vm_addr + vm_size);
  ...
  kr = mach_vm_purgable_control(mach_task_self(), vm_addr,
  ...
  T_ASSERT_EQ(state, VM_PURGABLE_NONVOLATILE,
              "NONVOLATILE->VOLATILE: state was %s", vm_purgable_state[state]);
  ...
  if (state != VM_PURGABLE_VOLATILE && state != VM_PURGABLE_EMPTY) {
    T_ASSERT_FAIL("VOLATILE->EMPTY: state was %s", vm_purgable_state[state]);
  }
...
static kern_return_t get_reusable_size(uint64_t *reusable) {
  task_vm_info_data_t ti;
  mach_msg_type_number_t ti_count = TASK_VM_INFO_COUNT;
  kern_return_t kr;

  kr = task_info(mach_task_self(), TASK_VM_INFO, (task_info_t)&ti, &ti_count);
  T_QUIET;
...
T_DECL(madvise_shared, "test madvise shared for rdar://problem/2295713 logging \
    rethink needs madvise(MADV_FREE_HARDER)",
       T_META_RUN_CONCURRENTLY(false), T_META_ALL_VALID_ARCHS(true),
       T_META_TAG_VM_PREFERRED) {
  ...
  kr = vm_allocate(mach_task_self(), &vmaddr, vmsize, vmflags);
  T_QUIET;
  ...
  for (cp = (char *)(uintptr_t)vmaddr;
       cp < (char *)(uintptr_t)(vmaddr + vmsize); cp++) {
    *cp = 'x';
  }

  kr = vm_remap(mach_task_self(), &vmaddr2, vmsize, 0,              /* mask */
                VM_FLAGS_ANYWHERE, mach_task_self(), vmaddr, FALSE, /* copy */
  ...
  for (cp = (char *)(uintptr_t)vmaddr2;
       cp < (char *)(uintptr_t)(vmaddr2 + vmsize); cp++) {
    T_QUIET;
    T_EXPECT_EQ(*cp, 'x', "vmaddr=%p vmaddr2=%p %p:0x%x",
                (void *)(uintptr_t)vmaddr, (void *)(uintptr_t)vmaddr2,
                (void *)cp, (unsigned char)*cp);
    if (T_RESULT == T_RESULT_FAIL) {
      goto done;
    }
  }
  ...
  *cp = 'X';
  ...
  if (COMM_PAGE_READ(uint64_t, CPU_CAPABILITIES64) & kIsTranslated) {
    T_LOG("Skipping madvise reusable tests because we're running under "
          "translation.");
    goto done;
  }
#endif /* defined(__x86_64__) || defined(__i386__) */

  ret = mlock((char *)(uintptr_t)(vmaddr2 + vmsize1), vmsize2);
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(ret, "mlock()");

  footprint_before = task_footprint();

  ret = madvise((char *)(uintptr_t)vmaddr, vmsize1, MADV_FREE_REUSABLE);
  T_QUIET;
...
T_DECL(madvise_purgeable_can_reuse, "test madvise purgeable can reuse for \
...
#if defined(__x86_64__) || defined(__i386__)
  if (COMM_PAGE_READ(uint64_t, CPU_CAPABILITIES64) & kIsTranslated) {
    T_SKIP("madvise reusable is not supported under Rosetta translation. "
           "Skipping.)");
  }
#endif /* defined(__x86_64__) || defined(__i386__) */
  ...
  kr = vm_allocate(
      mach_task_self(), &vmaddr, vmsize,
      (VM_FLAGS_ANYWHERE | VM_FLAGS_PURGABLE | VM_MAKE_TAG(VM_MEMORY_MALLOC)));
  T_QUIET;
  T_EXPECT_MACH_SUCCESS(kr, "vm_allocate()");
  if (T_RESULT == T_RESULT_FAIL) {
    goto done;
  ...
  T_QUIET;
  T_EXPECT_TRUE(((ret == -1) && (errno == EINVAL)),
...
static bool validate_memory_is_zero(vm_address_t start, vm_size_t vmsize,
...
T_DECL(madvise_zero, "test madvise zero", T_META_TAG_VM_PREFERRED) {
  ...
  kr = vm_allocate(mach_task_self(), &vmaddr, vmsize,
  ...
  T_QUIET;
  T_LOG("waiting for first page to be paged out");
  do {
    ret = mincore((void *)vmaddr, 1, (char *)&vec);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "mincore(1st)");
  } while (vec & MINCORE_INCORE);
  T_QUIET;
  T_LOG("waiting for last page to be paged out");
  do {
    ret = mincore((void *)(vmaddr + vmsize - 1), 1, (char *)&vec);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "mincore(last)");
  } while (vec & MINCORE_INCORE);
...
T_DECL(madvise_zero_wired, "test madvise(MADV_ZERO_WIRED_PAGES)",
       T_META_TAG_VM_PREFERRED) {
  vm_address_t vmaddr;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "madvise(MADV_ZERO_WIRED_PAGES)");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "munmap()");
  T_ASSERT_EQ(*(char *)vmaddr_remap, 0, "wired pages are zero-filled on unmap");
  T_QUIET;
  T_ASSERT_EQ(validate_memory_is_zero(vmaddr_remap, vmsize, &non_zero_addr),
              true,
              "madvise(%p, %lu, MADV_ZERO_WIRED) did not zero-fill mem at %p",
              (void *)vmaddr, vmsize, (void *)non_zero_addr);
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "mprotect(PROT_READ)");
  ret = madvise((void *)vmaddr, vmsize, MADV_ZERO_WIRED_PAGES);
  // T_LOG("madv() ret %d errno %d\n", ret, errno);
  T_ASSERT_POSIX_FAILURE(ret, EPERM,
                         "madvise(MADV_ZERO_WIRED_PAGES) returns EPERM on "
                         "non-writable mapping ret %d errno %d",
                         ret, errno);
  ret = munmap((void *)vmaddr, vmsize);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "munmap()");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "munmap()");
  T_ASSERT_EQ(*(char *)vmaddr_remap, 'A',
              "RO wired pages NOT zero-filled on unmap");
  ret = munmap((void *)vmaddr_remap, vmsize);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "munmap()");
}
...
T_DECL(map_read_overwrite, "test overwriting vm map from other map - \
  ...
  vmaddr2 = 0;
  kr = mach_vm_allocate(mach_task_self(), &vmaddr2, vmsize1, VM_FLAGS_ANYWHERE);
  T_ASSERT_MACH_SUCCESS(kr, "vm_allocate()");
  ...
  kr = mach_vm_read_overwrite(mach_task_self(), vmaddr1 + sizeof(*ip), vmsize2,
                              vmaddr2 + sizeof(*ip), &vmsize2);
  T_ASSERT_MACH_SUCCESS(kr, "vm_read_overwrite()");

  ip = (uint32_t *)(uintptr_t)vmaddr2;
  for (i = 0; i < 1; i++) {
    T_QUIET;
    T_ASSERT_EQ(ip[i], DEST_PATTERN, "vmaddr2[%d] = 0x%x instead of 0x%x", i,
                ip[i], DEST_PATTERN);
  }
  for (; (mach_vm_size_t)i < (vmsize1 - 2) / sizeof(*ip); i++) {
    T_QUIET;
    T_ASSERT_EQ(ip[i], i, "vmaddr2[%d] = 0x%x instead of 0x%x", i, ip[i], i);
  }
...
T_DECL(copy_none_use_pmap, "test copy-on-write remapping of COPY_NONE vm \
  ...
  vmaddr1 = 0;
  kr = mach_vm_allocate(mach_task_self(), &vmaddr1, vmsize,
                        VM_FLAGS_ANYWHERE | VM_FLAGS_PURGABLE);
  T_ASSERT_MACH_SUCCESS(kr, "vm_allocate()");
  ...
  vmaddr2 = 0;
  kr = mach_vm_remap(mach_task_self(), &vmaddr2, vmsize, 0, /* mask */
  ...
  kr = mach_vm_remap(mach_task_self(), &vmaddr3, vmsize, 0, /* mask */
                     VM_FLAGS_ANYWHERE, mach_task_self(), vmaddr2,
                     TRUE, /* copy */
                     &curprot, &maxprot, VM_INHERIT_DEFAULT);
...
T_DECL(purgable_deny, "test purgeable memory is not allowed to be converted to \
  ...
  kr = vm_purgable_control(mach_task_self(), vmaddr, VM_PURGABLE_SET_STATE,
                           &state);
  T_ASSERT_EQ(kr, KERN_INVALID_ARGUMENT,
              "vm_purgable_control(VM_PURGABLE_DENY) -> 0x%x (%s)", kr,
...
T_DECL(vm_remap_zero, "test vm map of zero size - rdar://33114981",
  ...
  vmaddr2 = 0;
  vmsize = 0;
  kr = mach_vm_remap(mach_task_self(), &vmaddr2, vmsize, 0, VM_FLAGS_ANYWHERE,
                     mach_task_self(), vmaddr1, FALSE, &curprot, &maxprot,
                     VM_INHERIT_DEFAULT);
...
T_DECL(nested_pmap_trigger, "nested pmap should only be triggered from kernel \
  ...
  vmsize = PAGE_SIZE;
  kr = mach_make_memory_entry_64(mach_task_self(), &vmsize, sr_start,
                                 MAP_MEM_VM_SHARE | VM_PROT_READ, &mem_entry,
                                 MACH_PORT_NULL);
  T_ASSERT_MACH_SUCCESS(kr, "make_memory_entry(0x%llx)", sr_start);

  vmaddr = 0;
  kr = mach_vm_map(mach_task_self(), &vmaddr, vmsize, 0, VM_FLAGS_ANYWHERE,
                   mem_entry, 0, FALSE, VM_PROT_READ, VM_PROT_READ,
                   VM_INHERIT_DEFAULT);
  T_ASSERT_MACH_SUCCESS(kr, "vm_map()");
}

static const char *prot_str[] = {"---", "r--", "-w-", "rw-",
                                 "--x", "r-x", "-wx", "rwx"};
static const char *share_mode_str[] = {
    "---",       "COW",        "PRIVATE",         "EMPTY",
    "SHARED",    "TRUESHARED", "PRIVATE_ALIASED", "SHARED_ALIASED",
    "LARGE_PAGE"};

T_DECL(shared_region_share_writable,
       "sharing a writable mapping of the shared region shoudl not give write "
       "access to shared region - rdar://problem/74469953",
       T_META_ALL_VALID_ARCHS(true), T_META_TAG_VM_PREFERRED) {
  int ret;
  uint64_t sr_start;
  kern_return_t kr;
  ...
  T_QUIET;
  T_ASSERT_GE(sr_start, SHARED_REGION_BASE, "shared region starts below BASE");
  T_QUIET;
  T_ASSERT_LT(sr_start, SHARED_REGION_BASE + SHARED_REGION_SIZE,
  ...
  for (address = SHARED_REGION_BASE;
       address < SHARED_REGION_BASE + SHARED_REGION_SIZE; address += size) {
    size = 0;
    depth = 99;
    ...
    T_QUIET;
    T_EXPECT_MACH_SUCCESS(kr, "vm_region_recurse()");
    if (kr == KERN_INVALID_ADDRESS) {
      T_SKIP("could not find read-only nested mapping");
      T_END;
    }
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "vm_region_recurse()");
    T_LOG("0x%llx - 0x%llx depth:%d %s/%s %s 0x%x", address, address + size,
          depth, prot_str[info.protection], prot_str[info.max_protection],
          share_mode_str[info.share_mode], info.object_id);
    if (depth > 0 && (info.protection == VM_PROT_READ) &&
  ...
  remap_address = 0;
  remap_size = size;
  kr = mach_vm_remap(mach_task_self(), &remap_address, remap_size, 0,
                     VM_FLAGS_ANYWHERE | VM_FLAGS_RETURN_DATA_ADDR,
                     mach_task_self(), address, FALSE, &cur_prot, &max_prot,
                     VM_INHERIT_DEFAULT);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "vm_remap()");
  ...
  depth = 99;
  count = VM_REGION_SUBMAP_INFO_COUNT_64;
  kr = mach_vm_region_recurse(mach_task_self(), &tmp_address, &tmp_size, &depth,
                              (vm_region_recurse_info_t)&info, &count);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "vm_region_recurse()");
  T_LOG("0x%llx - 0x%llx depth:%d %s/%s %s 0x%x", tmp_address,
        tmp_address + tmp_size, depth, prot_str[info.protection],
        prot_str[info.max_protection], share_mode_str[info.share_mode],
        info.object_id);
  T_QUIET;
  T_ASSERT_EQ(tmp_address, address, "address hasn't changed");
  //	T_QUIET; T_ASSERT_EQ(tmp_size, size, "size hasn't changed");
  T_QUIET;
  T_ASSERT_GT(depth, 0, "still nested");
  T_QUIET;
  T_ASSERT_EQ(info.protection, VM_PROT_READ, "cur_prot still read-only");
  //	T_QUIET; T_ASSERT_EQ(info.max_protection, VM_PROT_READ, "max_prot still
  //read-only");
  ...
  T_LOG("0x%llx - 0x%llx depth:%d %s/%s %s 0x%x", tmp_address,
        ...
        info.object_id);
  T_QUIET;
  T_ASSERT_EQ(tmp_address, remap_address, "address hasn't changed");
  //	T_QUIET; T_ASSERT_EQ(tmp_size, size, "size hasn't changed");
  T_QUIET;
  T_ASSERT_EQ(info.protection, VM_PROT_READ, "new cur_prot read-only");
  //	T_QUIET; T_ASSERT_EQ(info.max_protection, VM_PROT_READ, "new max_prot
  //read-only");
  remap = *(uint32_t *)(uintptr_t)remap_address;
  T_QUIET;
  T_ASSERT_EQ(remap, before, "remap matches original");
  ...
  T_QUIET;
  T_EXPECT_MACH_SUCCESS(kr, "vm_deallocate()");
  ...
  kr = mach_vm_remap_new(mach_task_self(), &remap_address, remap_size, 0,
                         VM_FLAGS_ANYWHERE, mach_task_self(), address, FALSE,
                         &cur_prot, &max_prot, VM_INHERIT_DEFAULT);
  T_QUIET;
  T_EXPECT_MACH_SUCCESS(kr, "vm_remap_new()");
  if (kr == KERN_PROTECTION_FAILURE) {
    /* wrong but not a security issue... */
    goto skip_vm_remap_new_ro;
  }
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "vm_remap_new()");
  remap = *(uint32_t *)(uintptr_t)remap_address;
  T_QUIET;
  T_ASSERT_EQ(remap, before, "remap matches original");
  *(uint32_t *)(uintptr_t)remap_address = before + 1;
  ...
  *(uint32_t *)(uintptr_t)remap_address = before;
  ...
  T_ASSERT_EQ(info.protection, VM_PROT_READ, "cur_prot still read-only");
  T_QUIET;
  T_ASSERT_EQ(info.max_protection, VM_PROT_READ, "max_prot still read-only");
...
skip_vm_remap_new_ro:
  ...
  mem_entry = MACH_PORT_NULL;
  kr =
      mach_make_memory_entry_64(mach_task_self(), &remap_size, address,
                                MAP_MEM_VM_SHARE | VM_PROT_READ | VM_PROT_WRITE,
                                &mem_entry, MACH_PORT_NULL);
  T_QUIET;
  T_EXPECT_MACH_SUCCESS(kr, "mach_make_memory_entry_64(VM_SHARE)");
  if (kr == KERN_PROTECTION_FAILURE) {
    /* wrong but not a security issue... */
    goto skip_mem_entry_vm_share_ro;
  }
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_make_memory_entry_64(VM_SHARE)");
  remap_address = 0;
  kr = mach_vm_map(mach_task_self(), &remap_address, remap_size, 0, /* mask */
                   VM_FLAGS_ANYWHERE, mem_entry, 0,                 /* offset */
                   FALSE,                                           /* copy */
                   VM_PROT_READ | VM_PROT_WRITE, VM_PROT_READ | VM_PROT_WRITE,
                   VM_INHERIT_DEFAULT);
  ...
  *(uint32_t *)(uintptr_t)remap_address = before;
  ...
  //	T_QUIET; T_ASSERT_EQ(tmp_size, size, "size hasn't changed");
  T_QUIET;
  T_ASSERT_EQ(depth, 0, "new mapping is unnested");
  //	T_QUIET; T_ASSERT_EQ(info.protection, VM_PROT_READ, "new cur_prot
  //read-only"); 	T_QUIET; T_ASSERT_EQ(info.max_protection, VM_PROT_READ, "new
  //max_prot read-only");
  /* cleanup */
  kr = mach_vm_deallocate(mach_task_self(), remap_address, remap_size);
  T_QUIET;
...
skip_mem_entry_vm_share_ro:
  ...
  remap_size = size;
  mem_entry = MACH_PORT_NULL;
  kr = mach_make_memory_entry_64(mach_task_self(), &remap_size, address,
                                 VM_PROT_READ | VM_PROT_WRITE, &mem_entry,
                                 MACH_PORT_NULL);
  T_QUIET;
  T_ASSERT_EQ(kr, KERN_PROTECTION_FAILURE, "mach_make_memory_entry_64()");
  /* check that region is still nested */
  tmp_address = address;
  tmp_size = 0;
  depth = 99;
  ...
  T_ASSERT_MACH_SUCCESS(kr,
                        "mach_make_memory_entry_64(READ | WRITE | IS_MASK)");
  remap_address = 0;
  kr = mach_vm_map(mach_task_self(), &remap_address, remap_size, 0, /* mask */
                   VM_FLAGS_ANYWHERE, mem_entry, 0,                 /* offset */
                   FALSE,                                           /* copy */
                   ...
                   VM_INHERIT_DEFAULT);
  T_QUIET;
  T_ASSERT_EQ(kr, KERN_INVALID_RIGHT, "vm_map(read/write)");
  remap_address = 0;
  kr = mach_vm_map(mach_task_self(), &remap_address, remap_size, 0, /* mask */
                   VM_FLAGS_ANYWHERE, mem_entry, 0,                 /* offset */
                   FALSE,                                           /* copy */
                   VM_PROT_READ, VM_PROT_READ, VM_INHERIT_DEFAULT);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "vm_map(read only)");
  remap = *(uint32_t *)(uintptr_t)remap_address;
  T_QUIET;
  T_ASSERT_EQ(remap, before, "remap matches original");
  /* check that region is still nested */
  ...
  T_ASSERT_EQ(info.protection, VM_PROT_READ, "new cur_prot read-only");
  T_QUIET;
  T_ASSERT_EQ(info.max_protection, VM_PROT_READ, "new max_prot read-only");
  /* cleanup */
  kr = mach_vm_deallocate(mach_task_self(), remap_address, remap_size);
  T_QUIET;
  T_EXPECT_MACH_SUCCESS(kr, "vm_deallocate()");
  ...
  if (!(cur_prot & VM_PROT_WRITE)) {
    T_LOG("vm_remap(): 0x%llx not writable %s/%s", remap_address,
          prot_str[cur_prot], prot_str[max_prot]);
  ...
  *(uint32_t *)(uintptr_t)remap_address = before;
  ...
  T_ASSERT_GT(depth, 0, "still nested");
  T_QUIET;
  T_ASSERT_EQ(info.protection, VM_PROT_DEFAULT, "cur_prot still writable");
  T_QUIET;
  T_ASSERT_EQ((info.max_protection & VM_PROT_WRITE), VM_PROT_WRITE,
              "max_prot still writable");
  /* cleanup */
  kr = mach_vm_deallocate(mach_task_self(), remap_address, remap_size);
  T_QUIET;
  T_EXPECT_MACH_SUCCESS(kr, "vm_deallocate()");
  ...
  T_ASSERT_MACH_SUCCESS(kr, "vm_remap_new()");
  if (!(cur_prot & VM_PROT_WRITE)) {
    T_LOG("vm_remap_new(): 0x%llx not writable %s/%s", remap_address,
          prot_str[cur_prot], prot_str[max_prot]);
...
skip_vm_remap_new_rw:
  ...
  T_ASSERT_MACH_SUCCESS(kr, "mach_make_memory_entry_64(VM_SHARE)");
  T_QUIET;
  T_ASSERT_EQ(remap_size, size,
              "mem_entry(VM_SHARE) should cover whole mapping");
  //	T_LOG("AFTER MAKE_MEM_ENTRY(VM_SHARE) 0x%llx...", address);
  //fflush(stdout); fflush(stderr); getchar();
  remap_address = 0;
  kr = mach_vm_map(mach_task_self(), &remap_address, remap_size, 0, /* mask */
                   VM_FLAGS_ANYWHERE, mem_entry, 0,                 /* offset */
                   FALSE,                                           /* copy */
  ...
  T_QUIET;
  T_ASSERT_EQ(remap, before, "remap matches original");
  //	T_LOG("AFTER VM_MAP 0x%llx...", remap_address); fflush(stdout);
  //fflush(stderr); getchar();
  *(uint32_t *)(uintptr_t)remap_address = before + 1;
  //	T_LOG("AFTER WRITE 0x%llx...", remap_address); fflush(stdout);
  //fflush(stderr); getchar();
  after = *(uint32_t *)(uintptr_t)address;
  T_LOG("mem_entry(VM_SHARE): 0x%llx 0x%x -> 0x%x", address, before, after);
  *(uint32_t *)(uintptr_t)remap_address = before;
...
skip_mem_entry_vm_share_rw:
  ...
  *(uint32_t *)(uintptr_t)remap_address = before;
  ...
  //	T_QUIET; T_ASSERT_EQ(tmp_size, size, "size hasn't changed");
  T_QUIET;
  T_ASSERT_EQ(depth, 0, "no longer nested");
  T_QUIET;
  T_ASSERT_EQ(info.protection, VM_PROT_DEFAULT, "cur_prot still writable");
  T_QUIET;
  T_ASSERT_EQ((info.max_protection & VM_PROT_WRITE), VM_PROT_WRITE,
...
T_DECL(copyoverwrite_submap_protection, "test copywrite vm region submap \
  ...
  for (vmaddr = SHARED_REGION_BASE;
       vmaddr < SHARED_REGION_BASE + SHARED_REGION_SIZE; vmaddr += vmsize) {
    depth = 99;
    region_info_count = VM_REGION_SUBMAP_SHORT_INFO_COUNT_64;
    kr = mach_vm_region_recurse(mach_task_self(), &vmaddr, &vmsize, &depth,
    ...
    T_ASSERT_EQ(region_info_count, VM_REGION_SUBMAP_SHORT_INFO_COUNT_64,
                "vm_region_recurse(0x%llx) count = %d expected %d", vmaddr,
                region_info_count, VM_REGION_SUBMAP_SHORT_INFO_COUNT_64);

    T_LOG("--> region: vmaddr 0x%llx depth %d prot 0x%x/0x%x", vmaddr, depth,
    ...
    kr = mach_vm_copy(mach_task_self(), vmaddr, vmsize, vmaddr);
    if (kr == KERN_PROTECTION_FAILURE || kr == KERN_INVALID_ADDRESS) {
      T_PASS("vm_copy(0x%llx,0x%llx) expected prot error 0x%x (%s)", vmaddr,
             vmsize, kr, mach_error_string(kr));
      continue;
    }
    T_ASSERT_MACH_SUCCESS(kr, "vm_copy(0x%llx,0x%llx) prot 0x%x", vmaddr,
                          vmsize, region_info.protection);
    depth = 0;
    region_info_count = VM_REGION_SUBMAP_SHORT_INFO_COUNT_64;
    kr = mach_vm_region_recurse(mach_task_self(), &vmaddr, &vmsize, &depth,
                                (vm_region_info_t)&region_info,
    ...
    T_ASSERT_EQ(region_info_count, VM_REGION_SUBMAP_SHORT_INFO_COUNT_64,
                "vm_region_recurse() count = %d expected %d", region_info_count,
                VM_REGION_SUBMAP_SHORT_INFO_COUNT_64);

    T_ASSERT_EQ(depth, 0, "vm_region_recurse(0x%llx): depth = %d expected 0",
                vmaddr, depth);
    T_ASSERT_EQ((region_info.protection & VM_PROT_EXECUTE), 0,
                "vm_region_recurse(0x%llx): prot 0x%x", vmaddr,
...
T_DECL(wire_text, "test wired text for rdar://problem/16783546 Wiring code in \
  ...
  map_addr = (vm_address_t)(uintptr_t)ptrauth_strip(
      addr, ptrauth_key_function_pointer);
#else  /* __has_feature(ptrauth_calls) */
  map_addr = (vm_address_t)(uintptr_t)addr;
#endif /* __has_feature(ptrauth_calls) */
  remap_addr = 0;
  kr = vm_remap(mach_task_self(), &remap_addr, 4096, 0,               /* mask */
                VM_FLAGS_ANYWHERE, mach_task_self(), map_addr, FALSE, /* copy */
                &curprot, &maxprot, VM_INHERIT_DEFAULT);
  T_ASSERT_EQ(kr, KERN_SUCCESS, "vm_remap error 0x%x (%s)", kr,
              mach_error_string(kr));
  before = *addr;
  ...
  if (retval != 0) {
    saved_errno = errno;
    T_ASSERT_EQ(saved_errno, EPERM,
                "wire shared text error %d (%s), expected: %d", saved_errno,
                strerror(saved_errno), EPERM);
  } else if (after != before) {
    T_ASSERT_FAIL("shared text changed by wiring at %p 0x%x -> 0x%x",
                  (void *)addr, before, after);
  } else {
  ...
  if (retval != 0) {
    ...
    T_ASSERT_FAIL("text changed by wiring at %p 0x%x -> 0x%x", (void *)addr,
                  before, after);
  } else {
    T_PASS("wire text");
  }
...
T_DECL(remap_comm_page, "test remapping of the commpage - rdar://93177124",
  ...
  remap_addr = 0;
  kr = mach_vm_remap(mach_task_self(), &remap_addr, vmsize, 0, /* mask */
                     VM_FLAGS_ANYWHERE, mach_task_self(), commpage_addr,
                     TRUE, /* copy */
                     &curprot, &maxprot, VM_INHERIT_DEFAULT);
  if (kr == KERN_INVALID_ADDRESS) {
  ...
  T_ASSERT_MACH_SUCCESS(kr, "vm_remap() of commpage from 0x%llx",
                        commpage_addr);
}

/* rdar://132439059 */
T_DECL(mach_vm_remap_new_task_read_port,
       "Ensure shared, writable mappings cannot be created with a process's "
       "task read port using mach_vm_remap_new",
       T_META_TAG_VM_PREFERRED, T_META_RUN_CONCURRENTLY(true)) {
  ...
  kr = mach_vm_allocate(mach_task_self(), &private_data, size,
                        VM_FLAGS_ANYWHERE);
  T_QUIET;
  ...
  ret = pipe(fds);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "pipe");
  ...
  if (pid == 0) {
    ...
    ret = close(fds[1]);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "child: close write end");

    /* Check that the permissions are VM_PROT_DEFAULT/VM_PROT_ALL */
    kr = mach_vm_region_recurse(mach_task_self(), &private_data, &size, &depth,
                                (vm_region_recurse_info_t)&info, &count);
    T_ASSERT_MACH_SUCCESS(kr, "child: mach_vm_region_recurse");
    T_EXPECT_EQ_INT(info.protection, VM_PROT_DEFAULT,
                    "child: current protection is VM_PROT_DEFAULT");
    T_EXPECT_EQ_INT(info.max_protection, VM_PROT_ALL,
                    "child: maximum protextion is VM_PROT_ALL");

    /* The child tries to read data from the pipe (that never comes) */
    nbytes_read = read(fds[0], data, 2);
    T_QUIET;
    T_EXPECT_EQ_LONG(nbytes_read, 0L, "child: read 0 bytes");
    ...
    ret = close(fds[0]);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "close read end");
    ...
    kr = mach_vm_remap_new(mach_task_self(), &remap_addr, size, 0, /* mask */
                           VM_FLAGS_ANYWHERE, read_port, private_data,
                           FALSE, /* copy */
                           &cur_prot, &max_prot, VM_INHERIT_DEFAULT);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "parent: mach_vm_remap_new");

    /* Check that permissions of the remapped region are VM_PROT_NONE */
    kr = mach_vm_region_recurse(mach_task_self(), &remap_addr, &size, &depth,
                                (vm_region_recurse_info_t)&info, &count);
    T_ASSERT_MACH_SUCCESS(kr, "parent: mach_vm_region_recurse");
    T_EXPECT_EQ_INT(info.protection, VM_PROT_NONE,
                    "parent: current protection is VM_PROT_NONE");
    T_EXPECT_EQ_INT(info.max_protection, VM_PROT_NONE,
                    "parent: maximum protextion is VM_PROT_NONE");

    /* Tell the child it is done and can exit. */
    ret = close(fds[1]);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "parent: close write end");
...
static void zsm_vm_map(size_t size, int flags, mem_entry_name_port_t port,
                       memory_object_offset_t offset, boolean_t copy,
                       vm_prot_t cur_protection, vm_prot_t max_protection,
                       vm_inherit_t inheritance, mach_vm_address_t *out_addr,
                       size_t *out_size) {
  ...
  kern_return_t kr =
      mach_vm_map(mach_task_self(), &addr_info, size, 0, flags, port, offset,
...
zsm_vm_mach_make_memory_entry(mach_vm_address_t addr, size_t size, int flags,
  ...
  kern_return_t kr = mach_make_memory_entry(mach_task_self(), &size, addr,
                                            flags, &port, parent);
...
T_DECL(memory_entry_zero_sized, "Test that creating a zero-sized memory-entry "
                                "with parent fails correctly") {
  mach_vm_address_t addr = 0;
  size_t size = 0;
  kern_return_t kr;
  zsm_vm_map(0xa7c000, 0,                           /* flags */
             0,                                     /* port */
             0,                                     /* offset */
             0,                                     /* copy */
             VM_PROT_EXECUTE, VM_PROT_EXECUTE, 0x1, /* inheritance */
             &addr, &size);
  mem_entry_name_port_t parent_entry =
      zsm_vm_mach_make_memory_entry(addr, size, 0, 0, KERN_SUCCESS, false);

  zsm_vm_mach_make_memory_entry(0, 0, 0, parent_entry, KERN_INVALID_ARGUMENT,
                                true);
  ...
  zsm_vm_mach_make_memory_entry(PAGE_SIZE, 0, 0, parent_entry,
                                KERN_INVALID_ARGUMENT, true);
  zsm_vm_mach_make_memory_entry(PAGE_SIZE, 1, 0, parent_entry, KERN_SUCCESS,
                                true);
  zsm_vm_mach_make_memory_entry(PAGE_SIZE + 1, 0, 0, parent_entry, KERN_SUCCESS,
                                true);
...
T_DECL(memory_entry_null_obj,
  ...
  zsm_vm_map(0x604000,                        /* size */
             ...
             0,                               /* offset */
             TRUE,                            /* copy */
             VM_PROT_NONE, VM_PROT_NONE, 0x0, /* inheritance */
             &addr, &size);
  ...
  zsm_vm_mach_make_memory_entry(addr, /*size=*/0, /*flags=*/0x0, /*parent=*/0x0,
                                KERN_INVALID_ARGUMENT, true);

  // trying to get a 'copy' entry of a PROT_NONE entry
  zsm_vm_mach_make_memory_entry(addr, size, /*flags=*/0x0, /*parent=*/0x0,
                                KERN_PROTECTION_FAILURE, true);

  // get a 'share' entry of a PROT_NONE entry and remap it
  mem_entry_name_port_t np = zsm_vm_mach_make_memory_entry(
      addr, size, MAP_MEM_VM_SHARE, 0x0, KERN_SUCCESS, false);

  mach_vm_address_t m_addr = 0;
  size_t m_size = 0;
  zsm_vm_map(size, 0,                         /* size */
             np, 0,                           /* offset */
             FALSE,                           /* copy */
             VM_PROT_NONE, VM_PROT_NONE, 0x0, /* inheritance */
             &m_addr, &m_size);
...
T_DECL(vm_region_recurse_tpro_info,
       "Ensure metadata returned by vm_region_recurse correct reflects TPRO "
       "status",
       T_META_ENABLED(TARGET_CPU_ARM64E), XNU_T_META_SOC_SPECIFIC,
       T_META_ASROOT(true)) {
  T_SETUPBEGIN;
  ...
  kern_return_t kr = mach_vm_allocate(mach_task_self(), &non_tpro_allocation,
                                      alloc_size, VM_FLAGS_ANYWHERE);
  ...
  kr = vm_region_recurse_64(mach_task_self(), (vm_address_t *)&addr,
                            (vm_size_t *)&addr_size, &nesting_depth,
  ...
  T_ASSERT_TRUE(region_info.flags & VM_REGION_FLAG_TPRO_ENABLED,
                "Expected metadata to reflect a TPRO entry");
  ...
  kr = mach_vm_region_recurse(mach_task_self(), (mach_vm_address_t *)&addr,
                              (mach_vm_size_t *)&addr_size, &nesting_depth,
                              (vm_region_info_t)&short_info, &count);

  /* Then the short metadata also confirms that the region contains a TPRO entry
   */
  T_ASSERT_MACH_SUCCESS(kr, "Query TPRO-enabled region");
  T_ASSERT_TRUE(short_info.flags & VM_REGION_FLAG_TPRO_ENABLED,
                "Expected metadata to reflect a TPRO entry");

  /* And when we query the attributes of the region covering the non-TPRO
   * allocation */
  ...
  T_ASSERT_FALSE(region_info.flags & VM_REGION_FLAG_TPRO_ENABLED,
                 "Expected metadata to reflect no TPRO entry");
  ...
  /* Then the short metadata also confirms that the region does not contain a
   * TPRO entry */
  T_ASSERT_MACH_SUCCESS(kr, "Query non-TPRO region");
  T_ASSERT_FALSE(short_info.flags & VM_REGION_FLAG_TPRO_ENABLED,
                 "Expected metadata to reflect no TPRO entry");
...
T_DECL(
  ...
  void *jit_region = mmap(NULL, alloc_size, PROT_READ | PROT_WRITE | PROT_EXEC,
                          MAP_ANON | MAP_PRIVATE | MAP_JIT, -1, 0);
  T_ASSERT_NE_PTR(jit_region, MAP_FAILED, "MAP_JIT");

  /* And a non-JIT region */
  mach_vm_address_t non_jit_allocation;
  kern_return_t kr = mach_vm_allocate(mach_task_self(), &non_jit_allocation,
                                      alloc_size, VM_FLAGS_ANYWHERE);
  ...
  T_ASSERT_TRUE(region_info.flags & VM_REGION_FLAG_JIT_ENABLED,
                "Expected metadata to reflect a JIT entry");
   ...
   */
  T_ASSERT_MACH_SUCCESS(kr, "Query JIT-enabled region");
  T_ASSERT_TRUE(short_info.flags & VM_REGION_FLAG_JIT_ENABLED,
                "Expected metadata to reflect a JIT entry");

  /* And when we query the attributes of the region covering the non-JIT
   * allocation */
  ...
  T_ASSERT_FALSE(region_info.flags & VM_REGION_FLAG_JIT_ENABLED,
                 "Expected metadata to reflect no JIT entry");
   ...
   * JIT entry */
  T_ASSERT_MACH_SUCCESS(kr, "Query non-JIT region");
  T_ASSERT_FALSE(short_info.flags & VM_REGION_FLAG_JIT_ENABLED,
                 "Expected metadata to reflect no JIT entry");
