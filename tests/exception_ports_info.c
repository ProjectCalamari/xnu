...
T_DECL(exception_ports_info, "Test {task, thread}_get_exception_ports_info") {
  ...
  unsigned int kotype = 0, kobject = 0, exc_port3_kotype = 0,
               exc_port3_kaddr = 0;
  ...
  kr =
      mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, &exc_port1);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "Allocated mach exception port");
  kr =
      mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, &exc_port2);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "Allocated mach exception port");
  kr =
      mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, &exc_port3);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "Allocated mach exception port");
   ...
   */
  kr = mach_port_insert_right(mach_task_self(), exc_port1, exc_port1,
                              MACH_MSG_TYPE_MAKE_SEND);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "Inserted a SEND right into the exception port");
  kr = mach_port_insert_right(mach_task_self(), exc_port2, exc_port2,
                              MACH_MSG_TYPE_MAKE_SEND);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "Inserted a SEND right into the exception port");
  kr = mach_port_insert_right(mach_task_self(), exc_port3, exc_port3,
                              MACH_MSG_TYPE_MAKE_SEND);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "Inserted a SEND right into the exception port");
  ...
  kr = task_set_exception_ports(
      mach_task_self(), EXC_MASK_GUARD, exc_port1,
  ...
  kr = task_set_exception_ports(
      mach_task_self(), EXC_MASK_RPC_ALERT, /* why can't be EXC_CRASH or
  ...
  kr = task_set_exception_ports(
      mach_task_self(),
      EXC_MASK_RESOURCE | EXC_MASK_BREAKPOINT | EXC_MASK_SYSCALL, exc_port3,
      (exception_behavior_t)(EXCEPTION_STATE_IDENTITY | MACH_EXCEPTION_CODES),
      THREAD_STATE_NONE);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "Set the exception port to my custom handler");
  ...
  kr = thread_get_exception_ports(mach_thread_self(), EXC_MASK_ALL, masks2,
                                  &count2, ports, behaviors2, flavors2);
  T_EXPECT_MACH_SUCCESS(kr, "thread_get_exception_ports(): 0x%x", kr);
  T_EXPECT_EQ(count2, 0, "should have 0 exception ports");

  kr = thread_get_exception_ports_info(mach_thread_self(), EXC_MASK_ALL, masks,
                                       &count, ports_info, behaviors, flavors);
  ...
  kr = task_get_exception_ports_info(mach_task_self(), EXC_MASK_ALL, masks,
                                     &count, ports_info, behaviors, flavors);
  T_EXPECT_MACH_SUCCESS(kr, "task_get_exception_ports_info(): 0x%x", kr);
  T_EXPECT_EQ(count, 4, "should have 4 masks"); /* Returns 3 if one exc_port
                                                   registers for EXC_CRASH */

  /* get exception ports */
  kr = task_get_exception_ports(mach_task_self(), EXC_MASK_ALL, masks2, &count2,
                                ports, behaviors2, flavors2);
  ...
  kr = mach_port_kernel_object(mach_task_self(), mach_task_self(), &kotype,
                               &kobject);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_port_kernel_object(): 0x%x", kr);
  T_LOG("task_self kobject: 0x%x", kobject);

  T_QUIET;
  T_EXPECT_MACH_SUCCESS(mach_port_space_info(mach_task_self(), &info_space,
                                             &table, &tblcnt, &tree, &treecnt),
  ...
  for (int i = 0; i < count; i++) {
    T_LOG("ports_info[%d].iip_port_object: 0x%x", i,
          ports_info[i].iip_port_object);

    if (ports_info[i].iip_port_object == exc_port1_kaddr) {
      T_EXPECT_NE(ports_info[i].iip_port_object, 0,
                  "on debug/kernel, port object should be non-zero: 0x%x",
                  ports_info[i].iip_port_object);
      T_EXPECT_EQ(ports_info[i].iip_receiver_object, kobject,
                  "receiver object should match task self kobject: 0x%x",
                  ports_info[i].iip_receiver_object);
      T_EXPECT_EQ(masks[i], EXC_MASK_GUARD,
    ...
    if (ports_info[i].iip_port_object == exc_port2_kaddr) {
      ...
      T_EXPECT_EQ(ports_info[i].iip_receiver_object, kobject,
                  "receiver object should match task self kobject: 0x%x",
                  ports_info[i].iip_receiver_object);
      T_EXPECT_EQ(masks[i], EXC_MASK_RPC_ALERT,
