...
static void parse_args(struct args *args) {
  args->verbose = 0;
  args->voucher = 0;
  ...
  args->num_msgs = 1;
  args->request_msg_size =
      sizeof(ipc_complex_message) - sizeof(mach_msg_trailer_t);
  args->reply_msg_size =
      sizeof(ipc_complex_message) - sizeof(mach_msg_trailer_t);
...
static int connect_to_server(void) {
  ...
  ret = mach_port_insert_right(mach_task_self(), reply_port, reply_port,
  ...
  mach_msg_return_t mret =
      mach_msg(request, option, (mach_msg_size_t)client_args.request_msg_size,
               sizeof(ipc_complex_message), reply_port, MACH_MSG_TIMEOUT_NONE,
               MACH_PORT_NULL);
...
static inline mach_port_type_t get_port_type(mach_port_t mp) {
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 4) {
    ...
    printf("Usage: ./port_table_limits_client <soft limit> <hard limit> "
           "<test_num>\n");
    goto fail_and_exit;
  }
  ...
  if (test_num == 2) {
    printf("client: Wait for a reply message from server before continuing "
           "port allocation\n");
