...
static int connect_to_server(void) {
  ...
  ret = task_set_special_port(mach_task_self(), TASK_RESOURCE_NOTIFY_PORT,
                              client_args.server_port);
  if (ret) {
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 4) {
    ...
    printf("Usage: ./kqworkloop_limits_client <soft limit> <hard limit> "
           "<test_num>\n");
    goto fail_and_exit;
  }
  ...
  while (ret == 0) {
    ...
    for (int j = 0; j < 10; j++) {
      ...
      dispatch_queue_t dq =
          dispatch_queue_create("leaked queue", DISPATCH_QUEUE_SERIAL);
      dispatch_source_t ds =
          dispatch_source_create(DISPATCH_SOURCE_TYPE_MACH_RECV, port, 0, dq);
