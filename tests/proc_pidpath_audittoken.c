...
static int idversion_for_pid(pid_t pid) {
  struct proc_uniqidentifierinfo uniqidinfo = {0};

  int ret = proc_pidinfo(pid, PROC_PIDUNIQIDENTIFIERINFO, 0, &uniqidinfo,
                         sizeof(uniqidinfo));
  if (ret <= 0) {
    perror("proc_pidinfo(PROC_PIDUNIQIDENTIFIERINFO)");
    T_ASSERT_FAIL("proc_pidinfo(%d, PROC_PIDUNIQIDENTIFIERINFO) failed "
  ...
  printf("%s>pid = %d, p_uniqueid = %lld\n", __FUNCTION__, pid,
         uniqidinfo.p_uniqueid);
  printf("%s>pid = %d, p_idversion = %d\n", __FUNCTION__, pid,
         uniqidinfo.p_idversion);
#endif
...
static void show_pidpaths(void) {
  ...
  for (pid_t pid = 1; ((pid < 1000) && (count <= 25)); pid++) {
    ...
    if (ret <= 0) {
      ...
      T_ASSERT_FAIL("proc_pidpath(%d) failed unexpectedly with errno %d", pid,
                    errno);
    }
    count++;
    ...
    if (ret <= 0) {
      ...
      T_ASSERT_FAIL(
          "proc_pidpath_audittoken(%d) failed unexpectedly with errno %d", pid,
          errno);
    }
    T_PASS("%5d %s\n", pid, buffer);
    ...
    ret = proc_pidpath_audittoken(&token, buffer, sizeof(buffer));
    T_ASSERT_LE(ret, 0,
                "proc_pidpath_audittoken() failed as expected due to incorrect "
                "idversion");
    T_ASSERT_EQ(errno, ESRCH, "errno is ESRCH as expected");
  }
}

T_DECL(proc_pidpath_audittoken, "Test proc_pidpath_audittoken()",
       T_META_ASROOT(false)) {
  show_pidpaths();
