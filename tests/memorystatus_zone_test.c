...
/* 60% is too high on bridgeOS to achieve without vm-pageshortage jetsams. Set
 * it to 40%. */
#if TARGET_OS_BRIDGE
...
static pthread_mutex_t test_mtx =
    PTHREAD_MUTEX_INITIALIZER; /* protects the next 3 things */
...
extern kern_return_t mach_zone_info_for_largest_zone(host_priv_t host,
                                                     mach_zone_name_t *name,
                                                     mach_zone_info_t *info);

static bool check_time(time_t start, int timeout) {
...
static void allocate_vm_stuff(int flags) {
  ...
  for (i = 0;; i++) {
    ...
    if ((mach_vm_allocate(mach_task_self(), &addr, (mach_vm_size_t)alloc_size,
                          vmflags)) != KERN_SUCCESS) {
      break;
    }
  ...
  for (uint64_t j = 0; j < NUM_GIVE_BACK && j < i; ++j) {
    mach_vm_deallocate(mach_task_self(), give_back[j],
  ...
  while (1) {
    usleep(500 * 1000);
    /* Exit if parent has exited. Ensures child processes don't linger around
...
static void allocate_from_generic_zone(void) {
  ...
  int kr =
      sysctlbyname("machdep.max_port_table_size", &old_limit, &size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kr, "sysctl kern.max_port_table_size failed");
  ...
  for (uint64_t j = 0; j < NUM_GIVE_BACK_PORTS && j < i; ++j) {
    int ret;
    ret = mach_port_mod_refs(mach_task_self(), give_back[j],
...
static void print_zone_info(mach_zone_name_t *zn, mach_zone_info_t *zi) {
  T_LOG("ZONE NAME: %-35sSIZE: %-25lluELEMENTS: %llu", zn->mzn_name,
        zi->mzi_cur_size, zi->mzi_count);
}
...
static void query_zone_info(void) {
  ...
  for (i = 0; i < current_test.num_zones; i++) {
    kr = mach_zone_info_for_zone(mach_host_self(), current_test.zone_names[i],
                                 &(zone_info_array[i]));
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "mach_zone_info_for_zone(%s) returned %d [%s]",
                          current_test.zone_names[i].mzn_name, kr,
                          mach_error_string(kr));
  }
  kr = mach_zone_info_for_largest_zone(mach_host_self(), &largest_zone_name,
                                       &largest_zone_info);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_zone_info_for_largest_zone returned %d [%s]",
...
static bool vme_zone_compares_to_vm_objects(void) {
  ...
  T_LOG("Comparing element counts of \"VM map entries\" and \"vm objects\" "
        "zones");
  for (i = 0; i < current_test.num_zones; i++) {
  ...
  T_LOG("# VM map entries as percentage of # vm objects = %llu",
        (vm_map_entry_element_count * 100) / vm_object_element_count);
  if (vm_map_entry_element_count >=
      ((vm_object_element_count * VMENTRY_TO_VMOBJECT_COMPARISON_RATIO) /
       100)) {
...
static bool verify_generic_jetsam_criteria(void) {
  ...
  if (strcmp(largest_zone_name.mzn_name, VME_ZONE)) {
    /* If vm objects is the largest zone and the VM map entries zone had
     * comparable # of elements, return false */
    if (!strcmp(largest_zone_name.mzn_name, VMOBJECTS_ZONE) &&
...
static void begin_test_teardown(void) {
  ...
  /*
   * Restore kern.zone_map_jetsam_limit to the default high value, to prevent
   * further jetsams. We should change the value of old_limit if
   * ZONE_MAP_JETSAM_LIMIT_DEFAULT changes in the kernel. We don't have a way to
   * capture what the original value was before the test, because the
   * T_META_SYSCTL_INT macro will have changed the value before the test starts
   * running.
   */
  ret = sysctlbyname("kern.zone_map_jetsam_limit", NULL, NULL, &old_limit,
                     sizeof(old_limit));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "sysctl kern.zone_map_jetsam_limit failed");
...
static void cleanup_and_end_test(void) {
  ...
  dispatch_async(dq_spawn, ^{
    /*
     * If the test succeeds, we will call dispatch_source_cancel twice, which is
     * fine since the operation is idempotent. Just make sure to not drop all
     * references to the dispatch sources (in this case we're not, we have
     * globals holding references to them), or we can end up with
...
static void setup_ktrace_session(void) {
  ...
  ktrace_set_dropped_events_handler(session, ^{
    T_FAIL("Dropped ktrace events; might have missed an expected jetsam event. "
           "Terminating early.");
  });
  ...
  ret = ktrace_events_single(
      ...
      ^(ktrace_event_t event) {
        ...
        /*
         * libktrace does not support DBG_FUNC_START/END in the event filter. It
         * simply ignores it. So we need to explicitly check for the end event
         * (a successful jetsam kill) here, instead of passing in
        ...
        if (event->arg2 == kMemorystatusKilledZoneMapExhaustion) {
          begin_test_teardown();
          T_LOG("[memorystatus_do_kill] jetsam reason: zone-map-exhaustion, "
                "pid: %d\n\n",
                (int)event->arg1);
          if (current_test.test_index == VME_ZONE_TEST ||
              current_test.test_index == VM_OBJECTS_ZONE_TEST) {
            /*
             * For the VM map entries zone we try to kill the leaking process.
             * Verify that we jetsammed one of the processes we spawned.
             *
             * For the vm objects zone we pick the leaking process via the VM
             * map entries zone, if the number of vm objects and VM map entries
             * are comparable. The test simulates this scenario, we should see a
             * targeted jetsam for the vm objects zone too.
             */
            pthread_mutex_lock(&test_mtx);
            for (i = 0; i < num_children; i++) {
            ...
            pthread_mutex_unlock(&test_mtx);
            /*
             * If we didn't see a targeted jetsam, verify that the largest zone
             * actually fulfilled the criteria for generic jetsams.
             */
            if (!received_jetsam_event && verify_generic_jetsam_criteria()) {
              received_jetsam_event = true;
              T_LOG("Did not receive jetsam event for a child, but generic "
                    "jetsam criteria holds");
            }
          } else {
            received_jetsam_event = true;
          ...
          T_QUIET;
          T_ASSERT_TRUE(received_jetsam_event, "Jetsam event not as expected");
        } else {
          /*
           * The test relies on the children being able to send a signal to the
           * parent, to continue spawning new processes that leak more zone
           * memory. If a child is jetsammed for some other reason, the parent
           * can get stuck waiting for a signal from the child, never being able
           * to make progress (We spawn only a single process at a time to
           * rate-limit the zone memory bloat.). If this happens, the test
           * eventually times out. So if a child is jetsammed for some reason
           * other than zone-map-exhaustion, end the test early.
           *
           * This typically happens when we end up triggering vm-pageshortage
           * jetsams before zone-map-exhaustion jetsams. Lowering the
           * zone_map_jetsam_limit if the zone map size was initially low should
           * help with this too. See
           * sysctlbyname("kern.zone_map_jetsam_limit"...) in run_test() below.
           */
          pthread_mutex_lock(&test_mtx);
          for (i = 0; i < num_children; i++) {
            if (child_pids[i] == (pid_t)event->arg1) {
              begin_test_teardown();
              T_PASS("Child pid %d was jetsammed due to reason %d. Terminating "
                     "early.",
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "ktrace_start");
}

static void query_zone_map_size(uint64_t *current, uint64_t *total) {
  int ret;
  uint64_t zstats[2];
  size_t zstats_size = sizeof(zstats);

  ret = sysctlbyname("kern.zone_map_size_and_capacity", &zstats, &zstats_size,
                     NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "sysctl kern.zone_map_size_and_capacity failed");

  T_LOG("Zone map capacity: %-30lldZone map size: %lld [%lld%% full]",
        zstats[1], zstats[0], (zstats[0] * 100) / zstats[1]);

#if (TARGET_OS_IPHONE && !TARGET_OS_SIMULATOR)
  int memstat_level;
  size_t memstat_level_size = sizeof(memstat_level);
  ret = sysctlbyname("kern.memorystatus_level", &memstat_level,
                     &memstat_level_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "sysctl kern.memorystatus_level failed");
...
static void spawn_child_process(void) {
  ...
  if (!test_ending) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(pid, "dt_launch_tool");
...
static void run_test(void) {
  ...
  if (new_limit < old_limit) {
    /*
     * We should be fine messing with the zone_map_jetsam_limit here, i.e.
     * outside of T_META_SYSCTL_INT. When the test ends, T_META_SYSCTL_INT will
     * restore the zone_map_jetsam_limit to what it was before the test anyway.
     */
    ret = sysctlbyname("kern.zone_map_jetsam_limit", NULL, NULL, &new_limit,
                       sizeof(new_limit));
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "sysctl kern.zone_map_jetsam_limit failed");
  ...
  zone_info_array = (mach_zone_info_array_t)calloc(
      (unsigned long)current_test.num_zones, sizeof *zone_info_array);

  /*
   * If the timeout specified by T_META_TIMEOUT is hit, the atend handler does
   * not get called. So we're queueing a dispatch block to fire after
   * TIMEOUT_SECS seconds, so we can exit cleanly.
   */
  ...
  /*
   * Create a dispatch source for the signal SIGUSR1. When a child is done
   * allocating zone memory, it sends SIGUSR1 to the parent. Only then does the
   * parent spawn another child. This prevents us from spawning many children at
   * once and creating a lot of memory pressure.
   */
  signal(SIGUSR1, SIG_IGN);
  dq_spawn = dispatch_queue_create("spawn_queue", DISPATCH_QUEUE_SERIAL);
  ds_signal =
      dispatch_source_create(DISPATCH_SOURCE_TYPE_SIGNAL, SIGUSR1, 0, dq_spawn);
  T_QUIET;
  T_ASSERT_NOTNULL(ds_signal, "dispatch_source_create: signal");
  ...
  /* Timer to query jetsam-relevant zone info every second. Print it every 5
   * seconds. */
  ds_timer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0,
                                    dispatch_queue_create("timer_queue", NULL));
  T_QUIET;
  T_ASSERT_NOTNULL(ds_timer, "dispatch_source_create: timer");
  dispatch_source_set_timer(ds_timer,
...
static void move_to_idle_band(void) {
  ...
  /*
   * We want to move the processes we spawn into the idle band, so that jetsam
   * can target them first. This prevents other important BATS tasks from
   * getting killed, specially in LTE where we have very few processes running.
   *
   * This is only needed for tests which (are likely to) lead us down the
   * generic jetsam path.
   */
  props.priority = JETSAM_PRIORITY_IDLE;
  props.user_data = 0;

  if (memorystatus_control(MEMORYSTATUS_CMD_SET_PRIORITY_PROPERTIES, getpid(),
...
/*
 * T_META_SYSCTL_INT(ZONEMAP_JETSAM_LIMIT_SYSCTL) changes the
 * zone_map_jetsam_limit to a lower value, so that the test can complete faster.
 * The test allocates zone memory pretty aggressively which can cause the system
 * to panic if the jetsam limit is quite high; a lower value keeps us from
 * panicking.
 */
T_DECL(memorystatus_vme_zone_test,
       "allocates elements from the VM map entries zone, verifies "
       "zone-map-exhaustion jetsams",
       T_META_ASROOT(true), T_META_TIMEOUT(1800),
  ...
  current_test = (test_config_struct){
      ...
      .num_zones = 1,
      .zone_names = (mach_zone_name_t[]){{.mzn_name = VME_ZONE}}};
  run_test();
}

T_DECL(memorystatus_vm_objects_zone_test,
       "allocates elements from the VM objects and the VM map entries zones, "
       "verifies zone-map-exhaustion jetsams",
       T_META_ASROOT(true), T_META_TIMEOUT(1800),
       /*		T_META_LTEPHASE(LTE_POSTINIT),
        */
       T_META_REQUIRES_SYSCTL_NE("kern.kasan.available", 1),
...
T_DECL(memorystatus_generic_zone_test,
       "allocates elements from a zone that doesn't have an optimized jetsam "
       "path, verifies zone-map-exhaustion jetsams",
       T_META_ASROOT(true), T_META_TIMEOUT(1800),
       /*		T_META_LTEPHASE(LTE_POSTINIT),
        */
       T_META_REQUIRES_SYSCTL_NE("kern.kasan.available", 1),
       ...
       T_META_SYSCTL_INT(ZONEMAP_JETSAM_LIMIT_SYSCTL),
       T_META_TAG_VM_PREFERRED) {
  current_test = (test_config_struct){.test_index = GENERIC_ZONE_TEST,
