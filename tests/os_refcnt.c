...
__abortlike void handle_panic(const char *func, char *str, ...);
#define panic(...) handle_panic(__func__, __VA_ARGS__)

#define ZPERCPU_STRIDE 128

static inline int zpercpu_count(void) {
  static int n;
  if (__improbable(n == 0)) {
    n = dt_ncpu();
  }
...
static inline void thread_wakeup(void *event) { abort(); }

#define zalloc_percpu(zone, flags)                                             \
  (uint64_t _Atomic *)calloc((size_t)zpercpu_count(), ZPERCPU_STRIDE)

#define zfree_percpu(zone, ptr) free(ptr)

static inline uint64_t _Atomic *zpercpu_get_cpu(uint64_t _Atomic *ptr,
                                                int cpu) {
...
T_GLOBAL_META(T_META_NAMESPACE("os_refcnt"), T_META_CHECK_LEAKS(false));
...
T_DECL(os_refcnt, "Basic atomic refcount") {
  ...
  (void)os_ref_release(&rc);
  T_QUIET;
  T_ASSERT_EQ_UINT(os_ref_get_count(&rc), 0, "release");
...
T_DECL(os_pcpu_refcnt, "Basic atomic refcount") {
  ...
  T_ASSERT_EQ_UINT(os_pcpu_ref_kill(rc, NULL), 2, "kill decreased count");
  T_ASSERT_EQ_UINT(os_pcpu_ref_count(rc), 2, "kill decreased count");
...
T_DECL(refcnt_raw, "Raw refcount") {
  ...
  T_ASSERT_EQ_UINT(os_ref_get_count_raw(&rc), 1,
                   "refcount correctly initialized");
  ...
  (void)os_ref_release_raw(&rc, NULL);
  T_QUIET;
  T_ASSERT_EQ_UINT(os_ref_get_count_raw(&rc), 0, "release");
...
static void do_bitwise_test(const os_ref_count_t bits) {
  os_ref_atomic_t rc;
  ...
  T_ASSERT_EQ_UINT(os_ref_get_count_mask(&rc, bits), 1,
                   "[%u bits] refcount initialized", bits);

  os_ref_retain_mask(&rc, bits, NULL);
  os_ref_retain_mask(&rc, bits, NULL);
  T_ASSERT_EQ_UINT(os_ref_get_count_mask(&rc, bits), 3,
                   "retain increased count");
  ...
  T_ASSERT_EQ_UINT(rc & ((1U << bits) - 1), reserved,
                   "Reserved bits not modified");

  os_ref_init_count_mask(&rc, bits, NULL, 1, reserved);
  T_ASSERT_TRUE(os_ref_retain_try_mask(&rc, bits, 0, NULL), "try retained");
  if (reserved) {
    T_ASSERT_FALSE(os_ref_retain_try_mask(&rc, bits, reserved, NULL),
                   "try reject");
  }
  ...
  (void)os_ref_release_mask(&rc, bits, NULL);
  T_QUIET;
  T_ASSERT_EQ_UINT(os_ref_get_count_mask(&rc, bits), 0, "release");

  T_ASSERT_FALSE(os_ref_retain_try_mask(&rc, bits, 0, NULL), "try fail");

  T_ASSERT_EQ_UINT(os_ref_get_bits_mask(&rc, bits), reserved,
                   "Reserved bits not modified");
}
...
void handle_panic(const char *func, char *__unused str, ...) {
  int ret = -1;
  if (!expect_panic) {
...
T_DECL(refcnt_resurrection, "Resurrection") {
  ...
  T_QUIET;
  T_EXPECT_EQ_UINT(n, 0, "reference not released");
...
static void worker_ref(os_ref_atomic_t *rc, unsigned long *count) {
  unsigned long n = 0;
...
static void worker_pcpu_ref(os_pcpu_ref_t rc, unsigned long *count) {
  unsigned long n = 0;
...
static void warmup_thread_pool(dispatch_group_t g, dispatch_queue_t rq) {
...
T_DECL(pcpu_perf, "Performance per-cpu") {
  ...
  T_PASS("%.2fM rounds per thread per second (atomic)",
         count / (zpercpu_count() * scale));
  ...
  T_PASS("%.2fM rounds per thread per second (pcpu)",
         count / (zpercpu_count() * scale));
