...
T_GLOBAL_META(T_META_NAMESPACE(TEST_VM_NAMESPACE),
...
static void ledger_init(void) {
  ...
  ret = sysctlbyname("kern.legacy_footprint_entitlement_mode",
                     &legacy_footprint_entitlement_mode, &oldlen, NULL, 0);
  ...
  T_WITH_ERRNO;
  T_ASSERT_EQ(
      ledger(LEDGER_INFO, (caddr_t)(uintptr_t)getpid(), (caddr_t)&li, NULL), 0,
      "ledger(LEDGER_INFO)");
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_GE(ledger(LEDGER_TEMPLATE_INFO, (caddr_t)templateInfo,
                     (caddr_t)&templateCnt, NULL),
              0, "ledger(LEDGER_TEMPLATE_INFO)");
  for (i = 0; i < templateCnt; i++) {
    if (!strncmp(templateInfo[i].lti_name, "phys_footprint",
                 strlen("phys_footprint"))) {
      footprint_index = i;
    } else if (!strncmp(templateInfo[i].lti_name, "page_table",
                        strlen("page_table"))) {
      pagetable_index = i;
    }
  }
...
static void get_ledger_info(uint64_t *phys_footprint, uint64_t *page_table) {
  int64_t count;

  count = ledger_count;
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_GE(ledger(LEDGER_ENTRY_INFO, (caddr_t)(uintptr_t)getpid(),
                     (caddr_t)lei, (caddr_t)&count),
              0, "ledger(LEDGER_ENTRY_INFO)");
  T_QUIET;
...
static mach_vm_address_t pre_warm_with_tag(mach_vm_size_t vm_size, int tag) {
  kern_return_t kr;
  ...
  T_QUIET;
  T_EXPECT_EQ(BigBufOnStack[0], 0xb, "BigBufOnStack[0] == 0x%x",
              BigBufOnStack[0]);
  T_QUIET;
  T_EXPECT_EQ(BigBufOnStack[sizeof(BigBufOnStack) - 1], 0xb,
              "BigBufOnStack[%lu] == 0x%x", sizeof(BigBufOnStack),
  ...
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS, "vm_allocate(%lld) error 0x%x (%s)", vm_size,
              kr, mach_error_string(kr));
  memset((char *)(uintptr_t)vm_addr, 'p', (size_t)vm_size);
  kr = mach_vm_deallocate(mach_task_self(), vm_addr, vm_size);
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS, "vm_deallocate() error 0x%x (%s)", kr,
...
static mach_vm_address_t pre_warm(mach_vm_size_t vm_size) {
  /*
...
T_DECL(phys_footprint_anonymous, "phys_footprint for anonymous memory",
  ...
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS, "vm_allocate() error 0x%x (%s)", kr,
              mach_error_string(kr));
  T_QUIET;
  ...
  T_EXPECT_EQ(footprint_after, footprint_expected,
              ...
              vm_size, footprint_before, footprint_after, footprint_expected,
  ...
  T_EXPECT_EQ(footprint_after, footprint_expected,
              "touched %lld bytes: "
              "footprint %lld -> %lld expected %lld delta %lld",
              dirty_size, footprint_before, footprint_after, footprint_expected,
              footprint_after - footprint_expected);
...
T_DECL(phys_footprint_file, "phys_footprint for mapped file",
       T_META_LTEPHASE(LTE_POSTINIT), T_META_TAG_VM_PREFERRED) {
  uint64_t footprint_before, pagetable_before;
  uint64_t footprint_after, pagetable_after;
  uint64_t footprint_expected;
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(buf = (char *)malloc(buf_size), "allocate %zu-byte buffer",
                   buf_size);
  ...
  T_WITH_ERRNO;
  T_QUIET;
  T_ASSERT_GE(fd = open(tmp_file_name, O_CREAT | O_RDWR), 0,
              "create temp file");
  T_WITH_ERRNO;
  T_QUIET;
  T_ASSERT_EQ(nbytes = write(fd, buf, buf_size), (ssize_t)buf_size,
  ...
  T_WITH_ERRNO;
  T_QUIET;
  T_ASSERT_NOTNULL(map_addr =
                       (char *)mmap(NULL, map_size, PROT_READ | PROT_WRITE,
                                    MAP_FILE | MAP_SHARED, fd, 0),
                   "mmap()");
  T_QUIET;
  T_EXPECT_EQ((mach_vm_address_t)map_addr, pre_vm_addr, "pre-warm mishap");
  /* ... should not change footprint */
  get_ledger_info(&footprint_after, &pagetable_after);
  footprint_expected = footprint_before;
  footprint_expected += (pagetable_after - pagetable_before);
  ...
  T_EXPECT_EQ(footprint_after, footprint_expected,
              ...
              "footprint %lld -> %lld expected %lld delta %lld",
              map_size, footprint_before, footprint_after, footprint_expected,
              footprint_after - footprint_expected);
  ...
  T_WITH_ERRNO;
  T_QUIET;
  T_ASSERT_EQ(munmap(map_addr, map_size), 0, "unmap file");
  /* ... should not impact footprint */
  get_ledger_info(&footprint_after, &pagetable_after);
  footprint_expected = footprint_before;
  footprint_expected += (pagetable_after - pagetable_before);
...
T_DECL(phys_footprint_purgeable, "phys_footprint for purgeable memory",
       T_META_LTEPHASE(LTE_POSTINIT), T_META_TAG_VM_PREFERRED) {
  uint64_t footprint_before, pagetable_before;
  uint64_t footprint_after, pagetable_after;
  uint64_t footprint_expected;
  ...
  T_QUIET;
  T_ASSERT_EQ(mach_vm_purgable_control(mach_task_self(), vm_addr,
                                       VM_PURGABLE_SET_STATE, &state),
              KERN_SUCCESS, "vm_purgable_control(VOLATILE)");
  T_QUIET;
  T_ASSERT_EQ(state, VM_PURGABLE_NONVOLATILE, "memory was non-volatile");
  /* ... should decrease footprint */
  get_ledger_info(&footprint_after, &pagetable_after);
  footprint_expected = footprint_before - dirty_size;
  footprint_expected += (pagetable_after - pagetable_before);
  ...
  T_ASSERT_EQ(mach_vm_purgable_control(mach_task_self(), vm_addr,
                                       VM_PURGABLE_SET_STATE, &state),
              KERN_SUCCESS, "vm_purgable_control(NONVOLATILE)");
  T_QUIET;
  T_ASSERT_EQ(state, VM_PURGABLE_VOLATILE, "memory was volatile");
  /* ... should increase footprint */
  get_ledger_info(&footprint_after, &pagetable_after);
  footprint_expected = footprint_before + dirty_size;
  footprint_expected += (pagetable_after - pagetable_before);
...
T_DECL(phys_footprint_purgeable_ownership,
  ...
  kr = mach_make_memory_entry_64(mach_task_self(), &me_size, vm_addr,
                                 VM_PROT_READ | VM_PROT_WRITE, &me_port,
                                 MACH_PORT_NULL);
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS, "make_memory_entry() error 0x%x (%s)", kr,
              mach_error_string(kr));
  T_QUIET;
  ...
  kr = mach_memory_entry_ownership(
      ...
      mach_task_self(), VM_LEDGER_TAG_DEFAULT,
#endif                         /* VM_LEDGER_TAG_UNCHANGED */
      0);
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS, "mach_memory_entry_ownership() error 0x%x (%s)",
              kr, mach_error_string(kr));
  /* ... should not change footprint */
  get_ledger_info(&footprint_after, &pagetable_after);
  footprint_expected = footprint_before;
  ...
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS, "mach_port_deallocate() error 0x%x (%s)", kr,
              mach_error_string(kr));
  /* ... should decrease footprint */
  get_ledger_info(&footprint_after, &pagetable_after);
  footprint_expected = footprint_before - dirty_size;
...
T_DECL(phys_footprint_multiple_purgeable_ownership,
       ...
       T_META_NAMESPACE(TEST_VM_NAMESPACE), T_META_LTEPHASE(LTE_POSTINIT),
  ...
  me_port = MACH_PORT_NULL;
  kr =
      mach_make_memory_entry_64(mach_task_self(), &me_size, vm_addr,
  ...
  get_ledger_info(&footprint_before, &pagetable_before);
  kr = mach_memory_entry_ownership(
      me_port, MACH_PORT_NULL, /* owner remains unchanged */
      VM_LEDGER_TAG_UNCHANGED, /* ledger-tag remains unchanged */
      0);
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS, "mach_memory_entry_ownership() error 0x%x (%s)",
...
T_DECL(phys_footprint_ledger_purgeable_owned,
  ...
  me_size = vm_size;
  me_port = MACH_PORT_NULL;
  kr = mach_make_memory_entry_64(mach_task_self(), &me_size, 0,
                                 (MAP_MEM_NAMED_CREATE | MAP_MEM_LEDGER_TAGGED |
                                  MAP_MEM_PURGABLE | VM_PROT_READ |
                                  VM_PROT_WRITE),
                                 &me_port, MACH_PORT_NULL);
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS, "make_memory_entry() error 0x%x (%s)", kr,
  ...
  get_ledger_info(&footprint_before, &pagetable_before);
  vm_addr = 0;
  kr = mach_vm_map(mach_task_self(), &vm_addr, vm_size, 0, /* mask */
                   VM_FLAGS_ANYWHERE, me_port, 0,          /* offset */
                   FALSE,                                  /* copy */
                   VM_PROT_READ | VM_PROT_WRITE, VM_PROT_READ | VM_PROT_WRITE,
                   VM_INHERIT_DEFAULT);
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS, "vm_map() error 0x%x (%s)", kr,
              mach_error_string(kr));
  T_QUIET;
  T_EXPECT_EQ(vm_addr, pre_vm_addr, "pre-warm mishap");
  /* ... should not change footprint */
...
T_DECL(phys_footprint_no_footprint_for_debug,
  ...
  kr = mach_memory_entry_ownership(me_port, mach_task_self(),
  ...
  T_EXPECT_EQ(kr, KERN_NO_ACCESS,
              "mach_memory_entry_ownership(NO_FOOTPRINT_FOR_DEBUG) fails "
              "without sysctl");

  /* let's get permission to hide debug memory */
  new_value = 1;
  ret = sysctlbyname("vm.task_no_footprint_for_debug", NULL, NULL, &new_value,
                     sizeof(new_value));
  ...
  T_QUIET;
  T_ASSERT_EQ(kr, KERN_SUCCESS, "task_create_identity_token");

  kr = mach_memory_entry_ownership(
      me_port, token, /* this time also try calling it with id token */
      VM_LEDGER_TAG_DEFAULT, VM_LEDGER_FLAG_NO_FOOTPRINT_FOR_DEBUG);
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS,
              "mach_memory_entry_ownership(NO_FOOTPRINT_FOR_DEBUG) succeeds "
              "after sysctl");
  ...
  footprint_expected = footprint_before;
  footprint_expected += (pagetable_after - pagetable_before);
  T_LOG("modifying no_footprint_for_debug memory does not increase "
        "phys_footprint");
  T_EXPECT_EQ(footprint_after, footprint_expected,
              "touched %lld bytes: "
              "footprint %lld -> %lld expected %lld delta %lld",
...
static size_t bytes_per_element(uint32_t format) {
  size_t bpe = 0;
  switch (format) {
...
static size_t bytes_per_pixel(uint32_t format) {
  size_t bpe = 0;
  switch (format) {
  case 32: // kCVPixelFormatType_32ARGB (ARGB8)
    bpe = 4;
...
static inline size_t roundSizeToMultiple(size_t size, size_t mult) {
  return ((size + mult - 1) / mult) * mult;
}
static inline void setIntValue(CFMutableDictionaryRef dict,
                               const CFStringRef key, int value) {
  CFNumberRef number = CFNumberCreate(0, kCFNumberIntType, &value);
  CFDictionarySetValue(dict, key, number);
  CFRelease(number);
}
static inline void setBoolValue(CFMutableDictionaryRef dict,
                                const CFStringRef key, bool value) {
  CFDictionarySetValue(dict, key, value ? kCFBooleanTrue : kCFBooleanFalse);
}
typedef void (^SurfacePlaneBlock)(void *data, size_t planeIndex, size_t width,
                                  size_t height, size_t rowbytes);
static IOReturn SurfaceApplyPlaneBlock(IOSurfaceRef surface,
...
static void ClearSurface(IOSurfaceRef surface) {
  const int zero = 0;
  (void)SurfaceApplyPlaneBlock(
      surface, ^(void *p, size_t i, __unused size_t w, size_t h, size_t rb) {
        if (i == 0) {
          memset(p, zero, rb * h);
        } else {
          memset(p, 128, rb * h);
...
static size_t SurfaceGetMemorySize(IOSurfaceRef surface) {
  size_t planeCount = IOSurfaceGetPlaneCount(surface);
...
static IOSurfaceRef CreateSurface(uint32_t pixelsWide, uint32_t pixelsHigh,
                                  uint32_t rowBytesAlignment, uint32_t fmt,
                                  bool purgeable, bool clear) {
...
T_DECL(phys_footprint_purgeable_iokit,
  ...
  IOSurfaceSetPurgeable(init_surface, kIOSurfacePurgeableNonVolatile,
                        &old_state);
  T_SETUPEND;
  ...
  if (legacy_footprint) {
    ...
    T_LOG(
        "LEGACY FOOTPRINT: creating purgeable IOSurface: no footprint impact");
...
T_DECL(phys_footprint_nonpurgeable_iokit,
  ...
  remap_addr = 0;
  kr =
      mach_vm_remap(mach_task_self(), &remap_addr, (mach_vm_size_t)surface_size,
                    0, VM_FLAGS_ANYWHERE, mach_task_self(),
                    (mach_vm_address_t)map_base, FALSE, /* copy */
                    &cur_prot, &max_prot, VM_INHERIT_DEFAULT);
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS, "vm_remap() error 0x%x (%s)", kr,
              mach_error_string(kr));
  get_ledger_info(&footprint_after, &pagetable_after);
  footprint_expected = footprint_before;
  footprint_expected += (pagetable_after - pagetable_before);
  ...
  get_ledger_info(&footprint_before, &pagetable_before);
  kr = mach_vm_deallocate(mach_task_self(), remap_addr,
