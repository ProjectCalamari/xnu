...
static void cleanup(void) {
  ...
  if (default_fake_max_mtu != 0) {
    ...
    (void)sysctlbyname("net.link.fake.max_mtu", NULL, NULL,
                       &default_fake_max_mtu, sizeof(int));
  }
}

static void init(int mtu) {
  T_ATEND(cleanup);

  if (mtu > 0) {
    size_t oldlen = sizeof(int);
    T_ASSERT_POSIX_SUCCESS(
        sysctlbyname("net.link.fake.max_mtu", &default_fake_max_mtu, &oldlen,
                     &mtu, sizeof(int)),
        "sysctl net.link.fake.max_mtu %d -> %d", default_fake_max_mtu, mtu);
  }
}

static int setup_feth_pair(int mtu) {
  int error = 0;
...
static int create_bpf_on_interface(const char *ifname, int *out_fd,
                                   int *out_bdlen, u_int write_size_max) {
  int bpf_fd = -1;
  int error = 0;
  int bdlen = 0;
  u_int value;
  ...
  T_ASSERT_POSIX_SUCCESS(bpf_setif(bpf_fd, ifname), "bpf set if %s", ifname1);
  ...
  T_LOG("write_size_max %u %s value %u", write_size_max,
        write_size_max != value ? "!=" : "==", value);
#else
  if (write_size_max > 0) {
...
static void make_bootp_packet(struct iovec *iov, u_int ip_len, int id) {
  u_int payload_len;

  if (ip_len == 0) {
    payload_len = (u_int)sizeof(dhcp_min_payload);
  } else {
    T_ASSERT_GE((size_t)ip_len,
                sizeof(struct ip) + sizeof(struct udphdr) +
                    sizeof(dhcp_min_payload),
                "ip_len");
  ...
  u_int frame_length = ethernet_udp4_frame_populate(
      (void *)pkt, pkt_size, &src_eaddr, src_ip, src_port, &dst_eaddr, dst_ip,
      dst_port, payload, payload_len);

  T_ASSERT_EQ(sizeof(struct bpf_hdr), BPF_WORDALIGN(sizeof(struct bpf_hdr)),
              "bpfhdr.bh_hdrlen == BPF_WORDALIGN(sizeof(struct bpf_hdr))");
...
static void do_bpf_write_batch(int count, const char *ifname, u_int ip_len,
                               bool expect_success, u_int write_size_max) {
  int bpf_fd = -1;
  int bdlen = 0;
  ...
  T_ASSERT_POSIX_ZERO(
      create_bpf_on_interface(ifname, &bpf_fd, &bdlen, write_size_max), NULL);
  ...
  for (i = 0; i < count; i++) {
    ...
    T_LOG("tv_sec %u tv_usec %u caplen %u datalen %u hdrlen %u",
          h0->bh_tstamp.tv_sec, h0->bh_tstamp.tv_usec, h0->bh_caplen,
          h0->bh_datalen, h0->bh_hdrlen);

    HexDump(iovs[i].iov_base, MIN((size_t)iovs[i].iov_len, 512));

    T_ASSERT_EQ((int)(iovs[i].iov_len % BPF_ALIGNMENT), 0,
                "iovs[i].iov_len %% BPF_ALIGNMENT == 0");
  }
  ...
  T_ASSERT_GE((size_t)nwritten, iovs[0].iov_len,
              "nwritten %lu >= iovs[0].iov_len %lu", nwritten, iovs[0].iov_len);
  ...
  for (i = 0; i < count && bp < ep; i++) {
    ...
    T_LOG("tv_sec %u tv_usec %u caplen %u datalen %u hdrlen %u",
          hp->bh_tstamp.tv_sec, hp->bh_tstamp.tv_usec, hp->bh_caplen,
          hp->bh_datalen, hp->bh_hdrlen);

    HexDump(bp, MIN((size_t)BPF_WORDALIGN(hp->bh_hdrlen + hp->bh_caplen), 512));

    /*
     * Note: The following will fail if there is parasitic traffic and that
     * should not happen over feth
     */
    unsigned char *p0 = iovs[i].iov_base;
    struct bpf_hdr *h0 = (struct bpf_hdr *)iovs[i].iov_base;
    ;

    T_ASSERT_EQ(h0->bh_caplen, hp->bh_caplen,
                "h0->bh_caplen %d == hp->bh_caplen %d", h0->bh_caplen,
                hp->bh_caplen);
    T_ASSERT_EQ(h0->bh_datalen, hp->bh_datalen,
                "h0->bh_datalen %d == hp->bh_datalen %d", h0->bh_datalen,
                hp->bh_datalen);

    T_ASSERT_EQ_INT(bcmp(h0->bh_hdrlen + p0, hp->bh_hdrlen + bp, hp->bh_caplen),
                    0, "bpf read same bytes as written iov %d", i);
...
static void test_bpf_write_batch(int count, u_int data_len, int mtu,
                                 bool expect_success, u_int write_size_max) {
  init(mtu);
