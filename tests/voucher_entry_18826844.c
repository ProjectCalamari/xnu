/*
 * Test that sending a message to a voucher with the same voucher as the voucher
 * port with only one send right count with move send before the copy send
 * doesn't panic.
 *
 * clang -o voucherentry voucherentry.c -ldarwintest -Weverything
...
T_DECL(voucher_entry, "voucher_entry", T_META_CHECK_LEAKS(false),
  ...
  kr = mach_port_get_refs(mach_task_self(), voucher, MACH_PORT_RIGHT_SEND,
                          &refs);
  ...
  T_ASSERT_MACH_SUCCESS(mach_port_construct(mach_task_self(), &opts, 0, &reply),
                        "make reply port");
  ...
  kr = mach_msg2(&request_msg_1,
                 MACH64_SEND_MSG | MACH64_RCV_MSG | MACH64_SEND_KOBJECT_CALL,
                 request_msg_1, request_msg_1.msgh_size, 0, MACH_PORT_NULL,
                 MACH_MSG_TIMEOUT_NONE, 0);

  T_ASSERT_MACH_ERROR(MACH_SEND_INVALID_DEST, kr,
  ...
  /* panic happens here */
  kr = mach_msg2(&request_msg_2,
                 MACH64_SEND_MSG | MACH64_RCV_MSG | MACH64_SEND_KOBJECT_CALL,
                 request_msg_2.hdr, request_msg_2.hdr.msgh_size,
                 sizeof(request_msg_2), reply, MACH_MSG_TIMEOUT_NONE, 0);
  ...
  T_ASSERT_MACH_ERROR(KERN_INVALID_NAME, kr,
