...
static void req_cooperative_wq_threads(qos_class_t qos, size_t num_threads) {
  int ret;

  for (size_t i = 0; i < num_threads; i++) {
    dispatch_group_enter(group);

    ret = _pthread_workqueue_add_cooperativethreads(
        1, _pthread_qos_class_encode(qos, 0, 0));
    assert(ret == 0);
  }
}

static void req_wq_threads(qos_class_t qos, size_t num_threads,
                           bool overcommit) {
  int ret;
  ...
  for (size_t i = 0; i < num_threads; i++) {
    ...
    ret = _pthread_workqueue_addthreads(
        1, _pthread_qos_class_encode(
               qos, 0, (overcommit ? _PTHREAD_PRIORITY_OVERCOMMIT_FLAG : 0)));
    assert(ret == 0);
  }
}
...
static inline bool thread_is_overcommit(pthread_priority_t priority) {
  return (priority & _PTHREAD_PRIORITY_OVERCOMMIT_FLAG) != 0;
}

static inline bool thread_is_nonovercommit(pthread_priority_t priority) {
  return (priority & (_PTHREAD_PRIORITY_OVERCOMMIT_FLAG |
                      _PTHREAD_PRIORITY_COOPERATIVE_FLAG)) != 0;
}

static inline bool thread_is_cooperative(pthread_priority_t priority) {
  return (priority & _PTHREAD_PRIORITY_COOPERATIVE_FLAG) != 0;
}

qos_class_t thread_has_qos(pthread_priority_t pri) {
  return _pthread_qos_class_decode(pri, NULL, NULL);
}

char *qos_to_str(qos_class_t qos) {
  switch (qos) {
...
static void worker_cooperative_then_overcommit(pthread_priority_t priority) {
...
int do_cooperative_then_overcommit() {
...
qos_class_t get_rand_qos_class(void) {
  switch (rand() % 6) {
  case 0:
...
int get_rand_num_thread_requests(void) { return rand() % (ncpus() * 2); }

uint64_t get_rand_spin_duration_nsecs(void) {
  /* Spin for at most half a second */
  return rand() % (NSEC_PER_SEC / 2);
}

void spin(uint64_t spin_duration_nsecs) {
  uint64_t duration =
      spin_duration_nsecs * timebase_info.denom / timebase_info.numer;
...
static void worker_cb_stress(pthread_priority_t priority) {
  if (test_should_end) {
    dispatch_group_leave(group);
    return;
  }
  ...
  if (thread_is_cooperative(priority)) {
    ...
    spin(get_rand_spin_duration_nsecs());
    req_cooperative_wq_threads(get_rand_qos_class(),
                               get_rand_num_thread_requests());
  } else {
