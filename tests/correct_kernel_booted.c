...
static bool get_macho_uuid(const char *cwd, const char *path, uuid_t uuid) {
  bool found = false;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "should stat %s%s", cwd, path);
  ...
  mapped = mmap(NULL, (size_t)stbuf.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NE(mapped, MAP_FAILED, "should map Mach-O binary at %s%s", cwd,
              path);
  (void)close(fd);
  ...
  for (unsigned int i = 0; i < ncmds; i++) {
    ...
    uint32_t cmdsize = should_swap ? OSSwapInt32(lcmd->cmdsize) : lcmd->cmdsize;
...
T_DECL(correct_kernel_booted,
       ...
       T_META_RUN_CONCURRENTLY(true), T_META_CHECK_LEAKS(false),
  ...
  int ret = sysctlbyname("kern.uuid", &kern_uuid_str, &kern_uuid_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "should get running kernel UUID");
  T_LOG("%s: running kernel", kern_uuid_str);

  ret = uuid_parse(kern_uuid_str, kern_uuid);
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "should parse kernel UUID into bytes");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "should change directory to %s", kernels_path);

  DIR *kernels_dir = opendir(kernels_path);
  T_QUIET;
  T_ASSERT_NOTNULL(kernels_dir, "should open directory at %s", kernels_path);
