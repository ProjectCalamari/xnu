...
static void update(uint64_t *a, uint64_t *c) { mach_get_times(a, c, NULL); }

T_DECL(mct_monotonic,
...
#endif
    tmp = to_ms(tmp);
    T_ASSERT_GE(tmp, multiple_test,
                "mach_continuous_time (%s) must be monotonic", test_type);

    // each successive call shouldn't be more than 100ms in the future
    T_ASSERT_LE(tmp - multiple_test, 100ULL,
                "mach_continuous_time (%s) should not jump forward too fast",
                test_type);
...
T_DECL(mat_monotonic,
  ...
  for (int i = 0; i < 20; i++) {
    ...
    tmp = mach_absolute_time();
    tmp = to_ms(tmp);
    T_ASSERT_GE(tmp, multiple_test, "mach_absolute_time (%s) must be monotonic",
                test_type);

    // each successive call shouldn't be more than 100ms in the future
    T_ASSERT_LE(tmp - multiple_test, 100ULL,
                "mach_absolute_time (%s) should not jump forward too fast",
                test_type);
...
T_DECL(mct_pause,
  ...
  T_ASSERT_LE(abs(after_diff - before_diff), 1,
              "mach_continuous_time and mach_absolute_time should not diverge");
}
...
T_DECL(
  ...
  T_ASSERT_LE(abs(after_diff - before_diff), 1,
              "mach_continuous_time_kernel and mach_absolute_time_kernel "
              "should not diverge");
}
#endif
...
T_DECL(mct_sleep, "Testing mach_continuous_time behavior over system sleep") {
  ...
  T_SKIP("Skipping test that sleeps the device; compile with MCT_SLEEP_TEST "
         "define to enable.");
#endif
  ...
  T_LOG("Testing mach_continuous_time is ~5 seconds ahead of "
        "mach_absolute_time after 5 second sleep");
  update(&abs_now, &cnt_now);
  before_diff = (int)(to_ms(cnt_now) - to_ms(abs_now));
  ...
  if ((after_diff - before_diff) < 4000) {
    T_LOG("Device slept for less than 4 seconds, did it really sleep? (%d ms "
          "change between abs and cont)",
  ...
  T_LOG("Calendar progressed: %d sec; continuous time progressed: %d sec; "
        "absolute time progressed %d sec",
...
T_DECL(mct_settimeofday,
  ...
  T_ASSERT_LT(abs(before - after), 1000,
              "mach_continuous_time should not jump more than 1s");
}

#ifdef HAS_KERNEL_TIME_TRAPS
T_DECL(mct_settimeofday_kern,
  ...
  T_ASSERT_LT(abs(before - after), 1000,
              "mach_continuous_time_kernel should not jump more than 1s");
}
#endif
...
T_DECL(mct_aproximate, "Testing mach_continuous_approximate_time()",
  ...
  T_EXPECT_LE(llabs((long long)absolute - (long long)approximate),
              (long long)(25 * NSEC_PER_MSEC), NULL);
}
...
T_DECL(mach_time_perf_instructions, "instructions retired for mach_time",
       T_META_TYPE_PERF, T_META_ASROOT(YES)) {
  {
    dt_stat_thread_instructions_t s =
...
T_DECL(mach_time_perf_instructions_kern,
