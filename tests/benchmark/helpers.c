    ...
    if (now != -1) {
      ...
      if (ret == &local_time) {
        snprintf(time_buffer, K_CTIME_BUFFER_LEN,
                 "%.2d/%.2d/%.2d %.2d:%.2d:%.2d", local_time.tm_mon + 1,
...
uint64_t timespec_difference_us(const struct timespec *a,
  ...
  if (b->tv_nsec > a->tv_nsec) {
    seconds_elapsed--;
    nsec_elapsed =
        kNumNanosecondsInSecond - (uint64_t)(b->tv_nsec - a->tv_nsec);
  } else {
    nsec_elapsed = (uint64_t)(a->tv_nsec - b->tv_nsec);
  }
  return (uint64_t)seconds_elapsed * kNumMicrosecondsInSecond +
         nsec_elapsed / kNumNanosecondsInMicrosecond;
}

unsigned char *map_buffer(size_t memsize, int flags) {
#if USE_MMAP
  int fd = -1;
  unsigned char *addr = (unsigned char *)mmap(
      NULL, memsize, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, fd, 0);
  ...
  kern_return_t kr = vm_allocate(mach_task_self(), &address, memsize,
