...
__inline static void yield(void) {
...
__inline static mach_port_name_t _os_get_self(void) {
  mach_port_name_t self =
      (mach_port_name_t)(uintptr_t)(void *)_os_tsd_get_direct(
...
__inline static void ull_lock(lock_t *lock, int id, uint opcode, uint flags) {
  ...
  do {
    count++;
    if ((count % 100000) == 0) {
      printf("[%d,%d]%s>top of loop count=%d\n", id, mach_id, __FUNCTION__,
             count);
    }
    u32 new = waiters ? (ULL_WAITERS | ull_locked) : ull_locked;
    prev = 0;
    __c11_atomic_compare_exchange_strong(lock, &prev, new, __ATOMIC_ACQUIRE,
                                         __ATOMIC_RELAXED);
    if (prev == 0) {
    ...
    if (!(value & ULL_WAITERS)) {
      ...
      __c11_atomic_compare_exchange_strong(lock, &prev, new, __ATOMIC_RELAXED,
                                           __ATOMIC_RELAXED);
      if (prev == value) {
...
__inline static void ull_unlock(lock_t *lock, int id, uint opcode, uint flags) {
  u32 thread_id = _os_get_self() & ~0x3u;
  u32 ull_locked = (opcode == UL_UNFAIR_LOCK) ? thread_id : 4u;
  u32 mach_id = _os_get_self() >> 2;
  u32 prev = ull_locked;
  __c11_atomic_compare_exchange_strong(lock, &prev, 0, __ATOMIC_RELEASE,
                                       __ATOMIC_RELAXED);
  if (prev == ull_locked) {
