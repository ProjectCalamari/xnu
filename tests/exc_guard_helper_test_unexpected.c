...
T_DECL(exc_guard_helper_test_unexpected_exc_guard,
       "provoke one guard exception type while exc_guard_helper is expecting "
       "another") {
  ...
  if ((child_pid = fork())) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(waited_pid, "waitpid");
    T_QUIET;
    T_ASSERT_EQ(waited_pid, child_pid, "waitpid");

    T_ASSERT_TRUE(WIFSIGNALED(status), "child should have crashed");
    T_ASSERT_EQ(WTERMSIG(status), SIGKILL,
                "child should have crashed with SIGKILL");
  } else {
    /* child */
    kern_return_t kr;
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "set fatal mach port behavior");
    ...
    if (block_raised_exc_guard_of_type(GUARD_TYPE_VIRT_MEMORY, &exc_info, ^{
          ...
          T_ASSERT_MACH_ERROR(kr, KERN_INVALID_VALUE,
                              "add too many send rights");
        })) {
      T_FAIL("Mach port guard exception unexpectedly caught by VM guard "
             "exception handler");
    }
