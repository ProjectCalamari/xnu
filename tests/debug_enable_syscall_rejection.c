...
T_GLOBAL_META(T_META_NAMESPACE("xnu.debug"), T_META_RADAR_COMPONENT_NAME("xnu"),
...
static int helper(char **args) {
  ...
  T_FAIL("helper terminated with unexpected condition (status: %i, signal: %i)",
         status, signal);
  return 2;
}

static void do_test(char const *msg, int expectation, ...) {
  ...
  while ((arg = va_arg(ap, char *)) != NULL) {
    T_QUIET;
    T_ASSERT_LT(i, (int)arg_max_count, "no args overflow");
    args[i++] = arg;
  }
...
T_DECL(debug_syscall_rejection_tests, "Verify that syscall rejection works",
  ...
  int ret = sysctlbyname("kern.debug_syscall_rejection_mode", &old_mode,
                         &old_mode_size, &new_mode, sizeof(new_mode));
  if (ret != 0) {
    T_ASSERT_FAIL("Syscall rejection mode switching failed: ret %d, errno %d",
                  ret, errno);
  }
  ...
  T_EXPECT_POSIX_SUCCESS(sysctlbyname("kern.syscall_rejection_masks", NULL,
                                      NULL, set_mask_12, set_mask_12_size),
  ...
  do_test("disallow testmask overrides dies", DEAD, ALLOW(0, 1), DENY(1, 12),
          NULL);
  ...
  for (int i = 0; i < slots - 1; i++) {
    ...
    T_QUIET;
    T_ASSERT_GE_INT(snprintf(pos1, max_pos_len, "%u", i), 0, "creating pos1");
    T_QUIET;
    T_ASSERT_GE_INT(snprintf(pos2, max_pos_len, "%u", i + 1), 0,
                    "creating pos2");
  ...
  new_mode = 1;
  T_EXPECT_POSIX_SUCCESS(sysctlbyname("kern.debug_syscall_rejection_mode", NULL,
  ...
  do_test("non-fatal: disallow testmask overrides", ALIVE, ALLOW(0, 1),
          DENY(1, 12), NULL);
  ...
  do_test("force-fatal: disallow testmask overrides", DEAD, ALLOW(0, 1),
          DENY(1, 12), "-F", NULL);
}
...
T_DECL(debug_enable_syscall_rejection_crash_count,
  ...
  int ret = debug_syscall_reject_config(masks, sizeof(masks) / sizeof(masks[0]),
                                        SYSCALL_REJECTION_FLAGS_DEFAULT);

  T_WITH_ERRNO;
  T_ASSERT_POSIX_SUCCESS(ret, "debug_syscall_reject_config");
  ...
  ret = debug_syscall_reject_config(masks, sizeof(masks) / sizeof(masks[0]),
                                    SYSCALL_REJECTION_FLAGS_ONCE);

  T_WITH_ERRNO;
  T_ASSERT_POSIX_SUCCESS(ret, "debug_syscall_reject_config once");
  ...
  T_WITH_ERRNO;
  T_ASSERT_POSIX_SUCCESS(ret, "debug_syscall_reject_config once ignore");
  ...
  ret = debug_syscall_reject_config(masks, sizeof(masks) / sizeof(masks[0]),
                                    SYSCALL_REJECTION_FLAGS_FORCE_FATAL);

  T_WITH_ERRNO;
  T_ASSERT_POSIX_SUCCESS(ret, "debug_syscall_reject_config fatal");
   ...
   * Guard (1)		5			3 without ONCE flag, 1 with ONCE
   * flag, final forced fatal crash Fatal (2)		1
