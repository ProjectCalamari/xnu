...
T_GLOBAL_META(T_META_NAMESPACE("xnu.syscall.pread"),
              T_META_RUN_CONCURRENTLY(true));

T_DECL(pread_regular_file, "test pread() on a regular file.") {
...
static void test_pread_should_fail(int fd, int expected_errno) {
...
T_DECL(pread_socket, "test pread() on a socket.") {
  int sockets[2];
...
T_DECL(pread_unix_shared_memory, "test pread() on unix shared memory.") {
...
T_DECL(pread_kqueue, "test pread() on kqueue.") {
  int queue = kqueue();
...
T_DECL(pread_pipe, "test pread() on pipe.") {
  int pipe_fds[2];
...
T_DECL(pread_read_from_null, "test pread() from null.") {
