...
static void *poll_kqueue(void *arg) {
  int fd = (int)arg;
  ...
  if (rv == -1 && errno == EBADF) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(rv, "kevent");
  }
...
static void run_test(void) {
  int fd = kqueue();
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(fd, "kqueue");

  pthread_t thread;
  int rv = pthread_create(&thread, NULL, poll_kqueue, (void *)(uintptr_t)fd);
  T_QUIET;
...
T_DECL(kqueue_filter_close,
  ...
  T_QUIET;
  T_EXPECT_MACH_SUCCESS(kr, "mach_port_allocate(RECEIVE)");
