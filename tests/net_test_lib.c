...
struct in_addr inet_class_c_subnet_mask = {.s_addr = htonl(IN_CLASSC_NET)};

ether_addr_t ether_broadcast = {{0xff, 0xff, 0xff, 0xff, 0xff, 0xff}};

/*
 * local utility functions
 */
static void siocll_start(int s, const char *ifname) {
...
static void nd_flags_set(int s, const char *if_name, uint32_t set_flags,
  ...
  if (new_flags != nd.ndi.flags) {
    ...
    T_ASSERT_POSIX_SUCCESS(result, "SIOCSIFINFO_FLAGS(%s) 0x%x", if_name,
                           nd.ndi.flags);
  }
}

static void siocprotoattach_in6(int s, const char *name) {
...
static void sioc_a_or_d_ifaddr(int s, char *ifname, struct in_addr addr,
                               struct in_addr mask, bool add) {
  ...
  T_ASSERT_POSIX_SUCCESS(ret, "%s %s %s %s", request_str, ifname, ntopbuf_ip,
                         ntopbuf_mask);
}

static void siocaifaddr(int s, char *ifname, struct in_addr addr,
                        struct in_addr mask) {
  sioc_a_or_d_ifaddr(s, ifname, addr, mask, true);
}

static void siocdifaddr(int s, char *ifname, struct in_addr addr,
                        struct in_addr mask) {
...
static int _dgram_socket_get(int *sock_p, int af) {
  int sock = *sock_p;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sock, "socket(SOCK_DGRAM, %s, 0)",
                         af == AF_INET ? "AF_INET" : "AF_INET6");
done:
  return sock;
}

static void _socket_close(int *sock_p) {
  int sock = *sock_p;
...
int inet_dgram_socket_get(void) {
  return _dgram_socket_get(&inet_dgram_socket, AF_INET);
}

void inet_dgram_socket_close(void) { _socket_close(&inet_dgram_socket); }

static int inet6_dgram_socket = NO_SOCKET;

int inet6_dgram_socket_get(void) {
  return _dgram_socket_get(&inet6_dgram_socket, AF_INET6);
}

void inet6_dgram_socket_close(void) { _socket_close(&inet6_dgram_socket); }

u_int ethernet_udp4_frame_populate(void *buf, size_t buf_len,
  ...
  udp_pseudo = (udp_pseudo_hdr_t *)(void *)(((char *)&ip_udp->udp) -
                                            sizeof(*udp_pseudo));
  ...
  ip_udp->udp.uh_sum = 0;
  ip_udp->udp.uh_sum = in_cksum(
      udp_pseudo, (int)(sizeof(*udp_pseudo) + sizeof(ip_udp->udp) + data_len));
...
u_int ethernet_udp6_frame_populate(void *buf, size_t buf_len,
                                   const ether_addr_t *src,
                                   struct in6_addr *src_ip, uint16_t src_port,
                                   const ether_addr_t *dst,
  ...
  udp6_pseudo = (udp6_pseudo_hdr_t *)(void *)(((char *)&ip6_udp->udp) -
                                              sizeof(*udp6_pseudo));
  ...
  ip6_udp->udp.uh_sum =
      in_cksum(udp6_pseudo,
               (int)(sizeof(*udp6_pseudo) + sizeof(ip6_udp->udp) + data_len));
...
void ifnet_get_lladdr(const char *ifname, ether_addr_t *eaddr) {
  int err;
  struct ifreq ifr;
...
int ifnet_set_lladdr(const char *ifname, ether_addr_t *eaddr) {
  int err;
  int this_errno = 0;
  struct ifreq ifr;
  int s = inet_dgram_socket_get();
  ...
  if (err != 0) {
    this_errno = errno;
    T_LOG("SIOCSIFLLADDR %s %s (%d)", ifname, strerror(this_errno), this_errno);
  } else {
...
void ifnet_attach_ip(char *name) {
  int err;
  struct ifreq ifr;
  int s = inet_dgram_socket_get();
...
void ifnet_start_ipv6(const char *ifname) {
...
int ifnet_destroy(const char *ifname, bool fail_on_error) {
  int err;
  struct ifreq ifr;
  int s = inet_dgram_socket_get();
...
int ifnet_set_flags(const char *ifname, uint16_t flags_set,
  ...
  if (flags_before == flags_after) {
    ...
    if (G_debug) {
      T_LOG("setflags(%s set 0x%x clear 0x%x) 0x%x => 0x%x", ifr.ifr_name,
            flags_set, flags_clear, flags_before, flags_after);
    }
  }
  return ret;
}
...
static int ifnet_create_common(const char *ifname, char *ifname_out,
...
int ifnet_create(const char *ifname) {
  return ifnet_create_common(ifname, NULL, 0);
}

int ifnet_create_2(char *ifname, size_t len) {
  return ifnet_create_common(ifname, ifname, len);
}

void ifnet_add_ip_address(char *ifname, struct in_addr addr,
                          struct in_addr mask) {
  int s = inet_dgram_socket_get();
...
void ifnet_remove_ip_address(char *ifname, struct in_addr addr,
                             struct in_addr mask) {
  int s = inet_dgram_socket_get();
...
int ifnet_set_mtu(const char *ifname, int mtu) {
  int error = 0;
  struct ifreq ifr = {0};
  int s = inet_dgram_socket_get();
...
int siocdrvspec(const char *ifname, u_long op, void *arg, size_t argsize,
                bool set) {
  struct ifdrv ifd;
  int s = inet_dgram_socket_get();
...
void fake_set_peer(const char *feth, const char *feth_peer) {
  ...
  ret = siocdrvspec(feth, IF_FAKE_S_CMD_SET_PEER, &iffr, sizeof(iffr), true);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "SIOCDRVSPEC(%s, IF_FAKE_S_CMD_SET_PEER, %s)",
                         feth, (feth_peer != NULL) ? feth_peer : "<none>");
  T_LOG("%s peer %s\n", feth, feth_peer);
  return;
}

void siocsifvlan(const char *vlan, const char *phys, uint16_t tag) {
  int result;
  struct ifreq ifr;
  int s = inet_dgram_socket_get();
  struct vlanreq vlr;
  ...
  T_ASSERT_POSIX_SUCCESS(result, "SIOCSIFVLAN(%s) %s %d", vlan, phys, tag);
}

u_int make_dhcp_payload(dhcp_min_payload_t payload, ether_addr_t *eaddr) {
  struct bootp *dhcp;
 ...
 */
static int IPv4RouteApply(IPv4RouteRef route, uint8_t cmd, int s) {
  size_t len;
  int ret = 0;
  route_msg rtmsg;
  union {
  ...
  if (route->ifindex == 0) {
    T_LOG("no interface specified, ignoring %s", inet_ntoa(route->dest));
    return ENXIO;
  }
  if (s < 0) {
  ...
  /* apply the route */
  len = (sizeof(rtmsg.hdr) + (unsigned long)(rtaddr.ptr - (char *)rtmsg.addrs));
  rtmsg.hdr.rtm_msglen = (u_short)len;
  if (write(s, &rtmsg, len) == -1) {
    ret = errno;
    T_LOG("write routing socket failed, (%d) %s", errno, strerror(errno));
  }
  return ret;
}
...
static int routing_socket_get(void) {
...
void route_add_inet_scoped_subnet(char *ifname, u_short if_index,
...
void network_interface_create(network_interface_t if_p, const if_name_t name) {
  int error;
...
void network_interface_destroy(network_interface_t if_p) {
  if (if_p->if_index != 0) {
...
static inline size_t network_interface_pair_list_size(size_t count) {
  return offsetof(network_interface_pair_list, list[count]);
}

network_interface_pair_list_t network_interface_pair_list_alloc(u_int n) {
  network_interface_pair_list_t list;

  list = (network_interface_pair_list_t)calloc(
      1, network_interface_pair_list_size(n));
  list->count = n;
  return list;
}

void network_interface_pair_list_destroy(network_interface_pair_list_t list) {
...
bool has_ipv4_default_route(void) {
  bool result = false;
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(rtm = (struct rt_msghdr *)calloc(1, RTM_BUFLEN), NULL);
  ...
  int fd;
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(fd = socket(PF_ROUTE, SOCK_RAW, 0), NULL);
...
bool has_ipv6_default_route(void) {
  bool result = false;
  struct rt_msghdr *rtm = NULL;
 ...
 * Bridge management
 */
int bridge_add_member(const char *bridge, const char *member) {
  struct ifbreq req;
  int ret;
...
static int rt_xaddrs(char *cp, const char *cplim, struct rt_addrinfo *rtinfo) {
  int i;
  struct sockaddr *sa;
...
static char *copy_if_info(unsigned int if_index, int af, int *ret_len_p) {
  char *buf = NULL;
  size_t buf_len = 0;
  int mib[6];
...
bool inet6_get_linklocal_address(unsigned int if_index,
  ...
  for (scan = buf; scan < buf_end; scan += rtm->rtm_msglen) {
    ...
    if (rtm->rtm_type == RTM_NEWADDR) {
      ...
      error = rt_xaddrs((char *)(ifam + 1), ((char *)ifam) + ifam->ifam_msglen,
                        &info);
      if (error) {
      ...
      if (sin6_p == NULL || sin6_p->sin6_len < sizeof(struct sockaddr_in6)) {
        continue;
      }
