...
static void check_myself(char *name) {
  ...
  T_LOG("my process name is '%s' (comm is '%s')", pinfo.pbi_name,
        pinfo.pbi_comm);
...
T_HELPER_DECL(spawned_helper, "spawned helper") { check_myself("child"); }

T_DECL(set_proc_name, "check process name is correct",
  ...
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "get binary path");

  ret = dt_launch_tool(&pid, (char *[]){binpath, "-n", "spawned_helper", NULL},
                       false, NULL, NULL);
