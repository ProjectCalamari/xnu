...
extern int __bsdthread_ctl(uintptr_t cmd, uintptr_t arg1, uintptr_t arg2,
                           uintptr_t arg3);

T_GLOBAL_META(T_META_NAMESPACE("xnu.workq"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("workq"),
              T_META_RUN_CONCURRENTLY(true));

T_DECL(invalid_allow_sigmask, "test that BSDTHREAD_CTL_WORKQ_ALLOW_SIGMASK "
                              "returns EINVAL for things like SIGKILL") {
  int ret = __bsdthread_ctl(BSDTHREAD_CTL_WORKQ_ALLOW_SIGMASK, sigmask(SIGKILL),
                            0, 0);
  T_ASSERT_EQ(ret, -1,
              "BSDTHREAD_CTL_WORKQ_ALLOW_SIGMASK should not work on prohibited "
              "signals");
}
...
T_DECL(preserve_sigmask_unblock,
       "test that BSDTHREAD_CTL_WORKQ_ALLOW_SIGMASK preserves unblocking "
       "SIGUSR1 on a workq thread") {
  __block int ret;
...
T_DECL(sigmask_signallable, "test that a workq thread can be signalled after "
                            "BSDTHREAD_CTL_WORKQ_ALLOW_SIGMASK") {
  __block int ret;
