...
T_GLOBAL_META(T_META_ASROOT(true), T_META_RUN_CONCURRENTLY(true));

T_DECL(processor_cpu_stat64,
       "ensure 64-bit processor statistics are reported correctly",
       T_META_NAMESPACE("xnu.arm"), T_META_RADAR_COMPONENT_NAME("xnu"),
       T_META_RADAR_COMPONENT_VERSION("arm"), T_META_OWNER("mwidmann"),
       T_META_TAG_VM_PREFERRED, T_META_ENABLED(false /* rdar://133956573 */)) {
#if !__arm64__
  ...
  T_QUIET;
  T_ASSERT_GT(cpu_count, (mach_msg_type_number_t)0, "non-zero CPU count");

  T_LOG("found %d CPUs", cpu_count);

  struct processor_cpu_stat64 *prestats = calloc(cpu_count, sizeof(*prestats));
  T_WITH_ERRNO;
  T_QUIET;
  ...
  for (int i = 0; i < (int)cpu_count; i++) {
    ...
    T_ASSERT_MACH_SUCCESS(kr, "processor_info(%d, PROCESSOR_CPU_STAT64, ...)",
                          i);
  ...
  struct processor_cpu_stat64 *poststats =
      calloc(cpu_count - 1, sizeof(*poststats));
  T_WITH_ERRNO;
  T_QUIET;
...
T_DECL(processor_cpu_info_order,
  ...
  kr = host_processor_info(mach_host_self(), PROCESSOR_BASIC_INFO,
                           &processor_count, &info_array, &info_count);

  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "host_processor_info(PROCESSOR_BASIC_INFO)");
  T_QUIET;
  T_ASSERT_NOTNULL(info_array, "valid processor port array");
  T_QUIET;
  T_ASSERT_GT(info_count, (mach_msg_type_number_t)0, "non-zero array");
  T_QUIET;
  T_ASSERT_GT(processor_count, (natural_t)0, "non-zero processor_count");
