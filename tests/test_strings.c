...
__printflike(1, 2)
    __attribute__((noreturn)) static void panic(const char *fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
...
#define T_COMPARE(A, AS, B, BS, EQ)                                            \
  T_ASSERT_EQ(strbufcmp_impl((A), (AS), (B), (BS)), (EQ),                      \
              "compare '%s'.%zu, '%s'.%zu", (A), (AS), (B), (BS))
...
#define T_COMPARE(A, AS, B, EQ)                                                \
  T_ASSERT_EQ(strlcmp_impl((A), (B), (AS)), (EQ), "compare '%s'.%zu, '%s'",    \
              (A), (AS), (B))
  // two identical strings
  char a[] = "hello";
  char b[] = "hello";
...
#define T_COMPARE(A, AS, B, BS, EQ)                                            \
  T_ASSERT_EQ(strbufcasecmp_impl((A), (AS), (B), (BS)), (EQ),                  \
              "case-insensitive compare '%s'.%zu, '%s'.%zu", (A), (AS), (B),   \
              (BS))
...
#define T_COMPARE(A, AS, B, EQ)                                                \
  T_ASSERT_EQ(strlcasecmp_impl((A), (B), (AS)), (EQ),                          \
              "case-insensitive compare '%s'.%zu, '%s'", (A), (AS), (B))
  // same tests as strcasecmp, then tests with individual characters
  // two identical strings
  char a[] = "hElLo";
  char b[] = "HeLlO";
...
#define T_CHAR_COMPARE(A, AS, B, BS, EQ)                                       \
  do {                                                                         \
    int r = strbufcasecmp_impl((A), (AS), (B), (BS));                          \
    if (r != (EQ))                                                             \
      T_FAIL("case-insensitive compare '0x%02hhx' to '0x%02hhx' was %i "       \
             "instead of %i",                                                  \
             *(A), *(B), r, (EQ));                                             \
  } while (0)
  // test each character
...
T_DECL(strbufcpy, "strbufcpy") {
  char dst[32];
  // empty dest
  T_ASSERT_EQ(strbufcpy_impl(NULL, 0, "hello", 5), NULL,
              "0-length destination");

#define T_CPY(A, AS, B, BS)                                                    \
  T_ASSERT_EQ(strbufcpy_impl((A), (AS), (B), (BS)), (char *)(A),               \
              "copy '%.*s'.%zu to dst.%zu", (int)(BS), (B), (size_t)(BS),      \
              (AS))
  // copy NUL terminated string that fits in dst
  char hello[] = "hello";
  memset(dst, 0, sizeof(dst));
  T_CPY(dst, sizeof(dst), hello, sizeof(hello));
  T_ASSERT_EQ(memcmp_impl(dst, (char[32]){"hello"}, sizeof(dst)), 0,
  ...
  T_ASSERT_EQ(strcmp_impl(hw1, "hello hello world"), 0,
              "check copy over self is 'hello hello world'");
...
T_DECL(strbufcat, "strbufcat") {
  ...
  // empty dst
  T_ASSERT_EQ(strbufcat_impl(NULL, 0, "hello", 5), NULL,
              "check concatenation to 0-length destination");

#define T_CAT_RESULT(RESULT)                                                   \
  T_ASSERT_EQ(strcmp_impl(dst, (RESULT)), 0,                                   \
              "check result of concatenation is '%s'", (RESULT));

#define T_CAT(TO_CAT, RESULT)                                                  \
  do {                                                                         \
    T_ASSERT_EQ(strbufcat_impl(dst, sizeof(dst), (TO_CAT), sizeof(TO_CAT)),    \
                (char *)dst, "check concatenation of '%s'", (TO_CAT));         \
    T_CAT_RESULT(RESULT);                                                      \
  } while (0)
  ...
  // append itself
  T_ASSERT_EQ(strbufcat_impl(dst, sizeof(dst), dst, sizeof(dst)), (char *)dst,
              "check concatenating self");
  T_CAT_RESULT("hello world!hello world!");

  // append bunch of 'a's
  T_ASSERT_EQ(strbufcat_impl(dst, sizeof(dst), "aaaaaaaaaa", 10), (char *)dst,
              "check concatenating 'aaaa...'");
...
T_DECL(libsa_overloads, "libsa_overloads") {
  ...
  T_ASSERT_EQ(strbuflen(buf, sizeof(buf)), (size_t)12,
              "strbuflen two arguments");

  T_ASSERT_LT(strbufcmp(buf, buf2), 0, "strbufcmp two arguments");
  T_ASSERT_LT(strbufcmp(buf, sizeof(buf), buf2, sizeof(buf2)), 0,
              "strbufcmp four arguments");

  T_ASSERT_LT(strbufcasecmp(buf, buf2), 0, "strbufcasecmp two arguments");
  T_ASSERT_LT(strbufcasecmp(buf, sizeof(buf), buf2, sizeof(buf2)), 0,
              "strbufcasecmp four arguments");

  T_ASSERT_NE(strbufcpy(buf, buf2), NULL, "strbufcpy two arguments");
  T_ASSERT_NE(strbufcpy(buf, sizeof(buf), buf2, sizeof(buf2)), NULL,
              "strbufcpy four arguments");

  T_ASSERT_NE(strbufcat(buf, buf2), NULL, "strbufcat two arguments");
  T_ASSERT_NE(strbufcat(buf, sizeof(buf), buf2, sizeof(buf2)), NULL,
              "strbufcat four arguments");
}
