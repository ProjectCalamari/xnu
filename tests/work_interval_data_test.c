...
static void set_realtime(pthread_t thread, uint64_t interval_nanos) {
  kern_return_t kr;
  thread_time_constraint_policy_data_t pol;

  mach_port_t target_thread = pthread_mach_thread_np(thread);
  T_QUIET;
  T_ASSERT_GT(target_thread, 0, "pthread_mach_thread_np");

  /* 1s 100ms 10ms */
  pol.period = (uint32_t)nanos_to_abs(interval_nanos);
  pol.constraint = (uint32_t)nanos_to_abs(interval_nanos);
  pol.computation =
      (uint32_t)nanos_to_abs(interval_nanos - 1000000); // 1 ms of leeway
...
static void create_coreaudio_work_interval(
    work_interval_t *wi_handle, work_interval_instance_t *wi_instance,
    mach_port_t *wi_port, bool enable_telemetry, uint32_t create_flags) {
  int ret = 0;
  create_flags |= WORK_INTERVAL_FLAG_GROUP | WORK_INTERVAL_FLAG_JOINABLE |
  ...
  *wi_instance = work_interval_instance_alloc(*wi_handle);
  T_QUIET;
  T_ASSERT_NE(*wi_instance, NULL, "work_interval_instance_alloc");
}

static void join_coreaudio_work_interval(mach_port_t *wi_port,
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "work_interval_join_port");
}
...
static void thread_barrier(void) {
  ...
  if (barrier_count[active_barrier_ind]) {
    ...
    while (barrier_count[local_active_barrier_ind]) {
      ...
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(ret, "pthread_cond_wait");
    }
  } else {
    ret = pthread_cond_broadcast(&barrier_cond);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "pthread_cond_broadcast");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "pthread_mutex_unlock");
}
 ...
 */
static void contribute_to_work_sum(void) {
...
static void *coreaudio_workload_fn(void *arg) {
...
static void start_helper_threads(unsigned int num_threads, pthread_t *threads,
  ...
  for (unsigned int i = 0; i < num_threads; i++) {
    ...
    ret = pthread_create(&threads[i], NULL, coreaudio_workload_fn,
                         &thread_datas[i]);
    T_QUIET;
...
static void start_work_interval_instance(uint64_t interval_length_abs,
                                         work_interval_instance_t wi_instance,
  ...
  // Sanity assertions that the work interval creation flags and interval id are
  // as expected
  T_QUIET;
  T_ASSERT_EQ(wi_instance->wi_create_flags & WORK_INTERVAL_FLAG_IGNORED, 0,
              "ignored flag start");
  T_QUIET;
  T_ASSERT_EQ(wi_instance->wi_create_flags & WORK_INTERVAL_TYPE_MASK,
              WORK_INTERVAL_TYPE_COREAUDIO, "coreaudio start");
  T_QUIET;
  T_ASSERT_NE(wi_instance->wi_interval_id, 0ULL, "nonzero wi_interval_id");

  ret = work_interval_instance_start(wi_instance);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "work_interval_instance_start");

  if (wi_instance->wi_instance_id == 0ULL && !logged_wi_instance_id_zero) {
    T_LOG("Note, wi_instance_id is 0, which is an acceptable condition for "
          "devices running legacy CLPC");
    logged_wi_instance_id_zero = true;
  }

  work_interval_instance_get_telemetry_data(wi_instance, wi_data,
                                            sizeof(struct work_interval_data));
}

static uint64_t
finish_work_interval_instance(work_interval_instance_t wi_instance,
                              work_interval_data_t wi_data) {
  int ret = 0;
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "work_interval_instance_finish");
...
static void
verify_monotonic_work_interval_data(struct work_interval_data *curr_data,
                                    struct work_interval_data *prev_data,
                                    bool supports_cpi) {
  if (prev_data != NULL) {
    T_QUIET;
    T_ASSERT_GE(curr_data->wid_external_wakeups,
                prev_data->wid_external_wakeups, "wid_external_wakeups");
    T_QUIET;
    T_ASSERT_GE(curr_data->wid_total_wakeups, prev_data->wid_total_wakeups,
                "wid_external_wakeups");
  }
  T_QUIET;
  T_ASSERT_GE(curr_data->wid_user_time_mach,
              prev_data == NULL ? 1 : prev_data->wid_user_time_mach,
              "monotonic wid_user_time_mach");
  T_QUIET;
  T_ASSERT_GE(curr_data->wid_system_time_mach,
              prev_data == NULL ? 1 : prev_data->wid_system_time_mach,
              "monotonic wid_system_time_mach");
  if (supports_cpi) {
    T_QUIET;
    T_ASSERT_GE(curr_data->wid_cycles,
                prev_data == NULL ? 1 : prev_data->wid_cycles,
                "monotonic wid_cycles");
    T_QUIET;
    T_ASSERT_GE(curr_data->wid_instructions,
                prev_data == NULL ? 1 : prev_data->wid_instructions,
                "monotonic wid_instructions");
  }
}

static void verify_zero_work_interval_data(struct work_interval_data *wi_data,
                                           bool supports_cpi) {
  T_QUIET;
  T_ASSERT_EQ(wi_data->wid_external_wakeups, 0, "zero wid_external_wakeups");
  T_QUIET;
  T_ASSERT_EQ(wi_data->wid_total_wakeups, 0, "zero wid_total_wakeups");
  T_QUIET;
  T_ASSERT_EQ(wi_data->wid_user_time_mach, 0ULL, "zero wid_user_time_mach");
  T_QUIET;
  T_ASSERT_EQ(wi_data->wid_system_time_mach, 0ULL, "zero wid_system_time_mach");
  if (supports_cpi) {
    T_QUIET;
    T_ASSERT_EQ(wi_data->wid_cycles, 0ULL, "zero wid_cycles");
    T_QUIET;
    T_ASSERT_EQ(wi_data->wid_instructions, 0ULL, "zero wid_instructions");
  }
}

static void run_work_interval_data_test(unsigned int num_iterations,
                                        uint64_t interval_nanos,
  ...
  ret = sysctlbyname("kern.monotonic.supported", &supports_cpi,
                     &supports_cpi_size, NULL, 0);
  if (ret < 0 || supports_cpi == 0) {
    T_LOG("Monotonic stats are unsupported on this platform. Skipping "
          "cycles/instructions stats validation");
  }
  ...
  create_coreaudio_work_interval(&wi_handle, &wi_instance, &wi_port,
                                 enable_telemetry, flags);
  ...
  start_helper_threads(num_helper_threads, wi_threads, wi_thread_datas,
                       wi_handle, &wi_port, num_iterations, interval_nanos);
  ...
  for (unsigned int i = 0; i < num_iterations; i++) {
    ...
    if (enable_telemetry) {
      verify_monotonic_work_interval_data(&finish_data, &start_data,
                                          supports_cpi);
    } else {
  ...
  if (enable_telemetry) {
    ...
    T_ASSERT_GE(finish_data.wid_total_wakeups, num_iterations,
                "wid_total_wakeups at least accounts for the usleep() wakeups");
  }
  T_ASSERT_TRUE(true,
                "Workload survived %u iterations without failures!!! Avg. work "
                "interval duration was %llu ns out of a requested %llu ns",
                num_iterations, duration_sum / num_iterations, interval_nanos);
}
...
T_DECL(work_interval_rt_coreaudio_quality_telemetry_data,
       "receiving accurate telemetry data as a coreaudio work interval",
       T_META_ASROOT(YES), XNU_T_META_SOC_SPECIFIC,
       T_META_ENABLED(TARGET_CPU_ARM64)) {
  run_work_interval_data_test(DEFAULT_ITERS, DEFAULT_INTERVAL_NS,
                              DEFAULT_THREAD_COUNT,
                              true, // enable_telemetry
                              0);   // no added flags
}

T_DECL(work_interval_rt_coreaudio_telemetry_disabled,
       "reading telemetry data should see all zeroes if it isn't enabled",
       T_META_ASROOT(YES), XNU_T_META_SOC_SPECIFIC,
       T_META_ENABLED(TARGET_CPU_ARM64)) {
  run_work_interval_data_test(DEFAULT_ITERS, DEFAULT_INTERVAL_NS,
                              DEFAULT_THREAD_COUNT,
...
T_DECL(work_interval_rt_coreaudio_telemetry_data_many_threads,
       "work interval telemetry data works with many joined threads",
       T_META_ASROOT(YES), XNU_T_META_SOC_SPECIFIC,
       T_META_ENABLED(TARGET_CPU_ARM64)) {
  run_work_interval_data_test(DEFAULT_ITERS, DEFAULT_INTERVAL_NS,
                              20,   // threads
...
T_DECL(work_interval_rt_coreaudio_telemetry_supported_with_other_flags,
       "telemetry supported when the other creation flags used by coreaudio "
       "are set",
       T_META_ASROOT(YES), XNU_T_META_SOC_SPECIFIC,
       T_META_ENABLED(TARGET_CPU_ARM64)) {
  ...
  T_LOG("Coreaudio work interval with auto-join, deferred finish, and "
        "unrestricted flags enabled");
  run_work_interval_data_test(DEFAULT_ITERS, DEFAULT_INTERVAL_NS,
                              DEFAULT_THREAD_COUNT, // threads
                              true,                 // enable_telemetry
                              WORK_INTERVAL_FLAG_ENABLE_AUTO_JOIN |
