...
T_GLOBAL_META(T_META_NAMESPACE("xnu.poll"), T_META_RUN_CONCURRENTLY(true));
...
T_DECL(sleep_with_no_fds,
  ...
  T_LOG("sleep(%d) ~= %llu mach absolute time units", SLEEP_TIME_SECS,
        sleep_time);

  begin_time = mach_absolute_time();
  T_ASSERT_POSIX_SUCCESS(poll(&pfd, 0, POLL_TIMEOUT_MS),
                         "poll() with 0 events and timeout %d ms",
                         POLL_TIMEOUT_MS);
 ...
 */
T_DECL(directories, "poll() with directories should return an error",
       T_META_TAG_VM_PREFERRED) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(file, "open(%s)", LAUNCHD_PATH);
  dir = open(".", O_RDONLY | O_NONBLOCK);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(dir, "open(\".\")");
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(pipe(pipes), NULL);

  /* just directory */
  pfd[0].fd = dir;
  T_EXPECT_POSIX_SUCCESS(poll(pfd, 1, -1), "poll() with a directory");
  T_QUIET;
  T_EXPECT_TRUE(pfd[0].revents & POLLNVAL,
  ...
  pfd[1].fd = dir;
  pfd[1].revents = 0;
  T_EXPECT_POSIX_SUCCESS(poll(pfd, 2, -1), "poll() with a file and directory");
  T_QUIET;
  T_EXPECT_TRUE(pfd[0].revents & POLLIN, "file should be readable");
  T_QUIET;
  T_EXPECT_TRUE(pfd[1].revents & POLLNVAL,
                "directory should be an invalid event");
  ...
  T_EXPECT_TRUE(pfd[0].revents & POLLNVAL,
                "directory should be an invalid event");
  T_QUIET;
  T_EXPECT_TRUE(pfd[1].revents & POLLIN, "file should be readable");
  ...
  pfd[1].fd = pipes[0];
  pfd[0].revents = 0;
  T_EXPECT_POSIX_SUCCESS(poll(pfd, 2, -1), "poll() with a file and pipe");
  T_QUIET;
  T_EXPECT_TRUE(pfd[0].revents & POLLIN, "file should be readable");
  T_QUIET;
  ...
  T_EXPECT_TRUE(pfd[1].revents & POLLNVAL,
                "directory should be an invalid event");
  T_QUIET;
  T_EXPECT_FALSE(pfd[2].revents & POLLIN, "pipe should not be readable");
  ...
  pfd[1].revents = 0;
  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, PIPE_DIR_TIMEOUT_SECS * NSEC_PER_SEC),
      dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
        T_ASSERT_FALSE(timed_out, "poll timed out after %d seconds",
                       PIPE_DIR_TIMEOUT_SECS);
      });

  T_EXPECT_POSIX_SUCCESS(poll(pfd, 3, -1), "poll() with a directory and pipe");
  timed_out = false;
...
static void *leak_thread(void *ptr) {
  ...
  while (1) {
    ...
    // PT: Note that this is exposed by libproc but isn't declared in the
    // header, hence the forward declaration here. It seems fine to expose this
    // in the header, but I'm unsure of the implications.
...
T_DECL(poll_dont_leak_kernel_pointers,
       "poll and proc_pidinfo should not leak kernel pointers",
       T_META_TAG_VM_PREFERRED) {
  pthread_t thr;
