...
T_GLOBAL_META(T_META_CHECK_LEAKS(false), T_META_LTEPHASE(LTE_POSTINIT));
...
static void do_test(int notify_type, void (^trigger_block)(void)) {
  ...
  T_ASSERT_MACH_SUCCESS(
      host_request_notification(mach_host_self(), notify_type, port), NULL);
...
T_DECL(host_notify_calendar_change,
       "host_request_notification(HOST_NOTIFY_CALENDAR_CHANGE)") {
...
T_DECL(host_notify_calendar_set,
       "host_request_notification(HOST_NOTIFY_CALENDAR_SET)") {
...
T_DECL(host_notify_twice,
  ...
  T_ASSERT_MACH_SUCCESS(host_request_notification(
                            mach_host_self(), HOST_NOTIFY_CALENDAR_SET, port),
