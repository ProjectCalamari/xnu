...
static void sysctl_set_integer(const char *name, int val, int *restore_val,
                               bool *was_set) {
  int error;
  size_t len;
...
static void sysctl_restore_integer(const char *name, int restore_val,
                                   bool was_set) {
  if (was_set) {
    int error;
...
static void fake_set_bsd_mode(bool enable) {
  sysctl_set_integer(FAKE_SYSCTL_BSD_MODE, enable ? 1 : 0, &fake_bsd_mode,
                     &fake_bsd_mode_was_set);
}

static void fake_restore_bsd_mode(void) {
  sysctl_restore_integer(FAKE_SYSCTL_BSD_MODE, fake_bsd_mode,
                         fake_bsd_mode_was_set);
}

static void fake_set_vlan_tagging(bool enable) {
  sysctl_set_integer(FAKE_SYSCTL_VLAN_TAGGING, enable ? 1 : 0,
                     &fake_vlan_tagging, &fake_vlan_tagging_was_set);
}

static void fake_restore_vlan_tagging(void) {
...
#if !TARGET_OS_BRIDGE
#if !TARGET_OS_OSX
static bool vlan_is_enabled(void) {
...
static void test_traffic_for_network_interfaces(network_interface_t one,
  ...
  inet_test_traffic(af, &server, one->if_name, one->if_index, two->if_name,
                    two->if_index);
}

static void test_traffic_for_pair(network_interface_pair_t pair, uint8_t af) {
  test_traffic_for_network_interfaces(&pair->one, &pair->two, af);
}

static void test_traffic_for_af(uint8_t af) {
...
static void initialize_vlan_pairs(u_int n, bool need_address) {
...
static void vlan_send_short_packet(void) {
  ether_addr_t eaddr;
...
static void vlan_test_check_skip(void) {
#if TARGET_OS_BRIDGE
...
static void vlan_test_traffic(bool hw_vlan) {
...
static void vlan_test_bridged_traffic(bool hw_vlan) {
  errno_t err;
  network_interface_pair_t feth_pair;
...
static void vlan_test_short_packet(void) {
  vlan_test_check_skip();
...
T_DECL(net_if_vlan_test_software_tagging, "vlan test traffic software tagging",
       T_META_ASROOT(true)) {
  vlan_test_traffic(false);
}

T_DECL(net_if_vlan_test_hardware_tagging, "vlan test hardware tagging",
       T_META_ASROOT(true)) {
  vlan_test_traffic(true);
}

T_DECL(net_if_vlan_test_software_tagging_bridged,
       "vlan test traffic software tagging bridged", T_META_ASROOT(true)) {
  vlan_test_bridged_traffic(false);
}

T_DECL(net_if_vlan_test_hardware_tagging_bridged,
       "vlan test hardware tagging bridged", T_META_ASROOT(true)) {
  vlan_test_bridged_traffic(true);
}

T_DECL(net_if_vlan_test_short_packet, "vlan test short packet",
       T_META_ASROOT(true)) {
