/* compile: xcrun -sdk macosx.internal clang -ldarwintest -o iopolicy iopolicy.c
 * -g -Weverything */
...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vfs.iopolicy"),
...
static void exit_cleanup(void) { (void)remove(g_testfile); }
...
T_DECL(iopol_type_vfs_disallow_rw_for_o_evtonly,
  ...
  T_ASSERT_NE(err, -1,
              "setiopolicy_np(IOPOL_TYPE_VFS_DISALLOW_RW_FOR_O_EVTONLY, "
              "IOPOL_SCOPE_PROCESS, 1)");

  T_WITH_ERRNO;
  err = getiopolicy_np(IOPOL_TYPE_VFS_DISALLOW_RW_FOR_O_EVTONLY,
                       IOPOL_SCOPE_PROCESS);
  T_ASSERT_EQ(err, IOPOL_VFS_DISALLOW_RW_FOR_O_EVTONLY_ON,
              "getiopolicy_np(IOPOL_TYPE_VFS_DISALLOW_RW_FOR_O_EVTONLY, "
              "IOPOL_SCOPE_PROCESS)");

  T_WITH_ERRNO;
  err = getiopolicy_np(IOPOL_TYPE_VFS_HFS_CASE_SENSITIVITY, IOPOL_SCOPE_THREAD);
  T_ASSERT_TRUE((err == -1) && (errno == EINVAL),
  ...
  err = setiopolicy_np(IOPOL_TYPE_VFS_DISALLOW_RW_FOR_O_EVTONLY,
                       IOPOL_SCOPE_PROCESS,
                       IOPOL_VFS_DISALLOW_RW_FOR_O_EVTONLY_OFF);
  T_ASSERT_TRUE((err == -1) && (errno == EINVAL),
                "setiopolicy_np(IOPOL_TYPE_VFS_DISALLOW_RW_FOR_O_EVTONLY, "
  ...
  T_ASSERT_TRUE((err == -1) && (errno == EBADF), "Trying to write: %s",
                g_testfile);

  T_WITH_ERRNO;
  err = (int)read(fd, g_testdata, sizeof(g_testdata));
  T_ASSERT_TRUE((err == -1) && (errno == EBADF), "Trying to read: %s",
                g_testfile);

  T_WITH_ERRNO;
  mapped =
      mmap(NULL, sizeof(g_testdata), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
