...
T_DECL(posix_spawn_alt_rosetta,
       "verify posix_spawn_set_alt_rosetta_np switches to alternative rosetta "
       "runtime",
  ...
  if (access("/Library/Apple/usr/libexec/oah/libRosettaRuntime", O_RDONLY) !=
      0) {
  ...
  if (access("/usr/local/libexec/rosetta/runtime_internal", O_RDONLY) != 0) {
    ...
    system("cp /Library/Apple/usr/libexec/rosetta/runtime "
  ...
  T_ASSERT_EQ(WIFEXITED(wait_ret), 1,
              "child process should have called exit()");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "posix_spawnattr_setarchpref_np");
  ...
  T_ASSERT_EQ(WIFEXITED(wait_ret), 1,
              "child process should have called exit()");
  T_ASSERT_EQ(WEXITSTATUS(wait_ret), 2,
              "running with alternative rosetta runtime should return 2");

  ret = posix_spawnattr_destroy(&spawnattr);
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "posix_spawnattr_destroy");
#else
