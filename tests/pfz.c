...
#define SIGN_COMMPAGE_PFZ_BASE_PTR(ptr)                                        \
  ptrauth_sign_unauthenticated(ptr, COMMPAGE_PFZ_BASE_AUTH_KEY,                \
                               COMMPAGE_PFZ_BASE_DISCRIMINATOR)
#define AUTH_COMMPAGE_PFZ_BASE_PTR(ptr)                                        \
  ptrauth_auth_data(ptr, COMMPAGE_PFZ_BASE_AUTH_KEY,                           \
                    COMMPAGE_PFZ_BASE_DISCRIMINATOR)
...
T_DECL(test_rdar_65270017, "Testing for rdar 65270017",
  ...
  struct OSAtomicFifoHeadWrapper {
    ...
    int data;
  } wrapped_head = {.first = NULL, .last = NULL, .opaque = 0, .data = 0xfeed};
...
void preheat(dispatch_queue_t dq) {
...
T_DECL(test_65270017_contended, "multithreaded testing for radar 65270017") {
  ...
  dispatch_queue_t queue =
      dispatch_queue_create("com.apple.libctests.threaded", 0);
  ...
  dispatch_apply(WIDE, global_q, ^(size_t i) {
    dispatch_apply(SMALL, global_q, ^(size_t idx) {
      OSAtomicFifoEnqueue(q1, &(elements[i][idx]),
                          offset); // contended enqueue on q1
    });
    ...
    do {
      ...
      T_QUIET;
      T_ASSERT_EQ(wrapped_q_head1.data, 0xfeed, "q1 data is valid");
      if (p) {
        OSAtomicFifoEnqueue(q2, p, offset);
        T_QUIET;
        T_ASSERT_EQ(wrapped_q_head2.data, 0xdead, "q2 data is valid");
        count++;
      }
    } while (p != NULL);
