...
static void network_interface_init(network_interface_t netif, const char *name,
                                   unsigned int unit,
...
static void bond_add_member(if_name_t bond, if_name_t member) {
  struct ifreq ifr;
  ...
  strlcpy(ibr.ibr_ibru.ibru_if_name, member, sizeof(ibr.ibr_ibru.ibru_if_name));

  bzero(&ifr, sizeof(ifr));
  strlcpy(ifr.ifr_name, bond, sizeof(ifr.ifr_name));
  ifr.ifr_data = (caddr_t)&ibr;
  result = ioctl(s, SIOCSIFBOND, &ifr);
  T_ASSERT_POSIX_SUCCESS(result, "SIOCSIFBOND(%s) %s", bond, member);
}

static void initialize_bond_pair(u_int n) {
...
static void vlan_interface_init(network_interface_t netif, const if_name_t phys,
...
static void initialize_vlan_pairs(u_int n) {
  network_interface_pair_t scan;
  ...
  for (size_t i = 0; i < n; i++, scan++) {
    ...
    vlan_interface_init(&scan->one, S_bond_pair.one.if_name, name, tag, 1);
    snprintf(name, sizeof(name), "%s%d", VLAN_NAME, vlan_unit++);
    vlan_interface_init(&scan->two, S_bond_pair.two.if_name, name, tag, 2);
  }
}

static void bond_test_traffic(bool hw_vlan) {
#if !TARGET_OS_OSX
...
T_DECL(net_if_bond_test_software_tagging, "bond test traffic software tagging",
       T_META_ASROOT(true)) {
  bond_test_traffic(false);
}

T_DECL(net_if_bond_test_hardware_tagging, "bond test hardware tagging",
       T_META_ASROOT(true)) {
