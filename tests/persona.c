...
T_GLOBAL_META(T_META_NAMESPACE("xnu.persona"), T_META_CHECK_LEAKS(false),
              T_META_RUN_CONCURRENTLY(true),
              T_META_ENABLED(!TARGET_OS_WATCH) // rdar://81809878
);

static uid_t _create_persona(int persona_type, uint32_t persona_info_version) {
...
T_DECL(persona_info_v2, "create and query persona PERSONA_INFO_V2",
  ...
  for (uint32_t version = PERSONA_INFO_V1; version <= PERSONA_INFO_V2;
       version++) {
    ...
    T_ASSERT_EQ(info.persona_type, PERSONA_MANAGED, "kpersona_info(v%d) type",
                version);
    T_ASSERT_EQ(info.persona_info_version, version,
                "kpersona_info(v%d) version", version);
  }
...
T_DECL(persona_v1_uid_is_unset,
       "create PERSONA_INFO_V1 and make sure its UID is unset",
       T_META_TAG_VM_PREFERRED) {
