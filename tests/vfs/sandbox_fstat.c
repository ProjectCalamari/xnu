 ...
 * sandbox_fstat sandbox_fstat.c -g -Weverything */
...
static void do_test(int expected_error) {
  int fd;
  struct stat sb;

  /* Test stat() */
  if (expected_error) {
    T_EXPECT_POSIX_FAILURE(stat(file, &sb), expected_error,
                           "Calling stat() should FAIL with '%s'",
                           strerror(expected_error));
  } else {
    T_EXPECT_POSIX_SUCCESS(stat(file, &sb),
                           "Calling stat() for the file should PASS");
  }

  /* Test fstat() while the file is open with the O_CREAT | O_WRONLY flags  */
  T_EXPECT_POSIX_SUCCESS(fd = open(file, O_CREAT | O_WRONLY, 0666),
                         "Opening with the O_CREAT | O_WRONLY flags");
  if (fd != -1) {
    if (expected_error) {
      T_EXPECT_POSIX_FAILURE(fstat(fd, &sb), expected_error,
                             "Calling fstat() should FAIL with '%s'",
                             strerror(expected_error));
    } else {
      T_EXPECT_POSIX_SUCCESS(fstat(fd, &sb),
                             "Calling fstat() for the test file should PASS");
    }
    close(fd);
  }

  T_EXPECT_POSIX_SUCCESS(fd = open(file_rsrcfork, O_CREAT | O_WRONLY, 0666),
                         "Opening rsrcfork with the O_CREAT | O_WRONLY flags");
  if (fd != -1) {
    T_EXPECT_POSIX_SUCCESS(fstat(fd, &sb),
                           "Calling fstat() for the rsrcfork should PASS");
    close(fd);
  }
}

T_DECL(sandbox_fstat,
       "Prevent the information disclosure on files opened with O_WRONLY while "
       "sandbox profile denies 'file-read-metadata'") {
#if (!RUN_TEST)
  T_SKIP("Not macOS");
#endif
  ...
  snprintf(file_rsrcfork, sizeof(file_rsrcfork), "%s/%s", file,
           _PATH_RSRCFORKSPEC);
