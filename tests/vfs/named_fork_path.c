 ...
 * named_fork_path named_fork_path.c -g -Weverything */
...
T_DECL(named_fork_path, "Named fork paths to check file-read-xattr and "
                        "file-write-xattr Sandbox permissions") {
#if (!RUN_TEST)
  T_SKIP("Not macOS");
#endif
  ...
  snprintf(template, sizeof(template), "%s/named_fork_path-XXXXXX",
           dt_tmpdir());
  ...
  T_ASSERT_POSIX_SUCCESS((fd = open(file, O_CREAT | O_RDWR, 0777)),
                         "Creating %s", file);
  T_ASSERT_POSIX_SUCCESS((fd2 = open(file2, O_CREAT | O_RDWR, 0777)),
                         "Creating %s", file2);
  T_ASSERT_POSIX_SUCCESS((fd3 = open(rsrc, O_CREAT | O_RDWR, 0777)),
                         "Creating %s", rsrc);

  /* Set ResourceFork extended attribute */
  T_ASSERT_POSIX_SUCCESS(
      fsetxattr(fd, XATTR_RESOURCEFORK_NAME, xattr, xattr_len, 0, 0),
      "Setting ResourceFork of %s to '%s'", file, xattr);
  ...
  T_ASSERT_POSIX_FAILURE(rename(file_rfork, rsrc), EPERM,
                         "Verifying rename from an ..namedfork/rsrc path isn't "
                         "a supported (EPERM)");
  T_ASSERT_POSIX_FAILURE(rename(rsrc, file_rfork), EPERM,
                         "Verifying trename to an ..namedfork/rsrc path isn't "
                         "a supported (EPERM)");

  /* Read ResourceFork extended attribute using getxattr() */
  T_ASSERT_EQ((ssize_t)xattr_len,
              fgetxattr(fd, XATTR_RESOURCEFORK_NAME, xattr_buff,
                        sizeof(xattr_buff), 0, 0),
              "Trying to get ResourceFork extended attribute");
  T_ASSERT_EQ(0, strncmp(xattr, xattr_buff, xattr_len),
              "Verifying ResourceFork extended content");

  /* Read ResourceFork extended attribute using the ..namedfork/rsrc path */
  T_ASSERT_POSIX_SUCCESS((fd_rfork = open(file_rfork, O_RDONLY, 0777)),
                         "Opening %s", file_rfork);
  T_ASSERT_EQ((ssize_t)xattr_len,
              read(fd_rfork, xattr_buff, sizeof(xattr_buff)),
              "Trying to read ResourceFork extended attribute");
  T_ASSERT_EQ(0, strncmp(xattr, xattr_buff, xattr_len),
              "Verifying ResourceFork extended content");
  ...
  /* Test ResourceFork extended attribute using fgetxattr(), fsetxattr() and
   * fremovexattr() */
  T_ASSERT_POSIX_FAILURE(
      fgetxattr(fd, XATTR_RESOURCEFORK_NAME, xattr_buff, sizeof(xattr_buff), 0,
                0),
      EPERM, "Verifying that fgetxattr() fails to get ResourceFork with EPERM");
  T_ASSERT_POSIX_FAILURE(
      fremovexattr(fd, XATTR_RESOURCEFORK_NAME, 0), EPERM,
      "Verifying that fremovexattr() fails to remove ResourceFork with EPERM");
  T_ASSERT_POSIX_FAILURE(
      fsetxattr(fd2, XATTR_RESOURCEFORK_NAME, xattr, xattr_len, 0, 0), EPERM,
      "Verifying that fsetxattr() fails to set ResourceFork with EPERM");

  /* Test ResourceFork extended attribute using the ..namedfork/rsrc path */
  T_ASSERT_POSIX_FAILURE((fd_rfork = open(file_rfork, O_RDONLY, 0777)), EPERM,
                         "Verifying that open() fails with EPERM");
  T_ASSERT_POSIX_FAILURE(
      (fd_rfork = open(file2_rfork, O_CREAT | O_RDONLY, 0777)), EPERM,
      "Verifying that open(O_CREAT) fails with EPERM");
  T_ASSERT_POSIX_FAILURE(unlink(file_rfork), EPERM,
                         "Verifying that unlink() fails with EPERM");
