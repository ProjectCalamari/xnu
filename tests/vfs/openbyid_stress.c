...
/* compile: xcrun -sdk macosx.internal clang -ldarwintest -o openbyid_stress
 * openbyid_stress.c -g -Weverything */
/* sign: codesign --force --sign - --timestamp=none --entitlements
 * openbyid_stress.entitlements openbyid_stress */
...
T_DECL(openbyid_stress, "Test that openbyid_np does not open the wrong file") {
  int fd;
  ...
  T_ASSERT_NOTNULL((timeout_source = dispatch_source_create(
                        DISPATCH_SOURCE_TYPE_TIMER, 0, 0, queue)),
                   "Creating dispatch source");

  dispatch_source_set_timer(timeout_source,
  ...
  snprintf(template, sizeof(template), "%s/openbyid_stress-XXXXXX",
           dt_tmpdir());
  ...
  T_ASSERT_POSIX_SUCCESS((fd = open(file1, O_CREAT | O_RDWR, 0777)),
                         "Creating %s", file1);
  T_ASSERT_POSIX_SUCCESS(close(fd), "Closing %s", file1);

  T_ASSERT_POSIX_SUCCESS((fd = open(file2, O_CREAT | O_RDWR, 0777)),
                         "Creating %s", file2);
  T_ASSERT_POSIX_SUCCESS(close(fd), "Closing %s", file2);

  T_ASSERT_POSIX_SUCCESS(stat(file1, &buf_stat), "Calling stat() on %s", file1);
  T_ASSERT_POSIX_SUCCESS(statfs(file1, &buf_statfs), "Calling statfs() on %s",
                         file1);

  T_LOG("File successfully opened: fsid {%d, %d}, inode %llu",
        buf_statfs.f_fsid.val[0], buf_statfs.f_fsid.val[1], buf_stat.st_ino);
  ...
  while (!timeout && !error) {
    ...
    if ((fd2 = openbyid_np(&buf_statfs.f_fsid, (fsobj_id_t *)&buf_stat.st_ino,
                           0)) < 0) {
    ...
    if (buf_statfs.f_fsid.val[0] != buf_statfs2.f_fsid.val[0] ||
      ...
      T_FAIL("Wrong file opened! fsid {%d, %d}, inode %llu",
             buf_statfs2.f_fsid.val[0], buf_statfs2.f_fsid.val[1],
             buf_stat2.st_ino);
      error = EINVAL;
      close(fd2);
      break;
