...
/* compile: xcrun -sdk macosx.internal clang -ldarwintest -o
 * getattrlist_mountextflags getattrlist_mountextflags.c -g -Weverything */
...
/* rdar://137970358: Disable the test for now until the root cause was
 * determined */
#if 0
...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vfs"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("vfs"), T_META_ENABLED(RUN_TEST),
              T_META_ASROOT(false), T_META_CHECK_LEAKS(false));

static void get_mount_path(const char *diskid, char *path) {
  int i, mntsize;
...
static void test_getattrlist(const char *path, const char *fstypename,
  ...
  T_ASSERT_POSIX_SUCCESS(
      getattrlist(path, &attrs, &attrbuf, sizeof(attrbuf),
  ...
  T_ASSERT_BITS_SET(
      attrbuf.returned_attrs.volattr,
      ATTR_VOL_FSTYPENAME | ATTR_VOL_MOUNTEXTFLAGS,
...
T_DECL(getattrlist_mountextflags, "test ATTR_VOL_MOUNTEXTFLAGS") {
#if (!RUN_TEST)
  ...
  snprintf(template, sizeof(template), "%s/getattrlist_mountextflags-XXXXXX",
           dt_tmpdir());
  ...
  /* Create disk image */
  snprintf(args, sizeof(args), "diskimagetool create -fs none -s 1m %s",
           image_path);
  T_ASSERT_POSIX_SUCCESS(do_exec(args), "Creating disk image %s", image_path);

  /* Attach disk image */
  snprintf(args, sizeof(args), "diskimagetool attach --external %s",
           image_path);
  T_ASSERT_POSIX_SUCCESS(do_exec(args), "Attaching disk image %s", image_path);

  /* Extract device identifier */
  T_ASSERT_POSIX_NOTNULL((diskp = strstr(output_buffer, "disk")),
                         "Extracting device identifier: %s", diskp);
  strlcpy(disk, diskp, PATH_MAX);

  /* Execute newfs_msdos disk image */
  snprintf(args, sizeof(args), "newfs_msdos -v MSDOS %s", disk);
  T_ASSERT_POSIX_SUCCESS(do_exec(args), "Executing newfs_msdos on disk %s",
                         disk);
  ...
  T_ASSERT_NE_STR(mount_path, "", "Got msdos filesystem mount path %s",
                  mount_path);
