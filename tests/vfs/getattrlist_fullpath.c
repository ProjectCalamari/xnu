 ...
 * getattrlist_fullpath getattrlist_fullpath.c -g -Weverything */
...
static char *fast_realpath(const char *path, bool follow) {
  struct {
    uint32_t size;
...
static void test_realpath(char *input, char *output) {
  T_ASSERT_EQ_STR(fast_realpath(input, false), output,
                  "Testing input '%s', output '%s'", input, output);
}

T_DECL(getattrlist_fullpath, "getattrlist ATTR_CMN_FULLPATH should preserve "
                             "input path prefix in output") {
  ...
  test_realpath("/.resolve/1/private/etc/hosts",
                "/.resolve/1/private/etc/hosts");
}
