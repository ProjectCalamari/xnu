/* compile: xcrun -sdk macosx.internal clang -ldarwintest -o o_search o_search.c
 * -g -Weverything */
...
static void exit_cleanup(void) { (void)unlink(g_testfile); }

T_DECL(o_search, "test O_SEARCH for open", T_META_ASROOT(false)) {
  ...
  T_ASSERT_POSIX_ZERO(chdir(tmpdir), "Setup: changing to tmpdir: %s", tmpdir);
  ...
  T_ASSERT_POSIX_SUCCESS(retval =
                             (int)write(fd, g_testfile, sizeof(g_testfile)),
                         "Write: %s", g_testfile);
  ...
  T_ASSERT_TRUE(
      (tmpdir_fd == -1) && (errno == EISDIR),
      "Trying to open directory O_EXEC: %s, tmpdir_fd = %d, errno = %d",
      g_testfile, tmpdir_fd, errno);

  T_ASSERT_POSIX_SUCCESS(tmpdir_fd = open(tmpdir, O_RDONLY), NULL);
  T_ASSERT_POSIX_SUCCESS(retval = (int)__getdirentries64(
                             tmpdir_fd, namebuf, sizeof(namebuf), &dirbyte),
                         NULL);
  T_ASSERT_POSIX_SUCCESS(close(tmpdir_fd), NULL);

  T_ASSERT_POSIX_SUCCESS(tmpdir_fd = open(tmpdir, O_SEARCH), NULL);
  retval =
      (int)__getdirentries64(tmpdir_fd, namebuf, sizeof(namebuf), &dirbyte);
  T_ASSERT_TRUE((retval == -1) && (errno == EBADF),
                "Trying to read directory opened with O_SEARCH: %s, retval = "
                "%d, errno = %d",
                tmpdir, retval, errno);

  fd = openat(tmpdir_fd, TEST_FILE, O_EXEC);
  T_ASSERT_TRUE((fd == -1) && (errno == EACCES),
                "Trying to open file for execute with perms 644: %s, retval = "
                "%d, errno = %d",
                tmpdir, retval, errno);

  T_ASSERT_POSIX_SUCCESS(retval = fchmodat(tmpdir_fd, TEST_FILE, 0744, 0),
                         NULL);
  ...
  T_ASSERT_TRUE((mapped == MAP_FAILED) && (errno == EACCES),
                "Trying to mmap file for read opened with O_EXEC: %s, mapped = "
                "%ld, errno = %d",
                g_testfile, (long)mapped, errno);

  mapped = mmap(NULL, PAGE_SIZE, PROT_WRITE, MAP_SHARED, fd, 0);
  T_ASSERT_TRUE((mapped == MAP_FAILED) && (errno == EACCES),
                "Trying to mmap file for write opened with O_EXEC: %s, mapped "
                "= %ld, errno = %d",
                g_testfile, (long)mapped, errno);
