...
/* compile: xcrun -sdk macosx.internal clang -ldarwintest -o linkat_flags
 * linkat_flags.c -g -Weverything */
...
static void verify_stat(nlink_t file_nlink, nlink_t sym_nlink) {
  int error;
  struct stat buf;
  ...
  if (error) {
    T_ASSERT_FAIL("Calling fstatat for the file failed with %s",
                  strerror(errno));
  }
  T_ASSERT_EQ(file_nlink, buf.st_nlink, "Validating file's nlink count of %d",
              file_nlink);
  ...
  if (error) {
    T_ASSERT_FAIL("Calling fstatat for the symlink failed with %s",
                  strerror(errno));
  }
  T_ASSERT_EQ(sym_nlink, buf.st_nlink, "Validating symlink's nlink count of %d",
              sym_nlink);
}
...
T_DECL(linkat_flags,
  ...
  T_ASSERT_POSIX_SUCCESS((fd = open(testdir, O_SEARCH, 0777)),
                         "Opening the test root directory");
  ...
  T_ASSERT_POSIX_SUCCESS((fd = open(file, O_CREAT | O_RDWR, 0777)),
                         "Creating %s", file);
  T_ASSERT_POSIX_SUCCESS(symlink(file, sym),
                         "Creating symbolic link %s ---> %s", sym, file);
  T_ASSERT_POSIX_SUCCESS(symlink(testdir_path, dirloop),
                         "Creating symbolic link %s ---> %s", dirloop,
                         testdir_path);
  ...
  {
    T_ASSERT_POSIX_SUCCESS(linkat(AT_FDCWD, file, AT_FDCWD, lfile1, 0),
                           "Calling linkat() while name1 is a file");
    verify_stat(2, 1);

    T_ASSERT_POSIX_SUCCESS(linkat(AT_FDCWD, sym, AT_FDCWD, lfile2, 0),
                           "Calling linkat() while name1 is a symbolic link");
    verify_stat(2, 2);

    T_ASSERT_POSIX_SUCCESS(linkat(AT_FDCWD, symloop, AT_FDCWD, lfile3, 0),
                           "Calling linkat() while name1 is a symbolic link "
                           "and it's path contains a symbolic");
    verify_stat(2, 3);
  }
  ...
  T_LOG("Testing linkat() using the AT_SYMLINK_FOLLOW flag");
  {
    T_ASSERT_POSIX_SUCCESS(
        linkat(AT_FDCWD, file, AT_FDCWD, lfile4, AT_SYMLINK_FOLLOW),
        "Calling linkat() while name1 is a file");
    verify_stat(3, 3);

    T_ASSERT_POSIX_SUCCESS(
        linkat(AT_FDCWD, sym, AT_FDCWD, lfile5, AT_SYMLINK_FOLLOW),
        "Calling linkat() while name1 is a symbolic link");
    verify_stat(4, 3);

    T_ASSERT_POSIX_SUCCESS(
        linkat(AT_FDCWD, symloop, AT_FDCWD, lfile6, AT_SYMLINK_FOLLOW),
        "Calling linkat() while name1 is a symbolic link and it's path "
        "contains a symbolic");
    verify_stat(5, 3);
  }
  ...
  {
    T_ASSERT_POSIX_SUCCESS(
        linkat(AT_FDCWD, file, AT_FDCWD, lfile7, AT_SYMLINK_NOFOLLOW_ANY),
        "Calling linkat() while name1 is a file %s", file);
    verify_stat(6, 3);

    T_ASSERT_POSIX_SUCCESS(
        linkat(AT_FDCWD, sym, AT_FDCWD, lfile8, AT_SYMLINK_NOFOLLOW_ANY),
        "Calling linkat() while name1 is a symbolic link");
    verify_stat(6, 4);

    T_ASSERT_POSIX_FAILURE(linkat(AT_FDCWD, symloop, AT_FDCWD, "invalid_path",
