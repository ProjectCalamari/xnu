/* compile: xcrun -sdk macosx.internal clang -ldarwintest -o getattrlist_fstype
 * getattrlist_fstype.c -g -Weverything */
...
T_DECL(getattrlist_fstype, "test ATTR_VOL_FSTYPENAME and ATTR_VOL_FSSUBTYPE",
       T_META_ASROOT(false)) {
  struct myattrbuf {
    uint32_t length;
  ...
  T_WITH_ERRNO;
  T_ASSERT_POSIX_ZERO(statfs(tmpdir, &sfs), "Setup: statfs'ing tmpdir: %s",
                      tmpdir);
  ...
  T_WITH_ERRNO;
  T_ASSERT_POSIX_ZERO(
      getattrlist(tmpdir, &attrs, &attrbuf, sizeof(attrbuf),
  ...
  T_ASSERT_TRUE(attrbuf.fstypename_ref.attr_dataoffset +
