...
T_DECL(lseek_not_allowed_on_pipe,
  ...
  T_ASSERT_POSIX_FAILURE(lseek(fildes[0], 0, SEEK_CUR), ESPIPE,
                         "lseek fails with appropriate errno");
}

T_DECL(lseek_not_allowed_on_fifo,
  ...
  T_ASSERT_POSIX_SUCCESS(
      mkfifo(path_template, (S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)),
      "setup: created FIFO in temp file's place");
  ...
  // Then it fails
  // And errno is set appropriately
  T_ASSERT_POSIX_FAILURE(lseek(fifo_fd, 0, SEEK_CUR), ESPIPE,
                         "lseek fails with appropriate errno");
}

T_DECL(lseek_not_allowed_on_tty,
  ...
  T_ASSERT_POSIX_SUCCESS(openpty(&primary_fd, &replica_fd, NULL, NULL, NULL),
                         "setup: open PTY");
  ...
  // Then it fails
  // And errno is set appropriately
  T_ASSERT_POSIX_FAILURE(lseek(primary_fd, 0, SEEK_CUR), ESPIPE,
                         "lseek fails with appropriate errno");
}
