...
T_DECL(lstat_symlink_trailing_slash,
       "Check symlinks-to-symlinks are resolved correctly when trailing "
  ...
  snprintf(lstat_testdir, sizeof(lstat_testdir),
           "%s/symlink_trailing_slash-lstat-XXXXXX", dt_tmpdir());
  ...
  // rdar://142559105 (lstat() of a name with trailing '/' is handled
...
T_DECL(access_symlink_trailing_slash,
       "Check access returns ENOTDIR when symlink points to a file and "
       "trailing slash was used") {
  T_ATEND(cleanup_access);
  T_SETUPBEGIN;

  // Create test root dir
  snprintf(access_testdir, sizeof(access_testdir),
           "%s/symlink_trailing_slash-access-XXXXXX", dt_tmpdir());
  T_ASSERT_POSIX_NOTNULL(mkdtemp(access_testdir),
                         "setup: create test root dir");
  printf("testdir is %s\n", access_testdir);
  ...
  T_ASSERT_POSIX_SUCCESS(symlink("test.f", "test.lnk"),
                         "setup: ln test.f test.lnk");
  T_ASSERT_POSIX_SUCCESS(mkdir("test.d", 0755), "setup: mkdir test.d");
  T_ASSERT_POSIX_SUCCESS(creat("test.d/test.df", 0755),
                         "setup: touch test.d/test.df");
  ...
  // The trailing slash in test.lnk should not cause access("test.lnk/test.f")
  // to fail
  T_ASSERT_POSIX_SUCCESS(unlink("test.lnk"), "rm test.lnk");
  T_ASSERT_POSIX_SUCCESS(symlink("test.d/", "test.lnk"),
                         "ln -s test.d/ test.lnk");
  T_ASSERT_POSIX_SUCCESS(access("test.lnk/test.df", R_OK),
                         "access test.lnk/test.df");
}
