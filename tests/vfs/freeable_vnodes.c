/* xcrun -sdk iphoneos.internal clang -ldarwintest -o freeable_vnodes
 * freeable_vnodes.c -g -Weverything */
...
static char *dlpaths[] = {
    ...
    "HomeAutomationFlowDelegatePlugin.bundle/HomeAutomationFlowDelegatePlugin",
    "/System/Library/NanoPreferenceBundles/Applications/"
    "NanoPassbookBridgeSettings.bundle/NanoPassbookBridgeSettings",
    "/System/Library/Assistant/FlowDelegatePlugins/"
    "MessagesFlowDelegatePlugin.bundle/MessagesFlowDelegatePlugin",
    "/System/Library/NanoPreferenceBundles/Discover/"
    "HealthAndFitnessPlugin.bundle/HealthAndFitnessPlugin",
    "/System/Library/NanoPreferenceBundles/SetupBundles/"
    "DepthCompanionSetup.bundle/DepthCompanionSetup",
    "/System/Library/PreferenceBundles/DigitalSeparationSettings.bundle/"
    "DigitalSeparationSettings",
    "/System/Library/Assistant/FlowDelegatePlugins/NotebookFlowPlugin.bundle/"
    "NotebookFlowPlugin",
    "/System/Library/NanoPreferenceBundles/Discover/UserGuidePlugin.bundle/"
    "UserGuidePlugin",
    "/System/Library/PreferenceBundles/NotificationsSettings.bundle/"
    "NotificationsSettings",
    "/System/Library/PreferenceBundles/AssistantSettings.bundle/"
    "AssistantSettings",
    "/System/Library/PreferenceBundles/SettingsCellular.bundle/"
    "SettingsCellular",
    "/System/Library/PreferenceBundles/FocusSettings.bundle/FocusSettings",
    "/private/preboot/Cryptexes/App/System/Library/PreferenceBundles/"
...
static char *dlpaths[] = {
    ...
    "/System/Library/Assistant/FlowDelegatePlugins/"
    "MessagesFlowDelegatePlugin.bundle/MessagesFlowDelegatePlugin",
    "/System/Library/PreferenceBundles/NanoAccessibilitySettings.bundle/"
    "NanoAccessibilitySettings",
    "/System/Library/Assistant/FlowDelegatePlugins/NotebookFlowPlugin.bundle/"
    "NotebookFlowPlugin",
    NULL};
...
static char *dlpaths[] = {
    "/System/Library/Assistant/FlowDelegatePlugins/"
    "HomeAutomationFlowDelegatePlugin.bundle/Contents/MacOS/"
    "HomeAutomationFlowDelegatePlugin",
    "/System/Library/Assistant/FlowDelegatePlugins/"
    "MessagesFlowDelegatePlugin.bundle/Contents/MacOS/"
    "MessagesFlowDelegatePlugin",
    "/System/Library/Assistant/FlowDelegatePlugins/NotebookFlowPlugin.bundle/"
    "Contents/MacOS/NotebookFlowPlugin",
    NULL};

#else

static char *dlpaths[] = {NULL};

#endif

static uint32_t get_sysctl_int(char *sysctl_name) {
...
static void run_vnoder(void) {
  ...
  T_LOG("trying to map %u files, progress every %u files\n", num_files,
        log_interval);
  /*
   * This loop can take an ideterminate amount of time on different devices
   * since the it is based on file creation (which is rate limited by the SEP)
   * and the number we will attempt to create. At some point it should be
   * replaced by walking a large hierarchy (like /System) and mapping those
   * files instead of creating files.
   */
  for (i = 0; i < num_files; ++i) {
    if (i == next_test_log_number) {
      ...
      if (current_num >= (maxvnodes + 5000)) {
        T_LOG("numvnodes is >= maxvnodes + 5000 (%u), done with creation and "
              "mmap loop\n",
              current_num);
        break;
      }
    ...
    T_ASSERT_POSIX_SUCCESS(
        fd = openat(dir_fd, filepath, O_CREAT | O_RDWR, 0666), NULL);
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(buffer = mmap(NULL, PAGE_SIZE, PROT_READ,
...
T_DECL(vnode_max_increase,
   ...
   */
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(socketpair(AF_UNIX, SOCK_STREAM, 0, sock_fd), NULL);
  ...
  for (i = 0; i < 5 && dlpaths[0] != NULL; i++) {
    for (int j = 0; dlpaths[j] != NULL; j++) {
      ...
      if (stat(dlpaths[j], &sb) != -1) {
        ...
        T_QUIET;
        T_ASSERT_POSIX_SUCCESS(
            fcntl(dlpath_fd, F_GETPATH, &path),
            "path is %s, iteration number is %d and path number is %d",
            dlpaths[j], i + 1, j + 1);
        T_QUIET;
  ...
  for (i = 0; i < timeout; i++) {
    sleep(1);
    deallocateable_vnodes =
        get_sysctl_int("vfs.vnstats.num_deallocable_vnodes");
    deallocateable_busy_vnodes =
        get_sysctl_int("vfs.vnstats.num_deallocable_busy_vnodes");

    T_LOG("deallocateable_vnodes after %d second%s : %d", i + 1,
          (i == 0) ? "" : "s", deallocateable_vnodes);
    T_LOG("deallocateable_busy_vnodes after %d second%s : %d", i + 1,
          (i == 0) ? "" : "s", deallocateable_busy_vnodes);
