...
T_DECL(test_os_proc_available_memory, "Basic available memory") {
  ...
  T_EXPECT_NE(remainingBytes, 0ULL,
              "os_proc_available_memory() should not return 0");
  T_EXPECT_NE(vm_info.limit_bytes_remaining, 0ULL,
              "vm_info.limit_bytes_remaining should not return 0");
  T_EXPECT_EQ(vm_info.limit_bytes_remaining, remainingBytes,
              "task_info --rev4 call returned value 0x%llx for "
              "vm_info.limit_bytes_remaining. Expected 0x%llx",
  ...
  T_EXPECT_EQ(count, TASK_VM_INFO_REV4_COUNT,
              ...
              count);
  T_EXPECT_EQ(remainingBytes, 0ULL,
              "os_proc_available_memory() should return 0");
  T_EXPECT_EQ(vm_info.limit_bytes_remaining, 0ULL,
              "vm_info.limit_bytes_remaining should return 0");
  T_EXPECT_EQ(vm_info.limit_bytes_remaining, remainingBytes,
              "task_info --rev4 call returned value 0x%llx for "
              "vm_info.limit_bytes_remaining. Expected 0x%llx",
              vm_info.limit_bytes_remaining, remainingBytes);
...
static int set_memlimit(pid_t pid, int32_t limit_mb) {
  ...
  return memorystatus_control(MEMORYSTATUS_CMD_SET_MEMLIMIT_PROPERTIES, pid, 0,
...
T_DECL(test_os_proc_available_memory, "Basic available memory") {
   ...
   * hard memory limit.
   */
  ret = memorystatus_control(MEMORYSTATUS_CMD_SET_PROCESS_IS_MANAGED, pid, 0,
                             NULL, 0);
  T_QUIET;
  ...
  ret = memorystatus_control(MEMORYSTATUS_CMD_SET_PROCESS_IS_MANAGED, pid, 1,
                             NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "memorystatus_control");
  available_memory = __memorystatus_available_memory();
