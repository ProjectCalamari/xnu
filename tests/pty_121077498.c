...
static void sigio_handler(__unused int sig) {
...
T_HELPER_DECL(pty_121077498_impl, "fork helper") {
  ...
  T_EXPECT_POSIX_FAILURE(read(primary, &c, 1), EAGAIN,
                         "read with no data returns EAGAIN");

  /* Set us up to detect when we get a SIGIO. */
  T_EXPECT_TRUE(SIG_ERR != signal(SIGIO, sigio_handler), "set SIGIO handler");
  T_EXPECT_POSIX_SUCCESS(sigprocmask(SIG_BLOCK, &sigio_set, NULL),
                         "block SIGIO");
  ...
  T_ASSERT_TRUE(sigismember(&received_set, SIGIO),
                "received SIGIO when we flushed");

  /* Reset state. */
  T_EXPECT_TRUE(SIG_ERR != signal(SIGIO, SIG_IGN), "reset SIGIO handler");
  T_EXPECT_POSIX_SUCCESS(sigprocmask(SIG_UNBLOCK, &sigio_set, NULL),
                         "unblock SIGIO");
...
T_DECL(pty_121077498, "Ability to use O_NONBLOCK and O_ASYNC on pty primary "
                      "without replica open.") {
  /*
