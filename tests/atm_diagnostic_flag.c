...
static uint32_t _save_atm_diagnostic_flag(void) {
  kern_return_t kr;
  kr = host_get_atm_diagnostic_flag(mach_host_self(), &_original);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "host_get_atm_diagnostic_flag()");
  T_LOG("Original ATM diagnostic flag: 0x%08x", _original);
  return _original;
}

static kern_return_t _mutate_atm_diagnostic_flag(uint32_t v) {
...
static void _reset_atm_diagnostic_flag(void) {
  if (!_needs_reset) {
    return;
  }
...
static void _toggle_atm_diagnostic_flag(void) {
  ...
  T_EXPECT_MACH_ERROR(KERN_NO_ACCESS, kr, "Deny change to atm_diagnostic_flag");
}
...
T_DECL(
    atm_diagnostic_flag_unentitled_unprivileged,
