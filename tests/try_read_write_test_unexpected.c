...
static void install_exception_handler(void) {
  ...
  T_QUIET;
  T_ASSERT_EQ(result, false, "try_write_byte to NULL");
  T_QUIET;
  T_ASSERT_EQ(kr, KERN_INVALID_ADDRESS, "try_write_byte to NULL");
}

static void test_crasher(void (^crashing_block)(void)) {
  pid_t child_pid;
  if ((child_pid = fork())) {
    /* parent */
    int status;
...
T_DECL(try_read_write_unexpected_trap,
  ...
  test_crasher(^{
    /*
     * Provoke a non-EXC_BAD_ACCESS exception outside of try_read_byte and
     * try_write_byte. The try_read_write exception handler should not catch it.
     */
