...
static testcase_t testcases[] = {
    testCase(Yfoo),        testCase(Xfoo),        testCase(Yatan),
    testCase(Xatan),       testCase(Xclean),      testCase(Xdirty),
    testCase(Xmmap_clean), testCase(Xmmap_dirty), testCase(Xcopyout),
    testCase(kernel),      testCase(Xwired)};

TestCase get_testcase(char *key) {
  int i;
...
int main(int argc, char **argv) {
  ...
  case Xdirty:
    ...
    break;
  case Xmmap_clean:
    page = (int *)mmap(NULL, PAGE_SIZE * 3, PROT_READ | PROT_WRITE,
  ...
  case BAD_TEST_CASE:
    ...
    printf("\nY{name} is a test, X{name} does the test after injecting an ECC "
           "error\n");
