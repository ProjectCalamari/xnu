...
T_DECL(bind_hypervisor, "vsock do not bind to hypervisor cid") {
  ...
  struct sockaddr_vm addr;
  int result =
      vsock_bind(VMADDR_CID_HYPERVISOR, VMADDR_PORT_ANY, &addr, &socket);
  T_ASSERT_POSIX_FAILURE(result, EADDRNOTAVAIL,
...
T_DECL(bind_same, "vsock bind same address and port") {
  ...
  T_ASSERT_POSIX_FAILURE(result, EADDRINUSE,
...
T_DECL(bind_privileged_non_root, "vsock bind on privileged port - non-root",
       T_META_ASROOT(false)) {
  if (geteuid() == 0) {
...
T_DECL(bind_privileged_root, "vsock bind on privileged port - root",
       T_META_ASROOT(true)) {
  if (geteuid() != 0) {
...
T_DECL(listen_any, "vsock listen on any port") {
  ...
  int socket;
  int result =
      vsock_listen(VMADDR_CID_ANY, VMADDR_PORT_ANY, &addr, 10, &socket);
  T_ASSERT_POSIX_SUCCESS(result, "vsock listen");
}

/* Connect */

T_DECL(connect_non_hypervisor,
...
T_DECL(connect_non_listening_host, "vsock connect to non-listening host port") {
  ...
  T_ASSERT_POSIX_FAILURE(result, EAGAIN,
                         "vsock connect non-listening host port");
}

T_DECL(connect_non_listening_hypervisor,
...
T_DECL(connect_non_blocking, "vsock connect non-blocking") {
  ...
  if (result != 0 && errno != EINPROGRESS) {
    T_ASSERT_FAIL(
        "vsock connect should succeed or return EINPROGRESS. errno: %u", errno);
  }
...
T_DECL(shutdown_writes, "vsock shutdown - writes") {
  ...
  // send() hits us with a SIGPIPE if peer closes. ignore this and catch the
  // error code.
...
T_DECL(talk_self_double, "vsock talk to self - double sends") {
  ...
  for (int i = 0; i < 64; i++) {
    ...
    // Receive a message.
    ssize_t read_bytes = read(socketB, buffer, 1024);
    T_ASSERT_EQ_LONG(strlen(msg) * 2, (unsigned long)read_bytes,
                     "read all bytes");
...
T_DECL(talk_self_connections, "vsock talk to self - too many connections") {
  ...
  int listen_socket;
  int result =
      vsock_listen(VMADDR_CID_ANY, port, &listen_addr, backlog, &listen_socket);
  T_ASSERT_POSIX_SUCCESS(result, "vsock listen");
...
T_DECL(talk_self_large_writes, "vsock talk to self with large writes") {
  ...
  for (int i = 0; i < 64; i++) {
    ...
    T_ASSERT_EQ_INT(0, memcmp(random, buffer, size),
                    "sent and received same data");
  }
...
T_DECL(vsock_pcblist_simple, "vsock pcblist sysctl - simple") {
  ...
  T_ASSERT_POSIX_SUCCESS(result, "vsock pcblist get buffer size (result %d)",
                         result);
...
T_DECL(vsock_pcblist_added, "vsock pcblist sysctl - socket added") {
  ...
  // Create some socket to discover in the pcblist after making the first
  // sysctl.
  struct sockaddr_vm addr;
  int socket;
  const uint32_t port = vsock_get_available_port();
...
T_DECL(vsock_private_connect_without_entitlement,
       "vsock private connect should fail without entitlement") {
  int socket;
...
T_DECL(vsock_private_bind_without_entitlement,
       "vsock private bind should fail without entitlement") {
  int socket;
  struct sockaddr_vm addr;
