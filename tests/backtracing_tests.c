...
static const char *user_bt[USER_FRAMES] = {
    "backtrace_thread", "recurse_a", "recurse_b", "recurse_a",
    "recurse_b",        "recurse_a", "recurse_b", "recurse_a",
    "recurse_b",        "recurse_a", "recurse_b", "expect_callstack",
};
...
static const char *async_bt[ASYNC_FRAMES] = {
    "level1_func",
    "level2_func",
    "backtrace_thread_async",
    "expect_async_callstack",
};

static void expect_frame(struct callstack_exp *cs, CSSymbolRef symbol,
                         unsigned long addr, unsigned int bt_idx) {
  if (CSIsNull(symbol)) {
    if (!cs->in_syscall_setup) {
      T_FAIL("invalid symbol for address %#lx at frame %d", addr, bt_idx);
    }
    return;
  }
    ...
    if (cs->in_syscall_setup) {
      if (strcmp(name, cs->callstack[cs->callstack_len - 1]) == 0) {
        ...
        T_LOG("found start of controlled stack at frame %u, expected "
              "index %zu",
              cs->syscall_frames, cs->callstack_len - 1);
      } else {
        T_LOG("found syscall setup symbol %s at frame %u", name, bt_idx);
      }
    }
    if (!cs->in_syscall_setup) {
      if (cs->nchecked >= cs->callstack_len) {
        ...
        T_EXPECT_EQ_STR(name, cs->callstack[frame_idx],
                        "frame %2zu: saw '%s', expected '%s'", frame_idx, name,
                        cs->callstack[frame_idx]);
      }
      cs->nchecked++;
    }
  } else {
...
static bool is_kernel_64_bit(void) {
...
backtrace_current_thread_wrapper(enum test_scenario scenario, uint64_t *bt,
                                 size_t *bt_filled) {
  int ret = sysctlbyname("kern.backtrace.user", bt, bt_filled, NULL, scenario);
...
static CSSymbolicatorRef get_symbolicator(void) {
...
expect_callstack(enum test_scenario scenario) {
  ...
  for (unsigned int i = 0; i < bt_len; i++) {
    ...
    CSSymbolRef symbol =
        CSSymbolicatorGetSymbolWithAddressAtTime(user_symb, addr, kCSNow);
...
expect_async_callstack(void) {
  ...
  T_EXPECT_GE(bt_len, (unsigned int)ASYNC_FRAMES,
              "at least %u frames should be present in backtrace",
              ASYNC_FRAMES);
...
static void *backtrace_thread_async(void *__unused arg) {
...
#if __has_feature(ptrauth_calls)
#define __stack_context_auth                                                   \
  __ptrauth(ptrauth_key_process_dependent_data, 1, 0xc31a)
...
T_DECL(backtrace_user, "test that the kernel can backtrace user stacks",
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(
      pthread_create(&thread, NULL, backtrace_thread, (void *)USER_SCENARIO),
...
T_DECL(backtrace_user_bounds,
  ...
  T_ASSERT_MACH_SUCCESS(kr, "should succeed in writing to backtrace page");
  ...
  error =
      sysctlbyname("kern.backtrace.user", bt_start, &bt_filled_after, NULL, 0);
  T_EXPECT_POSIX_SUCCESS(
      error, "sysctlbyname(\"kern.backtrace.user\") just before guard page");
  T_EXPECT_EQ(
      bt_filled, bt_filled_after,
...
T_DECL(backtrace_user_async,
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(
      pthread_create(&thread, NULL, backtrace_thread_async, NULL),
      "create additional thread to backtrace");
...
T_DECL(backtrace_user_resume,
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(
      pthread_create(&thread, NULL, backtrace_thread, (void *)RESUME_SCENARIO),
      "create additional thread to backtrace");
  T_QUIET;
  T_ASSERT_POSIX_ZERO(pthread_join(thread, NULL), NULL);
}
...
T_DECL(backtrace_kernel_pack_unpack,
  ...
  T_EXPECT_POSIX_SUCCESS(
      error, "sysctlbyname(\"kern.backtrace.kernel_tests\", PACK_UNPACK)");
}
...
T_DECL(backtrace_kernel_packed,
  ...
  T_EXPECT_POSIX_SUCCESS(
      error, "sysctlbyname(\"kern.backtrace.kernel_tests\", PACKED)");
}

#pragma mark - utilities

static void __attribute__((noinline, not_tail_called)) spin_forever(void) {
  while (true) {
    ;
  }
}

static void check_stack(uintptr_t fp, uintptr_t ctx) {
  if ((fp - ctx) != 0x8) {
    fprintf(stderr,
            "stack frame is not set up properly: "
