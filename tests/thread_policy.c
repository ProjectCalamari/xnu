...
static qos_class_t thread_qos_to_qos_class(uint32_t thread_qos) {
  switch (thread_qos) {
...
static qos_class_t get_thread_requested_qos(void) {
  ...
  kern_return_t kr = thread_policy_get(
      thread_port, THREAD_REQUESTED_STATE_POLICY,
...
T_DECL(thread_policy_requested_state, "THREAD_REQUESTED_STATE_POLICY",
       T_META_ASROOT(NO)) {
  qos_class_t main_thread_qos = get_thread_requested_qos();
  T_ASSERT_EQ(main_thread_qos, qos_class_main(),
              "main thead requested qos matches");
}
