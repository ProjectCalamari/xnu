...
static inline kern_return_t service_port_set_throttled(int is_throttled) {
  return mach_port_set_attributes(
      mach_task_self(), service_port, MACH_PORT_SERVICE_THROTTLED,
      (mach_port_info_t)(&is_throttled), MACH_PORT_SERVICE_THROTTLED_COUNT);
}

static inline kern_return_t service_port_get_throttled(int *is_throttled) {
  natural_t count = 0;
  kern_return_t kr;

  kr = mach_port_get_attributes(mach_task_self(), service_port,
                                MACH_PORT_SERVICE_THROTTLED,
                                (mach_port_info_t)(is_throttled), &count);
  T_QUIET;
  T_ASSERT_EQ(count, MACH_PORT_SERVICE_THROTTLED_COUNT, NULL);

  return kr;
}

T_DECL(mach_service_port, "Create a port with a service port label",
  ...
  kr = mach_port_is_connection_for_service(mach_task_self(), connection_port,
                                           service_port, &fpid);
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "get throttled flag value on port");
...
kern_return_t do_mach_notify_port_destroyed(mach_port_t notify,
  ...
  T_LOG("Received a service port destroyed notification notify = 0x%x name = "
        "0x%x",
        notify, name);
  ...
  if (name != service_port) {
    T_FAIL("do_mach_notify_port_destroyed: name 0x%x != service_port: 0x%x",
           name, service_port);
  }

  struct mach_service_port_info sp_info = {};
  kr =
      mach_port_get_service_port_info(mach_task_self(), service_port, &sp_info);
  ...
  T_ASSERT_EQ(sp_info.mspi_domain_type, SERVICE_DOMAIN, "Service domain = %u",
              sp_info.mspi_domain_type);

  mach_port_guard_info_t mpgi = {SP_CONTEXT};
  kr = mach_port_assert_attributes(
      mach_task_self(), service_port, MACH_PORT_GUARD_INFO,
      (mach_port_info_t)&mpgi, MACH_PORT_GUARD_INFO_COUNT);
  T_ASSERT_MACH_SUCCESS(kr, "mach_port_assert_attributes");

  return KERN_SUCCESS;
}

kern_return_t do_mach_notify_port_deleted(__unused mach_port_t notify,
                                          __unused mach_port_name_t name) {
  return KERN_SUCCESS;
}

kern_return_t do_mach_notify_no_senders(__unused mach_port_t notify,
                                        __unused mach_port_mscount_t mscount) {
  return KERN_SUCCESS;
}

kern_return_t do_mach_notify_send_once(__unused mach_port_t notify) {
  return KERN_SUCCESS;
}

kern_return_t do_mach_notify_dead_name(__unused mach_port_t notify,
                                       __unused mach_port_name_t name) {
  return KERN_SUCCESS;
}
...
T_DECL(mach_fake_service_port,
       "Create a connection port with a fake service port",
       T_META_CHECK_LEAKS(false)) {
  mach_port_t connection_port;
  ...
  mach_port_options_t opts = {
      .flags = MPO_CONNECTION_PORT | MPO_SERVICE_PORT | MPO_INSERT_SEND_RIGHT |
               MPO_CONTEXT_AS_GUARD,
      .service_port_info = &sp_info,
  };

  kr =
      mach_port_construct(mach_task_self(), &opts, SP_CONTEXT, &service_port_2);
  T_ASSERT_MACH_ERROR(kr, KERN_INVALID_ARGUMENT,
                      "mach_port_construct with extra flags %u",
                      service_port_2);
  ...
  kr = mach_port_construct(mach_task_self(), &opts2, SP_CONTEXT,
                           &service_port_2);
  T_ASSERT_MACH_ERROR(kr, KERN_INVALID_ARGUMENT,
                      "mach_port_construct with missing service port info %u",
                      service_port_2);
  ...
  kr = mach_port_construct(mach_task_self(), &opts3, SP_CONTEXT,
                           &fake_service_port);
  T_ASSERT_MACH_SUCCESS(kr, "mach_port_construct with missing flag %u",
                        fake_service_port);

  struct mach_service_port_info sp_info3 = {};
  kr = mach_port_get_service_port_info(mach_task_self(), fake_service_port,
                                       &sp_info3);
  T_ASSERT_MACH_ERROR(kr, KERN_INVALID_CAPABILITY,
  ...
  T_ASSERT_MACH_ERROR(kr, KERN_INVALID_CAPABILITY,
                      "mach_port_construct connection port %u",
                      connection_port);

  T_LOG("done");
}

T_DECL(mach_dead_service_port,
       "Create a connection port with a dead service port",
       T_META_CHECK_LEAKS(false)) {
  mach_port_t connection_port;
  mach_port_t service_port_2;
  ...
  kr = mach_port_mod_refs(mach_task_self(), service_port_2,
