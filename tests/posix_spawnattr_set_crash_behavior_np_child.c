...
static bool _should_spin(char *mode) {
  // These tests are signaled by the parent
  char *spin_modes[] = {
      "spin", "reason", "reason_signal", "clean", "dirty",
  };
...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(argv[1], "crash") == 0) {
    abort_with_reason(OS_REASON_TEST, TEST_REASON_CODE, "Test forcing crash",
    ...
    dispatch_source_t source =
        dispatch_source_create(DISPATCH_SOURCE_TYPE_SIGNAL, SIGUSR1, 0, NULL);
