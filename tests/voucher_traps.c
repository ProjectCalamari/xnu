 ...
 *
 * clang -o voucher_traps voucher_traps.c -ldarwintest -Weverything
...
static mach_port_t get_user_data_port(mach_msg_type_number_t *size) {
#define DATA "Hello World!"
  ...
  kern_return_t kr = host_create_mach_voucher(
      mach_host_self(), (mach_voucher_attr_raw_recipe_array_t)&buf, sizeof(buf),
      &port);
#if !TARGET_OS_OSX
...
T_DECL(voucher_extract_attr_recipe, "voucher_extract_attr_recipe",
   ...
   */
  alloc_addr = (mach_vm_address_t)round_page(
      MACH_VOUCHER_ATTR_MAX_RAW_RECIPE_ARRAY_SIZE + 1);
  kr = mach_vm_allocate(mach_task_self(), &alloc_addr, alloc_sz,
                        VM_FLAGS_ANYWHERE);
   ...
   */
  kr = mach_voucher_extract_attr_recipe_trap(
      port, MACH_VOUCHER_ATTR_KEY_USER_DATA, recipe, recipe_size);
