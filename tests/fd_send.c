...
T_GLOBAL_META(T_META_NAMESPACE("xnu.fd"), T_META_RUN_CONCURRENTLY(true));

#define SOCKETPAIR(pair)                                                       \
  T_ASSERT_POSIX_SUCCESS(socketpair(PF_LOCAL, SOCK_STREAM, 0, pair),           \
                         "socketpair")

static errno_t send_fd(int sock, int fd) {
...
static errno_t recv_fd(int sock, int *fdp) {
  u_char c;
  struct iovec iovec[1];
  struct msghdr msg;
  struct cmsghdr *cmsghdrp;
...
T_DECL(send, "test for 30465592", T_META_TAG_VM_PREFERRED) {
  ...
  if (child != 0) {
    ...
    T_QUIET;
    T_ASSERT_EQ(recv_fd(pair[1], &fd), 0, "recv_fd");
    T_QUIET;
    T_ASSERT_NE(fd, -1, "received a proper fd");
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(close(fd), "close(fd)");
...
T_DECL(send_stress, "test for 67133384", T_META_TAG_VM_PREFERRED) {
  ...
  dispatch_apply(10, NULL, ^(size_t worker) {
    ...
    dispatch_group_async(g, q, ^{
      ...
      for (int i = 0; i < n; i++) {
        T_QUIET;
        T_ASSERT_EQ(recv_fd(pair[1], &tmp), 0, "recv_fd");
        T_QUIET;
        T_ASSERT_NE(tmp, -1, "received a proper fd");
        T_QUIET;
        T_EXPECT_POSIX_SUCCESS(close(tmp), "close(tmp)");
      }
    });
    dispatch_release(q);
    ...
    for (int i = 0; i < n; i++) {
      int tmp = dup(fd);
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(tmp, "dup");
      T_QUIET;
      T_ASSERT_EQ(send_fd(pair[0], tmp), 0, "send_fd");
      T_QUIET;
      T_EXPECT_POSIX_SUCCESS(close(tmp), "close(tmp)");
    }
    ...
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(close(pair[0]), "close(pair[0])");
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(close(pair[1]), "close(pair[1])");
  });
}
