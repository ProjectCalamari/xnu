...
static void *spin_thread(__unused void *arg) {
  spin_for_duration(8);
  return NULL;
}

static void *spin_bound_thread(void *arg) {
  char type = (char)arg;
...
T_DECL(test_cluster_bound_thread_timeshare,
       "Make sure the low priority bound threads get CPU in the presence of "
       "non-bound CPU spinners",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "pthread_attr_setdetachstate");
  ...
  trace_handle_t trace =
      begin_collect_trace(argc, argv, "test_cluster_bound_thread_timeshare");
  ...
  for (unsigned int i = 0; i < ncpu * SPINNER_THREAD_LOAD_FACTOR; i++) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(rv, "pthread_create (non-bound)");
  }
  ...
  rv = pthread_create(&bound_thread, &attr, spin_bound_thread,
                      (void *)(uintptr_t)'P');
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "pthread_create (P-bound)");
  ...
  kr = thread_info(thread_port, THREAD_BASIC_INFO,
                   (thread_info_t)&bound_thread_info, &count);
  if (kr != KERN_SUCCESS) {
  ...
  uint64_t bound_usr_usec =
      (uint64_t)bound_thread_info.user_time.seconds * USEC_PER_SEC +
      (uint64_t)bound_thread_info.user_time.microseconds;

  T_ASSERT_GT(bound_usr_usec, 75000ULL,
              "Check that bound thread got atleast 75ms CPU time");
  T_PASS("Low priority bound threads got some CPU time in the presence of high "
         "priority unbound spinners");
}

static uint64_t observe_thread_user_time(pthread_t thread,
  ...
  kr = thread_info(port, THREAD_BASIC_INFO, (thread_info_t)&basic_thread_info,
                   &count);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "thread_info(THREAD_BASIC_INFO)");
  before_user_us =
      (uint64_t)basic_thread_info.user_time.seconds * USEC_PER_SEC +
  ...
  T_QUIET;
  T_ASSERT_GE(after_user_us, before_user_us, "increasing user_time values");
  return after_user_us - before_user_us;
}

T_DECL(cluster_soft_binding,
       "Make sure that cluster-binding is \"soft\" and a bound thread can run "
       "elsewhere when"
  ...
  trace_handle_t trace =
      begin_collect_trace(argc, argv, "cluster_soft_binding");
  T_SETUPEND;
  ...
  for (unsigned int p = 0; p < platform_nperflevels(); p++) {
    ...
    T_QUIET;
    T_EXPECT_TRUE(all_cores_recommended,
                  "Not all cores are recommended for scheduling");
    ...
    uint64_t recommended_user_us =
        observe_thread_user_time(bound_thread, observe_seconds);
    T_LOG("%c-bound thread ran %lluus with all cores recommended", (char)arg,
          recommended_user_us);
    T_QUIET;
    T_EXPECT_GE(recommended_user_us * 1.0,
                runtime_threshold * observe_seconds * USEC_PER_SEC,
                "%c-bound thread didn't run at least %f of %d seconds",
                (char)arg, runtime_threshold, observe_seconds);
    ...
    uint64_t derecommended_user_us =
        observe_thread_user_time(bound_thread, observe_seconds);
    T_LOG("%c-bound thread ran %lluus with %c-cores derecommended", (char)arg,
          derecommended_user_us, (char)arg);
    T_EXPECT_GE(recommended_user_us * 1.0,
                runtime_threshold * observe_seconds * USEC_PER_SEC,
                "%c-bound thread ran at least %f of %d seconds when %c-cores "
                "were derecommended",
...
static void *spin_cluster_binding(void *) {
  ...
  for (int t = 0; t < num_cluster_bind_trials; t++) {
    ...
    if (!unbind) {
      ...
      T_QUIET;
      T_EXPECT_EQ(running_on_cluster, bind_cluster,
                  "Failed to reach the bound cluster");
...
T_DECL(cluster_bind_migrate,
       "Ensure cluster-binding triggers a context-switch if needed to get to "
       "the bound cluster",
       T_META_ENABLED(TARGET_CPU_ARM64),
       T_META_MAYFAIL("rdar://132360557, need a reasonable expectation that "
                      "cores will not quickly disable")) {
  T_SETUPBEGIN;
  if (!platform_is_amp()) {
    T_SKIP("Platform is symmetric, skipping cluster-binding test");
  }
  ...
  if (strcmp(policy_name, "edge") != 0) {
    T_SKIP("Platform is running the \"%s\" scheduler, which lacks strong "
           "enough cluster-binding",
           policy_name);
  }
  ...
  trace_handle_t trace =
      begin_collect_trace(argc, argv, "cluster_bind_migrate");
  T_SETUPEND;

  pthread_t *threads = create_threads(
      dt_ncpu(), 31, eJoinable, QOS_CLASS_UNSPECIFIED, eSchedDefault,
      DEFAULT_STACK_SIZE, spin_cluster_binding, NULL);
  for (int i = 0; i < dt_ncpu(); i++) {
    pthread_join(threads[i], NULL);
  }

  if (T_FAILCOUNT == 0) {
    T_PASS("Correctly migrated to the bound cluster for %d trials",
           num_cluster_bind_trials);
  } else {
    T_FAIL("%d fails for %d cluster-bind attempts", T_FAILCOUNT,
           num_cluster_bind_trials);
  }
  end_collect_trace(trace);
}
