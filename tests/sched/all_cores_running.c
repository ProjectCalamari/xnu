...
/*
 * As a successor of clpc_disabling_cores_test_21636137, this test ensures that
 * threads are naturally being scheduled on all of the logical cores (without
 * binding). The test fails if CLPC has derecommended any cores.
 */

static _Atomic uint64_t visited_cores_bitmask = 0;
static uint64_t spin_deadline_timestamp = 0;

static void *spin_thread_fn(__unused void *arg) {
  while (mach_absolute_time() < spin_deadline_timestamp) {
    unsigned int curr_cpu = _os_cpu_number();
    atomic_fetch_or_explicit(&visited_cores_bitmask, (1ULL << curr_cpu),
...
static void start_threads(pthread_t *threads, void *(*start_routine)(void *),
                          int priority, unsigned int num_threads) {
  int rv;
  pthread_attr_t attr;
...
static void init_host_and_cpu_count(void) {
  kern_return_t kr;
  host_t priv_host;
  ...
  T_QUIET;
  T_ASSERT_EQ(cpu_count, (unsigned int)dt_ncpu(),
              "cpu counts between host_processors() and hw.ncpu don't match");
}

static void record_cpu_loads(struct processor_cpu_load_info *cpu_loads) {
  kern_return_t kr;
  mach_msg_type_number_t info_count = PROCESSOR_CPU_LOAD_INFO_COUNT;
  for (unsigned int i = 0; i < cpu_count; i++) {
    kr = processor_info(cpu_ports[i], PROCESSOR_CPU_LOAD_INFO, &host,
                        (processor_info_t)&cpu_loads[i], &info_count);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "processor_info");
  }
}

static void cpu_loads_delta(struct processor_cpu_load_info *start_loads,
  ...
  for (unsigned int i = 0; i < cpu_count; i++) {
    ...
    for (int state = CPU_STATE_USER; state < CPU_STATE_MAX; state++) {
      T_QUIET;
      T_ASSERT_GE(finish_loads[i].cpu_ticks[state],
                  start_loads[i].cpu_ticks[state],
                  "non-monotonic ticks for state %d", state);
      delta_loads[i].cpu_ticks[state] =
          finish_loads[i].cpu_ticks[state] - start_loads[i].cpu_ticks[state];
      delta_sum += delta_loads[i].cpu_ticks[state];
    }
    T_QUIET;
    T_ASSERT_GT(delta_sum, 0ULL,
                "Failed to read meaningful load data for the core. Was the "
                "amfi_get_out_of_my_way=1 boot-arg missing?");
    non_idle_ticks[i] = delta_loads[i].cpu_ticks[CPU_STATE_USER] +
                        delta_loads[i].cpu_ticks[CPU_STATE_SYSTEM];
...
T_DECL(all_cores_running,
  ...
  trace_handle_t trace_handle =
      begin_collect_trace(argc, argv, "sched_all_cores_running");
  ...
  spin_deadline_timestamp =
      mach_absolute_time() + nanos_to_abs(spin_seconds * NSEC_PER_SEC);
  unsigned int num_threads = (unsigned int)dt_ncpu() * 2;
  T_LOG("Launching %u threads to spin for %lld seconds...", num_threads,
        spin_seconds);
   ...
   */
  T_ASSERT_EQ((unsigned int)__builtin_popcountll(final_visited_cores_bitmask),
              cpu_count, "[%s] Each core ran at least one of the test threads",
              platform_train_descriptor());
  for (unsigned int i = 0; i < cpu_count; i++) {
    T_QUIET;
    T_ASSERT_GT(non_idle_ticks[i], 0,
                "One or more cores were idle during the work period");
  }
...
T_DECL(recommended_cores_mask,
       "Tests that the mask of recommended cores includes all logical cores "
       "according to hw.ncpu",
       XNU_T_META_SOC_SPECIFIC) {
  int ret;
  ...
  T_ASSERT_LE(ncpu, 64,
              "Core count isn't too high to reflect in the system's 64-bit "
              "wide core masks");
  ...
  while (!passed_test && tries < MAX_RETRIES) {
    ...
    ret = sysctlbyname("kern.sched_recommended_cores", &recommended_cores_mask,
                       &recommended_cores_mask_size, NULL, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "kern.sched_recommended_cores");
    T_LOG("kern.sched_recommended_cores:     0x%016llx",
          recommended_cores_mask);
    ...
    if ((recommended_cores_mask & expected_set_mask) == expected_set_mask) {
      ...
      T_LOG("Missing expected bits. Sleeping for %u seconds before retrying",
            sleep_seconds);
      sleep(sleep_seconds);
      tries++;
    }
  ...
  T_ASSERT_EQ(passed_test, 1,
              "[%s] kern.sched_recommended_cores reflects that all expected "
