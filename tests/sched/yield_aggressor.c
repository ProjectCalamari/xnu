...
/* Code and logic taken from Daniel Chimene's yield-aggressor.c test
 * (rdar://47327537) */

static const size_t MAX_PDJ_PATH_LEN = 256;

static void sched_yield_loop(uint64_t iterations) {
  for (uint64_t i = 0; i < iterations; i++) {
    sched_yield();
  }
}

static void swtch_loop(uint64_t iterations) {
  for (uint64_t i = 0; i < iterations; i++) {
    swtch();
  }
}

static void swtch_pri_loop(uint64_t iterations) {
  for (uint64_t i = 0; i < iterations; i++) {
    swtch_pri(0);
  }
}

static void thread_switch_loop(uint64_t iterations) {
  for (uint64_t i = 0; i < iterations; i++) {
    thread_switch(MACH_PORT_NULL, SWITCH_OPTION_NONE, MACH_MSG_TIMEOUT_NONE);
  }
}

static void thread_switch_wait_loop(uint64_t iterations) {
  for (uint64_t i = 0; i < iterations; i++) {
    thread_switch(MACH_PORT_NULL, SWITCH_OPTION_WAIT, MACH_MSG_TIMEOUT_NONE);
  }
}

static void thread_switch_depress_loop(uint64_t iterations) {
  for (uint64_t i = 0; i < iterations; i++) {
...
static void *thread_fn(__unused void *arg) {
  kern_return_t kr;
...
static void record_cpu_time(struct cpu_time *cpu_time) {
  host_cpu_load_info_data_t load;
  kern_return_t kr;
  mach_msg_type_number_t count = HOST_CPU_LOAD_INFO_COUNT;

  kr = host_statistics(mach_host_self(), HOST_CPU_LOAD_INFO, (int *)&load,
                       &count);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "host_statistics");
...
static void write_independent_variables(pdwriter_t writer) {
  pdwriter_record_variable_str(writer, "yield_variant",
                               name_table[curr_yield_type]);
...
static void write_time_values(pdwriter_t writer, struct cpu_time *delta_times,
                              uint64_t elapsed_usecs, double idle_ratio) {
  pdwriter_new_value(writer, "system_time", pdunit_milliseconds_cpu,
                     delta_times->sys * MS_PER_CPU_TICK);
  write_independent_variables(writer);

  pdwriter_new_value(writer, "user_time", pdunit_milliseconds_cpu,
                     delta_times->user * MS_PER_CPU_TICK);
  write_independent_variables(writer);

  pdwriter_new_value(writer, "idle_time", pdunit_milliseconds_cpu,
                     delta_times->idle * MS_PER_CPU_TICK);
  write_independent_variables(writer);

  pdwriter_new_value(writer, "wall_clock_time", pdunit_microseconds,
                     elapsed_usecs);
  write_independent_variables(writer);

  /* Main metric of note, with a threshold in perfmeta to guard against
   * regression */
  pdwriter_new_value(writer, "idle_time_ratio", pdunit_percent_cpus,
                     idle_ratio);
  write_independent_variables(writer);
}

static void run_yielding_test(yield_type_t yield_type, unsigned int num_iters,
                              unsigned int thread_count, int thread_pri,
                              pdwriter_t writer) {
  T_SETUPBEGIN;
  ...
  T_LOG("All %u threads finished yielding %u times each", num_threads,
        num_iterations);
  ...
  T_QUIET;
  T_ASSERT_GT(total_ticks, 0,
              "CPU load stats failed to update, likely due to "
              "host_statistics() rate limit");
...
T_DECL(
    ...
    T_META_BOOTARGS_SET("amfi_get_out_of_my_way=1"), T_META_ASROOT(true)) {
  /* Warn if amfi_get_out_of_my_way is not set and fail later on if we actually
   * run into the rate limit */
  size_t kernel_bootargs_size = sizeof(kernel_bootargs);
  int rv = sysctlbyname("kern.bootargs", kernel_bootargs, &kernel_bootargs_size,
                        NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "kern.bootargs");
  if (strstr(kernel_bootargs, "amfi_get_out_of_my_way=1") == NULL) {
    T_LOG("WARNING: amfi_get_out_of_my_way=1 boot-arg is missing, required to "
          "reliably capture CPU load data");
  }

  char pdj_path[MAX_PDJ_PATH_LEN];
  pdwriter_t writer = pdwriter_open_tmp("xnu", "scheduler.yield_aggressor", 0,
                                        0, pdj_path, MAX_PDJ_PATH_LEN);
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(writer, "pdwriter_open_tmp");

  /*
   * Thread count is NCPU * 3 in order to ensure that there are enough yielding
   * threads to keep all of the cores busy context-switching between them. NCPU
   * * 1 threads would not be sufficient to guarantee this, because a core
   * temporarily keeps two threads off of the run-queues at a time while
   * performing a context-switch (rather than only the one thread it is running
   * during normal execution). Lastly, we choose NCPU * 3 rather than NCPU * 2
   * because doing so empirically reduces the variance of values betweens runs.
   */
  unsigned int thread_count = (unsigned int)dt_ncpu() * 3;

  for (yield_type_t yield_type = SCHED_YIELD;
       yield_type <= THREAD_SWITCH_DEPRESS; yield_type++) {
    wait_for_quiescence_default(argc, argv);
    run_yielding_test(yield_type, DEFAULT_NUM_ITERS, thread_count,
                      DEFAULT_THREAD_PRI, writer);
  }
