...
T_GLOBAL_META(
    ...
    T_META_RADAR_COMPONENT_VERSION("scheduler"), T_META_TAG_VM_NOT_ELIGIBLE,
...
static void *make_tg_and_spin(__unused void *arg) {
  int ret;
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, 0, "work_interval_create");
  ...
  uint64_t spin_deadline =
      start_timestamp + nanos_to_abs((SLEEP_SECONDS - 2) * NSEC_PER_SEC);
...
static void start_threads(pthread_t *threads, void *(*start_routine)(void *),
                          int priority, int num_threads) {
  int rv;
  pthread_attr_t attr;

  rv = pthread_attr_init(&attr);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(rv, "pthread_attr_init");
  ...
  for (int i = 0; i < num_threads; i++) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_ZERO(rv, "pthread_attr_setschedparam");

    rv = pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(rv, "pthread_attr_setdetachstate");

    /* Make the thread stacks smaller, so pthread will let us make more */
    rv = pthread_attr_setstacksize(&attr, PTHREAD_STACK_MIN);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(rv, "pthread_attr_setstacksize");

    rv = pthread_create(&threads[i], &attr, start_routine, NULL);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(rv, "pthread_create");
  }

  rv = pthread_attr_destroy(&attr);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(rv, "pthread_attr_destroy");
}

static uint64_t old_preemption_debug_mode = 0;
static size_t old_preemption_debug_mode_size =
    sizeof(old_preemption_debug_mode);

static void restore_preemption_disable_debug_mode(void) {
  int ret =
      sysctlbyname("kern.sched_preemption_disable_debug_mode", NULL, NULL,
                   &old_preemption_debug_mode, old_preemption_debug_mode_size);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "kern.sched_preemption_disable_debug_mode");
  T_LOG("kern.sched_preemption_disable_debug_mode restored to previous value: "
        "%llu",
        old_preemption_debug_mode);
}

static uint64_t old_interrupt_debug_mode = 0;
static size_t old_interrupt_debug_mode_size = sizeof(old_interrupt_debug_mode);

static void restore_interrupt_disable_debug_mode(void) {
  int ret =
      sysctlbyname("kern.interrupt_masked_debug_mode", NULL, NULL,
                   &old_interrupt_debug_mode, old_interrupt_debug_mode_size);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "kern.interrupt_masked_debug_mode");
  T_LOG("kern.interrupt_masked_debug_mode restored to previous value: %llu",
        old_interrupt_debug_mode);
}

static uint64_t old_interrupt_disable_timeout = 0;
static size_t old_interrupt_disable_timeout_size =
    sizeof(old_interrupt_disable_timeout);

static void restore_interrupt_disable_timeout(void) {
  int ret = sysctlbyname("kern.interrupt_masked_threshold_mt", NULL, NULL,
                         &old_interrupt_disable_timeout,
                         old_interrupt_disable_timeout_size);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "kern.interrupt_masked_threshold_mt");
  T_LOG("kern.interrupt_masked_threshold_mt restored to previous value: %llu",
        old_interrupt_disable_timeout);
}
...
static const char *ktrace_file_short_name =
    "overload_runqueue_with_thread_groups.ktrace";

static void save_collected_ktrace(char *trace_path) {
  int ret;

  T_LOG("ktrace file saved at \"%s\"", trace_path);
  ret = chmod(trace_path, 0666);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "chmod");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "dt_resultfile marking \"%s\" for collection",
                         compressed_path);
  T_LOG("\"%s\" marked for upload", compressed_path);

  char *tar_args[] = {"/usr/bin/tar", "-czvf", compressed_path, trace_path,
                      NULL};
  pid_t tar_pid = dt_launch_tool_pipe(
      tar_args, false, NULL,
  ...
  T_QUIET;
  T_ASSERT_TRUE(tar_pid, "[tar] pid %d", tar_pid);
  ret = dt_waitpid(tar_pid, NULL, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "dt_waitpid");
...
/*
 * Parse the recorded ktrace file to see if we crossed the set
 * interrupt-disabled timeout and thus failed the test.
 */
static void search_for_interrupt_disable_timeout_tracepoint(char *trace_path) {
  __block int ret;
  ...
  ktrace_events_single(
      read_session, MACHDBG_CODE(DBG_MACH_SCHED, MACH_INT_MASKED_EXPIRED),
      ^(ktrace_event_t e) {
        if (offending_thread == NULL) {
          T_LOG("Interrupts were held disabled for %llu ns, crossing the %llu "
                "ns threshold:",
                abs_to_nanos(e->arg1), INTERRUPT_DISABLE_TIMEOUT_NS);
          ...
          offending_thread =
              ktrace_get_name_for_thread(read_session, e->threadid);
  ...
  ktrace_set_completion_handler(read_session, ^{
    if (offending_thread == NULL) {
      T_PASS("Scheduler survived %d simulatenously runnable thread groups "
             "without disabling interrupts for more than %llu ns!",
             NUM_THREADS, INTERRUPT_DISABLE_TIMEOUT_NS);
    } else {
      save_collected_ktrace(trace_path);
      T_FAIL("Interrupts held disabled for more than %llu ns by thread \"%s\"",
             INTERRUPT_DISABLE_TIMEOUT_NS, offending_thread);
    }
    T_END;
  });
...
T_DECL(overload_runqueue_with_thread_groups,
       "Overload the runqueue with distinct thread groups to verify that the "
       "scheduler"
       "does not trip an interrupts-disabled timeout whenever it scans the "
       "runqueue",
       T_META_ASROOT(true), XNU_T_META_SOC_SPECIFIC,
  ...
  /* Configure the preemption-disabled debug mode as well, to avoid panicking if
   * the test fails */
  ret =
      sysctlbyname("kern.sched_preemption_disable_debug_mode",
                   &old_preemption_debug_mode, &old_preemption_debug_mode_size,
                   &emit_tracepoint_mode, sizeof(emit_tracepoint_mode));
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "kern.sched_preemption_disable_debug_mode");
  ...
  ret = sysctlbyname("kern.interrupt_masked_threshold_mt",
                     &old_interrupt_disable_timeout,
                     &old_interrupt_disable_timeout_size, &disable_timeout,
  ...
  ret = ktrace_events_filter(session, "C0x01",
                             ^(__unused ktrace_event_t event){
                             }); // records scheduler tracepoints
  T_QUIET;
  ...
  T_LOG("Creating %d threads at pri %d, each with a unique thread group",
        NUM_THREADS, BACKGROUND_PRI);
  ...
  T_LOG("Waiting %llu seconds to see if the scheduler can handle it...",
        SLEEP_SECONDS);
  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, (int64_t)(SLEEP_SECONDS * NSEC_PER_SEC)),
