...
static void csw_perf_test_init(void) {
  int spawn_ret, pid;
  char *const clpcctrl_args[] = {"/usr/local/bin/clpcctrl", "-f", "5000", NULL};
  spawn_ret =
      posix_spawn(&pid, clpcctrl_args[0], NULL, NULL, clpcctrl_args, environ);
  waitpid(pid, &spawn_ret, 0);
}

static void csw_perf_test_cleanup(void) {
  int spawn_ret, pid;
...
static pthread_t make_thread(uint32_t thread_id, uint32_t priority, bool fixpri,
...
static void spin_for_timeout(uint32_t seconds) {
...
void record_perfcontrol_stats(const char *sysctlname, const char *units,
                              const char *info) {
  int data = 0;
  size_t data_size = sizeof(data);
  T_ASSERT_POSIX_ZERO(sysctlbyname(sysctlname, &data, &data_size, NULL, 0),
                      "%s", sysctlname);
  T_PERF(info, data, units, info);
}

/* Disable the test on MacOS for now */
T_DECL(perf_csw, "context switch performance", T_META_TAG_PERF,
  ...
  T_ASSERT_POSIX_ZERO(sysctlbyname("kern.monotonic.supported", &mt_supported,
                                   &mt_supported_size, NULL, 0),
                      "sysctlbyname kern.monotonic.supported");

  for (uint32_t thread_id = 0; thread_id < n_spinners; thread_id++) {
    threads[thread_id].thread =
        make_thread(thread_id, THR_SPINNER_PRI, true, &spin_thread);
  }
