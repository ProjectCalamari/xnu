...
#else
kern_return_t mach_get_times(uint64_t *absolute_time, uint64_t *continuous_time,
                             struct timespec *tp);
#endif
...
T_GLOBAL_META(T_META_RUN_CONCURRENTLY(false), T_META_CHECK_LEAKS(false),
              T_META_ALL_VALID_ARCHS(true), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("scheduler"),
              T_META_OWNER("chimene"), T_META_ENABLED(TARGET_OS_OSX),
...
/* Some statistics APIs return host abstime instead of Rosetta-translated
 * abstime */
static uint64_t abs_to_nanos_host(uint64_t abstime) {
  if (is_rosetta) {
...
static void fill_thread_stats(uint32_t i) {
  struct threadentry *entry = &gThreadList[i];

  int rv = thread_selfcounts(THSC_TIME_CPI, &entry->self_stats,
                             sizeof(entry->self_stats));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "thread_selfcounts(THSC_TIME_CPI)");
}
...
T_DECL(setitimer,
  ...
  if (geteuid() == 0) {
    trace = begin_collect_trace_fmt(COLLECT_TRACE_FLAG_DISABLE_SYSCALLS, argc,
                                    argv, "test_setitimer");
  }
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "pthread_setschedparam");

  kr = mach_timebase_info(&timebase_info);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_timebase_info");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "sysctlbyname(hw.ncpu)");
  ...
  kr = semaphore_create(mach_task_self(), &gMainWaitForWorkers,
                        SYNC_POLICY_FIFO, 0);
  T_QUIET;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "pthread_attr_init");

  struct sched_param child_param = {.sched_priority = 37};

  rv = pthread_attr_setschedparam(&attr, &child_param);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "pthread_attr_set_qos_class_np");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "pthread_attr_destroy");
  ...
  for (uint32_t i = 0; i < nthreads; i++) {
    T_LOG("Thread %p (0x%llx) checked in, stack %p/%p\n",
          (void *)gThreadList[i].thread, gThreadList[i].tid,
          gThreadList[i].stack_addr, (void *)gThreadList[i].stack_size);
  }
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "sigaction(SIGALRM)");

  rv = sigaction(SIGVTALRM, &sigact, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "sigaction(SIGVTALRM)");

  rv = sigaction(SIGPROF, &sigact, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "sigaction(SIGPROF)");

  rv = sigaction(SIGXCPU, &sigact, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "sigaction(SIGXCPU)");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "setitimer(ITIMER_REAL)");

  rv = setitimer(ITIMER_VIRTUAL, &itime, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "setitimer(ITIMER_VIRTUAL)");

  rv = setitimer(ITIMER_PROF, &itime, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "setitimer(ITIMER_PROF)");

  struct rlimit rlim = {};

  rv = getrlimit(RLIMIT_CPU, &rlim);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "getrlimit(RLIMIT_CPU)");

  rlim.rlim_cur = 1;
  rv = setrlimit(RLIMIT_CPU, &rlim);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "setrlimit(RLIMIT_CPU)");

  rv = pthread_mutex_lock(&gShouldExitMutex);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "pthread_mutex_lock(&gShouldExitMutex)");

  kr = semaphore_signal_all(gWorkersStart);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "semaphore_signal_all()");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "getrusage(RUSAGE_SELF, &ru_start)");

  kr = mach_get_times(&time_start, NULL, &timenow);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_get_times()");
  ...
  do {
    ...
    rv = pthread_cond_timedwait(&gShouldExitCondition, &gShouldExitMutex,
                                &timeout);
    if (rv == ETIMEDOUT) {
      ...
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(rv, "getrusage(RUSAGE_SELF, &ru_end)");

      break;
    } else {
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(
          rv, "pthread_cond_timedwait(&gShouldExitCondition, ...)");
    }
  } while (true);

  rv = pthread_mutex_unlock(&gShouldExitMutex);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "pthread_mutex_unlock(&gShouldExitMutex)");

  for (uint32_t i = 1; i < nthreads; i++) {
    rv = pthread_join(gThreadList[i].thread, NULL);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(rv, "pthread_join");
  }
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "proc_pid_rusage");
  ...
  T_LOG("times(): Real Time: %jd (%3.3f seconds), User Time %jd (%3.3f "
        "seconds), System Time %jd (%3.3f seconds)\n",
        real_ticks, (double)real_ticks / (double)ticks, user_ticks,
        (double)user_ticks / (double)ticks, system_ticks,
        (double)system_ticks / (double)ticks);
  ...
  T_LOG("rusage(): User Time %ld.%06d, System Time %ld.%06d\n",
        ru_udelta.tv_sec, ru_udelta.tv_usec, ru_sdelta.tv_sec,
        ru_sdelta.tv_usec);

  uint64_t total_user_time_ns = abs_to_nanos_host(ru.ri_user_time);
  double total_user_time_s =
      (double)total_user_time_ns / (uint64_t)NSEC_PER_SEC;

  uint64_t total_system_time_ns = abs_to_nanos_host(ru.ri_system_time);
  double total_system_time_s =
      (double)total_system_time_ns / (uint64_t)NSEC_PER_SEC;
  ...
  double total_runnable_time_s =
      (double)total_runnable_time_ns / (uint64_t)NSEC_PER_SEC;

  uint64_t total_pending_time_ns = total_runnable_time_ns - (total_time_ns);
  double total_pending_time_s =
      (double)total_pending_time_ns / (uint64_t)NSEC_PER_SEC;

  uint64_t total_p_time_ns =
      abs_to_nanos_host(ru.ri_user_ptime + ru.ri_system_ptime);
  double total_p_time_s = (double)total_p_time_ns / (uint64_t)NSEC_PER_SEC;

  T_LOG("total usage: time: %3.3f user: %3.3f kernel: %3.3f runnable: %3.3f "
        "pending: %3.3f pcore: %3.3f\n",
        total_time_s, total_user_time_s, total_system_time_s,
        total_runnable_time_s, total_pending_time_s, total_p_time_s);

  /*
   * "Good" data looks like:
   *
   * total usage: time: 77.696 user: 16.570 kernel: 61.126 runnable: 79.951
   * pending: 2.255 pcore: 72.719 Thread        ALRM VTALRM   PROF   XCPU inst
   * cycle               user                  kernel          offcore  type
   * 0x16f78f000      0    251    811      0  27680301973  28913501188
   * 3706622958 (  38.14%)   6012631083 (  61.86%)    2.81%  statfs 0x16f81b000
   * 0      2    889      0  27962710058  28780576123    439297291 (   4.53%)
   * 9259942583 (  95.47%)    3.01%  stat 0x16f8a7000      0    251    836 0
   * 27558331077  28889228535   3699010000 (  38.08%)   6016015083
   * (  61.92%)    2.85%  statfs 0x16f933000      0      0    939      0
   * 28078084696  28880195679    443067500 (   4.56%)   9269807666
   * (  95.44%)    2.87%  stat 0x16f9bf000      0    283    874      0
   * 27691851016  28969873070   3710916750 (  38.16%)   6012783541
   * (  61.84%)    2.76%  statfs 0x16fa4b000      0      2    908      1
   * 27945063330  28769971396    438583000 (   4.53%)   9252694291
   * (  95.47%)    3.09%  stat 0x16fad7000      0    262    889      0
   * 27328496429  28772748055   3689245375 (  38.03%)   6011061458
   * (  61.97%)    3.00%  statfs 0x16fb63000      0      0    914      0
   * 27942195343  28757254100    439690166 (   4.53%)   9256659500
   * (  95.47%)    3.04%  stat 0x1fe2bb400   1001      0      3      0 72144372
   * 102339334      3532125 (   9.35%)     34249208 (  90.65%)   99.62%  main
   */
  ...
  for (uint32_t i = 0; i < nthreads; i++) {
    uint64_t user_time =
        abs_to_nanos_host(gThreadList[i].self_stats.ttci_user_time_mach);
    uint64_t system_time =
        abs_to_nanos_host(gThreadList[i].self_stats.ttci_system_time_mach);
    ...
    double percentage_not_running =
        (double)(not_running_time) / (double)test_duration_ns * 100;
    ...
    T_LOG("0x%010llx %6d %6d %6d %6d %12lld %12lld %12lld (%7.2f%%) %12lld "
          "(%7.2f%%) %7.2f%% %s%s\n",
          gThreadList[i].tid, gThreadList[i].alrm_count,
          gThreadList[i].vtalrm_count, gThreadList[i].prof_count,
          gThreadList[i].xcpu_count,
          gThreadList[i].self_stats.ttci_instructions,
          gThreadList[i].self_stats.ttci_cycles, user_time, percentage_user,
          system_time, percentage_system, percentage_not_running,
   ...
   *
   * This can happen if the main thread is preempted for the entire 10ms
   * duration, though. Being high priority, it shouldn't be delayed for more
   * than 10ms too often. Allow up to 10% to deliver to other threads.
   */
  if ((double)gThreadList[0].alrm_count * 100 / total_alrm < 90.0) {
    T_FAIL("SIGALRM delivered to non-main thread more than 10%% of the time "
           "(%d of %d)",
           gThreadList[0].alrm_count, total_alrm);
  }
   ...
   * threads to have an advantage and account for 80% (non-scientific) of
   * events, since the other threads will spend more time in kernel mode.
   */
  if (total_vtalrm_in_cpubound * 100 / total_vtalrm < 80) {
    T_FAIL("SIGVTALRM delivered to threads without extra userspace spin (only "
           "%d of %d)",
           total_vtalrm_in_cpubound, total_vtalrm);
  }

  /*
   * SIGPROF is delivered based on user+system time, and we expect it to be
   * distributed among non-blocked threads (so not the main thread, which only
   * handles SIGALRM).
   */
  if (gThreadList[0].prof_count * 100 / total_prof > 1) {
    T_FAIL("SIGPROF delivered to main thread more than 1%% (%d of %d)",
           gThreadList[0].prof_count, total_prof);
  }
...
static void *stat_thread(void *arg) {
  ...
  /* This wait can be aborted by one of the signals, so we make sure to wait for
   * the first iteration of main */
  kr = semaphore_wait_signal(gWorkersStart, gMainWaitForWorkers);
  if (kr != KERN_ABORTED) {
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "semaphore_wait_signal()");
  }
  ...
  do {
    ...
    if (rv != 0) {
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(rv, "stat");
    }
...
#define SYSCALL_ITERATIONS_BETWEEN_SPINS (10000)
    ...
    if (rv != 0) {
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(rv, "statfs");
    }
  } while (os_atomic_load(&gShouldExit, relaxed) == false);
...
static void alrm_handler(int signum, struct __siginfo *info __unused,
                         void *uap) {
...
static void *cpu_thread_main(void *arg) {
  bool *spin = arg;
  while (*spin) {
  }
  return NULL;
...
static void sigprof_received(int __unused sig) {
...
T_DECL(setitimer_prof,
  ...
  uint64_t end_timeout_ns =
      expected_end_ns + (EXTRA_TIMEOUT_SECS * NSEC_PER_SEC);
  ...
  T_EXPECT_LE(last_ns, end_timeout_ns,
              "received SIGPROF within the timeout (%.9gs < %.9gs)",
              (double)(last_ns - start_ns) / 1e9,
              (double)(end_timeout_ns - start_ns) / 1e9);
  T_LOG("SIGPROF was delivered %+.6gms after expected duration",
        (double)(last_ns - expected_end_ns) / 1e6);
  T_EXPECT_GE(last_ns, expected_end_ns,
              "received SIGPROF after enough time (%.9gs > %.9gs)",
              (double)(last_ns - start_ns) / 1e9,
              (double)(expected_end_ns - start_ns) / 1e9);
}
...
T_DECL(setitimer_prof_multi_threaded,
  ...
  uint64_t expected_end_ns =
      start_ns + (EXPECTED_PROF_DURATION_SECS * NSEC_PER_SEC);
  uint64_t end_timeout_ns =
      expected_end_ns + (EXTRA_TIMEOUT_SECS * NSEC_PER_SEC);
  ...
  for (unsigned int i = 0; i < PROF_EXTRA_THREAD_COUNT; i++) {
    int error = pthread_create(&cpu_threads[i], NULL, cpu_thread_main,
                               &spin_while_true);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(error, "create thread %d", i);
  }
...
static void sigprof_received_usage(int __unused sig) {
  sigprof_received_usage_mach = __thread_selfusage();
}

T_DECL(setitimer_prof_latency, "measure the latency of delivering SIGPROF",
       T_META_TAG_PERF, T_META_TAG_VM_NOT_ELIGIBLE, T_META_TIMEOUT(10)) {
  T_SETUPBEGIN;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "mach_timebase_info");

  char pd_path[PATH_MAX] = "";
  pdwriter_t pd = pdwriter_open_tmp("xnu", "setitimer_sigprof_latency", 1, 0,
                                    pd_path, sizeof(pd_path));
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(pd, "opened perfdata file");
  T_SETUPEND;

  T_LOG("running %d iterations, %lluus each", ITERATION_COUNT,
        ITIMER_PERF_PROF_US);
  for (int i = 0; i < ITERATION_COUNT; i++) {
    ...
    while (sigprof_received_usage_mach == 0) {
      ...
      // Issue a trivial syscall (getpid has a fast path without making a
      // syscall) to ensure the system checks the timer.
      (void)getppid();
    }

    uint64_t const end_usage_mach =
        start_usage_mach +
