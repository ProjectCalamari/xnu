...
void disable_verbose_sched_utils(void) {
  T_QUIET;
  T_ASSERT_TRUE(verbosity_enabled, "verbosity was enabled");
  verbosity_enabled = false;
}

void reenable_verbose_sched_utils(void) {
  T_QUIET;
  T_ASSERT_EQ(verbosity_enabled, false, "verbosity was disabled");
...
unsigned int platform_nperflevels(void) {
  if (num_perf_levels == 0) {
    int ret;
    ret = sysctlbyname("hw.nperflevels", &num_perf_levels,
                       &(size_t){sizeof(num_perf_levels)}, NULL, 0);
    T_QUIET;
...
const char *platform_perflevel_name(unsigned int perflevel) {
  if (perflevel_names[perflevel][0] == 0) {
    ...
    snprintf(sysctl_name, sizeof(sysctl_name), "hw.perflevel%d.name",
             perflevel);
    ret = sysctlbyname(sysctl_name, &perflevel_names[perflevel],
                       &(size_t){sizeof(perflevel_names[perflevel])}, NULL, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "%s", sysctl_name);
  }
...
unsigned int platform_perflevel_ncpus(unsigned int perflevel) {
  if (perflevel_ncpus[perflevel] == 0) {
    int ret;
    char sysctl_name[64] = {0};
    snprintf(sysctl_name, sizeof(sysctl_name), "hw.perflevel%d.logicalcpu",
             perflevel);
    ret = sysctlbyname(sysctl_name, &perflevel_ncpus[perflevel],
                       &(size_t){sizeof(perflevel_ncpus[perflevel])}, NULL, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "%s", sysctl_name);
  }
...
bool platform_is_amp(void) {
  ...
  if (verbosity_enabled && !reported_is_amp) {
    T_LOG("🛰️ Platform has %d perflevels (%s)", platform_nperflevels(),
          is_amp ? "AMP" : "SMP");
...
bool platform_is_virtual_machine(void) {
  int ret;
  int vmm_present = 0;
  ret = sysctlbyname("kern.hv_vmm_present", &vmm_present,
                     &(size_t){sizeof(vmm_present)}, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kern.hv_vmm_present");
  if (vmm_present) {
...
char *platform_sched_policy(void) {
  int ret;
  ret = sysctlbyname("kern.sched", sched_policy_name,
                     &(size_t){sizeof(sched_policy_name)}, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kern.sched");
  if (verbosity_enabled) {
    T_LOG("🛰️ Platform is running the \"%s\" scheduler policy",
          sched_policy_name);
  }
...
char bind_to_cluster_of_type(char type) {
  int ret;
  char old_type;
  ret = sysctlbyname("kern.sched_thread_bind_cluster_type", &old_type,
                     &(size_t){sizeof(old_type)}, &type, sizeof(type));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kern.sched_thread_bind_cluster_type");
  return old_type;
}
...
int bind_to_cluster_id(int cluster_id) {
  ...
  ret = sysctlbyname("kern.sched_thread_bind_cluster_id", &old_cluster,
                     &(size_t){sizeof(old_cluster)}, &cluster_id,
                     sizeof(cluster_id));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kern.sched_thread_bind_cluster_id");
  return old_cluster;
}
...
pthread_attr_t *create_pthread_attr(int priority, detach_state_t detach_state,
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "pthread_attr_init");

  struct sched_param param = {.sched_priority = priority};
  ret = pthread_attr_setschedparam(attr, &param);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "pthread_attr_setschedparam");

  if (detach_state == eDetached) {
    ret = pthread_attr_setdetachstate(attr, PTHREAD_CREATE_DETACHED);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "pthread_attr_setdetachstate");
  }

  if (qos_class != QOS_CLASS_UNSPECIFIED) {
    ret = pthread_attr_set_qos_class_np(attr, qos_class, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "pthread_attr_set_qos_class_np");
  }
  ...
  if (sched_policy != eSchedDefault) {
    ...
    case eSchedDefault:
      T_QUIET;
      T_ASSERT_FAIL("unexpected sched_policy");
      break;
    }
    ret = pthread_attr_setschedpolicy(attr, (int)sched_policy);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "pthread_attr_setschedpolicy");
  }

  if (stack_size != DEFAULT_STACK_SIZE) {
    ret = pthread_attr_setstacksize(attr, stack_size);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(ret, "pthread_attr_setstacksize");
  }
  return attr;
}
...
void create_thread_pri(pthread_t *thread_handle, int priority,
                       void *(*func)(void *), void *arg) {
  pthread_attr_t *attr =
      create_pthread_attr(priority, eJoinable, QOS_CLASS_UNSPECIFIED,
                          eSchedDefault, DEFAULT_STACK_SIZE);
...
pthread_t *create_threads(int num_threads, int priority,
  ...
  pthread_attr_t *attr = create_pthread_attr(priority, detach_state, qos_class,
                                             sched_policy, stack_size);

  pthread_t *thread_handles =
      (pthread_t *)malloc(sizeof(pthread_t) * (size_t)num_threads);
  for (int i = 0; i < num_threads; i++) {
    create_thread(&thread_handles[i], attr, func,
                  arg_array == NULL ? NULL : arg_array[i]);
  }
...
bool wait_for_quiescence_default(int argc, char *const argv[]) {
  return wait_for_quiescence(argc, argv, default_idle_threshold,
                             default_timeout_sec);
}
...
bool wait_for_quiescence(int argc, char *const argv[], double idle_threshold,
  ...
  if (verbosity_enabled) {
    T_LOG("🕒 Waiting up to %d second(s) for the system to quiesce above "
          "%.2f%% idle...",
  ...
  while (waited_seconds < timeout_seconds) {
    kr = host_statistics(mach_host_self(), HOST_CPU_LOAD_INFO,
                         (host_info_t)&host_load, &count);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "host_statistics HOST_CPU_LOAD_INFO");
    ...
    if (waited_seconds >= 1) {
      ...
      double total_delta = idle_delta +
                           (user_ticks[ind] - user_ticks[old_ind]) +
                           (system_ticks[ind] - system_ticks[old_ind]);
      if (total_delta > 0.0) {
  ...
  if (verbosity_enabled) {
    if (quiesced) {
      T_LOG("🕒 System quiesced to %.2f%% idle within %d second(s)",
            idle_ratio * 100.0, waited_seconds);
    } else {
      T_LOG("🕒 Failed to quiesce within %.2f%% idle after %d second(s)",
            idle_threshold * 100.0, waited_seconds);
    }
  }
  return quiesced;
}
...
static void sched_utils_sigint_handler(int sig) {
  T_QUIET;
  T_EXPECT_EQ(sig, SIGINT, "unexpected signal received");
  T_ASSERT_TRUE(
      false,
      "SIGINT received. Failing test to induce ATEND handlers for cleanup...");
}

static void register_atend_handler(void) {
...
static void clpcctrl_cleanup(void) {
...
uint64_t execute_clpcctrl(char *clpcctrl_args[], bool read_value) {
  ...
  while (clpcctrl_args[arg_ind] != NULL) {
    T_QUIET;
    T_ASSERT_LT(arg_ind + 1, 100, "too many clpcctrl args");
  ...
  pid_t pid = dt_launch_tool_pipe(
      full_clpcctrl_args, false, NULL,
  ...
  T_QUIET;
  T_EXPECT_TRUE(ret, "dt_waitpid for clpcctrl");
...
bool check_recommended_core_mask(uint64_t *core_mask) {
  ...
  ret = sysctlbyname("kern.sched_recommended_cores", &recommended_cores,
                     &recommended_cores_size, NULL, 0);
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(ret, "sysctlbyname(kern.sched_recommended_cores)");

  if (verbosity_enabled) {
    uint64_t expected_recommended_mask = ~0ULL >> (64 - dt_ncpu());
    T_LOG("📈 kern.sched_recommended_cores: %016llx, expecting %016llx if all "
          "are recommended",
...
static bool trace_requested(int argc, char *const argv[]) {
  for (int i = 0; i < argc; i++) {
  ...
  T_QUIET;
  T_ASSERT_TRUE(tracing_enabled || !trace_save_requested,
                "Can't use both --save-trace and --no-trace");
  return tracing_enabled;
}

static void atend_trace_cleanup(void) {
  int ret;
  for (int i = 0; i < handle_ind; i++) {
    ...
    T_QUIET;
    T_EXPECT_EQ(handle->status, ENDED, "ended trace");
    if (handle->status == ENDED &&
        ((T_FAILCOUNT > 0) || trace_save_requested)) {
    ...
    T_QUIET;
    T_WITH_ERRNO;
    T_EXPECT_POSIX_SUCCESS(ret, "kill SIGKILL");
  }
}

static bool sched_utils_tracing_supported(void) {
#if TARGET_OS_BRIDGE
  /*
...
static const int waiting_timeout_sec =
    60 * 2; /* 2 minutes, allows trace post-processing to finish */
#else       /* !(TARGET_OS_WATCH || TARGET_OS_TV) */
static const int waiting_timeout_sec =
    60 * 3 + 30; /* 3 minutes and 30 seconds for slower targets */
#endif      /* !(TARGET_OS_WATCH || TARGET_OS_TV) */

trace_handle_t begin_collect_trace_fmt(collect_trace_flags_t flags, int argc,
                                       char *const argv[], char *fmt, ...) {
  /* Check trace requirements */
  if (!sched_utils_tracing_supported() || !trace_requested(argc, argv)) {
    return NULL;
  }
  T_QUIET;
  T_ASSERT_EQ(geteuid(), 0,
              "🖊️ Tracing requires the test to be run as root user");

  int ret;
  struct trace_handle *handle = &handles[handle_ind++];
  T_QUIET;
  T_ASSERT_LE(handle_ind, MAX_TRACES, "Ran out of trace handles");
  ...
  snprintf(full_filename, MAXPATHLEN, "%s/%s.atrc", dt_tmpdir(),
           handle->short_name);
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_TRUE(ret == 0 || errno == ENOENT, "remove trace file");
  ...
  char *wait_on_start_args[] = {"/usr/bin/notifyutil", "-1", begin_notification,
                                NULL};
  ret = dt_launch_tool(&handle->wait_on_start_pid, wait_on_start_args, false,
                       NULL, NULL);
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_EQ(ret, 0, "dt_launch_tool");
  char *wait_on_end_args[] = {"/usr/bin/notifyutil", "-1", end_notification,
                              NULL};
  ret = dt_launch_tool(&handle->wait_on_end_pid, wait_on_end_args, false, NULL,
                       NULL);
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_EQ(ret, 0, "dt_launch_tool");
  ...
  /* Launch trace record */
  char *trace_args_base[18] = {trace_bin,
                               "record",
                               handle->abs_filename,
                               "--plan",
                               "default",
                               "--unsafe",
                               "--kdebug-filter-include",
                               "C0x01",
                               "--omit",
                               "Logging",
                               "--kdebug-buffer-size",
  ...
  const unsigned trace_args_cap =
      32; /* INCREASE THIS if there are too many trace args */
  ...
  for (unsigned i = 0; i < sizeof(trace_args_base) / sizeof(char *); ++i) {
    ...
    T_QUIET;
    T_ASSERT_LT(trace_args_len, trace_args_cap, "too many trace args");
  }
  ...
  pid_t trace_pid = dt_launch_tool_pipe(
      trace_args, false, NULL,
      ^bool(char *data, __unused size_t data_size,
            __unused dt_pipe_data_handler_context_t *context) {
  ...
  T_LOG("🖊️ Starting trace collection for \"%s\" trace[%u]",
        handle->trace_filename, trace_pid);

  /* Wait for tracing to start */
  int signal_num = 0;
  ret = dt_waitpid(handle->wait_on_start_pid, NULL, &signal_num,
                   waiting_timeout_sec);
  T_QUIET;
  T_EXPECT_TRUE(ret, "dt_waitpid for trace start signal_num %d", signal_num);
...
void end_collect_trace(trace_handle_t handle) {
  ...
  T_QUIET;
  T_EXPECT_EQ(trace_state->status, STARTED, "trace was started");

  /* Notify trace to stop tracing */
  char *wait_on_start_args[] = {notifyutil_bin, "-p", trigger_end_notification,
                                NULL};
  pid_t trigger_end_pid = 0;
  ret = dt_launch_tool(&trigger_end_pid, wait_on_start_args, false, NULL, NULL);
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_EQ(ret, 0, "dt_launch_tool for notify end trace");

  /* Wait for tracing to actually stop */
  T_LOG("🖊️ Now waiting on trace to finish up...");
  int signal_num = 0;
  int exit_status = 0;
  ret = dt_waitpid(trace_state->wait_on_end_pid, &exit_status, &signal_num,
                   waiting_timeout_sec);
  T_QUIET;
  T_EXPECT_TRUE(ret, "dt_waitpid for trace stop, exit status %d signal_num %d",
                exit_status, signal_num);
...
void save_collected_trace(trace_handle_t handle) {
  ...
  struct trace_handle *trace_state = (struct trace_handle *)handle;
  T_QUIET;
  T_EXPECT_EQ(trace_state->status, ENDED, "trace was ended");
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(ret, "dt_resultfile marking \"%s\" for collection",
                      compressed_path);
  T_LOG("🖊️ \"%s\" marked for upload", compressed_path);

  char *compress_args[] = {
      aa_bin, "archive",           "-i", trace_state->trace_filename,
      "-d",   (char *)dt_tmpdir(), "-o", compressed_path,
      NULL};
  pid_t aa_pid = dt_launch_tool_pipe(
      compress_args, false, NULL,
      ^bool(__unused char *data, __unused size_t data_size,
            __unused dt_pipe_data_handler_context_t *context) {
        T_LOG("🖊️ [aa] %s", data);
        return false;
      },
      ...
      BUFFER_PATTERN_LINE, NULL);
  T_QUIET;
  T_EXPECT_TRUE(aa_pid, "🖊️ [aa] pid %d", aa_pid);
  int exit_status = 0;
  int signal_num = SIGPIPE;
  ret = dt_waitpid(aa_pid, &exit_status, &signal_num, 0);
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_TRUE(ret, "dt_waitpid for aa, exit status %d signal num %d",
                exit_status, signal_num);

  /* Lax permissions in case a user wants to open the compressed file without
   * sudo */
  ret = chmod(compressed_path, 0666);
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_ZERO(ret, "chmod");

  T_LOG(
      "🖊️ Finished saving trace (%s), which is available compressed at \"%s\"",
...
void discard_collected_trace(trace_handle_t handle) {
  ...
  struct trace_handle *trace_state = (struct trace_handle *)handle;
  T_QUIET;
  T_EXPECT_TRUE(trace_state->status == ENDED || trace_state->status == SAVED,
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_EXPECT_POSIX_SUCCESS(ret, "remove trace file");

  if (trace_state->status == ENDED) {
    T_LOG("🖊️ Deleted recorded trace file at \"%s\"",
          trace_state->abs_filename);
  }
