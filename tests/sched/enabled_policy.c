...
static void get_device_name(char *device_name, size_t device_name_len) {
  int ret;
  ret = sysctlbyname("hw.target", device_name, &device_name_len, NULL, 0);
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "sysctlbyname hw.target");
}

static void get_kern_version(char *kern_version, size_t kern_version_len) {
  int ret;
  ret = sysctlbyname("kern.version", kern_version, &kern_version_len, NULL, 0);
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "sysctlbyname kern.version");
}

static bool platform_is_arm64(void) {
  int ret;
  int is_arm64 = 0;
...
T_DECL(enabled_policy, "Verify that the expected scheduler policy is running",
  ...
  if (!platform_is_amp()) {
    T_ASSERT_EQ_STR(policy_name, "clutch",
                    "SMP platform should be running the Clutch scheduler");
    T_END;
  }

  T_ASSERT_EQ_STR(
      policy_name, "edge",
      "Non-exempt AMP platform should be running the Edge scheduler");
}
