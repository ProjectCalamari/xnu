...
T_GLOBAL_META(T_META_NAMESPACE("xnu.scheduler"),
              ...
              T_META_RADAR_COMPONENT_VERSION("scheduler"), T_META_TAG_PERF,
...
static void environment_init(void) {
  ...
  if (platform_is_amp()) {
     ...
     * compete over the same cores irrespective of CLPC's cluster
     * recommendations
     */
...
static uint32_t make_work_interval(work_interval_t *handle,
                                   uint32_t work_type_flags) {
  int ret;
  uint32_t work_interval_flags =
...
static void start_threads(pthread_t *threads, struct thread_data *thread_datas,
                          work_interval_t *handle,
                          uint32_t work_interval_flags) {
  int ret;
...
static uint64_t snapshot_user_time_usec(pthread_t *threads) {
  kern_return_t kr;
  ...
  for (unsigned int i = 0; i < num_cores; i++) {
    ...
    kr = thread_info(thread_port, THREAD_BASIC_INFO, (thread_info_t)&info,
                     &count);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "thread_info");
    uint64_t thread_usr_usec =
        (uint64_t)(info.user_time.seconds) * USEC_PER_SEC +
...
T_DECL(
  ...
  uint32_t lower_pri_flags = make_work_interval(
      &lower_pri_handle,
      WORK_INTERVAL_TYPE_DEFAULT | WORK_INTERVAL_FLAG_HAS_WORKLOAD_ID);
  uint32_t higher_pri_flags =
      make_work_interval(&higher_pri_handle, WORK_INTERVAL_TYPE_DEFAULT);
  ...
  start_threads(lower_threads, lower_thread_datas, &lower_pri_handle,
                lower_pri_flags);
  ...
  start_threads(higher_threads, higher_thread_datas, &higher_pri_handle,
                higher_pri_flags);

  T_SETUPEND;

  /* Snapshot thread runtimes */
  uint64_t start_lower_priority_runtime_usec =
      snapshot_user_time_usec(lower_threads);
  uint64_t start_higher_priority_runtime_usec =
      snapshot_user_time_usec(higher_threads);
   ...
   * higher priority thread groups, to determine whether the lower priority
   * group has been starved
   */
  uint64_t finish_lower_priority_runtime_usec =
      snapshot_user_time_usec(lower_threads);
  uint64_t finish_higher_priority_runtime_usec =
      snapshot_user_time_usec(higher_threads);

  uint64_t lower_priority_runtime =
      finish_lower_priority_runtime_usec - start_lower_priority_runtime_usec;
  uint64_t higher_priority_runtime =
      finish_higher_priority_runtime_usec - start_higher_priority_runtime_usec;

  T_QUIET;
  T_ASSERT_GT(lower_priority_runtime, 10000LL,
              "lower priority thread group got at least 10ms of CPU time");
  T_QUIET;
  T_ASSERT_GT(higher_priority_runtime, 10000LL,
              "higher priority thread group got at least 10ms of CPU time");

  /* Record the observed runtime ratio */
  char pdj_path[MAX_PDJ_PATH_LEN];
  pdwriter_t writer =
      pdwriter_open_tmp("xnu", "scheduler.thread_group_fairness", 0, 0,
                        pdj_path, MAX_PDJ_PATH_LEN);
  T_QUIET;
  T_WITH_ERRNO;
  ...
  double total_runtime =
      (double)(lower_priority_runtime + higher_priority_runtime);
  ...
  pdwriter_new_value(writer, "Thread Group Runtime Ratio",
                     PDUNIT_CUSTOM(runtime_ratio), runtime_ratio_value);
  pdwriter_record_larger_better(writer);
  pdwriter_close(writer);
  /* Ensure that the perfdata file can be copied by BATS */
  T_QUIET;
  T_ASSERT_POSIX_ZERO(chmod(pdj_path, 0644), "chmod");

  T_END;
}

static uint64_t get_thread_group_cpu_time(int sched_bucket) {
  int ret;
  uint64_t cpu_stats[2];
  size_t cpu_stats_len = sizeof(uint64_t) * 2;
  ret = sysctlbyname("kern.clutch_bucket_group_cpu_stats", cpu_stats,
                     &cpu_stats_len, &sched_bucket, sizeof(sched_bucket));
  if (ret != 0 && errno == ENOTSUP) {
    T_LOG("Test only supported on Clutch/Edge scheduler (current policy is "
          "\"%s\") "
          "platforms on development/debug build variants",
          platform_sched_policy());
    T_SKIP("kern.clutch_bucket_group_cpu_stats development-only sysctl not "
           "present");
  }
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_POSIX_SUCCESS(ret, "kern.clutch_bucket_group_cpu_stats");
  return cpu_stats[0];
}
...
static void *boost_while_working(void *arg) {
  int ret;
  ...
  while (mach_absolute_time() < mach_deadline) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_ZERO(ret, "pthread_set_qos_class_self_np UI");
    T_QUIET;
    T_ASSERT_EQ(qos_class_self(), high_qos, "qos_class_self");
    ...
    T_QUIET;
    T_ASSERT_POSIX_ZERO(ret, "pthread_set_qos_class_self_np IN");
    T_QUIET;
    T_ASSERT_EQ(qos_class_self(), low_qos, "qos_class_self");
    usleep(2 * 1000); // 2ms
  }
  ...
  if (is_recorder) {
    ...
    T_QUIET;
    T_ASSERT_GE(cpu_time_end_high, cpu_time_begin_high,
                "non-monotonic thread group CPU time");
    uint64_t high_cpu_time = cpu_time_end_high - cpu_time_begin_high;
    T_QUIET;
    T_ASSERT_GE(cpu_time_end_low, cpu_time_begin_low,
                "non-monotonic thread group CPU time");
    uint64_t low_cpu_time = cpu_time_end_low - cpu_time_begin_low;

    T_QUIET;
    T_ASSERT_GT(high_cpu_time + low_cpu_time, 0ULL,
                "CPU not attributed to either expected bucket");
    T_LOG("High ticks: %llu, Low ticks: %llu, High-to-low ratio: %.3f",
          high_cpu_time, low_cpu_time,
          high_cpu_time * 1.0 / (high_cpu_time + low_cpu_time));
    T_EXPECT_GE(high_cpu_time, low_cpu_time,
                "More work accounted to the high QoS");
...
T_DECL(interactivity_cpu_accounting,
...
#if TARGET_OS_WATCH
       T_META_MAYFAIL(
           "Watches too noisy with high priority spinners (rdar://150323037)"),
#elif TARGET_OS_TV
       T_META_MAYFAIL(
           "TVs too noisy with high priority audio (rdar://149974201)"),
#endif
       T_META_ASROOT(YES)) {
  ...
  kern_return_t kr =
      task_policy_get(mach_task_self(), TASK_POLICY_STATE,
                      (task_policy_t)&policy_state, &count, &get_default);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "task_policy_get(self, TASK_POLICY_STATE)");
  int requested_app_type = (policy_state.requested & POLICY_REQ_APPTYPE_MASK) >>
                           POLICY_REQ_APPTYPE_SHIFT;
  T_QUIET;
  T_ASSERT_EQ(requested_app_type, TASK_APPTYPE_APP_DEFAULT,
              "Test needs to be run like an application for QoS boosting above "
              "pri 37 to succeed");
