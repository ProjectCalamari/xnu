 ...
 * the Clutch runqueue logic, followed by needed functions from
 * osfmk/kern/thread.c for operating on the __runq field
 */
struct thread {
  int id;
...
#define TH_SFLAG_BOUND_SOFT                                                    \
  0x20000 /* thread is soft bound to a cluster; can run anywhere if bound      \
             cluster unavailable */
...
#define recount_thread_time_mach(thread)                                       \
  (thread->mock_recount_time.user_time + thread->mock_recount_time.system_time)
...
uint64_t thread_group_get_id(struct thread_group *tg) { return tg->tg_id; }

#if CONFIG_SCHED_EDGE

bool thread_shared_rsrc_policy_get(thread_t thread,
