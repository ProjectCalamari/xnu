...
/*
 * Include non-kernel header dependencies to make up for the equivalent kernel
 * header dependencies which are not safe to compile in a userspace binary
 */
...
extern unsigned int ml_get_cpu_number_type(cluster_type_t cluster_type,
...
#define TUNABLE(type_t, var, boot_arg, default_value) type_t var = default_value
