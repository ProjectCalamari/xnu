...
processor_t current_processor(void) { return cpus[_curr_cpu]; }
...
unsigned int ml_get_cpu_number_type(cluster_type_t cluster_type, bool logical,
  ...
  (void)logical;
  if (available) {
    return recommended_cpu_count_for_type[cluster_type_to_test_cpu_type(
        cluster_type)];
  } else {
...
static void edge_impl_set_cluster_type(processor_set_t pset,
                                       test_cpu_type_t type) {
  switch (type) {
...
static void edge_impl_init_runqueues(void) {
  ...
  for (int i = 0; i < curr_hw_topo.num_psets; i++) {
    ...
    bzero(&psets[i]->pset_execution_time,
          sizeof(psets[i]->pset_execution_time));
    ...
    cpu_count_for_type[curr_hw_topo.psets[i].cpu_type] +=
...
void impl_init_runqueue(void) {
  assert(curr_hw_topo.num_psets == 0);
  curr_hw_topo = single_core;
  edge_impl_init_runqueues();
}

void impl_init_migration_harness(test_hw_topology_t hw_topology) {
  assert(curr_hw_topo.num_psets == 0);
  curr_hw_topo = hw_topology;
  edge_impl_init_runqueues();
}

struct thread_group *impl_create_tg(int interactivity_score) {
  return clutch_impl_create_tg(interactivity_score);
}

test_thread_t impl_create_thread(int root_bucket, struct thread_group *tg,
                                 int pri) {
  return clutch_impl_create_thread(root_bucket, tg, pri);
}

void impl_set_thread_processor_bound(test_thread_t thread, int cpu_id) {
  _curr_cpu = cpu_id;
  clutch_impl_set_thread_processor_bound(thread, cpu_id);
}

void impl_set_thread_cluster_bound(test_thread_t thread, int cluster_id) {
...
void impl_cpu_set_thread_current(int cpu_id, test_thread_t thread) {
  _curr_cpu = cpu_id;
  ...
  sched_bucket_t bucket =
      ((((thread_t)thread)->state & TH_IDLE) ||
       (((thread_t)thread)->bound_processor != PROCESSOR_NULL))
          ? TH_BUCKET_SCHED_MAX
          : ((thread_t)thread)->th_sched_bucket;
  os_atomic_store(&cpus[cpu_id]->processor_set->cpu_running_buckets[cpu_id],
                  bucket, relaxed);
  ...
  if (rt_pset_has_stealable_threads(pset)) {
    rt_choose_next_processor_for_spill_IPI(
        pset, cpus[cpu_id], &next_rt_processor, &next_rt_ipi_type);
...
test_thread_t impl_cpu_clear_thread_current(int cpu_id) {
  _curr_cpu = cpu_id;
  test_thread_t thread = clutch_impl_cpu_clear_thread_current(cpu_id);
  pset_update_processor_state(cpus[cpu_id]->processor_set, cpus[cpu_id],
                              PROCESSOR_IDLE);
  os_atomic_store(&cpus[cpu_id]->processor_set->cpu_running_buckets[cpu_id],
...
void impl_cpu_enqueue_thread(int cpu_id, test_thread_t thread) {
  _curr_cpu = cpu_id;
...
test_thread_t impl_cpu_dequeue_thread_compare_current(int cpu_id) {
  _curr_cpu = cpu_id;
  ...
  if (rt_runq_count(pset) > 0) {
    ...
    return sched_clutch_choose_thread(cpus[cpu_id], MINPRI,
                                      cpus[cpu_id]->active_thread, 0);
  }
}

bool impl_processor_csw_check(int cpu_id) {
  _curr_cpu = cpu_id;
  assert(cpus[cpu_id]->active_thread != NULL);
...
void impl_pop_tracepoint(uint64_t *clutch_trace_code, uint64_t *arg1,
                         uint64_t *arg2, uint64_t *arg3, uint64_t *arg4) {
  clutch_impl_pop_tracepoint(clutch_trace_code, arg1, arg2, arg3, arg4);
}

int impl_choose_pset_for_thread(test_thread_t thread) {
...
bool impl_thread_avoid_processor(test_thread_t thread, int cpu_id,
                                 bool quantum_expired) {
  _curr_cpu = cpu_id;
  return sched_edge_thread_avoid_processor(
      cpus[cpu_id], (thread_t)thread, quantum_expired ? AST_QUANTUM : AST_NONE);
}

void impl_cpu_expire_quantum(int cpu_id) {
  _curr_cpu = cpu_id;
  sched_edge_quantum_expire(cpus[cpu_id]->active_thread);
  cpus[cpu_id]->first_timeslice = FALSE;
}

test_thread_t impl_steal_thread(int cpu_id) {
  _curr_cpu = cpu_id;
  return sched_edge_processor_idle(psets[cpu_id_to_pset_id(cpu_id)]);
}

bool impl_processor_balance(int cpu_id) {
  _curr_cpu = cpu_id;
  return sched_edge_balance(cpus[cpu_id], psets[cpu_id_to_pset_id(cpu_id)]);
}

void impl_set_current_processor(int cpu_id) { _curr_cpu = cpu_id; }

void impl_set_tg_sched_bucket_preferred_pset(struct thread_group *tg,
                                             int sched_bucket, int cluster_id) {
  ...
  sched_edge_update_preferred_cluster(clutch, modify_bitmap,
                                      tg_bucket_preferred_cluster);
}

void impl_set_pset_load_avg(int cluster_id, int QoS, uint64_t load_avg) {
  assert(QoS > 0 && QoS < TH_BUCKET_SCHED_MAX);
  pset_array[cluster_id]->pset_load_average[QoS] = load_avg;
}

void edge_set_thread_shared_rsrc(test_thread_t thread, bool native_first) {
...
void impl_set_pset_derecommended(int cluster_id) {
  processor_set_t pset = pset_array[cluster_id];
  pset->recommended_bitmask = 0;
  atomic_bit_clear(&pset->node->pset_recommended_map, cluster_id,
                   memory_order_relaxed);
  recommended_cpu_count_for_type[cluster_type_to_test_cpu_type(
      pset->pset_type)] -= bit_count(pset->cpu_bitmask);
}

void impl_set_pset_recommended(int cluster_id) {
  processor_set_t pset = pset_array[cluster_id];
  pset->recommended_bitmask = pset->cpu_bitmask;
  atomic_bit_set(&pset->node->pset_recommended_map, cluster_id,
                 memory_order_relaxed);
  recommended_cpu_count_for_type[cluster_type_to_test_cpu_type(
      pset->pset_type)] += bit_count(pset->cpu_bitmask);
}

void impl_pop_ipi(int *cpu_id, test_ipi_type_t *ipi_type) {
...
bool impl_thread_should_yield(int cpu_id) {
  _curr_cpu = cpu_id;
  assert(cpus[cpu_id]->active_thread != NULL);
  return sched_edge_thread_should_yield(cpus[cpu_id],
                                        cpus[cpu_id]->active_thread);
}

void impl_send_ipi(int cpu_id, test_thread_t thread, test_ipi_event_t event) {
  sched_ipi_type_t triggered_ipi = sched_ipi_action(
      cpus[cpu_id], (thread_t)thread, (sched_ipi_event_t)event);
  sched_ipi_perform(cpus[cpu_id], triggered_ipi);
}

int rt_pset_spill_search_order_at_offset(int src_pset_id, int offset) {
  return psets[src_pset_id]
      ->sched_rt_spill_search_order.spso_search_order[offset];
}

void rt_pset_recompute_spill_order(int src_pset_id) {
  sched_rt_config_pset_push(psets[src_pset_id]);
}

uint32_t impl_qos_max_parallelism(int qos, uint64_t options) {
  return sched_edge_qos_max_parallelism(qos, options);
}

int *impl_iterate_pset_search_order(int src_pset_id, uint64_t candidate_map,
                                    int sched_bucket) {
  ...
  while (sched_iterate_psets_ordered(
      starting_pset, &starting_pset->spill_search_order[sched_bucket],
      candidate_map, &istate)) {
    psets[ind++] = istate.spis_pset_id;
  }
  return psets;
}

test_thread_t impl_rt_choose_thread(int cpu_id) {
  return sched_rt_choose_thread(cpus[cpu_id]);
}

void sched_rt_spill_policy_set(unsigned policy) {
  impl_sched_rt_spill_policy_set(policy);
}

void sched_rt_steal_policy_set(unsigned policy) {
