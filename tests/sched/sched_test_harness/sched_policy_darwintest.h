...
static void sched_policy_speak(char *message) {
  char *fun_level = getenv("SCHED_FUN");
  if ((fun_level != NULL) && (strcmp(fun_level, "MAX") == 0)) {
    char *say_args[] = {"/usr/local/bin/say_anything",
                        "-v",
                        "damon",
                        "-r 210",
                        T_NAME,
...
static void sched_policy_final_pass(void) {
  if (T_FAILCOUNT == 0) {
...
#define SCHED_POLICY_TEST_NAME(policy_name, test_name)                         \
  PASTER(policy_name, test_name)
#define SCHED_POLICY_T_DECL(test_name, description, ...)                       \
  T_DECL(SCHED_POLICY_TEST_NAME(TEST_RUNQ_POLICY, test_name), description,     \
         T_META_TAG_VM_PREFERRED, ##__VA_ARGS__)
...
#define SCHED_POLICY_PASS(message, ...)                                        \
  ({                                                                           \
    ...
    if (T_FAILCOUNT <= sched_policy_fails_so_far) {                            \
      ...
      T_PASS(expanded_message, ##__VA_ARGS__,                                  \
             (T_PASSCOUNT - sched_policy_passes_so_far));                      \
      sched_policy_passed_subtests++;                                          \
    } else {                                                                   \
      strcat(expanded_message, message);                                       \
      strcat(expanded_message, SCHED_FAIL_MSG);                                \
      T_FAIL(expanded_message, ##__VA_ARGS__,                                  \
             (T_FAILCOUNT - sched_policy_fails_so_far),                        \
             (T_PASSCOUNT - sched_policy_passes_so_far + T_FAILCOUNT -         \
              sched_policy_fails_so_far));                                     \
    }                                                                          \
...
static void sched_policy_push_metadata(char *metada_name, uint64_t value) {
  snprintf(metadata_log[metadata_ind++], 256, "%s %llu", metada_name, value);
}

static void sched_policy_pop_metadata(void) {
  T_QUIET;
  T_EXPECT_GT(metadata_ind, 0, "no metadata left to pop");
  metadata_ind--;
}

#define MAX_METADA_DUMP_STR (MAX_METADATA_STR * MAX_METADATA)
static char metadata_dump[MAX_METADA_DUMP_STR];
static char *sched_policy_dump_metadata(void) {
  metadata_dump[0] = '(';
