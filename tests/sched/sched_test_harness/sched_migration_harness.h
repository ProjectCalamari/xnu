...
extern void set_tg_sched_bucket_preferred_pset(struct thread_group *tg,
                                               int sched_bucket,
                                               int cluster_id);
...
extern bool choose_pset_for_thread_expect(test_thread_t thread,
...
extern void cpu_send_ipi_for_thread(int cpu_id, test_thread_t thread,
                                    test_ipi_event_t event);
#define QOS_PARALLELISM_REALTIME 0x2
#define QOS_PARALLELISM_CLUSTER_SHARED_RESOURCE 0x4
extern bool max_parallelism_expect(int qos, uint64_t options,
                                   uint32_t expected_parallelism);
extern int iterate_pset_search_order_expect(int src_pset_id,
                                            uint64_t candidate_map,
                                            int sched_bucket,
                                            int *expected_pset_ids,
                                            int num_psets);
