...
#define KERNEL_DEBUG_CONSTANT_IST(a0, a1, a2, a3, a4, a5, a6)                  \
  clutch_impl_log_tracepoint(a1, a2, a3, a4, a5)
...
/* Clutch policy code under-test, safe to include now after satisfying its
 * dependencies */
...
int root_bucket_to_highest_pri[TH_BUCKET_SCHED_MAX] = {
    MAXPRI_USER,     BASEPRI_FOREGROUND, BASEPRI_USER_INITIATED,
    BASEPRI_DEFAULT, BASEPRI_UTILITY,    MAXPRI_THROTTLE};
...
static char test_cpu_type_to_char(test_cpu_type_t cpu_type) {
  switch (cpu_type) {
...
void clutch_impl_init_topology(test_hw_topology_t hw_topology) {
  ...
  for (int i = 0; i < hw_topology.num_psets; i++) {
    if (pset_node_by_cpu_type[hw_topology.psets[i].cpu_type] ==
        PSET_NODE_NULL) {
      pset_node_by_cpu_type[hw_topology.psets[i].cpu_type] =
          (pset_node_t)malloc(sizeof(struct pset_node));
  ...
  for (int i = 0; i < hw_topology.num_psets; i++) {
    ...
    printf(" (%d: %d %c CPUs)", i, hw_topology.psets[i].num_cpus,
           test_cpu_type_to_char(hw_topology.psets[i].cpu_type));
    for (int c = total_cpus; c < total_cpus + hw_topology.psets[i].num_cpus;
         c++) {
      if (c == 0) {
        cpus[0] = &cpu0;
      } else {
      ...
      thread_t idle_thread = clutch_impl_create_thread(
          TH_BUCKET_SHARE_BG, not_real_idle_tg, IDLEPRI);
  ...
  printf(" }\n");
  /* After mock idle thread creation, reset thread/TG start IDs, as the idle
   * threads shouldn't count! */
  unique_tg_id = 0;
  unique_thread_id = 0;
}
...
void clutch_impl_init_params(void) {
  /* Read out Clutch-internal fields for use by the test harness */
  clutch_interactivity_score_max =
      2 * sched_clutch_bucket_group_interactive_pri;
  for (int b = TH_BUCKET_FIXPRI; b < TH_BUCKET_SCHED_MAX; b++) {
    clutch_root_bucket_wcel_us[b] =
        sched_clutch_root_bucket_wcel_us[b] == SCHED_CLUTCH_INVALID_TIME_32
            ? 0
            : sched_clutch_root_bucket_wcel_us[b];
    clutch_root_bucket_warp_us[b] =
        sched_clutch_root_bucket_warp_us[b] == SCHED_CLUTCH_INVALID_TIME_32
            ? 0
            : sched_clutch_root_bucket_warp_us[b];
  }
  CLUTCH_THREAD_SELECT = MACH_SCHED_CLUTCH_THREAD_SELECT;
}

void clutch_impl_init_tracepoints(void) {
...
struct thread_group *clutch_impl_create_tg(int interactivity_score) {
  ...
  if (interactivity_score != INITIAL_INTERACTIVITY_SCORE) {
    for (int bucket = TH_BUCKET_SHARE_FG; bucket < TH_BUCKET_SCHED_MAX;
         bucket++) {
      tg->tg_sched_clutch.sc_clutch_groups[bucket]
          .scbg_interactivity_data.scct_count = interactivity_score;
      tg->tg_sched_clutch.sc_clutch_groups[bucket]
          .scbg_interactivity_data.scct_timestamp = mach_absolute_time();
    }
  }
...
test_thread_t clutch_impl_create_thread(int root_bucket,
                                        struct thread_group *tg, int pri) {
  assert((sched_bucket_t)root_bucket == sched_convert_pri_to_bucket(pri) ||
         (sched_bucket_t)root_bucket == TH_BUCKET_FIXPRI);
  assert(tg != NULL);
...
void impl_set_thread_sched_mode(test_thread_t thread, int mode) {
  ((thread_t)thread)->sched_mode = (sched_mode_t)mode;
}

bool impl_get_thread_is_realtime(test_thread_t thread) {
  return ((thread_t)thread)->sched_pri >= BASEPRI_RTQUEUES;
}

void clutch_impl_set_thread_processor_bound(test_thread_t thread, int cpu_id) {
  ((thread_t)thread)->bound_processor = cpus[cpu_id];
}

void clutch_impl_cpu_set_thread_current(int cpu_id, test_thread_t thread) {
...
test_thread_t clutch_impl_cpu_clear_thread_current(int cpu_id) {
...
static bool is_logged_clutch_trace_code(uint64_t clutch_trace_code) {
...
static bool is_logged_trace_code(uint64_t trace_code) {
  if (KDBG_EXTRACT_CLASS(trace_code) == DBG_MACH &&
      KDBG_EXTRACT_SUBCLASS(trace_code) == DBG_MACH_SCHED_CLUTCH) {
...
void clutch_impl_log_tracepoint(uint64_t trace_code, uint64_t a1, uint64_t a2,
                                uint64_t a3, uint64_t a4) {
  if (is_logged_trace_code(trace_code)) {
    if (curr_tracepoint_ind < MAX_LOGGED_TRACEPOINTS) {
      logged_tracepoints[curr_tracepoint_ind * NUM_TRACEPOINT_FIELDS + 0] =
          KDBG_EXTRACT_CODE(trace_code);
      ...
      printf("Ran out of pre-allocated memory to log tracepoints (%d "
             "points)...will no longer log tracepoints\n",
...
void clutch_impl_pop_tracepoint(uint64_t *clutch_trace_code, uint64_t *arg1,
                                uint64_t *arg2, uint64_t *arg3,
                                uint64_t *arg4) {
  assert(expect_tracepoint_ind < curr_tracepoint_ind);
  *clutch_trace_code =
      logged_tracepoints[expect_tracepoint_ind * NUM_TRACEPOINT_FIELDS + 0];
...
static test_thread_t impl_dequeue_realtime_thread(processor_set_t pset) {
  thread_t thread = rt_runq_dequeue(&pset->rt_runq);
  pset_update_rt_stealable_state(pset);
  return thread;
}

void impl_set_thread_realtime(test_thread_t thread, uint32_t period,
                              uint32_t computation, uint32_t constraint,
                              bool preemptible, uint8_t priority_offset,
                              uint64_t deadline) {
...
void impl_sched_rt_spill_policy_set(unsigned policy) {
  sched_rt_spill_policy = policy;
}

void impl_sched_rt_steal_policy_set(unsigned policy) {
  sched_rt_steal_policy = policy;
}

void impl_sched_rt_init_completed() { sched_rt_init_completed(); }
...
sched_ipi_type_t sched_ipi_action(processor_t dst, thread_t thread,
  ...
  return SCHED(ipi_policy)(dst, thread,
                           (dst->active_thread == dst->idle_thread), event);
}
