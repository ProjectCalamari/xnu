...
void init_migration_harness(test_hw_topology_t hw_topology) {
...
void set_current_processor(int cpu_id) {
  fprintf(_log, "\tset current_processor() to cpu id %d\n", cpu_id);
  impl_set_current_processor(cpu_id);
}

void set_pset_load_avg(int cluster_id, int QoS, uint64_t load_avg) {
...
bool ipi_expect(int cpu_id, test_ipi_type_t ipi_type) {
  ...
  fprintf(
      _log, "%s: expected ipi to cpu %d type %u, found ipi to cpu %d type %u\n",
      pass ? "PASS" : "FAIL", cpu_id, ipi_type, found_cpu_id, found_ipi_type);
  return pass;
}

bool cpu_check_should_yield(int cpu_id, bool yield_expected) {
  bool yielding = impl_thread_should_yield(cpu_id);
  fprintf(_log, "%s: would yield on cpu %d? %d, expecting to yield? %d\n",
          yielding == yield_expected ? "PASS" : "FAIL", cpu_id, yielding,
          yield_expected);
  return yielding == yield_expected;
}

void cpu_send_ipi_for_thread(int cpu_id, test_thread_t thread,
                             test_ipi_event_t event) {
  fprintf(_log, "requesting IPI to cpu %d thread %p event %u\n", cpu_id,
          (void *)thread, event);
  impl_send_ipi(cpu_id, thread, event);
}

bool max_parallelism_expect(int qos, uint64_t options,
                            uint32_t expected_parallelism) {
  uint32_t found_parallelism = impl_qos_max_parallelism(qos, options);
  fprintf(
      _log,
      "expected parallelism %u for QoS %d options %llx, found parallelism %u\n",
      expected_parallelism, qos, options, found_parallelism);
  return found_parallelism == expected_parallelism;
}

int iterate_pset_search_order_expect(int src_pset_id, uint64_t candidate_map,
                                     int sched_bucket, int *expected_pset_ids,
                                     int num_psets) {
  int *search_order =
      impl_iterate_pset_search_order(src_pset_id, candidate_map, sched_bucket);
  fprintf(
      _log,
      "for src pset %d candidate map %llx bucket %d, we found search order:\t",
  ...
  for (int i = 0; (i < num_psets) && (search_order[i] != -1); i++) {
    ...
    if ((expected_pset_ids[i] != search_order[i]) &&
        (first_failure_ind == -1)) {
      first_failure_ind = i;
    }
  }
  fprintf(_log, "\n\t%s: expected search order:\t",
          (first_failure_ind == -1) ? "PASS" : "FAIL");
