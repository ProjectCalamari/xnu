...
static void workload_config_load(void) {
  ...
  size_t len = 0;
  int ret = sysctlbyname("kern.workload_config", NULL, &len,
                         (void *)(const void *)workload_config_plist,
                         strlen(workload_config_plist));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "sysctlbyname(kern.workload_config)");
}
...
static void setup_workgroup(void) {
  int ret;
  ...
  T_ASSERT_POSIX_SUCCESS(
      ret, "os_workgroup_set_flags(OS_WORKGROUP_ATTR_NONPROPAGATING)");
  g_rt_workgroup =
      os_workgroup_create_with_workload_id("rttimer", "com.apple.test", &attr);
  T_QUIET;
  T_ASSERT_NOTNULL(g_rt_workgroup, "created the test workgroup");
}

static thread_basic_info_data_t thread_info_get() {
  thread_basic_info_data_t value;
  mach_msg_type_number_t info_count = THREAD_BASIC_INFO_COUNT;
  thread_info(pthread_mach_thread_np(pthread_self()), THREAD_BASIC_INFO,
              (thread_info_t)&value, &info_count);
  return value;
}

static void make_realtime() {
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(ret, "thread_policy_set self to realtime");
}
...
static void *cpu_waster(void *arg) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "asprintf");
  ret = pthread_setname_np(name);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "pthread_setname_np(\"%s\")", name);

  while (1) {
    uint64_t time_quanta_in_ns = wasteCPUTimeQuanta;
    if (wasteCPUTimeQuantaRandomVariationPercent) {
      uint64_t maximum_possible_variation_in_ns =
          wasteCPUTimeQuanta * wasteCPUTimeQuantaRandomVariationPercent /
          100ULL;
      uint64_t actual_variation_in_ns =
          arc4random_uniform((uint32_t)maximum_possible_variation_in_ns);
      time_quanta_in_ns += actual_variation_in_ns;
    }

    uint64_t time_acitve_in_ns =
        time_quanta_in_ns * wasteCPUTimePercentActive / 100ULL;
...
static void *perform_test(__unused void *arg) {
  make_realtime();

  T_LOG("Requested                 Test        Average    Worst");
  T_LOG("WAIT(ns)   CPU(us)  cpu%%  Elapsed(ns) Miss(ns)   Miss(ns)");

  for (uint64_t delay_in_ns = waitStart; delay_in_ns <= waitEnd;
       delay_in_ns += waitStep) {
    uint64_t delay_in_abs = nanos_to_abs(delay_in_ns);

    uint64_t test_start_time = mach_absolute_time();
    uint64_t test_desired_end_time =
        test_start_time + nanos_to_abs(testDuration);
    ...
    } while ((test_actual_end_time = mach_absolute_time()) <
             test_desired_end_time);

    thread_basic_info_data_t end_info = thread_info_get();

    uint64_t user_delta_micros =
        ((end_info.user_time.seconds * USEC_PER_SEC) +
         end_info.user_time.microseconds) -
        ((start_info.user_time.seconds * USEC_PER_SEC) +
         start_info.user_time.microseconds);

    uint64_t system_delta_micros =
        ((end_info.system_time.seconds * USEC_PER_SEC) +
         end_info.system_time.microseconds) -
        ((start_info.system_time.seconds * USEC_PER_SEC) +
         start_info.system_time.microseconds);

    uint64_t total_delta_micros = user_delta_micros + system_delta_micros;
    uint64_t test_actual_elapsed =
        abs_to_nanos(test_actual_end_time - test_start_time);
    avg_elapsed_reading = total_elapsed_time / elapsed_reading_count;

    T_LOG("%09llu, %7llu, %4.1f, %10llu, %09llu, %09llu", delay_in_ns,
          total_delta_micros, (double)end_info.cpu_usage / 10.0,
          test_actual_elapsed, avg_elapsed_reading, worst_miss);

    T_QUIET;
    T_EXPECT_LE(avg_elapsed_reading, 500 * NSEC_PER_USEC,
                "average miss is <=0.5ms.");
    if (avg_elapsed_reading > 500 * NSEC_PER_USEC) {
      sched_kdebug_test_fail(delay_in_ns, total_delta_micros,
                             avg_elapsed_reading, worst_miss);
    }
  }
...
static void *calibration(__unused void *arg) {
  make_realtime();
  ...
  T_LOG("mach_absolute_time minimum resolution:      %llu ns",
        abs_to_nanos(1ULL));
  T_LOG("averaged minimum measurement time:          %llu ns",
        delta_measurement / CALIBRATION_CYCLES);
...
T_DECL(rttimer, "Check that realtime thread timer's average miss is <= 0.5ms",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(proc_disable_wakemon(getpid()),
                         "proc_disable_wakemon(getpid())");
  ...
  /* Create the workgroup. The main thread does not need to join and become
   * realtime. */
  setup_workgroup();

  /* Calibration */
  ret = pthread_create_with_workgroup_np(&thread, g_rt_workgroup, NULL,
                                         calibration, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "pthread_create(calibration)");
  ret = pthread_join(thread, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "pthread_join(calibration)");
  ...
  ret = pthread_create_with_workgroup_np(&thread, g_rt_workgroup, NULL,
                                         perform_test, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "pthread_create(perform_test) no-load");
  ret = pthread_join(thread, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "pthread_join(perform_test) no-load");

  /* Heavy-load tests */
  int thread_count = 2 * dt_ncpu();
  T_LOG("");
  T_LOG("Performing heavy-load tests. Spawning %d default priority cpu waster "
        "threads.",
        thread_count);
  for (int i = 0; i < thread_count; i++) {
    ret = pthread_create(&thread, NULL, cpu_waster, (void *)(uintptr_t)i);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "pthread_create(cpu_waster#%d)", i);
  }
  ret = pthread_create_with_workgroup_np(&thread, g_rt_workgroup, NULL,
                                         perform_test, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "pthread_create(perform_test) heavy-load");
  ret = pthread_join(thread, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "pthread_join(perform_test) heavy-load");
