 ...
 */
static void *thread_sync(void *ctx) {
...
static void setup_stress_event(struct endpoint *ep) {
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_POSIX_SUCCESS(pipe(&ep->fd[0]), "pipe()");

  T_LOG("th[0x%lx]: fd:{%d,%d}, ep@%p", (uintptr_t)pthread_self(), ep->fd[0],
        ep->fd[1], (void *)ep);
}
 ...
 */
static void recycle_fds(struct endpoint *ep) {
 ...
 */
static void write_data(struct endpoint *ep) {
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_POSIX_SUCCESS(write(ep->fd[1], "X", 1),
                         "th[0x%lx] write_data(fd=%d)",
                         (uintptr_t)pthread_self(), ep->fd[1]);
}

static void *do_stress_events(struct endpoint *ep) {
  unsigned write_freq =
 ...
 */
static void *do_select(void *arg) {
 ...
 * threads.
 */
static int create_thread_backoff(pthread_t *pthread, const pthread_attr_t *attr,
  ...
  for (tries = 0; tries < 3; tries++) {
    ...
    T_LOG("warning: pthread_create failed with %d (%s), backing off for %d "
          "seconds...",
          ret, strerror(ret), BACKOFF_SLEEP_SECONDS);
...
static void test_select_stress(int nthreads, uint64_t duration_seconds) {
  uint64_t deadline;
  ...
  T_LOG("Select stress test: %d threads, for %lld seconds", nthreads,
        duration_seconds);
 ...
 */
static void test_select_sleep(uint32_t niterations, unsigned long usecs) {
  int ret;
  struct timeval tv;
  tv.tv_sec = 0;
  tv.tv_usec = usecs;
...
T_DECL(select_sleep,
       "select sleep test for rdar://problem/20804876 Gala: select with no FDs "
       "leaks waitq table objects (causes asserts/panics)",
...
T_DECL(select_stress,
       "select stress test for rdar://problem/20804876 Gala: select with no "
       "FDs leaks waitq table objects (causes asserts/panics)",
       T_META_TAG_VM_PREFERRED) {
  char *env_sval = NULL;
  ...
  T_ASSERT_MACH_SUCCESS(
      semaphore_create(mach_task_self(), &g_sync_sem, SYNC_POLICY_FIFO, 0),
      "semaphore_create(g_sync_sem)");
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(
      semaphore_create(mach_task_self(), &g_thread_sem, SYNC_POLICY_FIFO, 0),
