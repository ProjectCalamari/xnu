...
/* -*- compile-command: "xcrun --sdk macosx.internal make -C tests
 * recvmsg_x_test" -*- */
...
static void send_packets(int sendSocket, u_int packetCount, struct sockaddr *to,
                         int proto) {
  u_int nmsgs = NMSGS;
  ...
  while (1) {
    ...
    T_EXPECT_POSIX_SUCCESS(sentMsgsCount =
                               sendmsg_x(sendSocket, msgList, nmsgs, 0),
                           "sendmsg_x()");
    if (sentMsgsCount < 0) {
      break;
    } else {
...
static bool receive_packets(int recvSocket) {
  uint8_t maxPacketsToRead = NMSGS;
  int i;
  struct msghdr_x msglist[NMSGS];
  char bytes[NMSGS][100];
  struct iovec vec[NMSGS];
  const socklen_t cmsg_size =
      (socklen_t)MAX(CMSG_SPACE(sizeof(struct in_pktinfo)),
  ...
  while (1) {
    ...
    if (npkts < 0) {
      ...
      T_EXPECT_POSIX_SUCCESS(npkts, "recvmsg_x() npkts %ld total_received %ld",
                             npkts, total_received);
      break;
    }
    ...
    for (i = 0; i < npkts; i++) {
      ...
      if ((msg->msg_controllen < (socklen_t)sizeof(struct cmsghdr)) ||
        ...
        success = false;
        T_LOG("msg[%d] bad  control message len=%d (< %u?) msg_flags 0x%x "
              "socket %d",
              i, msg->msg_controllen, cmsg_size, msg->msg_flags, recvSocket);
      } else {
        T_LOG("msg[%d] good control message len=%d (< %u?) msg_flags 0x%x "
              "socket %d",
              i, msg->msg_controllen, cmsg_size, msg->msg_flags, recvSocket);
      }
      for (struct cmsghdr *cm = (struct cmsghdr *)CMSG_FIRSTHDR(msg);
           cm != NULL; cm = (struct cmsghdr *)CMSG_NXTHDR(msg, cm)) {
        T_LOG(" cmsg_level %u cmsg_type %u cmsg_len %u", cm->cmsg_level,
              cm->cmsg_type, cm->cmsg_len);

        if (cm->cmsg_level == IPPROTO_IP && cm->cmsg_type == IP_RECVTOS &&
          ...
          struct in6_pktinfo *pktinfo =
              (struct in6_pktinfo *)(void *)CMSG_DATA(cm);
          char addr[40];
          ...
          struct in_pktinfo *pktinfo =
              (struct in_pktinfo *)(void *)CMSG_DATA(cm);
          char spec_dst[20];
          char addr[20];

          inet_ntop(AF_INET, &pktinfo->ipi_spec_dst, spec_dst,
                    sizeof(spec_dst));
    ...
    if (total_received >= maxPacketsToRead) {
      // Since we received max number of packets in the last loop, it is not
      // clear if there are any more left in the socket buffer. So we need to
      // try again
      break;
    }
  }
...
T_DECL(recvmsg_x_ipv4_udp, "revcmsg_x() ipv4", T_META_TAG_VM_PREFERRED) {
  ...
  int recvSocket;
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(recvSocket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP),
                         "socket()");
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(
      bind(recvSocket, (const struct sockaddr *)&addr, sizeof(addr)), "bind()");

  socklen_t addrLen = sizeof(addr);
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(
      getsockname(recvSocket, (struct sockaddr *)&addr, &addrLen),
      "getsockname()");

  int one = 1;
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(setsockopt(recvSocket, IPPROTO_IP, IP_RECVPKTINFO,
                                    (void *)&one, sizeof(one)),
                         "setsockopt(IP_RECVPKTINFO)");

  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(
      setsockopt(recvSocket, IPPROTO_IP, IP_RECVTOS, (void *)&one, sizeof(one)),
      "setsockopt(IP_RECVTOS)");

  int flags = fcntl(recvSocket, F_GETFL, 0);
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(fcntl(recvSocket, F_SETFL, flags | O_NONBLOCK),
                         "fcntl()");

  int sendSocket;
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(sendSocket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP),
                         "sendSocket socket()");
...
T_DECL(recvmsg_x_ipv6_udp, "exercise revcmsg_x() ", T_META_TAG_VM_PREFERRED) {
  struct sockaddr_in6 addr = {.sin6_len = sizeof(addr),
  ...
  int recvSocket;
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(recvSocket = socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP),
                         "socket()");
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(
      bind(recvSocket, (const struct sockaddr *)&addr, sizeof(addr)), "bind()");
  ...
  int one = 1;
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(setsockopt(recvSocket, IPPROTO_IPV6, IPV6_RECVPKTINFO,
                                    (void *)&one, sizeof(one)),
                         "setsockopt(IPV6_RECVPKTINFO)");

  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(setsockopt(recvSocket, IPPROTO_IPV6, IPV6_RECVTCLASS,
                                    (void *)&one, sizeof(one)),
  ...
  int sendSocket;
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(sendSocket = socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP),
                         "sendSocket socket()");
