...
T_DECL(thread_group_set,
       "Checks that new threads get a THREAD_GROUP_SET tracepoint with a "
       "non-zero tid",
  ...
  ktrace_events_single(
      session, MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_SET),
      ^(ktrace_event_t e) {
  ...
  T_ASSERT_POSIX_SUCCESS(ktrace_start(session, dispatch_get_main_queue()),
                         "ktrace_start");

  T_EXPECT_POSIX_SUCCESS(pthread_create(&thread, NULL, newthread, NULL),
                         "pthread_create");
