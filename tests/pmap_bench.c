...
T_DECL(pmap_call_benchmark, "pmap call overhead benchmark",
  ...
  dt_stat_time_t s = dt_stat_time_create(
      "average pmap function call overhead for %d calls", num_loops);
  while (!dt_stat_stable(s)) {
    dt_stat_token start = dt_stat_time_begin(s);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(sysctlbyname("kern.pmap_call_overhead_test", NULL,
                                        NULL, &num_loops, sizeof(num_loops)),
                           "invoke pmap call overhead test sysctl");
...
T_DECL(pmap_page_protect_benchmark, "pmap_page_protect() overhead benchmark",
  ...
  for (ppo_in.num_aliases = 1; ppo_in.num_aliases <= 128;
       ppo_in.num_aliases <<= 1) {
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(sysctlbyname("kern.pmap_page_protect_overhead_test",
                                        &duration, &duration_size, &ppo_in,
                                        sizeof(ppo_in)),
                           "invoke pmap_page_protect() overhead test sysctl");
    T_LOG("%u-loop duration (in ticks) for %u aliases: %llu", ppo_in.num_loops,
          ppo_in.num_aliases, duration);
  }
}
