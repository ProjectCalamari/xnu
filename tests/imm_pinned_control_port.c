 ...
 * rdar://70585367 (Disallow immovable port stashing with *_set_special_port()
 * and mach_port_register())
 */
...
kern_return_t catch_mach_exception_raise_state(
    ...
    const mach_exception_data_t code, mach_msg_type_number_t code_count,
    int *flavor, const thread_state_t old_state,
    mach_msg_type_number_t old_state_count, thread_state_t new_state,
    mach_msg_type_number_t *new_state_count) {
#pragma unused(exception_port, exception, code, code_count, flavor, old_state, \
...
kern_return_t catch_mach_exception_raise_identity_protected(
    mach_port_t exception_port, uint64_t thread_id, mach_port_t task_id_token,
    exception_type_t exception, mach_exception_data_t codes,
    mach_msg_type_number_t codeCnt) {
#pragma unused(exception_port, codeCnt)
  task_t task;
  pid_t pid;
  kern_return_t kr =
      task_identity_token_get_task_port(task_id_token, TASK_FLAVOR_READ, &task);
  ...
  T_LOG("Caught exception type: %d code: 0x%llx", exception,
        (uint64_t)codes[0]);
...
static mach_port_t alloc_exception_port(void) {
  kern_return_t kret;
...
static void test_immovable_port_stashing(void) {
  kern_return_t kr;
  mach_port_t port;

  kr = task_set_special_port(mach_task_self(), TASK_BOOTSTRAP_PORT,
                             mach_task_self());
  T_EXPECT_EQ(kr, KERN_INVALID_RIGHT,
              "should disallow task_set_special_port() with immovable port");

  kr = thread_set_special_port(mach_thread_self(), THREAD_KERNEL_PORT,
                               mach_thread_self());
  T_EXPECT_EQ(kr, KERN_INVALID_RIGHT,
              "should disallow task_set_special_port() with immovable port");
  ...
  T_ASSERT_MACH_SUCCESS(
      ...
      "mach_port_allocate");
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(mach_port_insert_right(mach_task_self(), port, port,
  ...
  T_EXPECT_MACH_SUCCESS(
      kr, "mach_ports_register() should succeed with movable port");
}

static void test_task_thread_port_values(void) {
  ...
  T_EXPECT_NE(port, mach_task_self(),
              "TASK_KERNEL_PORT should not match mach_task_self()");
  mach_port_deallocate(mach_task_self(), port);

  kr = task_for_pid(mach_task_self(), getpid(), &port);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "task_for_pid()");
  T_EXPECT_EQ(port, mach_task_self(),
              "task_for_pid(self) should match mach_task_self()");
  mach_port_deallocate(mach_task_self(), port);
  ...
  T_QUIET;
  T_ASSERT_GE(threadCount, 1, "should have at least 1 thread");
  ...
  if (threadCount > 0) {
    mach_vm_deallocate(mach_task_self(), (mach_vm_address_t)threadList,
  ...
  vm_deallocate(mach_task_self(), (vm_address_t)psets,
                (vm_size_t)pcnt * sizeof(mach_port_t));

  kr = processor_set_tasks_with_flavor(pset_priv, TASK_FLAVOR_CONTROL,
                                       &taskList, &tcnt);
  T_QUIET;
  ...
  if (tcnt > 0) {
    mach_vm_deallocate(mach_task_self(), (mach_vm_address_t)taskList,
                       tcnt * sizeof(mach_port_t));
  }
}

static void test_imm_pinned_control_port(const char *test_prog_name) {
  ...
  int ret = sysctlbyname("kern.task_exc_guard_default", &task_exc_guard,
                         &te_size, NULL, 0);
  ...
  for (int i = 0; i < MAX_TEST_NUM; i++) {
    ...
    T_QUIET;
    T_ASSERT_NE(exc_port, MACH_PORT_NULL, "Create a new exception port");

    /* Create exception serving thread */
    ret =
        pthread_create(&s_exc_thread, NULL, exception_server_thread, &exc_port);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "pthread_create exception_server_thread");

    /* Initialize posix_spawn attributes */
    posix_spawnattr_init(&attrs);

    int err = posix_spawnattr_setexceptionports_np(
        &attrs, EXC_MASK_GUARD | EXC_MASK_CORPSE_NOTIFY, exc_port,
        (exception_behavior_t)(EXCEPTION_IDENTITY_PROTECTED |
                               MACH_EXCEPTION_CODES),
        0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(err, "posix_spawnattr_setflags");
    ...
    err = posix_spawn(&client_pid, child_args[0], NULL, &attrs, &child_args[0],
                      environ);
    T_ASSERT_POSIX_SUCCESS(
        err, "posix_spawn control_port_options_client = %d test_num = %d",
        client_pid, i);
    ...
    if (WIFEXITED(child_status)) {
      if (WEXITSTATUS(child_status)) {
        ...
        /* Skipping test because CS_OPS_CLEARPLATFORM is not supported in the
         * current BATS container */
    ...
    T_LOG("Exception code: Received code = 0x%llx Expected code = 0x%llx",
          exc_id, test_exception_code[i]);
    T_EXPECT_EQ(exc_id, test_exception_code[i],
                "Exception code: Received == Expected");
  }
}

T_DECL(imm_pinned_control_port_hardened,
       "Test pinned & immovable task and thread control ports for platform "
       "restrictions binary",
...
T_DECL(imm_pinned_control_port,
       "Test pinned & immovable task and thread control ports for first party "
       "binary",
       T_META_IGNORECRASHES(".*pinned_rights_child.*"),
