...
static bool check_current_cs_flags(code_signing_config_t expected_cs_config) {
  code_signing_config_t cur_cs_config = 0;
  size_t cs_config_size = sizeof(cur_cs_config);
  sysctlbyname("security.codesigning.config", &cur_cs_config, &cs_config_size,
               NULL, 0);
  return cur_cs_config & expected_cs_config;
}

static bool unrestricted_debugging() {
  /* AMFI often disables security features if debugging is unrestricted */
  bool unrestricted_debugging =
      check_current_cs_flags(CS_CONFIG_UNRESTRICTED_DEBUGGING);
...
static bool ipc_hardening_disabled() {
   ...
   */
  bool enforcement_disabled =
      check_current_cs_flags(CS_CONFIG_GET_OUT_OF_MY_WAY);
...
static bool is_release_kernel() {
  int kernel_type = 0;
  size_t kernel_type_size = sizeof(kernel_type);
  int r;
  r = sysctlbyname("kern.development", &kernel_type, &kernel_type_size, NULL,
                   0);
  if (r < 0) {
    T_WITH_ERRNO;
...
kern_return_t catch_mach_exception_raise_state_identity_protected(
    ...
    __unused int *flavor, __unused thread_state_t old_state,
...
#pragma unused(exception_port, thread_id, task_id_token, exception, codes,     \
               codeCnt)
...
#pragma unused(exception_port, thread_id, task_id_token)
  ...
  T_LOG("Caught exception type: %d code[0]: 0x%llx code[1]:0x%llx", exception,
        codes[0], codes[1]);
...
static void
reply_port_defense(const bool thirdparty_hardened, int test_index,
  ...
  ret = sysctlbyname("kern.task_exc_guard_default", &task_exc_guard, &te_size,
                     NULL, 0);
  ...
  /*
   * It is allowed for us to set an exception port because we are entitled test
   * process. If you are using this code as an example for platform binaries,
   * use `EXCEPTION_IDENTITY_PROTECTED` instead of `EXCEPTION_DEFAULT`
   */
  int err = posix_spawnattr_setexceptionports_np(
      &attrs, EXC_MASK_GUARD, exc_port,
  ...
  err = posix_spawn(&client_pid, child_args[0], NULL, &attrs, &child_args[0],
                    environ);
  T_ASSERT_POSIX_SUCCESS(err, "posix_spawn reply_port_defense_client = %d",
                         client_pid);
  ...
  T_LOG("Exception code: Received code = 0x%llx Expected code = 0x%llx",
        received_exception_code, expected_exception_code);
  T_EXPECT_EQ(received_exception_code, expected_exception_code,
              "Exception code: Received == Expected");
}

T_DECL(reply_port_defense, "Test reply port semantics violations",
...
T_DECL(test_unentitled_thread_set_exception_ports,
  ...
  reply_port_defense(true, test_num, expected_exception_code,
                     triggers_exception);
  // reply_port_defense(false, test_num, expected_exception_code,
  // triggers_exception);
}
...
T_DECL(kobject_reply_port_defense,
       ...
       T_META_IGNORECRASHES(".*reply_port_defense_client.*"),
       T_META_TAG_VM_PREFERRED, T_META_CHECK_LEAKS(false),
       T_META_ENABLED(
           !TARGET_OS_OSX &&
           !TARGET_OS_BRIDGE)) { /* disable on macOS due to BATS boot-args */
  if (ipc_hardening_disabled()) {
    T_SKIP("hardening disabled due to boot-args");
  }
  int test_num = 9;
...
T_DECL(test_alloc_weak_reply_port,
  ...
  /* rdar://136996362 (iOS+ telemetry for restricting 1P usage of provisional
   * reply port) */
  reply_port_defense(true, test_num, expected_exception_code,
                     triggers_exception);
  reply_port_defense(false, test_num, expected_exception_code,
...
T_DECL(test_move_service_port, "service ports are immovable",
       T_META_IGNORECRASHES(".*reply_port_defense_client.*"),
       T_META_CHECK_LEAKS(false)) {
  int test_num = 11;
  mach_exception_data_type_t expected_exception_code =
...
T_DECL(test_send_immovability,
       "ensure that send immovability is set on ports, even if they are not "
       "copied out",
       T_META_IGNORECRASHES(".*reply_port_defense_client.*"),
       T_META_CHECK_LEAKS(false)) {
...
T_DECL(
    test_service_port_as_exception_port,
