...
T_DECL(pipe_drain, "test a pipe with multiple read descriptor could close one "
                   "descriptor and drain that descriptor") {
  int pipe_fd[2];
  int dup_fd;
  int ret;
  char buf[10] = "Hello";
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(sigaction(SIGPIPE, &sa, NULL), NULL);
