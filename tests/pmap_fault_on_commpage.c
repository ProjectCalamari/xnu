...
T_GLOBAL_META(T_META_NAMESPACE("xnu.arm"), T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_ENABLED(TARGET_CPU_ARM64), T_META_OWNER("xi_han"),
...
#else
/* On arm64_32, _COMM_PAGE_START_ADDRESS is out of normal VA range, so a SIGSEGV
 * is expected if there's a fault. */
...
static void test_handler(int signum) {
  ...
  if (test_state == TEST_STATE_TESTING_READ) {
    ...
    T_FAIL("unexpected %s in test state %u", SIGNAL_EXPECTED_STR,
           (unsigned int)test_state);
  }
}

T_DECL(pmap_commpage_access_test,
       "Verify system behavior on user access to the commpage",
