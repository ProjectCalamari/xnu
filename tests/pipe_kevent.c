...
T_DECL(pipe_noblock_kevent,
       "Set a pipe and no block and setup EVFLT_WRITE kevent on it and make "
       "sure it does not fire when the pipe is full") {
  int fd[2], write_fd;
  dispatch_queue_t dq1 = dispatch_queue_create(
      "com.apple.test.pipe_noblock_kevent.queue", DISPATCH_QUEUE_SERIAL);
  ...
  dispatch_source_t write_source = dispatch_source_create(
      DISPATCH_SOURCE_TYPE_WRITE, (uintptr_t)write_fd, 0, dq1);
  ...
  T_LOG("Arming a timer for 15 seconds to exit, assuming kevent will block "
        "before that");
