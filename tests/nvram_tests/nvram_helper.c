...
// NVRAM helper functions from
// https://stashweb.sd.apple.com/projects/COREOS/repos/system_cmds/browse/nvram.tproj/nvram.c

/**
 * @brief Print the given firmware variable.
 */
static void PrintVariable(const void *key, const void *value) {
  ...
  if (nameBuffer &&
      CFStringGetCString(key, nameBuffer, nameLen, kCFStringEncodingUTF8)) {
  ...
  if (typeID == CFBooleanGetTypeID()) {
    ...
    if (valueBuffer && CFStringGetCString(value, valueBuffer, valueLen,
                                          kCFStringEncodingUTF8)) {
...
static CFTypeRef ConvertValueToCFTypeRef(CFTypeID typeID, const char *value) {
  ...
  if (typeID == CFBooleanGetTypeID()) {
    ...
    valueRef =
        CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &number);
  } else if (typeID == CFStringGetTypeID()) {
    ...
    for (cnt = cnt2 = 0; cnt < length; cnt++, cnt2++) {
      if (value[cnt] == '%') {
        if ((cnt + 2 > length) || !ishexnumber(value[cnt + 1]) ||
    ...
    valueRef =
        CFDataCreate(kCFAllocatorDefault, (const UInt8 *)valueCopy, cnt2);
  } else {
    return 0;
  }
 ...
 */
static kern_return_t GetVariable(const char *name,
...
/**
 * @brief Returns variable type. 0xFF if variable doesn't exist or on error
 * creating CFString
 */
 ...
 */
static kern_return_t SetVariable(const char *name, const char *value,
                                 io_registry_entry_t optionsRef) {
  CFStringRef nameRef;
  CFTypeRef valueRef;
  CFTypeID typeID;
  ...
  if (valueRef) {
    ...
    // if it's one of the delete/sync keys, it has to be an OSString since the
    // "value" will be the variable name.
    if (strncmp(name, kIONVRAMDeletePropertyKey,
                strlen(kIONVRAMDeletePropertyKey)) == 0 ||
        strncmp(name, kIONVRAMDeletePropertyKeyWRet,
                strlen(kIONVRAMDeletePropertyKeyWRet)) == 0 ||
        strncmp(name, kIONVRAMSyncNowPropertyKey,
                strlen(kIONVRAMSyncNowPropertyKey)) == 0) {
      ...
      CFTypeID types[] = {CFDataGetTypeID(), CFStringGetTypeID(),
                          CFNumberGetTypeID(), CFBooleanGetTypeID()};
      for (unsigned long i = 0; i < sizeof(types) / sizeof(types[0]); i++) {
        valueRef = ConvertValueToCFTypeRef(types[i], value);
        if (valueRef != 0) {
          result = IORegistryEntrySetCFProperty(optionsRef, nameRef, valueRef);
          if (result == KERN_SUCCESS || result == kIOReturnNoMemory ||
 ...
 */
static kern_return_t DeleteVariable(const char *name,
                                    io_registry_entry_t optionsRef) {
  // Since delete always returns ok, read to make sure it is deleted.
  if (SetVariable(kIONVRAMDeletePropertyKey, name, optionsRef) ==
      KERN_SUCCESS) {
 ...
 */
static kern_return_t DeleteVariableWRet(const char *name,
                                        io_registry_entry_t optionsRef) {
 ...
 */
static kern_return_t SyncNVRAM(const char *name,
                               io_registry_entry_t optionsRef) {
...
static const char *GetOpString(nvram_op op) {
  switch (op) {
  case OP_GET:
    return "read";
  case OP_SET:
...
static const char *GetRetString(kern_return_t ret) {
  switch (ret) {
  case KERN_SUCCESS:
    return "success";
  case KERN_FAILURE:
...
void TestVarOp(nvram_op op, const char *var, const char *val,
  ...
  case OP_SYN:
    ...
    break;
  case OP_OBL:
    // Obliterate NVram (system guid deletes all variables in system region,
    // common guid deletes all non-system variables)
  ...
  if (exp_ret == kIOReturnInvalid) {
    T_PASS("Operation %s for variable %s returned %s(%#x) but doesn't have an "
           "expected return\n",
           GetOpString(op), var, GetRetString(ret), ret);
    return;
  }

  // Allow passing in a value other than KERN_SUCCESS || KERN_FAILURE to assert
  // against otherwise remain as pass/fail
  ...
  T_ASSERT_EQ(ret, exp_ret,
              "Operation %s for variable %s returned %s(%#x) as expected\n",
              GetOpString(op), var, GetRetString(ret), ret);
}
