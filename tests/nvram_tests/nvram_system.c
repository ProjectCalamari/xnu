...
// xcrun -sdk macosx.internal make -C tests nvram_system && sudo
// ./tests/build/sym/nvram_system

// Test that writing, reading, and deleting of system variables with system
// entitlement should succeed
...
// Test that system guid is translated to common guid on devices without system
// namespace support
T_DECL(TestSysNonAS, "Test system guids for devices without system namespace") {
  const char *varWithSys = SystemNVRAMGuidString ":"
                                                 "varToTestSys";
...
T_DECL(TestEntRdSys, "Test read entitled system variables") {
  char *varToTest = SystemNVRAMGuidString ":"
                                          "testEntRdSys";
...
// Test that writing of entitled system variables without entitlement should
// fail
...
// Test that deleting of entitled system variables without entitlement should
// fail
...
T_DECL(TestNVRAMResetSys, "Test NVRAM Reset for system region variables") {
  ...
  const char *varToTestWSys = SystemNVRAMGuidString ":"
                                                    "testVar2";
...
T_DECL(TestNVRAMOblitSys, "Test NVRAM Obliterate for system region") {
  ...
  if (getopt(argc, argv, "r") == 'r') {
    // Set variables with common namespace, random namespace and system
    // namespace
    TestVarOp(OP_SET, varToTest, DefaultSetVal, KERN_SUCCESS, optionsRef);
    TestVarOp(OP_SET, varToTestWSys, DefaultSetVal, KERN_SUCCESS, optionsRef);
    ...
    // Now, obliterate common region and make sure all non-sys variables are
    // deleted
    TestVarOp(OP_SET, varToTestWSys, DefaultSetVal, KERN_SUCCESS, optionsRef);
