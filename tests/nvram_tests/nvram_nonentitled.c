...
T_GLOBAL_META(T_META_NAMESPACE("xnu.nvram"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("nvram"));

static io_registry_entry_t optionsRef = IO_OBJECT_NULL;

// xcrun -sdk iphoneos.internal make -C tests nvram_nonentitled && sudo
// ./tests/build/sym/nvram_nonentitled
...
T_DECL(TestNVRAMSync, "Test NVRAM Sync") {
  ...
  // However, we do not return an error for that not being able to sync due to
  // the rate limit
  ...
  TestVarOp(OP_SYN, kIONVRAMForceSyncNowPropertyKey, NULL, KERN_SUCCESS,
            optionsRef);
...
T_DECL(TestIOKitMatch, "Test IOKit matching for property") {
  ...
  CFDictionaryAddValue(matchingDict, CFSTR(kIOPropertyExistsMatchKey),
                       propToTest);
  ...
  T_ASSERT_NE(service, IO_OBJECT_NULL, "Got service match for property %s",
              varToTest);
...
T_DECL(TestSystem, "Test system guids") {
  const char *varToTest = SystemNVRAMGuidString ":"
                                                "varToTest2";

  optionsRef = CreateOptionsRef();

  TestVarOp(OP_SET, varToTest, DefaultSetVal, kIOReturnNotPrivileged,
            optionsRef);
...
// Test that variables with KernelOnly bit set cannot be modified from user
// space
...
// Test that variables with NeverAllowedToDelete bit set cannot be deleted even
// with ResetNVram() To reset nvram, call the test with -r argument: sudo
// ./tests/build/sym/nvram_nonentitled -n xnu.nvram.TestImmutable -- -r
...
// Test that variables with ResetNVRAMOnlyDelete bit set can be deleted only by
// ResetNVram To reset nvram, call the test with -r argument: sudo
// ./tests/build/sym/nvram_nonentitled -n xnu.nvram.TestResetOnlyDel -- -r
...
T_DECL(TestTypes, "Test nvram types") {
  ...
  T_ASSERT_EQ(GetVarType(boolVar, optionsRef), CFBooleanGetTypeID(),
              "Verified %s type as boolean.\n", boolVar);
  T_ASSERT_EQ(GetVarType(numVar, optionsRef), CFNumberGetTypeID(),
              "Verified %s type as number.\n", numVar);
  T_ASSERT_EQ(GetVarType(strVar, optionsRef), CFStringGetTypeID(),
              "Verified %s type as string.\n", strVar);
  T_ASSERT_EQ(GetVarType(dataVar, optionsRef), CFDataGetTypeID(),
              "Verified %s type as data.\n", dataVar);
...
T_DECL(TestNVRAMReset, "Test NVRAM Reset") {
  ...
  const char *varToTestWRand = RandomNVRAMGuidString ":"
                                                     "testVar2";
...
T_DECL(TestNVRAMOblit, "Test NVRAM Obliterate") {
  ...
  const char *varToTestWRand = RandomNVRAMGuidString ":"
                                                     "testVar2";
  const char *oblitNonSys = CommonNVRAMGuidString ":"
                                                  "ObliterateNVRam";
