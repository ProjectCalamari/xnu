...
static void udp_port_scan(void) {
  int v4_udp_fd;
  ...
  for (in_port_t port = 1; port > 0 && port <= IPPORT_HILASTAUTO; port++) {
     ...
     */
    ssize_t sent =
        sendto(v4_udp_fd, buffer, len, 0, (struct sockaddr *)&sin, sin.sin_len);
    int saved_errno = errno;
    T_QUIET;
    T_ASSERT_TRUE(sent >= 0 || errno == ECONNREFUSED,
                  "sendto() to port %u: errno = %d (%s)", port, saved_errno,
...
static int open_raw_ipv4_socket(void) {
  int fd;
...
static int open_raw_ipv6_socket(void) {
  int fd;
...
static void close_raw_socket(int fd) {
  int optval;
  socklen_t optlen = sizeof(optval);

  T_ASSERT_POSIX_SUCCESS(
      getsockopt(fd, SOL_SOCKET, SO_NUMRCVPKT, &optval, &optlen),
