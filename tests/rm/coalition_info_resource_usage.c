...
T_GLOBAL_META(T_META_NAMESPACE("xnu.rm"), T_META_RADAR_COMPONENT_NAME("xnu"),
...
static pid_t spawn_helper_in_coalition(char *helper_name, uint64_t coal_id) {
  int ret;
  ...
  ret = posix_spawnattr_setcoalition_np(&attr, coal_id, COALITION_TYPE_RESOURCE,
...
static uint64_t get_qos_sum(uint64_t coalition_id) {
...
static void terminate_and_reap_coalition(void) {
  T_LOG("coalition_terminate");
  coalition_terminate(coalition_id, 0);
  T_LOG("coalition_reap");
  coalition_reap(coalition_id, 0);
}

T_DECL(coalition_info_resource_usage_qos_monotonic,
       "Make sure CPU time QoS values are accumulated from dead tasks",
       T_META_ASROOT(true), T_META_SYSCTL_INT("kern.unrestrict_coalitions=1"),
