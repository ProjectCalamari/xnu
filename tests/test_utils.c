...
bool is_development_kernel(void) {
  ...
  dispatch_once(&is_development_once, ^{
    ...
    T_ASSERT_POSIX_SUCCESS(
        sysctlbyname("kern.development", &dev, &dev_size, NULL, 0), NULL);
...
bool process_is_translated() {
  ...
  dispatch_once(&is_translated_once, ^{
    ...
    if (sysctlbyname("sysctl.proc_translated", &out_value, &inout_size, NULL,
                     0) != 0) {
      /*
       ...
       */
      T_QUIET;
      T_ASSERT_POSIX_ERROR(errno, ENOENT,
                           "sysctlbyname(sysctl.proc_translated)");
      is_translated = false;
    } else {
      T_QUIET;
      T_ASSERT_GE(inout_size, sizeof(out_value),
                  "sysctlbyname(sysctl.proc_translated)");
...
pid_t launch_background_helper(const char *variant, bool start_suspended,
  ...
  launch_tool_args = (char *[]){testpath, "-n", variant_cpy, NULL};
...
void set_process_memorystatus_managed(pid_t pid) {
  kern_return_t ret = memorystatus_control(
      MEMORYSTATUS_CMD_SET_PROCESS_IS_MANAGED, pid, 1, NULL, 0);
  T_QUIET;
