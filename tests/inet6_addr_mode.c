...
static void set_sockaddr_in6(struct sockaddr_in6 *sin6_p,
...
static int inet6_difaddr(const char *name, const struct in6_addr *addr) {
...
static int inet6_aifaddr(const char *name, const struct in6_addr *addr,
                         const struct in6_addr *dstaddr, int prefix_length,
                         int flags, u_int32_t valid_lifetime,
...
static void create_fake_interface(void) {
  int error;
...
T_DECL(inet6_addr_mode_auto_to_manual,
       "inet6 address mode-switching (auto -> manual)") {
  ...
  T_ASSERT_POSIX_SUCCESS(inet6_aifaddr(ifname1, &lladdr, NULL, 64, 123,
  ...
  T_ASSERT_POSIX_SUCCESS(inet6_aifaddr(ifname1, &newaddr, NULL, 64,
  ...
  T_ASSERT_POSIX_SUCCESS(inet6_aifaddr(ifname1, &newaddr, NULL, 64, 0,
                                       ND6_INFINITE_LIFETIME,
                                       ND6_INFINITE_LIFETIME),
                         NULL);
...
T_DECL(inet6_addr_mode_manual_to_auto,
       "inet6 address mode-switching (manual -> auto)") {
  struct in6_addr lladdr;
  struct in6_addr newaddr;
  unsigned int if_index;
