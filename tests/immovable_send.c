...
T_DECL(catch_immovable_send_exception,
       "Send guard port descriptor to another process",
       T_META_IGNORECRASHES(".*immovable_send_client.*")) {
  ...
  sysctlbyname("kern.task_exc_guard_default", &task_exc_guard, &te_size, NULL,
               0);
  ...
  kr = task_get_exception_ports(task, EXC_MASK_GUARD, &mask, &maskCount,
