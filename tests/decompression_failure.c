...
static void dirty_page(const vm_address_t address) {
...
static bool try_to_corrupt_page(vm_address_t page_va) {
  int val;
  size_t size = sizeof(val);
  int result = sysctlbyname("vm.compressor_inject_error", &val, &size, &page_va,
                            sizeof(page_va));
  return result == 0;
}

static void create_corrupted_regions(void) {
  uint64_t hw_memsize;

  size_t size = sizeof(unsigned int);
  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname("vm.pagesize", &vm_pagesize, &size, NULL, 0),
      "read vm.pagesize");
...
#endif
  const uint64_t effective_memsize =
      (hw_memsize > max_memsize) ? max_memsize : hw_memsize;
...
T_DECL(decompression_failure,
       ...
       // Disable software checks in development builds, as these would result
       // in panics.
       T_META_BOOTARGS_SET("vm_compressor_validation=0"), T_META_ASROOT(true),
  ...
  if (sysctlbyname("vm.compressor_inject_error", &value, &size, NULL, 0) != 0) {
