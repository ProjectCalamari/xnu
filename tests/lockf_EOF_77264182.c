...
T_GLOBAL_META(T_META_NAMESPACE("xnu.ipc"), T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_OWNER("jonathan_w_adams"), T_META_RUN_CONCURRENTLY(TRUE));
...
T_DECL(lockf_EOF_77264182,
  ...
  T_ASSERT_POSIX_SUCCESS((fd = open("lockf_EOF_test", O_CREAT | O_RDWR, 0666)),
                         "open(lockf_EOF_test)");
  ...
  for (loops = 0; loops < 100; loops++) {
    struct flock fl = {.l_start = OFF_MAX - loops,
                       .l_len = 0,
                       .l_pid = getpid(),
                       .l_type = F_WRLCK,
                       .l_whence = SEEK_SET};
