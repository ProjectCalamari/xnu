...
static void *fd_select_close_helper(void *ctx) {
  int fd = *(int *)ctx;
...
static void *fd_stress_dup2_close_fun(void *ctx) {
  ...
  for (int i = 1; i <= count; i++) {
    ...
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(rc, "dup2(%d, 42)", STDIN_FILENO);
    if (thno == 3) {
      rc = close(42);
      if (rc == -1) {
        T_QUIET;
        T_EXPECT_POSIX_FAILURE(rc, EBADF, "close(42)");
      }
    }
    if (i % 1000 == 0) {
...
T_DECL(fd_stress_dup2_close, "Stress test races between dup2 and close",
  ...
  for (int i = 0; i < 4; i++) {
    rc =
        pthread_create(&th[i], NULL, fd_stress_dup2_close_fun, (void *)(long)i);
...
static void *confine_thread(void *data) {
...
T_DECL(confined_fileport_race, "test for rdar://69922255",
  ...
  for (int i = 0; i < 100 * 1000; i++) {
    ...
    if (fileport_makeport(fd, &p) == 0) {
      T_QUIET;
      T_ASSERT_EQ(fcntl(fd, F_GETCONFINED), 0,
