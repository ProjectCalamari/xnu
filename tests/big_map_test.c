...
static void get_release_type(char *release_type, size_t release_type_len) {
  int ret;
  T_ASSERT_POSIX_SUCCESS(ret = sysctlbyname("kern.osreleasetype", release_type,
                                            &release_type_len, NULL, 0),
                         "sysctlbyname kern.osreleasetype");
}

static int64_t get_hw_memsize(int64_t memsize) {
  int ret;
  size_t size = sizeof(memsize);
  T_ASSERT_POSIX_SUCCESS(
      ret = sysctlbyname("hw.memsize", &memsize, &size, NULL, 0),
...
T_DECL(big_map_test,
       "Test that loads large blobs into memory up to 60 percent of gigs "
       "available.",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(fd = mkstemp(file_path), NULL);
  for (int gigs = 1; gigs <= max_memory_gib; gigs++) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ftruncate(fd, bytes), "ftruncate");

    void *p = mmap(NULL, bytes, PROT_READ, MAP_FILE | MAP_SHARED, fd, 0);

    T_QUIET;
    T_ASSERT_NE(p, MAP_FAILED, "map");

    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(munmap(p, bytes), "munmap");
  }
}
