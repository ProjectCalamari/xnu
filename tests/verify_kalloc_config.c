...
static void run_test(void) {
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_memory_info");
  T_QUIET;
  T_ASSERT_EQ(nameCnt, infoCnt, "zone name and info counts don't match");
  ...
  for (i = 0; i < nameCnt; i++) {
    ...
    if (strncmp(z_name, kalloc_str, strlen(kalloc_str)) == 0) {
      ...
      T_QUIET;
      T_ASSERT_NOTNULL(size_ptr, "couldn't find size in name");
      size = strtoul(size_ptr, NULL, 10);
      T_LOG("ZONE NAME: %-25s ELEMENT SIZE: %llu", z_name, size);
      T_QUIET;
      T_ASSERT_EQ(size, info[i].mzi_elem_size,
  ...
  if ((name != NULL) && (nameCnt != 0)) {
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "vm_deallocate name");
  }
  ...
  if ((info != NULL) && (infoCnt != 0)) {
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "vm_deallocate info");
  }
  ...
  if ((wiredInfo != NULL) && (wiredInfoCnt != 0)) {
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "vm_deallocate wiredInfo");
  }
