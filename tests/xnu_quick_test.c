...
T_DECL(syscall, "xnu_quick_test for syscall", T_META_TAG_VM_NOT_PREFERRED) {
  ...
  *my_pathp = 0x00;
  ...
  T_ASSERT_MACH_SUCCESS(create_random_name(my_pathp, 1),
                        "Create random test file");
  /* use an indirect system call to open our test file.
   * I picked open since it uses a path pointer which grows to 64 bits in an
   * LP64 environment.
   */
  T_EXPECT_NE(my_fd = syscall(SYS_open, my_pathp, (O_RDWR | O_EXCL), 0), -1,
              "Attempt to open file using indirect syscall %s", my_pathp);
...
T_DECL(fork_wait4_exit,
  ...
  if (my_pid == 0) {
    ...
    T_WITH_ERRNO;
    T_ASSERT_TRUE(my_err == 0, "stat call with path: \"%s\" returned \"%d\"",
                  &g_target_path[0], errno);
    exit(44);
  }
  ...
  /* kind of just guessing on these values so if this fails we should take a
   * closer look at the returned rusage structure.
   */
  T_ASSERT_FALSE((my_usage.ru_utime.tv_sec > 1 ||
                  my_usage.ru_stime.tv_sec > 1 || my_usage.ru_majflt > 1000 ||
                  my_usage.ru_msgsnd > 100),
                 "wait4 returned rusage structure");
