 ...
 */
static int sysctl_get_all(const char *oid_name, void **buffer, size_t *len,
  ...
  *buffer = NULL;
  ...
  T_ASSERT_POSIX_SUCCESS(
      ret = sysctlbyname(oid_name, *buffer, len, newp, newlen), NULL);
  if (ret != 0) {
    if (errno == ENOMEM) {
      free(*buffer);
      *buffer = NULL;
 ...
 * amount of data.
 */
static int sysctl_get(const char *oid_name, void **buffer, size_t *len,
                      size_t total_size) {
  int ret;
...
T_DECL(skmem_arena_sysctl_get_all,
       "Get all entries in kern.skywalk.stats.arena") {
  void *buffer;
  size_t len;
...
T_DECL(skmem_region_sysctl_get_all,
       "Get all entries in kern.skywalk.stats.region") {
  void *buffer;
  size_t len;
...
T_DECL(skmem_cache_sysctl_get_all,
       "Get all entries in kern.skywalk.stats.cache") {
  void *buffer;
  size_t len;
...
T_DECL(skmem_arena_sysctl_get_single,
       "Get a single entry in kern.skywalk.stats.arena") {
  void *buffer;
  size_t len;
  size_t total_size;

  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname(SK_STATS_ARENA, NULL, &total_size, NULL, 0), NULL);
...
T_DECL(skmem_region_sysctl_get_single,
       "Get a single entry in kern.skywalk.stats.region") {
  void *buffer;
  size_t len;
  size_t total_size;

  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname(SK_STATS_REGION, NULL, &total_size, NULL, 0), NULL);
...
T_DECL(skmem_cache_sysctl_get_single,
       "Get a single entry in kern.skywalk.stats.cache") {
  void *buffer;
  size_t len;
  size_t total_size;

  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname(SK_STATS_CACHE, NULL, &total_size, NULL, 0), NULL);
...
T_DECL(skmem_arena_sysctl_get_over,
       "Ask for more entries than kern.skywalk.stats.arena") {
  void *buffer;
  size_t len;
  size_t total_size;
...
T_DECL(skmem_region_sysctl_get_over,
       "Ask for more entries than kern.skywalk.stats.region") {
  void *buffer;
  size_t len;
  size_t total_size;
...
T_DECL(skmem_cache_sysctl_get_over,
       "Ask for more entries than kern.skywalk.stats.cache") {
  void *buffer;
  size_t len;
  size_t total_size;
