...
static void do_test(int type) {
  ...
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(sock[0], SOL_SOCKET, SO_DEBUG, &optval, sizeof(optval)),
      "setsockopt(SO_DEBUG)");

  struct timeval tv = {.tv_sec = 1, .tv_usec = 0};
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(sock[0], SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(struct timeval)),
      "setsockopt(SO_RCVTIMEO)");

  struct iovec iov0 = {.iov_base = NULL, .iov_len = 0};

  char cmsg_buf[CMSG_SPACE(sizeof(int))];
  struct msghdr msghdr1 = {.msg_name = NULL,
                           .msg_namelen = 0,
                           .msg_iov = &iov0,
                           .msg_iovlen = 1,
  ...
  *((int *)CMSG_DATA(cmsg)) = sock[0];
  ...
  struct msghdr msghdr2 = {.msg_name = NULL,
                           .msg_namelen = 0,
                           .msg_iov = &iov1,
                           .msg_iovlen = 1,
                           .msg_control = NULL,
  ...
  struct msghdr msghdr3 = {.msg_name = NULL,
                           .msg_namelen = 0,
                           .msg_iov = &iov2,
                           .msg_iovlen = 1,
                           .msg_control = NULL,
...
T_DECL(send_zero_payload_dgram, "repro-124040738 SOCK_DGRAM",
       T_META_ASROOT(true)) {
  do_test(SOCK_DGRAM);
}

T_DECL(send_zero_payload_stream, "repro-124040738 SOCK_STREAM",
       T_META_ASROOT(true)) {
  do_test(SOCK_STREAM);
}
