...
T_DECL(point_to_point_mdn_utun, "Point-to-point mDNS on utun interface",
  ...
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(udp_fd = socket(AF_INET, SOCK_DGRAM, 0), NULL);
  ...
  T_ASSERT_EQ_INT(ifr.ifr_point_to_point_mdns, 0,
                  "point_to_point_mdns off by default");

  ifr.ifr_point_to_point_mdns = 1;
  T_ASSERT_POSIX_SUCCESS(ioctl(udp_fd, SIOCSPOINTOPOINTMDNS, &ifr), NULL);
  T_ASSERT_POSIX_SUCCESS(ioctl(udp_fd, SIOCGPOINTOPOINTMDNS, &ifr), NULL);
  T_ASSERT_EQ_INT(ifr.ifr_point_to_point_mdns, 1,
                  "point_to_point_mdns turned on");

  ifr.ifr_point_to_point_mdns = 0;
  T_ASSERT_POSIX_SUCCESS(ioctl(udp_fd, SIOCSPOINTOPOINTMDNS, &ifr), NULL);
  T_ASSERT_POSIX_SUCCESS(ioctl(udp_fd, SIOCGPOINTOPOINTMDNS, &ifr), NULL);
  T_ASSERT_EQ_INT(ifr.ifr_point_to_point_mdns, 0,
                  "point_to_point_mdns turned off");
