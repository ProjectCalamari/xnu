...
T_DECL(int_time, "interrupt time collection") {
  ...
  T_ASSERT_EQ_INT(retval, 0,
                  "proc_current_thread_schedinfo succeeded (2nd time)");
  T_ASSERT_NE(info_new.int_time_ns, (uint64_t)-3,
              "info.int_time_ns was filled (2nd time)");

  int64_t duration_ns =
      (int64_t)info_new.int_time_ns - (int64_t)info.int_time_ns;
  ...
  /*
   * If the kernel says we spent more than 10 seconds in an interrupt context,
   * something is definitely wrong.
   */

  int64_t const limit_ns = 10 * (int64_t)NSEC_PER_SEC;
  T_EXPECT_FALSE(
      duration_ns > limit_ns,
      "Kernel claims we spent less than 10 seconds in interrupt context");
}
