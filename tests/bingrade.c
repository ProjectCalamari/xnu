...
T_GLOBAL_META(T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("arm"), T_META_OWNER("jwilkey"));

T_DECL(bingrade_vm_force_arm64_32,
       "Test forced arm64_32 binary grading policy for VM",
       T_META_RUN_CONCURRENTLY(true),
       T_META_REQUIRES_SYSCTL_EQ("kern.hv_vmm_present", 1),
       T_META_BOOTARGS_SET("force-arm64-32=1"), T_META_ENABLED(TARGET_OS_WATCH),
  ...
  /* 32-bit process should succeed. */
  {
    ...
    T_QUIET;
    T_ASSERT_POSIX_ZERO(rc, "32-bit process should spawn.");
    pid = waitpid(pid, &status, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(pid, NULL);
    T_QUIET;
  ...
  /* Fat binary should select 32-bit process. */
  {
    ...
    const int rc = posix_spawn(&pid, argv[0], NULL, NULL, argv, NULL);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(rc, "Fat binary should spawn.");
    pid = waitpid(pid, &status, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(pid, NULL);
    T_QUIET;
...
T_DECL(gestalt_vm_force_arm64_32,
       "Test forced arm64_32 mode host_info CPU architecture",
       T_META_RUN_CONCURRENTLY(true),
       T_META_REQUIRES_SYSCTL_EQ("kern.hv_vmm_present", 1),
       T_META_BOOTARGS_SET("force-arm64-32=1"), T_META_ENABLED(TARGET_OS_WATCH),
       T_META_TAG_VM_PREFERRED) {
  ...
  kr = host_info(mach_host_self(), HOST_PREFERRED_USER_ARCH, (host_info_t)&hi,
                 &count);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "host_info");
  T_QUIET;
  T_ASSERT_EQ(count, HOST_PREFERRED_USER_ARCH_COUNT, NULL);
