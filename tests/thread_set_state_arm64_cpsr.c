...
T_GLOBAL_META(T_META_NAMESPACE("xnu.arm"), T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_OWNER("justin_unger"), T_META_RUN_CONCURRENTLY(true));

#define PSR64_USER_MASK (0xFU << 28)
#define PSR64_OPT_BITS                                                         \
  (0x01 << 12) // user-writeable bits that may or may not be set, depending on
               // hardware/device/OS/moon phase

#if __arm64__
__attribute__((noreturn)) static void phase2() {
  kern_return_t err;
  ...
  T_QUIET;
  T_ASSERT_EQ(nzcv & PSR64_USER_MASK, PSR64_USER_MASK,
              "All condition flags are set");

  err = thread_get_state(mach_thread_self(), ARM_THREAD_STATE64,
                         (thread_state_t)&ts, &count);
  T_QUIET;
  T_ASSERT_EQ(err, KERN_SUCCESS, "Got own thread state after corrupting CPSR");

  T_QUIET;
  T_ASSERT_EQ(ts.__cpsr & ~(PSR64_USER_MASK | PSR64_OPT_BITS), 0,
              "No privileged fields in CPSR are set");
...
T_DECL(thread_set_state_arm64_cpsr,
  ...
  err = thread_set_state(mach_thread_self(), ARM_THREAD_STATE64,
