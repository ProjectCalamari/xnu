...
T_DECL(kdebug_trace_syscall, "test that kdebug_trace(2) emits correct events",
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(s, "created session");
  ...
  T_ASSERT_POSIX_SUCCESS(
      kdebug_trace(TRACE_DEBUGID, 0xfeedfacefeedface, 2, 3, 4), NULL);
  ktrace_end(s, 0);
...
T_DECL(kdebug_trace_string_syscall,
       ...
       T_META_ENABLED(IS_64BIT), T_META_TAG_VM_PREFERRED) {
  start_controlling_ktrace();
  ...
  T_QUIET;
  T_ASSERT_EQ(traced_len, sizeof(traced_string) - 1,
  ...
  ktrace_events_single(s, TRACE_STRING_GLOBAL, ^(struct trace_point *tp) {
    if (tp->debugid & DBG_FUNC_START && tp->arg1 == TRACE_DEBUGID) {
      tracing_string = true;
      string_cpu = tp->cpuid;
      memcpy(observed_string + string_offset, &tp->arg3, sizeof(uint64_t) * 2);
      string_offset += sizeof(uint64_t) * 2;
    } else if (tracing_string && string_cpu == tp->cpuid) {
      memcpy(observed_string + string_offset, &tp->arg1, sizeof(uint64_t) * 4);
  ...
  T_WITH_ERRNO;
  T_ASSERT_NE(str_id, (uint64_t)0, "kdebug_trace_string(2)");
  ktrace_end(s, 0);
...
T_DECL(kdebug_signpost_syscall,
  ...
  ktrace_events_single(
      s, APPSDBG_CODE(DBG_APP_SIGNPOST, SIGNPOST_SINGLE_CODE),
  ...
  ktrace_events_single_paired(
      s, APPSDBG_CODE(DBG_APP_SIGNPOST, SIGNPOST_PAIRED_CODE),
      ^(struct trace_point *start, struct trace_point *end) {
        paired_seen++;
        T_PASS("paired signposts are traced");

        T_EXPECT_EQ(start->arg1, 5ULL,
                    "argument 1 of start signpost is correct");
        T_EXPECT_EQ(start->arg2, 6ULL,
                    "argument 2 of start signpost is correct");
        T_EXPECT_EQ(start->arg3, 7ULL,
                    "argument 3 of start signpost is correct");
        T_EXPECT_EQ(start->arg4, 8ULL,
                    "argument 4 of start signpost is correct");
  ...
  ktrace_set_completion_handler(s, ^(void) {
    T_QUIET;
    T_EXPECT_NE(single_seen, 0, "did not see single tracepoint before timeout");
    T_QUIET;
    T_EXPECT_NE(paired_seen, 0, "did not see single tracepoint before timeout");
    ktrace_session_destroy(s);
    T_END;
  });
...
T_DECL(syscall_tracing, "ensure that syscall arguments are traced propertly",
       T_META_TAG_VM_PREFERRED) {
  ktrace_session_t s = ktrace_session_create();
  T_QUIET;
  T_WITH_ERRNO;
  ...
  ktrace_events_single(
      s, BSDDBG_CODE(DBG_BSD_EXCP_SC, telemetry_syscall_no),
  ...
  ktrace_set_completion_handler(s, ^{
    T_ASSERT_TRUE(seen, "should have seen a syscall event for kevent_id(2)");
    ktrace_session_destroy(s);
    T_END;
  });
...
T_DECL(
  ...
  do {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(sysctl(mib, 3, &buf_info, &needed, NULL, 0), NULL);
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ktrace_set_use_existing(s), NULL);
  ...
  ktrace_events_all(s, ^(struct trace_point *tp) {
    if (events == 0) {
      T_EXPECT_EQ(
          tp->debugid, (unsigned int)TRACE_LOST_EVENTS,
...
static void _assert_tracing_state(bool enable, const char *msg) {
  kbufinfo_t bufinfo = {0};
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sysctl((int[]){CTL_KERN, KERN_KDEBUG, KERN_KDGETBUF},
                                3, &bufinfo, &(size_t){sizeof(bufinfo)}, NULL,
                                0),
                         "get kdebug buffer info");
  T_QUIET;
  T_ASSERT_NE(bufinfo.nkdbufs, 0, "tracing should be configured");
  T_ASSERT_NE(bufinfo.nolog, enable, "%s: tracing should%s be enabled", msg,
              enable ? "" : "n't");
}

#define DRAIN_TIMEOUT_NS (1 * NSEC_PER_SEC)

static void _drain_until_event(uint32_t debugid) {
...
T_DECL(disabling_event_match,
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(error, "configured session");
  ...
  T_ASSERT_POSIX_SUCCESS(sysctl(
                             ...
                             3, &matchers, &matchers_size, NULL, 0),
                         "set event disable matcher");
  size_t size = 0;
  T_ASSERT_POSIX_SUCCESS(sysctl(
                             (int[]){
                                 CTL_KERN,
                             ...
                             4, NULL, &size, NULL, 0),
                         "enabled event disable matching");
  T_ASSERT_POSIX_SUCCESS(sysctl(
                             (int[]){
                                 CTL_KERN,
                                 KERN_KDEBUG,
...
T_DECL(reject_old_events,
  ...
  __block int events = 0;
  ktrace_events_single(
      s, KDBG_EVENTID(DBG_BSD, DBG_BSD_KDEBUG_TEST, 1),
      ^(struct trace_point *tp) {
        events++;
...
T_DECL(ascending_time_order,
  ...
  ktrace_events_all(s, ^(struct trace_point *tp) {
    if (tp->timestamp < prev_ts) {
      in_order = false;
      T_LOG("%" PRIu64 ": %#" PRIx32 " (cpu %d)", prev_ts, prev_debugid,
            prev_cpu);
      T_LOG("%" PRIu64 ": %#" PRIx32 " (cpu %d)", tp->timestamp, tp->debugid,
            tp->cpuid);
      ktrace_end(s, 1);
    }
  });
  ...
  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, ORDERING_TIMEOUT_SEC * NSEC_PER_SEC),
      dispatch_get_main_queue(), ^{
...
__attribute__((aligned(8))) static const char map_uuid[16] = "map UUID";

__attribute__((aligned(8))) static const char unmap_uuid[16] = "unmap UUID";

__attribute__((aligned(8))) static const char sc_uuid[16] = "shared UUID";
...
expect_dyld_image_info(struct trace_point *tp, const uint64_t *exp_uuid,
                       uint64_t exp_load_addr, fsid_t *exp_fsid,
                       fsobj_id_t *exp_fsobjid, int order) {
  ...
  if (order == 0) {
    ...
    T_QUIET;
    T_EXPECT_EQ(uuid[0], exp_uuid[0], NULL);
    T_QUIET;
    T_EXPECT_EQ(uuid[1], exp_uuid[1], NULL);
    T_QUIET;
    T_EXPECT_EQ(load_addr, exp_load_addr, NULL);
    T_QUIET;
    T_EXPECT_EQ(fsid.val[0], exp_fsid->val[0], NULL);
    T_QUIET;
    T_EXPECT_EQ(fsid.val[1], exp_fsid->val[1], NULL);
  } else if (order == 1) {
    fsobj_id_t fsobjid;
    ...
    T_QUIET;
    T_EXPECT_EQ(fsobjid.fid_objno, exp_fsobjid->fid_objno, NULL);
    T_QUIET;
    T_EXPECT_EQ(fsobjid.fid_generation, exp_fsobjid->fid_generation, NULL);
  } else {
  ...
  if (order == 0) {
    ...
    T_QUIET;
    T_EXPECT_EQ(uuid[0], (uint32_t)exp_uuid[0], NULL);
    T_QUIET;
    T_EXPECT_EQ(uuid[1], (uint32_t)(exp_uuid[0] >> 32), NULL);
    T_QUIET;
    T_EXPECT_EQ(uuid[2], (uint32_t)exp_uuid[1], NULL);
    T_QUIET;
    T_EXPECT_EQ(uuid[3], (uint32_t)(exp_uuid[1] >> 32), NULL);
  } else if (order == 1) {
    uint32_t load_addr;
    fsid_t fsid;
    fsobj_id_t fsobjid;
    ...
    T_QUIET;
    T_EXPECT_EQ(load_addr, (uint32_t)exp_load_addr, NULL);
    T_QUIET;
    T_EXPECT_EQ(fsid.val[0], exp_fsid->val[0], NULL);
    T_QUIET;
    T_EXPECT_EQ(fsid.val[1], exp_fsid->val[1], NULL);
...
static void expect_dyld_events(ktrace_session_t s, const char *name,
                               uint32_t base_code, const char *exp_uuid,
                               uint64_t exp_load_addr, fsid_t *exp_fsid,
...
T_DECL(dyld_events, "test that dyld registering libraries emits events",
  ...
  expect_dyld_events(s, "mapping", DBG_DYLD_UUID_MAP_A, map_uuid, MAP_LOAD_ADDR,
                     &map_fsid, &map_fsobjid, saw_mapping);
  expect_dyld_events(s, "unmapping", DBG_DYLD_UUID_UNMAP_A, unmap_uuid,
                     UNMAP_LOAD_ADDR, &unmap_fsid, &unmap_fsobjid,
                     saw_unmapping);
  expect_dyld_events(s, "shared cache", DBG_DYLD_UUID_SHARED_CACHE_A, sc_uuid,
                     SC_LOAD_ADDR, &sc_fsid, &sc_fsobjid, saw_shared_cache);
  ...
  T_EXPECT_MACH_SUCCESS(
      task_register_dyld_image_infos(mach_task_self(), &info, 1),
      "registered dyld image info");
  ...
  T_EXPECT_MACH_SUCCESS(
      task_unregister_dyld_image_infos(mach_task_self(), &info, 1),
      "unregistered dyld image info");
...
static const uint32_t dev_evts[EXP_KERNEL_EVENTS] = {
    BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 0), BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 1),
    BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 2), BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 3),
    BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 4),
};

static const uint32_t rel_evts[EXP_KERNEL_EVENTS] = {
    BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 5), BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 6),
    BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 7), BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 8),
    BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 9),
};

static const uint32_t filt_evts[EXP_KERNEL_EVENTS] = {
    BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 10), BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 11),
    BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 12), BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 13),
    BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 14),
};

static const uint32_t noprocfilt_evts[EXP_KERNEL_EVENTS] = {
    BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 15), BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 16),
    BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 17), BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 18),
    BSDDBG_CODE(DBG_BSD_KDEBUG_TEST, 19),
};

static void expect_event(struct trace_point *tp, const char *name,
                         unsigned int *events, const uint32_t *event_ids,
  ...
  *events += 1;
  for (i = 0; i < event_idx; i++) {
    T_QUIET;
    T_EXPECT_EQ(((uint64_t *)&tp->arg1)[i], (uint64_t)i + 1, NULL);
  }
  for (; i < 4; i++) {
    T_QUIET;
    T_EXPECT_EQ(((uint64_t *)&tp->arg1)[i], (uint64_t)0, NULL);
  }
}

static void expect_release_event(struct trace_point *tp, unsigned int *events) {
  expect_event(tp, "release", events, rel_evts,
               sizeof(rel_evts) / sizeof(rel_evts[0]));
}

static void expect_development_event(struct trace_point *tp,
                                     unsigned int *events) {
  expect_event(tp, "dev", events, dev_evts,
               sizeof(dev_evts) / sizeof(dev_evts[0]));
}

static void expect_filtered_event(struct trace_point *tp,
                                  unsigned int *events) {
  expect_event(tp, "filtered", events, filt_evts,
               sizeof(filt_evts) / sizeof(filt_evts[0]));
}

static void expect_noprocfilt_event(struct trace_point *tp,
                                    unsigned int *events) {
  expect_event(tp, "noprocfilt", events, noprocfilt_evts,
               sizeof(noprocfilt_evts) / sizeof(noprocfilt_evts[0]));
}

static void expect_kdbg_test_events(ktrace_session_t s, bool use_all_callback,
                                    void (^cb)(unsigned int dev_seen,
...
T_DECL(kernel_events, "ensure kernel macros work", T_META_TAG_VM_PREFERRED) {
  ...
  expect_kdbg_test_events(
      s, false,
      ^(unsigned int dev_seen, unsigned int rel_seen, unsigned int filt_seen,
        unsigned int noprocfilt_seen) {
        /*
...
T_DECL(kernel_events_noprocfilt,
  ...
  for (size_t i = 0; i < sizeof(noprocfilt_evts) / sizeof(noprocfilt_evts[0]);
       i++) {
    T_QUIET;
    T_ASSERT_POSIX_ZERO(
        ktrace_ignore_process_filter_for_event(s, noprocfilt_evts[i]),
...
static void *kdebug_abuser_thread(void *ctx) {
...
T_DECL(stress, "emit events on all but one CPU with a small buffer",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kperror, "kperf_timer_period_set %llu ns", TIMER_NS);
  kperror = kperf_timer_action_set(0, 1);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kperror, "kperf_timer_action_set");
  kperror = kperf_action_samplers_set(
      1, KPERF_SAMPLER_TINFO | KPERF_SAMPLER_TH_SNAPSHOT |
             KPERF_SAMPLER_KSTACK | KPERF_SAMPLER_USTACK |
             KPERF_SAMPLER_MEMINFO | KPERF_SAMPLER_TINFO_SCHED |
             KPERF_SAMPLER_TH_DISPATCH | KPERF_SAMPLER_TK_SNAPSHOT |
             KPERF_SAMPLER_SYS_MEM | KPERF_SAMPLER_TH_INSTRS_CYCLES);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kperror, "kperf_action_samplers_set");
  /* You monster... */
  ...
  T_QUIET;
  T_ASSERT_GT(ncpus, 0, "realistic number of CPUs");

  pthread_t *threads = calloc((unsigned int)ncpus - 1, sizeof(pthread_t));
  T_WITH_ERRNO;
  T_QUIET;
  T_ASSERT_NOTNULL(threads, "calloc(%d threads)", ncpus - 1);
  ...
  ktrace_set_completion_handler(s, ^{
    ...
    T_WITH_ERRNO;
    T_QUIET;
    T_ASSERT_NOTNULL(sread, "ktrace_session_create");

    int error = ktrace_set_file(sread, filepath);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(error, "ktrace_set_file %s", filepath);

    ktrace_file_t f = ktrace_file_open(filepath, false);
    T_QUIET;
    T_WITH_ERRNO;
    T_ASSERT_NOTNULL(f, "ktrace_file_open %s", filepath);
    uint64_t first_timestamp = 0;
    error = ktrace_file_earliest_timestamp(f, &first_timestamp);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(error, "ktrace_file_earliest_timestamp");
    ...
    ktrace_events_all(sread, ^(struct trace_point *tp) {
      ...
      uint64_t delta_ns = 0;
      T_QUIET;
      T_EXPECT_GE(tp->timestamp, prev_timestamp,
                  "timestamps are monotonically increasing");
      int converror = ktrace_convert_timestamp_to_nanoseconds(
          sread, tp->timestamp - prev_timestamp, &delta_ns);
      T_QUIET;
      T_ASSERT_POSIX_ZERO(converror, "convert timestamp to ns");
      if (prev_timestamp && delta_ns > GAP_THRESHOLD_NS) {
        if (tp->debugname) {
          T_LOG("gap: %gs at %llu - %llu on %d: %s (%#08x)",
                (double)delta_ns / 1e9, prev_timestamp, tp->timestamp,
                tp->cpuid, tp->debugname, tp->debugid);
        } else {
          T_LOG("gap: %gs at %llu - %llu on %d: %#x", (double)delta_ns / 1e9,
                prev_timestamp, tp->timestamp, tp->cpuid, tp->debugid);
        }
...
#define DECR_TRAP (0x1090000)
        ...
        if (eventid != INTERRUPT && eventid != PERF_CPU_IDLE &&
          ...
          T_QUIET;
          T_EXPECT_EQ(tp->debugid, lost_events,
    ...
    ktrace_events_single_paired(
        sread, TRACE_WRITING_EVENTS,
        ^(struct trace_point *start, struct trace_point *end) {
          uint64_t delta_ns;
          int converror = ktrace_convert_timestamp_to_nanoseconds(
              sread, start->timestamp - last_write, &delta_ns);
          T_QUIET;
          T_ASSERT_POSIX_ZERO(converror, "convert timestamp to ns");

          uint64_t dur_ns;
          converror = ktrace_convert_timestamp_to_nanoseconds(
              sread, end->timestamp - start->timestamp, &dur_ns);
          T_QUIET;
          T_ASSERT_POSIX_ZERO(converror, "convert timestamp to ns");

          T_LOG("write: %llu (+%gs): %gus on %d: %llu events", start->timestamp,
                (double)delta_ns / 1e9, (double)dur_ns / 1e3, end->cpuid,
                end->arg1);
    ...
    ktrace_set_completion_handler(sread, ^{
      ...
      if (last_timestamp) {
        int converror = ktrace_convert_timestamp_to_nanoseconds(
            sread, last_timestamp - first_timestamp, &duration_ns);
        T_QUIET;
        T_ASSERT_POSIX_ZERO(converror, "convert timestamp to ns");
    ...
    T_QUIET;
    T_ASSERT_POSIX_ZERO(starterror, "ktrace_start read session");
...
/* Just kidding... for now. */
  ...
  for (int i = 0; i < (ncpus - 1); i++) {
    int error = pthread_create(&threads[i], NULL, kdebug_abuser_thread,
                               (void *)(uintptr_t)i);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(error, "pthread_create abuser thread %d", i);
  }

  int error = ktrace_start_writing_file(s, filepath, ktrace_compression_none,
                                        NULL, NULL);
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, ABUSE_SECS * NSEC_PER_SEC),
                 dispatch_get_main_queue(), ^{
                   ...
                   for (int i = 0; i < (ncpus - 1); i++) {
                     ...
                     T_QUIET;
                     T_EXPECT_POSIX_ZERO(joinerror, "pthread_join thread %d",
                                         i);
                   }
                 });
...
T_DECL(round_trips,
  ...
  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, ROUND_TRIPS_TIMEOUT_SECS * NSEC_PER_SEC),
      dispatch_get_main_queue(), ^{
...
T_DECL(event_coverage, "ensure events appear up to the end of tracing",
       T_META_TAG_VM_PREFERRED, T_META_ENABLED(false) /* rdar://134505849 */) {
  start_controlling_ktrace();
  ...
  ktrace_set_completion_handler(s, ^{
    ktrace_session_destroy(s);
    T_QUIET;
    T_EXPECT_GT(events, 0ULL, "should have seen some events");
    T_END;
  });
  ...
  dispatch_source_set_timer(
      timer,
      dispatch_time(DISPATCH_TIME_NOW, HEARTBEAT_INTERVAL_SECS * NSEC_PER_SEC),
  ...
  dispatch_source_set_event_handler(timer, ^{
    ...
    T_LOG("heartbeat %u at time %lld, seen %" PRIu64 " events, "
          "current event time %lld",
          heartbeats, mach_absolute_time(), events, current_timestamp);

    if (current_timestamp > 0) {
      T_EXPECT_GT(current_timestamp, last_timestamp,
                  "event timestamps should be increasing");
      T_QUIET;
      T_EXPECT_GT(events, last_events, "number of events should be increasing");
    }
...
static unsigned int get_nevents(void) {
  kbufinfo_t bufinfo = {0};
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sysctl((int[]){CTL_KERN, KERN_KDEBUG, KERN_KDGETBUF},
                                3, &bufinfo, &(size_t){sizeof(bufinfo)}, NULL,
...
static unsigned int set_nevents(unsigned int nevents) {
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      sysctl((int[]){CTL_KERN, KERN_KDEBUG, KERN_KDSETBUF, (int)nevents}, 4,
             NULL, 0, NULL, 0),
      "set kdebug buffer size");

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      sysctl((int[]){CTL_KERN, KERN_KDEBUG, KERN_KDSETUP, (int)nevents}, 4,
             NULL, 0, NULL, 0),
      "setup kdebug buffers");

  unsigned int nevents_allocated = get_nevents();

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sysctl((int[]){CTL_KERN, KERN_KDEBUG, KERN_KDREMOVE},
                                3, NULL, 0, NULL, 0),
...
static unsigned int _mb_of_events(unsigned int event_count) {
...
T_DECL(set_buffer_size, "ensure large buffer sizes can be set",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname("hw.memsize", &memsize, &(size_t){sizeof(memsize)}, NULL, 0),
      "sysctl hw.memsize");
  T_SETUPEND;
  ...
  // Use hexadecimal representation to prevent failure signaturization on these
  // values.
...
static void *donothing(__unused void *arg) { return NULL; }
...
T_DECL(long_names, "ensure long command names are reported",
  ...
  int ret =
      sysctlbyname("kern.procname", NULL, NULL, longname, sizeof(longname));
  T_ASSERT_POSIX_SUCCESS(ret, "use sysctl kern.procname to lengthen the name");
  ...
  ktrace_events_single(
      ktsess, TRACE_STRING_NEWTHREAD, ^(struct trace_point *tp) {
        if (ktrace_get_pid_for_thread(ktsess, tp->threadid) == getpid()) {
          saw_newthread = true;
  ...
  ktrace_set_completion_handler(ktsess, ^{
    ...
    T_EXPECT_TRUE(saw_newthread, "should have seen the new thread");
    T_END;
  });
...
T_DECL(lookup_long_paths, "lookup long path names", T_META_TAG_VM_PREFERRED) {
  start_controlling_ktrace();

  int ret = chdir("/tmp");
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "chdir to /tmp");
  ...
  do {
    ...
    if (ret >= 0 || errno != EEXIST) {
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(ret, "mkdir of %d nested directory", i);
    }
    ret = chdir(dir);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "chdir to %d nested directory", i);
  } while (i++ < 40);
  ...
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(s, "created session");
  ktrace_events_single(s, VFS_LOOKUP,
                       ^(struct trace_point *tp __unused){
                       });
  ktrace_set_vnode_paths_enabled(s, KTRACE_FEATURE_ENABLED);
...
static void expect_kernel_task_tracing(void) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "sysctl(ktrace.configured_by)");
...
T_DECL(early_boot_tracing, "ensure early boot strings are present",
       T_META_BOOTARGS_SET("trace=100000"), XNU_T_META_SOC_SPECIFIC,
       T_META_TAG_VM_NOT_ELIGIBLE,
...
T_DECL(typefilter_boot_arg, "ensure typefilter is set up correctly at boot",
  ...
  T_EXPECT_TRUE(typefilt[0xfeed / 8], "specified class is set in typefilter");
...
T_DECL(instrs_and_cycles_on_proc_exit,
  ...
  int status;
  dispatch_queue_t q =
      dispatch_queue_create("com.apple.kdebug-test", DISPATCH_QUEUE_SERIAL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(signal(SIGCHLD, sighandler),
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_POSIX_ZERO(error, "trace single event");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(status, "spawn process");
  if (status == 0) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_ZERO(error, "rusage");
    error = waitpid(pid, &status, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(error, "waitpid");
...
static void *get_thread_counters(void *ptr) {
...
T_DECL(instrs_and_cycles_on_thread_exit,
  ...
  int error;
  pthread_t *threads = calloc((unsigned int)(NO_OF_THREADS), sizeof(pthread_t));
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(threads, "calloc(%d threads)", NO_OF_THREADS);
  tc_info_t tc_infos = calloc((unsigned int)(NO_OF_THREADS),
                              sizeof(struct thread_counters_info));
  T_WITH_ERRNO;
  T_QUIET;
  T_ASSERT_NOTNULL(tc_infos, "calloc(%d thread counters)", NO_OF_THREADS);
  ...
  error = ktrace_events_single(
      s, DBG_MT_INSTRS_CYCLES_THR_EXIT, ^(ktrace_event_t tp) {
  ...
  for (int i = 0; i < NO_OF_THREADS; i++) {
    error = pthread_create(&threads[i], NULL, get_thread_counters,
                           (void *)&tc_infos[i]);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(error, "pthread_create thread %d", i);
  }
  T_SETUPEND;
  for (int i = 0; i < NO_OF_THREADS; i++) {
    error = pthread_join(threads[i], NULL);
    T_QUIET;
    T_EXPECT_POSIX_ZERO(error, "pthread_join thread %d", i);
  }
...
T_DECL(direct_file_writing, "ensure direct file writes work correctly",
  ...
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(trace_file, "can open file as a trace file");
