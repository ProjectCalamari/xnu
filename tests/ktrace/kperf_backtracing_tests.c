...
static void expect_frame(const char **bt, unsigned int bt_len,
                         CSSymbolRef symbol, uint64_t addr, unsigned int bt_idx,
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(name, NULL);
  T_EXPECT_EQ_STR(name, bt[frame_idx], "frame %2u: saw '%s', expected '%s'",
 ...
 * start sampling).
 */
static void expect_backtrace(ktrace_session_t s, uint64_t tid,
                             unsigned int *stacks_seen, bool kern,
                             const char **bt, unsigned int bt_len,
  ...
  if (kern) {
    ...
    dispatch_once(&kern_symb_once, ^(void) {
      ...
      T_QUIET;
      T_ASSERT_FALSE(CSIsNull(kern_symb), NULL);
    });
    symb = kern_symb;
  } else {
    ...
    dispatch_once(&user_symb_once, ^(void) {
      ...
      T_QUIET;
      T_ASSERT_FALSE(CSIsNull(user_symb), NULL);
      T_QUIET;
      T_ASSERT_TRUE(CSSymbolicatorIsTaskValid(user_symb), NULL);
    });
    symb = user_symb;
  }
  ...
  ktrace_events_single(s, hdr_debugid, ^(struct trace_point *tp) {
    ...
    stacks++;
    if (!(tp->arg1 & 1)) {
      T_FAIL("invalid %s stack on thread %#" PRIx64, kern ? "kernel" : "user",
             tp->threadid);
      return;
    }
    ...
    T_QUIET;
    T_EXPECT_GE(hdr_frames, bt_len, "at least %u frames in header", bt_len);
    T_QUIET;
    T_EXPECT_LE(hdr_frames, bt_len + allow_larger,
  ...
  ktrace_events_single(s, data_debugid, ^(struct trace_point *tp) {
    ...
    for (; i < 4 && frames < hdr_frames; i++, frames++) {
      ...
      CSSymbolRef symbol =
          CSSymbolicatorGetSymbolWithAddressAtTime(symb, addr, kCSNow);
...
static const char *kernel_bt[KERNEL_FRAMES] = {"unix_syscall64",
                                               "kdebug_trace64"};
 ...
 */
static void *backtrace_thread(void *arg) {
  ...
  T_LOG("backtrace thread calling into %d frames (already at %d frames)", calls,
        RECURSE_START_OFFSET);
  (void)recurse_a(notify_spinning, calls);
  return NULL;
}

static uint64_t create_backtrace_thread(void *(*thread_fn)(void *),
                                        dispatch_semaphore_t notify_spinning) {
  ...
  dispatch_once(&backtrace_once, ^{
    ...
    T_QUIET;
    T_ASSERT_NOTNULL(backtrace_started, NULL);

    if (!notify_spinning) {
      backtrace_go = dispatch_semaphore_create(0);
      T_QUIET;
      T_ASSERT_NOTNULL(backtrace_go, NULL);
    }
  });

  T_QUIET;
  T_ASSERT_POSIX_ZERO(
      pthread_create(&thread, NULL, thread_fn, (void *)notify_spinning), NULL);
  T_QUIET;
  T_ASSERT_NOTNULL(thread, "backtrace thread created");
  dispatch_semaphore_wait(backtrace_started, DISPATCH_TIME_FOREVER);

  T_QUIET;
  T_ASSERT_POSIX_ZERO(pthread_threadid_np(thread, &tid), NULL);
  T_QUIET;
  T_ASSERT_NE(tid, UINT64_C(0), "backtrace thread created does not have ID 0");
...
static void start_backtrace_thread(void) {
  T_QUIET;
...
T_DECL(kperf_stacks_kdebug_trig,
  ...
  T_ASSERT_POSIX_SUCCESS(
      kperf_kdebug_filter_add_debugid(filter, TRIGGERING_DEBUGID), NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kperf_kdebug_filter_set(filter), NULL);
  (void)kperf_action_count_set(1);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      kperf_action_samplers_set(1, KPERF_SAMPLER_USTACK | KPERF_SAMPLER_KSTACK),
      NULL);
  T_QUIET;
...
T_DECL(kperf_ustack_timer, "test that user backtraces on a timer are correct",
       T_META_ASROOT(true)) {
  static unsigned int stacks_seen = 0;
  ktrace_session_t s;
  uint64_t tid;
...
__attribute__((noinline, not_tail_called)) static void
...
T_DECL(kperf_ustack_trunc, "ensure stacks are marked as truncated") {
  ...
  ktrace_events_single(s, PERF_STK_UHDR, ^(struct trace_point *tp) {
    ...
    T_EXPECT_BITS_SET(tp->arg1, CALLSTACK_VALID, "found valid callstack");
...
T_DECL(kperf_ustack_maxlen, "ensure stacks up to 256 frames can be captured") {
  ...
  ktrace_events_single(s, PERF_STK_UHDR, ^(struct trace_point *tp) {
    ...
    T_LOG("found %llu frame stack", tp->arg2);
    T_EXPECT_BITS_SET(tp->arg1, CALLSTACK_VALID, "found valid callstack");
    T_EXPECT_EQ(tp->arg2, UINT64_C(256), "found the correct number of frames");
    saw_stack = true;
  });
