...
static const uint32_t frame_eventid =
    KDBG_EVENTID(DBG_BSD, DBG_BSD_KDEBUG_TEST, 1);

static ktrace_session_t future_events_session(void) {
  ktrace_session_t ktsess = ktrace_session_create();
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(ktsess, "failed to create session");
...
T_DECL(iop_events_disable,
  ...
  ktrace_events_single(ktsess, KDBG_EVENTID(DBG_BSD, DBG_BSD_KDEBUG_TEST, 0xff),
                       ^(struct trace_point *tp __unused) {
...
static void expect_convert_between_abs_cont(bool abs_to_cont) {
