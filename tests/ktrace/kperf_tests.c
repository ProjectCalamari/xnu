...
static void *spinning_thread(void *semp) {
  T_QUIET;
...
static void start_tracing_with_timeout(ktrace_session_t s,
  ...
  ktrace_events_single(
      s, DISPATCH_AFTER_EVENT, ^(__unused struct trace_point *tp) {
        T_LOG("arming timer to stop tracing after %d seconds", timeout_secs);
        dispatch_after(
            dispatch_time(DISPATCH_TIME_NOW, timeout_secs * NSEC_PER_SEC), q, ^{
...
static void configure_kperf_timer_samplers(uint64_t period_ns,
                                           uint32_t samplers) {
  T_SETUPBEGIN;

  (void)kperf_action_count_set(1);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kperf_action_samplers_set(1, samplers), NULL);
  (void)kperf_timer_count_set(1);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      kperf_timer_period_set(0, kperf_ns_to_ticks(period_ns)), NULL);
  T_QUIET;
...
static double timestamp_secs(ktrace_session_t s, uint64_t timestamp) {
  uint64_t ns = 0;
  T_QUIET;
  T_ASSERT_POSIX_ZERO(
      ktrace_convert_timestamp_to_nanoseconds(s, timestamp, &ns), NULL);
...
T_DECL(kperf_sample_active_cpus, "make sure that kperf samples all active CPUs",
  ...
  int ncpus = dt_ncpu();
  T_QUIET;
  T_ASSERT_LT(ncpus, MAX_CPUS, "only supports up to %d CPUs", MAX_CPUS);
  T_LOG("found %d CPUs", ncpus);

  int nthreads = ncpus - 1;
  T_QUIET;
  T_ASSERT_LT(nthreads, MAX_THREADS, "only supports up to %d threads",
              MAX_THREADS);
  ...
  ktrace_set_completion_handler(s, ^{
    ...
    T_LOG("over %.1f seconds, saw %" PRIu64 " timer fires, %" PRIu64
          ...
          timestamp_secs(s, last_timestamp - first_timestamp), nfires, nsamples,
          (double)nsamples / (double)nfires, saturation);
    T_ASSERT_GT(saturation, 95.0, "saw reasonable percentage of full samples");
  ...
  void (^switch_cb)(struct trace_point *, const char *name) =
      ^(struct trace_point *tp, const char *name) {
        ...
        if (stopafter) {
          T_LOG("%.7g: %s on %d: %llx", timestamp_secs(s, tp->timestamp), name,
                tp->cpuid, tp->arg2);
        }
      };
  ...
  ktrace_events_single(s, PERF_TMR_FIRE, ^(struct trace_point *tp) {
    T_QUIET;
    T_ASSERT_EQ((tp->debugid & DBG_FUNC_START), 0,
    ...
    if (last_expected == 0) {
      ...
      sum_saturation += (double)(nsamples - last_nsamples) / last_expected;
    }
  ...
  for (int i = 0; i < nthreads; i++) {
    T_QUIET;
    T_ASSERT_POSIX_ZERO(
        pthread_create(&threads[i], NULL, &spinning_thread, &thread_spinning),
        NULL);
...
T_DECL(kperf_timer_fires_enough_times,
  ...
  ktrace_events_single(s, PERF_TMR_FIRE, ^(struct trace_point *tp) {
    nfires++;
    if (first_fire_ns == 0) {
      ktrace_convert_timestamp_to_nanoseconds(s, tp->timestamp, &first_fire_ns);
    }
    ktrace_convert_timestamp_to_nanoseconds(s, tp->timestamp, &last_fire_ns);
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(
      pthread_create(&thread, NULL, &spinning_thread, &thread_spinning), NULL);
  dispatch_semaphore_wait(thread_spinning, DISPATCH_TIME_FOREVER);
  ...
  ktrace_set_completion_handler(s, ^{
    ...
    double duration_secs =
        (double)(last_fire_ns - first_fire_ns) / NSEC_PER_SEC;
...
#define CIRQ_INIT(TYPE, NSLOTS)                                                \
  (struct cirq) { .nslots = NSLOTS, .tail_slot = 0, .slot_size = sizeof(TYPE), }

static inline void *cirq_get(struct cirq *cq, unsigned int i) {
  return (char *)cq + sizeof(*cq) + (cq->slot_size * i);
}

static void *cirq_top(void *vcq) {
  struct cirq *cq = vcq;
...
static void *cirq_push(void *vcq) {
  struct cirq *cq = vcq;
  unsigned int tail_slot = cq->tail_slot;
...
static void cirq_for(void *vcq, void (^iter)(void *elt)) {
  struct cirq *cq = vcq;
...
static void cpu_oversample_log(struct cpu_oversample *cpu, unsigned int cpuid) {
  T_LOG("CPU %d timer latencies:", cpuid);
  __block int i = -HISTORY_LEN;
  cirq_for(&cpu->timer_latencies, ^(void *viv) {
    struct instval *iv = viv;
    T_LOG("\t%llu timer latency %d: %llu", iv->iv_instant_ns, i, iv->iv_val);
    i++;
  });
...
T_DECL(kperf_timer_not_oversampling,
  ...
  int ncpus = dt_ncpu();
  T_QUIET;
  T_ASSERT_GT(ncpus, 0, "should see positive number of CPUs");

  struct cpu_oversample *per_cpu =
      calloc((unsigned int)ncpus, sizeof(per_cpu[0]));
  T_QUIET;
  T_WITH_ERRNO;
  ...
  ktrace_events_single(
      s, KDBG_EVENTID(DBG_BSD, DBG_BSD_EXCP_SC, SYS_stack_snapshot_with_config),
      ^(struct trace_point *tp) {
        bool start = tp->debugid & DBG_FUNC_START;
        uint64_t cur_ns = relns_from_abs(s, tp->timestamp);
        T_LOG("%llu: %s stackshot syscall from process %s", cur_ns,
              start ? "start" : "finish", tp->command);
        in_stackshot = start;
        if (!start) {
  ...
  ktrace_events_single(s, DECR_TRAP_LATENCY, ^(struct trace_point *tp) {
    uint64_t cur_ns = relns_from_abs(s, tp->timestamp);
  ...
  ktrace_events_single(s, PERF_TMR_FIRE, ^(struct trace_point *tp) {
    ...
    *fire_ns = cur_ns;
    ...
    struct instval *last_timer_latency = cirq_top(&cur_cpu->timer_latencies);
    ...
    if (first_fire_ns == 0) {
      ...
      uint64_t fire_period_adj_ns =
          fire_period_ns + last_fire_latency_ns + timer_latency_ns;
      // Within 5% is still a valid period -- adjust this for running in
      // potentially-noisy automation.
      uint64_t fire_period_pct_adj_ns =
          fire_period_adj_ns + FIRE_PERIOD_THRESHOLD_NS / 20;
      ...
      if (too_short) {
        T_LOG("%llu: period of timer fire %llu is %llu + %llu + %llu = "
              ...
              timer_latency_ns, fire_period_adj_ns, FIRE_PERIOD_THRESHOLD_NS);
        ...
        if (cur_cpu == last_cpu) {
          ...
          T_LOG("previous profile timer fired on CPU %d", last_fire_cpuid);
        ...
        if (nfires < (unsigned int)ncpus) {
          T_LOG("ignoring timer fire %llu as context may be missing", nfires);
        } else {
          if (in_stackshot) {
            T_LOG("skipping assertion because stackshot is "
                  "happening");
          } else if (last_stackshot_ns != 0 && cur_ns > last_stackshot_ns &&
  ...
  ktrace_set_completion_handler(s, ^{
    ...
    T_PASS("saw %" PRIu64 " timer fires (%g fires/second) without "
           "oversampling",
           nfires, (double)nfires / (double)duration_secs);
...
static void expect_kdebug_trigger(const char *filter_desc,
  ...
  ktrace_events_single(s, PERF_STK_KHDR, ^(struct trace_point *tp) {
    ...
    T_LOG("saw kernel stack with %" PRIu64 " frames, flags = %#" PRIx64,
          tp->arg2, tp->arg1);
  });
  ktrace_events_single(s, PERF_STK_UHDR, ^(struct trace_point *tp) {
    missing_user_stacks--;
    T_LOG("saw user stack with %" PRIu64 " frames, flags = %#" PRIx64, tp->arg2,
          tp->arg1);
  });
  ...
  (void)kperf_action_count_set(1);
  T_ASSERT_POSIX_SUCCESS(
      kperf_action_samplers_set(1, KPERF_SAMPLER_KSTACK | KPERF_SAMPLER_USTACK),
      NULL);
...
T_DECL(kperf_kdebug_trigger_classes,
       "test that kdebug trigger samples on classes", T_META_TAG_VM_PREFERRED) {
  start_controlling_ktrace();
...
T_DECL(kperf_kdebug_trigger_debugids,
  ...
  const uint32_t debugids[] = {TRIGGER_DEBUGID};
...
static void reset_kperf(void) {
  (void)kperf_reset();
  (void)sysctlbyname("kperf.debug_level", NULL, NULL, &(int){0}, sizeof(int));
}
...
static void expect_stacks_traced(void (^setup)(ktrace_session_t s),
                                 void (^complete)(void)) {
  ktrace_session_t s;
...
T_DECL(kperf_lightweight_pet,
  ...
  configure_kperf_stacks_timer(-1, 10, false);
  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname("kperf.lightweight_pet", NULL, NULL, &set, sizeof(set)),
      NULL);
  T_ASSERT_POSIX_SUCCESS(kperf_timer_pet_set(0), NULL);

  expect_stacks_traced(
      ^(ktrace_session_t s){
      },
      ^(void) {
        T_END;
...
T_DECL(kperf_pmc_config_only,
  ...
  ktrace_events_single(s, PERF_KPC_CONFIG, ^(__unused struct trace_point *tp) {
    saw_kpc_config = true;
  });
  ktrace_events_single(s, PERF_KPC_REG, ^(__unused struct trace_point *tp) {
    saw_kpc_reg = true;
  });
  ktrace_events_single(s, PERF_KPC_REG32, ^(__unused struct trace_point *tp) {
    saw_kpc_reg = true;
  });
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      kperf_timer_period_set(0, kperf_ns_to_ticks(TIMER_PERIOD_NS)), NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kperf_timer_action_set(0, 1), NULL);
...
static void skip_if_monotonic_unsupported(void) {
  int r;
  int supported = 0;
...
T_DECL(kperf_sample_instrs_cycles, "ensure instructions and cycles are sampled",
       T_META_TAG_VM_NOT_ELIGIBLE) {
  T_SETUPBEGIN;
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(sess, "ktrace_session_create");
  ...
  ktrace_set_completion_handler(sess, ^{
    ...
    T_EXPECT_EQ(nzeroes, UINT64_C(0), "saw no events with 0 instructions");
    T_END;
  });
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      kperf_action_samplers_set(1, KPERF_SAMPLER_TH_INSTRS_CYCLES), NULL);
  ...
  T_ASSERT_POSIX_ZERO(ktrace_start(sess, dispatch_get_main_queue()), NULL);
...
#define LABEL                                                                  \
  "this_is_a_test_label_that_is_very_long_and_will_be_truncated"               \
...
T_DECL(kperf_dispatch_labels, "ensure dispatch queue labels are sampled",
  ...
  ktrace_events_single(sess, PERF_DISPSAMPLE, ^(struct trace_point *tp) {
    if (tp->debugid & DBG_FUNC_END) {
      T_LOG("ending sample event returned %d (%s)", (int)tp->arg1,
            strerror((int)tp->arg1));
    }
  });
  ...
  ktrace_set_completion_handler(sess, ^{
    T_EXPECT_GT(nlabels, 0ULL, "saw %" PRIu64 " dispatch queue label events",
                nlabels);
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      kperf_action_samplers_set(1, KPERF_SAMPLER_TH_DISPATCH), NULL);
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kperf_kdebug_filter_set(kdfilt),
                         "set filter for kperf");
  T_ASSERT_POSIX_SUCCESS(kperf_sample_set(1), "start kperf sampling");
...
static pid_t spawn_bless_helper(void) {
  char test_path[MAXPATHLEN] = {0};
  int ret = proc_pidpath(getpid(), test_path, MAXPATHLEN);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "query path for current process");
  ...
  T_QUIET;
  T_ASSERT_GT(child_pid, 0, "child pid is valid");
  return child_pid;
}

static int test_child_process(pid_t child_pid) {
