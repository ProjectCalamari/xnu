...
#define PORTS_COUNT 0x1000
  ...
  for (size_t i = 0; i < PORTS_COUNT; i++) {
    ...
    request->header = (mach_msg_header_t){
        ...
        .msgh_bits = MACH_MSGH_BITS_SET(MACH_MSG_TYPE_MAKE_SEND, 0, 0,
    ...
    T_QUIET;
    T_ASSERT_EQ(kr, MACH_MSG_SUCCESS, "mach_msg");
    ...
    T_QUIET;
    T_ASSERT_EQ(nevents, (int)1, "kevent_qos");
    T_QUIET;
    T_ASSERT_EQ(out_events[0].udata, (uint64_t)0x42424242, "kevent_qos");
    T_QUIET;
    T_ASSERT_BITS_SET(reply->header.msgh_bits, MACH_MSGH_BITS_COMPLEX,
                      "message is complex");
    T_QUIET;
    T_ASSERT_EQ(reply->body.msgh_descriptor_count, (mach_msg_size_t)2,
                "mach_msg");
  ...
  for (size_t i = 0; i < PORTS_COUNT; i++) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "kevent_id");
  }
  return NULL;
}

T_DECL(test_knote_use_after_free,
       "Verify knote use-after-free issue does not reproduce - rdar://96567281 "
       "(Knote Use-after-Free in XNU)",
  ...
  pthread_t thknote;
  T_ASSERT_POSIX_ZERO(
      pthread_create(&thknote, NULL, th_allocate_knotes, &context),
      "pthread_create");
...
#define KNOTE_PORT_COUNT 2
  ...
  for (size_t i = 0; i < KNOTE_PORT_COUNT; i++) {
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "mach_port_allocate knote_port");
  }
...
#pragma pack(0)
  ...
  kr = mach_msg(&local->header, MACH_SEND_MSG, sizeof(sync_knote_msg_local_t),
  ...
  int nevents = kevent_qos(kq, &event, 1, out_events, 1, NULL, NULL, 0);
  T_QUIET;
  T_ASSERT_EQ(nevents, (int)1, "kevent_qos nevents");
  T_QUIET;
  T_ASSERT_EQ(remote->sequence, (uint64_t)0x6666666666666666,
              "kevent_qos remote->sequence");
  ...
  T_QUIET;
  T_ASSERT_EQ(ret, (int)0, "kevent_qos return");
  ...
  kr = _kernelrpc_mach_port_insert_right_trap(task, sync_port, sync_port,
                                              MACH_MSG_TYPE_MOVE_RECEIVE);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "_kernelrpc_mach_port_insert_right_trap");
}
