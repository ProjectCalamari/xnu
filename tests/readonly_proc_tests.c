...
T_GLOBAL_META(T_META_NAMESPACE("xnu.bsd"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("bsd"), T_META_OWNER("chrisjd"));
...
T_DECL(readonly_proc_tests, "invoke the read-only proc unit test",
  ...
  int ret;
  ret = sysctlbyname("debug.test.readonly_proc_test", &result, &s, &value,
                     sizeof(value));
